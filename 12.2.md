...
FreeBSD 12.2-RELEASE 是个 RELEASE 版本。可以在 [https://www.FreeBSD.org/releases/](https://www.freebsd.org/releases/) 或其镜像网站上找到。获取此版本（或其他版本）的更多信息，请参见 [FreeBSD 手册](https://docs.freebsd.org/en/books/handbook/mirrors) 中的 [获取 FreeBSD 附录](https://docs.freebsd.org/en/books/handbook/mirrors)。

建议所有用户在安装 FreeBSD 之前查阅发布勘误。该勘误会在发布周期结束时或发布后更新，提供“最新”信息。通常，它包含已知的错误、安全公告和文档修正。最新的 FreeBSD 12.2-RELEASE 勘误副本可以在 FreeBSD 网站上找到。

本文档介绍了自 12.1-RELEASE 以来 FreeBSD 中最显著的新功能或变更。通常，文中描述的变更是 12-STABLE 分支特有的，除非特别标注为合并特性。

典型的发行说明条目记录了自 12.1-RELEASE 以来发布的安全公告、新驱动程序或硬件支持、新命令或选项、主要 bug 修复或第三方软件升级。它们还可能列出对主要 Port/包或发布工程实践的变更。显然，发行说明无法列出所有版本之间对 FreeBSD 所做的每一个变更；本文档主要聚焦于安全公告、用户可见的变更和重要的架构改进。

## 从旧版 FreeBSD 升级

[freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 工具支持在 RELEASE 版本之间进行二进制升级（以及各安全分支的快照）。二进制升级过程将更新未修改的用户空间工具和作为官方 FreeBSD 发行版一部分分发的未修改的 **GENERIC** 内核。[freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 工具要求升级的主机具有互联网连接。

基于源代码的升级（即从源代码重新编译 FreeBSD 基本系统）也受支持，根据 **/usr/src/UPDATING** 中的说明进行。
...
| 错误通告 | 日期 | 主题 |
| :------- | :--- | :--- |
| [FreeBSD-EN-19:19.loader](https://www.freebsd.org/security/advisories/FreeBSD-EN-19:19.loader.asc) | 2019 年 10 月 27 日 | UEFI 加载器内存碎片化 |
...
已更新驱动程序 [ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 到版本 1.11.29。[[r363876]](http://svn.freebsd.org/viewvc/base?view=revision&revision=363876)（由 Intel Corporation 赞助）
