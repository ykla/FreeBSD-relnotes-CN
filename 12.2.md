...
典型的发行说明条目记录了自 12.1-RELEASE 以来发布的安全公告、新驱动程序或硬件支持、新命令或选项、主要 bug 修复或第三方软件升级。它们还可能列出对主要 Ports/包或发布工程实践的变更。显然，发行说明无法列出各个版本之间对 FreeBSD 所做的每一个变更；本文档主要聚焦于安全公告、用户可见的变更和重要的架构改进。
...
| 公告 | 日期 | 主题 |
| :--- | :--- | :--- |
| [FreeBSD-SA-19:25.mcepce](https://www.freebsd.org/security/advisories/FreeBSD-SA-19:25.mcepce.asc) | 2019 年 11 月 12 日 | 页面大小变更时的机器检查异常 |
...
[cron(8)](https://man.freebsd.org/cgi/man.cgi?query=cron&sektion=8&format=html) 工具已更新，支持在 [crontab(5)](https://man.freebsd.org/cgi/man.cgi?query=crontab&sektion=5&format=html) 中使用两个新参数，`-n` 和 `-q`，分别用于抑制成功运行时的邮件通知和抑制命令执行的日志记录。[[r353134]](http://svn.freebsd.org/viewvc/base?view=revision&revision=353134)
...
[fsck_msdosfs(8)](https://man.freebsd.org/cgi/man.cgi?query=fsck_msdosfs&sektion=8&format=html) 工具已更新，包含多项增强功能，包括减少内存占用、新增参数 `-M`（该参数禁用 [mmap(2)](https://man.freebsd.org/cgi/man.cgi?query=mmap&sektion=2&format=html) 的使用）等。[[r357568]](http://svn.freebsd.org/viewvc/base?view=revision&revision=357568)
