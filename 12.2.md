...
典型的发布说明条目记录了自 12.1-RELEASE 以来发布的安全通告、新驱动程序或硬件支持、新命令或选项、主要 bug 修复或贡献的软件升级。它们还可能列出对主要 Port/包或发布工程实践的变更。显然，发布说明无法列出所有版本之间对 FreeBSD 所做的每一个变更；本文档主要聚焦于安全通告、用户可见的变更和重要的架构改进。
...
本节介绍了对用户空间应用程序、贡献软件和系统实用程序的变更和新增内容。
