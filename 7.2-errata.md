...
[20090507] 在 [fxp(4)](http://www.freebsd.org/cgi/man.cgi?query=fxp&sektion=4&manpath=FreeBSD+7.2-stable) 驱动程序中发现错误。当启用 TSO 且接收端的 MSS 小于接口 MTU 设置时，会导致 TCP 性能下降。作为变通方法，可使用 [ifconfig(8)](http://www.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8&manpath=FreeBSD+7.2-stable) 的 `-tso` 参数（按接口设置）或将 `net.inet.tcp.tso` 设置为 `0`（全局设置）来禁用 TSO。计划在发行后发布勘误通知修复此问题。
