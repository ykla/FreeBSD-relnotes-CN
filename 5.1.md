...
本节描述了自 5.0-RELEASE 以来，FreeBSD 中许多用户可见的新功能或更改。它包括特有的 5-CURRENT 分支中的项目，以及一些可能最近合并到其他分支（在 FreeBSD 5.0-RELEASE 之后）的功能。这些项目标记为[已合并]。

典型的发行说明项目记录了在 5.0-RELEASE 之后发布的安全公告、新的驱动程序或硬件支持、新的命令或选项、重要的错误修复或第三方的软件升级。它们还可能列出主要 Port/包或发布工程实践的变化。显然，发行说明不能列出每个在版本之间对 FreeBSD 所做的更改；本文件主要关注安全公告、用户可见的更改以及主要架构改进。
...
[pw(8)](http://www.freebsd.org/cgi/man.cgi?query=pw&sektion=8&manpath=FreeBSD+5.1-release) 现在可以新增以 `$` 字符结尾的用户；此更改旨在帮助管理 **Samba** 服务。[已合并]
