...
典型的发行说明条目记录了在 5.0-RELEASE 之后发布的安全公告、新的驱动程序或硬件支持、新的命令或选项、重要的错误修复或第三方的软件升级。它们还可能列出主要 Ports/包或发布工程实践的变化。显然，发行说明不能列出 FreeBSD 在版本之间的每一项变更；本文件主要关注安全公告、用户可见的变更以及主要架构改进。
