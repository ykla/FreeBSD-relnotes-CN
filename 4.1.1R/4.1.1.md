...
如需了解 FreeBSD 及 4.1.1-RELEASE 目录结构的信息（尤其是通过软盘安装时！），请参阅 **ABOUT.TXT** 文件。
...
- **syslogd(8)** 可使用 `-n` 选项，用于禁用每次请求时的 DNS 查询。
...
AMI FastDisk 控制器中真正克隆 BusLogic MultiMaster 的版本亦受支持。
