...
还升级或新增了许多软件包，例如 XFree86 3.1、xview 3.2、elm、nntp、mh、InterViews 以及几十个其他工具，这些工具已移植并可作为附加组件使用。有关完整的摘要，请查看 Ports（或 package 集合）。
...
FreeBSD 的核心不包含会限制出口到美国以外的 DES 代码。对于仅在美国境内使用的用户，我们提供了一个核心发行版的附加包，其中包含通常使用 DES 的程序。单独提供的辅助软件包任何人都可以使用。此外，还有一个为非美国用户准备的、可（从美国境外）自由出口的欧洲 DES 发行版，可以在 FreeBSD FAQ 中找到其详细信息。
...
- **支持许多新的文件系统类型**
  新增了许多文件系统类型，例如堆叠文件系统（stackable filesystems）、联合文件系统（union filesystems）、“门户”（portals）、kernfs、简单的日志结构化文件系统、新的 NFS 版本（NQNFS）等。虽然其中某些文件系统尚不够完善，还需要额外的工作来实现更强的稳健性，但这些改进为未来的发展奠定了基础。
...
此版本中提供了一个实验性功能，可以直接执行 gzip 压缩的二进制文件。启用后，你只需 gzip 压缩二进制文件，移除 `.gz` 扩展名并将其设置为可执行文件。尽管启用此功能会显著降低速度并增加内存消耗，但对于笔记本用户来说可能是值得的，最多可节省约 10 MB 的磁盘空间。
...
如果你的机器可以通过互联网发送邮件，建议使用 `send-pr` 命令提交错误报告。你提交的错误报告会由我们忠实的错误归档程序认真归档，我们会尽力尽快回应所有报告的问题。
