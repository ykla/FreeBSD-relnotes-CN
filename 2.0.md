...
许多软件包也升级了或新增，例如 XFree86 3.1、xview 3.2、elm、nntp、mh、InterViews 以及几十个其他工具，这些工具已移植并可作为附加组件使用。有关完整的摘要，请查看 Ports 或 package。
...
FreeBSD 的核心不包含会限制出口到美国以外的 DES 代码。对于仅在美国境内使用的用户，我们提供了一个核心发行版的附加包，其中包含通常使用 DES 的程序。单独提供的辅助软件包任何人都可以使用。此外，还有一个为非美国用户准备的、可自由出口的欧洲 DES 发行版，可以在 FreeBSD FAQ 中找到其详细信息。
...
要启用 S/Key 支持，可在 **/etc** 中创建一个 `skey.access` 文件，并根据需求进行填写。详见：
...
光盘售价为 $39.95，美国、加拿大和墨西哥运费 $5，其他国家 $10。支持 Visa、Mastercard、American Express，并提供美国货到付款服务。加州居民需加 8.25% 销售税。
...
如果你的机器可以通过互联网发送邮件，建议使用 `send-pr` 命令提交错误报告。我们的忠实错误归档程序会接收错误报告，我们会尽力尽快回应所有报告的问题。
