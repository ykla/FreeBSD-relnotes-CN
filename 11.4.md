...
典型的发行说明条目会记录在 11.3-RELEASE 之后发布的安全公告、新的驱动程序或硬件支持、新的命令或选项、重要的 bug 修复或第三方软件升级。它们还可能列出对主要 Port/软件包或发布工程实践的变更。显然，发行说明无法列出每一个在发布之间对 FreeBSD 所做的变更；本文档主要聚焦于安全公告、用户可见的变更和主要的架构改进。
...
> **重要**：
>
> 升级 FreeBSD 应仅在备份 _所有_ 数据和配置文件后进行。
...
增加了修复以正确链接支持 DTrace 的 Port 与 lld 的功能。[r361217](http://svn.freebsd.org/viewvc/base?view=revision&revision=361217)
