...
[bge(4)](http://www.freebsd.org/cgi/man.cgi?query=bge&sektion=4&manpath=FreeBSD+7.4-RELEASE) 驱动现在支持 BCM5705 或更新版本的 Broadcom 控制器的硬件 MAC 统计信息，这些统计信息可以通过 [sysctl(8)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&manpath=FreeBSD+7.4-RELEASE) 变量 `dev.bge.<tt class="REPLACEABLE">N</tt>.stats.*` 访问，为诊断驱动问题提供有用信息。
