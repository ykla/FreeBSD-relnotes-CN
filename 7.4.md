...
典型的发行说明条目记录了自 7.3-RELEASE 之后发布的安全公告、新的驱动程序或硬件支持、新的命令或选项、重要的错误修复或第三方软件升级。它们也可能列出重大 Port/包或发布工程实践的更改。显然，发行说明无法列出发布之间对 FreeBSD 所做的每一个更改；本文件主要聚焦于安全公告、用户可见的变化和重大架构改进。

### 2.1 安全公告

以下安全公告中描述的问题已被修复。更多信息，请参考来自 [http://security.FreeBSD.org/](http://security.freebsd.org/) 的个别公告。
...
FreeBSD 的 TCP 重新组装实现得到了改进。已修复影响 SMP 系统的长期存在的会计错误，`net.inet.tcp.reass.maxqlen` [sysctl(8)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&manpath=FreeBSD+7.4-RELEASE) 变量已被弃用，取而代之的是基于接收套接字缓冲区大小的每连接动态限制。FreeBSD 接收端现在比以前更好地处理丢包（特别是由队列溢出引起的丢包），从而提高了连接吞吐量。
