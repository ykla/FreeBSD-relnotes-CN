...
- 使用新的 `if_multiaddrs` 列表来处理多播地址，而不是以前涉及 `struct in_ifaddr` 和 `arpcom` 的糟糕方法，摆脱了可憎的 `multi_kludge`。
- 引入了用于以太网驱动程序的新的 `if_media` 选择方法，来自 Jason Thorpe 为 NetBSD 实现的版本。[已合并：也在 2.2.5 及以后版本的 2.2-STABLE 分支中]
- 多会话 ISO-9660 CD-ROM 现在完全支持。默认情况下，将挂载最后一个会话（包括根挂载）。对于非根挂载，`mount_cd9660(8)` 可以接受一个参数来挂载特定的会话，而不是默认的会话。
...
- 实现了标志 `SA_NOCLDWAIT`，提供了 System V 选项，允许进程表示希望永远不会对于已死亡的子进程接收到僵尸进程或 SIGCHLD 信号。
...
- 异步 IO 现已实现（目前在非 SMP 环境下），并增加了对内核辅助线程的支持。
...
- 所有支持的网络协议已更新，以避免“大型 switch 语句”`pr_usrreq()`，并将进程指针传递给每个可能需要进程凭证或想要休眠的用户请求，取代了以前偶尔正确执行的 `curproc` 检查和 `SS_PRIV` 套接字状态标志。后者现已被消除，连同用于清除它的 `SO_PRIVSTATE` 套接字选项也一并移除。现在，协议还可以覆盖通用的发送、接收和轮询例程，这将使得在后续版本中能够实现更高效的协议特定的实现。最后，网络代码的许多部分已被修改，不再在 mbuf 中存储套接字地址和其他元信息，为最终消除它们做准备。现在返回套接字地址的机制仍然高度可能会变化，我们正在进行实验以发现最有效的方法。
...
- Vnode 空闲列表处理已被改进。仅当没有人关心它们时，vnode 才会出现在空闲列表中。
- 内核在可能的情况下，通过名称缓存中存储的数据为 `getcwd()` 提供支持。
- 实现了基于中断的配置挂钩机制。这能使驱动程序推迟部分配置，直到中断完全启用后再进行。这加速了启动过程，因为避免了对子设备探测（例如 SCSI 总线探测）等操作的忙等待。
- 内核中的 `timeout(9)` 系统已被彻底改进。这为 callout 的插入和删除提供了 O(1) 的时间复杂度，而软时钟中处理的工作量为 O(哈希链长度)。可以在以下链接找到原始论文：[http://www.cs.berkeley.edu/~amc/research/timer/](http://www.cs.berkeley.edu/~amc/research/timer/)
- 驱动程序缓冲队列的变化以处理有序事务。这旨在为文件系统代码中的数据和元数据写入排序提供支持（一旦完全实现后即启用）。
- 现在支持 EISA 共享中断，使用了最初用于支持 PCI 共享中断的框架。
- 支持 Comtrol Rocketport 卡。
- IPFW 的数据包和字节计数器已从 32 位扩展到 64 位，在 `ipfw` 中新增了 `FWD` 操作以支持透明代理，且 divert 操作略有变化——更多信息请参阅 `natd(8)` 和 `ipfw(8)` 的手册页。
...
98/1/20 现在已包含对 LAND 攻击的更强大的保护。

CERT 公告 CA-97.17 中提到的 suidperl 漏洞也被认为已修复。

KerberosIV 完成合并。

### 1.3. 用户空间变化

- **默认二进制类型**（及编译工具链）已从 `a.out` 切换为 `ELF`。这为我们带来了许多新型编译器技术（其中大部分不支持 `a.out`），使可执行文件体积更小，并为 C++ 等语言提供了更好的支持，等诸多其他优势。旧的 `a.out` 库和二进制文件当然仍然可以继续使用，并为过渡期提供了同时安装两种类型的选项。

- **Perl5** 已替代 **Perl4**，成为系统的标准组件。

- **默认用户名长度** 增加到 16 个字符。注意：旧的 utmp/wtmp 文件不支持此变更，因为数据记录仍然是旧大小。有关转换工具，请参阅 **/usr/src/tools/3.0-upgrade**。

- 更紧凑的 **/etc/rc.conf** 文件已取代 **/etc/sysconfig**。[已合并：也适用于 2.2.1 及更高版本的 2.2-STABLE 分支]

- `fdisk(8)` 现在将磁盘切片从 1 到 4 编号，而不是从 0 到 3。这使其与 **/dev** 中设备名称使用的编号保持一致。

- **Amd 自动挂载程序** 已从 1993 年的 4.4BSD 版本更新为最新的 am-utils 版本。映射选项发生了一些变化，并支持新配置文件 **/etc/amd.conf**。详见“man 5 amd.conf”。
...
- 新的 **VGA 库**（**/usr/src/lib/libvgl**）现已存在，用于在 syscons tty 上进行简单的 VGA 图形操作（类似于 Linux 的 libSVGA）。[已合并：也适用于 2.2.5 及更高版本的 2.2-STABLE 分支]

- **Xntpd** 的系统日志记录功能已移至独立的日志设施（`LOG_NTP`，用户空间名称为“ntp”）。
...
- **/bin/sh** 的信号和 trap 处理已重写。除其他事项外，这使得 tty 模式下的 emacs 在由 `system(2)` 调用（例如通过邮件代理）时可以正常工作，并修复了一些语法错误。[已合并：也适用于 2.2.8 及更高版本的 2.2-STABLE 分支]

- `systat(1)`、`iostat(8)`、`rpc.rstatd(8)` 和 `vmstat(8)` 已重构为使用新的 `devstat(3)` 库和 `devstat(9)` 统计子系统。这些工具（除了 `rpc.rstatd(8)`）现在打印出更有用的统计数据，并能够查看系统中所有设备的统计数据，而不仅限于前 8 个设备。
...
镜像站点信息，请参阅文件 `MIRROR.SITES`。请从网络条件上距离你最近的镜像站点下载发行版。欢迎更多的镜像站点加入！如需成为官方镜像站点，请联系 `freebsd-admin@FreeBSD.org`。
