...
这是我们第一次发布 3.0-CURRENT 版本，主要面向早期采用者和开发者。一些文档内容可能尚未更新，如果遇到此类问题，请报告。当然，任何安装失败或崩溃也应尽快报告，可以通过发送邮件到 `freebsd-bugs@FreeBSD.org` 或使用命令 `send-pr` 来报告（喜欢基于网页的界面的人也可以查看此页面）。

有关 FreeBSD 及 3.0-RELEASE 目录布局的信息（特别是如果你是从软盘安装！），请参阅 `ABOUT.TXT`。有关安装说明，请参见 `INSTALL.TXT` 和 `HARDWARE.TXT` 文件。

这也远不是 3.0-current（HEAD）分支上的最后一个发布版本，日常快照发布将在此发布后照常继续进行。请从以下地址安装：**ftp://current.FreeBSD.org/pub/FreeBSD**
...
- Network Computing Services, Inc. 的 Host ATM Research Platform（“HARP”）软件集成到系统中。更多信息请参见 **/usr/src/share/examples/atm**。
- SMP（对称多处理）分支合并完成。虽然内核目前大部分还不支持重入，但相关工作正在进行。
- 4.4BSD-Lite2 的代码（最终）合并完成。
...
- ed0（wd8xxxx, 3c503, NE2000, HP Lan+）以太网设备的默认 IRQ 从 IRQ 5 变更为 IRQ 10。
  ed1 以太网设备移除。请使用 Userconfig 工具将 ed0 的值变更为与你的网卡设置匹配。[已合并：这两个变更也出现在 2.2.6 及以后版本的 2.2-STABLE 分支中]
- 负责维护日期时间的代码重写完成。新增功能包括：内核和用户空间均支持纳秒级时间，NTPD 进行连续调整而非逐步调整，支持与高精度外部时间信号的同步。
- 向打印机驱动程序新增了对 PPS API（见 draft-mogul-pps-api-02.txt）的支持，用于通过并行打印机端口的 TTL 上升沿输入。
- 使用新的 `if_multiaddrs` 列表来处理多播地址，而不是以前涉及 `struct in_ifaddr` 和 `arpcom` 的糟糕方法，移除了可憎的 `multi_kludge`。
- 引入了用于以太网驱动程序的新的 `if_media` 选择方法，来自 Jason Thorpe 为 NetBSD 实现的版本。[已合并：也在 2.2.5 及以后版本的 2.2-STABLE 分支中]
- 多会话 ISO-9660 CD-ROM 现已获得完全支持。默认情况下，将挂载最后一个会话（包括根挂载）。对于非根挂载，**mount_cd9660(8)** 可以接受一个参数来挂载特定的会话，而不是默认的会话。
...
- `vfork()` 现在通过利用新的共享语义而完全可用，并且测得了显著的速度提升。若出现问题，可以通过 `sysctl` 变量 `kern.fast_vfork` 禁用它。由于 4.4BSD（net2、Lite 和 Lite2）中的 `popen()` 实现存在 bug，因此来自旧版本和其他 BSD 平台的静态链接二进制文件会出现问题。`rfork()` 也可以访问这些功能，旨在支持内核辅助线程。
- 在 Berkeley Software Design, Inc.、Jonathan Lemmon、Mike Smith、Sean Eric Fagan 和 John Dyson 的贡献下，内核新增对 VM86 的支持，并且 BSD/OS 贡献的 doscmd 移植完成。
- 实现了标志 `SA_NOCLDWAIT`，提供了 System V 选项，允许进程表示希望永远不接收已死亡子进程的僵尸进程或 SIGCHLD 信号。
- 实现了 **poll(2)**，其核心源自 NetBSD 实现。`select()` 和 `poll()` 系统调用都使用了 poll 设备、文件和 vnode 操作例程。
- 实现了类似于 OpenBSD 的 **issetugid(2)** 调用。我们在更多情况下设置该标志——我们的实现略微更为谨慎。
- 异步 IO 实现完成（目前在非 SMP 环境下），并增加了对内核辅助线程的支持。
- 其他一些用于与其他系统兼容的杂项系统调用：**getsid(2)**、**setpgid(2)**、**nanosleep(2)**。
- 新的系统调用 **signanosleep(2)** 类似于 **nanosleep(2)**，但使用特定的信号掩码来确定哪些信号会唤醒休眠。简而言之，这是“等待一组特定的信号，最多持续一定时间”。
- **sleep(3)** 和 **usleep(3)** 现在是基于 **signanosleep(2)** 实现的，并且现在具有正确的 SIGALRM 交互语义，**sleep(3)** 会正确返回剩余时间。
- 实现了内核链接器，旨在替代 `lkm` 系统及其伴随的糟糕设计。
- 所有支持的网络协议更新完成，以避免“大型 switch 语句”`pr_usrreq()`，并将进程指针传递给每个可能需要进程凭证或想要休眠的用户请求，取代了以前偶尔正确执行的 `curproc` 检查和 `SS_PRIV` 套接字状态标志。后者现已被消除，连同用于清除它的 `SO_PRIVSTATE` 套接字选项也一并移除。现在，协议还可以覆盖通用的发送、接收和轮询例程，这将使得在后续版本中能够实现更高效的协议特定的实现。最后，网络代码的许多部分修改完成，不再在 mbuf 中存储套接字地址和其他元信息，为最终消除它们做准备。现在返回套接字地址的机制仍然高度可能会变化，我们正在进行实验以发现最有效的方法。
- 现在可以通过 `sysctl` 禁用对多播 ICMP ECHO REQUEST（“ping”）和地址掩码请求包的响应。`netstat` 程序将打印出这些事件发生的次数。
- 修复了 `ffs` 中一个微妙且很少遇到的 bug。
- VFS 名称缓存重新设计完成，以提高可追踪性和效率。
- `VOP_LOOKUP()` 的通用部分移入系统范围的函数中，文件系统可以依赖它来处理标准内容。
- Vnode 空闲列表处理改进完成。仅当没有被引用时，vnode 才会出现在空闲列表中。
- 内核在可能的情况下，通过名称缓存中存储的数据为 `getcwd()` 提供支持。
- 实现了基于中断的配置挂钩机制。这能使驱动程序推迟部分配置，直到中断完全启用后再进行。这加速了启动过程，因为避免了对子设备探测（例如 SCSI 总线探测）等操作的忙等待。
- 内核中的 **timeout(9)** 系统彻底改进完成。这为 callout 的插入和删除提供了 O(1) 的时间复杂度，而软时钟中处理的工作量为 O(哈希链长度)。可以在以下链接找到原始论文：[http://www.cs.berkeley.edu/~amc/research/timer/](http://www.cs.berkeley.edu/~amc/research/timer/)
- 驱动程序缓冲队列的变更，用于处理有序事务。这旨在为文件系统代码中的数据和元数据写入排序提供支持（一旦完全实现后即启用）。
- 现在支持 EISA 共享中断，使用了最初用于支持 PCI 共享中断的框架。
- 支持 Comtrol Rocketport 卡。
- IPFW 的数据包和字节计数器从 32 位扩展到 64 位，在 `ipfw` 中新增了 `FWD` 操作以支持透明代理，且 divert 操作略有变化——更多信息请参阅 **natd(8)** 和 **ipfw(8)** 的手册页。
- 新的即插即用（PnP）支持，能使你（重新）配置 PnP 设备。同时，支持由 PnP 部分检测到的调制解调器并自动附加。[已合并：同样出现在 2.2.6 及之后版本的 2.2-STABLE 分支中]
...
- 增加了对 ATAPI 软盘驱动器（LS-120）的支持。[已合并：同样出现在 2.2.7 及之后版本的 2.2-STABLE 分支中]
...
97/8/31 games 的 setuid 改为 setgid。使得任何游戏的漏洞都无害（仅分数文件易受攻击）。请将任何问题报告给 **<eivind@FreeBSD.org>**（已知分数文件所有权问题）。

97/12/3 增加了 Intel 建议的 F00F bug 修复。如果你没有 Pentium 处理器，内核选项 **NO_F00F_HACK** 可禁用它。

98/1/20 现包含对 LAND 攻击的更强保护。

CERT 公告 CA-97.17 中提到的 suidperl 漏洞也被认为修复完成。

KerberosIV 完成合并。

### 1.3. 用户空间变化

- **默认二进制类型**（及编译工具链）从 `a.out` 切换为 `ELF`。这为我们带来了许多新型编译器技术（其中大部分不支持 `a.out`），使可执行文件体积更小，并为 C++ 等语言提供了更好的支持，等诸多其他优势。旧的 `a.out` 库和二进制文件当然仍然可以继续使用，并为过渡期提供了同时安装两种类型的选项。

- **Perl5** 替代了 **Perl4**，成为系统的标准组件。

- **默认用户名长度** 增加到 16 个字符。注意：旧的 utmp/wtmp 文件不支持此变更，因为数据记录仍然是旧大小。有关转换工具，请参阅 **/usr/src/tools/3.0-upgrade**。

- 更紧凑的 **/etc/rc.conf** 文件取代了 **/etc/sysconfig**。[已合并：也适用于 2.2.1 及更高版本的 2.2-STABLE 分支]

- **fdisk(8)** 现在将磁盘切片从 1 到 4 编号，而不是从 0 到 3。这使其与 **/dev** 中设备名称使用的编号保持一致。

- **Amd 自动挂载程序** 从 1993 年的 4.4BSD 版本更新为最新的 am-utils 版本。映射选项发生了一些变化，并支持新配置文件 **/etc/amd.conf**。详见“man 5 amd.conf”。

- **picobsd** 包（用于创建自定义 FreeBSD 引导软盘和“迷你系统”）引入到 **/usr/src/release/picobsd**。更多信息请参阅 **/usr/src/release/picobsd/README.html**。

- **finger(1)** 在网络操作时，不再在发送请求后立即关闭套接字，而是等待远程端先关闭。（规范对此未明确规定，我们选择与大多数服务器兼容的行为。）这意味着现在可以使用 MIT 目录，并 finger 某些有问题的 Linux 机器上的用户。

- **fetch(1)** 新增了一个标志，使其能够与某些处理不当的 HTTP 实现通信，这些实现会因请求消息后立即关闭连接而出错。

- **netstat(1)** 现在使用 **sysctl(3)** 检索更多统计数据组，并以正确的无符号格式打印大多数数据。

- 新的 **VGA 库**（**/usr/src/lib/libvgl**）现已提供，用于在 syscons tty 上进行简单的 VGA 图形操作（类似于 Linux 的 libSVGA）。[已合并：也适用于 2.2.5 及更高版本的 2.2-STABLE 分支]

- **Xntpd** 的系统日志记录功能移至独立的日志设施（`LOG_NTP`，用户空间名称为“ntp”）。

- 新增的 ioctl 对：**SIOC[SG]IFGENERIC**。其目的是为将任意 ioctl 子命令传递到网络接口驱动程序提供挂钩。例如，这对于 PPP 驱动程序在驱动程序与用户空间工具之间传递 CHAP 名称、密钥或可变选项是必要的。

- **sppp(4)** 自 FreeBSD 2.2.X 以来得到了显著改进。现在采用完整的 PPP 状态机，支持更多的 LCP 和 IPCP 协商，适合拨号按需连接（例如经常运行在 ISDN 上的连接）。还支持 PAP 或 CHAP 认证。用户空间的配套工具 **spppcontrol(8)** 也是第一个利用上述 SIOC[SG]IFGENERIC ioctl 命令的程序。

- **moused(8)** 修改后支持带有“滚轮”的各种鼠标。它还会自动识别支持 PnP COM 设备标准的鼠标，因此用户不再需要在命令行上指定鼠标协议类型。[已合并：也适用于 2.2.6 及更高版本的 2.2-STABLE 分支]

- **ppp(8)** 支持包括 **PPP 多链路协议**（RFC1990）、**PPP 回拨**（带 CBCP 扩展）和客户端侧 DNS 协商在内的多种新功能。可能的配置冲突详见源目录中的 `README.changes` 文件。

- POSIX 线程库 `libc_r` 新增了根据单一 UNIX 规范版本 2 定义的 **线程读/写锁**。

- 系统文件现在归用户 `root` 和组 `wheel` 所有。UID 0 远比 `bin` 更受保护，尤其是在 NFS 上。

- **/bin/sh** 的信号和 trap 处理重写完成。此外，这使得 tty 模式下的 emacs 在由 **system(2)** 调用（例如通过邮件代理）时可以正常工作，并修复了一些语法错误。[已合并：也适用于 2.2.8 及更高版本的 2.2-STABLE 分支]

- **systat(1)**、**iostat(8)**、**rpc.rstatd(8)** 和 **vmstat(8)** 重构为使用新的 **devstat(3)** 库和 **devstat(9)** 统计子系统。这些工具（除了 **rpc.rstatd(8)**）现在打印出更有用的统计数据，并能够查看系统中所有设备的统计数据，而不仅限于前 8 个设备。

## 2. 支持的配置

FreeBSD 目前可以运行在各种基于 ISA、VLB、EISA 和 PCI 总线的 PC 上，支持从 386sx 到奔腾级的机器（虽然不推荐使用 386sx）。系统提供对通用 IDE 或 ESDI 硬盘配置、各种 SCSI 控制器、网络和串行卡的支持。
...
**SCSI 外设支持：** 对所有支持的 SCSI 控制器提供完全支持，包括 SCSI-I 和 SCSI-II 外设，如硬盘、光盘、磁带机（包括 DAT 和 8mm Exabyte）、介质交换器、处理器目标设备和 CD-ROM 驱动器。

- 支持 CDROM 命令的 WORM 设备可由 CDROM 驱动程序进行只读访问
- WORM/CD-R/CD-RW 的写入支持由 `cdrecord` 提供，且该工具可以在 Ports 树中找到
...
- CAM SCSI 磁带驱动程序对旧的（且许多情况下有问题的）磁带机支持有限。例如，旧的 SCSI-1 磁带机（如 Exabyte 8200 或老旧的 QIC 型磁带机）可能无法在 CAM 磁带驱动程序下正常工作。这显然是一个已知问题，我们正在处理。
- 大多数基本兼容 SCSI-2 的新磁带机应该能正常运行，例如 DAT（DDS-1、2 和 3）、DLT 和较新的 Exabyte 8mm 磁带机。
- 如果想了解你特定的磁带驱动器是否受支持，最好的方法是试一下！
...
镜像站点信息，请参阅文件 `MIRROR.SITES`。请从网络距离最近的镜像站点下载发行版。欢迎更多的镜像站点加入！如需成为官方镜像站点，请联系 `freebsd-admin@FreeBSD.org`。

如果你无法访问互联网，而电子邮件是你唯一的办法，仍然可以通过发送邮件到 `ftpmail@ftpmail.vix.com` 来获取文件。在邮件中输入关键字“help”，即可获取有关如何通过该机制下载文件的更多信息。不过请注意，这种方式可能会通过邮件传输几十兆字节的数据，因此仅在 **绝对必要时** 使用此方法！
...
- 使用源代码（通过 **/usr/src**）
- 还可使用 `sysinstall`（二进制）升级选项

### 使用源代码

如果选择使用源代码升级，需要注意以下两种目标：
...
因此，建议希望生成动态内核组件的用户采用新的 **KLD（Kernel Loadable Driver）可加载内核模块机制**，而不是旧的 LKM 格式。LKM 格式即将被弃用并停止支持！
...
如果你的机器支持 Internet 邮件连接，提交错误报告的首选方法是使用命令 `send-pr`，或者通过以下链接访问 CGI 脚本提交报告：[http://www.FreeBSD.org/send-pr.html](http://www.FreeBSD.org/send-pr.html)。

我们可靠的错误归档程序会接收并归档通过此方式提交的错误报告。你可以放心，我们会尽力尽快响应所有报告的错误。此外，这些错误报告会在 FreeBSD 网站的支持部分公开显示，既有助于修复问题，也能作为“指引”提醒其他用户注意潜在问题。

如果由于某些原因无法使用命令 `send-pr` 提交错误报告，可以尝试发送邮件至：**<freebsd-bugs@FreeBSD.org>**

需要注意的是，`send-pr` 本身是个 shell 脚本，应该可以轻松迁移到其他系统中。我们更希望你通过此界面提交报告，因为这便于我们追踪问题报告。然而，在提交之前，请尽量确认该问题是否修复完成。

如果你有任何问题或需要技术支持，请发送邮件至：**<freebsd-questions@FreeBSD.org>**

此外，由于这是一个志愿者项目，我们始终欢迎更多愿意帮助的朋友——目前已有的功能增强需求远超我们自身的能力范围！如果你想讨论技术问题或提供帮助，请发送邮件至：**<freebsd-hackers@FreeBSD.org>**
...
- **Doug Rabson** 和 **John Birrell**，使 FreeBSD/alpha 得以实现，以及 NetBSD 项目提供了大量间接帮助。
- **Peter Wemm** 贡献了全新的内核模块系统（**Doug Rabson** 也给予了大力协助）。
- 全球成千上万的 FreeBSD 用户和测试者，没有他们，这一版本的发布根本不可能实现。
