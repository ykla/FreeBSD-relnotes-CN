...
新增了 [ipfw(4)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=4&manpath=FreeBSD+4.7-RELEASE) 的新版（通常称为“IPFW2”）作为选项。它在内核中采用了类似 [bpf(4)](http://www.freebsd.org/cgi/man.cgi?query=bpf&sektion=4&manpath=FreeBSD+4.7-RELEASE) 指令的可变大小规则表示法。尽管大部分外部可见的行为（如通过 [ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+4.7-RELEASE)）保持不变，[ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+4.7-RELEASE) 现在支持匹配字段之间的逻辑或连接操作。此新版本默认未启用。如需使用：
...
通过升级 **OpenSSL**，修复了其多个缓冲区溢出漏洞。更多信息请参阅安全公告 [FreeBSD-SA-02:33](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-02:33.openssl.asc)。
