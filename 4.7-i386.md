...
本节描述了自 4.6-RELEASE 以来 FreeBSD 中最为明显的新功能或已更改的功能。发行说明中的典型条目包括新驱动程序或硬件支持、新命令或选项、主要错误修复以及外部软件升级。此外，还列出了 4.6-RELEASE 发布后针对基本系统发布的安全公告。
...
[sh(1)](http://www.freebsd.org/cgi/man.cgi?query=sh&sektion=1&manpath=FreeBSD+4.7-RELEASE) 的内置 `cd` 命令现在支持 `-L` 和 `-P` 参数，分别用于调用逻辑模式或物理模式。默认使用逻辑模式，但可以通过 `sh(1)` 的物理选项更改默认设置。
