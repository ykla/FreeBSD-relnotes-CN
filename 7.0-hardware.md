...
```ini
$FreeBSD: src/release/doc/en_US.ISO8859-1/hardware/article.sgml,v 1.326.2.1.2.1 2008/02/06 19:47:28 delphij Exp $
```
...
Sun、Sun Microsystems、Java、Java Virtual Machine、JavaServer Pages、JDK、JRE、JSP、JVM、Netra、Solaris、StarOffice、Sun Blade、Sun Enterprise、Sun Fire、SunOS 和 Ultra 是 Sun Microsystems, Inc. 在美国及其他国家的商标或注册商标。

许多制造商和销售商用于区分其产品的名称被声明为商标。在本文档中出现这些名称时，如果 FreeBSD 项目已知该商标声明，则名称后会附有“™”或“®”符号。

## 1 引言

本文档包含 FreeBSD 7.0-RELEASE 的硬件兼容性说明。其中列出了 FreeBSD 支持的硬件平台，以及各类硬件设备（存储控制器、网络接口等），并给出已知可正常工作的设备实例。
...
* Intel 64-bit Xeon(TM) ("Nacona")。该处理器采用 90nm 工艺制造，工作频率为 2.80 至 3.60 GHz（FSB 800MHz），搭配 Intel E7520/E7525/E7320 芯片组。
* Intel Pentium(R) 4 Processor supporting Intel EM64T ("Prescott")。该处理器采用 90nm 工艺制造，使用 FC-LGA775 封装，工作频率为 3.20F/3.40F/3.60F GHz，搭配 Intel 925X Express 芯片组。对应的 S-Spec 编号为 SL7L9、SL7L8、SL7LA、SL7NZ、SL7PZ 和 SL7PX。注意，标号为 5xx 系列的处理器不支持 EM64T。
...
截至目前，测试过的最大内存配置为 8GB。SMP 支持近期完成且相当稳健。
...
所有拥有 1 个以上处理器的系统均支持 SMP。

通常，FreeBSD/sparc64 系统必须使用串行控制台。虽然可使用 OpenFirmware 控制台引导或安装系统，但该控制台设备无法使用。

如果你有不在此列表中的系统，它可能尚未经过 FreeBSD 7.0-RELEASE 测试。我们鼓励你尝试并致信 [FreeBSD SPARC porting mailing list](http://lists.FreeBSD.org/mailman/listinfo/freebsd-sparc64) 反馈结果，包括哪些设备可用、哪些不可用。
...
以下系统不受 FreeBSD 支持。原因可能是缺少处理器支持（UltraSPARC III）、系统设计的某处异常导致 FreeBSD 不稳定，或缺少对足够板载设备的支持以使 FreeBSD 普遍可用。
...
在可能的情况下，列出适用于每个设备或每类设备的驱动。如果相关驱动在 FreeBSD 基本发行版中拥有手册页（多数驱动都有），则在此处给出引用。关于所支持的设备、控制器等具体型号等信息，可在手册页中查阅。
...
[i386, ia64, amd64] [aac(4)](https://man.freebsd.org/cgi/man.cgi?query=aac&sektion=4&format=html) 驱动支持的控制器包括：
...
[i386, pc98, amd64] [amd(4)](https://man.freebsd.org/cgi/man.cgi?query=amd&sektion=4&format=html) 驱动支持的控制器包括：
...
[i386] [asr(4)](https://man.freebsd.org/cgi/man.cgi?query=asr&sektion=4&format=html) 驱动目前支持的适配器包括以下：
...
[i386, amd64] 真正的 BusLogic MultiMaster 兼容克隆版 AMI FastDisk 主机适配器也受 [bt(4)](https://man.freebsd.org/cgi/man.cgi?query=bt&sektion=4&format=html) 驱动支持。

[i386, ia64, amd64] [ciss(4)](https://man.freebsd.org/cgi/man.cgi?query=ciss&sektion=4&format=html) 驱动支持的控制器包括：
...
[i386, ia64, amd64] [dpt(4)](https://man.freebsd.org/cgi/man.cgi?query=dpt&sektion=4&format=html) 驱动为以下 RAID 适配器提供支持：
...
> **注意**：[amd64, i386] 支持从这些控制器引导。不支持 EISA 适配器。

[sparc64] [esp(4)](https://man.freebsd.org/cgi/man.cgi?query=esp&sektion=4&format=html) 驱动为各种系统和外围板卡中常见的 Qlogic FAS216 和 FAS408 SCSI 控制器芯片提供支持。其中包括大多数 Sun Ultra 1e 和 Ultra 2 机器中的 Qlogic SCSI 卡。对于 Qlogic PCI SCSI 主机适配器，应使用 isp(4) 驱动而非 [esp(4)](https://man.freebsd.org/cgi/man.cgi?query=esp&sektion=4&format=html) 驱动。
...
[i386] 以下控制器受 [ida(4)](https://man.freebsd.org/cgi/man.cgi?query=ida&sektion=4&format=html) 驱动支持：
...
[i386, ia64, amd64] [iir(4)](https://man.freebsd.org/cgi/man.cgi?query=iir&sektion=4&format=html) 驱动支持的控制器包括：
...
[i386, ia64, amd64] SRCU31 和 SRCU31L 可通过 Intel 提供的固件更新进行升级。

[i386, amd64] [ips(4)](https://man.freebsd.org/cgi/man.cgi?query=ips&sektion=4&format=html) 驱动支持的控制器包括：
...
[i386, ia64, amd64] [mlx(4)](https://man.freebsd.org/cgi/man.cgi?query=mlx&sektion=4&format=html) 驱动支持的控制器包括：
...
[i386, ia64, amd64] 支持所有主要固件版本（2.x、3.x、4.x 和 5.x），但建议始终升级到控制器可用的最新固件。未列出的兼容 Mylex 控制器应该可以工作，但尚未经过验证。

> **注意**：[amd64, i386] 支持从这些控制器引导。不支持 EISA 适配器。

[i386, ia64, amd64] [mly(4)](https://man.freebsd.org/cgi/man.cgi?query=mly&sektion=4&format=html) 驱动支持的控制器包括：
...
[i386, ia64, amd64] 未列出的兼容 Mylex 控制器应该可以工作，但尚未经过验证。
...
* LSI Logic FC909A（双通道 1Gb/s 光纤通道）
* LSI Logic FC919, LSI Logic 7102XP-LC（单通道 2Gb/s 光纤通道）
* LSI Logic FC929, LSI Logic FC929X, LSI Logic 7202XP-LC（双通道 2Gb/s 光纤通道）
* LSI Logic FC949X（双通道 4Gb/s 光纤通道）
* LSI Logic FC949ES（双通道 4Gb/s 光纤通道 PCI-Express）

[mpt(4)](https://man.freebsd.org/cgi/man.cgi?query=mpt&sektion=4&format=html) 驱动支持的 Ultra 320 SCSI 控制器芯片可在许多系统的板载设备中找到，包括：

* Dell PowerEdge 1750 至 2850
* IBM eServer xSeries 335

这些系统还包含 Integrated Raid Mirroring 和 Integrated Raid Mirroring Enhanced，本驱动同样予以支持。SAS 控制器芯片也出现在许多新的基于 AMD/Opteron 的系统上，如 Sun 4100。注意，该控制器可同时驱动 SAS 和 SATA 硬盘或两者的混合使用。这些控制器可用的 Integrated Raid Mirroring 支持充其量只是勉强可用。光纤通道控制器芯片组在多种速度和系统中受支持。Apple 光纤通道 HBA 实际上就是 FC949ES 卡。本驱动还支持光纤通道卡的目标模式。可通过 LSI Logic 固件实用程序设置核心的所需角色来启用此支持，该实用程序用于确定卡可承担的角色——无需单独编译。

[i386, pc98, sparc64, amd64] [ncr(4)](https://man.freebsd.org/cgi/man.cgi?query=ncr&sektion=4&format=html) 驱动为以下 NCR/Symbios SCSI 控制器芯片提供支持：
...
[i386, pc98, sparc64, amd64] 已知以下扩展板受支持：

* I-O DATA SC-98/PCI (PC-98)
* I-O DATA SC-PCI (PC-98)

[i386, pc98] 以下设备目前受 [ncv(4)](https://man.freebsd.org/cgi/man.cgi?query=ncv&sektion=4&format=html) 驱动支持：
...
[i386, pc98] [nsp(4)](https://man.freebsd.org/cgi/man.cgi?query=nsp&sektion=4&format=html) 驱动支持的控制器包括：
...
[i386, pc98] [stg(4)](https://man.freebsd.org/cgi/man.cgi?query=stg&sektion=4&format=html) 驱动支持的控制器包括：
...
[i386, pc98] 注意，Adaptec 2920C 受 ahc(4) 驱动支持。
...
[sym(4)](https://man.freebsd.org/cgi/man.cgi?query=sym&sektion=4&format=html) 支持的 SCSI 控制器可以板载在主板上，也可以是以下扩展板之一：
...
[i386, amd64] 对于 Tekram DC-310/U 和 DC-390F/U/UW/U2B/U2W/U3W PCI SCSI 主机适配器，使用 sym(4) 驱动。
...
[i386] [vpo(4)](https://man.freebsd.org/cgi/man.cgi?query=vpo&sektion=4&format=html) 驱动支持以下并行转 SCSI 接口：

* Adaptec AIC-7110 Parallel to SCSI interface（内置在 Iomega ZIP 驱动器中）
* Iomega Jaz Traveller interface
* Iomega MatchMaker SCSI interface（内置在 Iomega ZIP+ 驱动器中）

[i386] wds(4) 驱动支持 WD7000 SCSI 控制器。

对于所有受支持的 SCSI 控制器，均全面支持 SCSI-I、SCSI-II 和 SCSI-III 外围设备，包括硬盘、光盘、磁带机（包括 DAT、8mm Exabyte、Mammoth 和 DLT）、介质更换器、处理器目标设备和 CD-ROM 驱动器。支持 CD-ROM 命令的 WORM 设备可由 CD-ROM 驱动（如 [cd(4)](https://man.freebsd.org/cgi/man.cgi?query=cd&sektion=4&format=html)）进行只读访问。WORM/CD-R/CD-RW 写入支持由 [cdrecord(1)](https://man.freebsd.org/cgi/man.cgi?query=cdrecord&sektion=1&manpath=FreeBSD+Ports) 提供，该程序是 Ports Collection 中 [`sysutils/cdrtools`](http://www.FreeBSD.org/cgi/url.cgi?ports/sysutils/cdrtools/pkg-descr) port 的一部分。

目前支持以下类型的 CD-ROM 系统：

* SCSI 接口（也包括 ProAudio Spectrum 和 SoundBlaster SCSI）（[cd(4)](https://man.freebsd.org/cgi/man.cgi?query=cd&sektion=4&format=html)）
* [i386] Sony 专用接口（所有型号）（[scd(4)](https://man.freebsd.org/cgi/man.cgi?query=scd&sektion=4&format=html)）
* ATAPI IDE 接口（[acd(4)](https://man.freebsd.org/cgi/man.cgi?query=acd&sektion=4&format=html)）

[i386] 以下设备不再维护：

* Mitsumi 专用 CD-ROM 接口（所有型号）（[mcd(4)](https://man.freebsd.org/cgi/man.cgi?query=mcd&sektion=4&format=html)）
...
[i386, pc98, amd64, powerpc] [axe(4)](https://man.freebsd.org/cgi/man.cgi?query=axe&sektion=4&format=html) 驱动支持基于 ASIX Electronics AX88172 的 USB 以太网适配器，包括：
...
[i386, amd64] [bce(4)](https://man.freebsd.org/cgi/man.cgi?query=bce&sektion=4&format=html) 驱动为基于 Broadcom NetXtreme II 系列千兆以太网控制器的各种 NIC 提供支持，包括以下：
...
[i386, pc98, sparc64, ia64, amd64] [bge(4)](https://man.freebsd.org/cgi/man.cgi?query=bge&sektion=4&format=html) 驱动为基于 Broadcom BCM570x 系列千兆以太网控制器芯片的各种 NIC 提供支持，包括以下：
...
[i386, pc98, ia64, amd64, powerpc] 以下设备受 [cdce(4)](https://man.freebsd.org/cgi/man.cgi?query=cdce&sektion=4&format=html) 驱动支持：

* Prolific PL-2501 Host-to-Host Bridge Controller
* Sharp Zaurus PDA
* Terayon TJ-715 DOCSIS Cable Modem

[amd64, i386] 基于 Crystal Semiconductor CS89x0 的 NIC（[cs(4)](https://man.freebsd.org/cgi/man.cgi?query=cs&sektion=4&format=html) 驱动）
...
[i386, amd64] [cxgb(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgb&sektion=4&format=html) 驱动支持基于 T3 和 T3B 芯片组的 10 千兆以太网适配器：
...
目前已知的、可与 [dc(4)](https://man.freebsd.org/cgi/man.cgi?query=dc&sektion=4&format=html) 驱动配合工作的 NIC 如下：
...
[i386, pc98] 支持 C-Bus、ISA、PCI 和 PC Card 设备。

[em(4)](https://man.freebsd.org/cgi/man.cgi?query=em&sektion=4&format=html) 驱动支持基于 Intel 82540、82541ER、82541PI、82542、82543、82544、82545、82546、82546EB、82546GB、82547、82571、82572 和 82573 控制器芯片的千兆以太网适配器：
...
目前已知的、可与 [gem(4)](https://man.freebsd.org/cgi/man.cgi?query=gem&sektion=4&format=html) 驱动配合工作的扩展卡如下：

* Sun Gigabit Ethernet PCI 2.0/3.0 (GBE/P) (part no. 501-4373)

[hme(4)](https://man.freebsd.org/cgi/man.cgi?query=hme&sektion=4&format=html) 驱动支持许多 Sun UltraSPARC 工作站和服务器型号的板载以太网接口。[hme(4)](https://man.freebsd.org/cgi/man.cgi?query=hme&sektion=4&format=html) 驱动支持的卡包括：
...
[i386, amd64] [msk(4)](https://man.freebsd.org/cgi/man.cgi?query=msk&sektion=4&format=html) 驱动为基于 Marvell/SysKonnect Yukon II 的千兆以太网控制器芯片的各种 NIC 提供支持，包括：
...
[i386, pc98] [my(4)](https://man.freebsd.org/cgi/man.cgi?query=my&sektion=4&format=html) 驱动为基于 Myson 芯片组的各种 NIC 提供支持。支持的型号包括：
...
[i386, amd64] [nxge(4)](https://man.freebsd.org/cgi/man.cgi?query=nxge&sektion=4&format=html) 驱动支持 [http://www.neterion.com/how/pricing.html](http://www.neterion.com/how/pricing.html) 中列出的 Neterion Xframe 10 千兆以太网适配器。
...
[i386, pc98, ia64, amd64] [sf(4)](https://man.freebsd.org/cgi/man.cgi?query=sf&sektion=4&format=html) 驱动支持的适配器包括：
...
[pc98] [snc(4)](https://man.freebsd.org/cgi/man.cgi?query=snc&sektion=4&format=html) 驱动还支持 National Semiconductor NS46C46 作为 64 * 16 位 Microwave Serial EEPROM。
...
[i386, amd64, sparc64] [stge(4)](https://man.freebsd.org/cgi/man.cgi?query=stge&sektion=4&format=html) 驱动为基于 Sundance/Tamarack TC9021 的千兆以太网控制器芯片的各种 NIC 提供支持，包括：
...
[i386, pc98, amd64, sparc64] [ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动支持基于 Alteon Tigon I 和 II 芯片的千兆以太网适配器。[ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动已通过以下适配器测试：
...
[i386, pc98, amd64, sparc64] 以下适配器应该也受支持，但尚未经过测试：
...
[i386, pc98, amd64] [tl(4)](https://man.freebsd.org/cgi/man.cgi?query=tl&sektion=4&format=html) 驱动还支持各种 Compaq Prosignia 服务器和 Compaq Deskpro 台式机的内置以太网适配器，包括：
...
[amd64, i386, pc98] 基于 SMC 83c17x (EPIC) 的以太网 NIC（[tx(4)](https://man.freebsd.org/cgi/man.cgi?query=tx&sektion=4&format=html) 驱动）

[i386, pc98, ia64, amd64] [txp(4)](https://man.freebsd.org/cgi/man.cgi?query=txp&sektion=4&format=html) 驱动支持以下卡：
...
3C656 系列 CardBus 网卡和 3C556 系列 MiniPCI 网卡均内置专有调制解调器。[xl(4)](https://man.freebsd.org/cgi/man.cgi?query=xl&sektion=4&format=html) 驱动和其他任何驱动均不支持该调制解调器。

---

### 3.3 Token Ring 接口

[i386] [oltr(4)](https://man.freebsd.org/cgi/man.cgi?query=oltr&sektion=4&format=html) 驱动支持以下基于 ISA 的 Olicom 令牌环适配器：
...
[i386, pc98] DEC DEFPA PCI（[fpa(4)](https://man.freebsd.org/cgi/man.cgi?query=fpa&sektion=4&format=html) 驱动）
...
[i386, pc98] 基于 Midway 的 ATM 接口（[en(4)](https://man.freebsd.org/cgi/man.cgi?query=en&sektion=4&format=html) 驱动）

[i386, pc98 sparc64] FORE Systems, Inc. PCA-200E ATM PCI 适配器（hfa 和 [fatm(4)](https://man.freebsd.org/cgi/man.cgi?query=fatm&sektion=4&format=html) 驱动）

[i386, pc98] 基于 IDT NICStAR 77201/211 的 ATM 适配器（[idt(4)](https://man.freebsd.org/cgi/man.cgi?query=idt&sektion=4&format=html) 驱动）

[i386, pc98 sparc64] FORE Systems, Inc. HE155 和 HE622 ATM 接口（[hatm(4)](https://man.freebsd.org/cgi/man.cgi?query=hatm&sektion=4&format=html) 驱动）

[i386, pc98] 基于 IDT77252 的 ATM 网卡（[patm(4)](https://man.freebsd.org/cgi/man.cgi?query=patm&sektion=4&format=html) 驱动）

---

### 3.6 无线网络接口

[amd64, i386, pc98] Cisco/Aironet 802.11b 无线适配器（[an(4)](https://man.freebsd.org/cgi/man.cgi?query=an&sektion=4&format=html) 驱动）

[i386, pc98, amd64, sparc64] [ath(4)](https://man.freebsd.org/cgi/man.cgi?query=ath&sektion=4&format=html) 驱动支持所有 Atheros Cardbus 或 PCI 网卡，但不支持基于 AR5005VL 芯片组的网卡。受支持的网卡列表可参见 [http://customerproducts.atheros.com/customerproducts/default.asp](http://customerproducts.atheros.com/customerproducts/default.asp)。

[i386, pc98, amd64] [awi(4)](https://man.freebsd.org/cgi/man.cgi?query=awi&sektion=4&format=html) 驱动支持的网卡包括：
...
[i386, pc98, amd64] [cnw(4)](https://man.freebsd.org/cgi/man.cgi?query=cnw&sektion=4&format=html) 驱动支持的网卡包括：

* Xircom CreditCard Netwave
* NetWave AirSurfer

[i386, amd64] Intel PRO/Wireless 2100 MiniPCI 网络适配器（[ipw(4)](https://man.freebsd.org/cgi/man.cgi?query=ipw&sektion=4&format=html) 驱动）

[i386, amd64] Intel PRO/Wireless 2200BG/2915ABG MiniPCI 和 2225BG PCI 网络适配器（[iwi(4)](https://man.freebsd.org/cgi/man.cgi?query=iwi&sektion=4&format=html) 驱动）
...
[i386, amd64] 最新列表可参见 [http://damien.bergamini.free.fr/ral/list.html](http://damien.bergamini.free.fr/ral/list.html)。

[i386, pc98] Raytheon Raylink 2.4GHz 无线适配器（[ray(4)](https://man.freebsd.org/cgi/man.cgi?query=ray&sektion=4&format=html) 驱动）
...
[i386, amd64] 最新列表可参见 [http://ralink.rapla.net/](http://ralink.rapla.net/)。

[amd64, i386, pc98] Lucent Technologies WaveLAN/IEEE 802.11b 无线网络适配器，以及使用 Lucent Hermes、Intersil PRISM-II、Intersil PRISM-2.5、Intersil Prism-3 和 Symbol Spectrum24 芯片组的兼容产品（[wi(4)](https://man.freebsd.org/cgi/man.cgi?query=wi&sektion=4&format=html) 驱动）

[i386] NCR / AT&T / Lucent Technologies WaveLan T1 速率 ISA/无线电 LAN 网卡（[wl(4)](https://man.freebsd.org/cgi/man.cgi?query=wl&sektion=4&format=html) 驱动）
...
[i386] Granch SBNI12 点对点通信适配器（[sbni(4)](https://man.freebsd.org/cgi/man.cgi?query=sbni&sektion=4&format=html) 驱动）

[i386] Granch SBNI16 SHDSL 调制解调器（[sbsh(4)](https://man.freebsd.org/cgi/man.cgi?query=sbsh&sektion=4&format=html) 驱动）

[i386] [cm(4)](https://man.freebsd.org/cgi/man.cgi?query=cm&sektion=4&format=html) 驱动支持以下网卡型号：
...
[amd64 i386] "PC 标准"基于 8250、16450 和 16550 的串行端口（[sio(4)](https://man.freebsd.org/cgi/man.cgi?query=sio&sektion=4&format=html) 驱动）
...
[i386] ARNET 串行卡（[ar(4)](https://man.freebsd.org/cgi/man.cgi?query=ar&sektion=4&format=html) 驱动）
...
* Boca BB1004 4 端口串行卡（**不**支持调制解调器）
* Boca IOAT66 6 端口串行卡（支持调制解调器）
* Boca BB1008 8 端口串行卡（**不**支持调制解调器）
...
[i386] [rc(4)](https://man.freebsd.org/cgi/man.cgi?query=rc&sektion=4&format=html) 驱动提供对 SDL Communications RISCom/8 板卡的支持。
...
[i386, amd64] Specialix SI/XIO/SX 多端口串行卡，包括较早的 SIHOST2.x 和"增强型"（基于 transputer，即 JET）主机卡（ISA、EISA 和 PCI）均受支持。注意，较新的 SX+ PCI 卡目前不受支持。（[si(4)](https://man.freebsd.org/cgi/man.cgi?query=si&sektion=4&format=html) 驱动）
...
[i386, amd64] 部分板载 CS4610 芯片搭配 CS423x ISA 编解码器，而非 CS4297 AC97 编解码器。[snd_csa(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_csa&sektion=4&format=html) 驱动暂不支持此类配置。
...
* Creative Sound Blaster Audigy 2 和 Creative Sound Blaster Audigy 4（CA0102 芯片组）。PCM 支持仅限 48kHz/16 位立体声（不支持该芯片组的 192kHz/24 位部分）。
* Creative Sound Blaster Audigy 2 Value（CA0108 芯片组）。PCM 支持仅限 48kHz/16 位立体声（不支持该芯片组的 192kHz/24 位部分）。该卡不支持 MIDI。

[i386, amd64] [snd_emu10kx(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_emu10kx&sektion=4&format=html) 驱动不支持以下声卡（尽管其名称与某些受支持的声卡相似）：

* Creative Sound Blaster Live! 24-Bit，标识为
* Creative Sound Blaster Audigy LS / ES，标识为
...
[i386, amd64] 与 Intel HDA 规范兼容的通用音频芯片组应该可用，但尚未经过验证。以下编解码器已经过验证可用：
...
[i386, amd64] 注意，采用 ES18xx 芯片组的较旧 ESS ISA 卡通过 snd_ess(4) 和/或 snd_sbc(4) 支持。
...
[pc98] NEC PC-9801-73、86 及兼容产品（nss 驱动）

* NEC A-MATE 内置声卡
* Q-Vision WaveStar、WaveMaster

[pc98] NEC X-MATE、CanBe、ValueStar 内置声卡（mss 驱动）

[pc98] Creative Technologies SoundBlaster(98)（[sb(4)](https://man.freebsd.org/cgi/man.cgi?query=sb&sektion=4&format=html) 驱动）
...
[amd64, i386, ia64, pc98] FreeBSD 支持一系列 USB 外设；已知可正常工作的设备在本节中列出。由于大多数 USB 设备具有通用性，在少数例外情况下，任何属于特定类别的设备都会受支持，即使未在此明确列出。

> **注意**：[amd64, i386, ia64, pc98] USB 以太网适配器可在 [以太网接口](https://www.freebsd.org/releases/7.0R/hardware.html#ETHERNET) 章节中找到。

> **注意**：[amd64, i386, ia64, pc98] USB 蓝牙适配器可在 [蓝牙](https://www.freebsd.org/releases/7.0R/hardware.html#BLUETOOTH) 章节中找到。
...
[i386, pc98, amd64, powerpc] [ulpt(4)](https://man.freebsd.org/cgi/man.cgi?query=ulpt&sektion=4&format=html) 驱动提供对 USB 打印机和并行打印机转换电缆的支持，包括以下型号：
...
[i386, pc98, amd64, powerpc] 由于 USB 扫描仪没有标准设备类别，此驱动仅识别其自身表中明确列出 USB ID 的设备。截至目前，受支持的设备如下：
...
[i386, pc98, amd64] [ng_bt3c(4)](https://man.freebsd.org/cgi/man.cgi?query=ng_bt3c&sektion=4&format=html) 驱动提供对 3Com/HP 3CRWB6096-A PCCARD 蓝牙适配器的支持。
...
[i386, pc98, amd64] [hifn(4)](https://man.freebsd.org/cgi/man.cgi?query=hifn&sektion=4&format=html) 驱动支持各种包含 Hifn 7751、7951、7811、7955 和 7956 芯片组的卡，例如：
...
> **注意**：关于特定显卡及其与 **Xorg** 兼容性的信息可参见 [http://www.x.org/](http://www.x.org/)。

[amd64, i386, ia64, pc98] 键盘包括：

* [i386] AT 键盘（[atkbd(4)](https://man.freebsd.org/cgi/man.cgi?query=atkbd&sektion=4&format=html) 驱动）
...
> **注意**：[moused(8)](https://man.freebsd.org/cgi/man.cgi?query=moused&sektion=8&format=html) 提供了在 FreeBSD 中使用指点设备的更多信息。关于在 **Xorg** 中使用指点设备的信息可参见 [http://www.x.org/](http://www.x.org/)。
