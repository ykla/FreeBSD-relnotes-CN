...
* 10.4-RELEASE 是首个全面支持 eMMC 存储的 FreeBSD 发行版，包括 eMMC 分区、TRIM 以及高达 HS400 的总线速度模式。但请注意，DDR52、HS200 和 HS400 模式的可用性尤其需要实际的 `sdhci(4)` 前端以及所用硬件的支持。另请注意，Intel(R) Apollo Lake 芯片组的 SDHCI 控制器部分受若干严重的硅片缺陷影响。显然，目前已有的变通方案是否足以避免在附加 `sdhci(4)` 时出现超时，取决于特定的 Apollo Lake 平台。
* 在使用 GPT 磁盘标签时，`fsck_ffs(8)` 工具现在能够找到备用超级块。
* `aesni(4)` 驱动不再在多个线程的多个会话之间共享单个 FPU 上下文，解决了使用 `aesni(4)` 加速 `ipsec(4)` 时出现的问题。
* 新增对 Intel(R) i219(4)/ i219(5) 设备的 Kaby Lake 代支持至 `em(4)` 驱动。
* `em(4)` 驱动现在能够为 Intel(R) i217、i218 和 i219 芯片启用局域网唤醒（WOL）。注意，此前为这些设备启用 WOL 的失败尝试所留下的失效接口配置现在将实际生效。例如，`\`ifconfig em0 wol\`` 会激活所有 WOL 变体（包括 wol_mcast），这可能并非所愿。
...
* Mellanox(R) ConnectX-4 系列适配器现在由新增的 `mlx5ib(4)` 驱动支持。
...
* Amazon(R) EC2(TM)：
  ...
  AMI 完成第三方特定验证后，也将可在 Amazon(R) Marketplace 中使用：
...
许多公司捐赠了设备、网络访问或人力来支持 FreeBSD 10.4 的发布工程活动，包括 The FreeBSD Foundation、Yahoo!、NetApp、Internet Systems Consortium、ByteMark Hosting、Sentex Communications、New York Internet、Juniper Networks、NLNet Labs、iXsystems 和 Yandex。
...
喜爱 FreeBSD？请通过向 The FreeBSD Foundation [捐赠](https://www.freebsdfoundation.org/donate/) 来支持本版本及未来版本！
