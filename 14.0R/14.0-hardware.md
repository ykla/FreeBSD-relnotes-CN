# FreeBSD 14.0-RELEASE 硬件兼容列表

## 引言

本硬件兼容性说明包含 FreeBSD 14.0 的硬件兼容性信息。其中列出了 FreeBSD 支持的硬件平台以及各类硬件设备（存储控制器、网络接口等），并给出已知可正常工作的设备实例。
...
FreeBSD 维护对 i386（x86）作为 Tier 2 架构的支持。不建议用于新安装。

> **警告**：此架构的支持将在 FreeBSD 15.0 中移除，因此强烈建议所有用户和管理员将其安装升级为 [AMD64](#64-位-x86amd64-架构支持)。
...
Allwinner (64-bit)，更多信息请参阅 [Wiki page](https://wiki.freebsd.org/arm/Allwinner)：
...
树莓派，更多信息请参阅 [Wiki page](https://wiki.freebsd.org/arm/Raspberry%20Pi)：

* 树莓派 3
* 树莓派 4

RockChip (64-bit)，更多信息请参阅 [Wiki page](https://wiki.freebsd.org/arm/RockChip)：
...
Allwinner (32-bit)，更多信息请参阅 [Wiki page](https://wiki.freebsd.org/arm/Allwinner)：
...
NXP i.MX6 (32-bit)，更多信息请参阅 [Wiki page](https://wiki.freebsd.org/arm/imx6)：
...
更多信息请参阅 [Wiki page](https://wiki.freebsd.org/riscv)。
...
* SCSI 接口（也包括 ProAudio Spectrum 和 SoundBlaster SCSI）（[cd(4)](https://man.freebsd.org/cgi/man.cgi?query=cd&sektion=4&format=html)）
...
Elastic Network Adapter（ENA）系列的 FreeBSD 内核驱动（[ena(4)](https://man.freebsd.org/cgi/man.cgi?query=ena&sektion=4&format=html) 驱动）
...
使用较旧 Broadcom 芯片组（BCM4301、BCM4303 和 BCM4306 rev 2）的用户必须使用 **bwi(4)**，因为 v4 版本的固件不支持这些芯片。较新的固件过大，无法放入这些旧芯片。
...
[wpi(4)](https://man.freebsd.org/cgi/man.cgi?query=wpi&sektion=4&format=html) 驱动支持 Intel PRO/Wireless 3945ABG Mini PCIe 网络适配器。
...
> **注意**：
>
> USB 网络接口可在 [以太网接口](https://www.freebsd.org/releases/14.0R/hardware.html#ethernet) 或 [无线网络接口](https://www.freebsd.org/releases/14.0R/hardware.html#wlan) 中查阅。

> **注意**：
>
> USB 蓝牙适配器可在 [蓝牙](https://www.freebsd.org/releases/14.0R/hardware.html#bluetooth) 中查阅。

> **注意**：
>
> USB 人机接口设备可在 [其他设备](https://www.freebsd.org/releases/14.0R/hardware.html#misc) 中查阅。
...
[ubser(4)](https://man.freebsd.org/cgi/man.cgi?query=ubser&sektion=4&format=html) 驱动支持 BWCT 控制台管理串行适配器。
...
*	Jablotron PC-60B
*	Kamstrup M-Bus Master MultiPort 250D 及 Optical Eye/3 线公用仪表接口
*	Kyocera GPS
*	Link Instruments MS-019 and MS-028 示波器/逻辑分析仪/模式发生器
...
> 唯一基于 7811 的板卡（比 7751 更快，且带有随机数生成器）。
...
> 参见 *<http://www.soekris.com/>*。包含 7951 芯片，支持对称加密和随机数运算。

Soekris Engineering vpn1401 and vpn1411

> 参见 *<http://www.soekris.com/>*。包含 7955 芯片，支持对称加密和随机数运算。
...
* HID 键盘（[hkbd(4)](https://man.freebsd.org/cgi/man.cgi?query=hkbd&sektion=4&format=html) 驱动），通过 USB、I2C 或 Apple SPI
...
* HID 鼠标（[hms(4)](https://man.freebsd.org/cgi/man.cgi?query=hms&sektion=4&format=html) 驱动）和触摸设备（[hmt(4)](https://man.freebsd.org/cgi/man.cgi?query=hmt&sektion=4&format=html) 驱动）通过 USB、I2C、Hyper-V 或 Apple SPI
...
> **注意**：
>
> **moused(8)** 包含关于在 FreeBSD 控制台中使用传统指针设备的更多信息。
...
*	集成于 Family 15h Models 60h-6Fh、70h-7Fh 处理器的 AMD FCH
*	集成于 Family 16h Models 00h-0Fh、30h-3Fh 处理器的 AMD FCH
