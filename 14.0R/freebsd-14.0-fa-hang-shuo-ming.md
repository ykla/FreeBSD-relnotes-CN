...
在 DPAA2（第二代数据路径加速架构，出现在一些 NXP SoC 中的硬件级网络架构）中有所改进。它运行由 NXP 提供的固件，该固件提供 DPAA2 对象作为抽象层，并提供 `dpni` 网络接口。DPAA2 通道之间的分离已经显着改进，以隔离对 DMA 资源和清理操作的访问，并在网络负载较重（1 Gbit/s 链接）时避免内核崩溃。其他改进包括 FDT / ACPI MDIO 支持，通过 DPAA2 进行网络启动以及单独的命令门户（DPMCP）支持。
...
合成文件系统（[devfs(5)](https://man.freebsd.org/cgi/man.cgi?query=devfs&sektion=5&format=html)，[procfs(5)](https://man.freebsd.org/cgi/man.cgi?query=procfs&sektion=5&format=html) 等）现在报告已使用 0 块，以免它们被报告为 100％ 已满。[88a795e80c03](https://cgit.freebsd.org/src/commit/?id=88a795e80c03f)
