# FreeBSD 12.4-RELEASE 发行公告

日期：2022 年 12 月 5 日

* 修订：0.0：初始发行公告。
* 修订：1.1a：更正 AWS URL 和公告日期。
* 修订：1.1b：更正重复的 RPI-B 条目，新增 RPI2。

FreeBSD 发布工程团队很高兴地宣布 FreeBSD 12.4-RELEASE 现已发布。这是 stable/12 分支的第五个版本。

部分亮点如下：

* **ena(4)** 内核驱动已更新至 2.6.1 版本。
* **if_epair(4)** 驱动现在允许使用多个核心处理流量，从而提高性能。
* **unbound(8)** 工具已更新至 1.16.3 版本。
* **telnetd(8)** 守护进程已弃用。
* **tcpdump(1)** 工具现在允许用户在规则上设置数字，这些数字将作为 pflog 标头的一部分显示。
* OpenSSL 已更新至 1.1.1q。
* OpenSSH 已更新至 9.1p1。
* LLVM 工具链套件已更新至 13.0.0 版本。
* **dma(8)** 工具已更新至 2022-01-27 快照。
* **file(1)** 工具已更新至 5.43 版本。
* **libarchive(3)** 库已更新至 3.6.0 版本。
* 以及更多……
...
FreeBSD 12.4-RELEASE 现已支持 amd64、i386、powerpc、powerpc64、powerpc64le、powerpcspe、armv6、armv7、aarch64 和 riscv64 架构。

FreeBSD 12.4-RELEASE 可从可启动 ISO 镜像或通过网络安装。部分架构还支持从 USB 闪存盘安装。所需文件可按以下说明下载。
...
FreeBSD 12.4-RELEASE 也可从多家供应商处购买 CD-ROM 或 DVD 介质。提供 FreeBSD 12.4 相关产品的供应商之一是：

* FreeBSD Mall, Inc. <https://www.freebsdmall.com>

预安装的虚拟机镜像也可用于 amd64（x86_64）、i386（x86_32）、AArch64（arm64）和 RISCV 架构，提供 `QCOW2`、`VHD` 和 `VMDK` 磁盘镜像格式，以及原始（未格式化）镜像。

FreeBSD 12.4-RELEASE amd64 还可在以下云托管平台上使用：
...
AMI 也可在 Amazon Marketplace 中获取：<https://aws.amazon.com/marketplace/pp/prodview-5u5eckd56z6my>
...
FreeBSD 12.4-RELEASE 预计还将在 Google Compute Engine(TM) Marketplace 中提供，待其完成第三方特定验证后，访问：<https://console.cloud.google.com/launcher/browse?filter=category:os&filter=price:free>
...
FreeBSD 12.4-RELEASE 可从以下站点通过 https 下载：

* <https://download.freebsd.org/releases/ISO-IMAGES/12.4/>

FreeBSD 12.4-RELEASE 虚拟机镜像可从以下地址下载：

* <https://download.freebsd.org/releases/VM-IMAGES/12.4-RELEASE/>

关于安装 FreeBSD 或将现有机器更新至 12.4-RELEASE 的说明，请参阅：

* <https://www.FreeBSD.org/releases/12.4R/installation/>

## 支持

根据新的 FreeBSD 支持模型，FreeBSD 12 发行系列将至少支持至 2023 年 1 月 31 日。本点发行版 FreeBSD 12.4-RELEASE 将支持至 FreeBSD 12.4-RELEASE 生命周期结束。其他支持信息可在以下地址找到：

* <https://www.FreeBSD.org/security/>

请注意，12.3 将支持至 12.4 发行日期起三个月后，截至本文撰写时该日期尚未确定。

## 致谢

许多公司捐赠了设备、网络访问或人力时间来支持 FreeBSD 12.4 的发布工程活动，包括：
...
12.4-RELEASE 的发布工程团队包括：

| 成员 | 角色 |
| :--- | :--- |
| Glen Barber <gjb@FreeBSD.org> | 发布工程负责人，12.4-RELEASE 发布工程师 |
