...
FreeBSD 12.3-RELEASE 的发行说明总结了 12-STABLE 开发分支上对 FreeBSD 基本系统所做的变更。本文档列出了自上一个版本以来发布的适用安全公告，以及对 FreeBSD 内核和用户空间的重大变更。同时，还提供了关于升级的一些简要说明。
...
FreeBSD 12.3-RELEASE 的这个发行版本是一个 release 类型的发行版。它可通过 [https://www.FreeBSD.org/releases/](https://www.freebsd.org/releases/) 或其镜像站点获取。获取该版本（及其他版本）发行版的更多信息，请参阅 [FreeBSD 手册](https://docs.freebsd.org/en/books/handbook/) 的 [获取 FreeBSD 附录](https://docs.freebsd.org/en/books/handbook/mirrors)。

建议所有用户在安装 FreeBSD 前查阅发行勘误文档（勘误）。该文档包含在发布周期后期或发布之后发现的最新信息，通常包括已知的错误、安全公告和文档修正。可以在 FreeBSD 网站上找到 FreeBSD 12.3-RELEASE 的最新勘误文档。

本文档介绍了自 12.2-RELEASE 以来 FreeBSD 中新增或变更的最直观的用户功能。通常，这些变更是 12-STABLE 分支特有的，除非特别标注为 MERGED 特性。

发行说明通常记录自 12.2-RELEASE 之后发布的安全公告、新增的驱动程序或硬件支持、新的命令或选项、重大错误修复或第三方软件的升级。它们还可能列出主要的 Ports/软件包变更或发布工程实践的更新。不过，发行说明不可能涉及 FreeBSD 两个版本之间的所有变更，本文档主要集中于安全公告、用户可见的变更和主要的架构改进。

## 从旧版 FreeBSD 升级

可以使用 [freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 工具在 RELEASE 版本之间（以及各种安全分支的快照版本）进行二进制升级。二进制升级过程将更新未修改的用户空间工具以及作为 FreeBSD 官方发行版一部分的未修改 **GENERIC** 内核。[freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 工具要求被升级的主机具备互联网连接。

基于源代码的升级（通过重新编译 FreeBSD 基本系统源码进行的升级）也得到支持，具体步骤请参见 **/usr/src/UPDATING** 文件中的说明。

> **重要**：
>
> 升级 FreeBSD 前，必须备份所有数据和配置文件。
...
`caroot` CA 捆绑处理器已更新，支持带有 `DISTRUST_AFTER` 条目的证书。

[rc(8)](https://man.freebsd.org/cgi/man.cgi?query=rc&sektion=8&format=html) 脚本 **/etc/rc.final** 将在所有用户进程终止后运行。
...
实用程序 [bhyve(8)](https://man.freebsd.org/cgi/man.cgi?query=bhyve&sektion=8&format=html) 中 [nvme(4)](https://man.freebsd.org/cgi/man.cgi?query=nvme&sektion=4&format=html) 模拟的大型 IO 支持问题修复完成。
...
实用程序 [cpuset(1)](https://man.freebsd.org/cgi/man.cgi?query=cpuset&sektion=1&format=html) 现在允许 Jail 修改子 Jail 的根目录。

实用程序 [cron(8)](https://man.freebsd.org/cgi/man.cgi?query=cron&sektion=8&format=html) 现在会引入用户或登录类的环境变量。

实用程序 [daemon(8)](https://man.freebsd.org/cgi/man.cgi?query=daemon&sektion=8&format=html) 新增参数 `-H`，用于捕获 `SIGHUP` 信号并重新打开输出文件。此功能用于支持 [newsyslog(8)](https://man.freebsd.org/cgi/man.cgi?query=newsyslog&sektion=8&format=html) 操作。

实用程序 [diff(1)](https://man.freebsd.org/cgi/man.cgi?query=diff&sektion=1&format=html) 现在在指定 `-q` 时也会遵循其他参数，例如 `-w`。

实用程序 [elfctl(1)](https://man.freebsd.org/cgi/man.cgi?query=elfctl&sektion=1&format=html) 新增参数 `-l`，可忽略未知变量，从而使其能在不同版本的 FreeBSD 间工作，并忽略未实现的功能。

实用程序 [etcupdate(8)](https://man.freebsd.org/cgi/man.cgi?query=etcupdate&sektion=8&format=html) 现在支持恢复模式，可用于还原一个或多个文件。

实用程序 [etcupdate(8)](https://man.freebsd.org/cgi/man.cgi?query=etcupdate&sektion=8&format=html) 新增了参数 `-D`，用于指定目标目录。
...
实用程序 [fstyp(8)](https://man.freebsd.org/cgi/man.cgi?query=fstyp&sektion=8&format=html) 现在可以使用参数 `-l` 检测并显示 exFAT 文件系统。
...
实用程序 [growfs(8)](https://man.freebsd.org/cgi/man.cgi?query=growfs&sektion=8&format=html) 现在可以运行在 `RW` 挂载的文件系统上。
...
实用程序 [mksnap_ffs(8)](https://man.freebsd.org/cgi/man.cgi?query=mksnap_ffs&sektion=8&format=html) 中导致 `Panic: snapacct_ufs2: bad block` 的崩溃问题修复完成。
...
实用程序 [newsyslog(8)](https://man.freebsd.org/cgi/man.cgi?query=newsyslog&sektion=8&format=html) 新增了参数 `E`，用于防止轮转空日志文件。
...
[rc(8)](https://man.freebsd.org/cgi/man.cgi?query=rc&sektion=8&format=html) 脚本 **rc.d/jail** 变更了某关键字，以修复对嵌套 Jail 的支持。
...
[unzip(1)](https://man.freebsd.org/cgi/man.cgi?query=unzip&sektion=1&format=html) 在目标归档文件包含损坏的名称时引发的段错误修复完成。
...
实用程序 [wpl_cli(8)](https://man.freebsd.org/cgi/man.cgi?query=wpl_cli&sektion=8&format=html) 新增了动作文件事件功能，可将事件传递给文件。

### 第三方软件

[awk(1)](https://man.freebsd.org/cgi/man.cgi?query=awk&sektion=1&format=html) 的 `metamode` 修复已合并，同时代码与上游同步到版本 20210221。
...
[libarchive(3)](https://man.freebsd.org/cgi/man.cgi?query=libarchive&sektion=3&format=html) 库的符号链接处理错误修复完成。
...
krpc 和 nfsd 模块间的内部 KAPI 已更新（参见 `UPDATING`）。

[powf(3)](https://man.freebsd.org/cgi/man.cgi?query=powf&sektion=3&format=html) 库中当 `x` 接近 `1` 且 `|y|` 远大于 `1` 时可能出现的错误结果修复完成，同时从 NetBSD 导入了一套测试工具。
...
新增了 [sysctl(8)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&format=html) 项 `debug.uma_reclaim`。
...
驱动程序 [cam(4)](https://man.freebsd.org/cgi/man.cgi?query=cam&sektion=4&format=html) 的 SCSI 快速拔插问题修复完成。
...
设备驱动程序 [em(4)](https://man.freebsd.org/cgi/man.cgi?query=em&sektion=4&format=html) 的共享代码已进行多项更新。
...
设备驱动程序 [ixgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=ixgbe&sektion=4&format=html) 的共享代码已更新。

设备驱动程序 [ixgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=ixgbe&sektion=4&format=html) 的 x550em 10G NIC 链路状态问题修复完成，其中自动协商功能未正确报告。

设备驱动程序 [ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 新增了可调参数 `hw.ix.flow_control`。

设备驱动程序 [ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 的共享代码已更新，2.5G 和 5G 网络速度问题也修复完成。

设备驱动程序 [iwm(4)](https://man.freebsd.org/cgi/man.cgi?query=iwm&sektion=4&format=html) 现支持 Intel Killer Wireless-AC 1550i。

文件系统驱动程序 [msdosfs(5)](https://man.freebsd.org/cgi/man.cgi?query=msdosfs&sektion=5&format=html) 的挂起问题修复完成。
...
netgraph 节点 [ng_source(4)](https://man.freebsd.org/cgi/man.cgi?query=ng_source&sektion=4&format=html) 现在可以注入到任意 netgraph 网络中。
...
防火墙 [pf(4)](https://man.freebsd.org/cgi/man.cgi?query=pf&sektion=4&format=html) 收到多项错误修复和更新。
...
对 `UFS/FFS` 中嵌入式符号链接的处理修复已合并。

解决 NFSv4.1 Linux 客户端挂载卡在 `CLOSE_WAIT` 状态问题的修复已合并。

对 NFSv4.1/4.2 挂载从过期租约中恢复的修复已合并。

## 引导加载程序变更

本节涉及引导加载程序、启动菜单和其他与启动相关的变更。

### 引导加载程序变更

引导加载程序现在支持从内存磁盘启动操作系统。

引导加载程序现在支持没有特性的池。

引导加载程序现在接受 zfs 特性 `com.delphix:bookmark_written` 和 `com.datto:bookmark_v2`。

新增了 OID `hint.dev.X.disabled`，添加到 lua 加载器以在启动过程中防止设备附加。
...
在 `wpa` EAP/PEAP MSCHAPv2 身份验证过程中发生的段错误修复完成。
...
对记录 EC2 AMI Ids 的支持添加到 **release/Makefile.ec2**，以允许 SSM 参数名称采用类似 **/aws/service/freebsd/amd64/base/ufs/12.3/RELEASE** 的格式，使用公共前缀 **/aws/service/freebsd**。
...
由于 i486 和 i586 CPU 主要用于嵌入式市场，因此预期对普通用户的影响最小，因为这些 CPU 类型的新硬件已经很久未出现，而这些系统的部署基础从统计数据来看正在接近退休年龄。

在做出此变更时考虑了多个因素。例如，i486 不支持 64 位原子操作，尽管它们可以在内核中模拟，但无法在用户空间模拟。此外，32 位的 amd64 库自诞生起就是 i686。

由于大多数 32 位测试由开发人员在 64 位硬件上通过 lib32 库进行（使用内核选项 `COMPAT_FREEBSD32`），此变更确保了更好的覆盖率和用户体验。这也符合大多数 Linux 发行版的做法，Linux 发行版已经采用这种做法有一段时间了。
