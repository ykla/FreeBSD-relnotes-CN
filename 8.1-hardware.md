# FreeBSD 8.1-RELEASE 硬件兼容列表

## 引言

本文档包含 FreeBSD 8.1-RELEASE 的硬件兼容性说明。其中列出了 FreeBSD 支持的硬件平台，以及各类硬件设备（存储控制器、网络接口等），并给出已知可正常工作的设备实例。
...
本节介绍 FreeBSD 在 Fujitsu SPARC64(R) 和 Sun UltraSPARC(R) 平台上当前已知支持的系统。关于各种硬件设计的背景信息，请参阅 [Sun System Handbook](http://sunsolve.sun.com/handbook_pub/)。
...
如果你有不在此列表中的系统，它可能尚未经过 FreeBSD 8.1-RELEASE 测试。我们鼓励你尝试并致信 [FreeBSD SPARC porting mailing list](http://lists.FreeBSD.org/mailman/listinfo/freebsd-sparc64) 反馈结果，包括哪些设备可用、哪些不可用。
...
* Sun Fire V880
* Sun Fire V890 (support first appeared in 8.1-RELEASE, non-mixed UltraSPARC IV/IV+ CPU-configurations only)
* Netra 20/Netra T4
...
* Sun Fire V490 (support first appeared in 8.1-RELEASE, non-mixed UltraSPARC IV/IV+ CPU-configurations only)

自 8.1-RELEASE 起，基于 Fujitsu SPARC64(R) V 的 sparc64 系统也受 FreeBSD 支持，包括以下已知可工作的系统：
