...
- **twe(4) 驱动更新**
  针对 3Ware Escalade 控制器的 twe(4) 驱动已更新。
...
- 修复了一个漏洞，该漏洞可能使攻击者中断 TCP 和 UDP 的“会话”。
...
- **burncd(8)** 现在支持 `-m` 选项，进入多会话模式（默认行为是将磁盘关闭为单会话）。还新增了 `-l` 选项，用于从文件名中获取映像文件列表；可以使用 `-` 作为标准输入的文件名。

- **tar(1)** 现在支持 `TAR_RSH` 变量，主要用于启用将 **ssh(1)** 用作传输工具。

- 修复了 **make(1)** 中的多个 bug，包括空后缀行为错误、当前目录权限假设不当和潜在的缓冲区溢出问题。

- 新的 **CPUTYPE** 配置选项可以控制在编译过程中应用处理器特定的优化，适用于 OpenSSL 等代码。
...
- **dump(8)** 现在支持将 `nodump` 标志继承到层级结构中。
