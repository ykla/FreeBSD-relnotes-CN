# FreeBSD 4.3-RELEASE 发行说明（2001 年 4 月 20 日）

**发行说明**

**FreeBSD 4.3-RELEASE 版本**

如果遇到任何安装失败或系统崩溃的问题，请使用命令 `send-pr` 进行报告（如果你更喜欢基于网页的界面，也可以访问 [http://www.FreeBSD.org/send-pr.html](http://www.FreeBSD.org/send-pr.html)）。

关于 FreeBSD 和 4.3-RELEASE 目录布局的信息（特别是使用软盘安装时），请参阅 `ABOUT.TXT`。安装说明请参阅 `INSTALL.TXT` 和 `HARDWARE.TXT` 文件。
...
- **实现崩溃转储的写合并功能**
  当 SCSI 和 IDE 磁盘禁用写缓存时，这项功能尤为有用，可以显著缩短大内存转储的完成时间（可能需要长达一小时）。

- **pccard 驱动和 pccardc(8)**
  现在支持在卡插入和移除时发出多种“蜂鸣音类型”。

- **twe(4) 驱动更新**
  针对 3Ware Escalade 控制器的 **twe(4)** 驱动进行了更新。

- **an(4) 驱动增强**
  对 Cisco Aironet 卡新增有线等效隐私（WEP）加密支持，可通过 **ancontrol(8)** 配置。

- **wi(4) 和 an(4) 驱动默认模式变更**
  默认模式变更为 BSS（基础架构）模式，而非之前的 ad-hoc 模式。

- **新增 ray(4) 驱动**
  新增对 Webgear Aviator 无线网卡的支持，其操作可通过 **raycontrol(8)** 调整。

- **新增 aac(4) 驱动**
  提供对 Adaptec FSA 系列 PCI-SCSI RAID 控制器的支持。

- **Linksys 快速以太 PCCARD 配置变更**
  **ed(4)** 驱动支持的 Linksys 快速以太 PCCARD 卡现在必须在 **pccard.conf(5)** 配置文件中添加 `flag 0x80000`。该参数为强制要求，否则无法识别这些卡。

- **修复 ed(4) 驱动中的崩溃问题**
  修复了因短数据包和 BPF 或桥接同时激活而导致的崩溃问题。

- **修复 FFS 超大文件系统的超级块损坏问题**
  修复了 FFS 中可能导致超大文件系统超级块损坏的 bug。

- **ISO-9660 文件系统增强**
  新增支持加载字符转换例程的挂钩功能。**sysutils/cd9660_unicode** Port 中包含一组常用的字符转换例程。

- **新增 NFS 哈希函数**
  实现了基于 Fowler/Noll/Vo 哈希算法的新 NFS 哈希函数，通过提高 nfsnode 哈希表的效率来优化 NFS 性能。
...
**cd(4)** 驱动现在支持写操作。这使得可以对 DVD-RAM、PD 以及类似的设备进行写入，这些设备通常被识别为 CD 设备。注意，此变更仅适用于随机访问可写设备，而不影响仅支持顺序写入的设备（例如 CD-R 驱动器，这类设备由 **cdrecord(1)** 支持）。

### “make buildkernel”流程变更

- 构建内核时，配置名称现在从变量 `KERNCONF` 获取（`KERNEL` 仍然可用，但不推荐使用）。
...
3. **ESS Maestro-3/Allegro 驱动新增**
   新增对 ESS Maestro-3/Allegro 的驱动支持，但由于许可限制，该驱动无法编译进内核。要使用此驱动，请在 **/boot/loader.conf** 文件中添加以下行：
...
6. **其他驱动更新**
   ...
   - **ipfw(8)**：新增 `me` 功能，支持在 IP 地址动态变更的接口上匹配数据包。
...
1. **配置灵活性提升**
   **ata(4)** 驱动的各种选项现在成为引导加载程序可调选项，而非内核配置时的选项。
...
2. **主板支持**
   ...
   - 验证了 API CS20 1U 高度服务器的正确运行。

3. **新增系统支持**
   - 增加对 AlphaServer 2100A (“Lynx”) 的支持。

4. **引导问题和解决方案**
   - AlphaServer 4000 和 4100 无法从 FreeBSD 安装软盘或安装 CDROM 引导。解决方法是使用 `dd` 将 2.88MB 的软盘镜像写入硬盘并从硬盘引导安装程序。一旦启动 **sysinstall(8)**，即可正常安装。
...
- 修复了入站 TCP 数据段可能被错误地视为“已建立”连接的问题（参见安全公告 FreeBSD-SA-01:08）。

#### crontab(8) 修复

- 修复了允许用户读取系统中任何文件（只要是有效的 **crontab(5)** 语法）的漏洞（参见安全公告 FreeBSD-SA-01:09）。

#### inetd(8) 漏洞修复

- 修复了可能允许读取 wheel 用户可访问文件前 16 字节内容的漏洞（参见安全公告 FreeBSD-SA-01:11）。
...
- 新增代码来防止（而不是仅仅通过连接限制来缓解）一种攻击，该攻击可通过在检测到 RSA 失败时重新生成服务器密钥来猜测服务器密钥（不是主机密钥）（参见安全公告 FreeBSD-SA-01:24）。

#### ICMP 修复

- 修复了可能使攻击者中断 TCP 和 UDP 的“会话”的漏洞。
...
- 修复了 **ntpd(8)** 中可被远程利用的漏洞（参见安全公告 FreeBSD-SA-01:31）。
...
- **ftpd(8)** 现在使用此功能（参见安全公告 FreeBSD-SA-01:33）。
...
- **Binutils**：升级至 2.10.1。
- **OpenSSL**：升级至 0.9.6，并新增对机器相关的 ASM 优化支持，通过新的 `CPUTYPE/MACHINE_CPU` Makefile 变量激活。
- **file(1)**：版本更新至 3.33，转换为 contrib 模式引入。
- **groff(1)** 及其相关工具升级至 FSF 版本 1.16.1。
...
- OpenSSH 升级到 2.3.0 版本。该版本新增了对 Rijndael 加密算法的支持。

- OpenSSH 增加了与 Kerberos 的兼容性。

- OpenSSH 进行了修改，以提高对流量分析的抵抗力，要求“非回显”字符仍然在空包中回显，同时对发送的密码进行填充，以防泄露密码长度的信息。

- **syslogd(8)**：新增了“LOG_CONSOLE”设施（默认关闭），可以用于记录 **/dev/console** 的输出。
- **cdcontrol(1)**：现在通过 `CDROM` 环境变量选择默认设备。
- **软件包与 Ports**：所有软件包和 Ports 现在包含一个“origin”指令，使得像 **pkg_version(1)** 这样的程序更容易确定软件包的构建目录。

- **pkg_info(1)**：现在可以接受参数 `-g`，用于验证已安装软件包与其记录的校验和是否一致（检查是否在安装后被修改）。此机制的安全性仅依赖于 **/var/db/pkg** 的内容，如果用于审计目的，安全性也仅有此保障。

- **pkg_create(1)** 和 **pkg_add(1)**：现在可以处理使用 **bzip2(1)** 压缩的软件包。**pkg_add(1)** 将使用 `PACKAGEROOT` 环境变量来确定新软件包的镜像站点。

- **pkg_info(1)**：现在支持对已安装软件包名进行通配符匹配。选项 `-G` 禁用此行为，选项 `-x` 启用正则表达式匹配，代替 shell 通配符匹配。

- **pkg_sign(1)** 和 **pkg_check(1)**：新增功能，可以为二进制软件包文件进行数字签名和验证签名。

- **pkg_update(1)**：新增工具，用于更新已安装的软件包及其依赖关系。

- **pkg_delete(1)**：现在可以执行软件包名的通配符/正则表达式匹配。此外，还新增了选项 `-a`，用于删除所有软件包，以及选项 `-i`，提供类似 **rm(1)** 风格的交互式确认。

- **pkg_create(1)**：现在以依赖关系的顺序记录依赖项，而不是按命令行指定的顺序。这改进了 `pkg_add -r` 的功能。

- **pkg_version(1)**：现在包含一个版本号比较程序，符合《Porters Handbook》的规范。同时新增了选项 `-t`，用于测试地址比较。

- **awk**：从 gawk-3.0.4 升级到 gawk-3.0.6，修复了多个非关键性 bug，并进行了性能优化。

- **fsck(8)**：在接收到 `SIGINFO` 信号（通常通过控制终端的 Control-T）后，会显示当前阶段号及相关进度信息。
...
- **config(8)**：现在更好地将本应为错误的警告转换为实际的致命错误，并带有退出代码。这样可以确保 `make buildkernel` 不会在没有人工查阅错误的情况下悄悄忽略错误并生成无效内核。

- 在 **/etc/ssh/sshd_config** 中，参数 `ConnectionsPerPeriod` 弃用，取而代之的是参数 `MaxStartups`。

- **find(1)** 现在支持参数 `-empty`，用于检查文件或目录是否为空，如果为空则返回 true。

- **find(1)** 现在支持 `-iname` 和 `-ipath` 主选项，用于进行不区分大小写的匹配；同时支持 `-regexp` 和 `-iregexp` 主选项，用于正则表达式匹配。参数 `-E` 现在启用扩展正则表达式。

- **ldconfig(8)** 现在会检查目录的所有权和权限，以提高安全性；可以使用参数 `-i` 禁用这些检查。

- **sendmail(8)** 及相关工具从版本 8.11.1 升级到 8.11.3。更多信息请参见 **/usr/src/contrib/sendmail/RELEASE_NOTES**。

- 新增 **make.conf** 配置选项：`SENDMAIL_MC` 和 `SENDMAIL_ADDITIONAL_MC`。更多信息请参见 **/etc/defaults/make.conf**。

- **/etc/mail** 中的 Makefile 现在支持：新的 `SENDMAIL_MC` 配置选项；从 `.mc` 文件构建 `.cf` 文件；通用的映射重建；重建别名文件；以及停止、启动和重启 sendmail。

- **vidcontrol(1)** 现在接受参数 `-g`，用于选择 VESA_800x600 光栅文本模式中的自定义文本几何形状。

- 新增 **rfork_thread(3)** 库调用，作为 **rfork(2)** 的辅助函数。使用此函数可以避免实现复杂的栈切换代码。

- **compat3x** 分发版更新，包含 FreeBSD 3.5.1-RELEASE 中的库。

- **gperf** 更新至 2.7.2。

- 与基本系统中其他大多数网络工具保持一致，**lpr(1)**、**lpd(8)**、**syslogd(8)** 和 **logger(1)** 现在均支持 IPv6。

- 当请求删除多个软件包时，**pkg_delete(1)** 现在将尝试按依赖关系顺序删除软件包，而不是按命令行中指定的顺序。

- **burncd(8)** 现在支持 `-m` 选项，进入多会话模式（默认行为是将光盘关闭为单会话）。还新增了 `-l` 选项，用于从文件名中获取映像文件列表；可以使用 `-` 作为标准输入的文件名。

- **tar(1)** 现在支持变量 `TAR_RSH`，主要用于启用将 **ssh(1)** 用作传输工具。

- 修复了 **make(1)** 中的多个 bug，包括空后缀行为错误、当前目录权限假设不当和潜在的缓冲区溢出问题。

- 新的 `CPUTYPE` **make.conf** 变量可以控制在编译过程中应用处理器特定的优化，适用于 OpenSSL 等代码。

- 启动时的 **syscons** 配置移至机器无关的 **rc.syscons**。
...
- 从 NetBSD 导入 **lastlogin(1)** 工具（打印每个用户的最后登录时间）。

- **/usr/src/share/examples/BSD_daemon/** 现在包含一个可伸缩的 Beastie 图形。

- **bc** 从 1.04 升级至 1.06。

- **savecore(8)** 现在支持选项 `-k`，用于在保存崩溃转储后防止崩溃转储被清除。它还会尽量避免将大量零写入崩溃转储文件，以节省空间和时间。

- **tcsh** 更新到 6.10 版本。

- 变量 `CVS_RSH` 的默认值（由 **cvs(1)** 使用）现在是 ssh，而不是 rsh。

- **disklabel(8)** 现在支持以千字节、兆字节或吉字节表示的分区大小，除了以扇区为单位的表示方式。

- **Kerberos IV** 更新至 1.0.5 版本。

- **Heimdal** 更新至 0.3e 版本。

- **dump(8)** 现在支持标志 `nodump` 沿层级结构向下继承。

- **ISC DHCP 客户端** 更新至 2.0pl5 版本。
...
- 许多手册页有所改进，改善了格式标记和内容。

- `lprm -` 现在可以用于远程打印机队列。

- **mergemaster(8)** 现在支持从 **/etc/mergemaster.rc** 文件加载配置，并在需要时提示用户运行推荐的命令（如 `newaliases`）。

- **ftpd(8)** 现在支持参数 `-r` 用于只读模式，以及参数 `-E` 来禁用 EPSV。它还进行了一些修复，减少信息泄漏，并增加了指定编译时端口范围的能力。

- **rc(8)** 现在有一个框架来处理 **rc.conf(5)** 变量之间的依赖关系。

- libfetch 用于被动 FTP 获取的默认 TCP 端口范围变更；这影响了 **fetch(1)** 的行为，现在 **fetch(1)** 增加了选项 `-U` 来恢复旧行为。

- **portmap(8)** 现在支持 `-h` 选项，用于指定应绑定的 IP 地址。此选项可以多次指定，通常对于多宿主主机是必要的。
...
**3ware Escalade ATA RAID 控制器：** 5000、6000 和 7000 系列的所有型号均受支持。
...
[注：目前正将 UltraStor 驱动程序移植到新的 CAM SCSI 框架中，但尚未估算何时完成或是否会完成。]
...
其他 FreeBSD 软件镜像位置，请参阅文件 **MIRROR.SITES**。请尽量从距离你最近（就网络而言）的站点下载。如果你想成为官方镜像站点，欢迎随时联系我们：<freebsd-admin@FreeBSD.org>。

如果你无法访问互联网且只能通过电子邮件获取文件，你仍然可以通过向 `ftpmail@ftpmail.vix.com` 发送邮件来获取文件 —— 在邮件中加入“help”关键字，以获取更多关于如何通过这种方式获取文件的信息。然而，请注意，这将通过邮件传送大量数据（通常是几十兆字节），应该仅作为最后手段使用！
...
提交 bug 报告的首选方法是使用命令 `send-pr`，或者通过以下 CGI 脚本提交：[http://www.FreeBSD.org/send-pr.html](http://www.FreeBSD.org/send-pr.html)

我们忠实的 bugfiler 程序会妥善归档错误报告，你可以放心，我们会尽力尽快响应所有报告的错误。通过这种方式提交的错误报告也会在我们的网站支持部分可见，因此不仅作为错误报告有价值，还为其他用户关注潜在问题提供了“路标”。

如果由于某种原因你无法使用命令 `send-pr` 提交 bug 报告，可以尝试将报告发送到：`freebsd-bugs@FreeBSD.org`

请注意，`send-pr` 本身是一个 shell 脚本，应该可以很容易地移动到完全不同的系统上。我们更倾向于你使用这个接口，因为它更方便我们追踪问题报告。然而，在提交之前，请尽量确认该问题是否已经被修复。
...
此外，由于这是一个志愿者项目，我们始终欢迎有额外时间和能力提供帮助的人——我们现在所期望的改进远超过我们自己能够处理的数量！如果你有技术问题或提供帮助的意愿，请通过邮件联系：`freebsd-hackers@FreeBSD.org`
...
FreeBSD 凝聚了来自世界各地成百上千人的累积努力，这些人为了带给你这个版本，付出了巨大的努力。有关 FreeBSD 项目工作人员的完整列表，请参阅：[http://www.FreeBSD.org/handbook/staff.html](http://www.FreeBSD.org/handbook/staff.html)
