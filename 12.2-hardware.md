# FreeBSD 12.2-RELEASE 硬件兼容列表

## 引言

本文档包含 FreeBSD 12.2-RELEASE 的硬件兼容性说明。其中列出了 FreeBSD 支持的硬件平台，以及各类硬件设备（存储控制器、网络接口等），并给出已知可正常工作的设备实例。
...
FreeBSD 通常支持对称多处理器（SMP）系统，但在某些情况下，BIOS 或主板缺陷可能导致问题。查阅 A.SMP 的归档可能获得一些线索。
...
FreeBSD 通常可在基于 i386 的笔记本电脑上运行，但对某些硬件特性（如声音、图形、电源管理和 PCCARD 扩展槽）的支持程度因机器而异。这些特性在不同机器上的表现差异较大，通常需要在 FreeBSD 中进行特殊处理以规避硬件缺陷或其他异常。
...
ACPI 依赖于每台机器 BIOS 提供的 Differentiated System Descriptor Table（DSDT）。部分机器的 DSDT 有缺陷或不完整，导致 ACPI 无法正常工作。部分机器的替换 DSDT 可在 http://acpi.sourceforge.net/[ACPI4Linux] 项目的 http://acpi.sourceforge.net/dsdt/index.php[DSDT] 部分找到。FreeBSD 可使用这些 DSDT 覆盖 BIOS 提供的 DSDT；更多信息请参阅 [acpi(4)](https://man.freebsd.org/cgi/man.cgi?query=acpi&sektion=4&format=html) 手册页。
...
使用 `GENERIC` 内核时，未配备 [creator(4)](https://man.freebsd.org/cgi/man.cgi?query=creator&sektion=4&format=html)（Sun Creator、Sun Creator3D 和 Sun Elite3D）或 [ffb(4)](https://man.freebsd.org/cgi/man.cgi?query=ffb&sektion=4&format=html)（Sun PGX 和 Sun PGX64，以及例如 Sun Blade(TM) 100、Sun Blade(TM) 150、Sun Ultra(TM) 5 和 Sun Ultra(TM) 10 板载的 ATI Mach64 芯片）驱动所支持帧缓冲的 FreeBSD/sparc64 系统必须使用串行控制台。

如果你有不在此列表中的系统，它可能尚未经过 FreeBSD 12.2-RELEASE 测试。我们鼓励你尝试并致信 A.SPARC 反馈结果，包括哪些设备可用、哪些不可用。
...
[amd64, i386, sparc64] IDE/ATA controllers ([ata(4)](https://man.freebsd.org/cgi/man.cgi?query=ata&sektion=4&format=html) driver)
...
[hptiop(4)](https://man.freebsd.org/cgi/man.cgi?query=hptiop&sektion=4&format=html) 驱动还支持以下 SAS and SATA RAID controllers that are already End-of-Life：
...
[isci(4)](https://man.freebsd.org/cgi/man.cgi?query=isci&sektion=4&format=html) 驱动为 Intel C600 SAS controllers 提供支持。
...
所有主要固件版本（2.x、3.x、4.x 和 5.x）均受支持，但始终建议升级到该控制器可用的最新固件。

未列出的兼容 Mylex 控制器应当也能工作，但尚未经过验证。
...
[mpt(4)](https://man.freebsd.org/cgi/man.cgi?query=mpt&sektion=4&format=html) 驱动支持的 Ultra 320 SCSI 控制器芯片可在许多系统的板载中出现，包括：

* Dell PowerEdge 1750 thru 2850
* IBM eServer xSeries 335

这些系统还包含 Integrated RAID Mirroring 和 Integrated RAID Mirroring Enhanced，该驱动同样支持。

SAS 控制器芯片也出现在许多基于 AMD/Opteron 的新系统上，例如 Sun 4100。注意，此控制器可同时驱动 SAS 和 SATA 硬盘，或同时混合使用两者。这些控制器可用的 Integrated RAID Mirroring 充其量只有有限的支持。

光纤通道控制器芯片组在多种速率和系统中受支持。Apple 光纤通道 HBA 实际上就是 FC949ES 卡。

此驱动还支持光纤通道卡的目标模式。可通过 LSI Logic 固件工具设置核心的目标角色来启用此支持，该工具用于设定卡可承担的角色——无需单独编译。
...
注意，此硬件仅对 ATA DMA 命令支持命令排队和基于 FIS 的交换。ATAPI 和非 DMA ATA 命令按每个端口逐一执行。

[ncr(4)](https://man.freebsd.org/cgi/man.cgi?query=ncr&sektion=4&format=html) 驱动支持以下 NCR/Symbios SCSI 控制器芯片：
...
[sym(4)](https://man.freebsd.org/cgi/man.cgi?query=sym&sektion=4&format=html) 支持的 SCSI 控制器可以板载于主板上，或位于以下附加板卡之一：
...
对于所有受支持的 SCSI 控制器，均完整支持 SCSI-I、SCSI-II 和 SCSI-III 外围设备，包括硬盘、光盘、磁带机（包括 DAT、8mm Exabyte、Mammoth 和 DLT）、介质更换器、处理器目标设备和 CD-ROM 驱动器。支持 CD-ROM 命令的 WORM 设备可由 CD-ROM 驱动（如 [cd(4)](https://man.freebsd.org/cgi/man.cgi?query=cd&sektion=4&format=html)）进行只读访问。WORM/CD-R/CD-RW 写入支持由 [cdrecord(1)](https://man.freebsd.org/cgi/man.cgi?query=cdrecord&sektion=1&format=html) 提供，该程序是 Ports Collection 中 sysutils/cdrtools port 的一部分。

目前已支持以下类型的 CD-ROM 系统：
...
[bce(4)](https://man.freebsd.org/cgi/man.cgi?query=bce&sektion=4&format=html) 驱动支持基于 QLogic NetXtreme II 系列千兆以太网控制器的多种 NIC，包括以下：
...
[bge(4)](https://man.freebsd.org/cgi/man.cgi?query=bge&sektion=4&format=html) 驱动支持基于 Broadcom BCM570x 系列千兆以太网控制器芯片的多种 NIC，包括以下：
...
[bnxt(4)](https://man.freebsd.org/cgi/man.cgi?query=bnxt&sektion=4&format=html) 驱动支持基于 Broadcom NetXtreme-C 和 NetXtreme-E 系列千兆以太网控制器芯片的多种 NIC，包括以下：
...
[bxe(4)](https://man.freebsd.org/cgi/man.cgi?query=bxe&sektion=4&format=html) 驱动支持基于 QLogic NetXtreme II 系列 10Gb 以太网控制器芯片的多种 NIC，包括以下：
...
[ed(4)](https://man.freebsd.org/cgi/man.cgi?query=ed&sektion=4&format=html) 驱动支持以下以太网网卡：
...
支持 ISA、PCI 和 PC Card 设备。

[ed(4)](https://man.freebsd.org/cgi/man.cgi?query=ed&sektion=4&format=html) 驱动不支持以下 Ethernet NICs：
...
Intel Ethernet 700 系列中配备 SFP+/SFP28/QSFP+ 接口的大多数适配器，其固件要求使用 Intel 认证模块；这些认证模块列表如下。驱动无法禁用此认证检查。
...
[msk(4)](https://man.freebsd.org/cgi/man.cgi?query=msk&sektion=4&format=html) 驱动支持基于 Marvell/SysKonnect Yukon II 千兆以太网控制器芯片的多种 NIC，包括：
...
[my(4)](https://man.freebsd.org/cgi/man.cgi?query=my&sektion=4&format=html) 驱动支持基于 Myson 芯片组的多种 NIC。受支持的型号包括：
...
[pcn(4)](https://man.freebsd.org/cgi/man.cgi?query=pcn&sektion=4&format=html) 驱动支持基于 AMD PCnet/FAST、PCnet/FAST+、PCnet/FAST III、PCnet/PRO 和 PCnet/Home 快速以太网芯片的适配器和嵌入式控制器：
...
[sis(4)](https://man.freebsd.org/cgi/man.cgi?query=sis&sektion=4&format=html) 驱动支持基于 Silicon Integrated Systems SiS 900 和 SiS 7016 的快速以太网适配器和嵌入式控制器，以及基于 National Semiconductor DP83815（MacPhyter）和 DP83816 芯片的快速以太网适配器。受支持的适配器包括：
...
[sn(4)](https://man.freebsd.org/cgi/man.cgi?query=sn&sektion=4&format=html) 驱动支持基于 SMC91Cxx 的 ISA 和 PCMCIA 卡，包括：
...
Farallon EtherWave 和 EtherMac 卡有两种变体。[ep(4)](https://man.freebsd.org/cgi/man.cgi?query=ep&sektion=4&format=html) 驱动支持 595 和 895 卡。这些卡正面有蓝色箭头和 3Com 标志。正面有红色箭头的 Farallon 595a 卡也称为 EtherWave 和 EtherMac。它们由 [sn(4)](https://man.freebsd.org/cgi/man.cgi?query=sn&sektion=4&format=html) 驱动支持。
...
[stge(4)](https://man.freebsd.org/cgi/man.cgi?query=stge&sektion=4&format=html) 驱动支持基于 Sundance/Tamarack TC9021 千兆以太网控制器芯片的多种 NIC，包括：
...
[ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动支持基于 Alteon Tigon I 和 II 芯片的千兆以太网适配器。[ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动已用以下适配器测试通过：
...
[tl(4)](https://man.freebsd.org/cgi/man.cgi?query=tl&sektion=4&format=html) 驱动支持基于 Texas Instruments ThunderLAN 的以太网和快速以太网适配器，包括大量 Compaq PCI 以太网适配器。此外还支持：
...
[tl(4)](https://man.freebsd.org/cgi/man.cgi?query=tl&sektion=4&format=html) 驱动还支持多种 Compaq Prosignia 服务器和 Compaq Deskpro 台式机内置的以太网适配器，包括：
...
[amd64, i386] SMC 83c17x (EPIC)-based Ethernet NICs ([tx(4)](https://man.freebsd.org/cgi/man.cgi?query=tx&sektion=4&format=html) driver)
...
[vx(4)](https://man.freebsd.org/cgi/man.cgi?query=vx&sektion=4&format=html) 驱动支持以下卡：

* 3Com 3c590 EtherLink III PCI
* 3Com 3c595 Fast EtherLink III PCI in 10 Mbps mode

[wb(4)](https://man.freebsd.org/cgi/man.cgi?query=wb&sektion=4&format=html) 驱动支持基于 Winbond W89C840F 的快速以太网适配器和嵌入式控制器，包括：

* Trendware TE100-PCIE

[xe(4)](https://man.freebsd.org/cgi/man.cgi?query=xe&sektion=4&format=html) 驱动支持以下卡：
...
3C656 系列 CardBus 卡和 3C556 系列 MiniPCI 卡均内置专有调制解调器。[xl(4)](https://man.freebsd.org/cgi/man.cgi?query=xl&sektion=4&format=html) 驱动和其他任何 FreeBSD 驱动均不支持此调制解调器。
...
[amd64, i386] Cisco/Aironet 802.11b wireless adapters (4 [an(4)](https://man.freebsd.org/cgi/man.cgi?query=an&sektion=4&format=html) driver)

[ath(4)](https://man.freebsd.org/cgi/man.cgi?query=ath&sektion=4&format=html) 驱动支持所有 Atheros Cardbus 和 PCI 卡，但不包括基于 AR5005VL 芯片组的卡。
...
[bwi(4)](https://man.freebsd.org/cgi/man.cgi?query=bwi&sektion=4&format=html) 驱动使用较旧的 v3 版本 Broadcom 固件。虽然此旧版固件支持大多数 BCM43xx 部件，但 [bwn(4)](https://man.freebsd.org/cgi/man.cgi?query=bwn&sektion=4&format=html) 驱动对它所支持的较新芯片工作得更好。如果使用较旧的 Broadcom 芯片组（BCM4301、BCM4303 和 BCM4306 rev 2），必须使用 [bwi(4)](https://man.freebsd.org/cgi/man.cgi?query=bwi&sektion=4&format=html) 驱动。[bwn(4)](https://man.freebsd.org/cgi/man.cgi?query=bwn&sektion=4&format=html) 使用的 v4 版本固件不支持这些芯片。
...
[otus(4)](https://man.freebsd.org/cgi/man.cgi?query=otus&sektion=4&format=html) 驱动支持 Atheros AR9170 USB IEEE 802.11b/g/n 无线网络适配器，包括：
...
[rsu(4)](https://man.freebsd.org/cgi/man.cgi?query=rsu&sektion=4&format=html) 驱动支持 Realtek RTL8188SU/RTL8192SU USB IEEE 802.11b/g/n 无线网络适配器，包括：
...
Realtek RTL8192C, RTL8188E, RTL8812A and RTL8821A based PCIe IEEE 802.11b/g/n wireless network adapters (MAN.RTWN.4 [rtwn(4)](https://man.freebsd.org/cgi/man.cgi?query=rtwn&sektion=4&format=html) driver)
...
[uath(4)](https://man.freebsd.org/cgi/man.cgi?query=uath&sektion=4&format=html) 驱动应能在以下适配器上工作：
...
[amd64, i386] Lucent Technologies WaveLAN/IEEE 802.11b wireless network adapters and workalikes using the Lucent Hermes, Intersil PRISM-II, Intersil PRISM-2.5, Intersil Prism-3, and Symbol Spectrum24 chipsets ([wi(4)](https://man.freebsd.org/cgi/man.cgi?query=wi&sektion=4&format=html) driver)
...
[ce(4)](https://man.freebsd.org/cgi/man.cgi?query=ce&sektion=4&format=html) 驱动支持以下 Tau-PCI/32 WAN 适配器型号：

* Cronyx Tau-PCI/32
* Cronyx Tau-PCI/32-Lite

[cx(4)](https://man.freebsd.org/cgi/man.cgi?query=cx&sektion=4&format=html) 驱动支持以下卡：
...
[cp(4)](https://man.freebsd.org/cgi/man.cgi?query=cp&sektion=4&format=html) 驱动支持以下 Tau-PCI WAN 适配器型号：
...
[ctau(4)](https://man.freebsd.org/cgi/man.cgi?query=ctau&sektion=4&format=html) 驱动支持以下卡：
...
[uart(4)](https://man.freebsd.org/cgi/man.cgi?query=uart&sektion=4&format=html) 驱动支持以下 UART 类：
...
[scc(4)](https://man.freebsd.org/cgi/man.cgi?query=scc&sektion=4&format=html) 驱动支持以下 SCC 类：
...
[rc(4)](https://man.freebsd.org/cgi/man.cgi?query=rc&sektion=4&format=html) 驱动为 SDL Communications RISCom/8 板卡提供支持。
...
[snd_audiocs(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_audiocs&sektion=4&format=html) 驱动支持以下音频设备：
...
部分板载 CS4610 芯片搭配的是 CS423x ISA 编解码器，而非 CS4297 AC97 编解码器。此类配置目前尚不被 snd_csa 驱动支持。
...
[snd_emu10kx(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_emu10kx&sektion=4&format=html) 驱动不支持以下 sound cards (although they have names similar to some supported ones)：
...
[snd_hda(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hda&sektion=4&format=html) 驱动支持具有 PCI 类 4（多媒体）和子类 3（HDA）、兼容 Intel HDA 规范的控制器。

[snd_hda(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hda&sektion=4&format=html) 驱动支持两百多种不同的控制器和 CODEC。此处没有必要全部列出，因为在大多数情况下，特定的 CODEC 配置和接线比 CODEC 本身的类型更重要。
...
注意，采用 ES18xx 芯片组的旧款 ESS ISA 声卡由 [snd_ess(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_ess&sektion=4&format=html) 和/或 [snd_sbc(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_sbc&sektion=4&format=html) 支持。
...
[bktr(4)](https://man.freebsd.org/cgi/man.cgi?query=bktr&sektion=4&format=html) 驱动支持基于 Brooktree Bt848/849/878/879 芯片的视频采集卡，以及 Pinnacle PCTV 卡，包括：
...
[amd64, i386] USB 2.0 controllers using the EHCI interface ([ehci(4)](https://man.freebsd.org/cgi/man.cgi?query=ehci&sektion=4&format=html) driver)

[amd64, i386] Hubs

[amd64, i386] Keyboards ([ukbd(4)](https://man.freebsd.org/cgi/man.cgi?query=ukbd&sektion=4&format=html) driver)
...
[amd64, i386] Mice ([ums(4)](https://man.freebsd.org/cgi/man.cgi?query=ums&sektion=4&format=html) driver)

[ulpt(4)](https://man.freebsd.org/cgi/man.cgi?query=ulpt&sektion=4&format=html) 驱动支持 USB 打印机和并口打印机转换电缆，包括以下：
...
[amd64, i386] Audio Devices ([uaudio(4)](https://man.freebsd.org/cgi/man.cgi?query=uaudio&sektion=4&format=html) driver)
...
[fwohci(4)](https://man.freebsd.org/cgi/man.cgi?query=fwohci&sektion=4&format=html) 驱动支持 PCI/CardBus FireWire 接口卡。该驱动支持以下 IEEE 1394 OHCI 芯片组：
...
[ng_bt3c(4)](https://man.freebsd.org/cgi/man.cgi?query=ng_bt3c&sektion=4&format=html) 驱动为 3Com/HP 3CRWB6096-A PCCARD 蓝牙适配器提供支持。
...
[hifn(4)](https://man.freebsd.org/cgi/man.cgi?query=hifn&sektion=4&format=html) 驱动支持包含 Hifn 7751、7951、7811、7955 和 7956 芯片组的多种卡，例如：
...
[safe(4)](https://man.freebsd.org/cgi/man.cgi?query=safe&sektion=4&format=html) 驱动支持包含以下任意芯片的卡：

* SafeNet 1141
* SafeNet 1741

[ubsec(4)](https://man.freebsd.org/cgi/man.cgi?query=ubsec&sektion=4&format=html) 驱动支持包含以下任意芯片的卡：
...
[amd64, i386] Floppy drives ([fdc(4)](https://man.freebsd.org/cgi/man.cgi?query=fdc&sektion=4&format=html) driver)
...
* [amd64, i386] USB keyboards ([ukbd(4)](https://man.freebsd.org/cgi/man.cgi?query=ukbd&sektion=4&format=html) driver)

[amd64, i386] Pointing devices including:

* [amd64, i386] Bus mice and compatible devices ([mse(4)](https://man.freebsd.org/cgi/man.cgi?query=mse&sektion=4&format=html) driver)
...
* [amd64, i386] USB mice ([ums(4)](https://man.freebsd.org/cgi/man.cgi?query=ums&sektion=4&format=html) driver)

> **注意**：
>
> [moused(8)](https://man.freebsd.org/cgi/man.cgi?query=moused&sektion=8&format=html) has more information on using pointing devices with FreeBSD. Information on using pointing devices with Xorg can be found at http://www.x.org/.
