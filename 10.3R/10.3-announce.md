...
* **ifconfig(8)** 工具现在在指定参数 `-v` 且 NIC 驱动提供此类信息时报告 SFP/SFP+ 光模块数据，即 **cxgbe(4)**、**ixgbe(4)**、**mlx5en(4)** 和 **sfxge(4)**。
* **jail(8)** 工具更新，新增参数 `-l`，可确保在使用时目标 Jail 环境干净。此外，**jail(8)** 在未指定命令时默认会在目标 Jail 内运行一个 shell。
