...
* Linux(R) 兼容层经过大幅改进，现在能够在 amd64（x86_64）上运行 64 位应用程序，支持 1:1 线程、VDSO 以及足以适用于大多数程序的 `epoll(7)` 系列子集。
* `em(4)` 和 `igb(4)` 驱动分别更新至版本 7.6.1 和 2.5.3。其中带来了对 Intel(R) Skylake 代及更新芯片组中 i219/i219(2)/i219(3) 硬件的支持。
* `isp(4)` 驱动更新和改进：现在还支持 16 Gbps Fibre Channel 适配器，改进了目标模式支持并完成了多 ID（NPIV）功能。
* `ixgbe(4)` 驱动更新至 Intel(R) FreeBSD Networking Group 版本 3.1.13-k，并新增对 X552 和 X550T 的支持。
...
要将 FreeBSD/arm 镜像写入 SD 卡，请使用 `dd(1)` 工具，将 _KERNEL_ 替换为系统对应的内核配置名称。
