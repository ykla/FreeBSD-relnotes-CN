...
本发行说明所适用的快照分发表示 10.3-STABLE 开发分支上位于 10.2-RELEASE 与未来的 10.4-RELEASE 之间的一个点。有关沿此分支的预构建二进制快照分发信息，可在 [https://www.FreeBSD.org/releases/](https://www.freebsd.org/releases/) 上找到。
...
[amd64, i386] RELEASE 版本之间的二进制升级（以及各种安全分支的快照）可以使用 [freebsd-update(8)](http://www.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8) 工具进行。二进制升级过程将更新未修改的用户空间工具，以及作为官方 FreeBSD 发布一部分分发的未修改的 **GENERIC** 内核。升级的主机需要具备互联网连接才能使用 [freebsd-update(8)](http://www.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8) 工具。
...
[ar(1)](http://www.freebsd.org/cgi/man.cgi?query=ar&sektion=1) 工具现在支持参数 `-D`，以防止插入真实的 mtime、uid、gid 和文件模式值。这被称为“确定性模式”，有助于使生成的归档文件可重现。此行为默认为启用，可以通过指定参数 `-U` 来禁用。[（r287326,288202）](http://svn.freebsd.org/viewvc/base?view=revision&revision=287326,288202)（由 FreeBSD 基金会赞助）

[camcontrol(8)](http://www.freebsd.org/cgi/man.cgi?query=camcontrol&sektion=8) 的 `fwdownload` 子命令改进：变更包括对 SATA 驱动器的更好支持，向 IBM LTO 驱动器下载固件，参数 `-q` 用于抑制信息输出，以及 `opcodes` 子命令用于执行 `REPORT SUPPORTED OPCODES` 服务操作。[（r286965）](http://svn.freebsd.org/viewvc/base?view=revision&revision=286965)

[cp(1)](http://www.freebsd.org/cgi/man.cgi?query=cp&sektion=1) 工具已更新，新增了参数 `-s`，用于创建指向指定源的符号链接。[（r291774）](http://svn.freebsd.org/viewvc/base?view=revision&revision=291774)
...
[ifconfig(8)](http://www.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8) 工具现在在指定参数 `-v` 并且 NIC 驱动程序提供时，会报告 SFP/SFP+ 数据。[（r286810）](http://svn.freebsd.org/viewvc/base?view=revision&revision=286810)（由 Yandex LLC 赞助）

修复了 [inetd(8)](http://www.freebsd.org/cgi/man.cgi?query=inetd&sektion=8) 守护进程中的若干错误，这些错误会在定义了 RPC 条目并且在参数 `-a` 中指定了 IPv6 地址时导致崩溃。[（r288048）](http://svn.freebsd.org/viewvc/base?view=revision&revision=288048)

[jail(8)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8) 工具更新，新增参数 `-l`，可确保在使用时目标 Jail 环境干净。此外，[jail(8)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8) 在未指定命令时会在目标 Jail 内启动一个 shell。[（r286064）](http://svn.freebsd.org/viewvc/base?view=revision&revision=286064)

[last(1)](http://www.freebsd.org/cgi/man.cgi?query=last&sektion=1) 工具现在支持 `reboot` 作为伪用户名，打印所有系统重启条目（`SHUTDOWN_TIME` 和 `BOOT_TIME` 记录）。此功能在 FreeBSD 9.0 中被错误删除。[（r286952）](http://svn.freebsd.org/viewvc/base?view=revision&revision=286952)
...
[pciconf(8)](http://www.freebsd.org/cgi/man.cgi?query=pciconf&sektion=8) 工具已更新，在存在时将使用来自 `misc/pciids` 软件包的 PCI ID 数据库，若不存在则回退到 FreeBSD 基本系统中的 PCI ID 数据库。[（r287746）](http://svn.freebsd.org/viewvc/base?view=revision&revision=287746)
...
在 [nfsuserd(8)](http://www.freebsd.org/cgi/man.cgi?query=nfsuserd&sektion=8) 中新增了对参数 `-manage-gids` 的支持。此选项可以通过设置 [rc.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5) 变量 `nfs_server_managegids` 为 `YES` 在启动时启用。[（r292231）](http://svn.freebsd.org/viewvc/base?view=revision&revision=292231)

[pkill(1)](http://www.freebsd.org/cgi/man.cgi?query=pkill&sektion=1) 工具现在在选项 `-j` 中除支持 [jail(2)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=2) ID 外，也支持 [jail(2)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=2) 名称。[（r287269）](http://svn.freebsd.org/viewvc/base?view=revision&revision=287269)

[pw(8)](http://www.freebsd.org/cgi/man.cgi?query=pw&sektion=8) 工具的子命令 `userdel` 和 `usermod` 现在支持参数 `-y`。[（r287084）](http://svn.freebsd.org/viewvc/base?view=revision&revision=287084)

解析库已更新，如果 **/etc/resolv.conf** 的修改时间发生变化，则会重新加载该文件。[（r292462）](http://svn.freebsd.org/viewvc/base?view=revision&revision=292462)（由 Dell, Inc. 赞助）

新增“reroot”支持的初步实现，添加到 [reboot(8)](http://www.freebsd.org/cgi/man.cgi?query=reboot&sektion=8) 工具中，允许从临时源文件系统挂载根文件系统，而无需完全重启系统。[（r293744）](http://svn.freebsd.org/viewvc/base?view=revision&revision=293744)（由 FreeBSD 基金会赞助）
...
[file(1)](http://www.freebsd.org/cgi/man.cgi?query=file&sektion=1) 工具已更新至版本 5.25。[(r290152)](http://svn.freebsd.org/viewvc/base?view=revision&revision=290152)
...
[bsdinstall(8)](http://www.freebsd.org/cgi/man.cgi?query=bsdinstall&sektion=8) 工具已更新，支持在基于 EFI 的系统上安装 ZFS。[(r295264)](http://svn.freebsd.org/viewvc/base?view=revision&revision=295264)（由 ScaleEngine, Inc. 赞助）

### **/etc/rc.d** 脚本

`rc.d/netwait` 脚本已更新，能够等待在启动过程中较晚附加的网络接口，如某些 USB 网卡。[(r294680)](http://svn.freebsd.org/viewvc/base?view=revision&revision=294680)

使用 `firewall_type="SIMPLE"` 设置的防火墙规则，现在使用 [ipfw(4)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=4) `tables` 来阻止要拦截的地址。[(r287091)](http://svn.freebsd.org/viewvc/base?view=revision&revision=287091)
...
新增函数 `thread_create()` 作为 API，用于在内核空间中创建用户空间线程。[(r286843)](http://svn.freebsd.org/viewvc/base?view=revision&revision=286843)

### 内核错误修复

[kqueue(2)](http://www.freebsd.org/cgi/man.cgi?query=kqueue&sektion=2) 系统调用已更新，以处理写入大于 2GB 的文件的事件。[(r288167)](http://svn.freebsd.org/viewvc/base?view=revision&revision=288167)（由 Multiplay 赞助）

### 内核配置

[amd64, i386] 已从 **GENERIC** 内核中移除 [pms(4)](http://www.freebsd.org/cgi/man.cgi?query=pms&sektion=4) 驱动。[(r286132)](http://svn.freebsd.org/viewvc/base?view=revision&revision=286132)（由 FreeBSD 基金会赞助）
...
已修复一个可能导致加载器可调变量 `kern.racct.enable` 无法正常工作的错误。[(r286322)](http://svn.freebsd.org/viewvc/base?view=revision&revision=286322)（由 FreeBSD 基金会赞助）
...
[uart(4)](http://www.freebsd.org/cgi/man.cgi?query=uart&sektion=4) 驱动现在支持通过 sysctl 变量 `dev.uart.pps_mode` 和 `dev.uart.0.pps_mode` 对 PPS 信号源进行运行时配置。`0`、`1` 和 `2` 分别对应禁用、在 CTS 线路上捕获脉冲和在 DCD 线路上捕获脉冲。默认值为 `2`。[(r287037)](http://svn.freebsd.org/viewvc/base?view=revision&revision=287037)

[uftdi(4)](http://www.freebsd.org/cgi/man.cgi?query=uftdi&sektion=4) 驱动现在支持 `UFTDIIOC_READ_EEPROM`、`UFTDIIOC_WRITE_EEPROM` 和 `UFTDIIOC_ERASE_EEPROM` [ioctl(2)](http://www.freebsd.org/cgi/man.cgi?query=ioctl&sektion=2)，用于读取/写入附加到控制器芯片的串行 EEPROM。[（r287035）](http://svn.freebsd.org/viewvc/base?view=revision&revision=287035)
...
CTL 高可用性实现已重写。[(r288732)](http://svn.freebsd.org/viewvc/base?view=revision&revision=288732)（由 iXsystems 赞助）
...
[isp(4)](http://www.freebsd.org/cgi/man.cgi?query=isp&sektion=4) 驱动更新和改进：添加对 16Gbps FC 卡的支持、改进目标模式支持、完成多 ID（NPIV）功能。（由 iXsystems 赞助）

### 网络驱动

[ixgbe(4)](http://www.freebsd.org/cgi/man.cgi?query=ixgbe&sektion=4) 驱动已更新至版本 3.1.13-k。[(r295524)](http://svn.freebsd.org/viewvc/base?view=revision&revision=295524)（由 Limelight Networks, Intel Corporation 赞助）
...
[ismt(4)](http://www.freebsd.org/cgi/man.cgi?query=ismt&sektion=4) 驱动新增，支持最新的 Intel® SMBus 2.0 控制器。[(r293675)](http://svn.freebsd.org/viewvc/base?view=revision&revision=293675)
...
[zfs(8)](http://www.freebsd.org/cgi/man.cgi?query=zfs&sektion=8) `l2arc` 代码已更新，以便在收集写入到 `l2arc` 设备的缓冲区时考虑 `ashift`。[(r287665)](http://svn.freebsd.org/viewvc/base?view=revision&revision=287665)（由 ClusterHQ 赞助）
...
EFI 实现已添加初始 ZFS 引导支持。[(r294999)](http://svn.freebsd.org/viewvc/base?view=revision&revision=294999)（由 Multiplay 赞助）

UEFI 引导加载程序已更新，以支持多个 ZFS 引导环境，例如 `sysutils/beadm` 提供的环境。[(r295475)](http://svn.freebsd.org/viewvc/base?view=revision&revision=295475)（由 ScaleEngine, Inc. 赞助）
...
[epair(4)](http://www.freebsd.org/cgi/man.cgi?query=epair&sektion=4) 虚拟以太网接口和 [lagg(4)](http://www.freebsd.org/cgi/man.cgi?query=lagg&sektion=4) 伪接口现在支持 **VIMAGE** 内核。[(r287594,287723)](http://svn.freebsd.org/viewvc/base?view=revision&revision=287594,287723)
...
[lagg(4)](http://www.freebsd.org/cgi/man.cgi?query=lagg&sektion=4) 伪接口中的 [sysctl(3)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=3) 变量 `net.link.lagg.N` 已被移除，取而代之的是每接口的 [ifconfig(8)](http://www.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8) 参数和选项。命令 `ifconfig -v` 显示它们。[(r287723)](http://svn.freebsd.org/viewvc/base?view=revision&revision=287723)

已修复 [lagg(4)](http://www.freebsd.org/cgi/man.cgi?query=lagg&sektion=4) 伪接口中的 bug，该 bug 可能导致系统 panic。[(r287723)](http://svn.freebsd.org/viewvc/base?view=revision&revision=287723)

已修复 [pf(4)](http://www.freebsd.org/cgi/man.cgi?query=pf&sektion=4) 包过滤器中的一个 bug，该 bug 可能导致没有参数 `log` 的规则记录匹配的包。[(r286125)](http://svn.freebsd.org/viewvc/base?view=revision&revision=286125)（由 Netgate 赞助）

已修复 FreeBSD IPv6 栈中的一个 bug，该 bug 在从链路级地址表中删除 L2 地址时未触发 `LLENTRY_DELETED` 事件。[(r286316)](http://svn.freebsd.org/viewvc/base?view=revision&revision=286316)

FreeBSD IPv6 栈中的过时 API `SIOCGDRLST_IN6` 和 `SIOCGPRLST_IN6` 已被移除。[(r287733)](http://svn.freebsd.org/viewvc/base?view=revision&revision=287733)
