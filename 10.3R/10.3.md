...
[camcontrol(8)](http://www.freebsd.org/cgi/man.cgi?query=camcontrol&sektion=8) 的 `fwdownload` 子命令改进：变更包括对 SATA 驱动器的更好支持，向 IBM LTO 驱动器下载固件，`-q` 参数用于抑制信息输出，以及 `opcodes` 子命令用于执行 `REPORT SUPPORTED OPCODES` 服务操作。[（r286965）](http://svn.freebsd.org/viewvc/base?view=revision&revision=286965)
...
修复了 [inetd(8)](http://www.freebsd.org/cgi/man.cgi?query=inetd&sektion=8) 守护进程中的若干错误，这些错误会在定义了 RPC 条目并且在 `-a` 参数中指定了 IPv6 地址时导致崩溃。[（r288048）](http://svn.freebsd.org/viewvc/base?view=revision&revision=288048)

[jail(8)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8) 工具更新，新增 `-l` 参数，可确保在使用时目标 Jail 环境干净。此外，[jail(8)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8) 在未指定命令时会在目标 Jail 内启动一个 shell。[（r286064）](http://svn.freebsd.org/viewvc/base?view=revision&revision=286064)
...
[pkill(1)](http://www.freebsd.org/cgi/man.cgi?query=pkill&sektion=1) 工具现在在 `-j` 选项中除支持 [jail(2)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=2) ID 外，也支持 [jail(2)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=2) 名称。[（r287269）](http://svn.freebsd.org/viewvc/base?view=revision&revision=287269)
...
已修复 [pf(4)](http://www.freebsd.org/cgi/man.cgi?query=pf&sektion=4) 包过滤器中的一个 bug，该 bug 可能导致没有 `log` 参数的规则记录匹配的包。[(r286125)](http://svn.freebsd.org/viewvc/base?view=revision&revision=286125)（由 Netgate 赞助）
