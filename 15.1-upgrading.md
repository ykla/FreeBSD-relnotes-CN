...
升级操作系统前，请备份 _所有_ 文件系统，阅读 [最新勘误](15.1-errata.md) 了解已知问题与变通方案，并阅读 [发行说明](15.1.md) 了解主要变更与不兼容性。
...
若显示包名（如 `FreeBSD-runtime-15.0`），请参阅下文的 [通过基系统包升级](#通过基系统包升级)。
...
4. **升级用户态**
   ...
   > **提示**：若从 14.4-RELEASE 升级且无法重新安装所有第三方软件，可安装 [misc/compat14x](https://www.freshports.org/misc/compat14x) package。

5. **删除旧文件**

   再次运行 [freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 删除过时的基系统文件：
...
通过打包基系统部署的系统使用 [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html) 升级。
...
3. **升级基系统**
   ...
   部分包可能需要额外的配置步骤，如运行 `service <name> setup`。
   请按提示操作。

4. **升级第三方内核模块**

   若安装了第三方内核模块，也必须随基系统一起升级：
