...
典型的发行说明项记录了自 10.3-RELEASE 发布以来发布的安全通告、新驱动程序或硬件支持、新命令或选项、主要的 bug 修复，或贡献的软件升级。它们还可能列出主要的 Port/包或发布工程实践的变更。显然，发行说明无法列出每个版本间对 FreeBSD 所做的所有变更；本文档主要集中在安全通告、用户可见的变更和主要的架构改进上。
...
本节涉及了用户空间应用程序、贡献软件和系统实用程序的变更和新增内容。
...
[jexec(8)](http://www.freebsd.org/cgi/man.cgi?query=jexec&sektion=8&manpath=freebsd-release-ports) 工具已更新，新增了一个参数 `-l`，确保在目标 jail 中使用时环境清洁。此外，[jexec(8)](http://www.freebsd.org/cgi/man.cgi?query=jexec&sektion=8&manpath=freebsd-release-ports) 在未指定命令时会在目标 jail 中运行一个 shell。 [(r285420)](http://svn.freebsd.org/viewvc/base?view=revision&revision=285420)
...
[jail(8)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8&manpath=freebsd-release-ports) 框架已更新，允许在 jail 中挂载 [linprocfs(5)](http://www.freebsd.org/cgi/man.cgi?query=linprocfs&sektion=5&manpath=freebsd-release-ports) 和 [linsysfs(5)](http://www.freebsd.org/cgi/man.cgi?query=linsysfs&sektion=5&manpath=freebsd-release-ports)。 [(r285685)](http://svn.freebsd.org/viewvc/base?view=revision&revision=285685)
...
无线网络堆栈已修改，默认不再显示物理无线设备。要查看系统上可用的无线设备，请运行 `sysctl net.wlan.devices`。 [(r287197)](http://svn.freebsd.org/viewvc/base?view=revision&revision=287197) (由 Netflix, Nginx, Inc. 赞助)

新增了一个工具 [sesutil(8)](http://www.freebsd.org/cgi/man.cgi?query=sesutil&sektion=8&manpath=freebsd-release-ports)，用于管理 [ses(4)](http://www.freebsd.org/cgi/man.cgi?query=ses&sektion=4&manpath=freebsd-release-ports)（SCSI 环境服务）设备。 [(r287473)](http://svn.freebsd.org/viewvc/base?view=revision&revision=287473) (由 Gandi.net 赞助)
...
新增了一个 [rc(8)](http://www.freebsd.org/cgi/man.cgi?query=rc&sektion=8&manpath=freebsd-release-ports) 脚本 `growfs`，如果 `/firstboot` 存在并且在 [rc.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5&manpath=freebsd-release-ports) 中启用了 `growfs_enable`，则该脚本将在启动时调整根文件系统的大小以填充设备。[(r273955)](http://svn.freebsd.org/viewvc/base?view=revision&revision=273955)

`mrouted` [rc(8)](http://www.freebsd.org/cgi/man.cgi?query=rc&sektion=8&manpath=freebsd-release-ports) 脚本已从基础系统中移除。等效的脚本可以通过 `net/mrouted` 端口获得。[(r275299)](http://svn.freebsd.org/viewvc/base?view=revision&revision=275299)
...
[readline(3)](http://www.freebsd.org/cgi/man.cgi?query=readline&sektion=3&manpath=freebsd-release-ports) 库现在在基础系统中静态链接，且不再安装共享库，这样 Ports Collection 可以使用库的现代版本。[(r268461)](http://svn.freebsd.org/viewvc/base?view=revision&revision=268461)
...
[libxo(3)](http://www.freebsd.org/cgi/man.cgi?query=libxo&sektion=3&manpath=freebsd-release-ports) 库已导入到基础系统中。[(r273562)](http://svn.freebsd.org/viewvc/base?view=revision&revision=273562)（由 Juniper Networks, Inc. 赞助）

为 Chelsio Terminator 5 基于 iWARP 的网卡添加了用户空间库，允许用户空间 RDMA 应用程序在兼容的 NIC 上运行。[(r273806)](http://svn.freebsd.org/viewvc/base?view=revision&revision=273806)（由 Chelsio Communications 赞助）
...
`libgomp` 库现在仅在从基础系统构建 GCC 时才构建。一个最新版本可以在 Ports 中作为 `devel/libiomp5-devel` 获取。[(r282973)](http://svn.freebsd.org/viewvc/base?view=revision&revision=282973)（由 FreeBSD 基金会赞助）
...
堆栈保护器已升级为“强”级别，增强了对缓冲区溢出的保护。尽管这显著提高了系统安全性，但进行了广泛的测试，以确保不会对性能或功能产生可测量的副作用。[(r288669)](http://svn.freebsd.org/viewvc/base?view=revision&revision=288669)
...
已修复 FreeBSD/powerpc 中的 [dtrace(1)](http://www.freebsd.org/cgi/man.cgi?query=dtrace&sektion=1&manpath=freebsd-release-ports) 堆栈追踪功能，使用 `trapexit()` 和 `asttrapexit()` 函数，而不是在已寻址的内核空间内进行检查。[(r271697)](http://svn.freebsd.org/viewvc/base?view=revision&revision=271697)
...
[arm] `rpi.dtb` 模块现在默认安装到 Raspberry Pi 系统的 `/boot/dtb/` 路径下。[(r278340)](http://svn.freebsd.org/viewvc/base?view=revision&revision=278340)
...
为 FreeBSD/i386 添加了 `PAE_TABLES` 内核配置选项，它指示 [pmap(9)](http://www.freebsd.org/cgi/man.cgi?query=pmap&sektion=9&manpath=freebsd-release-ports) 在保持 32 位物理地址大小的同时，使用 PAE 格式的页面表。使用此选项可以通过在现代 i386 处理器上启用“不可执行”映射来增强应用级安全性。与 `PAE` 选项不同，`PAE_TABLES` 保持与非 `PAE` 内核的内核二进制接口（KBI）兼容，允许非 `PAE` 内核模块和驱动程序与启用 `PAE_TABLES` 的内核一起工作。此外，系统限制被调整为最大 4GB 的内存，以避免内核虚拟地址空间（KVA）耗尽。[(r281495)](http://svn.freebsd.org/viewvc/base?view=revision&revision=281495)（由 FreeBSD 基金会赞助）

添加了 `SIFTR` 内核配置，允许将 [siftr(4)](http://www.freebsd.org/cgi/man.cgi?query=siftr&sektion=4&manpath=freebsd-release-ports) 静态编译到内核中。[(r282215)](http://svn.freebsd.org/viewvc/base?view=revision&revision=282215)
...
`kern.osrelease` 和 `kern.osreldate` 现在可以配置为 [jail(8)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8&manpath=freebsd-release-ports) 参数。[(r279361)](http://svn.freebsd.org/viewvc/base?view=revision&revision=279361)

[devfs(5)](http://www.freebsd.org/cgi/man.cgi?query=devfs&sektion=5&manpath=freebsd-release-ports) 设备文件系统已变更，以秒级精度更新读/写操作的时间戳。添加了新的 [sysctl(8)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&manpath=freebsd-release-ports) `vfs.devfs.dotimes`，当设置为非零值时，启用这些操作的默认精度时间戳。[(r280949)](http://svn.freebsd.org/viewvc/base?view=revision&revision=280949)（由 iXsystems 和 FreeBSD 基金会赞助）
...
本节涵盖自 10.3-RELEASE 以来的设备和设备驱动程序的变更和新增内容。
...
[vt(4)](http://www.freebsd.org/cgi/man.cgi?query=vt&sektion=4&manpath=freebsd-release-ports) 驱动程序已更新，替换了位图形式的 `kern.vt.spclkeys` [sysctl(8)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&manpath=freebsd-release-ports)，并使用了独立的 `kern.vt.kbd_*` 变体。[(r273178)](http://svn.freebsd.org/viewvc/base?view=revision&revision=273178)
...
[psm(4)](http://www.freebsd.org/cgi/man.cgi?query=psm&sektion=4&manpath=freebsd-release-ports) 驱动程序已更新，增加了对较新款 Synaptics® 触控板和较新款 Lenovo™ 笔记本电脑上的 ClickPad® 鼠标的改进支持。[(r281440)](http://svn.freebsd.org/viewvc/base?view=revision&revision=281440)
...
已添加对 Broadcom 芯片组 BCM57764、BCM57767、BCM57782、BCM57786 和 BCM57787 的支持到[bge(4)](http://www.freebsd.org/cgi/man.cgi?query=bge&sektion=4&manpath=freebsd-release-ports)中。[ (r258830)](http://svn.freebsd.org/viewvc/base?view=revision&revision=258830)
...
从 FreeBSD 11 起，已停止对 FreeBSD/ia64（Itanium，安腾）的支持。[(r268351)](http://svn.freebsd.org/viewvc/base?view=revision&revision=268351)
...
[arm64] FreeBSD/arm64 端口已增加初始的 SMP 支持。[(r285316)](http://svn.freebsd.org/viewvc/base?view=revision&revision=285316)（由 FreeBSD 基金会提供赞助）
...
>**注意**
>
>已将那些广告支持此功能但未能正确支持的驱动器列入黑名单。遇到启用 NCQ TRIM 后流量问题的系统，可以将可调参数 `kern.cam.ada.%d.quirks` 设置为 `2`（适用于 512k 扇区）或 `3`（适用于 4096k 扇区），并将 `%d` 替换为驱动器编号。
...
新增了一个 [ttys(5)](http://www.freebsd.org/cgi/man.cgi?query=ttys&sektion=5&manpath=freebsd-release-ports) 类，`3wire`。它类似于现有的终端类，但没有定义波特率。[(r262955)](http://svn.freebsd.org/viewvc/base?view=revision&revision=262955)
...
Dummynet AQM，作为 ipfw/dummynet 的独立实现，已被导入到基础系统中。[(r300779)](http://svn.freebsd.org/viewvc/base?view=revision&revision=300779)
...
发布工程构建工具已更新，以支持为 FreeBSD/arm 平台构建映像，无需外部实用工具，前提是支持的板卡在 Ports 中有相应的 `u-boot` Port。[(r282693)](http://svn.freebsd.org/viewvc/base?view=revision&revision=282693)（由 FreeBSD 基金会赞助）
