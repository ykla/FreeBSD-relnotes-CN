...
已更改 `/dev/acpi` 上的组权限，以允许 `operator` 组的用户调用 [acpiconf(8)](https://man.freebsd.org/cgi/man.cgi?query=acpiconf&sektion=8&format=html) 来暂停系统。 [[r335864]](http://svn.freebsd.org/viewvc/base?view=revision&revision=335864) （由 DARPA，AFRL 资助）
...
[init(8)](https://man.freebsd.org/cgi/man.cgi?query=init&sektion=8&format=html) 工具已更新，支持执行非 [sh(1)](https://man.freebsd.org/cgi/man.cgi?query=sh&sektion=1&format=html) 编写的可执行文件，例如 Python。 [[r337321]](http://svn.freebsd.org/viewvc/base?view=revision&revision=337321) （由 DARPA, AFRL 赞助）
