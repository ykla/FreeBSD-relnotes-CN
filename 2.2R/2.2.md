...
- **性能优化**：
  ...
  - Poul-Henning Kamp 开发的 `phkmalloc` 替换了旧的、拙劣的 BSD malloc 实现，显著减少了虚拟内存消耗，并提供了检测到内存分配滥用时中止程序、在已分配或已释放区域填充垃圾数据以检测程序语义问题等功能。
...
请注意，`send-pr` 本身是个脚本，应该可以很容易地移植到完全不同的系统上。我们更希望你能使用这个接口，因为它能更方便地追踪问题报告。然而，在提交之前，请尽量确认该问题是否已经修复。
