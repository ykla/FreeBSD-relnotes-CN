...
- **GDB**：gdb 4.16 从 `-current` 分支合并了，大部分第三方源代码现在存放在 **/usr/src/contrib** 下。
- **硬件支持**：
  - 更新了对 DEC DEFPA/DEFEA FDDI 硬件的支持。
  - Javier Martin Rueda 的驱动程序 `ex` 合并了，支持 Intel EtherExpress Pro/10 网卡。
  - 驱动程序 `de` 现在可以识别使用 DE21140A 芯片的网卡，例如流行的 SMC9332BDT（10/100 Mbit/s）。
  ...
  - 增加了对 SDL RISCom N2pci 同步串行卡以及 Cyclades Cyclom-Y（多端口异步串行）PCI 适配器的支持，包括多个控制器和 32-Y 型号（如果你当前正在使用 Cyclades 串行适配器，应重新创建 **/dev** 条目并删除旧的条目）。
  - 更新了对使用 DEC DC21X4X 芯片组的以太网适配器的支持。
  - 增加了对 SNMP 风格接口 MIB 的支持，包括 `de`（DEC 21x4x）和 `ed`（SMC/WD/Novell）驱动程序的完整 RFC 1650 兼容 MIB。
  - ATAPI CD-ROM 支持据报告已在大量驱动器上正常工作；基本遵循 ATAPI 标准的驱动器都可能兼容。
  - 内核中提供了许多新驱动程序，数量多到难以一一列举。Tekram 为其 DC390 和 DC390T 控制器（基于 AMD 53c974）提供了驱动程序，该驱动程序也能驱动其他基于该芯片的 SCSI 控制器。当然，Tekram 慷慨地为 FreeBSD 项目提供了驱动程序支持，我们鼓励你购买他们的产品。`ed` 和 `lnc` 驱动程序现在支持相应 PCI 以太网卡的自动配置，包括许多 NE2000 兼容卡和 AMD PCnet 芯片。SDL RISCom N2 支持是新增的，Cyclades 驱动程序的 PCI 版本也是新增的。
- **性能优化**：
  - 删除了旧的 Makefile 选项 `HAVE_FPU`，现在数学库能够自动选择使用浮点协处理器或浮点模拟器，这将加速那些之前不愿重新编译 `libm` 的站点上使用浮点的程序。
  - Poul-Henning Kamp 开发的 `phkmalloc` 替换了旧的、拙劣的 BSD malloc 实现，通常为客户端节省大量虚拟内存，并提供了检测到内存分配滥用时中止程序、在已分配或已释放区域填充垃圾数据以检测程序语义问题等精巧功能。
  - 更新至 gcc 2.7.2.1，并增加了对弱符号（weak symbols）的支持。
- **文件系统**：
  - `async` 挂载选项支持 UFS 文件系统进行异步元数据更新（类似于 Linux ext2fs 中的默认行为），显著提升了文件系统操作速度（如 `rm -r`），但可能在系统崩溃时增加风险。安装过程本身也使用了该特性，从而大幅加快了安装速度。（在快速机器上，仅从 SCSI CD-ROM 安装基本发行版现在不到 5 分钟即可完成！）
  - 2.2 是首个包含完全 CD-R 支持的版本，支持 Plasmon RF41xx、HP4020i、HP6020i 和 Philips CDD2000 驱动。该驱动程序仍在开发中（特别是在扩展对其他设备的支持方面），但目前已被证明是稳定的。
- **NFS 支持**：
  - 在从 2.0.5/2.1.X 分支分离后不久，对 NFSv3 客户端和服务器的支持就进入了 2.2 源代码。此外，NFS 还提供了一些其他选项，如将 NFSv2 服务器切换到异步写模式（虽违反规范，但在 SGI Irix 等系统中已有先例）。
- **兼容性改进**：
  - Linux 模拟现在完全可用，包括 ELF 格式支持，并提供了所需的共享库和 Slackware 开发环境的 Ports。
  - 类似地，SysV COFF 模拟（即 SCO 模拟）据报告现在也能正常工作。
  - FreeBSD 还支持原生 ELF 二进制文件，尽管尚未决定是否、何时以及如何将其作为默认二进制格式。新增了工具 `brandelf`，用于标记非共享链接的 ELF 二进制文件，以应对内核无法判断应该使用哪个镜像激活器（FreeBSD、Linux、未来可能是 SysV）的情况。这规避了 ELF 对象格式中存在的主要缺陷，即缺少标记所属 ABI 的字段。
- **其他改进**：
  - 许多东西迁移到 **/usr/src/contrib**，并相应地更新和清理了源代码树。
  - 增加了对编译时共享库 ld 路径的支持。
  - sgmlfmt 更新至 `instant`。
  - `netatalk` 实现的 AppleTalk 已集成到源代码中，大部分集成工作由 Wistle Communications Corp. 提供。
  - 手册的第 9 节开始编写，用于描述“官方”内核编程接口，欢迎志愿者参与文档编写。
  - 内核配置选项的处理大幅从旧的 `-D` Makefile 变通方法转向使用“opt_foo.h”内核包含文件的系统，使 Makefile 依赖关系恢复正常。预计每次运行 **config(8)** 时清空整个编译目录的旧方法将很快被淘汰。除非你变更了奇怪的选项，现在可以考虑使用 **config(8)** 的选项 `-n`，或者设置环境变量 `NO_CONFIG_CLOBBER`，如果你认为 CPU 时间宝贵的话。有关其工作原理的说明，请参见手册中的注释。
  - 对 APM BIOS 的支持现在有了很大改善。
  - **/stand/sysinstall** 进一步向通用系统管理工具发展。
  - `syscons` 和 `psm` 驱动程序现在具有新的底层共享键盘驱动程序，消除了它们之间之前存在的许多交互问题。`syscons` 现在支持在文本模式下使用 **moused(8)** 实现剪切和粘贴功能。

## 2. 支持的配置

FreeBSD 当前可运行在各种基于 ISA、VLB、EISA 和 PCI 总线的 PC 上，支持从 386sx 到 Pentium 等级的机器（虽然不推荐使用 386sx）。系统支持通用的 IDE 或 ESDI 硬盘配置，以及多种 SCSI 控制器、网卡和串口卡。
...
- Adaptec 274X/284X/2940/3940 系列（窄型/宽型/双通道）ISA/EISA/PCI SCSI 控制器
- Adaptec AIC7850 板载 SCSI 控制器
- 基于 Adaptec AIC-6260 和 AIC-6360 的板卡，包括 AHA-152x 和 SoundBlaster SCSI 卡

**注意**：SoundBlaster SCSI 卡无板载 BIOS，因此**不**能用作启动设备（需要 BIOS 将启动设备映射到系统 BIOS 的 I/O 向量）。不过，这些卡仍然可以用于外部磁带机、CD-ROM 等设备。基于 AIC-6x60 的其他无启动 ROM 的卡也是如此。如果某些卡具备启动 ROM（通常会在系统启动时显示提示信息），则可以从中启动。具体信息请参考系统或板卡文档。
...
请注意，`send-pr` 本身是个 shell 脚本，应该可以很容易地移植到完全不同的系统上。我们更希望你能使用这个接口，因为它能更方便地追踪问题报告。然而，在提交之前，请尽量确认该问题是否已经修复。
