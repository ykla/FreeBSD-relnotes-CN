...
自 2003 年中以来，FreeBSD/amd64 支持 AMD64（“Hammer”）和 Intel EM64T 架构，现为 Tier-1 平台（受完全支持的架构）之一，应在 FreeBSD 操作系统的各个方面（包括安装和开发环境）均达到生产质量。
...
在许多方面，FreeBSD/amd64 与 FreeBSD/i386 在驱动支持上类似。通常，已在其他 64 位平台上正常工作的驱动应该可用。
...
用户在调试 ACPI 相关问题时，可能会发现禁用部分 ACPI 功能会有所帮助。[acpi(4)](https://man.freebsd.org/cgi/man.cgi?query=acpi&sektion=4&format=html) 手册页提供了通过 loader 可调参数进行此操作的更多信息。
...
[mpt(4)](https://man.freebsd.org/cgi/man.cgi?query=mpt&sektion=4&format=html) 驱动支持的 Ultra 320 SCSI 控制器芯片可在许多系统中以板载形式找到，包括：
...
SAS 控制器芯片也出现在许多新的基于 AMD/Opteron 的系统上，例如 Sun 4100。注意，此控制器可同时驱动 SAS 和 SATA 驱动器或两者的混合。这些控制器可用的集成 RAID 镜像，其支持情况充其量有限。
...
> **注意**：
>
> [amd64, i386, pc98] USB 以太网适配器可在列出 [以太网接口](https://www.freebsd.org/releases/11.1R/hardware.html#ethernet) 的章节中找到。

> **注意**：
>
> [amd64, i386, pc98] USB 蓝牙适配器可在 [Bluetooth](https://www.freebsd.org/releases/11.1R/hardware.html#bluetooth) 章节中找到。
...
> **注意**：
>
> [moused(8)](https://man.freebsd.org/cgi/man.cgi?query=moused&sektion=8&format=html) 手册页提供了在 FreeBSD 中使用指针设备的更多信息。关于在 Xorg 中使用指针设备的信息可在 `http://www.x.org/` 找到。

[amd64, i386] “PC standard”并口（[ppc(4)](https://man.freebsd.org/cgi/man.cgi?query=ppc&sektion=4&format=html) 驱动）

[pc98] “PC-9821 standard”并口（[ppc(4)](https://man.freebsd.org/cgi/man.cgi?query=ppc&sektion=4&format=html) 驱动）

[i386, amd64] PC 兼容操纵杆（[joy(4)](https://man.freebsd.org/cgi/man.cgi?query=joy&sektion=4&format=html) 驱动）

[pc98] SoundBlaster(98) 的操纵杆端口（[joy(4)](https://man.freebsd.org/cgi/man.cgi?query=joy&sektion=4&format=html) 驱动）
...
[i386] 基于 Xilinx XC6200 的可重配置硬件卡，兼容来自 [Virtual Computers](http://www.vcc.com/) 的 HOT1（xrpu 驱动）。
