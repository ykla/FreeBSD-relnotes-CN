# FreeBSD 12.1-RELEASE 硬件兼容列表

## 引言

本文档包含 FreeBSD 12.1-RELEASE 的硬件兼容性说明。其中列出了 FreeBSD 支持的硬件平台，以及各类硬件设备（存储控制器、网络接口等），并给出已知可正常工作的设备实例。
...
所有内置 USB 的 Apple PowerPC 机器均受支持，此外还包括少量非 Apple 机器，如 POWER7 上的 KVM。
...
如果你有不在此列表中的系统，它可能尚未经过 FreeBSD 12.1-RELEASE 测试。我们鼓励你尝试并致信 A.SPARC 反馈结果，包括哪些设备可用、哪些不可用。
...
[amd64, i386, sparc64] IDE/ATA 控制器（[ata(4)](https://man.freebsd.org/cgi/man.cgi?query=ata&sektion=4&format=html) 驱动）
...
[mpt(4)](https://man.freebsd.org/cgi/man.cgi?query=mpt&sektion=4&format=html) 驱动支持的 Ultra 320 SCSI 控制器芯片也内置于许多系统中，包括：
...
[amd64, i386] 基于 SMC 83c17x (EPIC) 的以太网 NIC（[tx(4)](https://man.freebsd.org/cgi/man.cgi?query=tx&sektion=4&format=html) 驱动）
...
[i386, amd64] Intel Dual Band Wireless AC 3160/7260/7265 IEEE 802.11ac 网络适配器（[iwm(4)](https://man.freebsd.org/cgi/man.cgi?query=iwm&sektion=4&format=html) 驱动）

[i386, amd64] Intel Wireless WiFi Link 4965AGN IEEE 802.11n PCI 网络适配器（[iwn(4)](https://man.freebsd.org/cgi/man.cgi?query=iwn&sektion=4&format=html) 驱动）

[i386, amd64] Marvell Libertas IEEE 802.11b/g PCI 网络适配器（[malo(4)](https://man.freebsd.org/cgi/man.cgi?query=malo&sektion=4&format=html) 驱动）
...
[amd64, i386] Lucent Technologies WaveLAN/IEEE 802.11b 无线网络适配器及兼容设备，使用 Lucent Hermes、Intersil PRISM-II、Intersil PRISM-2.5、Intersil Prism-3 和 Symbol Spectrum24 芯片组（[wi(4)](https://man.freebsd.org/cgi/man.cgi?query=wi&sektion=4&format=html) 驱动）
...
[amd64, i386] "PC standard" 基于 8250、16450 和 16550 的串行端口（[sio(4)](https://man.freebsd.org/cgi/man.cgi?query=sio&sektion=4&format=html) 驱动）
...
* Boca BB1004 4 端口串行卡（_不_支持调制解调器）
* Boca IOAT66 6 端口串行卡（支持调制解调器）
* Boca BB1008 8 端口串行卡（_不_支持调制解调器）
...
* [amd64, i386] Decision Computers PCCOM 4 端口串行和双端口 RS232/422/485
...
* [amd64, i386] Middle Digital, Weasle 串行端口
...
* [amd64, i386] SIIG Cyber Serial（单和双）PCI 16C550/16C650/16C850
...
部分板载 CS4610 芯片搭配的是 CS423x ISA 编解码器，而非 CS4297 AC97 编解码器。此类配置目前尚不受 snd_csa 驱动支持。
...
[snd_emu10kx(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_emu10kx&sektion=4&format=html) 驱动不支持以下声卡（尽管其名称与某些受支持的声卡相似）：

* Creative Sound Blaster Live! 24-Bit，由 FreeBSD 识别为
* Creative Sound Blaster Audigy LS / ES，由 FreeBSD 识别为
...
[amd64, i386] 支持多种 USB 外设；已知可正常工作的设备在本节中列出。由于大多数 USB 设备具有通用性，除部分例外，任何属于给定类别的设备均受支持，即使未在此明确列出。

> **注意**：
>
> [amd64, i386] USB 以太网适配器可在 [以太网接口](https://www.freebsd.org/releases/12.1R/hardware.html#ethernet) 一节中查阅。


> **注意**：
>
> [amd64, i386] USB 蓝牙适配器可在 [蓝牙](https://www.freebsd.org/releases/12.1R/hardware.html#bluetooth) 一节中查阅。
...
> **注意**：
>
> 关于特定显卡及与 Xorg 兼容性的信息，请参阅 <http://www.x.org/>。
...
* [amd64, i386] 总线鼠标及兼容设备（[mse(4)](https://man.freebsd.org/cgi/man.cgi?query=mse&sektion=4&format=html) 驱动）
...
> **注意**：
>
> [moused(8)](https://man.freebsd.org/cgi/man.cgi?query=moused&sektion=8&format=html) 提供了在 FreeBSD 中使用指点设备的更多信息。关于在 Xorg 中使用指点设备的信息，请参阅 <http://www.x.org/>。
