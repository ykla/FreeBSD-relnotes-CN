...
- 更新了 DPT SCSI RAID 控制器（包括对 EISA 卡的支持），现默认启用。
- 新增对 MSDOS FAT32（Win95 长文件名）的支持。
- 修复了 SCSI 换盘器代码中的多个错误。
- 新增对基于 Crystal Semiconductor CS8920 的以太网卡的支持。
- 移除了失效的 LFS 代码。
- 更新了驱动程序 Specialix SI/XIO/SX。
- dmesg（历史缓冲区）现在可以由用户调整大小。请参阅 **/sys/i386/conf/LINT** 中的选项 `MSGBUF_SIZE`。
- 修复了 Adaptec aic7870/aic7880 芯片组的多个错误。
- 现在在启动时能正确识别 Pentium II 处理器。
...
- inetd(8) 现在支持服务速率限制。
- 实用工具 ppp(8) 进行了重大更新。详见手册页。
- 更新并清理了许多（非常多！）手册页和其他文档。
- `libc_r`（POSIX pthread 支持的一部分）现在默认包含在系统中，并整合了多项错误修复。
- ls(1) 新增多个选项，详见 `man ls` 手册页。
- cvs(1) 更新至版本 1.9.26。
- **/etc** 的多个部分借鉴 3.0 版本的部分功能进行了更新。
- as(1) 现在支持 fildll/fistpll 指令。
- 改进了安装过程的多个方面。
- 修复了 curses(3) 中的一些小的定位错误。
...
- XFree86 更新至 3.3.2.3——这是重要的安全版本，包含来自 The Open Group 的变更，修复了本地用户可能利用的多个 root 权限漏洞。
- 加密仓库从 3.0 分支更新。
- 升级了 Ports 中的 popper 和 imap，修复了一些严重的安全漏洞（请参阅 Bugtrax）。
- 修复了多个实用程序（如 rcp(1) 和 more(1) 等）中的缓冲区溢出漏洞。
- 在 BIND 的多个“易攻击”位置新增边界检查，并对 BIND 的许多部分进行了重要更新。

## 2. 支持的配置

FreeBSD 目前可以运行在多种基于 ISA、VLB、EISA 和 PCI 总线的 PC 上，从 386sx 到 Pentium 级别的机器（虽然不推荐使用 386sx）。同时支持通用 IDE 或 ESDI 驱动配置、各种 SCSI 控制器、网卡和串行卡。
...
**注意**：无法从 SoundBlaster 卡引导，因为它们没有板载 BIOS，而引导设备映射到系统 BIOS I/O 向量中需要此功能。然而，这些卡完全可用于外部磁带、CD-ROM 等。对于任何没有引导 ROM 的基于 AIC-6x60 的卡也是如此。某些系统确实有引导 ROM，这通常会在系统首次开机或重置时显示消息，在这种情况下你也将能够从这些卡引导。请参阅你的系统/主板文档以了解更多信息。
...
**注意：** 目前**不**支持令牌环网卡，我们仍在等待有人为其捐赠驱动程序。有没有志愿者？
...
**注意：** FreeBSD 当前**不**支持 IBM 的 Microchannel (MCA) 总线。
...
你可以从 FreeBSD 的官方发布站点 `ftp.FreeBSD.org` 上通过 FTP 下载 FreeBSD 及其任何或所有可选软件包。

有关镜像 FreeBSD 软件的其他站点，请参阅文件 **MIRROR.SITES**。请从网络层面最接近你的站点下载发行版本。我们也欢迎更多镜像站点的加入！如果你希望成为官方镜像站点，请联系：<freebsd-admin@FreeBSD.org>。

如果你无法访问互联网，且电子邮件是你唯一的求助途径，仍然可以通过向 `ftpmail@ftpmail.vix.com` 发送邮件来下载文件。在邮件中包含关键字 `help` 以获取关于如何使用该机制获取文件的更多信息。但请注意，这种方式会通过邮件传输大量数据（*数十兆字节*），应仅在别无他法的情况下使用！
...
每张 `-RELEASE` 光盘售价为 $39.95，或通过 FreeBSD 订阅价为 $24.95。FreeBSD 3.0-SNAP 光盘售价为 $39.95，或通过 FreeBSD-SNAP 订阅价为 $14.95（`-RELEASE` 和 `-SNAP` 订阅是完全分开的）。订阅后，你将自动收到发布的更新。每次发货时会扣除信用卡费用，你可以随时取消订阅，无需承担额外费用。
...
你的建议、漏洞报告以及代码贡献始终受到重视——如果你发现任何问题，请随时报告（如果可以，最好附上修复方案）。

在具备互联网邮件连接的机器上，提交漏洞报告的首选方法是使用命令 `send-pr`，或访问 [http://www.FreeBSD.org/send-pr.html](http://www.FreeBSD.org/send-pr.html) 使用 CGI 脚本提交报告。 我们忠实的漏洞记录程序会尽职归档通过这种方式提交的漏洞报告，并且我们会尽力尽快回应所有报告的问题。以这种方式归档的漏洞报告也会显示在我们网站的支持部分，既可作为漏洞报告，也能为其他用户提供可能需要注意的问题的“指引”。

如果由于某种原因无法使用命令 `send-pr` 提交漏洞报告，你可以尝试将报告发送至以下地址：`freebsd-bugs@FreeBSD.org`

请注意，`send-pr` 本身是个 shell 脚本，即使在完全不同的系统上也很容易移植。我们更希望你使用此接口，因为这样可以更方便地跟踪问题报告。然而，在提交报告之前，请尽量确认该问题是否已经修复。

否则，对于任何问题或技术支持问题，请发送邮件至：`freebsd-questions@FreeBSD.org`

此外，由于这是一个志愿者项目，我们始终欢迎额外的人手加入。当前期望的改进远超我们自身的能力！如果你有技术问题或愿意提供帮助，请联系：`freebsd-hackers@FreeBSD.org`

请注意，这些邮件列表的流量可能非常大。如果你的邮件访问速度较慢或费用较高，并且只希望了解 FreeBSD 的重大事件，你可能会发现订阅以下邮件列表更为方便：`freebsd-announce@FreeBSD.org`
...
FreeBSD 是全球数十甚至数百名人士辛勤努力的累积成果，他们共同完成了这一版本的发布。有关 FreeBSD 项目工作人员的完整名单，请参见：`http://www.FreeBSD.org/handbook/staff.html`
