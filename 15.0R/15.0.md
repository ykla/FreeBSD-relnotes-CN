...
FreeBSD 15.0-RELEASE 的发行说明总结了 15-STABLE 开发线上对 FreeBSD 基本系统所做的变动。本文档列出了自上个 RELEASE 发布以来的相关安全公告，以及对 FreeBSD 内核和用户空间的重要变动。此外，本文还简要给出了若干关于更新的说明。

## 引言

本文件是 FreeBSD 15.0-RELEASE 的发行说明。它涉及了 FreeBSD 最近新增、变动及删除的特性，并提供了旧版本 FreeBSD 更新时的若干说明。

这些发行说明所对应的“RELEASE”发行版，代表 15-STABLE 开发分支上介于 14.0-RELEASE 与后续 15.1-RELEASE 之间的最新节点。可以在 [https://www.FreeBSD.org/releases/](https://www.freebsd.org/releases/) 找到关于此分支上预构建二进制“RELEASE”发行版的信息。要获取此（或其他）FreeBSD“RELEASE”发行版的更多信息，可参阅 [FreeBSD 手册](https://docs.freebsd.org/en/books/handbook/) 的附录 [获取 FreeBSD](https://docs.freebsd.org/en/books/handbook/mirrors)。

我们强烈建议所有用户在安装 FreeBSD 之前查阅发行勘误。发行勘误文档会随着发行周期末期或发行之后发现的“最新”信息而更新。通常它包含了已知缺陷、安全公告以及文档修正。可以在 FreeBSD 网站上找到 FreeBSD 15.0-RELEASE 的最新发行勘误。

本文档包含自 14.0-RELEASE 以来，在 FreeBSD 中对用户最为可见的新特性或变动。通常除非特别标记为特性“[已合并]”，此文档所述变动均为 15-STABLE 分支所独有。

典型的发行说明条目会记录：在 14.0-RELEASE 之后发布的近期安全公告、新的驱动/硬件支持、新的命令/选项、主要缺陷修复/第三方软件更新。它们也可能列出主要 Ports / 软件包以及发行工程实践的变动。当然，发行说明不可能列出 FreeBSD 在两个发行版本之间的每一项变动；本文主要聚焦安全公告、用户可见变动以及主要架构改进。
...
用户可以使用 [freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 实用程序在快照版本与 RELEASE 版本之间进行二进制更新。更多信息请参阅特定发行版的更新流程，[FreeBSD 15.0-RELEASE 更新信息](https://www.freebsd.org/releases/15.0R/installation/#upgrade-binary)，以及 FreeBSD 手册中的 [二进制更新流程](https://docs.freebsd.org/en/books/handbook/cutting-edge/#freebsdupdate-upgrade)。该过程会更新未修改的用户空间工具，以及作为官方 FreeBSD 发行版一部分分发的未修改 **GENERIC** 内核。[freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 实用程序要求待更新的主机具备 Internet 连接。

### 从软件包更新

> **警告**
>
> 对于那些使用 FreeBSD 15.0 的 PRERELEASE、ALPHA 和 BETA 构建的用户，由于 FreeBSD.org 基础设施中出现的后期变更，他们无法直接使用 [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html) 实用程序更新。

对于基于软件包的基本系统安装，用户需要手动从 `15.0-RELEASE` 标签的源代码树检出中复制所需文件，或者作为替代方案，从官方发行版基本系统软件包中强制安装软件包 `FreeBSD-pkg-bootstrap`。

推荐且最稳妥的方式，是使用 UTC 时间 2025 年 11 月 27 日 22:00（北京时间 2025 年 11 月 28 日 06:00）之后的 head、stable/15 或 releng/15.0 分支的源码树检出。
...
未安装最新源代码的用户可以采用安全性较低但更简单的方法，在安装后验证校验和。由于这些文件与架构无关，校验和在所有平台上都会匹配。在通过 [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html) 更新之前，需要启用 FreeBSD-base 包仓库：
...
两种方法之后都需要启用 FreeBSD-base 仓库。启用方法之一是运行以下命令：
...
用户可以通过编译 releng/15.0 分支来实现从旧版本源代码的更新。请参阅 **/usr/src/UPDATING** 中的说明。
...
| [FreeBSD-EN-25:12:efi](https://www.freebsd.org/security/advisories/FreeBSD-EN-25:12.efi.asc) | 2025 年 8 月 8 日 | [bsdinstall(8)](https://man.freebsd.org/cgi/man.cgi?query=bsdinstall&sektion=8&format=html) 在 IA32 UEFI 固件系统上未复制正确的 loader |
...
| [FreeBSD-EN-25:15:arm64](https://www.freebsd.org/security/advisories/FreeBSD-EN-25:15.arm64.asc) | 2025 年 9 月 16 日 | arm64 [syscall(2)](https://man.freebsd.org/cgi/man.cgi?query=syscall&sektion=2&format=html) 允许无特权用户触发内核 panic |
...
久经考验的 32 位硬件平台 i386、armv6 以及 32 位 powerpc 已经退役。对 32 位应用的支持仍然存在，由它们各自 64 位平台的 32 位兼容模式提供。armv7 平台仍是最后支持的 32 位平台。我们感谢这些平台多年来的服务。
...
Kerberos v5 认证服务 [krb5kdc(8)](https://man.freebsd.org/cgi/man.cgi?query=krb5kdc&sektion=8&format=html) 在 [daemon(8)](https://man.freebsd.org/cgi/man.cgi?query=daemon&sektion=8&format=html) 下增加了新变量 `kdc_restart`。用户在 [rc.conf(5)](https://man.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5&format=html) 中设置 `kdc_restart="YES"` 后，可在 kdc 异常终止时自动重启。设置 `kdc_restart_delay="N"` 可指定重启 kdc 前的延迟秒数。[abc4b3088941](https://cgit.freebsd.org/src/commit/?id=abc4b3088941)

[periodic(8)](https://man.freebsd.org/cgi/man.cgi?query=periodic&sektion=8&format=html) `daily` 脚本现在默认在邮件中显示的上下文有所减少，以减小输出量。此行为可通过 [periodic.conf(5)](https://man.freebsd.org/cgi/man.cgi?query=periodic.conf&sektion=5&format=html) 中的变量 `daily_diff_flags` 控制。同样，安全脚本显示的变动上下文也比以前少，可通过变量 `security_status_diff_flags` 控制。[538994626b9f](https://cgit.freebsd.org/src/commit/?id=538994626b9f), [37dc394170a5](https://cgit.freebsd.org/src/commit/?id=37dc394170a5), [128e78ffb084](https://cgit.freebsd.org/src/commit/?id=128e78ffb084)

守护进程 [bsnmpd(1)](https://man.freebsd.org/cgi/man.cgi?query=bsnmpd&sektion=1&format=html) 不再支持传统 UDP 传输。未从 12.0-RELEASE 或更旧版本更新 **/etc/snmpd.config** 的用户，需要合并新的配置。特别是，用户需将传输定义从 OID `begemotSnmpdPortStatus` 改为 `begemotSnmpdTransInetStatus`。[9ba51cce8bbd](https://cgit.freebsd.org/src/commit/?id=9ba51cce8bbd)

`FreeBSD-base` 仓库现在定义在 **/etc/pkg/FreeBSD.conf** 中，默认处于禁用状态。在 15.0-RC1（如果运行 `releng/15.0`）或 11 月 15 日之前使用 pkgbase 安装（如果使用 `stable`/`main` 快照）的系统，需要删除定义在 **/usr/local/etc/pkg/repos/** 的 `FreeBSD-base` 仓库，并替换为这行：`FreeBSD-base: { enabled: yes }`。[5d832135a971](https://cgit.freebsd.org/src/commit/?id=5d832135a971)
...
工具 [env(1)](https://man.freebsd.org/cgi/man.cgi?query=env&sektion=1&format=html) 新增了切换目录的选项，这类似于 GNU 版 env 的功能，但不支持长选项。[08e8554c4a39](https://cgit.freebsd.org/src/commit/?id=08e8554c4a39)（由 Klara, Inc. 赞助）

现在工具 [ps(1)](https://man.freebsd.org/cgi/man.cgi?query=ps&sektion=1&format=html) 能自动移除“默认显示”中与显式请求的列重复的数据列。在此之前，如果用户请求添加某个“默认显示”（选项 `-j`、`-l`、`-u` 和 `-v`），与命令行前面显式请求列重复的列会被省略，但反向情况（默认显示出现在显式请求列之前）不会生效。此外，不同关键词但作为同一关键词别名的列也被视为持有相同数据，而不仅限于相同关键词的列。[cd768a840644](https://cgit.freebsd.org/src/commit/?id=cd768a840644)（由 FreeBSD 基金会赞助）

工具 [ps(1)](https://man.freebsd.org/cgi/man.cgi?query=ps&sektion=1&format=html) 的选项 `-O` 现在更加灵活和可预测。[ps(1)](https://man.freebsd.org/cgi/man.cgi?query=ps&sektion=1&format=html) 显示的列列表现在会首先在不考虑选项 `-O` 的情况下构建。然后，所有通过 `-O` 传入的列将最终插入到已构建的显示列中——如果存在第一列 PID 列，则插在其后；否则插入到开头，并保持它们作为参数 `-O` 的出现顺序。[5dad61d9b949](https://cgit.freebsd.org/src/commit/?id=5dad61d9b949)（由 FreeBSD 基金会赞助）

现在工具 [ps(1)](https://man.freebsd.org/cgi/man.cgi?query=ps&sektion=1&format=html) 的选项 `-a` 和 `-A` 总会显示所有进程。之前，当与影响进程选择的其他选项组合使用时，除了 `-X` 和 `-x`，选项 `-a` 不会生效（`-A` 会退化为仅 `-x`），这与其他选择选项的规则相矛盾——若指定了任何选择选项并选择了某个进程，该进程应被列出，这是 POSIX 所要求的，也是用户的自然预期。现在，指定 `-a` 或 `-A` 可列出所有进程，而不受诸如 `-U`、`-p`、`-G` 等其他选择选项影响，但 `-X` 和 `-x` 过滤选项仍然有效。特别地，如果只想列出特定 Jail 的进程，不应将 `-a` 与 `-J` 一起使用。实际上，`-J` 从未像其初衷那样作为过滤器起作用，之前只是由于 bug 与 `-a` 的偶然结合才生效。[93a94ce731a8](https://cgit.freebsd.org/src/commit/?id=93a94ce731a8)（由 FreeBSD 基金会赞助）

现在工具 [ps(1)](https://man.freebsd.org/cgi/man.cgi?query=ps&sektion=1&format=html) 会使用有效用户 ID 匹配当前用户的进程。此前使用的是真实用户 ID。这一变动使 [ps(1)](https://man.freebsd.org/cgi/man.cgi?query=ps&sektion=1&format=html) 在此行为上符合 POSIX 标准。[1aabbb25c9f9](https://cgit.freebsd.org/src/commit/?id=1aabbb25c9f9c4372)（由 FreeBSD 基金会赞助）

现在工具 [ps(1)](https://man.freebsd.org/cgi/man.cgi?query=ps&sektion=1&format=html) 的标志 `-U` 会按真实用户 ID 选择进程。这符合 POSIX 对 `-U` 的规范，也是大多数用户在大多数情况下真正需要的行为。此前，`-U` 会按有效用户 ID 选择进程（这是 POSIX 对选项 `-u` 的要求）。[995b690d1398](https://cgit.freebsd.org/src/commit/?id=995b690d1398)（由 FreeBSD 基金会赞助）
...
现在工具 [mdo(1)](https://man.freebsd.org/cgi/man.cgi?query=mdo&sektion=1&format=html) 能在目标凭据中完全指定所有用户和组。作为便捷功能，除了完全显式指定外，还能以基线开始，为所有属性提供默认值（可以是密码数据库中某个用户的登录凭据或当前凭据），然后选择性修改这些属性。手册页更新，描述了新选项及其相互作用。[4ffcb1a4a99c](https://cgit.freebsd.org/src/commit/?id=4ffcb1a4a99c)（由 FreeBSD 基金会赞助）（由谷歌（2025 编程之夏）赞助）

在单用户模式下启动时，现在 [init(8)](https://man.freebsd.org/cgi/man.cgi?query=init&sektion=8&format=html) 会将工作目录修改为 **/root**，仅在回退情况下使用 `/`。系统不再安装 **/.profile** 指向 **/root/.profile** 的链接。[b4b91207ab6f](https://cgit.freebsd.org/src/commit/?id=b4b91207ab6f), [ca771d7ae527](https://cgit.freebsd.org/src/commit/?id=ca771d7ae527)
...
Kerberos v5 数据库管理程序现在可以以一种格式导出 Heimdal KDC 数据库，以便 MIT KDC 导入。如何使用 `kadmin -l dump -f` 转移/转换 KDC 数据库，请参阅 [https://wiki.freebsd.org/Kerberos/Heimdal2MIT_KDC_Migration](https://wiki.freebsd.org/Kerberos/Heimdal2MIT_KDC_Migration)。[9fd3b28d4e0d](https://cgit.freebsd.org/src/commit/?id=9fd3b28d4e0d), [23fbea8cf2f3](https://cgit.freebsd.org/src/commit/?id=23fbea8cf2f3)
...
现在命令 [jail(8)](https://man.freebsd.org/cgi/man.cgi?query=jail&sektion=8&format=html) 支持了参数 `zfs.dataset`，用户可通过该参数将 ZFS 数据集列表附加到 Jail。[e0dfe185cbca](https://cgit.freebsd.org/src/commit/?id=e0dfe185cbca)

现在命令 [jail(8)](https://man.freebsd.org/cgi/man.cgi?query=jail&sektion=8&format=html) 支持了参数 meta 和 env，它们是与 Jail 关联的任意字符串。这些参数可用于给 Jail 添加特定元数据，或在 Jail 内安全传递信息。用户可以在创建 Jail 时添加这些参数，也可以稍后使用 [jail(8)](https://man.freebsd.org/cgi/man.cgi?query=jail&sektion=8&format=html) 进行修改。[30e6e008bc06](https://cgit.freebsd.org/src/commit/?id=30e6e008bc06)（由 SkunkWerks, GmbH 赞助）
...
One True Awk ([awk(1)](https://man.freebsd.org/cgi/man.cgi?query=awk&sektion=1&format=html)) 更新至第二版，新增了 `-csv` 支持和 UTF-8 支持。使用的快照版本为 2025-08-04。[b45a181a74c8](https://cgit.freebsd.org/src/commit/?id=b45a181a74c8)（由 Netflix 赞助）

系统参考手册工具链 [mandoc(1)](https://man.freebsd.org/cgi/man.cgi?query=mandoc&sektion=1&format=html) 更新至 1.14.6 版本，使用快照 2025-09-26。该版本改进了与 groff 和 DocBook 的兼容性，增强了 HTML 和 Markdown 输出，并弃用了 LIBRARY 部分。[c1c95add8c80](https://cgit.freebsd.org/src/commit/?id=c1c95add8c80), [80c12959679a](https://cgit.freebsd.org/src/commit/?id=80c12959679a), [4c07abdbacf4](https://cgit.freebsd.org/src/commit/?id=4c07abdbacf4), [06410c1b5163](https://cgit.freebsd.org/src/commit/?id=06410c1b5163), [59fc2b0166f7](https://cgit.freebsd.org/src/commit/?id=59fc2b0166f7)

[jemalloc(3)](https://man.freebsd.org/cgi/man.cgi?query=jemalloc&sektion=3&format=html) 库更新至 5.3.0 版本。[c43cad871720](https://cgit.freebsd.org/src/commit/?id=c43cad871720)

构建系统 [bmake(1)](https://man.freebsd.org/cgi/man.cgi?query=bmake&sektion=1&format=html) 更新至 2025-08-04，提供了多项调试改进，修复了如检测并拒绝 `gmake` 语法的 bug，并带来了功能改进，例如 `-j` 接受浮点数参数时将其作为可用 CPU 数量的倍数。

套件 [sendmail(8)](https://man.freebsd.org/cgi/man.cgi?query=sendmail&sektion=8&format=html) 更新至 8.18.1 版本，修复了 CVE-2023-51765。[58ae50f31e95](https://cgit.freebsd.org/src/commit/?id=58ae50f31e95)

[bc(1)](https://man.freebsd.org/cgi/man.cgi?query=bc&sektion=1&format=html) 计算器更新至 7.1.0 版本。[fdc4a7c8012b](https://cgit.freebsd.org/src/commit/?id=fdc4a7c8012b)
...
[bsddialog(1)](https://man.freebsd.org/cgi/man.cgi?query=bsddialog&sektion=1&format=html) 工具更新至 1.0.5。[0595e10ec773](https://cgit.freebsd.org/src/commit/?id=0595e10ec773)

解析器生成器 [byacc(1)](https://man.freebsd.org/cgi/man.cgi?query=byacc&sektion=1&format=html) 更新至 2024-01-09。[822ca3276345](https://cgit.freebsd.org/src/commit/?id=822ca3276345)

`libarchive` 库更新至 3.8.2。[8a0b57ba54f0](https://cgit.freebsd.org/src/commit/?id=8a0b57ba54f0)

`libcbor` 库更新至 0.11.0。[1755b9daa693](https://cgit.freebsd.org/src/commit/?id=1755b9daa693)（由 FreeBSD 基金会赞助）

`libcxxrt` 库更新至供应商快照 6f2fdfebcd62。[d0dcee46d971](https://cgit.freebsd.org/src/commit/?id=d0dcee46d971)

`libfido2` 库更新至 1.14.0。[128bace5102e](https://cgit.freebsd.org/src/commit/?id=128bace5102e)（由 FreeBSD 基金会赞助）

`libpcap` 库更新至 1.10.5。[26f21a6494b4](https://cgit.freebsd.org/src/commit/?id=26f21a6494b4)（由 FreeBSD 基金会赞助）

[ncurses(3)](https://man.freebsd.org/cgi/man.cgi?query=ncurses&sektion=3&format=html) 库更新至 6.5。[21817992b331](https://cgit.freebsd.org/src/commit/?id=21817992b331)

工具 [tcpdump(1)](https://man.freebsd.org/cgi/man.cgi?query=tcpdump&sektion=1&format=html) 更新至 4.99.5。[ec3da16d8bc1](https://cgit.freebsd.org/src/commit/?id=ec3da16d8bc1)（由 FreeBSD 基金会赞助）

DNS 验证解析器 `unbound` 更新至 1.24.1。[a988846174e0](https://cgit.freebsd.org/src/commit/?id=a988846174e0)

编译器基础设施 `llvm` 更新至 19.1.7-0-gcd708029e0b2。[dc3f24ea8a25](https://cgit.freebsd.org/src/commit/?id=dc3f24ea8a25)

文件系统 OpenZFS 更新至 zfs-2.4.0-rc4。[7b5b0f43eb06](https://cgit.freebsd.org/src/commit/?id=7b5b0f43eb06)

数据压缩工具 [xz(1)](https://man.freebsd.org/cgi/man.cgi?query=xz&sektion=1&format=html) 更新至 5.8.1。[128836d304d9](https://cgit.freebsd.org/src/commit/?id=128836d304d9)

分页器 [less(1)](https://man.freebsd.org/cgi/man.cgi?query=less&sektion=1&format=html) 更新至 v679。[76bafc906926](https://cgit.freebsd.org/src/commit/?id=76bafc906926)

文件识别工具 [file(1)](https://man.freebsd.org/cgi/man.cgi?query=file&sektion=1&format=html) 更新至 5.46。[ae316d1d1cff](https://cgit.freebsd.org/src/commit/?id=ae316d1d1cff)

数据压缩库 [zlib(3)](https://man.freebsd.org/cgi/man.cgi?query=zlib&sektion=3&format=html) 更新至 1.3.1。[6255c67c3d1a](https://cgit.freebsd.org/src/commit/?id=6255c67c3d1a)

时区数据库 `tzdata` 更新至 2025b。[475082194ac8](https://cgit.freebsd.org/src/commit/?id=475082194ac8)

OpenSSH 更新至 10.0p2。[8e28d84935f2](https://cgit.freebsd.org/src/commit/?id=8e28d84935f2)（由 FreeBSD 基金会赞助）

OpenSSL 更新至 3.5.4。[c0366f908ff4](https://cgit.freebsd.org/src/commit/?id=c0366f908ff4)

Lua 更新至 5.4.8。[3068d706eabe](https://cgit.freebsd.org/src/commit/?id=3068d706eabe)（由 Netflix 赞助）

Google Test C 测试框架更新至 1.15.2。值得注意的是，GoogleTest 1.15.x 现在正式要求 C++14（1.14.x 版本要求 C++11）。[1d67cec52542](https://cgit.freebsd.org/src/commit/?id=1d67cec52542)

 [vt(4)](https://man.freebsd.org/cgi/man.cgi?query=vt&sektion=4&format=html) 控制台字体 `spleen` 更新至 2.1.0。[26336203d32c](https://cgit.freebsd.org/src/commit/?id=26336203d32c)

MIT KRB5 1.22.1 Kerberos 替代了原有默认的 Heimdal 1.5.2。用户仍可通过标志 `WITHOUT_MITKRB5` 编译 Heimdal 1.5.2。Heimdal Kerberos 将在 FreeBSD 16 中彻底移除。另请参考 [用户空间应用变动](https://www.freebsd.org/releases/15.0R/relnotes/#userland-programs) 中关于标志 `kadmin -l dump -f` 的说明。[ee3960cba106](https://cgit.freebsd.org/src/commit/?id=ee3960cba106), [0b9a631e0724](https://cgit.freebsd.org/src/commit/?id=0b9a631e0724), [60f970b85e44](https://cgit.freebsd.org/src/commit/?id=60f970b85e44), [0d1496f0f1e7](https://cgit.freebsd.org/src/commit/?id=0d1496f0f1e7), [cbb6e747af98](https://cgit.freebsd.org/src/commit/?id=cbb6e747af98), [0559f30a882d](https://cgit.freebsd.org/src/commit/?id=0559f30a882d), [ae07a5805b19](https://cgit.freebsd.org/src/commit/?id=ae07a5805b19), [f58febc4cefa](https://cgit.freebsd.org/src/commit/?id=f58febc4cefa), [805498e49ae4](https://cgit.freebsd.org/src/commit/?id=805498e49ae4), [4cb1baa7d85c](https://cgit.freebsd.org/src/commit/?id=4cb1baa7d85c), [188138106b9f](https://cgit.freebsd.org/src/commit/?id=188138106b9f), [4680e7fcc70a](https://cgit.freebsd.org/src/commit/?id=4680e7fcc70a), [e447c252d0ec](https://cgit.freebsd.org/src/commit/?id=e447c252d0ec), [5f8493bbf479](https://cgit.freebsd.org/src/commit/?id=5f8493bbf479), [110111a6cca1](https://cgit.freebsd.org/src/commit/?id=110111a6cca1), [2a454b05f2c1](https://cgit.freebsd.org/src/commit/?id=2a454b05f2c1), [98d46e05ab08](https://cgit.freebsd.org/src/commit/?id=98d46e05ab08), [6b28571cb6ba](https://cgit.freebsd.org/src/commit/?id=6b28571cb6ba), [ca9ccf0ce9ad](https://cgit.freebsd.org/src/commit/?id=ca9ccf0ce9ad), [b98d0566b2bd](https://cgit.freebsd.org/src/commit/?id=b98d0566b2bd), [fb1ccc04adfe](https://cgit.freebsd.org/src/commit/?id=fb1ccc04adfe), [dd0ec030f8fd](https://cgit.freebsd.org/src/commit/?id=dd0ec030f8fd), [6c4771c73470](https://cgit.freebsd.org/src/commit/?id=6c4771c73470), [7b68893ffa9b](https://cgit.freebsd.org/src/commit/?id=7b68893ffa9b), [624b7beed5ac](https://cgit.freebsd.org/src/commit/?id=624b7beed5ac), [04764f21855a](https://cgit.freebsd.org/src/commit/?id=04764f21855a), [73ed0c7992fd](https://cgit.freebsd.org/src/commit/?id=73ed0c7992fd), [40a5abfc3f66](https://cgit.freebsd.org/src/commit/?id=40a5abfc3f66), [543b875a8ee4](https://cgit.freebsd.org/src/commit/?id=543b875a8ee4), [c791ea80b5f7](https://cgit.freebsd.org/src/commit/?id=c791ea80b5f7), [383e7290c0b5](https://cgit.freebsd.org/src/commit/?id=383e7290c0b5), [9a726ef24134](https://cgit.freebsd.org/src/commit/?id=9a726ef24134), [a245dc5d68c7](https://cgit.freebsd.org/src/commit/?id=a245dc5d68c7), [e26259f48afe](https://cgit.freebsd.org/src/commit/?id=e26259f48afe), [7d2cfb27d62f](https://cgit.freebsd.org/src/commit/?id=7d2cfb27d62f), [619feb9dd00e](https://cgit.freebsd.org/src/commit/?id=619feb9dd00e), [10eecc467f32](https://cgit.freebsd.org/src/commit/?id=10eecc467f32), [0c13e9c3c464](https://cgit.freebsd.org/src/commit/?id=0c13e9c3c464), [89c82750da1](https://cgit.freebsd.org/src/commit/?id=89c82750da1), [18a870751b03](https://cgit.freebsd.org/src/commit/?id=18a870751b03), [ce9c325a2e92](https://cgit.freebsd.org/src/commit/?id=ce9c325a2e92), [cb3eac927b5d](https://cgit.freebsd.org/src/commit/?id=cb3eac927b5d), [5105e1ebecc7](https://cgit.freebsd.org/src/commit/?id=5105e1ebecc7), [b9b0e105c357](https://cgit.freebsd.org/src/commit/?id=b9b0e105c357), [929f5966a9fd](https://cgit.freebsd.org/src/commit/?id=929f5966a9fd)（由 FreeBSD 基金会赞助）

驱动 [rtw88(4)](https://man.freebsd.org/cgi/man.cgi?query=rtw88&sektion=4&format=html) 更新至 Linux v6.17。导致设备可能无法认证（**译注：指无法连接带密码的 WiFi**）的问题仍在研究中。[c1d365f39e08](https://cgit.freebsd.org/src/commit/?id=c1d365f39e08)（由 FreeBSD 基金会赞助）

驱动 [rtw89(4)](https://man.freebsd.org/cgi/man.cgi?query=rtw89&sektion=4&format=html) 更新至 Linux v6.17。该驱动尚未充分测试，仍可能存在问题。[b35044b38f74](https://cgit.freebsd.org/src/commit/?id=b35044b38f74)（由 FreeBSD 基金会赞助）

驱动 [iwlwifi(4)](https://man.freebsd.org/cgi/man.cgi?query=iwlwifi&sektion=4&format=html) 更新至 Linux v6.17。基于 BE200 的芯片组需要更新固件，并需要额外的驱动修复，而这些修复未包含在本版本中。[69caa1cf3ce5](https://cgit.freebsd.org/src/commit/?id=69caa1cf3ce5)（由 FreeBSD 基金会赞助）

### 运行时库和 API

现在 `libutil` 中的例程 [setusercontext(3)](https://man.freebsd.org/cgi/man.cgi?query=setusercontext&sektion=3&format=html) 会在适当条件下，从用户主目录下的 **.login.conf** 文件以及系统文件 [login.conf(5)](https://man.freebsd.org/cgi/man.cgi?query=login.conf&sektion=5&format=html) 设置进程优先级（nice，优）。优先级现在可以设为 `inherit`（继承），表示应保持与父进程相同的优先级。类似地，也可以将 umask 设为 `inherit`（继承）。[c328e6c6ccaa](https://cgit.freebsd.org/src/commit/?id=c328e6c6ccaa), [d162d7e2ad32](https://cgit.freebsd.org/src/commit/?id=d162d7e2ad32), [f2a0277d3e51](https://cgit.freebsd.org/src/commit/?id=f2a0277d3e51)（由 Kumacom SAS 赞助）
...
`libcxxrt` 更新至上游版本 6f2fdfebcd62。[d9901a23bd2f](https://cgit.freebsd.org/src/commit/?id=d9901a23bd2f)
...
`libc` 中的兼容性函数使使用旧版本 FreeBSD 编译的可执行文件/库得以运行。但那些自身使用兼容性系统调用的可执行文件/库未能正确引用这些调用，导致运行时出现异常。此问题现已修复。[47f5f89dbd27](https://cgit.freebsd.org/src/commit/?id=47f5f89dbd27)（由 FreeBSD 基金会赞助）

函数 [readdir_r(3)](https://man.freebsd.org/cgi/man.cgi?query=readdir_r&sektion=3&format=html) 已被弃用，并可能在后续版本中移除。现在在程序中使用它将导致编译时和链接时警告。[2bd157bc732a](https://cgit.freebsd.org/src/commit/?id=2bd157bc732a)（由 Klara, Inc. 赞助）
...
[vt(4)](https://man.freebsd.org/cgi/man.cgi?query=vt&sektion=4&format=html) 的 Gallant 字体更新，新增 4300 余个字形，包括对希腊字母、西里尔字母、国际音标扩展、扩展拉丁字符、Zapf Dingbats、大量箭头、大量数学符号、类字母符号及带圈字母数字、像素级完美方框绘制、货币符号、更多标点符号、足以表示“コンニチハ”（你好）的片假名，以及在 U+e0a0 私有使用区的 Powerline 字形。[9e8c1ab0976c](https://cgit.freebsd.org/src/commit/?id=9e8c1ab0976c)

Unicode 支持更新至 16.0.0，CLDR 更新至 45.0.0。[ddfc6f84f242](https://cgit.freebsd.org/src/commit/?id=ddfc6f84f242)
...
系统添加了 [syscons(4)](https://man.freebsd.org/cgi/man.cgi?query=syscons&sektion=4&format=html) 的弃用通知。[syscons(4)](https://man.freebsd.org/cgi/man.cgi?query=syscons&sektion=4&format=html) 与 UEFI 不兼容，不支持 UTF-8，并且使用 Giant 锁。目前尚无具体移除时间表，但对 Giant 锁的支持预计将在一到两个主要版本周期内取消。[8c922db4f3d9](https://cgit.freebsd.org/src/commit/?id=8c922db4f3d9)（由 FreeBSD 基金会赞助）

系统移除了工具 `shar`，但用户仍可通过 Ports [sysutils/freebsd-shar](https://cgit.freebsd.org/ports/tree/sysutils/freebsd-shar/) 使用。[3fde39073c72](https://cgit.freebsd.org/src/commit/?id=3fde39073c72)

由于安全性不足，OpenSSH 遵循上游更新，移除了 DSA 签名算法。

系统移除了数据库 [publickey(5)](https://man.freebsd.org/cgi/man.cgi?query=publickey&sektion=5&format=html)，该数据库使用 DES 加密，我们希望 2025 年没人会再使用它。[9197c04a251b](https://cgit.freebsd.org/src/commit/?id=9197c04a251b)

## 云支持

本节介绍针对云环境的变动。

15.0-RELEASE 支持 cloudinit，包括对 `nuageinit` 启动脚本和对 `config-drive` 分区的支持。它兼容 OpenStack 及许多托管服务。详见 [cloud-init](https://cloud-init.io/) 网站及提交记录 [16a6da44e28d](https://cgit.freebsd.org/src/commit/?id=16a6da44e28d) [227e7a205edf](https://cgit.freebsd.org/src/commit/?id=227e7a205edf)（由 OVHcloud 赞助）
...
系统已在 powerpc 上实现例程 `fpu_kern_enter` 和 `fpu_kern_leave`，使其可在内核中使用依赖浮点和向量寄存器的加密函数 [ossl(4)](https://man.freebsd.org/cgi/man.cgi?query=ossl&sektion=4&format=html)。[91e53779b4fc](https://cgit.freebsd.org/src/commit/?id=91e53779b4fc)

arm64 上的传统 PCI 热插拔得到支持。[355f02cddbf0](https://cgit.freebsd.org/src/commit/?id=355f02cddbf0)（由 Arm Ltd 赞助）

Jail 现在可以通过 [jail_set(2)](https://man.freebsd.org/cgi/man.cgi?query=jail_set&sektion=2&format=html) 和 [jail_get(2)](https://man.freebsd.org/cgi/man.cgi?query=jail_get&sektion=2&format=html) 中的 Jail 描述符访问，也可以通过新的系统调用 **jail_attach_jd(2)** 和 **jail_remove_jd(2)** 操作。它们可通过文件描述符接口管理 Jail，从而避免 Jail ID 固有的竞态条件，并可选择控制 Jail 生命周期。[851dc7f859c2](https://cgit.freebsd.org/src/commit/?id=851dc7f859c2)

Jail 和 Jail 描述符现在有关联的过滤器 [kevent(2)](https://man.freebsd.org/cgi/man.cgi?query=kevent&sektion=2&format=html)，这些过滤器可跟踪 Jail 的创建、变动、附加和移除。[1bd74d201a53](https://cgit.freebsd.org/src/commit/?id=1bd74d201a53) [9d7f89ef2607](https://cgit.freebsd.org/src/commit/?id=9d7f89ef2607)

为 MAC 模块的 Jail 参数创建了新的通用 `mac` 节点。后续所有 MAC 模块的 Jail 参数都会显示在该节点下。MAC 的介绍请参见 [mac(4)](https://man.freebsd.org/cgi/man.cgi?query=mac&sektion=4&format=html)。首个使用该节点的模块是 [mac_do(4)](https://man.freebsd.org/cgi/man.cgi?query=mac_do&sektion=4&format=html)。[5041b20503db](https://cgit.freebsd.org/src/commit/?id=5041b20503db), [f3a06ced2568](https://cgit.freebsd.org/src/commit/?id=f3a06ced2568)（由 FreeBSD 基金会赞助）
...
系统移除了 [sysctl(8)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&format=html) 控件 `hw.snd.version`。[7398d1ece5cf](https://cgit.freebsd.org/src/commit/?id=7398d1ece5cf)（由 FreeBSD 基金会赞助）

系统弃用了 [sound(4)](https://man.freebsd.org/cgi/man.cgi?query=sound&sektion=4&format=html) 中的 `unit.*` 代码，作为其中一部分，还移除了 [loader(8)](https://man.freebsd.org/cgi/man.cgi?query=loader&sektion=8&format=html) 可调项 `hw.snd.maxunit`。[25723d66369f](https://cgit.freebsd.org/src/commit/?id=25723d66369f)（由 FreeBSD 基金会赞助）

部分 AMD GPU 用户使用 Port `drm-kmod` 中 amdgpu DRM 驱动（自 v5.15 起，`graphics/drm-515-kmod` Port）时出现的运行变缓和卡死问题已得到修复。尤其是搭载 Green Sardine、Polaris 10/20 和 Vega 芯片的显卡用户受到影响。近期 Intel GPU（13 代及以上）用户可能也受到影响。[718d1928f874](https://cgit.freebsd.org/src/commit/?id=718d1928f874), [4ca9190251bb](https://cgit.freebsd.org/src/commit/?id=4ca9190251bb), [986edb19a49c](https://cgit.freebsd.org/src/commit/?id=986edb19a49c), [9d1f3ce79d85](https://cgit.freebsd.org/src/commit/?id=9d1f3ce79d85), [da257e519bc0](https://cgit.freebsd.org/src/commit/?id=da257e519bc0)（由 FreeBSD 基金会赞助）

内存域（NUMA）迭代代码已得到改进和修复，特别是降低了部分 DRM 驱动的图形操作延迟。[da257e519bc0](https://cgit.freebsd.org/src/commit/?id=da257e519bc0), [83ad6d8d8eee](https://cgit.freebsd.org/src/commit/?id=83ad6d8d8eee), [b15ff7214020](https://cgit.freebsd.org/src/commit/?id=b15ff7214020)（由 FreeBSD 基金会赞助）
...
默认情况下，无特权进程不再能够调试、调度或向子 Jail 中属于同一 UID 的进程发送信号。系统添加了 [jail(8)](https://man.freebsd.org/cgi/man.cgi?query=jail&sektion=8&format=html) 设置 `allow.unprivileged_parent_tampering`，用于在父 Jail 和子 Jail 中的 UID 预期为同一用户时恢复先前行为。[8a5ceebece03](https://cgit.freebsd.org/src/commit/?id=8a5ceebece03)
...
在 amd64 平台上，系统重新设计了 `%fsbase`/`%gsbase` 寄存器及 TLS 基址的处理，这一改动使其对直接操作 CPU 上下文的应用程序更为有用。[68ba38dad3](https://cgit.freebsd.org/src/commit/?id=68ba38dad3)（由 FreeBSD 基金会赞助）
...
终端接口 [tty(4)](https://man.freebsd.org/cgi/man.cgi?query=tty&sektion=4&format=html) 现支持标志 `IUTF8`，可启用正确的 UTF-8 退格处理，该标志默认启用，适用于默认 UTF-8 本地化环境。[bb830e346bd5](https://cgit.freebsd.org/src/commit/?id=bb830e346bd5)
...
针对 Intel WiFi 设备的 [iwlwifi(4)](https://man.freebsd.org/cgi/man.cgi?query=iwlwifi&sektion=4&format=html) 驱动经过大量稳定性改进。（由 FreeBSD 基金会赞助）
...
`snd_clone` 框架已从声卡子系统中移除，包括相关 sysctl，使系统更简化。系统不再创建每通道节点（**/dev/dspX.Y**），仅保留主设备节点（**/dev/dspX**）。[e6c51f6db8d7](https://cgit.freebsd.org/src/commit/?id=e6c51f6db8d7)（由 FreeBSD 基金会赞助）
...
`ice_ddp` 更新到 1.3.41.0。[a9d78bb714e3](https://cgit.freebsd.org/src/commit/?id=a9d78bb714e3)（由英特尔赞助）
...
[nvme(4)](https://man.freebsd.org/cgi/man.cgi?query=nvme&sektion=4&format=html) 和 [nvmecontrol(8)](https://man.freebsd.org/cgi/man.cgi?query=nvmecontrol&sektion=8&format=html) 现已在所有架构上启用。[24687a65dd7f](https://cgit.freebsd.org/src/commit/?id=24687a65dd7f), [aba2d7f89dcf](https://cgit.freebsd.org/src/commit/?id=aba2d7f89dcf)（由 Chelsio Communications 和 Netflix 赞助）

驱动 [mpi3mr(4)](https://man.freebsd.org/cgi/man.cgi?query=mpi3mr&sektion=4&format=html) 版本更新至 8.14.0.2.0。[e6d4b221ba7c](https://cgit.freebsd.org/src/commit/?id=e6d4b221ba7c)

[mpi3mr(4)](https://man.freebsd.org/cgi/man.cgi?query=mpi3mr&sektion=4&format=html) 的 MPI Header 更新至版本 36，与最新的 MPI 规范保持一致，包括更新的结构、字段定义和与最新固件兼容所需的常量。[60cf1576501d](https://cgit.freebsd.org/src/commit/?id=60cf1576501d)

[mpi3mr(4)](https://man.freebsd.org/cgi/man.cgi?query=mpi3mr&sektion=4&format=html) 驱动现已纳入 **GENERIC** 配置。[e2b8fb2202c2](https://cgit.freebsd.org/src/commit/?id=e2b8fb2202c2)
...
[ena(4)](https://man.freebsd.org/cgi/man.cgi?query=ena&sektion=4&format=html) 驱动版本更新至 v2.8.1。[a1685d25601e](https://cgit.freebsd.org/src/commit/?id=a1685d25601e)（由亚马逊赞助）
...
在 BIOS 启动时，[smbios(4)](https://man.freebsd.org/cgi/man.cgi?query=smbios&sektion=4&format=html) 现在也会优先搜索 SMBIOS v3（64 位）入口点。这使得在仅提供 v3 表的 BIOS（如 Hetzner 虚拟机）上能够检测并报告正确的 SMBIOS 版本。对于同时提供 v2 和 v3 表的机器，也会优先使用 v3 表，与 EFI 启动情况一致。[bc7f6508363c](https://cgit.freebsd.org/src/commit/?id=bc7f6508363c)（由 FreeBSD 基金会赞助）
...
新增驱动 [udbc(4)](https://man.freebsd.org/cgi/man.cgi?query=udbc&sektion=4&format=html)，可通过 xHC 调试对目标设备进行主机端调试。[d566b6a70bcb](https://cgit.freebsd.org/src/commit/?id=d566b6a70bcb)（由 FreeBSD 基金会赞助）
...
[ichwd(4)](https://man.freebsd.org/cgi/man.cgi?query=ichwd&sektion=4&format=html) 驱动现已加入对 Intel 6300ESB I/O 控制器集线器看门狗定时器的支持。这主要面向 QEMU 用户，在该场景下该看门狗定时器作为 x86 虚拟机的默认且唯一定时器。[2b74ff5fceb6](https://cgit.freebsd.org/src/commit/?id=2b74ff5fceb6623f6)
...
总线驱动 [agp(4)](https://man.freebsd.org/cgi/man.cgi?query=agp&sektion=4&format=html) 已被弃用，并计划在 FreeBSD 16.0 中移除。[92af7c97e197](https://cgit.freebsd.org/src/commit/?id=92af7c97e197) [cadadd1a0398](https://cgit.freebsd.org/src/commit/?id=cadadd1a0398)

IBM PC 软盘控制器 [fdc(4)](https://man.freebsd.org/cgi/man.cgi?query=fdc&sektion=4&format=html) 及相关工具已被弃用，并计划在 FreeBSD 16.0 中移除。[4c736cfc69a7](https://cgit.freebsd.org/src/commit/?id=4c736cfc69a7)（由 FreeBSD 基金会赞助）

[firewire(4)](https://man.freebsd.org/cgi/man.cgi?query=firewire&sektion=4&format=html) 总线及相关驱动已被弃用，并计划在 FreeBSD 16.0 中移除。[fc889167c319](https://cgit.freebsd.org/src/commit/?id=fc889167c319)（由 FreeBSD 基金会赞助）

以太网驱动 [le(4)](https://man.freebsd.org/cgi/man.cgi?query=le&sektion=4&format=html) 已被弃用，并计划在 FreeBSD 16.0 中移除。[e4d6433e9c03](https://cgit.freebsd.org/src/commit/?id=e4d6433e9c03)（由 FreeBSD 基金会赞助）

[syscons(4)](https://man.freebsd.org/cgi/man.cgi?query=syscons&sektion=4&format=html) 已计划在后续版本中移除，其手册页已标记为已弃用，以提示用户迁移到 [vt(4)](https://man.freebsd.org/cgi/man.cgi?query=vt&sektion=4&format=html)。[2bc5b1d60512](https://cgit.freebsd.org/src/commit/?id=2bc5b1d60512)（由 FreeBSD 基金会赞助）

USB 802.11g 驱动 [upgt(4)](https://man.freebsd.org/cgi/man.cgi?query=upgt&sektion=4&format=html) 已被弃用，并计划在 FreeBSD 16.0 中移除。[7f8a5c5a1585](https://cgit.freebsd.org/src/commit/?id=7f8a5c5a1585)（由 FreeBSD 基金会赞助）
...
新增对 NVMe 命名空间动态调整大小的支持。[nvd(4)](https://man.freebsd.org/cgi/man.cgi?query=nvd&sektion=4&format=html) 和 [nda(4)](https://man.freebsd.org/cgi/man.cgi?query=nda&sektion=4&format=html) 驱动现已能够实时通知 geom 设备大小的变化。[86d3ec359a56](https://cgit.freebsd.org/src/commit/?id=86d3ec359a56)（由 Netflix 赞助）
...
UFS1 文件系统的 2038 年 1 月 19 日日期限制已延迟至 2106 年 2 月 7 日。仅影响旧 UFS1 格式的文件系统。详见提交说明。[1111a44301da](https://cgit.freebsd.org/src/commit/?id=1111a44301da)

### ZFS

为 `VOP_COPY_FILE_RANGE()` 新增块克隆支持。目前，仅有启用块克隆的 ZFS 文件系统支持此功能，NFSv4.2 也支持。详见 **pathconf(2)** 和 **copy_file_range(2)**。[37b2cb5ecb0f](https://cgit.freebsd.org/src/commit/?id=37b2cb5ecb0f)

### GEOM

对 vinum 卷的支持移除。[f87bb5967670](https://cgit.freebsd.org/src/commit/?id=f87bb5967670), [e51036fbf3f8](https://cgit.freebsd.org/src/commit/?id=e51036fbf3f8)

## 引导加载程序变更

本节涵盖引导加载程序、启动菜单及其他启动相关的变动。
...
现在，[loader(8)](https://man.freebsd.org/cgi/man.cgi?query=loader&sektion=8&format=html) 在读取其他配置文件后，会读取变量 `local_loader_conf_files` 中列出的本地配置文件，默认值为 **/boot/loader.conf.local**。[a25531db0fc2](https://cgit.freebsd.org/src/commit/?id=a25531db0fc2)
...
在 EFI 系统上，[loader(8)](https://man.freebsd.org/cgi/man.cgi?query=loader&sektion=8&format=html) 的控制台检测已得到改进。如果没有 ConOut 变量，则检查 ConIn；如果发现多个设备，则优先使用串口设备。[20a6f4779ac6](https://cgit.freebsd.org/src/commit/?id=20a6f4779ac6)（由 Netflix 赞助）

现在，[loader(8)](https://man.freebsd.org/cgi/man.cgi?query=loader&sektion=8&format=html) 的帧缓冲支持能使用仅文本的视频驱动，从而节省空间。[57ca2848c0aa](https://cgit.freebsd.org/src/commit/?id=57ca2848c0aa)（由 Netflix 赞助）
...
现在在 aarch64 和 amd64 上，通过 LinuxBoot 引导加载程序可以从 Linux 启动 FreeBSD。[46010641267](https://cgit.freebsd.org/src/commit/?id=46010641267)（由 Netflix 赞助）

BIOS 引导加载程序重新增加了对 gzip 和 bzip2 的支持，但默认移除了图形模式支持，以解决体积问题。（EFI 引导加载程序保持不变，仍支持所有模式）[4d3b05a8530e](https://cgit.freebsd.org/src/commit/?id=4d3b05a8530e)（由 Netflix 赞助）

现在，BIOS 引导加载程序能在表位于 4GB 以下时使用 SMBIOS v3（64 位）入口点。由于 BIOS 引导加载程序作为 BTX 客户端在 amd64 上仍为 32 位，因此无法访问 4GB 以上地址，但 64 位入口点可能引用 4GB 以下的结构表，可在 BIOS 未提供 32 位入口点时使用，例如 Hetzner 虚拟机。[7f005c6699f4](https://cgit.freebsd.org/src/commit/?id=7f005c6699f4)（由 FreeBSD 基金会赞助）

现在 BIOS 引导加载程序将优先使用 SMBIOS v3（64 位）入口点。当 32 位和 64 位入口点同时存在时，SMBIOS 规范规定 64 位入口点至少包含 32 位入口点引用的所有结构，即 32 位入口点仅为兼容性提供，而 64 位入口点更可能包含完整有效值。[3f744fb8b2c5](https://cgit.freebsd.org/src/commit/?id=3f744fb8b2c5)（由 FreeBSD 基金会赞助）

EFI 引导加载程序现在同样优先使用 SMBIOS v3（64 位）入口点，与 BIOS 启动保持一致。不同之处在于，EFI 加载器在 64 位平台上运行，因此 v3 入口点的结构表不受 4GB 限制。[96f77576e9ea](https://cgit.freebsd.org/src/commit/?id=96f77576e9ea)（由 FreeBSD 基金会赞助）
...
对 802 标准网络的 ARP（[arp(4)](https://man.freebsd.org/cgi/man.cgi?query=arp&sektion=4&format=html)）支持恢复；此前在移除 FDDI 支持时该支持被意外删除（这与以太网标准封装不同）。[d776dd5fbd48](https://cgit.freebsd.org/src/commit/?id=d776dd5fbd48)
...
[igc(4)](https://man.freebsd.org/cgi/man.cgi?query=igc&sektion=4&format=html) 驱动新增了 AIM（自适应中断调节）支持。[472a0ccf847a](https://cgit.freebsd.org/src/commit/?id=472a0ccf847a)（由 Rubicon Communications, LLC（“Netgate”）和 BBOX.io 赞助）

该功能也添加到了 [lem(4)](https://man.freebsd.org/cgi/man.cgi?query=lem&sektion=4&format=html)、[em(4)](https://man.freebsd.org/cgi/man.cgi?query=em&sektion=4&format=html) 和 [igb(4)](https://man.freebsd.org/cgi/man.cgi?query=igb&sektion=4&format=html) 驱动中。在 FreeBSD 12.0 中引入的 UDP 性能重大回退（包括通过 UDP 的 NFS）被认为已通过此变动修复。[49f12d5b38f6](https://cgit.freebsd.org/src/commit/?id=49f12d5b38f6)（由 Rubicon Communications, LLC（“Netgate”）和 BBOX.io 赞助）

[ip6addrctl(8)](https://man.freebsd.org/cgi/man.cgi?query=ip6addrctl&sektion=8&format=html) 现可附加并在 Jail 中运行。这将使管理 vnet Jail 的地址选择策略更加容易，尤其适用于轻量级 OCI 容器或精简 Jail。[b709f7b38cc4](https://cgit.freebsd.org/src/commit/?id=b709f7b38cc4)
...
[pf(4)](https://man.freebsd.org/cgi/man.cgi?query=pf&sektion=4&format=html) 新增了路由到池（route-to pool）选项“prefer-ipv6-nexthop”，允许通过 IPv6 网关路由 IPv4 数据包。[65c318630123](https://cgit.freebsd.org/src/commit/?id=65c318630123) [d2761422eb0a](https://cgit.freebsd.org/src/commit/?id=d2761422eb0a)（由 InnoGames GmbH 赞助）
...
协议 [pfsync(4)](https://man.freebsd.org/cgi/man.cgi?query=pfsync&sektion=4&format=html) 更新，可同步多个缺失属性。这修复了带有选项 route-to、af-to、rtable、dummynet、tags 和 scrub 的状态同步问题。如果需要与旧版本 FreeBSD 进行同步，可通过 `ifconfig pfsync0 version $VERSION` 配置协议版本。对于 `$VERSION`：13.X 版本使用 1301，14.X 版本使用 1400。对于运行 FreeBSD 15.0 的主机之间的同步，默认值为 `1500`。[99475087d63b](https://cgit.freebsd.org/src/commit/?id=99475087d63b)（由 InnoGames GmbH 赞助）
...
支持 FreeBSD 7 和 8 的 [ipfw(8)](https://man.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&format=html) 二进制兼容代码移除。[660255be1ed9](https://cgit.freebsd.org/src/commit/?id=660255be1ed9)（由 FreeBSD 基金会赞助）

为 [if_bridge(4)](https://man.freebsd.org/cgi/man.cgi?query=if_bridge&sektion=4&format=html)、[if_epair(4)](https://man.freebsd.org/cgi/man.cgi?query=if_epair&sektion=4&format=html) 及其他接口生成的稳定 MAC 地址将在 15.0 中发生变化，原因是实现中存在严重缺陷，导致高概率冲突，且部分生成的地址超出了预期的 OUI 范围。如果需要保留先前的 MAC 地址（例如用于 DHCP 租约），需注意持久化现有的 MAC 地址，例如在 [rc.conf(5)](https://man.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5&format=html) 中使用 `ifconfig_bridgeN="... ether aa:bb:cc:dd:ee:ff"`。[604d34c23f77](https://cgit.freebsd.org/src/commit/?id=604d34c23f77)

### 网络协议

网络栈已有大量改进，如 [sctp(4)](https://man.freebsd.org/cgi/man.cgi?query=sctp&sektion=4&format=html) 栈的性能提升和错误修复。
...
sysctl 变量 `net.inet.tcp.nolocaltimewait` 的默认值已从 `1` 改为 `0`。这意味着 FreeBSD 不再跳过远程地址为本地的端点的 `TIME_WAIT` 状态。新的 sysctl 变量 `net.inet.tcp.msl_local` 可用于控制这些端点在 `TIME_WAIT` 状态下的停留时间。`net.inet.tcp.nolocaltimewait` 已弃用，计划在 FreeBSD 16 中移除。[c3fc0db3bc50](https://cgit.freebsd.org/src/commit/?id=c3fc0db3bc50)（由 Netflix 赞助）

本地流套接字（`AF_UNIX/SOCK_STREAM`）和有序数据包流套接字（`AF_UNIX/SOCK_SEQPACKET`）已得到改进，可提升批量传输和往返时间性能。套接字 `SOCK_SEQPACKET` 已符合规范，现在表现为真正的流套接字，而在旧版 FreeBSD 中，它可能表现出数据报套接字的特性。依赖旧实现漏洞的应用可能需要调整。[d15792780760](https://cgit.freebsd.org/src/commit/?id=d15792780760)
...
继其他驱动之后，[iwlwififw(4)](https://man.freebsd.org/cgi/man.cgi?query=iwlwififw&sektion=4&format=html) 固件已从基本系统中移除，改为使用 Port 方式并支持 [fwget(8)](https://man.freebsd.org/cgi/man.cgi?query=fwget&sektion=8&format=html)。在从旧版本更新时，用户必须先安装固件包。（由 FreeBSD 基金会赞助）
...
在使用启动 ROM（即不使用 [bhyveload(8)](https://man.freebsd.org/cgi/man.cgi?query=bhyveload&sektion=8&format=html)）运行 [bhyve(8)](https://man.freebsd.org/cgi/man.cgi?query=bhyve&sektion=8&format=html) 虚拟机时，bhyve 现在假设启动 ROM 会启用 PCI BAR 解码。这与某些启动 ROM（尤其是过时的 `edk2-bhyve` 构建）不兼容。要恢复旧行为，请在 bhyve 配置中添加 `pci.enable_bars='true'`。注意，已将软件包 `uefi-edk2-bhyve` 重命名为 `edk2-bhyve`。[e962b37bf0ff](https://cgit.freebsd.org/src/commit/?id=e962b37bf0ff)（由创新英国赞助）
...
NVMM 虚拟机监控程序现已能被识别。[34f40baca641](https://cgit.freebsd.org/src/commit/?id=34f40baca641)

在 Hyper-V 下，TLB 刷新现在使用 hypercall 而非 IPI，从而最多提升了 40% 的 TLB 性能。[7ece5993b787](https://cgit.freebsd.org/src/commit/?id=7ece5993b787)（由微软赞助）

### Linux 二进制兼容层

对于所有二进制兼容层的 `stat()` 变体，标志 `AT_NO_AUTOMOUNT` 现在会被忽略（因为该标志指定的行为已与 FreeBSD 相符），从而提升了 Linux 应用的兼容性。[99d3ce80ba07](https://cgit.freebsd.org/src/commit/?id=99d3ce80ba07)（由 FreeBSD 基金会赞助）

Linux 系统调用 [inotify(2)](https://man.freebsd.org/cgi/man.cgi?query=inotify&sektion=2&format=html) 现已在 Linux 二进制兼容层中实现。（由 Klara, Inc. 赞助）

## 多媒体

音频栈有多项改进，如在 [mixer(8)](https://man.freebsd.org/cgi/man.cgi?query=mixer&sektion=8&format=html) 中支持热插拔，以及新增 [mididump(1)](https://man.freebsd.org/cgi/man.cgi?query=mididump&sektion=1&format=html) 工具。[cf9d2fb18433](https://cgit.freebsd.org/src/commit/?id=cf9d2fb18433)（由 FreeBSD 基金会赞助） [7224e9f2d4af](https://cgit.freebsd.org/src/commit/?id=7224e9f2d4af)（由 FreeBSD 基金会赞助）

新增实用工具 [sndctl(8)](https://man.freebsd.org/cgi/man.cgi?query=sndctl&sektion=8&format=html)，将查看和操作音频设备设置（sysctls、**/dev/sndstat**）的各类接口集中到一个工具中，其控制界面类似于 **mixer(8)**。[44e5a0150835](https://cgit.freebsd.org/src/commit/?id=44e5a0150835), [9a37f1024ceb](https://cgit.freebsd.org/src/commit/?id=9a37f1024ceb)（由 FreeBSD 基金会赞助）

`virtual_oss` 导入到基本系统。从 FreeBSD 15.0 开始，将不再构建 Port `audio/virtual_oss`。对于用户而言，唯一实际变化是安装过程：基本系统已提供所有功能，以下可选组件仍可通过 Ports 安装：
...
[builtin(1)](https://man.freebsd.org/cgi/man.cgi?query=builtin&sektion=1&format=html) 手册重写，信息更加简洁，并新增了 FreeBSD CLI 内置快捷键的章节。[42df4faf7004](https://cgit.freebsd.org/src/commit/?id=42df4faf7004)
...
手册 [build(7)](https://man.freebsd.org/cgi/man.cgi?query=build&sektion=7&format=html) 更新，加入了从源代码构建系统的说明。[275f61111f43](https://cgit.freebsd.org/src/commit/?id=275f61111f435)

[ccdconfig(8)](https://man.freebsd.org/cgi/man.cgi?query=ccdconfig&sektion=8&format=html) 现建议使用 [graid(8)](https://man.freebsd.org/cgi/man.cgi?query=graid&sektion=8&format=html) 和 [zfs(8)](https://man.freebsd.org/cgi/man.cgi?query=zfs&sektion=8&format=html) 替代 [gvinum(8)](https://man.freebsd.org/cgi/man.cgi?query=gvinum&sektion=8&format=html)。[55cb3a33d920](https://cgit.freebsd.org/src/commit/?id=55cb3a33d920)

[ps(1)](https://man.freebsd.org/cgi/man.cgi?query=ps&sektion=1&format=html) 手册重写，解释了基本原理，并更新了其中的说明来匹配实际情况。前言部分已改写，对 [ps(1)](https://man.freebsd.org/cgi/man.cgi?query=ps&sektion=1&format=html) 命令的各个方面提供了全面概述。若干选项和关键词的说明已修正来匹配其实际行为。还增补了 STANDARDS 和 BUGS 部分。[ddf144a04b53](https://cgit.freebsd.org/src/commit/?id=ddf144a04b53)（由 FreeBSD 基金会赞助）

[mac_do(4)](https://man.freebsd.org/cgi/man.cgi?query=mac_do&sektion=4&format=html) 手册页重写，以支持将多个用户和组作为单条规则的目标，这导致规则语法发生了变化。特别地，新增了 JAIL SUPPORT 和 SECURITY CONSIDERATIONS 部分。[bc201841d139](https://cgit.freebsd.org/src/commit/?id=bc201841d139)（由 FreeBSD 基金会赞助）

[mdo(1)](https://man.freebsd.org/cgi/man.cgi?query=mdo&sektion=1&format=html) 手册页的现有内容已增补，以记录对目标凭证中完全指定所有用户和组的新支持。现在的介绍部分更长，并新增了 SECURITY CONSIDERATIONS 部分。[20ebb6ec5ac0](https://cgit.freebsd.org/src/commit/?id=20ebb6ec5ac0)（由 FreeBSD 基金会赞助）（由谷歌（2025 编程之夏）赞助）
...
Lua [loader(8)](https://man.freebsd.org/cgi/man.cgi?query=loader&sektion=8&format=html) 模块的手册页已重新改写，优化了 [apropos(1)](https://man.freebsd.org/cgi/man.cgi?query=apropos&sektion=1&format=html) 的搜索结果。[5d59c1b4f14e](https://cgit.freebsd.org/src/commit/?id=5d59c1b4f14e)
...
本节介绍 FreeBSD Ports、软件包基础设施以及软件包管理和安装工具的变化。
...
在 **/etc/pkg/FreeBSD.conf** 中定义的仓库 `FreeBSD` 和 `FreeBSD-kmods` 已分别重命名为 `FreeBSD-ports` 和 `FreeBSD-ports-kmods`。若用户在 **/usr/local/etc/pkg/repos** 中覆盖了这些仓库，需要调整配置来匹配新名称。
