...
现在工具 [ps(1)](https://man.freebsd.org/cgi/man.cgi?query=ps&sektion=1&format=html) 的标志 `-U` 会按真实用户 ID 选择进程。这符合 POSIX 对 `-U` 的规范，也是大多数用户在大多数情况下真正需要的行为。此前，`-U` 会按有效用户 ID 选择进程（这是 POSIX 对 `-u` 选项的要求）。[995b690d1398](https://cgit.freebsd.org/src/commit/?id=995b690d1398)（由 FreeBSD 基金会赞助）
