...
现在命令 [jail(8)] 支持了参数 meta 和 env，它们是与 Jail 关联的任意字符串。这些参数可用于给 Jail 添加特定元数据，或在 Jail 内安全传递信息。用户可以在创建 Jail 时添加这些参数，也可以稍后使用 [jail(8)] 进行修改。[30e6e008bc06](https://cgit.freebsd.org/src/commit/?id=30e6e008bc06)（由 SkunkWerks, GmbH 赞助）
