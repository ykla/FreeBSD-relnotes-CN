...
要将 FreeBSD/arm 镜像写入 SD 卡，使用 [dd(1)](https://man.freebsd.org/cgi/man.cgi?query=dd&sektion=1&format=html) 工具，将 _KERNEL_ 替换为系统对应的内核配置名。
...
* Google(R) Compute Engine(TM):
  ...
  将 _INSTANCE_ 替换为 Google Compute Engine 实例的名称，将 _FILESYSTEM_ 替换为 `ufs` 或 `zfs`。
