# FreeBSD 15.0-RELEASE 发行公告

日期：2025 年 12 月 2 日

FreeBSD 发布工程团队很高兴地宣布 FreeBSD 15.0-RELEASE 现已发布。这是 stable/15 分支的第一个版本。

部分亮点：

* FreeBSD“base”系统现在可以使用 [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html) 软件包管理器进行安装和管理（参见下文 pkgbase）。

* FreeBSD 15.0 的发行版构件（安装镜像、虚拟机镜像等）全部在无需 root 权限的情况下生成。

* FreeBSD 现已具有原生的 inotify 实现，简化了目录监视和软件移植。

* OpenZFS 已升级到 2.4.0-rc4。

* OpenSSL 已升级到最新的长期支持（LTS）版本 3.5.4，包含对 QUIC 及现已标准化的抗量子算法 ML-KEM、ML-DSA 和 SLH-DSA 的支持。

* OpenSSH 已升级到 10.0p2，默认支持抗量子密钥协商。

完整的新功能、支持的硬件和已知问题列表，请参阅在线发行说明、硬件兼容性说明和勘误列表，地址如下：

* [https://www.FreeBSD.org/releases/15.0R/relnotes/](https://www.FreeBSD.org/releases/15.0R/relnotes/)
* [https://www.FreeBSD.org/releases/15.0R/hardware/](https://www.FreeBSD.org/releases/15.0R/hardware/)
* [https://www.FreeBSD.org/releases/15.0R/errata/](https://www.FreeBSD.org/releases/15.0R/errata/)
...
FreeBSD 15.0 的一项重大变更是引入了使用 [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html) 软件包管理器安装和管理基本系统的新方法。

在开发期间，此方法通常被称为“pkgbase”。

安装过程中，[bsdinstall(8)](https://man.freebsd.org/cgi/man.cgi?query=bsdinstall&sektion=8&format=html) 会提示用户在两种安装方法之间选择：

1. 发行版集合（传统方法）：这是先前 FreeBSD 发行版中使用的方法。
   以此方式安装的系统继续使用 [freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 实用程序进行更新。 对发行版集合的支持计划在 FreeBSD 16 中移除，但在 FreeBSD 15 stable 分支的整个生命周期内将继续支持（同时支持 freebsd-update）。

2. 软件包（pkgbase / 新方法）：基本系统作为一组软件包从“FreeBSD-base”仓库安装。
   以此方式安装的系统完全使用 [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html) 工具管理。 此方法默认用于所有虚拟机镜像和发布到公共云的镜像。 在 FreeBSD 15.0 中，pkgbase 作为技术预览提供，但预计在未来的发行版中将成为管理基本系统安装和升级的标准方法。

## 获取途径

FreeBSD 15.0-RELEASE 现已支持 amd64、aarch64、armv7、powerpc64、powerpc64le 和 riscv64 架构。

FreeBSD 15.0-RELEASE 可从可启动 ISO 镜像或通过网络安装。部分架构还支持从 USB 闪存盘安装。所需文件可按以下说明下载。

本消息底部包含发行版 ISO、闪存盘和 SD 卡镜像的 SHA512 和 SHA256 校验和。

发行版镜像的 PGP 签名校验和也可从以下地址获取：

* [https://www.FreeBSD.org/releases/15.0R/signatures/](https://www.FreeBSD.org/releases/15.0R/signatures/)

本公告的 PGP 签名版本可在以下地址获取：

* [https://www.FreeBSD.org/releases/15.0R/announce.asc](https://www.FreeBSD.org/releases/15.0R/announce.asc)
...
包含 FreeBSD 基本系统。还支持启动到基于“livefs”的救援模式。不含预构建软件包。
...
此外，可将其写入 USB 闪存盘（U 盘）用于 amd64 架构，在可从 USB 启动的机器上进行安装。还支持启动到基于“livefs”的救援模式。不含预构建软件包。
...
可写入 USB 闪存盘（U 盘），用于在可从 USB 启动的机器上进行安装。还支持启动到基于“livefs”的救援模式。不含预构建软件包。
...
可写入 USB 闪存盘（U 盘）用于启动机器，但介质本身不包含安装发行版，与 bootonly 镜像类似。还支持启动到基于“livefs”的救援模式。不含预构建软件包。
...
可写入 SD 卡用于启动受支持的 arm 系统。SD 卡镜像包含完整的 FreeBSD 安装，可安装到小至 5 GB 的 SD 卡。

为方便无控制台访问系统的用户，默认提供 `freebsd` 用户（密码为 `freebsd`）用于 [ssh(1)](https://man.freebsd.org/cgi/man.cgi?query=ssh&sektion=1&format=html) 访问。此外，`root` 用户密码设为 `root`；强烈建议获得系统访问权限后立即更改这两位用户的密码。

要将 FreeBSD/arm 镜像写入 SD 卡，使用 [dd(1)](https://man.freebsd.org/cgi/man.cgi?query=dd&sektion=1&format=html) 工具，将 *KERNEL* 替换为系统对应的内核配置名。
...
FreeBSD 15.0-RELEASE 还可从多家供应商购买 DVD 介质。我们预期以下供应商之一将提供基于 FreeBSD 15.0 的产品：

* FreeBSD Mall, Inc. [https://www.freebsdmall.com](https://www.freebsdmall.com)

amd64（x86_64）、AArch64（arm64）和 RISCV（riscv64）架构还提供预安装的虚拟机镜像，格式包括 `QCOW2`、`VHD` 和 `VMDK` 磁盘镜像格式，以及 raw（未格式化）镜像。

FreeBSD 15.0-RELEASE 还可在以下云托管平台上获取：

* FreeBSD Amazon EC2(TM):
  ...
  AMI 预计将在不久的将来在 AWS Marketplace 提供：
...
* Google Compute Engine(TM):
  ...
  将 *INSTANCE* 替换为 Google Compute Engine 实例的名称，将 *FILESYSTEM* 替换为 `ufs` 或 `zfs`。

  FreeBSD 镜像预计也将在不久的将来在 Google Compute Engine(TM) Marketplace 提供：[https://console.cloud.google.com/marketplace/product/freebsd-cloud/freebsd-15](https://console.cloud.google.com/marketplace/product/freebsd-cloud/freebsd-15)

* Microsoft Azure(TM):

  FreeBSD 虚拟机镜像可在 Azure Marketplace 获取：[https://marketplace.microsoft.com/product/virtual-machine/freebsd.freebsd-15_0](https://marketplace.microsoft.com/product/virtual-machine/freebsd.freebsd-15_0)

* Oracle Cloud Infrastructure:

  FreeBSD 虚拟机镜像预计将在不久的将来在 Oracle Cloud Infrastructure Marketplace 提供，支持 amd64 和 aarch64 架构，包括 UFS 和 ZFS 文件系统：[https://cloudmarketplace.oracle.com/marketplace/app/freebsd-release](https://cloudmarketplace.oracle.com/marketplace/app/freebsd-release)

## 下载

FreeBSD 15.0-RELEASE 可从以下站点通过 https 下载：

* [https://download.freebsd.org/releases/ISO-IMAGES/15.0/](https://download.freebsd.org/releases/ISO-IMAGES/15.0/)

FreeBSD 15.0-RELEASE 虚拟机镜像可从以下地址下载：

* [https://download.freebsd.org/releases/VM-IMAGES/15.0-RELEASE/](https://download.freebsd.org/releases/VM-IMAGES/15.0-RELEASE/)

FreeBSD 15.0-RELEASE BASIC-CI 镜像可从以下地址下载：

* [https://download.freebsd.org/releases/CI-IMAGES/15.0-RELEASE/](https://download.freebsd.org/releases/CI-IMAGES/15.0-RELEASE/)

FreeBSD 15.0-RELEASE OCI 容器镜像可从以下地址下载：

* [https://download.freebsd.org/releases/OCI-IMAGES/15.0-RELEASE/](https://download.freebsd.org/releases/OCI-IMAGES/15.0-RELEASE/)

也可在 Docker 和 GitHub 容器仓库中获取。

关于安装 FreeBSD 或将现有机器更新到 15.0-RELEASE 的说明，请参阅：

* [https://www.FreeBSD.org/releases/15.0R/installation/](https://www.FreeBSD.org/releases/15.0R/installation/)

## 支持

本版本 FreeBSD 15.0-RELEASE 将受支持至 2026 年 9 月 30 日。FreeBSD 15 发布系列将受支持至 2029 年 12 月 31 日。更多支持信息可在以下地址找到：
...
许多公司为支持 FreeBSD 15.0 的发布工程活动捐赠了设备、网络访问或人力，包括：
...
| [Your.org](https://www.your.org) |

15.0-RELEASE 的发布工程团队包括：
...
| Jake Freeland <jfree@FreeBSD.org> | 发布工程 |
| Xin Li <delphij@FreeBSD.org> | 发布工程、安全团队联络 |
| Ed Maste <emaste@FreeBSD.org> | 发布工程副负责人 |
| Mahdi Mokhtari <mmokhi@FreeBSD.org> | 发布工程 |
| Colin Percival <cperciva@FreeBSD.org> | 发布工程负责人、15.0-RELEASE 发布工程师 |
| Muhammad Moinur Rahman <bofh@FreeBSD.org> | 发布工程 |
| Lexi Winter <ivy@FreeBSD.org> | 发布工程、pkgbase 维护者 |
...
喜爱 FreeBSD？请向 FreeBSD 基金会[捐赠](https://www.freebsdfoundation.org/donate/)，支持本版本及未来的发行版！
