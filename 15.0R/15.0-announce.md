...
FreeBSD 发布工程团队很高兴地宣布 FreeBSD 15.0-RELEASE 现已发布。
这是 stable/15 分支的第一个版本。
...
1. 发行版集合（传统方法）：这是先前 FreeBSD 发行版中使用的方法。
   以此方式安装的系统继续使用 [freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 实用程序进行更新。
   对发行版集合的支持计划在 FreeBSD 16 中移除，但在 FreeBSD 15 stable 分支的整个生命周期内将继续支持（同时支持 freebsd-update）。

2. 软件包（pkgbase / 新方法）：基本系统作为一组软件包从“FreeBSD-base”仓库安装。
   以此方式安装的系统完全使用 [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html) 工具管理。
   此方法默认用于所有虚拟机镜像和发布到公共云的镜像。
   在 FreeBSD 15.0 中，pkgbase 作为技术预览提供，但预计在未来的发行版中将成为管理基本系统安装和升级的标准方法。

## 获取途径

FreeBSD 15.0-RELEASE 现已支持 amd64、aarch64、armv7、powerpc64、powerpc64le 和 riscv64 架构。

FreeBSD 15.0-RELEASE 可从可启动 ISO 镜像或通过网络安装。
部分架构还支持从 USB 闪存盘安装。
所需文件可按以下说明下载。
...
FreeBSD 15.0-RELEASE 还可从多家供应商购买 DVD 介质。
我们预期以下供应商之一将提供基于 FreeBSD 15.0 的产品：
...
* Google Compute Engine(TM):
  ...
  FreeBSD 镜像预计也将在不久的将来在 Google Compute Engine(TM) Marketplace 提供：
  [https://console.cloud.google.com/marketplace/product/freebsd-cloud/freebsd-15](https://console.cloud.google.com/marketplace/product/freebsd-cloud/freebsd-15)

* Microsoft Azure(TM):

  FreeBSD 虚拟机镜像可在 Azure Marketplace 获取：
  [https://marketplace.microsoft.com/product/virtual-machine/freebsd.freebsd-15_0](https://marketplace.microsoft.com/product/virtual-machine/freebsd.freebsd-15_0)

* Oracle Cloud Infrastructure:

  FreeBSD 虚拟机镜像预计将在不久的将来在 Oracle Cloud Infrastructure Marketplace 提供，支持 amd64 和 aarch64 架构，包括 UFS 和 ZFS 文件系统：
  [https://cloudmarketplace.oracle.com/marketplace/app/freebsd-release](https://cloudmarketplace.oracle.com/marketplace/app/freebsd-release)
...
本版本 FreeBSD 15.0-RELEASE 将受支持至 2026 年 9 月 30 日。
FreeBSD 15 发布系列将受支持至 2029 年 12 月 31 日。
更多支持信息可在以下地址找到：
...
喜爱 FreeBSD？
请向 FreeBSD 基金会[捐赠](https://www.freebsdfoundation.org/donate/)，支持本版本及未来的发行版！
