...
本硬件兼容性说明列出所有已知可在 FreeBSD 15.0 上工作的硬件。其他配置也可能可用，但尚未经过测试。欢迎对本列表提出反馈、更新和修正！
...
[arcmsr(4)](https://man.freebsd.org/cgi/man.cgi?query=arcmsr&sektion=4&format=html) 驱动支持以下控制卡：
...
此外，多个 HP Smart Array 控制器仅通过 PCI 子设备 ID 提供支持，因其型号名称不可用：0x3220, 0x3222, 0x3230, 0x3231, 0x3232, 0x3233, 0x3236, 0x3238, 0x3239, 0x323A, 0x323B, 0x323C, and 0x324B（所有 PCI 子厂商 ID 均为 0x103C）。
...
[et(4)](https://man.freebsd.org/cgi/man.cgi?query=et&sektion=4&format=html) 驱动支持 Agere ET1310 10/100/Gigabit以太网适配器。
...
[genet(4)](https://man.freebsd.org/cgi/man.cgi?query=genet&sektion=4&format=html) 驱动支持 树莓派 4 Model B 及相关系统上 Broadcom BCM2711 的以太网控制器部分。它使用 BCM54213PE PHY。
...
[ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 驱动支持 Intel Ethernet 700 系列。此系列中具有 SFP+/SFP28/QSFP+ cages 的大多数适配器具有要求使用 Intel 认证模块的固件；这些认证模块列出如下。驱动无法禁用此认证检查。
...
[mlx5ib(4)](https://man.freebsd.org/cgi/man.cgi?query=mlx5ib&sektion=4&format=html) 驱动支持 100Gb、50Gb、40Gb、25Gb 和 10Gb 网络适配器。
...
[uhso(4)](https://man.freebsd.org/cgi/man.cgi?query=uhso&sektion=4&format=html) 驱动应当能与 Option 的大多数设备配合工作以下设备已验证可正常工作：
...
较旧 Broadcom 芯片组（BCM4301、BCM4303 和 BCM4306 rev 2）的用户必须使用 **bwi(4)**，因为 v4 版本的固件不支持这些芯片。较新的固件过大，无法装入。
...
部分板载 CS4610 芯片搭配 CS423x ISA 编解码器，而非 CS4297 AC97 编解码器。此类配置目前尚未受 **snd_csa** 驱动支持。
...
> **注意**：
>
> USB 蓝牙适配器可在 [蓝牙](https://www.freebsd.org/releases/15.0R/hardware.html#bluetooth) 中找到。
>
> USB 人机接口设备可在 [其他设备](https://www.freebsd.org/releases/15.0R/hardware.html#misc) 中找到。
>
> USB 网络接口可在 [以太网接口](https://www.freebsd.org/releases/15.0R/hardware.html#ethernet) 或 [无线网络接口](https://www.freebsd.org/releases/15.0R/hardware.html#wlan) 中找到。
>
> USB 串行接口可在 [串行接口](https://www.freebsd.org/releases/15.0R/hardware.html#serial) 中找到。
...
[ig4(4)](https://man.freebsd.org/cgi/man.cgi?query=ig4&sektion=4&format=html) 支持基于 Synopsys DesignWare IP 的 I2C 控制器，可见于自第四代起的 Intel Core(TM) 处理器、IntelBay Trail、Apollo Lake SoC 系列以及部分 AMD 系统。
