...
已添加 smmsp 和 mailnull 用户到 `/etc/master.passwd` 中。如果没有设置 `confDEF_USER_ID`，默认情况下**sendmail**将使用 mailnull 用户以提高安全性。以前，如果 mailnull 用户不存在，则使用 daemon 用户。此更改可能会在向文件或程序（如[mail/majordomo](http://www.freebsd.org/cgi/url.cgi?ports/mail/majordomo/pkg-descr)）发送邮件时产生权限问题。通过在系统的配置文件 `* .mc` 中新增以下行，可以恢复先前的行为：
