...
本文档包含了 FreeBSD 4.5-RELEASE 在 IA-32 硬件平台上的发行说明。它描述了自 4.4-RELEASE 以来新增（或更改）的 FreeBSD 新特性。同时，它还提供了一些关于从先前版本升级的说明。
...
[nl(1)](http://www.freebsd.org/cgi/man.cgi?query=nl&sektion=1&manpath=FreeBSD+4.5-RELEASE)，一个行号过滤程序，已添加。
...
smmsp 和 mailnull 用户已添加到 `/etc/master.passwd` 中。如果没有设置 `confDEF_USER_ID`，默认情况下**sendmail**将使用 mailnull 用户以提高安全性。以前，如果 mailnull 用户不存在，则使用 daemon 用户。此更改可能会在向文件或程序（如[mail/majordomo](http://www.freebsd.org/cgi/url.cgi?ports/mail/majordomo/pkg-descr)）发送邮件时产生权限问题。通过在系统的配置文件 `* .mc` 中新增以下行，可以恢复先前的行为：
