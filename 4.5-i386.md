...
FreeBSD 4.5-RELEASE 的发行说明包含了自 4.4-RELEASE 以来在 FreeBSD 基本系统中所做更改的总结。列出了内核和用户空间的更改，以及自上次发布以来发布的适用于基本系统的安全公告。还包括了有关升级的简要说明。

## 1 引言

本文档包含了 FreeBSD 4.5-RELEASE 在 IA-32 硬件平台上的发行说明。它描述了自 4.4-RELEASE 以来添加（或更改）的 FreeBSD 新特性。同时，它还提供了一些关于从先前版本升级的说明。

此发行版的 FreeBSD 4.5-RELEASE 是一个正式发行版。可以在 [ftp://ftp.FreeBSD.org/pub/FreeBSD/releases/](ftp://ftp.freebsd.org/pub/FreeBSD/releases/) 或其所有镜像站点找到。有关如何获取此（或其他）版本的 FreeBSD 发行版的更多信息，请参阅 [FreeBSD 手册](http://www.freebsd.org/handbook/) 中的 [“获取 FreeBSD”](http://www.freebsd.org/handbook/mirrors.html) 附录。

## 2 新特性

本节描述了自 4.4-RELEASE 以来，FreeBSD 中最显著的新特性或变更。典型的发行说明项目包括新驱动程序或硬件支持、新命令或选项、重要的错误修复或贡献的软件升级。还列出了自 4.4-RELEASE 以来发布的适用于基本系统的安全公告。

由于篇幅限制，这里未列出许多对 FreeBSD 所做的其他更改。例如，文档已得到修正和改进，修复了小的 bug，审计并修正了不安全的编码实践，并清理了源代码。



### 2.1 内核更改

[amdpm(4)](http://www.freebsd.org/cgi/man.cgi?query=amdpm&sektion=4&manpath=FreeBSD+4.5-RELEASE) 驱动程序已添加，提供对 AMD 756 芯片组的系统监控功能的访问。
...
添加了一个新的内核选项 KVA_SPACE，可以用来重新配置内核虚拟地址空间的大小。
...
添加了支持 NEC PC-98NOTE 电源管理控制器的驱动程序 pmc。
...
如果正在构建带有模块的调试内核（即使用 makeoptions DEBUG=-g），则模块也将被构建为包含调试支持，以确保完整性。此更改的副作用是，使用调试内核构建并安装的模块现在会占用比以前更多的磁盘空间。
...
> **注意：** 尽管此功能在 FreeBSD 4.5-RELEASE 的 ISO 镜像中未使用，但它可能会在未来的版本中得到应用。与此同时，用户在构建自定义发行版时可以使用此功能。
...
FreeBSD 启动加载器现在支持 -p 参数，用于在探测阶段强制内核在每行输出后暂停。
...
[bge(4)](http://www.freebsd.org/cgi/man.cgi?query=bge&sektion=4&manpath=FreeBSD+4.5-RELEASE) 驱动程序已添加，支持 Broadcom BCM570x 系列千兆以太网控制器，包括 3Com 3c996-T、SysKonnect SK-9D21 和 SK-9D41，以及 Dell PowerEdge 2550 服务器上的内置千兆以太网 NIC。支持输出 TCP/IP 校验和卸载、巨帧和 VLAN 标签插入/去除，以及中断调节。
...
dgm 驱动程序已从 FreeBSD -CURRENT 更新。

[em(4)](http://www.freebsd.org/cgi/man.cgi?query=em&sektion=4&manpath=FreeBSD+4.5-RELEASE) 驱动程序已添加，支持基于 Intel 82542、82543 和 82544 千兆以太网控制器芯片的 NIC。该驱动程序支持 82543 和 82544 基适配器的发送/接收校验和卸载和巨帧。

[faith(4)](http://www.freebsd.org/cgi/man.cgi?query=faith&sektion=4&manpath=FreeBSD+4.5-RELEASE) 设备现在可加载、卸载和克隆。

[fxp(4)](http://www.freebsd.org/cgi/man.cgi?query=fxp&sektion=4&manpath=FreeBSD+4.5-RELEASE) 驱动程序现在支持 Intel 的可加载微代码，用于实现接收端中断合并和数据包捆绑，适用于支持这些功能的 NIC。此支持可以通过在 [ifconfig(8)](http://www.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8&manpath=FreeBSD+4.5-RELEASE) 中使用 link0 选项来启用。

[gx(4)](http://www.freebsd.org/cgi/man.cgi?query=gx&sektion=4&manpath=FreeBSD+4.5-RELEASE) 驱动程序已添加，支持基于 Intel 82542 和 82543 千兆以太网控制器芯片的 NIC。支持光纤和铜缆两种变体。两种板卡都支持 VLAN 标签/插入，并且 82543 还支持 TCP/IP 校验和卸载。

添加了 sbni 驱动程序，用于支持 Granch SBNI12 系列 ISA 和 PCI 点对点通信接口。Port [sysutil/sbniconfig](http://www.freebsd.org/cgi/url.cgi?ports/sysutil/sbniconfig/pkg-descr) 可以用于配置这些设备。

[sis(4)](http://www.freebsd.org/cgi/man.cgi?query=sis&sektion=4&manpath=FreeBSD+4.5-RELEASE) 驱动程序现在支持 SiS 900 风格的板载以太网控制器，适用于 SiS 635 和 735 主板芯片组。

[sis(4)](http://www.freebsd.org/cgi/man.cgi?query=sis&sektion=4&manpath=FreeBSD+4.5-RELEASE) 驱动程序现在支持 VLAN。

[vlan(4)](http://www.freebsd.org/cgi/man.cgi?query=vlan&sektion=4&manpath=FreeBSD+4.5-RELEASE) 设备现在可加载、卸载和克隆。

[wx(4)](http://www.freebsd.org/cgi/man.cgi?query=wx&sektion=4&manpath=FreeBSD+4.5-RELEASE) 驱动程序现在已弃用，并且正式不再维护。具有 Intel Pro/1000 千兆以太网接口的用户应使用 [em(4)](http://www.freebsd.org/cgi/man.cgi?query=em&sektion=4&manpath=FreeBSD+4.5-RELEASE) 驱动程序或 [gx(4)](http://www.freebsd.org/cgi/man.cgi?query=gx&sektion=4&manpath=FreeBSD+4.5-RELEASE) 驱动程序。（[em(4)](http://www.freebsd.org/cgi/man.cgi?query=em&sektion=4&manpath=FreeBSD+4.5-RELEASE) 驱动程序由 Intel 支持，但仅适用于 i386 架构。 [gx(4)](http://www.freebsd.org/cgi/man.cgi?query=gx&sektion=4&manpath=FreeBSD+4.5-RELEASE) 驱动程序由 FreeBSD 项目开发，支持多个平台。）
...
修复了 [xl(4)](http://www.freebsd.org/cgi/man.cgi?query=xl&sektion=4&manpath=FreeBSD+4.5-RELEASE) 驱动程序中的一个错误，涉及统计溢出中断处理，导致中等到高数据包速率时出现慢速问题；现已修复。
...
[aac(4)](http://www.freebsd.org/cgi/man.cgi?query=aac&sektion=4&manpath=FreeBSD+4.5-RELEASE) 驱动程序已经更新，包含了适配器发起的命令处理、磁盘设备的添加/删除、崩溃转储功能以及管理 CLI 所需的 [ioctl(2)](http://www.freebsd.org/cgi/man.cgi?query=ioctl&sektion=2&manpath=FreeBSD+4.5-RELEASE) 命令。该驱动程序现在完全符合 Adaptec 的要求，并获得了其批准。
...
FFS 的目录布局偏好算法（dirprefs）已经更改。它不再将目录块散布到磁盘上，而是尝试将相关的目录块聚集在一起。操作大规模目录层级（如 FreeBSD Ports 树）时，已经表现出显著的速度提升。对于新目录，这一变化是透明且自动的。

虚拟内存子系统现在默认支持 UFS 目录内存需求（此行为由 vfs.vmiodirenable sysctl 变量控制）。

修复了一个问题，该问题导致无法从 SCSI CDROM 挂载根文件系统（ATAPI CDROM 一直被支持）。
...
[isdnphone(8)](http://www.freebsd.org/cgi/man.cgi?query=isdnphone&sektion=8&manpath=FreeBSD+4.5-RELEASE) 现在支持一个 -k 选项，用于通过键盘功能发送消息到 PBX 或交换局。
...
FreeBSD 4.4-RELEASE 中已禁用每个用户的 \~/.login.conf 文件，以避免由一个 bug 引起的安全漏洞。该 bug 已经修复，且此功能已经重新启用。
...
[edquota(8)](http://www.freebsd.org/cgi/man.cgi?query=edquota&sektion=8&manpath=FreeBSD+4.5-RELEASE) 现在可以使用 -f 选项，将原型配额分配（通过 -p 指定）限制到单个文件系统。
...
[ftpd(8)](http://www.freebsd.org/cgi/man.cgi?query=ftpd&sektion=8&manpath=FreeBSD+4.5-RELEASE) 现在支持 -o 和 -O 选项，分别禁用 RETR 命令；前者适用于所有人，后者仅适用于访客用户。结合 -A 选项和适当的文件权限，这些可以用来创建相对安全的匿名 FTP 上传盒，供他人上传文件。
...
[ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+4.5-RELEASE) 现在默认不显示动态防火墙规则，除非传递 -d 选项。-e 选项列出过期的动态规则。

[ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+4.5-RELEASE) 添加了新的防火墙规则限制类型，用于限制地址对之间的会话数量。

[keyinfo(1)](http://www.freebsd.org/cgi/man.cgi?query=keyinfo&sektion=1&manpath=FreeBSD+4.5-RELEASE) 现在是一个 C 程序，而非 Perl 脚本。

libfetch 已同步到 FreeBSD -CURRENT 版本；其中一个新特性是现在支持认证回调。

libstand 现在支持包含 **bzip2** 压缩文件的文件系统。

本地化名称已重命名，以提高与 X11R6 及其他多个 UNIX 版本使用的名称的兼容性。例如，en_US.ISO_8859-1 本地化名称已更改为 en_US.ISO8859-1。/etc/locale.alias、/etc/man.alias 和 /etc/nls.alias 中的条目提供了向后兼容性。下面的表格总结了本地化更改：
...
[natd(8)](http://www.freebsd.org/cgi/man.cgi?query=natd&sektion=8&manpath=FreeBSD+4.5-RELEASE) 现在支持 -log\_ipfw\_denied 选项，用于记录因被 [ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+4.5-RELEASE) 规则阻止而无法重新注入的分组。
...
在 [newfs(8)](http://www.freebsd.org/cgi/man.cgi?query=newfs&sektion=8&manpath=FreeBSD+4.5-RELEASE) 中，每组的默认柱面数现在根据当前文件系统参数计算得出，这是最大允许值。可以通过 -c 选项进行覆盖。以前，默认值固定为 16。此更改有助于提高 [fsck(8)](http://www.freebsd.org/cgi/man.cgi?query=fsck&sektion=8&manpath=FreeBSD+4.5-RELEASE) 性能并减少碎片。

[newfs(8)](http://www.freebsd.org/cgi/man.cgi?query=newfs&sektion=8&manpath=FreeBSD+4.5-RELEASE) 创建的新文件系统的默认块和碎片大小现在分别为 16384 字节和 2048 字节（旧的默认值为 8192 字节和 1024 字节）。这一更改通常会提高性能，但会浪费一些磁盘空间。
...
[nl(1)](http://www.freebsd.org/cgi/man.cgi?query=nl&sektion=1&manpath=FreeBSD+4.5-RELEASE)，一个行号过滤程序，已被添加。

[pciconf(8)](http://www.freebsd.org/cgi/man.cgi?query=pciconf&sektion=8&manpath=FreeBSD+4.5-RELEASE) 现在支持 -v 选项，用于显示配置设备的厂商/设备信息，并与 -l 选项一起使用。默认的厂商/设备数据库可以在 /usr/share/misc/pci\_vendors 中找到。

[ping(8)](http://www.freebsd.org/cgi/man.cgi?query=ping&sektion=8&manpath=FreeBSD+4.5-RELEASE) 现在支持 -A 选项，在丢包时发出蜂鸣声。

[route(8)](http://www.freebsd.org/cgi/man.cgi?query=route&sektion=8&manpath=FreeBSD+4.5-RELEASE) 在更改间接路由时现在会更详细地显示信息，在修改的路由与网关路由相同的情况下。
...
[sysinstall(8)](http://www.freebsd.org/cgi/man.cgi?query=sysinstall&sektion=8&manpath=FreeBSD+4.5-RELEASE) 现在具有在安装过程中加载 KLDs 的能力。
...
**OpenSSH** 已更新至 2.9 版，新增了两个程序：[sftp(1)](http://www.freebsd.org/cgi/man.cgi?query=sftp&sektion=1&manpath=FreeBSD+4.5-RELEASE)和[ssh-keyscan(1)](http://www.freebsd.org/cgi/man.cgi?query=ssh-keyscan&sektion=1&manpath=FreeBSD+4.5-RELEASE)。其中的一些增强功能包括：现在支持重新加密现有的 SSH 会话，[ssh-agent(1)](http://www.freebsd.org/cgi/man.cgi?query=ssh-agent&sektion=1&manpath=FreeBSD+4.5-RELEASE) 现在支持 DSA 密钥的认证转发，并且在 [ssh(1)](http://www.freebsd.org/cgi/man.cgi?query=ssh&sektion=1&manpath=FreeBSD+4.5-RELEASE) 中添加了实验性的 **SOCKS4** 代理。

> **注意：** 协议 1 和 2 仍然是 `/etc/ssh/ssh_config` 中的默认协议设置。在 FreeBSD -CURRENT 中，默认协议为 2,1。

smmsp 和 mailnull 用户已被添加到 `/etc/master.passwd` 中。如果没有设置 confDEF_USER_ID，默认情况下**sendmail**将使用 mailnull 用户以提高安全性。以前，如果 mailnull 用户不存在，则使用 daemon 用户。此更改可能会在向文件或程序（如[mail/majordomo](http://www.freebsd.org/cgi/url.cgi?ports/mail/majordomo/pkg-descr)）发送邮件时产生权限问题。通过在系统的配置文件 `* .mc` 中添加以下行，可以恢复先前的行为：

```sh
    define(`confDEF_USER_ID', `daemon')
```

用户空间实用程序 **smbfs** 的版本 1.4.3 已被导入。[smbutil(1)](http://www.freebsd.org/cgi/man.cgi?query=smbutil&sektion=1&manpath=FreeBSD+4.5-RELEASE) 和 [mount_smbfs(8)](http://www.freebsd.org/cgi/man.cgi?query=mount_smbfs&sektion=8&manpath=FreeBSD+4.5-RELEASE)现在可以在基本系统中使用，无需安装 Port [net/smbfs](http://www.freebsd.org/cgi/url.cgi?ports/net/smbfs/pkg-descr)。请注意，[mount_smbfs(8)](http://www.freebsd.org/cgi/man.cgi?query=mount_smbfs&sektion=8&manpath=FreeBSD+4.5-RELEASE) 将自动将模块 smbfs.ko 加载到内核中，即使 LIBMCHAIN 和 LIBICONV 未编译到内核中。
...
- 完全重新安装 FreeBSD。严格来说，这不是一种升级方法，并且通常不如二进制升级方便，因为它需要你手动备份和恢复 `/etc` 目录的内容。然而，在你想要（或需要）更改磁盘分区的情况下，这可能是有用的。
...
本文件以及其他与版本相关的文档可以从 [ftp://ftp.FreeBSD.org/pub/FreeBSD/releases/](ftp://ftp.freebsd.org/pub/FreeBSD/releases/) 下载。
