...
- OpenSSH 1.2 已集成到基本系统中。OpenSSH 是一个免费的（BSD 许可）SSH v1 协议的完整实现，与其他 SSH v1 客户端和服务器（例如 Port `/usr/ports/security/ssh`）完全兼容。OpenSSH 提供了该 Port 的所有功能，实际上它基于一个较旧的版本，在该软件成为限制性许可之前的版本。FreeBSD 4.0 提供了 SSH 客户端/服务器功能，前提是您在 sysinstall 中选择安装 `DES` 加密分发包。
