...
FreeBSD 10.1-RELEASE 的发行说明包含了在 10.1-STABLE 开发线中对 FreeBSD 基本系统所做的更改的总结。本文列出了自上次发布以来发布的相关安全公告，以及 FreeBSD 内核和用户空间的重要更改。此外，还简要介绍了升级的相关事项。

## 引言

本文档包含了 FreeBSD 10.1-RELEASE 的发行说明。它介绍了 FreeBSD 中最近添加、修改或删除的功能。同时也提供了一些关于从先前版本升级到本版本的说明。

此版本的 FreeBSD 10.1-RELEASE 是一个发布版本。可以在 [`http://www.FreeBSD.org/releases/`](http://www.freebsd.org/releases/) 或其镜像站点找到。获取此版本（或其他版本）发行版的方法，请参阅 [FreeBSD 手册](https://docs.freebsd.org/en/books/handbook/) 中的 [获取 FreeBSD](https://docs.freebsd.org/en/books/handbook/#mirrors) 附录。

所有用户在安装 FreeBSD 之前，建议先查阅发布勘误。勘误文档会随着发布周期结束或发布后发现的“最新”信息而更新。通常，它包含已知的 bug、安全公告和文档修正。可以在 FreeBSD 网站上找到 FreeBSD 10.1-RELEASE 的最新勘误。

## 新特性

本节介绍了自 10.0-RELEASE 以来，FreeBSD 中最显著的新增或更改功能。

典型的发行说明条目包括记录 10.0-RELEASE 后发布的安全公告、新的驱动程序或硬件支持、新命令或选项、重要的错误修复或第三方软件升级。还可能列出主要 Port/软件包或发布工程实践的更改。显然，发行说明无法列出每个版本之间对 FreeBSD 所做的所有更改；本文档主要关注安全公告、用户可见的更改和主要架构改进。
...
[bhyve(4)](http://www.freebsd.org/cgi/man.cgi?query=bhyve&sektion=4) 虚拟机管理程序现在支持通过 ACPI S5 状态执行软关机功能。[(r261090)](http://svn.freebsd.org/viewvc/base?view=revision&revision=261090)
...
现在虚拟机管理程序 [bhyve(4)](http://www.freebsd.org/cgi/man.cgi?query=bhyve&sektion=4) 能从 [zfs(8)](http://www.freebsd.org/cgi/man.cgi?query=zfs&sektion=8) 文件系统启动。[(r268932)](http://svn.freebsd.org/viewvc/base?view=revision&revision=268932)

新增了驱动 [virtio_random(4)](http://www.freebsd.org/cgi/man.cgi?query=virtio_random&sektion=4)，允许 FreeBSD 虚拟机从虚拟机管理程序中获取熵。[(r268933)](http://svn.freebsd.org/viewvc/base?view=revision&revision=268933)

[bhyve(4)](http://www.freebsd.org/cgi/man.cgi?query=bhyve&sektion=4) 虚拟机管理程序已与 FreeBSD-CURRENT 中的版本同步。[(r270159)](http://svn.freebsd.org/viewvc/base?view=revision&revision=270159)

已添加多个增强功能，并修复了若干错误，包括：

* 当虚拟机以 `EPT Misconfiguration` 错误退出时，添加了事后调试支持。
* 虚拟机管理程序的 [virtio(4)](http://www.freebsd.org/cgi/man.cgi?query=virtio&sektion=4) API 从 32 位扩展到 64 位。
* 添加了识别虚拟 CPU 功能的支持。
* 添加了模拟传统 x86 任务切换的支持。
...
已为 UEFI 引导加载程序添加串行控制台和空控制台支持。[(r271880)](http://svn.freebsd.org/viewvc/base?view=revision&revision=271880)
...
NFS 服务器的内核 RPC 代码经过多项优化，显著提升了性能和对对称多处理（SMP）的可扩展性。（由 iXsystems 赞助）

iSCSI 发起程序已更新，支持在 iSCSI 设备配置了多个 IP 地址并跨不同网络接口时的重定向处理。此前，连接到此类 iSCSI 设备的客户端可能需要额外的客户端配置。[(r269065)](http://svn.freebsd.org/viewvc/base?view=revision&revision=269065)（由 FreeBSD 基金会赞助）

新增了 [sysctl(8)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8) 参数 `kern.iscsi.fail_on_disconnection`，允许 iSCSI 客户端在连接丢失时移除附加的磁盘设备，之前会一直停止 I/O，直到连接恢复。[(r265523)](http://svn.freebsd.org/viewvc/base?view=revision&revision=265523)（由 FreeBSD 基金会赞助）

已优化 iSCSI 传输代码，以合并 PDU 并避免锁争用。[(r265524)](http://svn.freebsd.org/viewvc/base?view=revision&revision=265524)（由 FreeBSD 基金会赞助）

iSCSI 子系统有多项性能优化，包括推迟唤醒，直到接收到足够的数据再进行文件读写，从而减少了 CPU 使用率并提高了大规模 I/O 工作负载下的吞吐量性能。[(r265524)](http://svn.freebsd.org/viewvc/base?view=revision&revision=265524)（由 iXsystems 赞助）

已为 iSCSI [ctld(8)](http://www.freebsd.org/cgi/man.cgi?query=ctld&sektion=8) 守护进程添加基于主机名和 IP 的访问限制支持。[(r263720)](http://svn.freebsd.org/viewvc/base?view=revision&revision=263720)（由 FreeBSD 基金会赞助）

[ctld(8)](http://www.freebsd.org/cgi/man.cgi?query=ctld&sektion=8) 守护进程已更新，允许覆盖“默认”`portal-group` 配置。[(r263725)](http://svn.freebsd.org/viewvc/base?view=revision&revision=263725)（由 FreeBSD 基金会赞助）

[ctld(8)](http://www.freebsd.org/cgi/man.cgi?query=ctld&sektion=8) 守护进程现在包括新的 `auth-group`，名为“default”，默认设置为 `deny`，并可以覆盖此设置。[(r263726)](http://svn.freebsd.org/viewvc/base?view=revision&revision=263726)（由 FreeBSD 基金会赞助）
...
[geom(8)](http://www.freebsd.org/cgi/man.cgi?query=geom&sektion=8) `label` 类现在能够感知调整大小后的分区。解决了一个问题：其中 `geom resize` 会调整分区大小，但 **/dev/gptid/** 中的标签提供者未被调整大小。[(r259328)](http://svn.freebsd.org/viewvc/base?view=revision&revision=259328)（由 FreeBSD 基金会赞助）
...
[geom(8)](http://www.freebsd.org/cgi/man.cgi?query=geom&sektion=8) 的 `MBR` 分区类中添加了新的 [sysctl(8)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8) 和 [loader(8)](http://www.freebsd.org/cgi/man.cgi?query=loader&sektion=8) 可调参数 `kern.geom.part.mbr.enforce_chs`。当设置为非零值时，**GEOM_PART_MBR** 将自动重新计算用户指定的偏移量和大小，以与磁盘几何结构对齐。[(r270552)](http://svn.freebsd.org/viewvc/base?view=revision&revision=270552)
...
已将驱动程序 [mrsas(4)](http://www.freebsd.org/cgi/man.cgi?query=mrsas&sektion=4) 添加到 amd64 和 i386 架构的 **GENERIC** 内核配置中。[(r271234)](http://svn.freebsd.org/viewvc/base?view=revision&revision=271234)
...
为 [fsck_ffs(8)](http://www.freebsd.org/cgi/man.cgi?query=fsck_ffs&sektion=8) 工具新增了参数 `-R`。启用该参数时，当检测到过多关键错误时，[fsck_ffs(8)](http://www.freebsd.org/cgi/man.cgi?query=fsck_ffs&sektion=8) 将自动重启自身。[(r260178)](http://svn.freebsd.org/viewvc/base?view=revision&revision=260178)（由 Netflix 提供）

[zfs(8)](http://www.freebsd.org/cgi/man.cgi?query=zfs&sektion=8) 文件系统已更新以支持“书签”功能。详见 [zfs(8)](http://www.freebsd.org/cgi/man.cgi?query=zfs&sektion=8)。[(r263407)](http://svn.freebsd.org/viewvc/base?view=revision&revision=263407)
...
为 [camcontrol(8)](http://www.freebsd.org/cgi/man.cgi?query=camcontrol&sektion=8) 添加了选项 `-b`，用于输出现有的总线及其父设备。[(r260177)](http://svn.freebsd.org/viewvc/base?view=revision&revision=260177)（由 Netflix 赞助）
...
[zfs(8)](http://www.freebsd.org/cgi/man.cgi?query=zfs&sektion=8) 用户空间实用程序增加了 `snapshot` 的别名，支持 `zfs list -t snap` 和 `zfs snap`。[(r263403)](http://svn.freebsd.org/viewvc/base?view=revision&revision=263403)

[zfs(8)](http://www.freebsd.org/cgi/man.cgi?query=zfs&sektion=8) 用户空间实用程序在 `zfs list` 中新增了选项 `-p`，可输出可解析的格式。[(r263405)](http://svn.freebsd.org/viewvc/base?view=revision&revision=263405)
...
[date(1)](http://www.freebsd.org/cgi/man.cgi?query=date&sektion=1) 工具新增选项 `-R`，可按 RFC 2822 规定的格式打印日期和时间。[(r265265)](http://svn.freebsd.org/viewvc/base?view=revision&revision=265265)
...
[pmcstat(8)](http://www.freebsd.org/cgi/man.cgi?query=pmcstat&sektion=8) 工具新增了选项 `-a`，指定后可在采样点生成完整的堆栈跟踪。[(r265604)](http://svn.freebsd.org/viewvc/base?view=revision&revision=265604)（由 Netflix 提供）
...
[gvinum(8)](http://www.freebsd.org/cgi/man.cgi?query=gvinum&sektion=8) 工具更新了强制重置配置的选项 `-f`，并修复了阻止 `-f` 正确创建配置的错误。[(r266014)](http://svn.freebsd.org/viewvc/base?view=revision&revision=266014)
...
[ps(1)](http://www.freebsd.org/cgi/man.cgi?query=ps&sektion=1) 工具新增选项 `-J`，可通过 [jail(8)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8) 的 ID 和名称过滤输出。参数 `0` 可用于 `-J`，仅列出主机系统运行的进程。[(r266279)](http://svn.freebsd.org/viewvc/base?view=revision&revision=266279)
...
[gstat(8)](http://www.freebsd.org/cgi/man.cgi?query=gstat&sektion=8) 工具新增选项 `-o`，启用后可显示如 `BIO_FLUSH` 的操作统计信息。[(r266610)](http://svn.freebsd.org/viewvc/base?view=revision&revision=266610)
...
**/etc/ttys** 文件新增了参数 `onifconsole`。当设备为活动内核控制台时，系统将通过串口控制台提供登录提示，否则此参数等效于 `off`。[(r267236)](http://svn.freebsd.org/viewvc/base?view=revision&revision=267236)
...
[bhyve(8)](http://www.freebsd.org/cgi/man.cgi?query=bhyve&sektion=8) 工具新增了 SMBIOS 支持，同时添加了参数 `-U`，可在系统信息结构中指定来宾的 UUID。[(r267450)](http://svn.freebsd.org/viewvc/base?view=revision&revision=267450)
...
>**注意**
>
> 参数 `-H` 当前仅与 `-r` 一起使用，以显示单个线程的资源使用情况，而非整个进程。

[sed(1)](http://www.freebsd.org/cgi/man.cgi?query=sed&sektion=1) 工具新增了参数 `-u`，当指定该参数时，将启用无缓冲输出。[(r268019)](http://svn.freebsd.org/viewvc/base?view=revision&revision=268019)
...
[gstat(8)](http://www.freebsd.org/cgi/man.cgi?query=gstat&sektion=8) 工具新增了参数 `-p`，当指定该参数时，仅显示物理提供者。[(r268791)](http://svn.freebsd.org/viewvc/base?view=revision&revision=268791)

[kldstat(8)](http://www.freebsd.org/cgi/man.cgi?query=kldstat&sektion=8) 工具已更新，允许在指定 `-n module.ko` 时使用参数 `-q`。[(r268903)](http://svn.freebsd.org/viewvc/base?view=revision&revision=268903)
...
* 新增参数 `-e`，用于设置 [loader(8)](http://www.freebsd.org/cgi/man.cgi?query=loader&sektion=8) 环境变量。
* 新增参数 `-C`，用于指定来宾控制台设备。
* 新增参数 `-H`，用于将主机路径传递给 [bhyveload(8)](http://www.freebsd.org/cgi/man.cgi?query=bhyveload&sektion=8)。
...
[jail(8)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8) 工具现支持为参数 `ip4.addr` 和 `ip6.addr` 提供额外的 [ifconfig(8)](http://www.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8) 参数。此更改允许在 [jail(8)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8) 中使用 [carp(4)](http://www.freebsd.org/cgi/man.cgi?query=carp&sektion=4) 接口。[(r269805)](http://svn.freebsd.org/viewvc/base?view=revision&revision=269805)
...
[iscsictl(8)](http://www.freebsd.org/cgi/man.cgi?query=iscsictl&sektion=8) 工具已更新，新增了参数 `-M`，允许在不需要移除并重新添加会话的情况下修改 iSCSI 会话参数。[(r269968)](http://svn.freebsd.org/viewvc/base?view=revision&revision=269968)

[mount_nfs(8)](http://www.freebsd.org/cgi/man.cgi?query=mount_nfs&sektion=8) 工具现已支持通过参数 `-o` 指定 NFS 版本为 `key=value` 形式的参数。例如，要指定 NFS 版本 4，可使用语法 `-o vers=4`。[(r270043)](http://svn.freebsd.org/viewvc/base?view=revision&revision=270043)
...
[lldb(1)](http://www.freebsd.org/cgi/man.cgi?query=lldb&sektion=1) 调试库已更新至 r202189 快照。[(r269024)](http://svn.freebsd.org/viewvc/base?view=revision&revision=269024)（由 DARPA, AFRL 赞助）
...
`release/scripts/pkg-stage.sh` 脚本已更新，在 DVD 安装程序的 `packages/` 目录结构中添加了指向光盘版 [pkg(8)](http://www.freebsd.org/cgi/man.cgi?query=pkg&sektion=8) 的符号链接。这使得 [pkg(7)](http://www.freebsd.org/cgi/man.cgi?query=pkg&sektion=7) 引导工具能够正确定位 DVD 中的 `Latest/pkg.txz` 软件包，而无需网络连接或显式提供安装介质上的路径。[(r271943)](http://svn.freebsd.org/viewvc/base?view=revision&revision=271943)（由 FreeBSD 基金会赞助）

FreeBSD 发布工程构建工具已更新，支持通过 [mkimg(1)](http://www.freebsd.org/cgi/man.cgi?query=mkimg&sektion=1) 在发布构建过程中生成虚拟机磁盘镜像。目前支持的磁盘镜像格式包括 QCOW2、VHD、VMDK 和原始格式。更多信息请参见 [release(7)](http://www.freebsd.org/cgi/man.cgi?query=release&sektion=7)。[(r273101)](http://svn.freebsd.org/viewvc/base?view=revision&revision=273101)（由 FreeBSD 基金会赞助）
...
[amd64, i386] 可以通过 [freebsd-update(8)](http://www.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8) 工具在不同的 RELEASE 版本之间（以及各安全分支的快照之间）进行二进制升级。二进制升级程序将更新未修改的用户空间工具以及作为官方 FreeBSD 发布版本一部分分发的未修改的 **GENERIC** 内核。[freebsd-update(8)](http://www.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8) 工具需要被升级的主机具有互联网连接。
