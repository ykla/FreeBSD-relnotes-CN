...
本硬件兼容性说明列出所有已知可在 FreeBSD 15.1 上工作的硬件。其他配置也可能可用，但尚未经过测试。欢迎对本列表提出反馈、更新和修正！
...
[FreeBSD/amd64](/platforms/amd64) 将 x86-64 作为完全支持的 [Tier-1](https://docs.freebsd.org/en/articles/committers-guide/#archs) 架构。发行介质预期可在所有至少配备 256 MiB 内存的 x86-64 机器上运行。本发行版整合了多项优化和增强功能，以充分发挥基于 FreeBSD/amd64 系统的全部能力，在各种工作负载下提供更佳性能。
...
RISC-V 支持标志着这一开源操作系统发展中的重要里程碑。FreeBSD 是首个内置支持 RISC-V 引导的操作系统，体现了其对新兴硬件架构的承诺。
...
本节介绍 FreeBSD 中各设备驱动当前已知支持的设备。如果相关驱动在 FreeBSD 基本系统中拥有手册页（多数驱动都有），则在此处给出引用。关于用法、配置以及所支持的设备、控制器等的具体型号等信息，可在手册页中查阅。
...
[aac(4)](https://man.freebsd.org/cgi/man.cgi?query=aac&sektion=4&format=html) 驱动支持以下来自 Adaptec AAC 家族的 Parallel SCSI、SATA 和 3G SAS RAID 控制器：
...
[aacraid(4)](https://man.freebsd.org/cgi/man.cgi?query=aacraid&sektion=4&format=html) 驱动支持以下 Adaptec 6G 和 12G SAS/SATA RAID 控制器：
...
[ahc(4)](https://man.freebsd.org/cgi/man.cgi?query=ahc&sektion=4&format=html) 驱动支持以下 VL/ISA/PCI 并行 SCSI 控制器和卡：
...
此外，配合 **ata(4)** 的 atamarvell 和 atajmicron 驱动，还支持传统 PATA + AHCI-SATA 组合控制器中的 AHCI 部分，例如 JMicron JMB36x 和 Marvell 88SE61xx。

**ahci** 驱动还支持使用 Intel Rapid Storage Technology (RST) 充当 **nvme(4)** PCI 桥的 AHCI 设备。若要使用 **nvme(4)** 设备，必须在 BIOS 中将 SATA 模式从 RST 改为 AHCI，否则须接受因中断共享导致 RST 启用时的性能。FreeBSD 会自动检测启用此扩展且处于 RST 模式的 AHCI 设备。此时，**ahci** 会为 **ahci** 设备附加 **nvme(4)** 子设备。

[ahd(4)](https://man.freebsd.org/cgi/man.cgi?query=ahd&sektion=4&format=html) 驱动支持以下 PCI/PCI-X 并行 SCSI 控制器：
...
[arcmsr(4)](https://man.freebsd.org/cgi/man.cgi?query=arcmsr&sektion=4&format=html) 驱动支持以下 Areca PCI-X/PCIe SATA/SAS/NVMe RAID 主机适配器：
...
[ata(4)](https://man.freebsd.org/cgi/man.cgi?query=ata&sektion=4&format=html) 驱动支持以下 ATA/SATA 控制器上的 IDE 接口：
...
上述部分芯片可配置为 AHCI 模式。此时由 **ahci(4)** 驱动支持。对于未知的 ATA 芯片组，在 PIO 模式下提供支持；若存在标准的 busmaster DMA 寄存器且包含有效设置，也会启用 DMA，但最高模式限于 UDMA33，因为无法获知芯片组能力及编程方法。

[ciss(4)](https://man.freebsd.org/cgi/man.cgi?query=ciss&sektion=4&format=html) 驱动支持实现 Common Interface for SCSI-3 Support Open Specification v1.04 的控制器，包括：
...
此外，多个 HP Smart Array 控制器仅通过 PCI 子设备 ID 提供支持，因其型号名称不可用： 0x3220, 0x3222, 0x3230, 0x3231, 0x3232, 0x3233, 0x3236, 0x3238, 0x3239, 0x323A, 0x323B, 0x323C 和 0x324B （所有 PCI 子厂商 ID 均为 0x103C）。
...
**hptiop** 驱动还支持以下已经停产（End-of-Life）的 SAS 和 SATA RAID 控制器：
...
以下控制器受 [ida(4)](https://man.freebsd.org/cgi/man.cgi?query=ida&sektion=4&format=html) 驱动支持：
...
[ips(4)](https://man.freebsd.org/cgi/man.cgi?query=ips&sektion=4&format=html) 驱动支持的控制器包括：
...
更新的 ServeRAID 控制器由 **aac(4)** 或 **mfi(4)** 驱动支持。

[isci(4)](https://man.freebsd.org/cgi/man.cgi?query=isci&sektion=4&format=html) 驱动为 Intel C600 `SAS` 控制器提供支持。

[isp(4)](https://man.freebsd.org/cgi/man.cgi?query=isp&sektion=4&format=html) 驱动支持以下光纤通道适配器：
...
[mlx(4)](https://man.freebsd.org/cgi/man.cgi?query=mlx&sektion=4&format=html) 驱动支持以下并行 SCSI RAID 控制器：
...
[mmcsd(4)](https://man.freebsd.org/cgi/man.cgi?query=mmcsd&sektion=4&format=html) 驱动为 MMC 和 SD 存储卡实现直接访问块设备。

以下控制器受 [mpi3mr(4)](https://man.freebsd.org/cgi/man.cgi?query=mpi3mr&sektion=4&format=html) 驱动支持：
...
[mpr(4)](https://man.freebsd.org/cgi/man.cgi?query=mpr&sektion=4&format=html) 驱动支持以下 SATA/SAS/NVMe RAID 控制器：
...
以下控制器受 [mps(4)](https://man.freebsd.org/cgi/man.cgi?query=mps&sektion=4&format=html) 驱动支持：
...
以下控制器受 [mpt(4)](https://man.freebsd.org/cgi/man.cgi?query=mpt&sektion=4&format=html) 驱动支持：
...
**mpt** 驱动所支持的 `Ultra 320 SCSI` 控制器芯片可在许多系统主板上找到，包括：

* Dell PowerEdge 1750 至 2850
* IBM eServer xSeries 335

这些系统还包含集成 RAID 镜像和增强型集成 RAID 镜像，本驱动也提供支持。

`SAS` 控制器芯片也存在于许多基于 AMD/Opteron 的新系统中， 例如 Sun 4100。 注意，该控制器可同时驱动 SAS 和 SATA 硬盘，或同时混合使用二者。 这些控制器可用的集成 RAID 镜像功能的支持充其量只能算是有限。

`Fibre Channel` 控制器芯片组在多种速度和系统中得到支持。 `Apple` Fibre Channel HBA 实际上是 `FC949ES` 卡。

本驱动还支持光纤通道卡的目标模式（target mode）。 可通过 LSI Logic 固件工具设置核心的目标角色以启用此支持—— 该工具用于设定卡可承担的角色，无需单独编译。

[mrsas(4)](https://man.freebsd.org/cgi/man.cgi?query=mrsas&sektion=4&format=html) 驱动支持以下 LSI/Broadcom SATA/SAS RAID 控制器：
...
注意，此硬件仅对 ATA DMA 命令支持命令队列和基于 FIS 的切换。 ATAPI 和非 DMA ATA 命令按端口逐条执行。

[nda(4)](https://man.freebsd.org/cgi/man.cgi?query=nda&sektion=4&format=html) 驱动通过 CAM 子系统支持经由 PCIe 或 NVMF （NVMe over Fabric） 连接的 NVMe （Non-Volatile Memory Express） 存储设备。

[nvd(4)](https://man.freebsd.org/cgi/man.cgi?query=nvd&sektion=4&format=html) 驱动支持使用 NVMe 命名空间的 NVMe 存储设备。

[ocs_fc(4)](https://man.freebsd.org/cgi/man.cgi?query=ocs_fc&sektion=4&format=html) 驱动支持以下 PCIe 光纤通道适配器：
...
[sdhci(4)](https://man.freebsd.org/cgi/man.cgi?query=sdhci&sektion=4&format=html) 驱动支持 SD 主机控制器规范。 通过 PCI 总线连接时， 控制器会自动配置。 许多 SoC 芯片提供直接映射到 I/O 内存的 SDHCI 控制器。 对于这些情况，可使用 **fdt(4)** 或 **acpi(4)** 方法配置控制器，相关方法由主板厂商提供。

与大多数支持通用标准的驱动不同， **sdhci** 需要大量 quirks 来应对硬件缺陷、 专有寄存器和定义不明确的电源管理。 虽然来自 Intel、Xilinx、Rockchip、Freescale、Ricoh 和 TI 的许多芯片组都有这些条目， 但使用某些控制器时可能产生次优性能。 当设备通过 **fdt(4)** 或 **acpi(4)** 配置时，最常需要 quirks 和自定义配置。
...
[smartpqi(4)](https://man.freebsd.org/cgi/man.cgi?query=smartpqi&sektion=4&format=html) 驱动支持的控制器包括但不限于：
...
**sym** 支持的 SCSI 控制器可内置于主板， 也可位于以下附加板卡上：
...
[ufshci(4)](https://man.freebsd.org/cgi/man.cgi?query=ufshci&sektion=4&format=html) 驱动支持实现 Universal Flash Storage Host Controller Interface 4.1 及更早版本的主机控制器和设备。

对于所有支持的 SCSI 控制器，均完整支持 SCSI-I、SCSI-II 和 SCSI-III 外设，包括硬盘、光盘、磁带机（含 DAT、8mm Exabyte、Mammoth 和 DLT）、介质更换器、处理器目标设备和 CD-ROM 驱动器。 支持 CD-ROM 命令的 WORM 设备可由 CD-ROM 驱动（如 [cd(4)](https://man.freebsd.org/cgi/man.cgi?query=cd&sektion=4&format=html)）以只读方式访问。 WORM/CD-R/CD-RW 写入支持由 [cdrecord(1)](https://man.freebsd.org/cgi/man.cgi?query=cdrecord&sektion=1&format=html) 提供，它是 Ports 中 [sysutils/cdrtools](http://www.freebsd.org/cgi/url.cgi?ports/sysutils/cdrtools/pkg-descr) 的一部分。
...
[aue(4)](https://man.freebsd.org/cgi/man.cgi?query=aue&sektion=4&format=html) 驱动支持以下基于 ADMtek AN986 Pegasus 芯片组的 USB 快速以太网适配器：
...
* I-O Data USB ETTX
* Kingston KNU101TX
* 包含 AN986 Pegasus 芯片组的 LinkSys USB10T 适配器， USB10TA, USB10TX, USB100TX, USB100H1
...
[axge(4)](https://man.freebsd.org/cgi/man.cgi?query=axge&sektion=4&format=html) 驱动支持以下 USB 千兆以太网控制器：

* ASIX Electronics AX88179A
* ASIX Electronics AX88179
* ASIX Electronics AX88178A

[bce(4)](https://man.freebsd.org/cgi/man.cgi?query=bce&sektion=4&format=html) 驱动为基于 QLogic NetXtreme II 千兆以太网控制器家族的各类 NIC 提供支持，包括以下型号：
...
[bge(4)](https://man.freebsd.org/cgi/man.cgi?query=bge&sektion=4&format=html) 驱动为基于 Broadcom BCM570x 千兆以太网控制器芯片家族的各类 NIC 提供支持，包括以下型号：
...
[bnxt(4)](https://man.freebsd.org/cgi/man.cgi?query=bnxt&sektion=4&format=html) 驱动支持以下 Broadcom 10Gb 至 400Gb 以太网控制器：
...
[bxe(4)](https://man.freebsd.org/cgi/man.cgi?query=bxe&sektion=4&format=html) 驱动为基于 QLogic NetXtreme II 10Gb 以太网控制器芯片家族的各类 NIC 提供支持，包括以下型号：
...
[cas(4)](https://man.freebsd.org/cgi/man.cgi?query=cas&sektion=4&format=html) 驱动支持的芯片如下：
...
[cdce(4)](https://man.freebsd.org/cgi/man.cgi?query=cdce&sektion=4&format=html) 驱动支持实现 USB Communication Device Class Ethernet Control Model (CDC ECM) 或 Network Control Model (CDC NCM) 协议的 USB 以太网接口，例如：
...
[cue(4)](https://man.freebsd.org/cgi/man.cgi?query=cue&sektion=4&format=html) 驱动支持基于 CATC USB-EL1210A 的 USB 以太网 适配器，包括：
...
[cxgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbe&sektion=4&format=html) 驱动支持基于 T7 ASIC 的 400Gb、200Gb、50Gb 和 10Gb 以太网适配器：
...
[cxgbev(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbev&sektion=4&format=html) 驱动支持基于 T6 ASIC 的 100Gb 和 25Gb 以太网适配器上的 Virtual Functions：
...
**cxgbev** 驱动支持基于 T5 ASIC 的 40Gb、10Gb 和 1Gb 以太网适配器上的 Virtual Functions：
...
**cxgbev** 驱动支持基于 T4 ASIC 的 10Gb 和 1Gb 以太网适配器上的 Virtual Functions：
...
* Compaq Presario 7900 系列桌面上的内置 10Mbps 以太网 （21143, non-MII）
...
已知内置于以下 Freescale system-on-chip 设备的千兆以太网控制器可在 [dtsec(4)](https://man.freebsd.org/cgi/man.cgi?query=dtsec&sektion=4&format=html) 驱动下工作：
...
[enic(4)](https://man.freebsd.org/cgi/man.cgi?query=enic&sektion=4&format=html) 驱动应支持所有已知的 Cisco VIC 卡。

[et(4)](https://man.freebsd.org/cgi/man.cgi?query=et&sektion=4&format=html) 驱动支持 Agere ET1310 10/100/Gigabit 以太网适配器。
...
[genet(4)](https://man.freebsd.org/cgi/man.cgi?query=genet&sektion=4&format=html) 驱动支持树莓派 4 Model B 及相关系统上 Broadcom BCM2711 的以太网控制器部分。 它使用 BCM54213PE PHY。
...
[ice(4)](https://man.freebsd.org/cgi/man.cgi?query=ice&sektion=4&format=html) 驱动支持以下 Intel 800 系列 1Gb 至 200Gb 以太网控制器：
...
**ice** 驱动支持此系列中具有 SFP28/QSFP28 cages 的部分适配器， 这些适配器的固件要求使用 Intel 认证模块； 这些认证模块列出如下。 驱动无法禁用此认证检查。
...
[igc(4)](https://man.freebsd.org/cgi/man.cgi?query=igc&sektion=4&format=html) 驱动支持以下 2.5Gb 以太网控制器：
...
以下设备受 [ipheth(4)](https://man.freebsd.org/cgi/man.cgi?query=ipheth&sektion=4&format=html) 驱动支持：
...
[ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 驱动支持 Intel Ethernet 700 系列。 此系列中具有 SFP+/SFP28/QSFP+ cages 的大多数适配器 具有要求使用 Intel 认证模块的固件；这些 认证模块列出如下。 驱动无法禁用此认证检查。
...
注意，适配器还支持所有符合 SFF-8431 v4.1 和 SFF-8472 v10.4 规范的被动和主动 限幅直连电缆。

此列表并非详尽无遗；请查阅产品文档以获取 最新支持的介质列表。
...
[kue(4)](https://man.freebsd.org/cgi/man.cgi?query=kue&sektion=4&format=html) 驱动支持基于 Kawasaki LSI KL5KLUSB101B 的 USB 以太网 适配器，包括：
...
[mgb(4)](https://man.freebsd.org/cgi/man.cgi?query=mgb&sektion=4&format=html) 驱动支持 Microchip PCIe 千兆以太网接口，包括：
...
[mlx5ib(4)](https://man.freebsd.org/cgi/man.cgi?query=mlx5ib&sektion=4&format=html) 驱动支持 100Gb、50Gb、40Gb、25Gb 和 10Gb 网络适配器。 ConnectX-4 支持：10/20/25/40/50/56/100Gb/s 速率。 ConnectX-4 LX 支持：10/25/40/50Gb/s 速率（且功耗更低）：
...
[mos(4)](https://man.freebsd.org/cgi/man.cgi?query=mos&sektion=4&format=html) 驱动支持的适配器包括：
...
[muge(4)](https://man.freebsd.org/cgi/man.cgi?query=muge&sektion=4&format=html) 驱动支持 Microchip USB 千兆以太网接口，包括：
...
[qlnxe(4)](https://man.freebsd.org/cgi/man.cgi?query=qlnxe&sektion=4&format=html) 驱动支持基于以下芯片组的 25/40/100 千兆以太网及 CNA 适配器：

* QLogic 45000 series
* QLogic 41000 series

[qlxgb(4)](https://man.freebsd.org/cgi/man.cgi?query=qlxgb&sektion=4&format=html) 驱动支持基于以下芯片组的 10 千兆以太网及 CNA 适配器：

* QLogic 3200 series
* QLogic 8200 series

[qlxgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=qlxgbe&sektion=4&format=html) 驱动支持基于以下芯片组的 10 千兆以太网及 CNA 适配器：

* QLogic 8300 series

[qlxge(4)](https://man.freebsd.org/cgi/man.cgi?query=qlxge&sektion=4&format=html) 驱动支持基于以下芯片组的 10 千兆以太网及 CNA 适配器：
...
[ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动支持基于 Alteon Tigon I 和 II 芯片的千兆以太网适配器。 **ti** 驱动已通过以下适配器测试：
...
已知以下 Freescale 系统级芯片（SoC）内置的千兆以太网控制器可与 [tsec(4)](https://man.freebsd.org/cgi/man.cgi?query=tsec&sektion=4&format=html) 驱动配合工作：
...
[ufoma(4)](https://man.freebsd.org/cgi/man.cgi?query=ufoma&sektion=4&format=html) 驱动支持的设备包括：

* SHARP FOMA SH902i
* KYOCERA PHS AH-K3001V (a.k.a Kyopon)
* SANYO Vodafone3G V801SA

[uhso(4)](https://man.freebsd.org/cgi/man.cgi?query=uhso&sektion=4&format=html) 驱动应当能与 Option 的大多数设备配合工作。 以下设备已验证可正常工作：
...
该设备包含一个名为“Zero-CD”的大容量存储设备，其中包含 Microsoft Windows 的驱动程序；这是设备的默认模式。 **uhso** 驱动会自动将设备从“Zero-CD”模式切换到调制解调器模式。 可通过 **sysctl(8)** 将 `hw.usb.uhso.auto_switch` 设置为 0 来禁用此行为。
...
3C656 系列 CardBus 卡和 3C556 系列 MiniPCI 卡均内置专有调制解调器。 **xl** 驱动和任何其他 FreeBSD 驱动均不支持此调制解调器。

### 以太网交换机

[adm6996fc(4)](https://man.freebsd.org/cgi/man.cgi?query=adm6996fc&sektion=4&format=html) 驱动支持 Infineon ADM6996FC 快速以太网交换机控制器。

[ar40xx_switch](https://man.freebsd.org/cgi/man.cgi?query=ar40xx&apropos=&&sektion=4&format=html) 驱动支持以下千兆以太网交换机控制器：

* Qualcomm IPQ 4019 Five-port Gigabit Ethernet Switch
* Qualcomm IPQ 4018 Five-port Gigabit Ethernet Switch

[arswitch(4)](https://man.freebsd.org/cgi/man.cgi?query=arswitch&sektion=4&format=html) 驱动支持以下以太网交换机控制器：
...
[e6000sw(4)](https://man.freebsd.org/cgi/man.cgi?query=e6000sw&sektion=4&format=html) 驱动支持以下千兆以太网交换机控制器：
...
[e6060sw(4)](https://man.freebsd.org/cgi/man.cgi?query=e6060sw&sektion=4&format=html) 驱动支持以下快速以太网交换机控制器：
...
[ip17x(4)](https://man.freebsd.org/cgi/man.cgi?query=ip17x&sektion=4&format=html) 驱动支持以下快速以太网交换机控制器：
...
[mtkswitch(4)](https://man.freebsd.org/cgi/man.cgi?query=mtkswitch&sektion=4&format=html) 驱动支持以下以太网交换机控制器：
...
[ath(4)](https://man.freebsd.org/cgi/man.cgi?query=ath&sektion=4&format=html) 驱动支持所有 Atheros Cardbus、ExpressCard、PCI 和 PCIe 卡，但基于 AR5005VL 芯片组的卡除外。

[ath_hal(4)](https://man.freebsd.org/cgi/man.cgi?query=ath_hal&sektion=4&format=html) 模块支持的卡包括以下这些：
...
**bwi** 驱动使用 Broadcom 固件的旧版 v3。 虽然旧版固件支持大多数 BCM43xx 部件，但 **bwn(4)** 驱动对其支持的较新芯片工作得更好。 如果你使用较旧的 Broadcom 芯片组（BCM4301、BCM4303 和 BCM4306 rev 2），必须使用 **bwi** 驱动。 **bwn(4)** 使用的 v4 版本固件不支持这些芯片。
...
较旧 Broadcom 芯片组（BCM4301、BCM4303 和 BCM4306 rev 2）的用户必须使用 **bwi(4)**， 因为 v4 版本的固件不支持这些芯片。 较新的固件过大，无法装入这些旧芯片。

[ipw(4)](https://man.freebsd.org/cgi/man.cgi?query=ipw&sektion=4&format=html) 驱动为 Intel PRO/Wireless 2100a/b MiniPCI 网络适配器提供支持。
...
[iwlwifi(4)](https://man.freebsd.org/cgi/man.cgi?query=iwlwifi&sektion=4&format=html) 驱动支持来自 **mvm** 子驱动程序的 PCIe 设备，涵盖以下芯片组世代：
...
**iwlwifi** 驱动支持来自 **mld** 子驱动程序的 PCIe 设备，涵盖以下芯片组世代：
...
[iwm(4)](https://man.freebsd.org/cgi/man.cgi?query=iwm&sektion=4&format=html) 驱动支持以下 PCIe Wi-Fi 设备：
...
[iwn(4)](https://man.freebsd.org/cgi/man.cgi?query=iwn&sektion=4&format=html) 驱动为以下设备提供支持：
...
[iwx(4)](https://man.freebsd.org/cgi/man.cgi?query=iwx&sektion=4&format=html) 驱动支持以下 M.2 无线网络适配器：
...
[malo(4)](https://man.freebsd.org/cgi/man.cgi?query=malo&sektion=4&format=html) 驱动支持的卡包括以下这些：
...
[mtw(4)](https://man.freebsd.org/cgi/man.cgi?query=mtw&sektion=4&format=html) 驱动支持基于 MediaTek MT7601U 的 USB 无线网络适配器，包括（但并非全部经过测试）：
...
[rtw88(4)](https://man.freebsd.org/cgi/man.cgi?query=rtw88&sektion=4&format=html) 驱动支持以下芯片组的 PCIe 设备：
...
[rtw89(4)](https://man.freebsd.org/cgi/man.cgi?query=rtw89&sektion=4&format=html) 驱动支持以下芯片组的 PCIe 设备：
...
[rtwn_pci(4)](https://man.freebsd.org/cgi/man.cgi?query=rtwn_pci&sektion=4&format=html) 驱动支持以下 PCIe Wi-Fi 设备：

* Realtek 802.11n wireless 8188  (RTL8188EE)
* Realtek 802.11n wireless 8192  (RTL8192CE)

[rtwn_usb(4)](https://man.freebsd.org/cgi/man.cgi?query=rtwn_usb&sektion=4&format=html) 驱动支持基于特定 Realtek RTL 8188/8192/8812 和 8821 芯片组的 USB 无线网络适配器，包括：
...
[uath(4)](https://man.freebsd.org/cgi/man.cgi?query=uath&sektion=4&format=html) 驱动应当能与以下适配器配合工作：
...
[wpi(4)](https://man.freebsd.org/cgi/man.cgi?query=wpi&sektion=4&format=html) 驱动为 Intel PRO/Wireless 3945ABG Mini PCIe 网络适配器提供支持。
...
[u3g(4)](https://man.freebsd.org/cgi/man.cgi?query=u3g&sektion=4&format=html) 驱动支持以下蜂窝调制解调器：

* Option GT 3G Fusion, GT Fusion Quad, etc. （仅 3G，不支持 WLAN）
...
还支持更多设备，完整列表请参见 **/sys/dev/usb/serial/u3g.c**。

[umb(4)](https://man.freebsd.org/cgi/man.cgi?query=umb&sektion=4&format=html) 驱动应当支持任何实现 MBIM 的 USB 设备，包括以下蜂窝调制解调器：
...
[scc(4)](https://man.freebsd.org/cgi/man.cgi?query=scc&sektion=4&format=html) 驱动支持以下类别的串行通信控制器：

* QUICC：Freescale/NXP 四重集成通信控制器。
* Z8530：基于 Zilog 8530 的串行通信控制器。

[puc(4)](https://man.freebsd.org/cgi/man.cgi?query=puc&sektion=4&format=html) 驱动支持以下 PCI/PCIe 多端口串行和并行适配器：
...
[ubser(4)](https://man.freebsd.org/cgi/man.cgi?query=ubser&sektion=4&format=html) 驱动为 BWCT 控制台管理串行适配器提供支持。

[uchcom(4)](https://man.freebsd.org/cgi/man.cgi?query=uchcom&sektion=4&format=html) 驱动支持以下 USB 转串行 UART 控制器：
...
[ucycom(4)](https://man.freebsd.org/cgi/man.cgi?query=ucycom&sektion=4&format=html) 驱动目前支持以下集成 Cypress USB 转 RS232 桥接芯片的设备：

* DeLorme Earthmate USB GPS 接收器

[uftdi(4)](https://man.freebsd.org/cgi/man.cgi?query=uftdi&sektion=4&format=html) 驱动支持以下 USB 转串行 UART 控制器：
...
[umcs(4)](https://man.freebsd.org/cgi/man.cgi?query=umcs&sektion=4&format=html) 驱动已在以下适配器上测试：
...
以下设备应能与 [uslcom(4)](https://man.freebsd.org/cgi/man.cgi?query=uslcom&sektion=4&format=html) 驱动配合使用：
...
部分板载 CS4610 芯片搭配 CS423x ISA 编解码器，而非 CS4297 AC97 编解码器。 **snd_csa** 驱动目前尚不支持此类配置。
...
* Creative Sound Blaster Audigy (CA0100 and CA0101 Chipset). 提供 PCM 和两个 MIDI 接口。
...
* Creative Sound Blaster Audigy 2 Value (CA0108 Chipset). PCM 支持仅限于 48kHz/16 位立体声（该芯片组的 192kHz/24 位部分不受支持）。 此卡不支持 MIDI。

**snd_emu10kx** 驱动 *不* 支持以下声卡（尽管它们的名称与某些受支持的卡相似）：

* Creative Sound Blaster Live! 24-Bit， FreeBSD 识别为 “`emu10k1x Soundblaster Live! 5.1`”。
* Creative Sound Blaster Audigy LS / ES， FreeBSD 识别为 “`CA0106-DAT Audigy LS`”。
...
[snd_hda(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hda&sektion=4&format=html) 驱动支持 PCI 类 04h（多媒体）、子类 03h（HDA）音频控制器和与 Intel High Definition Audio 1.0 规范兼容的编解码器。
...
默认情况下，每个 **pcm(4)** 设备对应声卡上的一个物理端口。 对于 ADAT 端口，支持 8 通道、4 通道和 2 通道格式。 ADAT 通道的有效数量为：单速（32kHz-48kHz）8 通道，倍速（64kHz-96kHz）4 通道，四倍速（128kHz-192kHz）2 通道。 根据所选采样率和通道格式，并非所有 pcm 通道都能映射到 ADAT 通道，反之亦然。
...
[snd_uaudio(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_uaudio&sektion=4&format=html) 驱动为 USB 音频类设备和 USB MIDI 类设备提供支持。
...
支持多种 USB 外设；已知可正常工作的设备在本节列出。 由于大多数 USB 设备具有通用性，除少数例外，任何属于给定类别的设备都会受支持，即使此处未明确列出。

> **注意**：
>
> USB 蓝牙适配器可在 [蓝牙](https://www.freebsd.org/releases/15.1R/hardware.html#bluetooth) 中找到。
>
> USB 人机接口设备可在 [其他设备](https://www.freebsd.org/releases/15.1R/hardware.html#misc) 中找到。
>
> USB 网络接口可在 [以太网接口](https://www.freebsd.org/releases/15.1R/hardware.html#ethernet) 或 [无线网络接口](https://www.freebsd.org/releases/15.1R/hardware.html#wlan) 中找到。
>
> USB 串行接口可在 [串行接口](https://www.freebsd.org/releases/15.1R/hardware.html#serial) 中找到。

[atp(4)](https://man.freebsd.org/cgi/man.cgi?query=atp&sektion=4&format=html) 驱动为以下产品 ID 提供支持：
...
如需查看触摸板的产品 ID，请在 **lshal(1)** 的输出中搜索‘Trackpad’，并查找属性 **usb_device.product_id**。

[atopcase(4)](https://man.freebsd.org/cgi/man.cgi?query=atopcase&sektion=4&format=html) 驱动支持以下 2015-2018 年生产的 MacBook：
...
[abtn(4)](https://man.freebsd.org/cgi/man.cgi?query=abtn&sektion=4&format=html) 驱动支持以下设备的扩展键盘按键（特殊 F 键）：

* Apple iBook 键盘
* Apple PowerBook 键盘

[akbd(4)](https://man.freebsd.org/cgi/man.cgi?query=akbd&sektion=4&format=html) 驱动支持的设备包括：
...
[ams(4)](https://man.freebsd.org/cgi/man.cgi?query=ams&sektion=4&format=html) 驱动支持的设备包括：
...
[dwcotg(4)](https://man.freebsd.org/cgi/man.cgi?query=dwcotg&sektion=4&format=html) 驱动支持许多嵌入式设备中采用的 Synopsys DesignWare USB 控制器。
...
[uipaq(4)](https://man.freebsd.org/cgi/man.cgi?query=uipaq&sektion=4&format=html) 驱动支持以下 iPAQ 设备：
...
[umass(4)](https://man.freebsd.org/cgi/man.cgi?query=umass&sektion=4&format=html) 驱动支持 USB 大容量存储设备，例如：
...
[rtsx(4)](https://man.freebsd.org/cgi/man.cgi?query=rtsx&sektion=4&format=html) 驱动支持以下 Realtek SD 卡读卡器：
...
[ulpt(4)](https://man.freebsd.org/cgi/man.cgi?query=ulpt&sektion=4&format=html) 驱动为 USB 打印机和并行打印机转换线缆提供支持，包括以下设备：
...
[sbp(4)](https://man.freebsd.org/cgi/man.cgi?query=sbp&sektion=4&format=html) 驱动支持 FireWire 串行总线协议 2（SBP-2）存储设备。
...
[ccr(4)](https://man.freebsd.org/cgi/man.cgi?query=ccr&sektion=4&format=html) 驱动支持基于 T6 ASIC 的适配器上的加密加速引擎：
...
[hifn(4)](https://man.freebsd.org/cgi/man.cgi?query=hifn&sektion=4&format=html) 驱动支持包含 Hifn 7751、7951、7811、7955 和 7956 芯片组的各类卡，例如：

Invertex AEON

> 已不再生产。 提供 128KB SRAM 型号和 2MB DRAM 型号。
...
> 唯一基于 7811 的板卡（速度比 7751 快， 并带有随机数发生器）。
...
> 详见 *<http://www.soekris.com/>*。 内含 7951，支持对称运算和随机数运算。

Soekris Engineering vpn1401 and vpn1411

> 详见 *<http://www.soekris.com/>*。 内含 7955，支持对称运算和随机数运算。
...
* Intel QuickAssist Adapter 8960/Intel QuickAssist Adapter 8970 （曾用名“Lewis Hill”）
...
[safexcel(4)](https://man.freebsd.org/cgi/man.cgi?query=safexcel&sektion=4&format=html) 驱动支持某些 Marvell 片上系统中所配备的 Inside Secure EIP-97 设备的加密加速功能。
...
* HID 鼠标（[hms(4)](https://man.freebsd.org/cgi/man.cgi?query=hms&sektion=4&format=html) 驱动）和触摸设备（[hmt(4)](https://man.freebsd.org/cgi/man.cgi?query=hmt&sektion=4&format=html) 驱动）通过 USB、I2C、Hyper-V 或 Apple SPI 连接
...
* PS/2 鼠标及兼容设备，包括许多笔记本指针设备（[psm(4)](https://man.freebsd.org/cgi/man.cgi?query=psm&sektion=4&format=html) 驱动）
...
[hgame(4)](https://man.freebsd.org/cgi/man.cgi?query=hgame&sektion=4&format=html) 驱动支持 HID 游戏手柄、摇杆和控制器，例如：
...
[ads111x(4)](https://man.freebsd.org/cgi/man.cgi?query=ads111x&sektion=4&format=html) 驱动为以下设备提供支持：
...
[at45d(4)](https://man.freebsd.org/cgi/man.cgi?query=at45d&sektion=4&format=html) 驱动为以下设备提供支持：
...
[gpioths(4)](https://man.freebsd.org/cgi/man.cgi?query=gpioths&sektion=4&format=html) 驱动为以下设备提供支持：

	DHT11       DHT12
	DHT21       DHT22
	AM3201      AM3202

所支持的设备彼此相似，主要差异在于精度和分辨率。 这些设备需要单根数据线进行通信，采用的自定义协议与 Maxim 的 1-wire(tm) 不兼容。 AM320x 设备还支持连接到 i2c 总线， 但本驱动仅支持单线连接方式。

[ig4(4)](https://man.freebsd.org/cgi/man.cgi?query=ig4&sektion=4&format=html) 支持基于 Synopsys DesignWare IP 的 I2C 控制器，可见于 自第四代起的 Intel Core(TM) 处理器、Intel Bay Trail、Apollo Lake SoC 系列以及部分 AMD 系统。
...
[mx25l(4)](https://man.freebsd.org/cgi/man.cgi?query=mx25l&sektion=4&format=html) 驱动支持以下 SPI 闪存设备：
...
当前版本的 [netfpga10g_nf10bmac(4)](https://man.freebsd.org/cgi/man.cgi?query=netfpga10g_nf10bmac&sektion=4&format=html) 驱动适用于 NetFPGA-10G Embedded CPU Ethernet Core version 1.00a 的一个 PIO 模式接口。

以下 PLX/Avago/Broadcom 芯片由 [ntb_hw_plx(4)](https://man.freebsd.org/cgi/man.cgi?query=ntb_hw_plx&sektion=4&format=html) 驱动支持：
...
[ow_temp(4)](https://man.freebsd.org/cgi/man.cgi?query=ow_temp&sektion=4&format=html) 驱动支持以下温度传感器：
...
[superio(4)](https://man.freebsd.org/cgi/man.cgi?query=superio&sektion=4&format=html) 驱动支持 Nuvoton（前身为 Winbond）和 ITE 生产的众多 Super I/O 控制器，包括：
...
[u2f(4)](https://man.freebsd.org/cgi/man.cgi?query=u2f&sektion=4&format=html) 驱动支持 FIDO/U2F 兼容的 USB 安全密钥。

[udl(4)](https://man.freebsd.org/cgi/man.cgi?query=udl&sektion=4&format=html) 驱动支持基于 DisplayLink DL-120 和 DL-160 图形芯片的 USB 显示设备，包括：
