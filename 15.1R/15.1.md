...
支持的硬件信息请参阅 [FreeBSD 15.1-RELEASE 硬件兼容列表](https://www.freebsd.org/releases/15.1R/hardware)。
...
[find(1)](https://man.freebsd.org/cgi/man.cgi?query=find&sektion=1&format=html) 工具新增选项 `-xattr` 和 `-xattrname`，用于基于扩展属性搜索文件。选项 `-xattr` 匹配具有任意扩展属性的文件，`-xattrname` 则允许按特定属性名过滤，可选择性添加命名空间前缀，如“user:”或“system:”。[5cbb1e05086c](https://cgit.freebsd.org/src/commit/?id=5cbb1e05086c)。（由 Klara, Inc. 赞助）
...
[bectl(8)](https://man.freebsd.org/cgi/man.cgi?query=bectl&sektion=8&format=html) 新增标志 `-E`，用于创建空引导环境而不克隆当前运行环境。用户可创建独立的新引导环境，用于手动安装或配置。[2b5087e9850b](https://cgit.freebsd.org/src/commit/?id=2b5087e9850b)。
...
[newfs(8)](https://man.freebsd.org/cgi/man.cgi?query=newfs&sektion=8&format=html) 工具新增标志 `-u`，用于禁用软更新及软更新日志，提供关闭 UFS2 文件系统默认软更新的方法。[f5a51cfba15c](https://cgit.freebsd.org/src/commit/?id=f5a51cfba15c)。（由 Klara, Inc.; NetApp, Inc. 赞助）

[zfs(8)](https://man.freebsd.org/cgi/man.cgi?query=zfs&sektion=8&format=html) clone 命令现接受选项 `-u`，阻止新创建的数据集自动挂载。用户可创建克隆而不立即挂载，便于脚本编写或为后续使用准备数据集。[6ab8f46c6c47](https://cgit.freebsd.org/src/commit/?id=6ab8f46c6c47)。

[zdb(8)](https://man.freebsd.org/cgi/man.cgi?query=zdb&sektion=8&format=html) 工具新增选项 `-O`，与 `-r` 配合使用时可直接指定对象 ID，允许按对象 ID 而非路径名复制文件。[ccb7c82aa15b](https://cgit.freebsd.org/src/commit/?id=ccb7c82aa15b)。
...
[dtrace(1)](https://man.freebsd.org/cgi/man.cgi?query=dtrace&sektion=1&format=html) 工具现支持 arm64 上的变量 `ustackdepth`，允许用户在该架构的 DTrace 脚本中查询用户空间栈深度。[00412df3265e](https://cgit.freebsd.org/src/commit/?id=00412df3265e)。

DTrace 内核诊断消息现使用更直观的 `dtrace` 前缀，改善日志搜索一致性。[7781f5e09ffc](https://cgit.freebsd.org/src/commit/?id=7781f5e09ffc)。

`bsdinstall` 脚本现默认执行 pkgbase 安装，组件通过变量 `COMPONENTS` 指定，同时若定义了 `DISTRIBUTIONS`，传统的 distset 安装仍可使用。[cb7880bb0900](https://cgit.freebsd.org/src/commit/?id=cb7880bb0900)。（由 ConnectWise 赞助）
...
[pkg(7)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=7&format=html) 软件包现已安装在使用 pkgbase 的 VM 和云镜像上，无需从 ports 仓库手动安装 pkg 即可更新基本系统。[c8d759230a0e](https://cgit.freebsd.org/src/commit/?id=c8d759230a0e)。（由 Amazon 赞助）
...
**bcm2838_xhci** 驱动现可通过兼容字符串正确识别树莓派 400 硬件，使该平台上的 xhci 初始化和 USB 功能（包括外部端口和内置键盘）正常工作。[861deac98c4c](https://cgit.freebsd.org/src/commit/?id=861deac98c4c)。
...
[zpool(8)](https://man.freebsd.org/cgi/man.cgi?query=zpool&sektion=8&format=html) prefetch 命令现支持 BRT（块引用表）元数据，改善了块克隆操作和克隆块释放的性能。参数 `-t` 现为可选；省略时，命令同时预取 DDT 和 BRT 元数据。[41878d57eaf3](https://cgit.freebsd.org/src/commit/?id=41878d57eaf3)。
...
新增了对 RFC 7217 稳定 IPv6 地址生成的支持，通过 ifconfig 标志 `stableaddr` 和相关 sysctl 实现。此功能默认禁用，保持现有行为。[ifconfig(8)](https://man.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8&format=html) 手册页已更新文档。[865e0b20ca3c](https://cgit.freebsd.org/src/commit/?id=865e0b20ca3c)。

由于实现中存在严重缺陷，导致高概率冲突及部分生成的地址超出预期 OUI 范围，[if_bridge(4)](https://man.freebsd.org/cgi/man.cgi?query=if_bridge&sektion=4&format=html)、[if_epair(4)](https://man.freebsd.org/cgi/man.cgi?query=if_epair&sektion=4&format=html) 及其他接口生成的稳定 MAC 地址将自 15.0 起发生变化。若需保留原 MAC 地址（例如用于 DHCP 租约），须注意持久化现有 MAC 地址，如在 [rc.conf(5)](https://man.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5&format=html) 中使用 `ifconfig_bridgeN="... ether aa:bb:cc:dd:ee:ff"`。[604d34c23f77](https://cgit.freebsd.org/src/commit/?id=604d34c23f77)。
...
[resolver(5)](https://man.freebsd.org/cgi/man.cgi?query=resolver&sektion=5&format=html) 手册页现记录了选项 `edns0`、`inet6`、`insecure1`、`insecure2`、`no-check-names` 和 `rotate`，这些选项由 [res_init(3)](https://man.freebsd.org/cgi/man.cgi?query=res_init&sektion=3&format=html) 解析但此前未文档化。[c988f316d6c6](https://cgit.freebsd.org/src/commit/?id=c988f316d6c6)。
...
新增已弃用 blacklist 工具（[blacklistctl(8)](https://man.freebsd.org/cgi/man.cgi?query=blacklistctl&sektion=8&format=html)、[blacklistd(8)](https://man.freebsd.org/cgi/man.cgi?query=blacklistd&sektion=8&format=html)、[blacklistd.conf(5)](https://man.freebsd.org/cgi/man.cgi?query=blacklistd.conf&sektion=5&format=html)、[libblacklist(3)](https://man.freebsd.org/cgi/man.cgi?query=libblacklist&sektion=3&format=html)）的手册页，记录了其弃用状态和细节。这为已弃用的 blacklist 工具提供了更新文档，这些工具现已重命名为 blocklist。[f935c0f66f75](https://cgit.freebsd.org/src/commit/?id=f935c0f66f75)。
