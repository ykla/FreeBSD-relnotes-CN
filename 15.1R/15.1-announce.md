# FreeBSD 15.1-RELEASE 发行公告

日期：2026 年 6 月 16 日

FreeBSD 发布工程团队很高兴地宣布 FreeBSD 15.1-RELEASE 现已发布。
这是 stable/15 分支的第二个版本。

部分亮点：

* iwlwifi(4) 及其他基于 LinuxKPI 的无线网络驱动现在基于 Linux v7.0。

* 使用 pkgbase 的 FreeBSD 云镜像现包含 pkg(8)，并支持首次启动时自动进行 pkgbase 软件包更新。

* 新增 `kern.sched.name` 可调参数，允许在启动时选择内核调度器。

* 在全面支持 C23 版本的 C 编程语言方面取得重大进展。

* Unicode 支持已更新到 Unicode 17.0.0 和 CLDR 48，新增 4,803 个字符。

完整的新功能、支持的硬件和已知问题列表，请参阅在线发行说明、硬件兼容性说明和勘误列表，地址如下：

* [https://www.FreeBSD.org/releases/15.1R/relnotes/](https://www.FreeBSD.org/releases/15.1R/relnotes/)
* [https://www.FreeBSD.org/releases/15.1R/hardware/](https://www.FreeBSD.org/releases/15.1R/hardware/)
* [https://www.FreeBSD.org/releases/15.1R/errata/](https://www.FreeBSD.org/releases/15.1R/errata/)
...
FreeBSD 项目将 FreeBSD 15.1-RELEASE 献给 Peter G. Neumann 以示纪念。他长期与 FreeBSD 开发者合作研究基于能力的（capability-based）安全；此外，通过对 Multics 等历史性项目的贡献，他开创了当今我们习以为常的众多操作系统安全核心概念。

## 获取途径

FreeBSD 15.1-RELEASE 现已支持 amd64、aarch64、armv7、powerpc64、powerpc64le 和 riscv64 架构。

FreeBSD 15.1-RELEASE 可从可启动 ISO 镜像或通过网络安装。
部分架构还支持从 USB 闪存盘安装。
所需文件可按以下说明下载。
...
* [https://www.FreeBSD.org/releases/15.1R/signatures/](https://www.FreeBSD.org/releases/15.1R/signatures/)

本公告的 PGP 签名版本可在以下地址获取：

* [https://www.FreeBSD.org/releases/15.1R/announce.asc](https://www.FreeBSD.org/releases/15.1R/announce.asc)
...
包含 FreeBSD 基本系统。还支持启动到基于“livefs”的救援模式。不含预构建的第三方软件包。
...
可写入 USB 闪存盘（U 盘），用于在可从 USB 启动的机器上进行安装。还支持启动到基于“livefs”的救援模式。不含预构建的第三方软件包。
...
为方便无控制台访问系统的用户，默认提供 `freebsd` 用户（密码为 `freebsd`）用于 `ssh(1)` 访问。此外，`root` 用户密码设为 `root`；强烈建议获得系统访问权限后立即更改这两位用户的密码。

要将 FreeBSD/arm 镜像写入 SD 卡，使用 `dd(1)` 工具，将 *KERNEL* 替换为系统对应的内核配置名。
...
amd64（x86_64）、i386（x86_32）、AArch64（arm64）和 RISCV（riscv64）架构还提供预安装的虚拟机镜像，格式包括 `QCOW2`、`VHD` 和 `VMDK` 磁盘镜像格式，以及 raw（未格式化）镜像。

FreeBSD 15.1-RELEASE 还可在以下云托管平台上获取：
...
* Google(R) Compute Engine(TM)：
  ...
  FreeBSD 镜像预计也可在 Google Compute Engine(TM) Marketplace 获取：

  [https://console.cloud.google.com/marketplace/browse?filter=category:os&filter=price:free&q=freebsd](https://console.cloud.google.com/marketplace/browse?filter=category:os&filter=price:free&q=freebsd)

* Microsoft Azure(TM)：

  FreeBSD 虚拟机镜像预计可在 Azure Marketplace 获取：

  [https://azuremarketplace.microsoft.com/marketplace/apps/freebsd.freebsd-15_1](https://azuremarketplace.microsoft.com/marketplace/apps/freebsd.freebsd-15_1)

## 下载

FreeBSD 15.1-RELEASE 可从以下站点通过 https 下载：

* [https://download.freebsd.org/releases/ISO-IMAGES/15.1/](https://download.freebsd.org/releases/ISO-IMAGES/15.1/)

FreeBSD 15.1-RELEASE 虚拟机镜像可从以下地址下载：

* [https://download.freebsd.org/releases/VM-IMAGES/15.1-RELEASE/](https://download.freebsd.org/releases/VM-IMAGES/15.1-RELEASE/)

FreeBSD 15.1-RELEASE BASIC-CI 镜像可从以下地址下载：

* [https://download.freebsd.org/releases/CI-IMAGES/15.1-RELEASE/](https://download.freebsd.org/releases/CI-IMAGES/15.1-RELEASE/)

FreeBSD 15.1-RELEASE OCI 容器镜像可从以下地址下载：

* [https://download.freebsd.org/releases/OCI-IMAGES/15.1-RELEASE/](https://download.freebsd.org/releases/OCI-IMAGES/15.1-RELEASE/)

也可在 Docker 和 GitHub 容器仓库中获取。

关于安装 FreeBSD 或将现有机器更新到 15.1-RELEASE 的说明，请参阅：

* [https://www.FreeBSD.org/releases/15.1R/installation/](https://www.FreeBSD.org/releases/15.1R/installation/)

## 支持

本点版本 FreeBSD 15.1-RELEASE 将受支持至 2027 年 3 月 31 日。
上一版本 FreeBSD 15.0-RELEASE 将于 2026 年 9 月 30 日到达生命周期终点。
...
许多公司为支持 FreeBSD 15.1 的发布工程活动捐赠了设备、网络访问或人力，包括：

| | |
| :--- | :--- |
| [Amazon](https://www.amazon.com) | [Fastly](https://www.fastly.com) |
| [FreeBSD 基金会](https://www.freebsdfoundation.org) | [Juniper Networks](https://www.juniper.net) |
| [New York Internet](https://www.nyi.net) | [Sentex Data Communications](https://www.sentex.ca) |
| [Tarsnap](https://www.tarsnap.com) | [365 Data Centers](https://365datacenters.com/) |
| [Your.org](https://www.your.org) | |

15.1-RELEASE 的发布工程团队包括：

| | |
| :--- | :--- |
| Antoine Brodin <antoine@FreeBSD.org> | 软件包构建 |
| Dave Cottlehuber <dch@FreeBSD.org> | 发布工程 |
| Sergio Carlavilla Delgado <carlavilla@FreeBSD.org> | 发布工程 |
| Marc Fonvieille <blackend@FreeBSD.org> | 发布工程、文档 |
| Craig Leres <leres@FreeBSD.org> | 发布工程 |
| Xin Li <delphij@FreeBSD.org> | 发布工程、安全团队联络 |
| Ed Maste <emaste@FreeBSD.org> | 发布工程副负责人 |
| Colin Percival <cperciva@FreeBSD.org> | 发布工程负责人、15.1-RELEASE 发布工程师 |
| Vladlen Popolitov <vladlen@FreeBSD.org> | 发布工程 |
| Muhammad Moinur Rahman <bofh@FreeBSD.org> | 发布工程 |
| Lexi Winter <ivy@FreeBSD.org> | 发布工程 |
| Alexander Ziaee <ziaee@FreeBSD.org> | 发布工程 |
...
喜爱 FreeBSD？
请通过向 FreeBSD 基金会[捐赠](https://www.freebsdfoundation.org/donate/) 来支持本版本及未来版本！
