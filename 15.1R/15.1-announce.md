...
FreeBSD 发布工程团队很高兴地宣布 FreeBSD 15.1-RELEASE 现已发布。这是 stable/15 分支的第二个版本。
...
* 新增可调参数 `kern.sched.name`，允许在启动时选择内核调度器。
...
FreeBSD 15.1-RELEASE 可从可启动 ISO 镜像或通过网络安装。部分架构还支持从 USB 闪存盘安装。所需文件可按以下说明下载。
...
* Google Compute Engine(TM):
  ...
  FreeBSD 镜像预计也可在 Google Compute Engine(TM) Marketplace 获取：[https://console.cloud.google.com/marketplace/browse?filter=category:os&filter=price:free&q=freebsd](https://console.cloud.google.com/marketplace/browse?filter=category:os&filter=price:free&q=freebsd)
...
本点版本 FreeBSD 15.1-RELEASE 将受支持至 2027 年 3 月 31 日。上一版本 FreeBSD 15.0-RELEASE 将于 2026 年 9 月 30 日到达生命周期终点。FreeBSD 15 发布系列将受支持至 2029 年 12 月 31 日。更多支持信息可在以下地址找到：
