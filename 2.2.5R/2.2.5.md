...
- 关于发布目录的布局信息，请参见文件 **ABOUT.TXT**。如果你是从软盘安装，请务必 *阅读这一部分！*
...
- **/usr/src/Makefile** 中的 `world` 目标经过改进，使其更加独立于主机系统，从而能通过源代码从非常旧的系统进行更轻松的引导。

- 对文档进行了许多修复。

- 通过 CERT 和其他计算机安全组织报告了许多安全增强。

- 安装程序得到了进一步的更新和修复，一些一年前的 bogon 终于在 2.2.5 中消除。

- BIND 和 sendmail 等重要子系统得到更新。

- 支持以太网媒体选择。替换了更为晦涩的标志 `link` 用法，`ifconfig` 中的新标志 `media` 允许通过名称选择多端口以太网卡上的特定接口（更多信息请参阅 `man ifconfig`）。

- 对 AHC（Adaptec 394x/294X）驱动程序和 AIC7xxx 汇编程序进行了显著改进。

- 对串行引导代码和 GDB 远程支持进行了增强。

- 为 CMD640 芯片组提供了变通方案（请参见 **/sys/i386/conf/LINT**）。

- 现在支持更新的 3Com 3c589D PCMCIA 卡。

- 新的 VGA 库（**/usr/src/lib/libvgl**）现已提供，用于在 syscons ttys 上进行简单的 VGA 图形显示（类似于 Linux 的 libSVGA）。

- 现在可以通过 printcap(5) 功能指定 lpd 和其他相关程序中的 TCP 连接超时，从而避免在大量网络打印服务器环境中出现长时间挂起的情况。

- 用户模式 ppp 更新，包含来自 3.0-current 的各种修复和增强。值得重新阅读手册页，因为以下一些变更可能会打乱当前的配置：

  - 命令 `set debug` 现在变为 `set log`。
  - LCP 日志拆分为 LCP、IPCP 和 CCP 日志，因此任何 `set log LCP` 行需要变更为 `set log LCP IPCP CCP` 以查看与之前相同的输出。
  - ppp 现在使用 syslogd 来写入日志文件。
  - ppp 现在默认禁用 LQR，并启用 openmode。
  - ppp 现在以 `network` 组安装，权限为 4550。你必须将 69 组（network）新增到 **/etc/group**。如果你希望允许用户运行 `ppp -direct ...`，必须让他们成为 `network` 组的成员来获得授权。客户端 ppp 现在要求用户 ID 为 0。

请参考 ppp(8) 手册页和手册中的相关部分，了解详细信息。

## 2. 支持的配置

FreeBSD 当前支持多种基于 ISA、VLB、EISA 和 PCI 总线的 PC，从 386sx 到 Pentium 级别的机器（尽管不推荐使用 386sx）。同时，也支持通用的 IDE 或 ESDI 硬盘配置、各种 SCSI 控制器、网络卡和串行卡。
...
**注意：** 你不能从 SoundBlaster 卡引导系统，因为它们没有板载 BIOS，而 BIOS 是必需的，用于将引导设备映射到系统 BIOS I/O 向量。不过，这些卡完全可以用于外部磁带、CD-ROM 等设备。对于任何没有引导 ROM 的基于 AIC-6x60 的卡也是如此。有些系统确实带有引导 ROM，通常会在系统首次开机或重置时显示相关信息，在这种情况下，你也可以从这些卡引导。请检查你的系统/主板文档以了解更多细节。
...
FreeBSD 2.2.5 将附带由用户贡献的驱动程序，用于 Future Domain 36C20 / Adaptec AHA2920 控制器。此驱动程序尚未完全支持，但基本功能已经实现。请查看 CD-ROM 上的 **/xperimnt** 部分。
...
请注意，当前**不**支持任何令牌环卡，因为我们仍在等待有人捐赠相关驱动程序。有人愿意吗？
...
- Stallion 多端口串行板：EasyIO、EasyConnection 8/32 和 8/64、ONboard 4/16 和 Brumby
...
- 软盘磁带驱动器（仅支持一些较旧型号，驱动程序较为陈旧）

FreeBSD 当前**不**支持 IBM 的微通道（MCA）总线。
...
你可以从 `ftp.FreeBSD.org` —— 官方的 FreeBSD 发行站点，FTP 下载 FreeBSD 及其任意或全部可选软件包。

有关其他 FreeBSD 软件镜像站点，请参见文件 `MIRROR.SITES`。请从网络距离最近的站点下载发行版。我们欢迎额外的镜像站点！如果你希望成为官方镜像站点，请联系 `freebsd-admin@FreeBSD.org` 了解更多详情。
...
每张 `-RELEASE` 光盘的价格为 39.95 美元，或 24.95 美元（附带 FreeBSD 订阅）。FreeBSD 3.0-SNAP 光盘的价格为 29.95 美元，或 14.95 美元（附带 FreeBSD-SNAP 订阅）。（`-RELEASE` 和 `-SNAP` 订阅是完全独立的）。有了订阅，你将自动接收每次发布的更新。每张光盘发货时会收取信用卡费用，你可以随时取消订阅，且不会产生进一步的义务。

在美国、加拿大和墨西哥，运输费用（按订单计，不按盘片计）为 5 美元，国际运输费用为 9 美元。他们接受 Visa、Mastercard、Discover、美国运通卡或美元支票，并且在美国境内货到付款。加利福尼亚州居民请加收 8.25% 的销售税。

如果你因任何原因不满意，光盘可无条件退货。

## 4. 报告问题、提出建议、提交代码

你的建议、错误报告和代码贡献一直受到重视——请随时报告你发现的任何问题（如果可能，请附带修复程序！）。

从具有互联网邮件连接的机器上提交错误报告的首选方法是使用命令 `send-pr`，或使用以下链接的 CGI 脚本：`http://www.FreeBSD.org/send-pr.html`。 我们忠实的 bugfiler 程序会认真归档错误报告，你可以放心，我们会尽最大努力尽快响应所有报告的错误。通过这种方式提交的错误报告也将在我们的官网的支持部分显示，因此，它们不仅作为错误报告有价值，还为其他用户提供了潜在问题的“指示标记”。

如果由于某种原因，你无法使用命令 `send-pr` 提交错误报告，你可以尝试将报告发送到：`freebsd-bugs@FreeBSD.org`

请注意，`send-pr` 本身是一个 shell 脚本，应该很容易将其移到完全不同的系统上。我们更倾向于你使用此接口，因为它更容易跟踪问题报告。然而，在提交之前，请尽量确认该问题是否已修复。

否则，对于任何问题或建议，请发送邮件至：`freebsd-questions@FreeBSD.org`

此外，由于这是一个志愿者项目，我们始终欢迎更多人手来帮忙——目前我们期望的增强功能远远超出了我们自己能够处理的范围！ 如果你有技术问题或愿意提供帮助，请通过邮件联系我们：`freebsd-hackers@FreeBSD.org`
...
FreeBSD 凝聚了全球数十甚至数百位个人的辛勤工作，他们为发布这个版本付出了巨大努力。有关 FreeBSD 项目工作人员的完整名单，请参阅：`http://www.FreeBSD.org/handbook/staff.html`

或者，如果你已经加载了文档分发包：`file:/usr/share/doc/handbook/staff.html`

额外的 FreeBSD 帮助者和 Beta 测试人员：
...
Larry Altneu 和 Wilko Bulte 提供了 Wangtek 和 Archive QIC-02 磁带驱动器用于测试和驱动程序开发。

蒙大拿州立大学的每一位成员给予了最初的支持。
