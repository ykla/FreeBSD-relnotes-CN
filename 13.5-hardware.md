# FreeBSD 13.5-RELEASE 硬件兼容列表

## 引言

本文档包含 FreeBSD 13.5 的硬件兼容性说明。其中列出了 FreeBSD 支持的硬件平台，以及各类硬件设备（存储控制器、网络接口等），并给出已知可正常工作的设备实例。
...
FreeBSD 支持 AMD64（"Hammer"）和 Intel(R) EM64T 架构。AMD64（又称 x86-64）是受完全支持的 Tier 1 架构。AMD64 支持所有现代 x86 处理器。本发行版整合了多项优化和增强功能，以充分发挥基于 FreeBSD/amd64 系统的全部能力，在各种工作负载下提供更佳性能。

### 32 位 x86（i386）架构支持

在 FreeBSD 持续演进并为现代架构优化的同时，我们仍致力于在本发行版中维护对老牌 i386（x86）架构的支持。尽管 i386 硬件日益少见，但对于仍依赖该架构的用户而言，无论是在遗留系统中还是在特定应用场景下，FreeBSD 仍是一个可靠且通用的选择。

### 64 位 ARM（aarch64）架构支持

FreeBSD 将 64 位 ARM（又称 arm64 或 AArch64）作为 Tier-1 架构予以支持。
...
Allwinner (64-bit)，更多支持信息请参阅 [Allwinner Wiki 页面](https://wiki.freebsd.org/arm/Allwinner)：
...
Raspberry Pi，更多支持信息请参阅 [Raspberry Wiki 页面](https://wiki.freebsd.org/arm/Raspberry%20Pi)：

* Raspberry Pi 3
* Raspberry Pi 4

RockChip (64-bit)，更多支持信息请参阅 [RockChip Wiki 页面](https://wiki.freebsd.org/arm/RockChip)：
...
Allwinner (32-bit)，更多支持信息请参阅 [Allwinner Wiki 页面](https://wiki.freebsd.org/arm/Allwinner)：
...
NXP i.MX6 (32-bit)，更多支持信息请参阅 [NXP Wiki 页面](https://wiki.freebsd.org/arm/imx6)：
...
更多支持信息请参阅 [RISC-V Wiki 页面](https://wiki.freebsd.org/riscv)。

### PowerPC 架构支持

许多 PowerPC 平台受到支持，包括但不限于：

* Apple PowerMac G4 及更新型号。
* IBM POWER4 及更新型号（使用 pseries VM 平台）。
* IBM POWER8 及更新型号（裸机 powernv）
** 大部分开发工作使用 Raptor TALOS-II 完成
...
在可能的情况下，列出适用于每个设备或每类设备的驱动。如果相关驱动在 FreeBSD 基础发行版中拥有手册页（多数驱动都有），则在此处给出引用。关于用法、配置以及所支持的设备、控制器等的具体型号等信息，可在手册页中查阅。
...
[ahc(4)](https://man.freebsd.org/cgi/man.cgi?query=ahc&sektion=4&format=html) 驱动支持以下 `SCSI` host 适配器 芯片 和 `SCSI` 控制器 卡：
...
上述部分芯片可配置为 AHCI 模式。在此情况下，它们改由 ahci(4) 驱动支持。

未知 ATA 芯片组在 PIO 模式下受支持；如果标准 busmaster DMA 寄存器存在且包含有效设置，则也会启用 DMA，但最高模式限于 UDMA33，因为无法确定芯片组的能力及编程方式。
...
[hptiop(4)](https://man.freebsd.org/cgi/man.cgi?query=hptiop&sektion=4&format=html) 驱动还支持以下 SAS 和 SATA RAID 控制器（已停产）：
...
[isci(4)](https://man.freebsd.org/cgi/man.cgi?query=isci&sektion=4&format=html) 驱动提供对 Intel C600 `SAS` 控制器。
...
支持所有主要的固件版本（2.x、3.x、4.x 和 5.x），但建议升级到该控制器可用的最新固件。

未列出的兼容 Mylex 控制器应能正常工作，但尚未经过验证。
...
这些系统还包含集成 RAID 镜像（Integrated RAID Mirroring）和增强型集成 RAID 镜像（Integrated RAID Mirroring Enhanced），该驱动也为它们提供支持。

`SAS` 控制器芯片也存在于许多基于 AMD/Opteron 的新系统中，如 Sun 4100。注意，该控制器可以同时驱动 SAS 和 SATA 驱动器，或两者的混合。这些控制器可用的集成 RAID 镜像支持非常有限。

`光纤通道` 控制器芯片组支持多种速度和系统。 `Apple` 光纤通道 HBA 实际上是 `FC949ES` 卡。

该驱动还支持光纤通道卡的目标模式（target mode）。可通过 LSI Logic 固件实用程序设置核心的所需角色来启用此支持，该实用程序用于确定卡可承担的角色——无需单独编译。
...
[pst(4)](https://man.freebsd.org/cgi/man.cgi?query=pst&sektion=4&format=html) 驱动支持 the Promise Supertrak SX6000 ATA hardware RAID 控制器。
...
[smartpqi(4)](https://man.freebsd.org/cgi/man.cgi?query=smartpqi&sektion=4&format=html) 驱动支持以下控制器：

*	HPE Gen10 Smart Array Controller Family
*	OEM Controllers based on the Microsemi Chipset

[sym(4)](https://man.freebsd.org/cgi/man.cgi?query=sym&sektion=4&format=html) 驱动提供对以下 Symbios/LSI Logic PCI SCSI 控制器的支持：
...
[sym(4)](https://man.freebsd.org/cgi/man.cgi?query=sym&sektion=4&format=html) 驱动支持的 SCSI 控制器可以内置于主板，也可以是以下扩展卡之一：
...
对于所有受支持的 SCSI 控制器，全面支持 SCSI-I、SCSI-II 和 SCSI-III 外设，包括硬盘、光盘、磁带机（包括 DAT、8mm Exabyte、Mammoth 和 DLT）、介质更换器、处理器目标设备和 CD-ROM 驱动器。WORM devices that support CD-ROM commands are supported for read-only access by the CD-ROM drivers (such as [cd(4)](https://man.freebsd.org/cgi/man.cgi?query=cd&sektion=4&format=html)). WORM/CD-R/CD-RW writing support is provided by [cdrecord(1)](https://man.freebsd.org/cgi/man.cgi?query=cdrecord&sektion=1&format=html), which is a part of the package:sysutils/cdrtools[] port in the Ports Collection.
...
[ae(4)](https://man.freebsd.org/cgi/man.cgi?query=ae&sektion=4&format=html) 驱动支持 Attansic/Atheros L2 PCIe 快速以太网 控制器，并已知支持以下硬件：

*	ASUS EeePC 701
*	ASUS EeePC 900

其他硬件可能可以也可能无法与此驱动配合工作。

[age(4)](https://man.freebsd.org/cgi/man.cgi?query=age&sektion=4&format=html) 驱动提供对基于 Attansic/Atheros L1 千兆以太网 控制器 芯片，包括：
...
当前版本的 `atse` 驱动支持 the 以太网 MegaCore as described in version 11.1 of Altera's documentation when the device is configured with internal FIFOs.
...
[axe(4)](https://man.freebsd.org/cgi/man.cgi?query=axe&sektion=4&format=html) 驱动支持 ASIX Electronics AX88172/AX88178/AX88772/AX88772A/AX88772B/AX88760 based USB 以太网 适配器，包括：
...
[axge(4)](https://man.freebsd.org/cgi/man.cgi?query=axge&sektion=4&format=html) 驱动支持 ASIX Electronics AX88178A/AX88179 USB 千兆以太网适配器。

[bce(4)](https://man.freebsd.org/cgi/man.cgi?query=bce&sektion=4&format=html) 驱动提供对基于 QLogic NetXtreme II 系列千兆以太网控制器的支持，包括：
...
[bge(4)](https://man.freebsd.org/cgi/man.cgi?query=bge&sektion=4&format=html) 驱动提供对基于 Broadcom BCM570x 系列千兆以太网控制器芯片的支持，包括：
...
[bnxt(4)](https://man.freebsd.org/cgi/man.cgi?query=bnxt&sektion=4&format=html) 驱动提供对基于 Broadcom NetXtreme-C 和 NetXtreme-E 系列千兆以太网控制器芯片的支持，包括：
...
[bxe(4)](https://man.freebsd.org/cgi/man.cgi?query=bxe&sektion=4&format=html) 驱动提供对基于 QLogic NetXtreme II 系列 10Gb 以太网控制器芯片的支持，包括：
...
[cas(4)](https://man.freebsd.org/cgi/man.cgi?query=cas&sektion=4&format=html) 驱动目前已知支持以下扩展卡：
...
[cue(4)](https://man.freebsd.org/cgi/man.cgi?query=cue&sektion=4&format=html) 驱动支持 CATC USB-EL1210A based USB 以太网 适配器，包括：
...
`cxgb` 驱动支持基于 T3 and T3B chipset 的 10 千兆 and 1 千兆以太网 适配器：

*	Chelsio 10GBase-CX4
*	Chelsio 10GBase-LR
*	Chelsio 10GBase-SR

[cxgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbe&sektion=4&format=html) 驱动支持基于 T6 ASIC 的 100Gb and 25Gb 以太网 适配器：
...
[cxgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbe&sektion=4&format=html) 驱动支持基于 T5 ASIC 的 40Gb, 10Gb and 1Gb 以太网 适配器：
...
[cxgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbe&sektion=4&format=html) 驱动支持基于 T4 ASIC 的 10Gb and 1Gb 以太网 适配器：
...
`cxgbev` 驱动支持基于 T6 ASIC 的 Virtual Functions on 100Gb and 25Gb 以太网 适配器：
...
`cxgbev` 驱动支持基于 T5 ASIC 的 Virtual Functions on 40Gb, 10Gb and 1Gb 以太网 适配器：
...
`cxgbev` 驱动支持基于 T4 ASIC 的 Virtual Functions on 10Gb and 1Gb 以太网 适配器：
...
[dc(4)](https://man.freebsd.org/cgi/man.cgi?query=dc&sektion=4&format=html) 驱动提供对以下芯片组的支持：
...
[dc(4)](https://man.freebsd.org/cgi/man.cgi?query=dc&sektion=4&format=html) 驱动目前已知支持以下网卡：
...
内置在以下 Freescale 片上系统设备中的千兆以太网控制器已知可与 **dtsec** 驱动配合工作：

*	P2041, P3041
*	P5010, P5020

此外，以下设备预计可正常工作，但尚未经过测试：

*	P4080, P4040
*	P5040

[em(4)](https://man.freebsd.org/cgi/man.cgi?query=em&sektion=4&format=html) 驱动支持基于 Intel 82540, 82541ER, 82541PI, 82542, 82543, 82544, 82545, 82546, 82546EB, 82546GB, 82547, 82571, 82572, 82573, 82574, 82575, 82576, and 82580 controller chips 的千兆以太网 适配器：
...
[gem(4)](https://man.freebsd.org/cgi/man.cgi?query=gem&sektion=4&format=html) 驱动目前已知支持以下扩展卡：

*	Sun Gigabit Ethernet PCI 2.0/3.0 (GBE/P)
	(part no. 501-4373)

`genet` 驱动支持 the 以太网 控制器 portion of the Broadcom BCM2711 on the Raspberry Pi 4 Model B and related systems. It utilizes the BCM54213PE PHY。

[gve(4)](https://man.freebsd.org/cgi/man.cgi?query=gve&sektion=4&format=html) 驱动支持 Google Cloud Compute Engine (GCE) 虚拟网卡 (gVNIC)。
...
[ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 驱动支持 Intel 以太网 700 系列。该系列中大多数带有 SFP+/SFP28/QSFP+ 接口的适配器，其固件要求使用 Intel 认证模块；这些认证模块列在下面。该认证检查无法由驱动禁用。

**ixl** 驱动支持带有以下 QSFP+ 模块的 40Gb 以太网适配器：
...
`kue` 驱动支持 Kawasaki LSI KL5KLUSB101B based USB 以太网 适配器，包括：
...
[mlx4en(4)](https://man.freebsd.org/cgi/man.cgi?query=mlx4en&sektion=4&format=html) 驱动支持以下 network 适配器：

*	Mellanox ConnectX-2 (ETH)
*	Mellanox ConnectX-3 (ETH)

`mlx4ib` 驱动支持以下 network 适配器：

*	Mellanox ConnectX-2 (IB)
*	Mellanox ConnectX-3 (IB)

[mlx5en(4)](https://man.freebsd.org/cgi/man.cgi?query=mlx5en&sektion=4&format=html) 驱动支持 200Gb, 100Gb, 50Gb, 40Gb, 25Gb and 10Gb ethernet 适配器。
...
`mlx5ib` 驱动支持 100Gb, 50Gb, 40Gb, 25Gb and 10Gb network 适配器。ConnectX-4 supports: 10/20/25/40/50/56/100Gb/s speeds. ConnectX-4 LX supports: 10/25/40/50Gb/s speeds (and reduced power consumption):
...
[msk(4)](https://man.freebsd.org/cgi/man.cgi?query=msk&sektion=4&format=html) 驱动提供对基于 Marvell/SysKonnect Yukon II 的千兆以太网控制器芯片的支持，包括：
...
`mxge` 驱动支持基于 Myricom LANai Z8E chips 的 10 千兆以太网 适配器：

*	Myricom 10GBase-CX4 (10G-PCIE-8A-C, 10G-PCIE-8AL-C)
*	Myricom 10GBase-R (10G-PCIE-8A-R, 10G-PCIE-8AL-R)
*	Myricom 10G XAUI over ribbon fiber (10G-PCIE-8A-Q, 10G-PCIE-8AL-Q)

[my(4)](https://man.freebsd.org/cgi/man.cgi?query=my&sektion=4&format=html) 驱动提供对基于 Myson 芯片组的支持。支持的型号包括：

*	Myson MTD800 PCI Fast Ethernet chip
*	Myson MTD803 PCI Fast Ethernet chip
*	Myson MTD89X PCI Gigabit Ethernet chip

[nfe(4)](https://man.freebsd.org/cgi/man.cgi?query=nfe&sektion=4&format=html) 驱动支持以下 NVIDIA MCP onboard 适配器：
...
[nge(4)](https://man.freebsd.org/cgi/man.cgi?query=nge&sektion=4&format=html) 驱动支持 National Semiconductor DP83820 and DP83821 based 千兆以太网 适配器，包括：
...
[oce(4)](https://man.freebsd.org/cgi/man.cgi?query=oce&sektion=4&format=html) 驱动支持以下 network 适配器：
...
`rtwn_usb` 驱动支持 Realtek RTL8188CUS/RTL8188RU/RTL8188EU/RTL8192CU/RTL8192EU/RTL8812AU/RTL8821AU based USB wireless network 适配器，包括：
...
`rue` 驱动支持 RealTek RTL8150 based USB 以太网 适配器，包括：
...
[sfxge(4)](https://man.freebsd.org/cgi/man.cgi?query=sfxge&sektion=4&format=html) 驱动支持 all 10Gb 以太网 适配器 基于 Solarflare SFC9000 family 控制器。
...
[sis(4)](https://man.freebsd.org/cgi/man.cgi?query=sis&sektion=4&format=html) 驱动支持基于 National Semiconductor DP83815 (MacPhyter) and DP83816 chips. Supported adapters include 的 Silicon Integrated Systems SiS 900 and SiS 7016 based 快速以太网 适配器 and embedded 控制器，as well as 快速以太网 适配器：
...
[ste(4)](https://man.freebsd.org/cgi/man.cgi?query=ste&sektion=4&format=html) 驱动支持 Sundance Technologies ST201 based 快速以太网 适配器 and embedded 控制器，包括：

*	D-Link DFE-530TXS
*	D-Link DFE-550TX
*	D-Link DFE-580TX

[stge(4)](https://man.freebsd.org/cgi/man.cgi?query=stge&sektion=4&format=html) 驱动提供对基于 Sundance/Tamarack TC9021 的千兆以太网控制器芯片的支持，包括：
...
[ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动支持基于 Alteon Tigon I 和 II 芯片的千兆以太网适配器。**ti** 驱动已在以下适配器上测试：
...
集成在以下 Freescale 系统级芯片设备中的千兆以太网控制器已知可与 **tsec** 驱动配合工作：
...
受支持的 3G 卡提供 ppp 或 mpd 连接所需的调制解调器端口，以及用于其他功能（附加命令端口、诊断端口、SIM 工具包端口）的额外端口（取决于具体设备）。

在某些此类设备中，存在由 umass(4) 驱动支持的大容量存储设备，其中包含 Windows 和 Mac OS X 驱动程序。设备以磁盘模式启动（TruInstall、ZeroCD 等），需要额外的命令才能切换到调制解调器模式。如果你的设备未自动切换，请尝试添加 quirks。参见 usbconfig(8) 和 usb_quirk(4)。
...
**uhso** 驱动应能与 Option 的大多数设备配合工作。以下设备已经过验证
...
该设备包含一个称为 "Zero-CD" 的大容量存储设备，其中含有 Microsoft Windows 的驱动程序；这是设备的默认模式。 **uhso** 驱动会自动将设备从 "Zero-CD" 模式切换到调制解调器模式。可以通过 sysctl(8) 将 `hw.usb.uhso.auto_switch` 设置为 0 来禁用此行为。

[urndis(4)](https://man.freebsd.org/cgi/man.cgi?query=urndis&sektion=4&format=html) 驱动支持许多 Android 设备的网络共享功能。
...
[bwi(4)](https://man.freebsd.org/cgi/man.cgi?query=bwi&sektion=4&format=html) 驱动支持 Broadcom BCM43xx based wireless devices，包括：
...
**otus** 驱动提供对 Atheros AR9170 USB IEEE 802.11b/g/n 无线网络适配器的支持，包括：
...
[ral(4)](https://man.freebsd.org/cgi/man.cgi?query=ral&sektion=4&format=html) 驱动支持 PCI/PCIe/CardBus wireless 适配器 基于 Ralink Technology chipsets，包括：
...
**rsu** 驱动提供对 Realtek RTL8188SU/RTL8192SU USB IEEE 802.11b/g/n 无线网络适配器的支持，包括：
...
`rum` 驱动支持基于 Ralink RT2501USB and RT2601USB chipsets, including 的 USB 2.0 wireless 适配器：
...
[run(4)](https://man.freebsd.org/cgi/man.cgi?query=run&sektion=4&format=html) 驱动支持以下 wireless 适配器：
...
[ural(4)](https://man.freebsd.org/cgi/man.cgi?query=ural&sektion=4&format=html) 驱动支持基于 Ralink Technology RT2500USB chipset, including 的 USB 2.0 wireless 适配器：
...
`ce` 驱动支持以下 models of Tau-PCI/32 WAN 适配器：
...
`cp` 驱动支持以下 models of Tau-PCI WAN 适配器：
...
*	Creative Sound Blaster Audigy 2 and Creative Sound Blaster Audigy 4 (CA0102
	Chipset).
	PCM 支持仅限于 48kHz/16 位立体声（该芯片组的 192kHz/24 位部分不受支持）。
*	Creative Sound Blaster Audigy 2 Value (CA0108 Chipset).
	PCM 支持仅限于
	48kHz/16 位立体声（该芯片组的 192kHz/24 位部分不受支持）。
	该卡不支持 MIDI。
...
`snd_fm801` 驱动支持基于 following chipset 的 audio devices：
...
`snd_hda` 驱动支持基于 following chipset 的控制器 having PCI class 4 (multimedia) and subclass 3 (HDA), compatible with Intel HDA specification.

`snd_hda` 驱动支持 more than two hundred different 控制器 and CODECs. There is no sense to list all of them here, as in most cases specific CODEC configuration and wiring are more important then type of the CODEC itself。
...
Altera University Program Secure Data Card IP Core ([altera_sdcard(4)](https://man.freebsd.org/cgi/man.cgi?query=altera_sdcard&sektion=4&format=html) driver)

[atp(4)](https://man.freebsd.org/cgi/man.cgi?query=atp&sektion=4&format=html) 驱动提供对以下产品 ID 的支持：
...
[ohci(4)](https://man.freebsd.org/cgi/man.cgi?query=ohci&sektion=4&format=html) 驱动支持所有 OHCI v1.0 兼容控制器，包括：
...
[uhci(4)](https://man.freebsd.org/cgi/man.cgi?query=uhci&sektion=4&format=html) 驱动支持 all UHCI v1.1 compliant 控制器，包括：
...
`rtsx` 驱动支持不同规范兼容的芯片。以下芯片已经过验证可正常工作：
...
`ulpt` 驱动为 USB 打印机和并行打印机转换电缆提供支持，包括以下：
...
[ubser(4)](https://man.freebsd.org/cgi/man.cgi?query=ubser&sektion=4&format=html) 驱动提供对 the BWCT console management serial adapters.
...
[uplcom(4)](https://man.freebsd.org/cgi/man.cgi?query=uplcom&sektion=4&format=html) 驱动支持以下设备 和 适配器：
...
[amd64, i386] Serial Bus Protocol 2 (SBP-2) storage devices ([sbp(4)](https://man.freebsd.org/cgi/man.cgi?query=sbp&sektion=4&format=html) driver)

### 蓝牙设备

`ng_ubt` 驱动支持 all Bluetooth USB devices that conform with the Bluetooth specification v1.1，包括：
...
[ccr(4)](https://man.freebsd.org/cgi/man.cgi?query=ccr&sektion=4&format=html) 驱动支持基于 T6 ASIC 的 the crypto accelerator engine included on 适配器：
...
**hifn** 驱动支持包含 Hifn 7751、7951、7811、7955 和 7956 芯片组的各种卡，例如：
...
> **注意**：
>
> [moused(8)](https://man.freebsd.org/cgi/man.cgi?query=moused&sektion=8&format=html) has more information on using pointing devices with FreeBSD.
...
`gpioths` 驱动提供对以下设备的支持：

	DHT11       DHT12
	DHT21       DHT22
	AM3201      AM3202

所支持的设备彼此都很相似，主要区别在于精度和分辨率。这些设备需要单根线缆进行数据通信，使用与 Maxim 的 1-wire(tm) 不兼容的自定义协议。AM320x 设备还支持连接到 i2c 总线，但此驱动仅支持单线连接选项。

**ig4** 支持基于 Synopsys DesignWare IP 的 I2C 控制器，这些控制器可见于第四代起的 Intel(R) Core(TM) 处理器、Intel(R) Bay Trail、Apollo Lake SoC 系列以及某些 AMD 系统。
...
**netfpga10g_nf10bmac** 驱动的当前版本与 NetFPGA-10G Embedded CPU Ethernet Core 版本 1.00a 的一个 PIO 模式接口配合工作。

以下 PLX/Avago/Broadcom 芯片受 **ntb_hw_plx** 驱动支持：
...
`sdhci` 驱动支持不同规格的兼容芯片。以下芯片已经过验证可正常工作：
...
[superio(4)](https://man.freebsd.org/cgi/man.cgi?query=superio&sektion=4&format=html) 驱动支持由 Nuvoton（前身为 Winbond）和 ITE 生产的多种 Super I/O 控制器。
