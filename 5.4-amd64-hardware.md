# FreeBSD 5.4-RELEASE amd64 硬件兼容列表

- 原文链接：[FreeBSD/amd64 5.4-RELEASE Hardware Notes](https://www.freebsd.org/releases/5.4R/hardware-amd64/)
...
本文档包含 FreeBSD 5.4-RELEASE 在 AMD64 硬件平台（也称为 FreeBSD/amd64 5.4-RELEASE）上的硬件兼容性说明。它列出了已知在此平台上工作的设备，以及一些在尝试为新设备配置支持时可能有用的引导时内核自定义说明。
...
真正的 BusLogic MultiMaster 克隆的 AMI FastDisk 主机适配器也由 `bt(4)` 驱动程序支持。
...
支持所有主要固件版本（2.x、3.x、4.x 和 5.x），但建议始终升级到控制器可用的最新固件。未列出的兼容 Mylex 控制器应该可以工作，但尚未经过验证。
...
`ncr(4)` 驱动程序为以下 NCR/Symbios SCSI 控制器芯片提供支持：
...
`sym(4)` 驱动程序支持的 SCSI 控制器可以内置于主板，或在以下附加板之一上：
...
`mpt(4)` 驱动程序支持的 SCSI 控制器芯片可在许多系统的板载上找到，包括：
...
对于 Tekram DC-310/U 和 DC-390F/U/UW/U2B/U2W/U3W PCI SCSI 主机适配器，请使用 `sym(4)` 驱动程序。
...
`ti(4)` 驱动程序支持基于 Alteon Tigon I 和 II 芯片的千兆以太网适配器。`ti(4)` 驱动程序已与以下适配器进行测试：
...
`tl(4)` 驱动程序支持基于 Texas Instruments ThunderLAN 的以太网和快速以太网适配器，包括大量的 Compaq PCI 以太网适配器。还支持：
...
3C656 系列 CardBus 卡和 3C556 系列 MiniPCI 卡都内置了专有调制解调器。`xl(4)` 驱动程序或其他任何驱动程序都不支持此调制解调器。
...
`bge(4)` 驱动程序为基于 Broadcom BCM570x 系列千兆以太网控制器芯片的各种 NIC 提供支持，包括：
...
Specialix SI/XIO/SX 多端口串行卡，包括较旧的 SIHOST2.x 和"enhanced"（基于 transputer，又称 JET）主机卡（支持 ISA、EISA 和 PCI）（`si(4)` 驱动程序）

### 3.10 音频设备

`snd_ad1816(4)` 驱动程序支持以下声卡：

- Analog Devices AD1816

`snd_cs4281(4)` 驱动程序支持以下声卡：

- Crystal Semiconductor CS4281

`snd_solo(4)` 驱动程序支持以下声卡：
...
`snd_cmi(4)` 驱动程序支持以下声卡：

- CMedia CMI8338
- CMedia CMI8738

`snd_csa(4)` 驱动程序支持以下声卡：

- Crystal Semiconductor CS461x/462x Audio Accelerator
- Crystal Semiconductor CS4280 Audio Controller

某些板载 CS4610 芯片伴随 CS423x ISA 编解码器而非 CS4297 AC97 编解码器。`snd_csa(4)` 驱动程序目前尚不支持此类配置。

`snd_es137x(4)` 驱动程序支持以下声卡：
...
`snd_fm801(4)` 驱动程序支持基于以下芯片组的音频设备：

- Forte Media FM801

`snd_gusc(4)` 驱动程序支持以下声卡：
...
`snd_vibes(4)` 驱动程序支持基于以下芯片组的音频设备：

- S3 SonicVibes

`snd_emu10k1(4)` 驱动程序支持以下声卡：
...
`snd_via82c686(4)` 驱动程序支持基于以下芯片组的音频设备：

- VIA 82C686A

`snd_ds1(4)` 驱动程序支持以下声卡：

- Yamaha DS-1
- Yamaha DS-1E

`snd_ich(4)` 驱动程序支持以下音频设备：
...
> **注意**
>
> USB 以太网适配器可在列出 [以太网接口](https://www.freebsd.org/releases/5.4R/hardware-amd64.html#ETHERNET) 的章节中找到。

> **注意**
>
> USB 蓝牙适配器可在 [蓝牙](https://www.freebsd.org/releases/5.4R/hardware-amd64.html#BLUETOOTH) 章节中找到。
...
`ulpt(4)` 驱动程序为 USB 打印机和并口打印机转换电缆提供支持，包括：
...
`ubser(4)` 驱动程序为 BWCT 控制台管理串行适配器提供支持。
...
`ng_bt3c(4)` 驱动程序为 3Com/HP 3CRWB6096-A PCCARD 蓝牙适配器提供支持。
...
`safe(4)` 驱动程序支持包含以下任意芯片的卡：

- SafeNet 1141
- SafeNet 1741

`ubsec(4)` 驱动程序支持包含以下任意芯片的卡：
...
> **注意**
>
> 有关特定视频卡及其与 **Xorg** 兼容性的信息，可在 <http://www.x.org/> 找到。
...
> **注意**
>
> `moused(8)` 提供了有关在 FreeBSD 中使用定位设备的更多信息。有关在 **Xorg** 中使用定位设备的信息，可在 <http://www.x.org/> 找到。
