...
由于 `setlocale()` 安全漏洞迫使我们沿着 2.1-STABLE 分支发布另一个版本，我们不仅修复了大量 bug，还进行了安全增强，并花了一些时间升级 `sysinstall` 以处理 MSDOSFS 安装 bug——这些 bug 给许多用户造成了麻烦——同时还升级了一些选定的工具。除此之外，2.1.7R 没有太多功能性变化。
...
- 支持 Buslogic 的真正 PCI 驱动程序（新驱动程序和探测顺序）。
...
FreeBSD 是一款免费且含完整源码、基于 4.4 BSD Lite 的发行版，适用于基于 Intel i386/i486/Pentium（或兼容）架构的 PC。它主要基于 U.C. Berkeley 的 CSRG 小组的软件，结合了一些来自 NetBSD、386BSD 和自由软件基金会的增强功能。

自从一年前发布 FreeBSD 2.0 以来，FreeBSD 的性能、功能和稳定性有了显著提升。最大变化是重构的虚拟内存（VM）系统，其中合并了 VM 和文件缓冲区缓存，这不仅提高了性能，还减少了 FreeBSD 的内存占用，使得 5MB 的配置成为更加可接受的最小配置。其他增强功能包括完全的 NIS 客户端和服务器支持、事务 TCP 支持、按需拨号 PPP、改进的 SCSI 子系统、早期的 ISDN 支持、对 FDDI 和快速以太网（100Mbit）适配器的支持、对 Adaptec 2940（WIDE 和窄型）和 3940 SCSI 适配器的改进支持，以及数百个 bug 修复。

我们认真听取了许多用户的意见和建议，并尝试提供我们希望更合理且易于理解的安装过程。对这个（不断发展的）过程的反馈尤为欢迎！

除了基础发行版，FreeBSD 还提供了新的移植软件集，其中包含超过 390 款常用程序。这个 Ports 列表涉及从 HTTP（WWW）服务器到游戏、编程语言、文本编辑器及几乎所有其他类别的软件。整个 Ports 只需要 10MB 的存储空间，所有 Port 都以“增量”的形式表示，即相对于原始源码的修改。这样，我们可以更容易地更新 Port，并大大减少 Ports 所占用的磁盘空间。要编译 Port，你只需要进入你想安装的程序目录，输入 `make`，然后让系统完成其余的工作。你构建的每个 Port 的完整原始发行版会动态地从 CDROM 或本地 FTP 站点获取，因此你只需足够的磁盘空间来构建你需要的 Port。（几乎）每个 Port 也都提供了一个预编译的“软件包”，可以通过简单的命令（`pkg_add`）进行安装。有关软件包的特别便捷的接口，请参见配置菜单中的新“软件包”选项。
...
FreeBSD 的核心不包含任何会妨碍其出口到美国以外地区的 DES 代码。核心发行版提供附加软件包，仅供美国国内使用，包含通常使用 DES 的程序。单独提供的辅助软件包任何人都可以使用。对于非美国用户，FreeBSD 还提供了一个可自由出口的 DES 版本，下载地址为 `ftp://ftp.internat.FreeBSD.org/pub/FreeBSD`。
...
由于这是一个志愿者项目，我们始终欢迎更多人加入到我们的开发中来！
