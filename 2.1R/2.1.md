...
FreeBSD 是一款免费提供完整源代码的 4.4 BSD Lite 版本，适用于基于 Intel i386/i486/Pentium（或兼容）的 PC。它主要基于加利福尼亚大学伯克利分校计算机系统研究小组（CSRG）提供的软件，并结合了 NetBSD、386BSD 和自由软件基金会的一些增强功能。
...
除了基础发行版，FreeBSD 还提供了新的软件 Ports，包含了 350 多个广受欢迎的程序。Ports 清单涉及从 http（WWW）服务器到游戏、语言、编辑器等几乎所有领域。整个 Ports 只需要 10MB 的存储空间，所有 Port 都以“增量”的形式表达，这使得我们能够更加轻松地更新 Ports，并大大减少 Ports 对磁盘空间的需求。要编译某个 Port，你只需进入想要安装的程序目录，输入 `make` 命令，系统会自动完成剩下的工作。你构建的每个 Port 的完整原始分发包都会从 CDROM 或本地 FTP 站点动态获取，因此你只需足够的磁盘空间来构建你需要的 Ports。（几乎）每个 Port 还提供了预编译的“软件包”，可以通过简单的命令（`pkg_add`）进行安装。请参见配置菜单中的新“软件包”选项，这是访问软件包的一个特别便捷的界面。
...
FreeBSD 的核心不包含任何会妨碍其出口到美国以外地区的 DES 代码。核心发行版中有附加软件包，仅在美国使用，包含通常使用 DES 的程序。提供的附加软件包可以供任何人使用。对于我们非美国用户，也有一个可以自由（从美国以外）出口的 DES 发行版，位于 [ftp://ftp.internat.FreeBSD.org/pub/FreeBSD](ftp://ftp.internat.FreeBSD.org/pub/FreeBSD)。
...
**注意：** 你不能从 SoundBlaster 卡启动，因为它们没有板载 BIOS，BIOS 是必需的，用于将启动设备映射到系统 BIOS 的 I/O 向量中。然而，它们完全可以用于外部磁带机、CDROM 等。对于没有启动 ROM 的任何其他 AIC-6x60 基板卡，情况相同。有些系统确实有启动 ROM，通常会在系统第一次启动或重置时显示某种信息，在这种情况下，你也可以从这些卡启动。详细信息请查阅你的系统/主板文档。
