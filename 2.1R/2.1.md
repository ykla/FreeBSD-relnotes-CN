...
FreeBSD 是一款免费提供完整源代码的 4.4 BSD Lite 版本，适用于基于 Intel i386/i486/Pentium（或兼容）的 PC。它主要基于加州大学伯克利分校计算机系统研究小组（CSRG）提供的软件，并结合了 NetBSD、386BSD 和自由软件基金会的一些增强功能。

自我们发布 FreeBSD 2.0 以来已有一年多时间，FreeBSD 的性能、功能集和稳定性都有了显著提升。最大的变化是重新设计的虚拟内存（VM）系统，结合了虚拟内存和文件缓冲区缓存，这不仅提高了性能，还减少了 FreeBSD 的内存占用，使得 5MB 配置成为更为合理的最小配置。其他增强功能包括完全支持 NIS 客户端和服务器，事务 TCP 支持，按需拨号 PPP，改进的 SCSI 子系统，早期 ISDN 支持，支持 FDDI 和快速以太网（100Mbit）适配器，改进对 Adaptec 2940（宽和窄）和 3940 SCSI 适配器的支持，以及数百个错误修复。

我们还认真考虑了许多用户的意见和建议，努力提供一个更为合理和易于理解的安装过程。对于这一不断发展的过程，我们特别欢迎你的反馈！

除了基本系统，FreeBSD 还提供了新的软件 Ports，包含了 350 多个广受欢迎的程序。Ports 清单涉及从 http（WWW）服务器到游戏、语言、编辑器等几乎所有领域。整个 Ports 只需要 10MB 的存储空间，所有 Port 都以“增量”的形式表达，这使得我们能够更加轻松地更新 Ports，并大大减少 Ports 对磁盘空间的需求。要编译某个 Port，你只需进入想要安装的程序目录，输入命令 `make`，系统会自动完成剩下的工作。你构建的每个 Port 的完整原始分发包都会从 CDROM 或本地 FTP 站点动态获取，因此你只需要足够的磁盘空间来构建你需要的 Ports。（几乎）每个 Port 还提供了预编译的“软件包”，可以通过简单的命令（`pkg_add`）进行安装。请参见配置菜单中的新“软件包”选项，这是访问软件包的一个特别便捷的界面。
...
FreeBSD 的核心不包含任何会妨碍其出口到美国以外地区的 DES 代码。核心发行版有一个附加软件包，仅供美国境内使用，其中包含通常使用 DES 的程序。单独提供的辅助软件包可以供任何人使用。对于我们非美国用户，也有一个可以自由（从美国以外）出口的 DES 发行版，位于 [ftp://ftp.internat.FreeBSD.org/pub/FreeBSD](ftp://ftp.internat.FreeBSD.org/pub/FreeBSD)。

如果你只需要 FreeBSD 的密码安全功能，并且不需要从不同主机（如 Suns、DEC 机器等）复制加密的密码到 FreeBSD 密码条目中，那么 FreeBSD 的基于 MD5 的安全性可能就足够了！我们认为，默认的安全模型足以与 DES 相抗衡，而且没有任何复杂的出口问题需要处理。如果你在美国以外（甚至在美国境内），不妨试一试！
...
**注意：** 你不能从 SoundBlaster 卡启动，因为它们没有板载 BIOS，而板载 BIOS 是将启动设备映射到系统 BIOS I/O 向量所必需的。然而，它们完全可以用于外部磁带机、CDROM 等。对于没有启动 ROM 的任何其他 AIC-6x60 基板卡，情况相同。有些系统确实有启动 ROM，通常会在系统第一次启动或重置时显示某种信息，在这种情况下，你也可以从这些卡启动。详细信息请查阅你的系统/主板文档。
...
你的建议、错误报告和代码贡献始终受到重视——如果你发现任何问题，请随时报告（如果可能，请附上修复程序！）。

从具有互联网邮件连接的机器提交错误报告的首选方法是使用命令 `send-pr`。错误报告将由我们忠实的 bugfiler 程序妥善归档，你可以确信我们会尽最大努力尽快回应所有报告的错误。通过这种方式提交的错误报告也会在我们的网页上显示在支持部分，因此，它们不仅作为错误报告有价值，而且还作为“警示标志”，提醒其他用户注意潜在的问题。

如果由于某些原因，你无法使用命令 `send-pr` 提交错误报告，你可以尝试将其发送到：`bugs@FreeBSD.org`

如果是其他问题或建议，请发送邮件至：`questions@FreeBSD.org`

此外，作为志愿者项目，我们始终欢迎愿意提供帮助的人——期望的增强功能远远超过我们独自能够应对的范围！如果你有技术性问题或愿意提供帮助，请发送邮件至：`hackers@FreeBSD.org`
...
FreeBSD 凝聚了来自全球数十甚至数百位个人的辛勤努力，他们为使这次发布成为现实付出了极大的努力。列举所有为 FreeBSD 贡献的人是非常困难的，甚至几乎是不可能的，但我们仍然会尝试（按字母顺序排列，当然）。如果你为我们贡献良多，但你的名字没有出现在此，请放心，遗漏完全是偶然的。如果你希望更新以下列表，请联系 <hackers@FreeBSD.org>：
