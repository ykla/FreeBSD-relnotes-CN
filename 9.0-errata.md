...
- 当接口未定义对应的 `ifconfig_<em0>_ipv6` 变量时，9.0-RELEASE 无法配置 [rc.conf(5)](https://man.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5&format=html) 变量 `ipv6_prefix_<em0>` 中指定的接口。此问题将在后续版本中修复。要在 9.0-RELEASE 上绕过此问题，请为 `ipv6_prefix_<em0>` 中指定的每个接口添加 `ifconfig_<em0>_ipv6` 行，如下所示：
