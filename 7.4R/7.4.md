...
典型的发行说明条目记录了自 7.3-RELEASE 之后发布的安全公告、新的驱动程序或硬件支持、新的命令或选项、重要的错误修复或第三方软件升级。它们也可能列出重大 Ports/包或发布工程实践的更改。显然，发行说明无法列出发布之间对 FreeBSD 所做的每一个更改；本文件主要聚焦于安全公告、用户可见的变化和重大架构改进。
...
[alq(9)](http://www.freebsd.org/cgi/man.cgi?query=alq&sektion=9&manpath=FreeBSD+7.4-RELEASE) 支持已得到改善。`alq_writen()` 和 `alq_getn()` 的 KPI 已扩展以支持可变长度的消息，这在 ALQ 创建时根据传递给 `alq_open()` 的参数启用。同时，新增 `ALQ_NOACTIVATE` 和 `ALQ_ORDERED` 参数，以便 ALQ 消费者可以分别更好地控制 I/O 调度和资源获取。这些扩展完全向后兼容。
