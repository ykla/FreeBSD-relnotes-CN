# FreeBSD 7.4-RELEASE 发行说明（2011 年 2 月 24 日）
...
FreeBSD 7.4-RELEASE 的发行说明包含了 FreeBSD 7.4-STABLE 开发线在基本系统上所做更改的摘要。本文件列出了自上次发布以来发布的适用安全公告，以及 FreeBSD 内核和用户空间的重大变化。此外，还简要介绍了升级的相关事项。

## 1 引言

本文档包含 FreeBSD 7.4-RELEASE 的发行说明。它介绍了 FreeBSD 最近新增、修改或删除的功能，并提供了一些关于从早期版本升级 FreeBSD 的说明。

此发行版是 FreeBSD 7.4-RELEASE 的正式发行版。可以在 [ftp://ftp.FreeBSD.org/](ftp://ftp.freebsd.org/) 或其任何镜像站点找到更多信息。更多信息可以参阅 [《获取 FreeBSD》附录](https://www.freebsd.org/doc/en_US.ISO8859-1/books/handbook/mirrors.html) 中的内容，该附录位于 [FreeBSD 手册](https://www.freebsd.org/doc/en_US.ISO8859-1/books/handbook/) 中。

建议所有用户在安装 FreeBSD 之前查阅发行勘误。勘误文档会更新包含发布周期末期或发布后发现的“最新信息”。通常，它包含已知的漏洞、安全公告以及文档的修正。可以在 FreeBSD 网站上找到 FreeBSD 7.4-RELEASE 的最新勘误副本。

## 2 新特性

本节介绍了自 7.3-RELEASE 以来，FreeBSD 中最显著的用户可见新功能或已更改功能。

典型的发行说明条目记录了自 7.3-RELEASE 之后发布的安全公告、新的驱动程序或硬件支持、新的命令或选项、重要的错误修复或第三方软件升级。它们也可能列出重大 Ports/包或发布工程实践的更改。显然，发行说明无法列出发布之间对 FreeBSD 所做的每一项更改；本文件主要聚焦于安全公告、用户可见的变化和重大架构改进。

### 2.1 安全公告

以下安全公告中描述的问题已修复完成。更多信息，请参考来自 [http://security.FreeBSD.org/](http://security.freebsd.org/) 的个别公告。
...
[alq(9)](http://www.freebsd.org/cgi/man.cgi?query=alq&sektion=9&manpath=FreeBSD+7.4-RELEASE) 支持已得到改善。`alq_writen()` 和 `alq_getn()` 的 KPI 已扩展以支持可变长度的消息，这在 ALQ 创建时根据传递给 `alq_open()` 的参数启用。同时，新增参数 `ALQ_NOACTIVATE` 和 `ALQ_ORDERED`，以便 ALQ 消费者可以分别更好地控制 I/O 调度和资源获取。这些扩展完全向后兼容。

[alq(9)](http://www.freebsd.org/cgi/man.cgi?query=alq&sektion=9&manpath=FreeBSD+7.4-RELEASE) 支持现在作为内核模块 `alq.ko` 提供。

FreeBSD 的 [memguard(9)](http://www.freebsd.org/cgi/man.cgi?query=memguard&sektion=9&manpath=FreeBSD+7.4-RELEASE) 框架已得到改进，现在能够在更长的时间内检测已分配内存的使用后释放问题。更多信息请参阅 [memguard(9)](http://www.freebsd.org/cgi/man.cgi?query=memguard&sektion=9&manpath=FreeBSD+7.4-RELEASE) 手册页面。

以下 [sysctl(8)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&manpath=FreeBSD+7.4-RELEASE) 变量为新增：`vm.kmem_map_size` 表示当前 kmem 映射的大小，`vm.kmem_map_free` 表示 kmem 映射中最大的连续空闲区域。[r213554](http://svn.freebsd.org/viewvc/base?view=revision&revision=213554), [r213556](http://svn.freebsd.org/viewvc/base?view=revision&revision=213556), [r213560](http://svn.freebsd.org/viewvc/base?view=revision&revision=213560)
...
[amd64, i386] 新增 qpi(4) 伪总线驱动程序。该驱动支持 Intel QPI 芯片组上的额外 PCI 总线，其中连接了每个插槽的内存控制器等硬件。

##### 2.2.1.1 多媒体支持

[acpi_video(4)](http://www.freebsd.org/cgi/man.cgi?query=acpi_video&sektion=4&manpath=FreeBSD+7.4-RELEASE) 驱动已更新。现已实现 LCD 亮度控制通知处理程序。
...
修复了 [bge(4)](http://www.freebsd.org/cgi/man.cgi?query=bge&sektion=4&manpath=FreeBSD+7.4-RELEASE) 驱动中长期存在的 ASF 心跳发送错误。
...
修复了 [bge(4)](http://www.freebsd.org/cgi/man.cgi?query=bge&sektion=4&manpath=FreeBSD+7.4-RELEASE) 驱动中导致 BCM57780 中 TSO 无法正常工作的一个错误。

修复了 [bge(4)](http://www.freebsd.org/cgi/man.cgi?query=bge&sektion=4&manpath=FreeBSD+7.4-RELEASE) 驱动中可能导致系统内存超过 4 GB 时性能不佳的一个错误。原因是除 BCM5755 及更高版本的 Broadcom 控制器外，所有 Broadcom 控制器在 4 GB 边界 DMA 处理上存在一个错误，并且使用了低效的缓冲区。
...
新增了为 Silicon Integrated Systems SiS190/191 Fast/Gigabit Ethernet 提供的 [sge(4)](http://www.freebsd.org/cgi/man.cgi?query=sge&sektion=4&manpath=FreeBSD+7.4-RELEASE) 驱动程序。它支持 TSO 和 VLAN 上的 TSO。
...
新增了加载器 tunable `net.link.ifqmaxlen`，它指定了发送接口队列长度的默认值。该参数的默认值为 `50`。
...
新增了 [netgraph(4)](http://www.freebsd.org/cgi/man.cgi?query=netgraph&sektion=4&manpath=FreeBSD+7.4-RELEASE) 节点 [ng_patch(4)](http://www.freebsd.org/cgi/man.cgi?query=ng_patch&sektion=4&manpath=FreeBSD+7.4-RELEASE)，它执行通过的数据包修改。修改仅限于对 8 位、16 位、32 位或 64 位无符号整数的 C 语言操作子集。
...
FreeBSD 的 TCP 重新组装实现得到了改进。修复了影响 SMP 系统的长期存在的会计错误，`net.inet.tcp.reass.maxqlen` [sysctl(8)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&manpath=FreeBSD+7.4-RELEASE) 变量已被弃用，取而代之的是基于接收套接字缓冲区大小的每连接动态限制。FreeBSD 接收端现在比以前更好地处理丢包（特别是由队列溢出引起的丢包），从而提高了连接吞吐量。
...
[chgrp(1)](http://www.freebsd.org/cgi/man.cgi?query=chgrp&sektion=1&manpath=FreeBSD+7.4-RELEASE) 和 [chown(8)](http://www.freebsd.org/cgi/man.cgi?query=chown&sektion=8&manpath=FreeBSD+7.4-RELEASE) 现在支持参数 `-x`，以防止递归操作遍历多个挂载点。

[cp(1)](http://www.freebsd.org/cgi/man.cgi?query=cp&sektion=1&manpath=FreeBSD+7.4-RELEASE) 现在支持参数 `-x`，以防止递归操作遍历多个挂载点。
...
[indent(1)](http://www.freebsd.org/cgi/man.cgi?query=indent&sektion=1&manpath=FreeBSD+7.4-RELEASE) 工具现在支持参数 `-ta`，将所有带有 `_t` 后缀的标识符视为类型。
...
[newsyslog(8)](http://www.freebsd.org/cgi/man.cgi?query=newsyslog&sektion=8&manpath=FreeBSD+7.4-RELEASE) 工具现在不再将 PID 文件不存在视为错误。新增了参数 `-P` 以恢复到旧行为。
...
[sysctl(8)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&manpath=FreeBSD+7.4-RELEASE) 工具现在支持参数 `-i`，以在检索个别 OID 时忽略失败。这允许将相同的 OID 列表传递给不同系统的 [sysctl(8)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&manpath=FreeBSD+7.4-RELEASE)，即使某些 OID 在某些系统上不存在，仍然可以尽可能多地获取信息。

#### 2.3.1 **/etc/rc.d** 脚本

现在，**rc.conf(5)** 支持一个变量 `firewall_coscripts`。这个变量应包含在防火墙启动或停止后需要执行的命令列表。
