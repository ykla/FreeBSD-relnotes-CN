# FreeBSD 13.1-RELEASE 发行公告

日期：2022 年 5 月 16 日

FreeBSD 发布工程团队很高兴地宣布 FreeBSD 13.1-RELEASE 现已发布。这是 stable/13 分支的第二个版本。

部分亮点如下：

* OpenSSH 升级至 8.8p1 版本。
* OpenSSL 升级至 1.1.1o 版本。
* 在 ssh 中启用了 FIDO/U2F 硬件认证器，使用新的公钥类型 `ecdsa-sk` 和 `ed25519-sk`，以及相应的证书类型。
* [ice(4)](https://man.freebsd.org/cgi/man.cgi?query=ice&sektion=4&format=html) 驱动升级至 1.34.2-k，添加了固件日志记录和初步的 DCB 支持。
* 添加了 [iwlwifi(4)](https://man.freebsd.org/cgi/man.cgi?query=iwlwifi&sektion=4&format=html) 驱动和 LinuxKPI 802.11 兼容层，以补充 [iwm(4)](https://man.freebsd.org/cgi/man.cgi?query=iwm&sektion=4&format=html) 对较新 Intel 无线芯片组的支持。
* ZFS 升级至 OpenZFS 2.1.4 版本。
* EC2 镜像现在默认使用 UEFI 引导，而非传统 BIOS。
* 以及更多……
...
FreeBSD 项目将 FreeBSD 13.1-RELEASE 题献给 386BSD 的联合创建者 Bill Jolitz，386BSD 构成了 FreeBSD 1.0 的基础。我们站在巨人的肩膀上。

## 获取途径

FreeBSD 13.1-RELEASE 现已支持 amd64、i386、powerpc、powerpc64、powerpc64le、powerpcspe、armv6、armv7、aarch64 和 riscv64 架构。

FreeBSD 13.1-RELEASE 可从可启动 ISO 镜像或通过网络安装。部分架构还支持从 USB 闪存盘安装。所需文件可按以下说明下载。
...
FreeBSD 13.1-RELEASE 也可从多家供应商处购买 CD-ROM 或 DVD 介质。提供 FreeBSD 13.1 相关产品的供应商之一是：

* FreeBSD Mall, Inc. <https://www.freebsdmall.com>

预安装的虚拟机镜像也可用于 amd64（x86_64）、i386（x86_32）、AArch64（arm64）和 RISCV 架构，提供 `QCOW2`、`VHD` 和 `VMDK` 磁盘镜像格式，以及原始（未格式化）镜像。

FreeBSD 13.1-RELEASE amd64 还可在以下云托管平台上使用：
...
这些 AMI ID 可从各区域的 Systems Manager Parameter Store 中使用以下键检索：
...
FreeBSD 13.1-RELEASE 预计还将在 Google Compute Engine(TM) Marketplace 中提供，待其完成第三方特定验证后，访问：<https://console.cloud.google.com/launcher/browse?filter=category:os&filter=price:free>
...
FreeBSD 13.1-RELEASE 可从以下站点通过 https 下载：

* <https://download.freebsd.org/ftp/releases/ISO-IMAGES/13.1/>

FreeBSD 13.1-RELEASE 虚拟机镜像可从以下地址下载：

* <https://download.freebsd.org/ftp/releases/VM-IMAGES/13.1-RELEASE/>

FreeBSD 13.1-RELEASE amd64 BASIC-CI 镜像可从以下地址下载：

* <https://download.freebsd.org/ftp/releases/CI-IMAGES/13.1-RELEASE/>

关于安装 FreeBSD 或将现有机器更新至 13.1-RELEASE 的说明，请参阅：

* <https://www.FreeBSD.org/releases/13.1R/installation/>

## 支持

根据新的 FreeBSD 支持模型，FreeBSD 13 发行系列将至少支持至 2026 年 1 月 31 日。本点发行版 FreeBSD 13.1-RELEASE 将至少支持至 FreeBSD 13.2-RELEASE 发布后三个月。其他支持信息可在以下地址找到：

* <https://www.FreeBSD.org/security/>

请注意，13.0 将支持至 13.1 发行日期起三个月后，截至本文撰写时该日期尚未确定。

## 致谢

许多公司捐赠了设备、网络访问或人力时间来支持 FreeBSD 13.1 的发布工程活动，包括：
...
13.1-RELEASE 的发布工程团队包括：

| 成员 | 角色 |
| :--- | :--- |
| Glen Barber <gjb@FreeBSD.org> | 发布工程负责人，13.1-RELEASE 发布工程师 |
