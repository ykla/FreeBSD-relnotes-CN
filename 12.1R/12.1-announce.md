# FreeBSD 12.1-RELEASE 发行公告

日期：2019 年 11 月 4 日

FreeBSD 发布工程团队很高兴地宣布 FreeBSD 12.1-RELEASE 现已发布。这是 stable/12 分支的第二个版本。

部分亮点如下：

* BearSSL 已导入到基本系统中。
* clang、llvm、lld、lldb、compiler-rt 工具和 libc++ 已更新至版本 8.0.1。
* OpenSSL 已更新至版本 1.1.1d。
* 若干用户空间工具更新。
* 以及更多……

完整的新功能和已知问题列表，请参阅在线发行说明和勘误列表，地址如下：

* <https://www.FreeBSD.org/releases/12.1R/relnotes/>
...
FreeBSD 项目将 FreeBSD 12.1-RELEASE 献给 Kurt Lidl，以资纪念。

## 获取途径

FreeBSD 12.1-RELEASE 现已支持 amd64、i386、powerpc、powerpc64、powerpcspe、sparc64、armv6、armv7 和 aarch64 架构。

FreeBSD 12.1-RELEASE 可从可启动 ISO 镜像或通过网络安装。部分架构还支持从 USB 闪存盘安装。所需文件可按以下说明下载。

本消息底部包含发行版 ISO、闪存盘和 SD 卡镜像的 SHA512 和 SHA256 校验和。

发行版镜像的 PGP 签名校验和也可从以下地址获取：

* <https://www.FreeBSD.org/releases/12.1R/signatures/>
...
FreeBSD 12.1-RELEASE 也可从多家供应商处购买 CD-ROM 或 DVD 介质。提供 FreeBSD 12.1 相关产品的供应商之一是：

* FreeBSD Mall, Inc. <https://www.freebsdmall.com>

预安装的虚拟机镜像也可用于 amd64（x86_64）、i386（x86_32）和 AArch64（arm64）架构，提供 `QCOW2`、`VHD` 和 `VMDK` 磁盘镜像格式，以及原始（未格式化）镜像。

FreeBSD 12.1-RELEASE amd64 还可在以下云托管平台上使用：
...
FreeBSD 12.1-RELEASE 在完成第三方特定验证后，预计还将在 Google Compute Engine(TM) Marketplace 中提供，地址如下：
...
FreeBSD 12.1-RELEASE 可从以下站点通过 https 下载：

* <https://download.freebsd.org/ftp/releases/ISO-IMAGES/12.1/>

FreeBSD 12.1-RELEASE 虚拟机镜像可从以下地址下载：

* <https://download.freebsd.org/ftp/releases/VM-IMAGES/12.1-RELEASE/>

关于安装 FreeBSD 或将现有机器更新至 12.1-RELEASE 的说明，请参阅：

* <https://www.FreeBSD.org/releases/12.1R/installation/>

## 支持

基于对 FreeBSD 支持模型审查的讨论，FreeBSD 12 发行系列将至少支持至 2020 年 11 月 30 日。更多信息，请参阅关于即将进行的支持模型讨论的 [官方公告](https://lists.freebsd.org/pipermail/freebsd-announce/2018-November/001854.html)。

* <https://www.FreeBSD.org/security/>

## 致谢

许多公司捐赠了设备、网络访问或人力时间来支持 FreeBSD 12.1 的发布工程活动，包括：
...
12.1-RELEASE 的发布工程团队包括：

| 成员 | 角色 |
| :--- | :--- |
| Glen Barber <gjb@FreeBSD.org> | 发布工程负责人，12.1-RELEASE 发布工程师 |
...
喜欢 FreeBSD？请通过向 FreeBSD 基金会[捐赠](https://www.freebsdfoundation.org/donate/) 来支持此版本及未来版本！
