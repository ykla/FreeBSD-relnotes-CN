...
在 386BSD 0.1 的分发版本中，修复了数百个漏洞，并将许多过时的软件升级到 GAMMA 版本中的最新发行版。1.1 版本解决了 BETA 和 GAMMA 用户反馈的许多首次运行问题。

此外，许多软件包已经移植并可作为附加组件使用，包括：XFree86 2.1、xview 3.2、elm、nntp、mh、InterViews 及其他数十种实用工具。
...
FreeBSD 的核心不包含 DES（数据加密标准）代码，这确保了它可以出口到美国以外的地区。核心发行版中提供了一款仅供美国境内使用的附加包，其中包含通常使用 DES 的程序。分开提供的辅助软件包任何人都可以使用。
...
**注意**: 请参阅 **/usr/src/KNOWNBUGS** 中的 `bt742a.c` 条目，了解可能存在的固件故障以及未公开的开关设置详情，这些信息可能对控制器的正常运行至关重要。
...
**注意**: FreeBSD 当前 **不支持** IBM 的微通道（MCA）总线，但相关支持似乎即将完成。有进展时会发布详细信息。
...
请注意：**Walnut Creek CDROM 不提供 FreeBSD 的技术支持**。有关技术支持，请联系 FreeBSD 团队。详见第 4 节。
...
由于这是一个志愿者项目，我们非常欢迎更多的帮助！目前需要改进的地方远远超出了我们团队的能力范围。
