# FreeBSD 10.0-RELEASE 硬件兼容列表

## 引言

本文档包含 FreeBSD 10.0-RELEASE 的硬件兼容性说明。其中列出了 FreeBSD 支持的硬件平台，以及各类硬件设备（存储控制器、网络接口等），并给出已知可正常工作的设备实例。
...
如果你有不在此列表中的系统，它可能尚未经过 FreeBSD 10.0-RELEASE 测试。我们鼓励你尝试并致信 [FreeBSD SPARC porting mailing list](http://lists.FreeBSD.org/mailman/listinfo/freebsd-sparc64) 反馈结果，包括哪些设备可用、哪些不可用。
...
[i386, amd64] [bxe(4)](https://man.freebsd.org/cgi/man.cgi?query=bxe&sektion=4&format=html) 驱动支持基于 Broadcom NetXtreme II 系列 10Gb Ethernet 控制器芯片的多种 NIC，包括以下：
...
使用较旧 Broadcom 芯片组（BCM4301、BCM4303 和 BCM4306 rev 2）的用户必须使用 bwi(4)，因为固件 v4 版本不支持这些芯片。较新的固件太大，无法装入这些旧芯片。
...
[pc98] Creative Technologies SoundBlaster(98) ([b(4)](https://man.freebsd.org/cgi/man.cgi?query=b&sektion=4&format=html) driver)
