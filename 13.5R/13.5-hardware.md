...
NXP i.MX6 (32-bit)，更多支持信息请参阅 [NXP i.MX6 Wiki 页面](https://wiki.freebsd.org/arm/imx6)：
...
** 在 AmigaOne X5000 和 AmigaOne A1222 上测试完成

## 支持的设备

本节介绍当前已知受 FreeBSD 支持的设备。其他配置也可能可用，但尚未经过测试。欢迎对本列表提出反馈、更新和修正。

在可能的情况下，列出适用于每个设备或每类设备的驱动。如果相关驱动在 FreeBSD 基本系统中拥有手册页（多数驱动都有），则在此处给出引用。关于所支持的设备、控制器等的具体型号等信息，可在手册页中查阅。
...
注意，此硬件仅对 ATA DMA 命令支持命令队列和基于 FIS 的切换。ATAPI 和非 DMA ATA 命令在每个端口上逐条执行。
...
[ae(4)](https://man.freebsd.org/cgi/man.cgi?query=ae&sektion=4&format=html) 驱动支持 Attansic/Atheros L2 PCIe 快速以太网控制器，并已知支持以下硬件：
...
[age(4)](https://man.freebsd.org/cgi/man.cgi?query=age&sektion=4&format=html) 驱动为基于 Attansic/Atheros L1 千兆以太网控制器芯片的 LOM（主板集成网络模块）提供支持，包括：
...
当前版本的 `atse` 驱动在设备配置为内部 FIFO 时，支持 Altera 文档 11.1 版本中所述的以太网 MegaCore。
...
`cxgb` 驱动支持基于 T3 和 T3B 芯片组的 10Gb 和 1Gb 以太网适配器：
...
`cxgbev` 驱动支持基于 T6 ASIC 的 100Gb 和 25Gb 以太网适配器上的虚拟功能（Virtual Function）：
...
`cxgbev` 驱动支持基于 T5 ASIC 的 40Gb、10Gb 和 1Gb 以太网适配器上的虚拟功能（Virtual Function）：
...
`cxgbev` 驱动支持基于 T4 ASIC 的 10Gb 和 1Gb 以太网适配器上的虚拟功能（Virtual Function）：
...
**genet** 驱动支持树莓派 4 Model B 及相关系统中 Broadcom BCM2711 上的以太网控制器部分。它使用 BCM54213PE PHY。
...
[liquidio(4)](https://man.freebsd.org/cgi/man.cgi?query=liquidio&sektion=4&format=html) 驱动支持以下网卡：
...
[msk(4)](https://man.freebsd.org/cgi/man.cgi?query=msk&sektion=4&format=html) 驱动提供对基于 Marvell/SysKonnect Yukon II 千兆以太网控制器芯片的各种网卡的支持，包括：
...
[my(4)](https://man.freebsd.org/cgi/man.cgi?query=my&sektion=4&format=html) 驱动提供对基于 Myson 芯片组的各种网卡的支持。支持的型号包括：
...
**rtwn_usb** 驱动支持基于 Realtek RTL8188CUS/RTL8188RU/RTL8188EU/RTL8192CU/RTL8192EU/RTL8812AU/RTL8821AU 的 USB 无线网络适配器，包括：
...
[stge(4)](https://man.freebsd.org/cgi/man.cgi?query=stge&sektion=4&format=html) 驱动提供对基于 Sundance/Tamarack TC9021 千兆以太网控制器芯片的各种网卡的支持，包括：
...
（关于上述各供应商受支持卡的完整列表，请参见 **/sys/dev/usb/serial/u3g.c**。）
...
**uhso** 驱动应能与 Option 的大多数设备配合工作。以下设备已通过验证
...
3C656 系列 CardBus 卡和 3C556 系列 MiniPCI 卡均内置专有调制解调器。**xl** 驱动和其他 FreeBSD 驱动均不支持此调制解调器。
...
[ural(4)](https://man.freebsd.org/cgi/man.cgi?query=ural&sektion=4&format=html) 驱动支持基于 Ralink Technology RT2500USB 芯片组的 USB 2.0 无线适配器：
...
**ce** 驱动支持以下型号的 Tau-PCI/32 WAN 适配器：

Cronyx Tau-PCI/32

> 两个 fractional/unframed E1 接口，
> 它们之间共享 32 个 HDLC 通道，适配器总吞吐量为
> 2048 kbps。

Cronyx Tau-PCI/32-Lite

> 单个 fractional/unframed E1 接口，
> 带 32 个 HDLC 通道。

**cp** 驱动支持以下型号的 Tau-PCI WAN 适配器：
...
> fractional E1 和 unframed E1 接口
...
**snd_hda** 驱动支持具有 PCI 类 4（多媒体）和子类 3（HDA）、与 Intel HDA 规范兼容的控制器。

**snd_hda** 驱动支持超过两百种不同的控制器和 CODEC。在此全部列出没有意义，因为在大多数情况下，特定的 CODEC 配置和接线比 CODEC 本身的类型更重要。
...
> **注意**：
>
> [amd64, i386] USB 以太网适配器可在列出[以太网接口](https://www.freebsd.org/releases/13.5R/hardware.html#ethernet)的章节中找到。

> **注意**：
>
> [amd64, i386] USB 蓝牙适配器可在[蓝牙](https://www.freebsd.org/releases/13.5R/hardware.html#bluetooth)章节中找到。
...
[uhci(4)](https://man.freebsd.org/cgi/man.cgi?query=uhci&sektion=4&format=html) 驱动支持所有 UHCI v1.1 兼容控制器，包括：
...
`ng_ubt` 驱动支持所有符合蓝牙规范 v1.1 的蓝牙 USB 设备，包括：
...
[ccr(4)](https://man.freebsd.org/cgi/man.cgi?query=ccr&sektion=4&format=html) 驱动支持基于 T6 ASIC 的适配器上的加密加速器引擎：
...
> **注意**：
>
> 有关特定显卡及与 Xorg 兼容性的信息，请参见 [http://www.x.org/](http://www.x.org/)。
...
> **注意**：
>
> [moused(8)](https://man.freebsd.org/cgi/man.cgi?query=moused&sektion=8&format=html) 提供了在 FreeBSD 中使用定点设备的更多信息。
> 有关在 Xorg 中使用定点设备的信息，请参见 [http://www.x.org/](http://www.x.org/)。
