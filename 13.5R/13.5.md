...
FreeBSD 13.5-RELEASE 的发行说明概述了在 13-STABLE 开发分支上的 FreeBSD 基本系统所做的变更。本文件列出了自上次发布以来发布的适用安全公告，以及对 FreeBSD 内核和用户空间的重要更改。此外，本文件还提供了一些关于升级的简要说明。
...
典型的发行说明条目包括 13.4-RELEASE 之后发布的安全公告、新增的驱动程序或硬件支持、新命令或参数、主要的 bug 修复或贡献软件的升级。此外，这些条目还可能列出对主要 Ports/软件包或发行工程实践的更改。显然，发行说明无法包含 FreeBSD 版本之间的每一项改动，因此本文件主要关注安全公告、用户可见的更改以及主要的架构改进。
...
更新弃用警告，说明 [gvinum(8)](https://man.freebsd.org/cgi/man.cgi?query=gvinum&sektion=8&format=html) 在 15.0 版本中已被移除。[8126ed28bda6](https://cgit.freebsd.org/src/commit/?id=8126ed28bda6)。
...
使 vnet [sysctl(9)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=9&format=html) 变量成为加载器可调参数。[d2a999c2e0a0](https://cgit.freebsd.org/src/commit/?id=d2a999c2e0a0)。完成 [3da1cf1e88f8](https://cgit.freebsd.org/src/commit/?id=3da1cf1e88f8) 的第二阶段。参数 `CTLFLAG_TUN` 的含义已扩展，使其自动检查是否存在一个内核环境变量，用于在早期启动期间初始化 sysctl。谨以此纪念 Hans Petter Selasky。

为 [sysctl(8)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&format=html) 添加了用于过滤 Jail 和 vnet 变量的参数。[09cbd68e4e47](https://cgit.freebsd.org/src/commit/?id=09cbd68e4e47)。这样用户就不必查阅源代码来确认变量是否属于 Jail / vnet 变量。
...
FreeBSD stable/14 及更早版本将保留现有的 32 位内核和系统支持。Ports 将保留对 stable/14 及更早分支上构建 Ports 和软件包的现有支持，只要这些分支仍然受到 Ports 的支持。然而，所有 32 位平台都是二级架构或三级架构，预计随着上游弃用 32 位平台，单个 Port 的支持将逐渐下降。
