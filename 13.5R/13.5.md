...
FreeBSD 13.5-RELEASE 的发行说明概述了在 13-STABLE 开发分支上的 FreeBSD 基本系统所做的变更。本文件列出了自上次发布以来公布的适用安全公告，以及对 FreeBSD 内核和用户空间的重要更改。此外，本文件还提供了一些关于升级的简要说明。

## 引言

本文件是 FreeBSD 13.5-RELEASE 的发行说明，介绍了 FreeBSD 最近新增、修改或删除的功能，并提供了从旧版升级的相关说明。

本发行说明所适用的 RELEASE 发行版，代表着 13-STABLE 开发分支自创建以来的最新进展。关于此分支上预构建的二进制 RELEASE 发行版的信息，请访问 [https://www.FreeBSD.org/releases/](https://www.freebsd.org/releases/)。

本发行说明所适用的 RELEASE 发行版，代表着 13-STABLE 开发分支自 13.4-RELEASE 以来的某个时间点。预计 13.5-RELEASE 将是 13-STABLE 分支的最终版本。有关该分支上预构建的二进制 RELEASE 发行版的信息，请访问 [https://www.FreeBSD.org/releases/](https://www.freebsd.org/releases/)。

FreeBSD 13.5-RELEASE 的此发行版是一个 RELEASE 发行版，可以在 [https://www.FreeBSD.org/releases/](https://www.freebsd.org/releases/) 及其任意镜像站找到。关于获取此版本（及其他 FreeBSD RELEASE 发行版）的更多信息，请参阅 [FreeBSD 手册](https://docs.freebsd.org/en/books/handbook/) 中的 [获取 FreeBSD 附录](https://docs.freebsd.org/en/books/handbook/mirrors)。

所有用户在安装 FreeBSD 之前都应查看发行勘误。勘误文档包含在发布周期后期或发布后发现的“最新动态”信息，通常涉及已知 bug、安全公告和文档更正。可以在 FreeBSD 网站上找到 FreeBSD 13.5-RELEASE 的最新勘误。

本文件介绍了自 13.4-RELEASE 以来在 FreeBSD 中新增或修改的对用户最具影响的功能。通常，文中所述的更改仅适用于 13-STABLE 分支，除非特别标注为 MERGED（已合并）功能。

典型的发行说明条目包括 13.4-RELEASE 之后发布的安全公告、新增的驱动程序或硬件支持、新命令或选项、主要的 bug 修复或第三方软件的升级。此外，这些条目还可能列出对主要 Ports/软件包或发行工程实践的更改。显然，发行说明无法包含 FreeBSD 版本之间的每一项改动，因此本文件主要关注安全公告、用户可见的更改以及主要的架构改进。

## 从旧版 FreeBSD 升级

可使用 [freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 工具完成 RELEASE 版本之间的二进制升级（以及各安全分支的快照）。有关特定于本版本的升级步骤，请参阅 [FreeBSD 13.5-RELEASE 升级信息](https://www.freebsd.org/releases/13.5R/installation/#upgrade-binary)，详细信息可见 FreeBSD 手册的 [二进制升级流程](https://docs.freebsd.org/en/books/handbook/cutting-edge/#freebsdupdate-upgrade)。该工具将更新未修改的用户空间实用程序，以及官方 FreeBSD 发行版中提供的未修改 **GENERIC** 内核。使用 [freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 升级的主机必须具有互联网连接。

基于源代码的更新（即通过重新编译 FreeBSD 基本系统升级）同样受到支持，相关说明请参考 **/usr/src/UPDATING**。
...
| 公告 | 日期 | 主题 |
| :--- | :--- | :--- |
| [FreeBSD-SA-24:15.bhyve](https://www.freebsd.org/security/advisories/FreeBSD-SA-24:15.bhyve.asc) | 2024 年 9 月 19 日 | [bhyve(8)](https://man.freebsd.org/cgi/man.cgi?query=bhyve&sektion=8&format=html) XHCI 仿真导致的越界读取访问 |
...
| [FreeBSD-SA-25:03.etcupdate](https://www.freebsd.org/security/advisories/FreeBSD-SA-25:03.etcupdate.asc) | 2025 年 1 月 29 日 | 无特权访问系统文件 |
...
| 勘误 | 日期 | 主题 |
| :--- | :--- | :--- |
| [FreeBSD-EN-24:16.pf](https://www.freebsd.org/security/advisories/FreeBSD-EN-24:16.pf.asc) | 2024 年 9 月 19 日 | pf 中错误的 ICMPv6 状态处理 |
...
`libpcap` 已连续更新至 1.10.3、1.10.4 和 1.10.5。[7aedea868535](https://cgit.freebsd.org/src/commit/?id=7aedea868535)、[e6efc827e47a](https://cgit.freebsd.org/src/commit/?id=e6efc827e47a) 和 [68ddf72800f8](https://cgit.freebsd.org/src/commit/?id=68ddf72800f8)。（由 FreeBSD 基金会赞助）

`tcpdump` 已连续更新至 4.99.4 和 4.99.5。[e5258a079df3](https://cgit.freebsd.org/src/commit/?id=e5258a079df3) 和 [094f44ea0358](https://cgit.freebsd.org/src/commit/?id=094f44ea0358)。（由 FreeBSD 基金会赞助）
...
`libusb` 的部分功能已合并自 1.0.16 版本。[02ef8e4061ab](https://cgit.freebsd.org/src/commit/?id=02ef8e4061ab)。（由 FreeBSD 基金会赞助）

`tzcode` 已更新至 2024b。[036ce2460cbc](https://cgit.freebsd.org/src/commit/?id=036ce2460cbc)。（由 Klara, Inc. 赞助）
...
OpenSSH 已连续更新至 9.8p1、9.9p1 和 9.9p2。[b74bb7f01193](https://cgit.freebsd.org/src/commit/?id=b74bb7f01193)、[cb8e164fbb15](https://cgit.freebsd.org/src/commit/?id=cb8e164fbb15) 和 [31dcdee20afc](https://cgit.freebsd.org/src/commit/?id=31dcdee20afc)。（由 FreeBSD 基金会赞助）

### 已弃用应用程序

更新弃用警告，说明 [gvinum(8)](https://man.freebsd.org/cgi/man.cgi?query=gvinum&sektion=8&format=html) 在 15.0 版本中将被移除。[8126ed28bda6](https://cgit.freebsd.org/src/commit/?id=8126ed28bda6)。

[shar(1)](https://man.freebsd.org/cgi/man.cgi?query=shar&sektion=1&format=html) 已被弃用，并已添加弃用通知。[0d946859c994](https://cgit.freebsd.org/src/commit/?id=0d946859c994)。**shar(1)** 程序虽然简单，但 sh 存档的基本思想充满风险，最好不要像 `$PATH` 中的程序和 man 页面那样被显著推广。可以通过 [tar(1)](https://man.freebsd.org/cgi/man.cgi?query=tar&sektion=1&format=html) 实现相同的功能。
...
将对 Realtek 8156/8156B 的支持从 [cdce(4)](https://man.freebsd.org/cgi/man.cgi?query=cdce&sektion=4&format=html) 转移到 [ure(4)](https://man.freebsd.org/cgi/man.cgi?query=ure&sektion=4&format=html)。[1b0af7617e6c](https://cgit.freebsd.org/src/commit/?id=1b0af7617e6c)。（由 FreeBSD 基金会赞助）

新增了对 Brainboxes USB 转串口适配器的支持。[c3a377dbbb87](https://cgit.freebsd.org/src/commit/?id=c3a377dbbb87)。

### 已弃用和已移除的驱动程序

已计划在 FreeBSD 15.0 中移除 [agp(4)](https://man.freebsd.org/cgi/man.cgi?query=agp&sektion=4&format=html)，且 man 页面现已声明其为弃用。[8375d2b9c653](https://cgit.freebsd.org/src/commit/?id=8375d2b9c653)。
...
修复了 cd9660 重复目录名称的问题。[79778b7aafc8](https://cgit.freebsd.org/src/commit/?id=79778b7aafc8)。此问题最初是在 FreeBSD 14.2-RELEASE 中引入的，导致在安装映像中创建带有重复短名称（第 2 级名称）的 cd9660 映像。（由 FreeBSD 基金会赞助）
...
将 `PF_DEFAULT_TO_DROP` 转换为 vnet 加载器可调参数 `net.pf.default_to_drop`。[cb162f659578](https://cgit.freebsd.org/src/commit/?id=cb162f659578)。[7f7ef494f11d](https://cgit.freebsd.org/src/commit/?id=7f7ef494f11d) 引入了编译时选项 `PF_DEFAULT_TO_DROP`，使 [pf(4)](https://man.freebsd.org/cgi/man.cgi?query=pf&sektion=4&format=html) 默认规则为丢弃。这个更改公开了一个 vnet 加载器可调参数 `net.pf.default_to_drop`，能让用户在不重新编译 [pf(4)](https://man.freebsd.org/cgi/man.cgi?query=pf&sektion=4&format=html) 模块的情况下更改默认规则。此更改类似于 [ipfw(4)](https://man.freebsd.org/cgi/man.cgi?query=ipfw&sektion=4&format=html) 的更改 [5f17ebf94db5](https://cgit.freebsd.org/src/commit/?id=5f17ebf94db5)。

为驱动程序 [igc(4)](https://man.freebsd.org/cgi/man.cgi?query=igc&sektion=4&format=html) 添加了 AIM。[eaa616f02193](https://cgit.freebsd.org/src/commit/?id=eaa616f02193)。[igc(4)](https://man.freebsd.org/cgi/man.cgi?query=igc&sektion=4&format=html) 是从 [igb(4)](https://man.freebsd.org/cgi/man.cgi?query=igb&sektion=4&format=html) 派生的，之前没有 AIM 实现。来自 e1000 的相同算法适用于此。AIM 算法是从旧的 igb 或树外驱动程序中重新引入的，并在获得使用其他驱动程序中英特尔代码的许可后进行了现代化处理。（由 Rubicon Communications, LLC/Netgate 和 BBOX.io 赞助）
...
`if_ix` 来自 [ixgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=ixgbe&sektion=4&format=html)，已应用 ix-3.3.38 的更改。[78d9eb6de856](https://cgit.freebsd.org/src/commit/?id=78d9eb6de856)。

`ixgbe_mbx` 来自 [ixgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=ixgbe&sektion=4&format=html)，已应用 ix-3.3.38 的更改。[fa00169e26ff](https://cgit.freebsd.org/src/commit/?id=fa00169e26ff)。

`ixgbe_phy` 来自 [ixgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=ixgbe&sektion=4&format=html)，已应用 ix-3.3.38 的更改。[b1dadbcebdfd](https://cgit.freebsd.org/src/commit/?id=b1dadbcebdfd)。

`if_sriov` 来自 [ixgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=ixgbe&sektion=4&format=html)，已应用 ix-3.3.38 的更改。[deea1953820e](https://cgit.freebsd.org/src/commit/?id=deea1953820e)。
...
本节涵盖了物理机器、虚拟机监控器和虚拟化环境的通用硬件支持，以及不适合归入本文档其他章节的硬件变更和更新。
...
修改 [sysctl(8)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&format=html) 使其能够在 Jail 中自行附加并运行。[5b0a5d8c1ea3](https://cgit.freebsd.org/src/commit/?id=5b0a5d8c1ea3)。这能让父 Jail 在子 Jail 没有安装 [sysctl(8)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&format=html) 时检索或设置内核状态（例如轻量级 OCI 容器或精简 Jail）。这在操作 Jail prison 或 vnet 的 sysctl 时特别有用。例如，`sysctl -j foo -Ja` 或 `sysctl -j foo net.fibs=2`。

修改 [ip6addrctl(8)](https://man.freebsd.org/cgi/man.cgi?query=ip6addrctl&sektion=8&format=html) 使其能够在 Jail 中自行附加并运行。[fa9926a62ae3](https://cgit.freebsd.org/src/commit/?id=fa9926a62ae3)。这将使得管理 vnet Jail 的地址选择策略变得更容易，特别是对于那些轻量级 OCI 容器或精简 Jail。

使 vnet [sysctl(9)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=9&format=html) 变量成为加载器可调参数。[d2a999c2e0a0](https://cgit.freebsd.org/src/commit/?id=d2a999c2e0a0)。完成 [3da1cf1e88f8](https://cgit.freebsd.org/src/commit/?id=3da1cf1e88f8) 的第二阶段。标志 `CTLFLAG_TUN` 的含义已扩展，使其自动检查是否存在一个内核环境变量，用于在早期启动期间初始化 sysctl。谨以此纪念 Hans Petter Selasky。

为 [sysctl(8)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&format=html) 添加了用于过滤 Jail 和 vnet 变量的参数。[09cbd68e4e47](https://cgit.freebsd.org/src/commit/?id=09cbd68e4e47)。这样用户就不必查阅源代码来确认变量是否属于 Jail prison / vnet 变量。

定义了通用的 `mac` 节点用于 Jail 的 MAC 参数。[ae2383c0dd16](https://cgit.freebsd.org/src/commit/?id=ae2383c0dd16)。用于 MAC/do。（由 FreeBSD 基金会赞助）
...
本节包含了手册页（[man(1)](https://man.freebsd.org/cgi/man.cgi?query=man&sektion=1&format=html)）和其他随基本系统提供的文档的更改。

### 手册页

在 [ccdconfig(8)](https://man.freebsd.org/cgi/man.cgi?query=ccdconfig&sektion=8&format=html) 中参考 [graid(8)](https://man.freebsd.org/cgi/man.cgi?query=graid&sektion=8&format=html) 和 [zfs(8)](https://man.freebsd.org/cgi/man.cgi?query=zfs&sektion=8&format=html)，而不是 [gvinum(8)](https://man.freebsd.org/cgi/man.cgi?query=gvinum&sektion=8&format=html)。[9e3c356f11a9](https://cgit.freebsd.org/src/commit/?id=9e3c356f11a9)。
...
在 bsdinstall zfsboot 中添加了选项，用于编辑 ZFS 池的创建选项。[6258b5bf0670](https://cgit.freebsd.org/src/commit/?id=6258b5bf0670)。此选项能在创建 ZFS 启动池之前，覆盖默认选项（`-O compress=lz4 -O atime=off`）。例如，可以将压缩算法设置为不同的选项。

跟踪 pkg 仓库数据库的位置。[ef6b3c58883d](https://cgit.freebsd.org/src/commit/?id=ef6b3c58883d)。pkg 以前将上游仓库数据库的副本存储在 **/var/db/pkg/repo-*.sqlite** 中。大约一年前，这些数据库被移动到了 **/var/db/pkg/repos/*/**，导致 FreeBSD 云镜像不再清理这些数据库。（由亚马逊赞助）

## 关于后续 FreeBSD 版本的一般说明

预计 FreeBSD 15.0 不会支持除 armv7 以外的 32 位平台。armv6、i386 和 powerpc 平台已弃用，并将被移除。64 位系统仍然可以运行旧的 32 位二进制文件。
...
通过选项 `COMPAT_FREEBSD32` 在 64 位平台上执行 32 位二进制文件的支持将至少持续到 stable/15 和 stable/16 分支。通过 `cc -m32` 编译单个 32 位应用程序的支持也将至少持续到 stable/15 分支，该分支在 **/usr/include** 中包含适当的头文件，在 **/usr/lib32** 中包含库文件。

FreeBSD 15.0 及之后的版本中，Ports 将不再支持已弃用的 32 位平台。这些未来版本将不包括已弃用的 32 位平台的二进制软件包，也不支持从 Ports 构建这些平台的软件包。

FreeBSD stable/14 及更早版本将保留现有的 32 位内核和系统支持。Ports 将保留在 stable/14 及更早分支上为 32 位系统构建 Ports 和软件包的现有支持，只要这些分支仍然受到 Ports 的支持。然而，所有 32 位平台都是二级架构或三级架构，预计随着上游弃用 32 位平台，单个 Port 的支持将逐渐下降。

根据当前的支持计划，stable/14 将在 FreeBSD 14.0-RELEASE 发布后的大约 5 年后结束生命周期（EOL）。stable/14 的 EOL 将标志着对已弃用的 32 位平台的支持结束，包括源代码发布、预构建的软件包和从 Ports 构建应用程序的支持。随着 14.0-RELEASE 在 2023 年 11 月发布，对已弃用的 32 位平台的支持将于 2028 年 11 月结束。
