...
* 发现 Amazon EC2™ 镜像存在一个问题，从早期 FreeBSD 版本升级时，虚拟机在引导期间会挂起。新的 EC2™ 安装不受影响，但运行早期版本的现有安装建议等待勘误通知解决此问题后再升级。计划在发行版之后发布解决此问题的勘误通知。
...
* [2016-10-21] ZFS 文件系统已新增 `sha512` 和 `skein` 校验和支持。此变更未在发行说明中提及。
