...
- 添加了 **高级电源管理监控守护进程（apmd(8)）**。该守护进程允许用户选择由用户空间处理的 APM 事件，并为给定事件指定命令。这使得 APM 行为可以以灵活的方式进行配置。有关详细信息，请参阅手册页。
...
您可以从以下地址订购 FreeBSD 3.3-RELEASE CD：
```
        Walnut Creek CDROM  
...
每张 -RELEASE CD 的费用为 $39.95 或者如果您订阅了 FreeBSD，则为 $24.95。FreeBSD SNAPshot CD（如果有的话）的费用为 $39.95 或者如果您订阅了 FreeBSD-SNAP，则为 $14.95（-RELEASE 和 -SNAP 订阅是完全独立的）。 有了订阅，您将自动收到更新版本的 CD。每次发货时将扣除信用卡费用，您可以随时取消订阅，无需承担其他义务。
...
或者，如果您已经加载了文档分发：`file:/usr/share/doc/handbook/staff.html`
