...
从具有互联网邮件连接的机器上提交错误报告的首选方法是使用 `send-pr` 命令，或者通过 [http://www.FreeBSD.org/send-pr.html](http://www.FreeBSD.org/send-pr.html) 使用 CGI 脚本。错误报告将通过我们的忠实 bugfiler 程序自动提交，您可以放心，我们会尽力尽快回复所有报告的错误。通过这种方式提交的错误也会在我们的网站的支持部分可见，因此它们既作为错误报告，也作为其他用户关注潜在问题的“指示参数”。
