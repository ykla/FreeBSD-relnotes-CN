...
[i386, sparc64, ia64, amd64] [ahd(4)](https://man.freebsd.org/cgi/man.cgi?query=ahd&sektion=4&format=html) 驱动支持以下内容：
...
[i386] [asr(4)](https://man.freebsd.org/cgi/man.cgi?query=asr&sektion=4&format=html) 驱动当前支持的适配器包括以下内容：
...
[i386, amd64] 真正的 BusLogic MultiMaster 克隆的 AMI FastDisk 主机适配器也受 [bt(4)](https://man.freebsd.org/cgi/man.cgi?query=bt&sektion=4&format=html) 驱动支持。
...
**注意：** [amd64, i386] 支持从这些控制器启动。不支持 EISA 适配器。

[sparc64] [esp(4)](https://man.freebsd.org/cgi/man.cgi?query=esp&sektion=4&format=html) 驱动为存在于多种系统和外围板卡中的 Qlogic FAS216 和 FAS408 SCSI 控制器芯片提供支持。这包括大多数 Sun Ultra 1e 和 Ultra 2 机器中的 Qlogic SCSI 卡。对于 Qlogic PCI SCSI 主机适配器，应使用 isp(4) 驱动代替 [esp(4)](https://man.freebsd.org/cgi/man.cgi?query=esp&sektion=4&format=html) 驱动。
...
[mpt(4)](https://man.freebsd.org/cgi/man.cgi?query=mpt&sektion=4&format=html) 驱动支持的 Ultra 320 SCSI 控制器芯片可见于许多系统，包括：

* Dell PowerEdge 1750 thru 2850
* IBM eServer xSeries 335

这些系统还包含集成 RAID 镜像（Integrated Raid Mirroring）和增强型集成 RAID 镜像（Integrated Raid Mirroring Enhanced），此驱动也支持它们。SAS 控制器芯片也出现在许多新的基于 AMD/Opteron 的系统上，如 Sun 4100。注意，此控制器可以同时驱动 SAS 和 SATA 驱动器或两者的混合。这些控制器可用的集成 RAID 镜像支持充其量只是差强人意。光纤通道控制器芯片组支持多种速度和系统。Apple 光纤通道 HBA 实际上就是 FC949ES 卡。此驱动还支持光纤通道卡的目标模式。此支持可通过 LSI Logic 固件实用程序设置核心的所需角色来启用，该实用程序确定卡可以承担的角色——无需单独编译。

[i386, pc98, sparc64, amd64] [ncr(4)](https://man.freebsd.org/cgi/man.cgi?query=ncr&sektion=4&format=html) 驱动支持以下 NCR/Symbios SCSI 控制器芯片：
...
[i386, pc98, sparc64, amd64] 已知支持以下附加板卡：
...
[sym(4)](https://man.freebsd.org/cgi/man.cgi?query=sym&sektion=4&format=html) 支持的 SCSI 控制器可以是主板嵌入式，或位于以下附加板卡上：
...
对于所有受支持的 SCSI 控制器，均提供对 SCSI-I、SCSI-II 和 SCSI-III 外围设备的完整支持，包括硬盘、光盘、磁带机（包括 DAT、8mm Exabyte、Mammoth 和 DLT）、介质更换器、处理器目标设备和 CD-ROM 驱动器。支持 CD-ROM 命令的 WORM 设备由 CD-ROM 驱动程序（如 [cd(4)](https://man.freebsd.org/cgi/man.cgi?query=cd&sektion=4&format=html)）提供只读访问支持。WORM/CD-R/CD-RW 写入支持由 [cdrecord(1)](http://www.FreeBSD.org/cgi/man.cgi?query=cdrecord&sektion=1&manpath=FreeBSD+Ports) 提供，该程序是 Ports Collection 中 [sysutils/cdrtools](http://www.FreeBSD.org/cgi/url.cgi?ports=sysutils/cdrtools/pkg-descr) port 的一部分。
...
[i386, pc98, amd64, powerpc] [axe(4)](https://man.freebsd.org/cgi/man.cgi?query=axe&sektion=4&format=html) 驱动支持基于 ASIX Electronics AX88172/AX88178/AX88772 的 USB 以太网适配器，包括：AX88172：
...
[i386, amd64] [bce(4)](https://man.freebsd.org/cgi/man.cgi?query=bce&sektion=4&format=html) 驱动支持基于 Broadcom NetXtreme II 系列千兆以太网控制器的各种网卡，包括以下：
...
[i386, pc98, sparc64, ia64, amd64] [bge(4)](https://man.freebsd.org/cgi/man.cgi?query=bge&sektion=4&format=html) 驱动支持基于 Broadcom BCM570x 系列千兆以太网控制器芯片的各种网卡，包括以下：
...
[i386, amd64] [cxgb(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgb&sektion=4&format=html) 驱动支持基于 T3 和 T3B 芯片组的 10 千兆和 1 千兆以太网适配器：
...
目前已知可与 [dc(4)](https://man.freebsd.org/cgi/man.cgi?query=dc&sektion=4&format=html) 驱动一起工作的 NIC 如下：
...
[i386, pc98] [ed(4)](https://man.freebsd.org/cgi/man.cgi?query=ed&sektion=4&format=html) 驱动支持以下以太网网卡：
...
[em(4)](https://man.freebsd.org/cgi/man.cgi?query=em&sektion=4&format=html) 驱动支持基于 Intel 82540、82541ER、82541PI、82542、82543、82544、82545、82546、82546EB、82546GB、82547、82571、82572、82573 和 82574 控制器芯片的千兆以太网适配器：
