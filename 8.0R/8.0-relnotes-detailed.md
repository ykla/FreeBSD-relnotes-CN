# FreeBSD 8.0-RELEASE 详细发行说明（2009 年 11 月 25 日）
...
此 FreeBSD 8.0-RELEASE 发行版为正式发行版。可以在 [ftp://ftp.FreeBSD.org/](ftp://ftp.freebsd.org/) 或其任何镜像站点找到。关于如何获取此版本（或其他版本）FreeBSD 发行版的更多信息，请参考 [FreeBSD 手册](https://www.freebsd.org/doc/en_US.ISO8859-1/books/handbook/) 中的 [“获取 FreeBSD”](https://www.freebsd.org/doc/en_US.ISO8859-1/books/handbook/mirrors.html) 附录。

在安装 FreeBSD 之前，强烈建议所有用户查看发行错误说明。错误说明文档会更新包含在发布周期后期或发布后发现的“临时”信息。通常，它包含已知错误、漏洞公告和文档修正。可以在 FreeBSD 网站上找到最新的 FreeBSD 8.0-RELEASE 错误说明副本。

## 2 新特性

本节介绍了自 7.0-RELEASE 以来 FreeBSD 中最显著的新功能或已更改的功能，以及先前版本的发行说明中所标注的更改 [7.1R] 和 [7.2R]。

典型的发行说明条目包括发布后 7.0-RELEASE 发布的安全公告、新的驱动程序或硬件支持、新的命令或选项、主要的错误修复，或第三方软件升级。它们也可能列出对主要 Port/软件包或发布工程实践的更改。显然，发行说明不能列出每个版本之间 FreeBSD 所做的每一项更改；本文档主要关注安全公告、用户可见的更改和主要的架构改进。
...
- 实现了新的虚拟化容器，名为“vimage”。默认情况下未启用。要启用此功能，请将以下内核选项添加到内核配置文件中并重新编译内核：
  ...
  vimage 是一个带有虚拟化 FreeBSD 网络栈的 Jail 容器。可以使用 [jail(8)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8&manpath=FreeBSD+8.0-RELEASE) 命令像这样创建 vimage：

```sh
# jail -c vnet name=vnet1 host.hostname=vnet1.example.net path=/ persist
```

  vimage 拥有自己的回环接口和独立的网络栈，包括 L3 路由表。系统上的网络接口可以通过使用 [ifconfig(8)](http://www.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8&manpath=FreeBSD+8.0-RELEASE) `vnet` 选项，在不同的 vimage Jail 之间及其外部进行移动。

  此外，新增 [epair(4)](http://www.freebsd.org/cgi/man.cgi?query=epair&sektion=4&manpath=FreeBSD+8.0-RELEASE) 虚拟接口驱动程序，以帮助 vimage Jail 之间的通信。它模拟了一对背对背连接的以太网接口。例如，以下命令创建一对 [epair(4)](http://www.freebsd.org/cgi/man.cgi?query=epair&sektion=4&manpath=FreeBSD+8.0-RELEASE) 接口：
...
- 在 Jail 中实现了带有 IPv6 的 SCTP（[sctp(4)](http://www.freebsd.org/cgi/man.cgi?query=sctp&sektion=4&manpath=FreeBSD+8.0-RELEASE)）。
- 通过使用 [cpuset(1)](http://www.freebsd.org/cgi/man.cgi?query=cpuset&sektion=1&manpath=FreeBSD+8.0-RELEASE) 实现了特定 CPU 绑定。请注意，当前实现允许 Jail 中的超级用户更改指定的 CPU 绑定。
...
- 请注意，[jail(8)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8&manpath=FreeBSD+8.0-RELEASE) 中的 Jail 和 prison 版本号已更新，以支持新功能。
...
[7.1R] 新增 [cpuset(2)] 系统调用，这是一个线程与 CPU 绑定以及 CPU 资源分组和分配的 API。
...
[7.1R] [ddb(4)] 调试器现在支持简单的脚本功能，支持由一组 [ddb(4)] 命令组成的命名脚本。这些命令可以通过 [ddb(4)] 或使用新的 [ddb(8)] 实用程序来管理。详见 [ddb(4)] 手册页。

[7.1R] [ddb(4)] 的命令 `ex` 现在支持 `/S` 模式，用于解释并打印请求地址处的符号。例如，`ex /S aio_swake` 打印当前通过 *aio_swake* 钩子注册的函数的名称。

[7.1R] 新增 [ddb(4)] 的命令 `show conifhk`，用于列出当前等待在 `run_interrupt_driven_config_hooks()` 中完成的钩子。

[7.1R] [fcntl(2)] 系统调用现支持命令 `F_DUP2FD`。这相当于 [dup(2)]，并与 Sun Solaris 和 IBM AIX 兼容。

[7.1R] FreeBSD 的 [linux(4)] ABI 现已支持 `sched_setaffinity()` 和 `sched_getaffinity()`，并使用实际的 CPU 亲和力设置原语。

[7.1R] 新增了 [procstat(1)] 实用程序。这是一个进程检查工具，提供了从 [procfs(5)] 中缺失的一些功能，以及用于监视和调试特定进程的新功能。

[7.1R] [rpc.lockd(8)] 的客户端功能已在 FreeBSD 内核中实现。此实现为 [flock(2)] 风格的锁提供了正确的语义，这些锁被 [lockf(1)] 命令行工具和 [pidfile(3)] 库使用。它还实现了从服务器重启后的恢复，并确保在获取锁之前，脏缓存块已写入服务器（允许多个客户端使用文件锁安全地共享数据）。此外，新增内核选项 **NFSLOCKD**，并默认启用。如果内核支持启用，[rpc.lockd(8)] 会自动检测并使用此功能。

[7.1R] FreeBSD 内核现支持新的内核转储文本转储格式。文本转储通过机械生成/提取的调试输出提供更高级的信息，而非简单的内存转储。此功能可用于生成简短的内核勘误，报告中包含丰富的调试信息，但不依赖于内核符号表或精确同步的源代码。更多信息请参见 [textdump(4)] 手册页。

[7.1R] [wait4(2)] 系统调用现支持参数 `WNOWAIT`，用于保持返回状态的进程处于可等待状态，并支持 `WSTOPPED`，该参数等同于 `WUNTRACED`。
...
**boot0** 引导加载程序现在保留其他操作系统使用的 `0x1b8` 偏移处的卷 ID。
...
[7.1R] [amd64, i386] 引导加载程序使用的 BTX 内核已更改为从实模式调用 BIOS 例程。此更改使得从 USB 设备启动 FreeBSD 成为可能。

[7.1R] [amd64, i386] 新增了 **gptboot** 引导加载程序，以支持从 GPT 标签磁盘启动。新增了 [ gpt(8)] 中的一个启动命令，可以通过写入所需的引导加载程序部分、创建新启动分区（如果需要）来使 GPT 磁盘可启动。
...
FreeBSD USB 子系统已重新实现，以支持现代设备并提高 SMP 可扩展性。新的实现包括无 Giant 锁的设备驱动程序、Linux 兼容层、[usbconfig(8)] 工具、完全支持分割事务和等时事务等。USB 设备的设备节点名称现在采用 **/dev/usb/\*bus\*.\*dev\*.\*endpoint\*** 的形式，**/dev/usbctl** 是主设备节点。请注意，[ugen(4)] 驱动程序为每个设备提供了节点，如 **/dev/ugen\*bus\*.\*dev\***，以保持向后兼容性。

[7.2R] [sparc64] FreeBSD 现在支持 Ultra SPARC III（Cheetah）处理器系列。

[7.2R] [acpi(4)] 子系统现在支持一个 [sysctl(8)] 变量 `debug.batt.batt_sleep_ms`。在一些具有智能电池的笔记本上，启用电池监控软件会导致 [atkbd(4)] 的按键丢失。这个 sysctl 变量通过在状态检查代码中添加毫秒级的延迟来作为解决方法。

[7.2R] [acpi_asus(4)] 驱动程序现在支持 Asus A8Sr 笔记本。

[7.2R] [powerpc] 添加了对 AltiVec（浮点和整数 SIMD 指令集）的支持。

[7.2R] 添加了 [cpuctl(4)](http://www.freebsd.org/cgi/man.cgi?query=cpuctl&sektion=4&manpath=FreeBSD+8.0-RELEASE) 驱动程序，它提供了特殊设备 **/dev/cpuctl** 作为与系统 CPU 的接口。该驱动程序的功能包括检索 CPUID 信息、读写机器特定寄存器（MSR）以及执行 CPU 固件更新。
...
[7.2R] [amd64, i386] 当 TSC 为 P-state 不变时，CPU 频率变化通知现在被禁用。此外，新增了加载器可调参数 `kern.timecounter.invariant_tsc`，可以通过将其设置为非零值强制启用该行为。
...
[7.2R] [sched_ule(4)](http://www.freebsd.org/cgi/man.cgi?query=sched_ule&sektion=4&manpath=FreeBSD+8.0-RELEASE) 调度器现在支持加载器可调参数 `machdep.hyperthreading_enabled`，就像 [sched_4bsd(4)](http://www.freebsd.org/cgi/man.cgi?query=sched_4bsd&sektion=4&manpath=FreeBSD+8.0-RELEASE) 一样。请注意，无法在运行时修改该参数。
...
FreeBSD 音频子系统得到了改进。改动包括每个通道的音量控制、高质量的定点带限 SINC 采样率转换器、比特完美模式、透明/自适应虚拟通道和独占流。更多信息请参阅 [snd(4)](http://www.freebsd.org/cgi/man.cgi?query=snd&sektion=4&manpath=FreeBSD+8.0-RELEASE) 手册页面。
...
[7.2R] 移除了 Aureal Vortex 1/2/Advantage PCI 的 [snd_au88x0(4)] 驱动程序，因为该驱动程序长期以来一直处于故障状态。
...
[fxp(4)](http://www.freebsd.org/cgi/man.cgi?query=fxp&sektion=4&manpath=FreeBSD+8.0-RELEASE) 驱动程序已改进。改动包括：
...
新增 [mwl(4)](http://www.freebsd.org/cgi/man.cgi?query=mwl&sektion=4&manpath=FreeBSD+8.0-RELEASE) 驱动程序，支持 Marvell 88W8363 IEEE 802.11n 无线网络设备。
...
[7.2R] [msk(4)](http://www.freebsd.org/cgi/man.cgi?query=msk&sektion=4&manpath=FreeBSD+8.0-RELEASE) 驱动程序已改进。修复了在某些条件下导致其挂起的问题。新增硬件 MAC 统计信息支持，用户可以通过 sysctl 变量 `dev.msk.N.stats` 获取该信息。

[7.2R] [nfe(4)](http://www.freebsd.org/cgi/man.cgi?query=nfe&sektion=4&manpath=FreeBSD+8.0-RELEASE) 驱动程序现在支持硬件 MAC 统计信息。

[7.2R] [re(4)](http://www.freebsd.org/cgi/man.cgi?query=re&sektion=4&manpath=FreeBSD+8.0-RELEASE) 驱动程序已改进。现在可以检测链接状态。新增了加载器可调参数 `hw.re.prefer_iomap`，用于禁用内存寄存器映射。该可调参数对 RTL8169SC 系列之外的所有控制器默认设置为 `0`。
...
[7.2R] [txp(4)](http://www.freebsd.org/cgi/man.cgi?query=txp&sektion=4&manpath=FreeBSD+8.0-RELEASE) 驱动程序现在支持所有受支持的架构。新增了对 [altq(4)](http://www.freebsd.org/cgi/man.cgi?query=altq&sektion=4&manpath=FreeBSD+8.0-RELEASE)、WoL、VLAN 启用时的校验和卸载支持，改进了链接状态变化处理，并新增了 MAC 统计信息 sysctl 变量 `dev.txp.N.stats`。新增了 sysctl 变量 `dev.txp.N.process_limit`，用于控制接收处理器中应服务的接收帧数量（默认为 64，合法范围为 16 到 128，单位为帧）。固件已更新至最新版本。

[7.1R] 新增 [ae(4)](http://www.freebsd.org/cgi/man.cgi?query=ae&sektion=4&manpath=FreeBSD+8.0-RELEASE) 驱动程序，支持 Attansic/Atheros L2 快速以太网控制器。

[7.1R] 新增 [jme(4)](http://www.freebsd.org/cgi/man.cgi?query=jme&sektion=4&manpath=FreeBSD+8.0-RELEASE) 驱动程序，支持基于 JMicron JMC250 千兆以太网和 JMC260 快速以太网控制器的 PCIe 适配器。

[7.1R] 新增 [age(4)](http://www.freebsd.org/cgi/man.cgi?query=age&sektion=4&manpath=FreeBSD+8.0-RELEASE) 驱动程序，支持 Attansic/Atheros L1 千兆以太网控制器。

[7.1R] 新增 [malo(4)](http://www.freebsd.org/cgi/man.cgi?query=malo&sektion=4&manpath=FreeBSD+8.0-RELEASE) 驱动程序，支持基于 Marvell Libertas 88W8335 的 PCI 网络适配器。

[7.1R] 新增 bm(4) 驱动程序，支持 Apple Big Mac (BMAC) 以太网控制器，该控制器出现在多个 Apple G3 型号中。

[7.1R] 新增 et(4) 驱动程序，支持 Agere ET1310 10/100/千兆以太网控制器。

[7.1R] 新增 [glxsb(4)](http://www.freebsd.org/cgi/man.cgi?query=glxsb&sektion=4&manpath=FreeBSD+8.0-RELEASE) 驱动程序，支持 AMD Geode LX 处理器中的安全模块。

[7.1R] 新增 [ale(4)](http://www.freebsd.org/cgi/man.cgi?query=ale&sektion=4&manpath=FreeBSD+8.0-RELEASE) 驱动程序，支持 Atheros AR8121/AR8113/AR8114 千兆/快速以太网控制器。此驱动程序在本版本的 **GENERIC** 内核中未启用。
...
[7.1R] 为 [msk(4)](http://www.freebsd.org/cgi/man.cgi?query=msk&sektion=4&manpath=FreeBSD+8.0-RELEASE) 驱动程序中的 TCP/UDP 硬件校验和卸载短帧时的错误添加了临时解决方法。请注意，对于需要硬件 VLAN 标签插入的帧，由于 VLAN 标签后的 mbuf 中校验和偏移量发生变化，校验和卸载的临时解决方法无法正常工作。因此，在这种情况下需要禁用 VLAN 接口的硬件校验和卸载。
...
新增一种用于加速 L2 和 L3 查找的流缓存基础设施。这被称为“流表”，在 amd64 和 i386 平台上默认启用。这也提供了状态感知的负载均衡，当与 **RADIX_MPATH** 一起使用时。

FreeBSD L2 地址转换表已重新实现，以减少并行处理时的锁争用并简化路由逻辑。新实现具有分别用于 ARP（IPv4）和 NDP（IPv6）的 L2 地址转换表，这些表与 L3 路由表分开，并支持路由表和 L2 信息的流表缓存。一个用户可见的变化是，克隆路由的概念（通过带有参数 `RTF_CLONING` 的条目生成的路由）已被弃用。这意味着路由参数 `RTF_CLONING`、`RTF_WASCLONE` 和 `RTF_LLINFO` 已过时。
...
[7.2R] 现支持对未绑定套接字的 IPv4 源地址选择，具体如下：
...
[7.2R] [ng_netflow(4)](http://www.freebsd.org/cgi/man.cgi?query=ng_netflow&sektion=4&manpath=FreeBSD+8.0-RELEASE) Netgraph 节点现在支持生成出口 netflow，替代或附加到入口 netflow。新增一个 `NGM_NETFLOW_SETCONFIG` 控制消息来控制这一新功能。

[7.2R] [tap(4)](http://www.freebsd.org/cgi/man.cgi?query=tap&sektion=4&manpath=FreeBSD+8.0-RELEASE) 以太网隧道软件网络接口现在支持新的 `TAPGIFNAME` 字符设备 ioctl。这是一个方便的快捷方式，通过字符设备的文件描述符获取网络接口名称。
...
[7.1R] [if_bridge(4)](http://www.freebsd.org/cgi/man.cgi?query=if_bridge&sektion=4&manpath=FreeBSD+8.0-RELEASE) 接口现在可以通过 [ifconfig(8)](http://www.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8&manpath=FreeBSD+8.0-RELEASE) 的参数 `ifmaxaddr` 限制桥接接口后面的源 MAC 数量。
...
[7.1R] [dummynet(4)](http://www.freebsd.org/cgi/man.cgi?query=dummynet&sektion=4&manpath=FreeBSD+8.0-RELEASE) 子系统现在支持快速模式操作，允许某些数据包绕过 dummynet 调度器。这可以在数据包流量低于管道带宽时实现更低的延迟和更少的开销，并消除子系统中的递归。新增 sysctl 变量 `net.inet.ip.dummynet.io_fast` 来启用此功能。
...
[7.1R] 由于缺乏多处理器支持，移除了 netatm 子系统。
...
[7.1R] 为 TCP 套接字新增套接字选项 `TCP_CONGESTION`，用于设置和获取拥塞控制算法。该名称旨在与 Linux 保持兼容性。
...
此外，[ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+8.0-RELEASE) 现支持操作规则 `setfib`。以下规则：
...
新增了用户空间实用工具 [mfiutil(8)](http://www.freebsd.org/cgi/man.cgi?query=mfiutil&sektion=8&manpath=FreeBSD+8.0-RELEASE)，适用于 [mfi(4)](http://www.freebsd.org/cgi/man.cgi?query=mfi&sektion=4&manpath=FreeBSD+8.0-RELEASE) 设备。该工具提供基本功能，包括监控控制器、阵列和驱动器状态，修改基本属性，创建/删除阵列和备用驱动器，以及刷新控制器固件。请注意，这是一个小型工具，并非 Ports 中的 MegaCLI 的替代品，后者提供官方支持并具备更多功能。
...
**[7.2R]** [glabel(8)](http://www.freebsd.org/cgi/man.cgi?query=glabel&sektion=8&manpath=FreeBSD+8.0-RELEASE) GEOM 类现在支持新的基于 UFS 的标签 `ufsid`，可通过唯一的文件系统 ID 引用包含 UFS 的设备。当启用 [glabel(8)](http://www.freebsd.org/cgi/man.cgi?query=glabel&sektion=8&manpath=FreeBSD+8.0-RELEASE) GEOM 类时，该文件系统 ID 会自动生成和检测。例如，新标签的形式为：**/dev/ufsid/48e69c8b5c8e1b43**。使用 GEOM 标签的优点在于避免设备重命名引起的问题，如移动驱动器或控制器时。
...
[7.2R] 新增 **sdhci(4)** 驱动程序。根据 SD 主机控制器规范，该驱动程序支持类别为 8 和子类别为 5 的 PCI 设备。
...
[7.1R] **ata(4)** 驱动程序现已支持加载器变量 `hw.ata.ata_dma_check_80pin`，可用于在某些问题系统（如特定笔记本和 Soekris 板）上禁用 80 针电缆检查。默认值为 1。

[7.1R] 修复了 **ata(4)** 驱动程序在 ServerWorks HT1000 芯片组上的数据损坏问题。

[7.1R] **ciss(4)** 驱动程序现已支持加载器调优变量 `hw.ciss.nop_message_heartbeat`，用于在 **ciss_periodic()** 中启用 NOP 消息轮询，可作为解决 ADAPTER HEARTBEAT FAILED 问题的替代方法。默认值为 0（禁用）。
...
UFS 文件系统的“危险专用”模式（“dangerously dedicated”mode）不再受支持。
...
[gvinum(8)](http://www.freebsd.org/cgi/man.cgi?query=gvinum&sektion=8&manpath=FreeBSD+8.0-RELEASE) 现在支持命令 `grow`，使用户可以更轻松地扩展 plex，而无需了解所有的实现细节。

FreeBSD 的 NFS 子系统现在在客户端和服务器端支持 `RPCSEC_GSS` 认证。这替代了 NFS 客户端和服务器的 RPC 实现，采用最初为支持 NFS 锁管理器而开发的新型 RPC 实现。该实现同时支持新的 RPC 实现和从原始 NFS 代码库继承的旧版实现，默认使用新的实现。要在客户端或服务器端使用 `RPCSEC_GSS`，必须构建一个包含 **KGSSAPI** 选项和 [crypto(4)](http://www.freebsd.org/cgi/man.cgi?query=crypto&sektion=4&manpath=FreeBSD+8.0-RELEASE) 设备的内核。详见 [gssd(8)](http://www.freebsd.org/cgi/man.cgi?query=gssd&sektion=8&manpath=FreeBSD+8.0-RELEASE) 手册页面。

FreeBSD 的 NFS 子系统现在包含新的实验性实现，支持 NFSv2、NFSv3 和 NFSv4。该功能默认未启用。要启用它，请将以下内核选项添加到内核配置文件中并重新构建内核：
...
[VFS(9)](http://www.freebsd.org/cgi/man.cgi?query=VFS&sektion=9&manpath=FreeBSD+8.0-RELEASE) 子系统中的路径名查找现在支持共享 vnode 锁定。此功能默认启用。将 sysctl 变量 `vfs.lookup_shared` 设置为 `0` 可禁用此功能。注意，与 sysctl 变量等效的内核选项 **LOOKUP_SHARED** 已移除。
...
[7.1R] [gvinum(8)](http://www.freebsd.org/cgi/man.cgi?query=gvinum&sektion=8&manpath=FreeBSD+8.0-RELEASE) 现在在创建 plex 时接受 *volume* 参数。
...
新增了 BSD 许可的 [cpio(1)](http://www.freebsd.org/cgi/man.cgi?query=cpio&sektion=1&manpath=FreeBSD+8.0-RELEASE) 工具以替代 **GNU cpio**，并且它现在是 FreeBSD 基本系统中的默认工具。
...
[df(1)](http://www.freebsd.org/cgi/man.cgi?query=df&sektion=1&manpath=FreeBSD+8.0-RELEASE) 工具的参数 `-h` 现在支持在指定参数 `-i` 时以人类可读的格式显示 inode 数量。

[df(1)](http://www.freebsd.org/cgi/man.cgi?query=df&sektion=1&manpath=FreeBSD+8.0-RELEASE) 工具现在支持参数 `-T`，用于在每项条目中显示文件系统类型。
...
[dhclient(8)](http://www.freebsd.org/cgi/man.cgi?query=dhclient&sektion=8&manpath=FreeBSD+8.0-RELEASE) 现在在调用 [route(8)](http://www.freebsd.org/cgi/man.cgi?query=route&sektion=8&manpath=FreeBSD+8.0-RELEASE) 命令时使用参数 `-n`。这消除了当获得租约但 DNS 服务不可用时的长时间延迟。
...
[fsck(8)](http://www.freebsd.org/cgi/man.cgi?query=fsck&sektion=8&manpath=FreeBSD+8.0-RELEASE) 工具现已支持参数 `-r`，用于释放多余未使用的 inode。减少预分配 inode 的数量可以缩短将来运行 fsck 的时间，并释放可供文件分配的空间。在预检模式（preen mode）下运行时，此参数会被忽略。

[freebsd-update(8)](http://www.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&manpath=FreeBSD+8.0-RELEASE) 现在支持在安装新内核时备份旧内核。备份内核会写入 **/boot/kernel.old**，如果该目录不存在或是之前由 `freebsd-update` 创建的。否则，`freebsd-update` 将生成新的目录名称以存放备份。此功能默认启用。
...
新增 [i2c(8)](http://www.freebsd.org/cgi/man.cgi?query=i2c&sektion=8&manpath=FreeBSD+8.0-RELEASE) 工具，用于 I2C 的诊断。

[ifconfig(8)](http://www.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8&manpath=FreeBSD+8.0-RELEASE) 现在支持 `vnet` 和 `-vnet` 选项，用于在带有 vimage 的 Jail 之间移动接口。
...
[route(8)](http://www.freebsd.org/cgi/man.cgi?query=route&sektion=8&manpath=FreeBSD+8.0-RELEASE) 工具现支持 `show`、`weights` 和 `sticky` 命令。详见 [route(8)](http://www.freebsd.org/cgi/man.cgi?query=route&sektion=8&manpath=FreeBSD+8.0-RELEASE) 手册页。

[rtld(1)](http://www.freebsd.org/cgi/man.cgi?query=rtld&sektion=1&manpath=FreeBSD+8.0-RELEASE) 现在支持新的环境变量 `LD_ELF_HINTS_PATH`，用于覆盖 rtld 提示文件。如果进程使用了 setuid 和/或 setgid，则该环境变量会被忽略。这一功能为使用非默认位置的自定义共享库提供了便利。

[rtld(1)](http://www.freebsd.org/cgi/man.cgi?query=rtld&sektion=1&manpath=FreeBSD+8.0-RELEASE) 现在支持在 rpath 和 soneeded 路径中进行动态字符串令牌替换。支持的令牌包括 `$ORIGIN`、`$OSNAME`、`$OSREL` 和 `$PLATFORM`。启用此替换需要在 `DT_FLAGS` 中设置参数 `DF_ORIGIN` 或在 `DF_FLAGS_1` 中设置 `DF_1_ORIGIN`，可以通过 **GNU ld** 的参数 `-z origin` 设置。对于 setuid/setgid 进程，此功能会被无条件禁用。`$ORIGIN` 的替换依赖于 FreeBSD 内核提供的 `AT_EXECPATH` 辅助信息。
...
[sysinstall(8)](http://www.freebsd.org/cgi/man.cgi?query=sysinstall&sektion=8&manpath=FreeBSD+8.0-RELEASE) 的菜单已简化，以减少与系统其他部分的混淆和重复。**Xorg** 窗口系统应像其他软件包一样安装。**Linux** 和 **OSF/1** 仿真的配置应通过内核重建完成。安装磁带媒体的支持已被移除，因为其被认为已不可用。支持 OLDCARD 的过时代码也已被移除。
...
[traceroute(8)](http://www.freebsd.org/cgi/man.cgi?query=traceroute&sektion=8&manpath=FreeBSD+8.0-RELEASE) 和 [traceroute6(8)](http://www.freebsd.org/cgi/man.cgi?query=traceroute6&sektion=8&manpath=FreeBSD+8.0-RELEASE) 现在支持参数 `-a`，用于显示每跳的 IP 地址对应的 AS 号。它会通过 `-A` 选项指定的 WHOIS 服务器查询该号码。如果未指定 `-A`，则默认使用 `whois.radb.net`。

[tzsetup(8)](http://www.freebsd.org/cgi/man.cgi?query=tzsetup&sektion=8&manpath=FreeBSD+8.0-RELEASE) 现在支持参数 `-s`，用于跳过关于是否将时钟调整为 UTC 的问题。
...
[7.2R] 添加了来自 NetBSD [btpand(8)](http://www.freebsd.org/cgi/man.cgi?query=btpand&sektion=8&manpath=FreeBSD+8.0-RELEASE) 守护进程，支持蓝牙网络接入点 (NAP)、群组自组织网络 (GN) 和个人区域网络用户 (PANU) 配置文件。
...
[7.2R] [du(1)](http://www.freebsd.org/cgi/man.cgi?query=du&sektion=1&manpath=FreeBSD+8.0-RELEASE) 工具现在支持参数 `-l`。当指定该参数时，具有多个硬链接的文件将被视为多个不同的文件。

[7.2R] [du(1)](http://www.freebsd.org/cgi/man.cgi?query=du&sektion=1&manpath=FreeBSD+8.0-RELEASE) 工具现在支持参数 `-A`，以显示表观大小而非磁盘使用量。这在操作压缩卷或稀疏文件时非常有用。

[7.2R] [du(1)](http://www.freebsd.org/cgi/man.cgi?query=du&sektion=1&manpath=FreeBSD+8.0-RELEASE) 工具现在支持 `-Bblocksize` 选项，用于以 *blocksize* 字节为单位计算块数。这不同于 `-k` 或 `-m` 选项或设置 `BLOCKSIZE` 的方式，它可以估算在具有指定 *blocksize* 的文件系统上，检查的文件层级结构所需的空间大小。除非使用 `-A` 模式，否则 *blocksize* 会向上取整为 512 的倍数。

[7.2R] [dumpfs(8)](http://www.freebsd.org/cgi/man.cgi?query=dumpfs&sektion=8&manpath=FreeBSD+8.0-RELEASE) 工具现在支持参数 `-f`，该参数可以按片段（块）编号列出文件系统中所有空闲的片段。这种新模式执行必要的运算，以生成绝对片段编号，而不是默认模式中打印的 cg 相对编号。

如果使用一次 `-f`，则连续片段范围会被压缩为 `X-Y` 格式，与常规 dumpfs 输出中的空闲块列表相同。如果使用两次，则会逐个打印所有块编号，支持更紧凑或脚本友好的表示方式。

[7.2R] [fetch(1)](http://www.freebsd.org/cgi/man.cgi?query=fetch&sektion=1&manpath=FreeBSD+8.0-RELEASE) 工具现在支持参数 `-i`，该参数支持 If-Modified-Since HTTP 1.1 请求。如果指定此参数，则文件仅在比本地文件的 mtime 更新时才会被下载。此外，**libfetch** 现在接受 URL 结构中的 mtime，并支持用于启用此行为的参数。

[7.2R] [fsck(8)](http://www.freebsd.org/cgi/man.cgi?query=fsck&sektion=8&manpath=FreeBSD+8.0-RELEASE) 工具现在支持参数 `-C`，用于检查清洁模式。在该模式下，首先检查文件系统是否被正常卸载，如果是，则跳过文件系统检查。否则，执行完整检查。

[7.2R] [fsck(8)](http://www.freebsd.org/cgi/man.cgi?query=fsck&sektion=8&manpath=FreeBSD+8.0-RELEASE) 工具现在支持参数 `-D`，用于损坏恢复模式，该模式将启用某些激进操作，使得 [fsck(8)](http://www.freebsd.org/cgi/man.cgi?query=fsck&sektion=8&manpath=FreeBSD+8.0-RELEASE) 能够处理存在严重数据损坏的文件系统。当磁盘数据损坏非常严重并导致 [fsck(8)](http://www.freebsd.org/cgi/man.cgi?query=fsck&sektion=8&manpath=FreeBSD+8.0-RELEASE) 崩溃时，这是最后的补救措施。

[7.2R] [getaddrinfo(3)](http://www.freebsd.org/cgi/man.cgi?query=getaddrinfo&sektion=3&manpath=FreeBSD+8.0-RELEASE) 函数现在支持 SCTP。

[7.2R] 修复了 [ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+8.0-RELEASE) 工具的一个错误，该错误会在即使指定参数 `-q` 时，仍然为 NAT 规则显示额外消息。

[7.2R] [ln(1)](http://www.freebsd.org/cgi/man.cgi?query=ln&sektion=1&manpath=FreeBSD+8.0-RELEASE) 工具现在支持参数 `-w`，用于检查源文件是否实际存在。如果指定该参数而文件不存在，[ln(1)](http://www.freebsd.org/cgi/man.cgi?query=ln&sektion=1&manpath=FreeBSD+8.0-RELEASE) 将发出警告消息。

[ln(1)](http://www.freebsd.org/cgi/man.cgi?query=ln&sektion=1&manpath=FreeBSD+8.0-RELEASE) 工具现在可以为符号链接创建硬链接，因为 POSIX.1-2008 要求参数 `-L` 和 `-P` 具有此行为。

[lpr(1)](http://www.freebsd.org/cgi/man.cgi?query=lpr&sektion=1&manpath=FreeBSD+8.0-RELEASE) 工具现在支持参数 `-m`，用于在作业完成后发送电子邮件，以及 `-t` 选项以设置作业标题。

[7.2R] [make(1)](http://www.freebsd.org/cgi/man.cgi?query=make&sektion=1&manpath=FreeBSD+8.0-RELEASE) 工具现在支持参数 `-p`，仅打印输入图，而不执行任何命令。输出与 `-d g1` 相同。结合 `-f /dev/null` 使用时，仅显示 [make(1)](http://www.freebsd.org/cgi/man.cgi?query=make&sektion=1&manpath=FreeBSD+8.0-RELEASE) 的内置规则。

[7.2R] [make(1)](http://www.freebsd.org/cgi/man.cgi?query=make&sektion=1&manpath=FreeBSD+8.0-RELEASE) 工具现在支持参数 `-Q`，使其在指定 `-j` 选项时不会生成文件横幅，同时具有参数 `-q` 的相同效果。

[7.2R] [make(1)](http://www.freebsd.org/cgi/man.cgi?query=make&sektion=1&manpath=FreeBSD+8.0-RELEASE) 工具现在支持变量 `.MAKE.JOB.PREFIX`。如果指定了 `-j` 和 `-v`，其输出的每个目标都会带有一个前缀标记 — *target*，该标记的第一部分可以通过该变量控制。
...
[7.2R] [netstat(1)](http://www.freebsd.org/cgi/man.cgi?query=netstat&sektion=1&manpath=FreeBSD+8.0-RELEASE) 工具现在支持参数 `-L`，可显示 [unix(4)](http://www.freebsd.org/cgi/man.cgi?query=unix&sektion=4&manpath=FreeBSD+8.0-RELEASE) 套接字的监听队列统计信息。
...
[7.2R] [pciconf(8)](http://www.freebsd.org/cgi/man.cgi?query=pciconf&sektion=8&manpath=FreeBSD+8.0-RELEASE) 工具新增参数 `-b`，用于列出每个设备分配资源的基址寄存器 (BAR)。

[7.2R] [powerd(8)](http://www.freebsd.org/cgi/man.cgi?query=powerd&sektion=8&manpath=FreeBSD+8.0-RELEASE) 程序有所改进，包括对 SMP 系统的更合理的 CPU 负载估算，以及新增的适用于交流电供电系统的 hiadaptive 模式。该模式以两倍于 adaptive 模式的速度提升 CPU 频率，以四倍更慢的速度降低频率，更倾向于较低的 CPU 负载，并在最大负载后的高频阶段增加了额外的延迟。
...
[7.2R] [stat(1)](http://www.freebsd.org/cgi/man.cgi?query=stat&sektion=1&manpath=FreeBSD+8.0-RELEASE) 工具现在支持参数 `-x`，可以八进制形式显示 suid、sgid 和粘性位。
...
[7.2R] [wc(1)](http://www.freebsd.org/cgi/man.cgi?query=wc&sektion=1&manpath=FreeBSD+8.0-RELEASE) 工具新增参数 `-L`，用于输出最长输入行中的字符数。
...
[7.1R] [chflags(1)](http://www.freebsd.org/cgi/man.cgi?query=chflags&sektion=1&manpath=FreeBSD+8.0-RELEASE) 现在支持参数 `-v` 输出详细信息，参数 `-f` 忽略错误，以及参数 `-h`，用于设置符号链接的参数，其语义与 [chmod(1)](http://www.freebsd.org/cgi/man.cgi?query=chmod&sektion=1&manpath=FreeBSD+8.0-RELEASE) 类似。

[7.1R] [cp(1)](http://www.freebsd.org/cgi/man.cgi?query=cp&sektion=1&manpath=FreeBSD+8.0-RELEASE) 现在支持参数 `-a`，该参数等同于参数 `-RpP`。
...
[7.1R] [cron(8)](http://www.freebsd.org/cgi/man.cgi?query=cron&sektion=8&manpath=FreeBSD+8.0-RELEASE) 工具现在支持参数 `-m`，用于覆盖默认的 cron 邮件收件人，除非在 crontab 文件中明确提供了 MAILTO= 行。
...
[7.1R] [fdisk(8)](http://www.freebsd.org/cgi/man.cgi?query=fdisk&sektion=8&manpath=FreeBSD+8.0-RELEASE) 现在支持参数 `-q`，以禁止显示任何警告。
...
[7.1R] [fwcontrol(8)](http://www.freebsd.org/cgi/man.cgi?query=fwcontrol&sektion=8&manpath=FreeBSD+8.0-RELEASE) 工具现在支持 `-f node` 选项，用于指定 *node* 作为下一个总线重置的根节点。
...
[7.1R] [kgdb(1)](http://www.freebsd.org/cgi/man.cgi?query=kgdb&sektion=1&manpath=FreeBSD+8.0-RELEASE) 命令现在支持命令 `add-kld *kld*`，用于定位一个 [kld(4)](http://www.freebsd.org/cgi/man.cgi?query=kld&sektion=4&manpath=FreeBSD+8.0-RELEASE) 并加载其符号。
...
[7.1R] [kgdb(1)](http://www.freebsd.org/cgi/man.cgi?query=kgdb&sektion=1&manpath=FreeBSD+8.0-RELEASE) 现在支持命令 `tid` 和其他与内核模块相关的命令，即使是远程目标。

[7.1R] 新增 [kvm_getcptime(3)](http://www.freebsd.org/cgi/man.cgi?query=kvm_getcptime&sektion=3&manpath=FreeBSD+8.0-RELEASE) 函数，用于获取内核中的全局 CPU 时间统计信息。
...
[7.1R] [mdconfig(8)](http://www.freebsd.org/cgi/man.cgi?query=mdconfig&sektion=8&manpath=FreeBSD+8.0-RELEASE) 命令现在支持 `-v`（详细）参数用于命令 `-l`。它一次显示所有 [md(4)](http://www.freebsd.org/cgi/man.cgi?query=md&sektion=4&manpath=FreeBSD+8.0-RELEASE) 设备的大小和后备存储。
...
[7.1R] [moused(8)](http://www.freebsd.org/cgi/man.cgi?query=moused&sektion=8&manpath=FreeBSD+8.0-RELEASE) 工具现在支持参数 `-L`，用于更改滚动速度，并使 `-U` 选项仅影响滚动阈值。
...
[7.1R] [ping6(8)](http://www.freebsd.org/cgi/man.cgi?query=ping6&sektion=8&manpath=FreeBSD+8.0-RELEASE) 现在支持参数 `-o`，使用此参数， [ping6(8)](http://www.freebsd.org/cgi/man.cgi?query=ping6&sektion=8&manpath=FreeBSD+8.0-RELEASE) 在接收到一个回复包后成功退出。

[7.1R] [ping6(8)](http://www.freebsd.org/cgi/man.cgi?query=ping6&sektion=8&manpath=FreeBSD+8.0-RELEASE) 现在支持参数 `-r` 和 `-R`，这与 [ping(8)](http://www.freebsd.org/cgi/man.cgi?query=ping&sektion=8&manpath=FreeBSD+8.0-RELEASE) 的参数 `-a` 和 `-A` 相同。
...
[7.1R] [realpath(1)](http://www.freebsd.org/cgi/man.cgi?query=realpath&sektion=1&manpath=FreeBSD+8.0-RELEASE) 工具现在支持参数 `-q`，以抑制警告，并接受多个路径作为命令行参数。

[7.1R] [rfcomm_pppd(8)](http://www.freebsd.org/cgi/man.cgi?query=rfcomm_pppd&sektion=8&manpath=FreeBSD+8.0-RELEASE) 现在支持参数 `-D`，以注册拨号网络服务（DUN），除了局域网访问 PPP（LAN）服务之外。
...
[7.1R] 修复了 [telnetd(8)](http://www.freebsd.org/cgi/man.cgi?query=telnetd&sektion=8&manpath=FreeBSD+8.0-RELEASE) 中的一个错误，该错误导致即使指定了 `-a off` 选项，仍会尝试进行身份验证。

[7.1R] [top(1)](http://www.freebsd.org/cgi/man.cgi?query=top&sektion=1&manpath=FreeBSD+8.0-RELEASE) 和 [vmstat(8)](http://www.freebsd.org/cgi/man.cgi?query=vmstat&sektion=8&manpath=FreeBSD+8.0-RELEASE) 命令现在支持参数 `-P`，显示每个 CPU 的统计信息。

[7.1R] 新增 [uuid_enc_le(3)](http://www.freebsd.org/cgi/man.cgi?query=uuid_enc_le&sektion=3&manpath=FreeBSD+8.0-RELEASE)、[uuid_dec_le(3)](http://www.freebsd.org/cgi/man.cgi?query=uuid_dec_le&sektion=3&manpath=FreeBSD+8.0-RELEASE)、[uuid_enc_be(3)](http://www.freebsd.org/cgi/man.cgi?query=uuid_enc_be&sektion=3&manpath=FreeBSD+8.0-RELEASE) 和 [uuid_dec_be(3)](http://www.freebsd.org/cgi/man.cgi?query=uuid_dec_be&sektion=3&manpath=FreeBSD+8.0-RELEASE) 函数。这些函数用于编码/解码 UUID 的二进制表示。
...
[7.1R] [rc.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5&manpath=FreeBSD+8.0-RELEASE) 现在支持变量 `dummynet_enable`，该变量允许在 `firewall_enable` 为 `YES` 时加载 [dummynet(4)](http://www.freebsd.org/cgi/man.cgi?query=dummynet&sektion=4&manpath=FreeBSD+8.0-RELEASE) 内核模块。
...
[7.1R] ppp [rc(8)](http://www.freebsd.org/cgi/man.cgi?query=rc&sektion=8&manpath=FreeBSD+8.0-RELEASE) 脚本现在支持多个实例。更多详情，请参阅 [rc.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5&manpath=FreeBSD+8.0-RELEASE) 中变量 `ppp_profile` 的描述。
...
[7.2R] 修复了 [pkg_create(1)](http://www.freebsd.org/cgi/man.cgi?query=pkg_create&sektion=1&manpath=FreeBSD+8.0-RELEASE) 工具中的一个错误，该错误导致参数 `-n` 无法正常工作。

[7.2R] FreeBSD Ports 现在支持在某些支持的 Port 中使用多个 [make(1)](http://www.freebsd.org/cgi/man.cgi?query=make&sektion=1&manpath=FreeBSD+8.0-RELEASE) 作业。当 Port 被标记为 `MAKE_JOBS_SAFE` 时，该功能会自动启用，并通过将 `-jX` 选项传递给供应商的顶层 Makefile 来提高构建阶段的 CPU 利用率。默认情况下，*X* 的值设置为 CPU 的数量，用户可以通过 [make(1)](http://www.freebsd.org/cgi/man.cgi?query=make&sektion=1&manpath=FreeBSD+8.0-RELEASE) 变量 `MAKE_JOBS_NUMBER` 来设置。更多详情，请参见 **ports/Mk/bsd.port.mk**。
...
[amd64, i386] 使用 [freebsd-update(8)](http://www.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&manpath=FreeBSD+8.0-RELEASE) 工具支持在 RELEASE 版本之间（以及各种安全分支的快照）升级。二进制升级过程将更新未修改的用户空间工具以及未修改的 **GENERIC**/SMP 内核，这些内核作为官方 FreeBSD 发行版的一部分进行分发。[freebsd-update(8)](http://www.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&manpath=FreeBSD+8.0-RELEASE) 工具要求被升级的主机具有互联网连接。
