...
[ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+5.2.1-RELEASE) 现在可以修改 [ipfw(4)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=4&manpath=FreeBSD+5.2.1-RELEASE) 中 `set 31` 的规则，该规则集为只读并用于默认规则。可以通过 `ipfw delete set 31` 命令删除它们，但不会通过 `ipfw flush` 命令删除。这实现了一种灵活的“持久规则”形式。更多细节可以在 [ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+5.2.1-RELEASE) 中找到。[已合并]
...
[**savecore(8)**](http://www.freebsd.org/cgi/man.cgi?query=savecore&sektion=8&manpath=FreeBSD+5.2.1-RELEASE) 现在支持 `-C` 参数，仅指示 coredump 文件的存在或不存在。
