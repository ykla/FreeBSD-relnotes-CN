...
（2004 年 1 月 10 日，2004 年 2 月 28 日更新）FreeBSD 中的 TCP 实现现在包含对某类 TCP MSS 资源耗尽攻击的防护，形式为对 TCP 段大小和速率的限制。第一个限制设置了允许的最小 TCP 最大段大小，由 `net.inet.tcp.minmss` sysctl 变量控制（默认值为 `216` 字节）。第二个限制由 `net.inet.tcp.minmssoverload` 变量设置，控制平均段大小小于 `net.inet.tcp.minmss` 的连接的最大速率。超过此数据包速率的连接将被重置并丢弃。由于此功能在 5.2-RELEASE 发布周期后期才添加，连接速率限制默认禁用，但可通过向 `net.inet.tcp.minmssoverload` 分配非零值手动启用。此功能添加到 FreeBSD 5.2-RELEASE 的时间太晚，未及纳入其发行说明。
