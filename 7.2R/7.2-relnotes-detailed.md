...
[fsck(8)](http://www.freebsd.org/cgi/man.cgi?query=fsck&sektion=8&manpath=FreeBSD+7.2-RELEASE) 工具现在支持 `-C` 参数，用于检查文件系统是否正常卸载。如果文件系统干净卸载，则跳过文件系统检查；否则，执行全面检查。
...
[make(1)](http://www.freebsd.org/cgi/man.cgi?query=make&sektion=1&manpath=FreeBSD+7.2-RELEASE) 工具现在支持 `-Q` 参数，除了与 `-q` 参数相同的效果外，还会在指定 `-j` 选项时不生成文件横幅。
