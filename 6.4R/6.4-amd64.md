# FreeBSD 6.4-RELEASE amd64 发行说明（2008 年 11 月 28 日）
...
本节介绍了自 6.3-RELEASE 以来，FreeBSD 中对用户最可见的新功能或变更内容。
...
FreeBSD 现在支持通过 BIOS 从带有 GPT 标签的磁盘启动。
...
[moused(8)](http://www.freebsd.org/cgi/man.cgi?query=moused&sektion=8&manpath=FreeBSD+6.4-RELEASE) 工具现在支持参数 `-L`，用于改变滚动速度，并改变了 `-U` 选项的行为，使其仅影响滚动阈值。
...
新增 `rfcomm_pppd_server` [rc(8)](http://www.freebsd.org/cgi/man.cgi?query=rc&sektion=8&manpath=FreeBSD+6.4-RELEASE) 脚本，该脚本允许在启动时以服务器模式启动 [rfcomm_pppd(8)](http://www.freebsd.org/cgi/man.cgi?query=rfcomm_pppd&sektion=8&manpath=FreeBSD+6.4-RELEASE)。可以同时启动多个配置文件。更多信息请参见 [rc.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5&manpath=FreeBSD+6.4-RELEASE)。
...
[rc.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5&manpath=FreeBSD+6.4-RELEASE) 现在支持在 `firewall_type` 为 `simple` 或 `client` 时，通过 `rc.firewall` 为防火墙规则集配置接口和附加网络。请参阅 `firewall_client_net`、`firewall_simple_iif`、`firewall_simple_inet`、`firewall_simple_oif` 和 `firewall_simple_onet`。
...
`libarchive` 实际上更新到 2.5.4b。请注意，内部版本号仍为 1.9.25，因为保留了 API/ABI 兼容性。
