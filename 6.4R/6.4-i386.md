# FreeBSD 6.4-RELEASE i386 发行说明（2008 年 11 月 28 日）

- 原文链接：[FreeBSD/i386 6.4-RELEASE Release Notes](https://www.freebsd.org/releases/6.4R/relnotes-i386/)
...
本文档包含 FreeBSD 6.4-RELEASE 在 i386 硬件平台上的发行说明，介绍了 FreeBSD 最近添加、变更或删除的功能，并提供了从早期版本升级到 FreeBSD 的一些注意事项。

本次发行的 FreeBSD 6.4-RELEASE 属于 RELEASE 发行版，可在 [ftp://ftp.FreeBSD.org/](ftp://ftp.freebsd.org/) 或其镜像站点上找到。有关获取此（或其他）FreeBSD 发行版的更多信息，请参阅 [FreeBSD 手册](http://www.freebsd.org/doc/en_US.ISO8859-1/books/handbook/) 中的 [“获取 FreeBSD”附录](http://www.freebsd.org/doc/en_US.ISO8859-1/books/handbook/mirrors.html)。

所有用户在安装 FreeBSD 之前，建议先查阅发行勘误文档。该文档包含发布周期后期或发布后发现的最新信息，通常包括已知的错误、安全公告以及文档更正。可以在 FreeBSD 网站上找到 FreeBSD 6.4-RELEASE 的最新勘误文档副本。

## 2 新增内容

本节介绍了自 6.3-RELEASE 以来，FreeBSD 中用户最直观的新功能或变更内容。

典型的发行说明条目包括：在 6.3-RELEASE 之后发布的最新安全公告、新的驱动程序或硬件支持、新命令或选项、重大错误修复或第三方软件的升级。此外，还可能列出主要 Ports/软件包或发布工程实践的变更。显然，发行说明无法列出 FreeBSD 在两次发布之间的每一处变更；本文档主要聚焦于安全公告、用户可见的变更和主要架构改进。

### 2.1 安全公告

以下安全公告中介绍的问题修复完成。更多信息请参阅 [http://security.FreeBSD.org/](http://security.freebsd.org/) 提供的各个公告。

| 公告 | 日期 | 主题 |
| :--- | :--- | :--- |
| [SA-08:03.sendfile](http://security.freebsd.org/advisories/FreeBSD-SA-08:03.sendfile.asc) | 2008 年 2 月 14 日 | [sendfile(2)](http://www.freebsd.org/cgi/man.cgi?query=sendfile&sektion=2&manpath=FreeBSD+6.4-RELEASE) 只写文件权限绕过 |
...
FreeBSD 内核的 [malloc(9)](http://www.freebsd.org/cgi/man.cgi?query=malloc&sektion=9&manpath=FreeBSD+6.4-RELEASE) 现在支持缓冲区损坏保护 (RedZone)。此功能可在运行时于 [free(9)](http://www.freebsd.org/cgi/man.cgi?query=free&sektion=9&manpath=FreeBSD+6.4-RELEASE) 和 [realloc(9)](http://www.freebsd.org/cgi/man.cgi?query=realloc&sektion=9&manpath=FreeBSD+6.4-RELEASE) 上检测缓冲区溢出和缓冲区下溢错误，并打印内存分配和释放位置的调用栈。详细信息请参阅内核选项 `DEBUG_REDZONE`。

FreeBSD 内核中实现了 [rpc.lockd(8)](http://www.freebsd.org/cgi/man.cgi?query=rpc.lockd&sektion=8&manpath=FreeBSD+6.4-RELEASE) 的客户端功能。此实现提供了适用于 [flock(2)](http://www.freebsd.org/cgi/man.cgi?query=flock&sektion=2&manpath=FreeBSD+6.4-RELEASE) 风格锁的正确语义，这些锁由命令行工具 [lockf(1)](http://www.freebsd.org/cgi/man.cgi?query=lockf&sektion=1&manpath=FreeBSD+6.4-RELEASE) 和库 [pidfile(3)](http://www.freebsd.org/cgi/man.cgi?query=pidfile&sektion=3&manpath=FreeBSD+6.4-RELEASE) 使用。此功能还实现了从服务器重启后的恢复，并确保在获取锁之前将脏缓存块写入服务器（允许多个客户端通过文件锁定安全地共享数据）。此外，新增内核选项 `options NFSLOCKD` 并默认启用。如果启用了该内核支持，[rpc.lockd(8)](http://www.freebsd.org/cgi/man.cgi?query=rpc.lockd&sektion=8&manpath=FreeBSD+6.4-RELEASE) 会自动检测并使用此功能。

#### 2.2.1 引导加载程序变更

FreeBSD 现在支持从 BIOS 启动使用 GPT 标签的磁盘。

引导加载程序使用的 BTX 内核变更为从实模式调用 BIOS 例程。此变更使 FreeBSD 可以从 USB 设备启动。
...
[dummynet(4)](http://www.freebsd.org/cgi/man.cgi?query=dummynet&sektion=4&manpath=FreeBSD+6.4-RELEASE) 子系统现在支持快速模式操作，该模式允许某些数据包绕过 dummynet 调度器。这在数据包流量低于管道带宽时可以实现更低的延迟和开销，同时消除子系统中的递归。新增的 sysctl 变量 `net.inet.ip.dummynet.io_fast` 可用于启用此功能。
...
[aac(4)](http://www.freebsd.org/cgi/man.cgi?query=aac&sektion=4&manpath=FreeBSD+6.4-RELEASE) 驱动现在为大于 2TB 的 RAID 阵列提供 64 位数组支持，并允许同时打开设备以向控制器发送命令。
...
[iir(4)](http://www.freebsd.org/cgi/man.cgi?query=iir&sektion=4&manpath=FreeBSD+6.4-RELEASE) 驱动的稳定性有所改进。
...
[cron(8)](http://www.freebsd.org/cgi/man.cgi?query=cron&sektion=8&manpath=FreeBSD+6.4-RELEASE) 工具现在支持参数 `-m`，用于覆盖 cron 邮件的默认收件人，除非在 `crontab` 文件中通过 `MAILTO=` 行明确指定。

[cvs(1)](http://www.freebsd.org/cgi/man.cgi?query=cvs&sektion=1&manpath=FreeBSD+6.4-RELEASE) 现在支持参数 `-n`，与参数 `-N` 的作用相反。
...
修复了 [morse(6)](http://www.freebsd.org/cgi/man.cgi?query=morse&sektion=6&manpath=FreeBSD+6.4-RELEASE) 程序输出语法错误的问题。
...
[moused(8)](http://www.freebsd.org/cgi/man.cgi?query=moused&sektion=8&manpath=FreeBSD+6.4-RELEASE) 工具现在支持参数 `-L`，用于变更滚动速度，并变更了 `-U` 选项的行为，使其仅影响滚动阈值。

[mv(1)](http://www.freebsd.org/cgi/man.cgi?query=mv&sektion=1&manpath=FreeBSD+6.4-RELEASE) 现在在跨设备将目录移动到现有目录时符合 POSIX 规范。

[periodic(8)](http://www.freebsd.org/cgi/man.cgi?query=periodic&sektion=8&manpath=FreeBSD+6.4-RELEASE) 现在支持在 [periodic.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=periodic.conf&sektion=5&manpath=FreeBSD+6.4-RELEASE) 中配置变量 `daily_status_mail_rejects_shorten`。该变量允许被拒绝的邮件报告按黑名单汇总拒绝情况，而不提供单个发送主机的详细信息。默认配置保留原始形式的报告。
...
修复了 [telnetd(8)](http://www.freebsd.org/cgi/man.cgi?query=telnetd&sektion=8&manpath=FreeBSD+6.4-RELEASE) 工具中的错误，该错误导致在指定 `-a all` 选项时仍尝试进行身份验证。

[top(1)](http://www.freebsd.org/cgi/man.cgi?query=top&sektion=1&manpath=FreeBSD+6.4-RELEASE) 和 [vmstat(8)](http://www.freebsd.org/cgi/man.cgi?query=vmstat&sektion=8&manpath=FreeBSD+6.4-RELEASE) 现在支持参数 `-P`，用于显示每个 CPU 的统计信息。
...
移除 `ike` [rc(8)](http://www.freebsd.org/cgi/man.cgi?query=rc&sektion=8&manpath=FreeBSD+6.4-RELEASE) 脚本。
...
`ppp` [rc(8)](http://www.freebsd.org/cgi/man.cgi?query=rc&sektion=8&manpath=FreeBSD+6.4-RELEASE) 脚本现在支持多个实例。更多信息请参见 [rc.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5&manpath=FreeBSD+6.4-RELEASE) 中变量 `ppp_profile` 的介绍。

新增 `rfcomm_pppd_server` [rc(8)](http://www.freebsd.org/cgi/man.cgi?query=rc&sektion=8&manpath=FreeBSD+6.4-RELEASE) 脚本，该脚本允许在启动时以服务器模式启动 [rfcomm_pppd(8)](http://www.freebsd.org/cgi/man.cgi?query=rfcomm_pppd&sektion=8&manpath=FreeBSD+6.4-RELEASE)。多个配置文件可以同时启动。更多信息请参见 [rc.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5&manpath=FreeBSD+6.4-RELEASE)。

`sysctl` [rc(8)](http://www.freebsd.org/cgi/man.cgi?query=rc&sektion=8&manpath=FreeBSD+6.4-RELEASE) 脚本现在除了加载 **/etc/sysctl.conf** 外，还支持加载 **/etc/sysctl.conf.local**。

[rc.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5&manpath=FreeBSD+6.4-RELEASE) 现在支持在 `firewall_type` 为 `simple` 或 `client` 时，通过 `rc.firewall` 配置接口和附加网络的防火墙规则集。请参阅 `firewall_client_net`、`firewall_simple_iif`、`firewall_simple_inet`、`firewall_simple_oif` 和 `firewall_simple_onet`。

### 2.4 第三方

**am-utils** 更新到版本 6.1.5。

**ISC BIND** 更新到版本 9.3.5-P2。

**bzip2** 从 1.0.4 更新到 1.0.5。

**NTP** 更新到版本 4.2.4p5。

**FILE** 更新到版本 4.21。

`libarchive` 虚拟更新到 2.5.4b。请注意，内部版本号仍为 1.9.25，因为 API/ABI 兼容性得到保留。

**ncurses** 库更新到版本 5.6-20080503。

**OpenPAM** 更新到 Hydrangea 发行版。

**sendmail** 从 8.14.2 更新到 8.14.3。

**tcsh** 更新到版本 6.15.00。

时区数据库从 **tzdata2007k** 发行版更新到 **tzdata2008e** 发行版。

### 2.5 Ports/软件包

`pkg_install` 工具升级到 2008 年 5 月 30 日的 8.0-CURRENT 快照。

移除了 [pkg_sign(1)](http://www.freebsd.org/cgi/man.cgi?query=pkg_sign&sektion=1&manpath=FreeBSD+6.4-RELEASE) 和 [pkg_check(1)](http://www.freebsd.org/cgi/man.cgi?query=pkg_check&sektion=1&manpath=FreeBSD+6.4-RELEASE) 工具。

### 2.6 发布工程与集成

**GNOME** 桌面环境的受支持版本 ([x11/gnome2](http://www.freebsd.org/cgi/url.cgi?ports/x11/gnome2/pkg-descr)) 从 2.20.1 更新到 2.22.3。

**KDE** 桌面环境的受支持版本 ([x11/kde3](http://www.freebsd.org/cgi/url.cgi?ports/x11/kde3/pkg-descr)) 从 3.5.8 更新到 3.5.10。
