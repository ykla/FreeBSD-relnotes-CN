# FreeBSD 6.4-RELEASE sparc64 发行说明（2008 年 11 月 28 日）

- 原文链接：[FreeBSD/sparc64 6.4-RELEASE Release Notes](https://www.freebsd.org/releases/6.4R/relnotes-sparc64/)
...
FreeBSD 6.4-RELEASE 的发行说明包含对 FreeBSD 基本系统在 6.4-STABLE 开发线上所做变更的概要说明。本文件列出了自上次发布以来公布的相关安全公告，以及对 FreeBSD 内核和用户空间的重大变更，同时还对升级过程提供了一些简要说明。

## 1 引言

本文档包含 FreeBSD 6.4-RELEASE 在 UltraSPARC 硬件平台上的发行说明，介绍了 FreeBSD 最近添加、变更或删除的功能，并提供了从早期版本升级到 FreeBSD 的一些注意事项。
...
本节介绍了自 6.3-RELEASE 以来，FreeBSD 中对用户最明显的新功能或变更内容。
...
[dummynet(4)](http://www.freebsd.org/cgi/man.cgi?query=dummynet&sektion=4&manpath=FreeBSD+6.4-RELEASE) 子系统现在支持 `fast` 模式操作，该模式允许某些数据包绕过 dummynet 调度器。这在数据包流量低于管道带宽时可以实现更低的延迟和开销，同时消除子系统中的递归。新增的 sysctl 变量 `net.inet.ip.dummynet.io_fast` 可用于启用此功能。
...
修复了 [morse(6)](http://www.freebsd.org/cgi/man.cgi?query=morse&sektion=6&manpath=FreeBSD+6.4-RELEASE) 程序中的语法错误。
...
[moused(8)](http://www.freebsd.org/cgi/man.cgi?query=moused&sektion=8&manpath=FreeBSD+6.4-RELEASE) 工具现在支持参数 `-L`，用于变更滚动速度，并变更 `-U` 选项的行为，使其仅影响滚动阈值。

跨设备将目录移动到现有目录时，[mv(1)](http://www.freebsd.org/cgi/man.cgi?query=mv&sektion=1&manpath=FreeBSD+6.4-RELEASE) 现在符合 POSIX 规范。
...
修复了 [telnetd(8)](http://www.freebsd.org/cgi/man.cgi?query=telnetd&sektion=8&manpath=FreeBSD+6.4-RELEASE) 工具中的一处错误，该错误导致在指定 `-a all` 选项时仍尝试进行身份验证。
...
`libarchive` 虚拟更新到 2.5.4b。请注意，内部版本号仍为 1.9.25，因为 API/ABI 兼容性得以保留。
