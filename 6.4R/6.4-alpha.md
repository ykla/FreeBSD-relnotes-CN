# FreeBSD 6.4-RELEASE alpha 发行说明（2008 年 11 月 28 日）

- 原文链接：[FreeBSD/alpha 6.4-RELEASE Release Notes](https://www.freebsd.org/releases/6.4R/relnotes-alpha/)
...
本文档包含 FreeBSD 6.4-RELEASE 在 Alpha/AXP 硬件平台上的发行说明，介绍了 FreeBSD 最近添加、变更或删除的功能，并提供了从早期版本升级到 FreeBSD 的一些注意事项。
...
以下安全公告中描述的问题修复完成。更多信息请参阅 [http://security.FreeBSD.org/](http://security.freebsd.org/) 提供的各个公告。
...
FreeBSD 内核的 [malloc(9)](http://www.freebsd.org/cgi/man.cgi?query=malloc&sektion=9&manpath=FreeBSD+6.4-RELEASE) 现在支持缓冲区损坏保护 (RedZone)。此功能可在运行时于 [free(9)](http://www.freebsd.org/cgi/man.cgi?query=free&sektion=9&manpath=FreeBSD+6.4-RELEASE) 和 [realloc(9)](http://www.freebsd.org/cgi/man.cgi?query=realloc&sektion=9&manpath=FreeBSD+6.4-RELEASE) 上检测缓冲区下溢和缓冲区溢出错误，并打印内存分配和释放位置的调用栈。详细信息请参阅内核选项 `DEBUG_REDZONE`。

FreeBSD 内核中实现了 [rpc.lockd(8)](http://www.freebsd.org/cgi/man.cgi?query=rpc.lockd&sektion=8&manpath=FreeBSD+6.4-RELEASE) 的客户端功能。此实现提供了适用于 [flock(2)](http://www.freebsd.org/cgi/man.cgi?query=flock&sektion=2&manpath=FreeBSD+6.4-RELEASE) 风格锁的正确语义，这些锁由命令行工具 [lockf(1)](http://www.freebsd.org/cgi/man.cgi?query=lockf&sektion=1&manpath=FreeBSD+6.4-RELEASE) 和库 [pidfile(3)](http://www.freebsd.org/cgi/man.cgi?query=pidfile&sektion=3&manpath=FreeBSD+6.4-RELEASE) 使用。此功能还实现了服务器重启后的恢复，并确保在获取锁之前将脏缓存块写入服务器（允许多个客户端通过文件锁定安全地共享数据）。此外，新增了内核选项 `NFSLOCKD`，默认启用。如果启用了该内核支持，[rpc.lockd(8)](http://www.freebsd.org/cgi/man.cgi?query=rpc.lockd&sektion=8&manpath=FreeBSD+6.4-RELEASE) 会自动检测并使用此功能。

#### 2.2.1 引导加载程序变更

FreeBSD 现在支持通过 BIOS 从使用 GPT 标签的磁盘启动。
...
[cron(8)](http://www.freebsd.org/cgi/man.cgi?query=cron&sektion=8&manpath=FreeBSD+6.4-RELEASE) 工具现在支持 `-m` 选项，用于覆盖 cron 邮件的默认收件人，除非在 `crontab` 文件中通过 `MAILTO=` 行明确指定。

[cvs(1)](http://www.freebsd.org/cgi/man.cgi?query=cvs&sektion=1&manpath=FreeBSD+6.4-RELEASE) 现在支持 `-n` 选项，与 `-N` 选项的作用相反。
...
修复了 [morse(6)](http://www.freebsd.org/cgi/man.cgi?query=morse&sektion=6&manpath=FreeBSD+6.4-RELEASE) 程序输出中的语法错误。
...
[moused(8)](http://www.freebsd.org/cgi/man.cgi?query=moused&sektion=8&manpath=FreeBSD+6.4-RELEASE) 工具现在支持 `-L` 选项，用于变更滚动速度，并变更了 `-U` 选项的行为，使其仅影响滚动阈值。

[mv(1)](http://www.freebsd.org/cgi/man.cgi?query=mv&sektion=1&manpath=FreeBSD+6.4-RELEASE) 现在在跨设备将目录移动到现有目录时遵循 POSIX 规范。
...
[top(1)](http://www.freebsd.org/cgi/man.cgi?query=top&sektion=1&manpath=FreeBSD+6.4-RELEASE) 和 [vmstat(8)](http://www.freebsd.org/cgi/man.cgi?query=vmstat&sektion=8&manpath=FreeBSD+6.4-RELEASE) 现在支持 `-P` 选项，用于显示每个 CPU 的统计信息。

[watch(8)](http://www.freebsd.org/cgi/man.cgi?query=watch&sektion=8&manpath=FreeBSD+6.4-RELEASE) 工具现在支持同时使用超过 10 个 [snp(4)](http://www.freebsd.org/cgi/man.cgi?query=snp&sektion=4&manpath=FreeBSD+6.4-RELEASE) 设备。

#### 2.3.1 /etc/rc.d 脚本

移除了 `ike` [rc(8)](http://www.freebsd.org/cgi/man.cgi?query=rc&sektion=8&manpath=FreeBSD+6.4-RELEASE) 脚本。
...
新增 `rfcomm_pppd_server` [rc(8)](http://www.freebsd.org/cgi/man.cgi?query=rc&sektion=8&manpath=FreeBSD+6.4-RELEASE) 脚本，该脚本允许在系统启动时以服务器模式运行 [rfcomm_pppd(8)](http://www.freebsd.org/cgi/man.cgi?query=rfcomm_pppd&sektion=8&manpath=FreeBSD+6.4-RELEASE)。可同时启动多个配置文件。更多信息请参见 [rc.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5&manpath=FreeBSD+6.4-RELEASE)。
...
`libarchive` 实质上更新到 2.5.4b。请注意，内部版本号仍为 1.9.25，因为保留了 API/ABI 兼容性。
