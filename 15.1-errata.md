...
1. 当 NFSv4.1 或 NFSv4.2 服务器无响应或重启时，[nfs(4)](https://man.freebsd.org/cgi/man.cgi?query=nfs&sektion=4&format=html) 客户端可能导致内核崩溃。崩溃后系统会自动重启。
   ...
   **状态**：已在 main 分支修复 - https://bugs.freebsd.org/294925

2. 启用 `zfskeys` [rc(8)](https://man.freebsd.org/cgi/man.cgi?query=rc&sektion=8&format=html) 脚本时，使用 `keylocation=prompt` 数据集的 [zfs(4)](https://man.freebsd.org/cgi/man.cgi?query=zfs&sektion=4&format=html) 加密池可能在启动时因等待密码短语而卡住，这与此前跳过挂载的行为不同。
   **临时解决方案**：使用 `zpool set cachefile=none <pool>` 使该池在启动时不被导入。
   **状态**：未决 - https://bugs.freebsd.org/296130
