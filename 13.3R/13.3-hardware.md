# FreeBSD 13.3-RELEASE 硬件兼容列表

## 引言

本文档包含 FreeBSD 13.3 的硬件兼容性说明。其中列出了 FreeBSD 支持的硬件平台，以及各类硬件设备（存储控制器、网络接口等），并给出已知可正常工作的设备实例。
...
[isci(4)](https://man.freebsd.org/cgi/man.cgi?query=isci&sektion=4&format=html) 驱动提供对 Intel C600 `SAS` 控制器的支持。
...
注意，此硬件仅对 ATA DMA 命令支持命令队列和基于 FIS 的切换。ATAPI 和非 DMA ATA 命令逐端口依次执行。
...
对于所有受支持的 SCSI 控制器，全面支持 SCSI-I、SCSI-II 和 SCSI-III 外设，包括硬盘、光盘、磁带机（包括 DAT、8mm Exabyte、Mammoth 和 DLT）、介质更换器、处理器目标设备和 CD-ROM 驱动器。支持 CD-ROM 命令的 WORM 设备可由 CD-ROM 驱动程序（如 [cd(4)](https://man.freebsd.org/cgi/man.cgi?query=cd&sektion=4&format=html)）进行只读访问。WORM/CD-R/CD-RW 写入支持由 [cdrecord(1)](https://man.freebsd.org/cgi/man.cgi?query=cdrecord&sektion=1&format=html) 提供，它是 Ports 中 `sysutils/cdrtools` port 的一部分。
...
当前版本的 `atse` 驱动在设备配置为内部 FIFO 时，支持 Altera 文档版本 11.1 中所述的以太网 MegaCore。
...
`genet` 驱动支持 树莓派 4 Model B 及相关系统上 Broadcom BCM2711 的以太网控制器部分。它使用 BCM54213PE PHY。
...
（参见 **/sys/dev/usb/serial/u3g.c** 以获取上述各厂商受支持卡片的完整列表。）
...
`upgt` 驱动支持基于 GW3887 芯片组的 USB 2.0 Conexant/Intersil PrismGT 系列无线适配器，包括：
...
[amd64、i386] FreeBSD 支持多种 USB 外设；已知可正常工作的设备在本节中列出。由于大多数 USB 设备具有通用性，除少数例外，任何给定类别的设备都会得到支持，即使在此处未明确列出。

> **注意**：
>
> [amd64、i386] USB 以太网适配器可在列出[以太网接口](https://www.freebsd.org/releases/13.3R/hardware.html#ethernet)的章节中找到。

> **注意**：
>
> [amd64、i386] USB 蓝牙适配器可在[蓝牙](https://www.freebsd.org/releases/13.3R/hardware.html#bluetooth)章节中找到。
...
`rtsx` 驱动支持兼容不同规范的芯片。以下芯片已经过验证可正常工作：
...
`sdhci` 驱动支持兼容不同规范的芯片。以下芯片已经过验证可正常工作：
