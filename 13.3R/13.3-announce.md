# FreeBSD 13.3-RELEASE 发行公告

日期：2024 年 3 月 5 日

FreeBSD 发布工程团队很高兴地宣布 FreeBSD 13.3-RELEASE 现已发布。这是 stable/13 分支的第四个版本。

部分亮点如下：

* LLVM 和 clang 编译器升级至 17.0.6 版本。
* OpenSSH 升级至 9.6p1 版本。
* Sendmail 升级至 8.18.1 版本。
* ZFS 升级至 OpenZFS 2.1.14。
* 为原生和基于 LinuxKPI 的 WiFi 驱动程序修复了许多稳定性问题。
...
FreeBSD 项目将 FreeBSD 13.3-RELEASE 题献给 Glen Barber，感谢他多年来作为发布工程师所作的贡献。

## 获取途径

FreeBSD 13.3-RELEASE 现已可用于 amd64、i386、powerpc、powerpc64、powerpc64le、powerpcspe、armv6、armv7、aarch64 和 riscv64 架构。

FreeBSD 13.3-RELEASE 可从可启动 ISO 镜像或通过网络安装。部分架构还支持从 USB 闪存盘安装。所需文件可按以下说明下载。
...
FreeBSD 13.3-RELEASE 也可从多家供应商购买 DVD。提供基于 FreeBSD 13.3 产品的供应商之一如下：

* FreeBSD Mall, Inc. <https://www.freebsdmall.com>

预安装的虚拟机镜像也可用于 amd64（x86_64）、i386（x86_32）、AArch64（arm64）和 RISCV（riscv64）架构，提供 `QCOW2`、`VHD` 和 `VMDK` 磁盘镜像格式，以及原始（未格式化）镜像。

FreeBSD 13.3-RELEASE amd64 还可在以下云托管平台上使用：
...
FreeBSD 13.3-RELEASE 预计也将在 Google Compute Engine(TM) Marketplace 上提供，待其完成第三方特定验证后可访问：<https://console.cloud.google.com/marketplace/browse?filter=category:os&filter=price:free&q=freebsd>

* Microsoft Azure(TM)：
FreeBSD 虚拟机镜像将在完成发布认证后于 Azure Marketplace 上提供。
...
FreeBSD 13.3-RELEASE 可通过 https 从以下站点下载：

* <https://download.freebsd.org/ftp/releases/ISO-IMAGES/13.3/>

FreeBSD 13.3-RELEASE 虚拟机镜像可从以下地址下载：

* <https://download.freebsd.org/ftp/releases/VM-IMAGES/13.3-RELEASE/>

FreeBSD 13.3-RELEASE amd64 BASIC-CI 镜像可从以下地址下载：

* <https://download.freebsd.org/ftp/releases/CI-IMAGES/13.3-RELEASE/>

有关安装 FreeBSD 或将现有机器更新至 13.3-RELEASE 的说明，请参阅：

* <https://www.FreeBSD.org/releases/13.3R/installation/>

## 支持

基于新的 FreeBSD 支持模型，FreeBSD 13 发行系列将至少支持至 2026 年 1 月 31 日。此版本 FreeBSD 13.3-RELEASE 将至少支持至 FreeBSD 13.4-RELEASE 发布后三个月。更多支持信息可在以下地址找到：

* <https://www.FreeBSD.org/security/>

请注意，13.2 将支持至 13.3 发布日期起三个月，即 2024 年 6 月 30 日。

## 致谢

许多公司捐赠了设备、网络访问或人力投入，以支持 FreeBSD 13.3 的发布工程活动，包括：
...
| Mike Karels <karels@FreeBSD.org> | 发布工程副负责人 |
| Xin Li <delphij@FreeBSD.org> | 发布工程、安全团队联络人 |
| Ed Maste <emaste@FreeBSD.org> | 安全官副负责人 |
| Colin Percival <cperciva@FreeBSD.org> | 发布工程负责人、13.3-RELEASE 发布工程师 |
...
### 13.3-RELEASE powerpc64 GENERIC64:
...
### 13.3-RELEASE powerpc64le GENERIC64LE:
...
### 13.3-RELEASE powerpcspe MPC85XXSPE:
...
### 13.3-RELEASE aarch64 PINE64-LTS:
...
### 13.3-RELEASE aarch64 ROCKPRO64:
...
### 13.3-RELEASE riscv64 GENERICSD:
