...
有关 FreeBSD 及 3.1-RELEASE 目录布局的信息（特别是如果你是从软盘安装！），请参阅 `ABOUT.TXT` 文件。有关安装说明，请参阅文件 `INSTALL.TXT` 和 `HARDWARE.TXT`。
...
- 系统控制台驱动程序（sc0）现在拆分为独立模块，部分为支持 USB 键盘、鼠标等做准备。
- 增加了对 Philips I2C/SMBUS 的支持。
- 初步支持 USB 设备（一些键盘、鼠标）。
- `lpt` 驱动程序现已弃用。请改用 `ppbus` 配合 `ppc` 驱动程序（详细信息请参阅 **ppc(4)** 和 **ppbus(4)** 手册页）。在下一个版本之前，将移除旧的 `lpt` 驱动程序，并将 `nlpt` 驱动程序重命名为 `lpt`。

### 1.2. 安全修复

详细信息请参阅 **ftp://ftp.FreeBSD.org/pub/FreeBSD/CERT/**：

- CERT 通告 CA-98-13-tcp-denial-of-service 问题关闭。
- FreeBSD-SA-98:08（IP 分片拒绝服务）问题关闭。
...
- 更新了 `AMD`、`texinfo`、`global` 等其他各种实用工具。

## 2. 支持的配置

FreeBSD 目前支持多种基于 ISA、VLB、EISA 和 PCI 总线的 PC，范围从 386sx 到 Pentium 类机器（虽然不推荐使用 386sx）。还支持通用 IDE 或 ESDI 驱动配置、各种 SCSI 控制器、网络和串行卡。
...
- DPT SmartCACHE Plus、SmartCACHE III、SmartRAID III、SmartCACHE IV 和 SmartRAID IV SCSI/RAID 控制器受支持。DPT SmartRAID/CACHE V 尚不支持。
...
对所有支持的 SCSI 控制器提供完全支持，包括 SCSI-I 和 SCSI-II 外设，如硬盘、光盘、磁带驱动器（包括 DAT 和 8mm Exabyte）、介质交换器、处理器目标设备和 CDROM 驱动器。CDROM 驱动程序可对支持 CDROM 命令的 WORM 设备进行只读访问。WORM/CD-R/CD-RW 写入支持由 `cdrecord` 提供，且该工具可以在 Ports 树中找到。
...
[注：目前正在将 AIC-6260/6360 和 UltraStor 驱动移植到新的 CAM SCSI 框架，但无法估计完成时间，也无法保证能否完成]
...
有关镜像 FreeBSD 软件的其他位置，请参阅文件 `MIRROR.SITES`。请从网络距离最近的站点下载分发版。欢迎提供更多的镜像站点！如果你希望成为官方镜像站点，请联系 `freebsd-admin@FreeBSD.org` 获取更多详情。

如果你无法访问互联网，且电子邮件是唯一选择，你仍然可以通过发送邮件到 `ftpmail@ftpmail.vix.com` 获取文件。请在邮件正文中写上关键字 `help` 以获取有关如何通过此机制获取文件的更多信息。然而，请注意，这将通过邮件发送 *大量* 数据，应仅作为最后手段使用！
...
在美国、加拿大和墨西哥，运费为每订单 $5（而非每光盘）；海外运费为 $9.00。他们接受 Visa、Mastercard、Discover、American Express 或美国美元支票，并在美国境内提供货到付款服务。加利福尼亚州居民需加收 8.25% 的销售税。

如果你因任何原因不满意，光盘提供无条件退货政策。

## 4. 从旧版 FreeBSD 升级

如果你正在从之前的 FreeBSD 版本升级，最有可能是从 2.2.x/2.1.x 版本升级（在某些较少的情况下），以下一些问题可能会影响你，当然，具体情况还取决于你选择的升级方式。有两种常见的 FreeBSD 升级方式：

- 使用源代码，通过 **/usr/src**
- 还可使用 `sysinstall` 的（二进制）升级选项。

使用源代码时，你需要了解两个目标：标准的 `upgrade` 目标，它将把 2.x 或 3.0 系统升级到 3.1，以及 `world` 目标，它将保持升级后的系统与自初次升级以来的所有变更同步。

使用二进制升级选项时，系统将直接升级到 3.1/ELF，同时为与旧版二进制文件的向后兼容性填充 **/<basepath>/lib/aout** 目录。

无论哪种方式，升级到 ELF 后，你将拥有更小的二进制文件，并能访问更多移植到其他 ELF 环境的编译器工具（我们的旧版且有些过时的 a.out 格式，其他软件项目大多不支持）。那些希望继续使用旧版 a.out 动态可执行文件的人，应确保安装 `compat22` 分发版。
...
从具有互联网邮件连接的计算机提交错误报告的首选方法是使用命令 `send-pr` 或通过访问 [http://www.FreeBSD.org/send-pr.html](http://www.FreeBSD.org/send-pr.html) 使用 CGI 脚本。我们忠实的 `bugfiler` 程序会妥善归档错误报告，你可以放心，我们会尽最大努力尽快回应所有报告的错误。通过这种方式提交的错误报告也会在我们网站的支持部分显示，因此它们既是错误报告，也是其他用户留意潜在问题的“路标”。

如果由于某种原因你无法使用命令 `send-pr` 提交错误报告，你可以尝试将报告发送至：`freebsd-bugs@FreeBSD.org`

请注意，`send-pr` 本身是个 shell 脚本，即使是在完全不同的系统上也应该容易移植。我们更倾向于你使用此界面，因为它可以更容易地跟踪问题报告。但是，在提交之前，请尽量确认该问题是否修复。

否则，若你有任何疑问或技术支持需求，请将邮件发送至：`freebsd-questions@FreeBSD.org`

此外，由于这是一项志愿者项目，我们始终欢迎愿意提供帮助的更多志愿者——目前，期望实现的改进比我们能独立处理的还要多！若要就技术事宜联系我们或愿意提供帮助，请将邮件发送至：`freebsd-hackers@FreeBSD.org`

请注意，这些邮件列表可能会产生 *大量* 流量，如果你有较慢或昂贵的邮件访问，并且仅想关注 FreeBSD 重要事件，你可能更希望订阅以下邮件列表：`freebsd-announce@FreeBSD.org`
...
FreeBSD 凝聚了来自世界各地的数十名甚至数百名个人的共同努力，他们努力为你带来了这个版本。欲了解 FreeBSD 项目工作人员的完整名单，请访问：[http://www.FreeBSD.org/handbook/staff.html](http://www.FreeBSD.org/handbook/staff.html)
...
- Steve Price 在构建和组织软件包方面的工作，以及 Satoshi Asami 在 Ports 方面一如既往的出色工作。
- Doug Rabson 和 John Birrell 实现了 FreeBSD/alpha，NetBSD 项目也提供了重要的间接帮助。
- Peter Wemm 贡献了新的内核模块系统（Doug Rabson 也给予了大力帮助）。
