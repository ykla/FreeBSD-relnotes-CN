...
- ed0（wd8xxxx, 3c503, NE2000, HP Lan+）以太网设备的默认 IRQ 已从 IRQ 5 更改为 IRQ 10。
  ed1 以太网设备已被移除。请使用 Userconfig 工具将 ed0 的值更改为与您的网络卡设置匹配。
  [已已合并：这两个更改也出现在 2.2.6 及以后版本的 2.2-STABLE 分支中]
...
- 引入了新的 if_media 选择方法，来自 Jason Thorpe 为 NetBSD 实现的版本。
  [已已合并：也在 2.2.5 及以后版本的 2.2-STABLE 分支中]
-  多会话 ISO-9660 CD-ROM 现在完全支持。默认情况下，将挂载最后一个会话（包括根目录挂载）。对于非根目录挂载，mount_cd9660(8) 可以接受一个参数来挂载特定的会话，而不是默认的会话。
- UPAGES 已从每个进程的地址空间中移除，这允许通过引用计数实现完全的地址空间和页表共享。
...
- 实现了 SA_NOCLDWAIT 标志，提供了 System V 选项，允许进程表示希望永远不会接收到僵尸进程或 SIGCHLD 信号。
- 实现了 poll(2)，其核心源自 NetBSD 实现。select() 和 poll() 系统调用都使用了 poll 设备、文件和 vnode 操作例程。
- 实现了类似于 OpenBSD 的 issetugid(2) 调用。我们在更多情况下设置该标志，实施更为谨慎。
...
- 实现了基于中断的配置挂钩机制。这允许驱动程序推迟部分配置，直到中断完全启用后再进行。这加速了启动过程，因为避免了对设备探测（例如 SCSI 总线探测）等操作的忙等待。
...
- 新的即插即用（PnP）支持，允许你重新配置 PnP 设备。同时，支持由 PnP 部分检测到的调制解调器并自动连接。[已合并：同样出现在 2.2.6 及之后版本的 2.2-STABLE 分支中]
...
- **fetch(1)** 添加了个新参数，允许与某些处理不当的 HTTP 实现通信，这些实现会因请求消息后立即关闭连接而出错。  
...
您可以通过以下方式订购 FreeBSD 3.0-RELEASE 和 2.2.x-RELEASE 的 CD-ROM：  
...
**价格**  
- 每张 -RELEASE CD 的价格为 $39.95，或通过 FreeBSD 订阅购买仅需 $24.95。  
- FreeBSD SNAPshot CDs（如有供应）价格为 $39.95，或通过 FreeBSD-SNAP 订阅购买仅需 $14.95。  
...
如果您正在从以前的 FreeBSD 版本升级，大多数情况下是从 2.2.x 或较少情况下的 2.1.x 版本升级，根据您选择的升级方法，以下一些问题可能会影响您。目前升级 FreeBSD 发行版的两种流行方法是：  
...
需要注意的是，`send-pr` 本身是一个 shell 脚本，应该可以轻松迁移到其他系统中。我们更希望您通过此界面提交报告，因为这便于我们追踪问题报告。然而，在提交之前，请尽量确认该问题是否已经被修复。  
