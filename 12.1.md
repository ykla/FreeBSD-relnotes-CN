...
FreeBSD 12.1-RELEASE 的发布说明包含了 FreeBSD 12-STABLE 开发线中对基本系统所做更改的总结。本文列出了自上次发布以来发布的相关安全公告，以及对 FreeBSD 内核和用户空间的重大更改。还提供了一些关于升级的简要说明。

## 目录

## 介绍

本文档包含 FreeBSD RELEASE.CURRENT 的发布说明。它描述了 FreeBSD 中最近新增、更改或删除的功能，并提供了一些关于从以前版本升级的说明。
...
所有用户都被鼓励在安装 FreeBSD 之前查阅发布说明中的 errata。errata 文档会在发布周期末期或发布后更新，包含“临时性”的信息，通常涉及已知的 bug、安全公告和文档修正。FreeBSD RELEASE.CURRENT 的最新 errata 版本可以在 FreeBSD 网站上找到。

本文档描述了自 RELEASE.PREV 以来 FreeBSD 中最具用户可见性的新增或更改功能。一般来说，除非特别标明为合并特性，否则此处描述的更改是独特于 RELEASE.BRANCH 分支的。

典型的发布说明项目包括记录自 RELEASE.PREV 后发布的最新安全公告、新驱动程序或硬件支持、新命令或选项、主要 bug 修复或第三方的软件升级。它们还可能列出对主要 Port/包或发布工程实践的更改。显然，发布说明无法列出每一个在发布之间对 FreeBSD 做出的更改；本文档主要聚焦于安全公告、用户可见的更改和主要的架构改进。
...
本节列出了自 RELEASE.PREV 以来的各种安全公告和 Errata 通知。
...
本节涵盖了用户空间应用程序、第三方的软件以及系统工具的更改和新增功能。
...
[sh(1)](https://man.freebsd.org/cgi/man.cgi?query=sh&sektion=1&format=html) 工具已更新，新增了 `pipefail` 选项，启用时会将管道的退出状态更改为管道中任何命令的最后一个非零退出状态。[[r345487]](http://svn.freebsd.org/viewvc/base?view=revision&revision=345487)
...
[nvmecontrol(8)](https://man.freebsd.org/cgi/man.cgi?query=nvmecontrol&sektion=8&format=html) 工具已更新，新增了一个新的子命令 `resv`，用于处理 NVMe 保留。[[r350952]](http://svn.freebsd.org/viewvc/base?view=revision&revision=350952)
...
本节涵盖了内核配置、系统调优和系统控制参数的变化，这些变化不属于其他类别。
...
本节涵盖自 RELEASE.PREV 以来设备和设备驱动程序的变化和新增内容。
...
本节涵盖了本地和网络存储子系统中的文件系统及其他存储的变化和新增内容。
...
本节涵盖了 FreeBSD Ports、包基础设施以及包维护和安装工具的变化。
...
做出此更改时考虑了多个因素。例如，i486 没有 64 位原子操作，尽管可以在内核中进行模拟，但无法在用户空间中模拟。此外，32 位 amd64 库自诞生以来一直是 i686。
