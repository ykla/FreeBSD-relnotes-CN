...
典型的发布说明项目包括记录自 RELEASE.PREV 后发布的最新安全公告、新驱动程序或硬件支持、新命令或选项、主要 bug 修复或贡献的软件升级。它们还可能列出对主要 Port/包或发布工程实践的更改。显然，发布说明无法列出每一个在发布之间对 FreeBSD 做出的更改；本文档主要聚焦于安全公告、用户可见的更改和主要的架构改进。
...
本节涵盖了用户空间应用程序、贡献的软件以及系统工具的更改和新增功能。
