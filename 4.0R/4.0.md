# FreeBSD 4.0-RELEASE 发行说明（2000 年 3 月 14 日）

**发行说明**

**FreeBSD 版本 4.0-RELEASE**

任何安装失败或崩溃问题应使用 send-pr 命令报告（喜欢基于网页的界面的用户也可以访问 `http://www.FreeBSD.org/send-pr.html`）。

有关 FreeBSD 及 4.0-RELEASE 目录布局的信息（特别是如果你是从软盘安装！），请参阅 `ABOUT.TXT`。安装说明请参阅 `INSTALL.TXT` 和 `HARDWARE.TXT` 文件。

要获取这些 4.0-STABLE 快照的最新版本，应始终访问：`ftp://current.FreeBSD.org/pub/FreeBSD`
...
- 现已支持 IA32 硬件调试寄存器。参见 **ptrace(2)** 和 **procfs(5)**。

- 新增了 Linux 模式下感知 **Jail(8)** 的 **sysctl(8)** 变量。

- 虚拟内存（VM）系统进行了大量 bug 修复和性能提升，尤其是 `mmap()` 及相关函数的改进。增加了选项 `MAP_NOSYNC`，更好地支持将共享文件用作 IPC 机制。虚拟内存系统的交换机制完全重写，性能显著提升，特别是在通过 NFS 进行交换时。
...
- 实现了新的 ATA/ATAPI 驱动程序。这个新子系统的目标是最大化现代 ATA/ATAPI 系统的性能。新的 `ata` 驱动程序支持所有主要芯片组，包括用于 PCI 卡控制器的 Promise 和 Abit/SIIG 等。支持总线主控 DMA 传输，最高可达新的 ATA/66 模式。驱动程序会自动为硬件设置最大传输模式，以最大化系统吞吐量。支持的设备包括所有符合 ATA 标准的磁盘、ATAPI CD-ROM、CD-R、CD-RW、DVD-ROM、DVD-RAM、LS120、ZIP 和磁带驱动器。该驱动程序还支持 PCCARD ATA 设备，并具有错误处理和超时代码，以避免 ATA/ATAPI 设备“挂起”问题。

- 编写了新工具 `burncd`，用于方便地控制 ATAPI CD-R 和 CD-RW 驱动器，能以多种格式（包括多会话模式）刻录 CD-R/RW 媒体。
...
- 为基于 Adaptec AIC-6915 快速以太网控制器的 Adaptec Duralink PCI 以太网适配器新增驱动支持。[已合并]

- 为基于 Sundance Technologies ST201 控制器的 PCI 快速以太网适配器新增驱动支持，包括 D-Link DFE-550TX。[已合并]

- 为 3Com 3c905C-TX 新增驱动支持。[已合并]

- 为基于 SMC SMC9xxx 的以太网适配器新增驱动支持。
...
- 为基于 Silicon Integrated Systems SiS 900 和 SiS 7016 以太网控制器的 PCI 快速以太网适配器新增驱动支持。[已合并]

- 为基于 Davicom DM9100 和 DM9102 以太网控制器的 PCI 快速以太网适配器新增驱动支持，包括 Jaton Corporation XpressNet。

- 新增了阻止传入 ICMP 重定向、传出 RST 帧和传入 SYN|FIN 帧的支持，以减少或消除某些类型的 DoS 攻击的影响。[已合并]

- 新增了转发 IP 数据报而不检查或减少 TTL 的支持，以使网关和防火墙不那么明显，从而减少暴露于攻击的风险。[已合并]

- 旧的 `sd`（SCSI 磁盘）向后兼容支持已被移除。任何在 **/etc/fstab** 中使用的 **/dev/sd\*** 必须替换为 **/dev/da\***。此外，任何在脚本中使用 **/dev/\*sd\*** 的地方也需要变更。即使你在 **/dev** 中有旧的 `sd` 设备条目，它们也不再有效。

- 移除了驱动 `al`、`ax`、`dm`、`pn` 和 `mx`，并由单一新驱动 (`dc`) 取代，以减少代码重复。新驱动支持所有旧驱动支持的芯片组，并且提供了对基于 DEC/Intel 21143 的 10/100 卡的改进支持。

- 为 3Com 3c450-TX HomeConnect PCI 以太网网卡新增驱动支持。[已合并]

- 为基于 ADMtek AN986 Pegasus 芯片的 USB 以太网适配器新增驱动支持，包括 LinkSys USB100TX、Billionton USB100、Melco Inc. LU-ATX、D-Link 650TX 和 SMC-2202USB。

- 为基于 Kawasaki LSI KL5KUSB101B 芯片的 USB 以太网适配器新增驱动支持，包括 LinkSys USB10T、Peracom USB Ethernet Adapter、3Com 3c19250、Entrega NET-USB-E45、ADS Technologies USB-10BT、ATen UC10T、Netgear EA101、D-Link DSB-650，以及 SMC 2102USB 和 2104USB。

- 集成了 IPfilter 版本 3.3.8。

- 为基于 CATC USB-EL1210A 芯片的 USB 以太网适配器新增驱动支持，包括 CATC Netmate 和 Netmate II，以及 Belkin F5U111。

- 为 Aironet 4500/4800 802.11 无线适配器新增驱动支持。包括 PCMCIA、PCI 和 ISA 型号。

- IPv6 支持已从 KAME 项目导入。包括内核 IPv6 协议栈（**sys/netinet6**）、TCP IPv6 支持、可配置的 IPv6 和 IPv4 隧道（通过 IPv6 或 IPv4 传输），以及 IPv6 TCP 到 IPv4 TCP 的转换网关支持。向 libc 新增了与协议无关的名称解析函数（getaddrinfo、getnameinfo 等）。

- 新进程的浮点异常（除零、下溢/溢出、无效范围等）现在默认禁用。使用 **fpsetmask(3)** 重新启用你需要的异常。请注意，整数除零不受 FPU 的控制，在此变更后仍会触发异常。还要注意，当浮点数/双精度浮点数转换为整数时，如果浮点数值过大，现在也不会触发异常（它无法与我们希望屏蔽的其他操作分开）。

### 1.2. 安全修复

- 在 FreeBSD 4.0 的开发过程中，应用了大量的安全增强和修复。大多数这些修复也回溯到了 3.X-STABLE 系列中。

- 新增了 **jail(2)** 系统调用和管理员命令（**jail(8)**），为创建安全的进程执行环境提供了更大的灵活性。

- OpenSSL v0.9.4（通用的加密和 SSL2/3/TLSv1 工具包）集成到基本系统中。未来它将用于为 FreeBSD 工具提供开箱即用的强加密支持。

- OpenSSH 1.2 集成到基本系统中。OpenSSH 是一款免费的（BSD 许可）、功能完整的 SSH v1 协议实现，与其他 SSH v1 客户端和服务器（例如 Port **/usr/ports/security/ssh**）完全兼容。OpenSSH 提供了该 Port 的所有功能，实际上它基于该软件成为限制性许可之前的较旧版本。如果你在 sysinstall 中选择安装 `DES` 加密分发包，FreeBSD 4.0 将开箱即用提供 SSH 客户端/服务器功能。

- Telnet 引入了新的加密认证机制，称为 SRA。SRA 使用 Diffie-Hellman 密钥交换来建立会话密钥，然后使用该密钥对用户名和密码进行 DES 加密。作为副作用，会话密钥还用于对会话本身进行 DES 加密。虽然 SRA 易受到中间人攻击，DH 参数较小，且 DES 显得过时，但其优点是无需对机器进行任何管理变更即可工作，至少比明文传输更安全。要使用该功能，你需要使用 `telnet -ax` 或设置 `.telnetrc` 文件以默认启用它。

- IPsec 支持已从 KAME 项目导入。这包括 IPsec 隧道模式，用于通过安全网关实现虚拟专用网络，以及 IPsec 传输模式，用于实现安全的套接字级通信。此外，内核内部的加密代码导入到 **sys/crypto** 中，IPsec 支持新增到以下用户空间应用程序中：**sbin/ping**、**usr.sbin/inetd**、**usr.sbin/rrenumd**、**usr.sbin/traceroute6**、**usr.sbin/rtadvd**、**usr.sbin/setkey**
...
通过 f2c 实现的 f77 模拟由本地 F77 编译器替代。

时区数据库已更新，以包括欧洲、前苏联以及中美洲和南美洲的所有最新时区变更。时区数据文件现在包含幻数，便于识别。

Groff/troff/eqn 已更新至版本 1.15。

Gdb 已更新至版本 4.18。

作为 FreeBSD 审计项目的一部分，应用了许多修复以提升 FreeBSD 代码的安全性。

FreeBSD 的线程库 `libc_r` 新增了许多功能和性能改进，使其几乎完全符合 POSIX 标准。此外，Linux 内核支持的 LinuxThreads 库现在作为 Port（**ports/devel/linuxthreads**）可用，可以用于原生的 FreeBSD 程序。
...
许多 Ports 已更新以支持 IPv6。请参见 `ipv6` 虚拟 Ports 类别以获取完整列表。

Sysinstall 启用了 PC 卡控制器和 **pccardd(8)** 以支持 PC 卡安装介质。

## 2. 支持的配置

FreeBSD 目前支持多种基于 ISA、VLB、EISA、MCA 和 PCI 总线的 PC，从 386sx 到 Pentium 级机器（尽管不推荐使用 386sx）。还支持通用 IDE 或 ESDI 驱动配置、各种 SCSI 控制器、网络卡和串行卡。
...
所有支持的 SCSI 控制器均提供对 SCSI-I 和 SCSI-II 外设的完整支持，包括硬盘、光盘、磁带驱动器（包括 DAT 和 8mm Exabyte）、介质更换器、处理器目标设备和 CDROM 驱动器。对于支持 CDROM 命令的 WORM 设备，CDROM 驱动程序提供只读访问支持。WORM/CD-R/CD-RW 写入支持通过 Ports 树中的 cdrecord 提供。
...
以下驱动在旧 SCSI 子系统下受支持，但在新的 CAM SCSI 子系统下尚不支持：
...
[ 注意：目前正将 UltraStor 驱动移植到新的 CAM SCSI 框架中，但尚不确定何时或能否完成。]
...
- 基于 ADMtek Inc. AL981 的 PCI 快速以太网网卡
- 基于 ADMtek Inc. AN985 的 PCI 快速以太网网卡
- 基于 ADMtek Inc. AN986 的 USB 以太网网卡，包括以下型号：
  - LinkSys USB100TX
  - Billionton USB100
  - Melco Inc. LU-ATX
  - D-Link DSB-650TX
...
- 基于 CATC USB-EL1210A 的 USB 以太网网卡，包括以下型号：
  - CATC Netmate
  - CATC Netmate II
  - Belkin F5U111

- 基于 Kawasaki LSI KU5KUSB101B 的 USB 以太网网卡，包括以下型号：
  - LinkSys USB10T
  - Entrega NET-USB-E45
...
- 基于 DEC DC21040、DC21041 或 DC21140 的网卡（SMC Etherpower 8432T、DE245 等）
...
- IETF“经典 IP 和 ARP over ATM”模型
  ...
  - RFC 1755，“支持 IP over ATM 的 ATM 信令”
...
- 基于 Xilinx XC6200 的可重新配置硬件卡，兼容 Virtual Computers（www.vcc.com）的 HOT1。
...
运输费用（按订单而非按光盘计算）在美国、加拿大和墨西哥为 $5.00，海外为 $9.00。接受 Visa、Mastercard、Discover、American Express 信用卡或以美元支付的支票，并在美国境内提供货到付款服务。加利福尼亚州居民请额外加收 8.25% 的销售税。
...
你的建议、错误报告和代码贡献始终受到重视——如果你发现任何问题，请随时报告（如果可以的话，最好附带一个修复方案！）。

在具有互联网邮件连接的机器上提交错误报告的首选方法是使用命令 `send-pr`，或使用位于 [http://www.FreeBSD.org/send-pr.html](http://www.FreeBSD.org/send-pr.html) 的 CGI 脚本。我们忠实的 bugfiler 程序会妥善归档错误报告，你可以确信我们会尽最大努力尽快回应所有报告的错误。以这种方式提交的错误报告也将在我们的官网的支持部分显示，因此，它们不仅作为错误报告有价值，还为其他用户提供有关潜在问题的“警示”。

如果由于某种原因，你无法使用命令 `send-pr` 提交错误报告，你可以尝试发送邮件至：`freebsd-bugs@FreeBSD.org`

请注意，`send-pr` 本身是一个 shell 脚本，应该可以轻松移植，即使是到完全不同的系统。我们更希望你使用这个接口，因为它使跟踪问题报告变得更加容易。然而，在提交之前，请尽量确认问题是否已被修复。

否则，如有任何疑问或技术支持需求，请发送邮件至：`freebsd-questions@FreeBSD.org`
...
FreeBSD 凝聚了来自世界各地的许多个人（甚至上百人）辛勤工作的累积成果，他们为此版本的发布付出了巨大努力。有关 FreeBSD 项目工作人员的完整名单，请参见：`http://www.FreeBSD.org/handbook/staff.html`
...
- Justin M. Seger <jseger@FreeBSD.org>，几乎凭一己之力将 Ports 转换为 ELF 格式。
- Doug Rabson <dfr@FreeBSD.org> 和 John Birrell <jb@FreeBSD.org>，让 FreeBSD/alpha 成为现实，以及 NetBSD 项目提供的大量间接帮助。
- Peter Wemm <peter@FreeBSD.org>，贡献了新的内核模块系统（Doug Rabson 也给予了大量帮助）。
- 以及全球成千上万的 FreeBSD 用户和测试人员，没有他们，这个版本根本无法完成。

我们真诚希望你喜爱这个版本的 FreeBSD！
