...
- IPv6 支持已从 KAME 项目导入。包括内核 IPv6 协议栈（**sys/netinet6**）、TCP IPv6 支持、可配置的 IPv6 和 IPv4 隧道（通过 IPv6 或 IPv4 传输），以及 IPv6 TCP 到 IPv4 TCP 的转换网关支持。已向 libc 新增与协议无关的名称解析函数（getaddrinfo、getnameinfo 等）。
...
- OpenSSL v0.9.4（通用的加密和 SSL2/3/TLSv1 工具包）已集成到基本系统中。未来它将用于为 FreeBSD 工具提供开箱即用的强加密支持。

- OpenSSH 1.2 已集成到基本系统中。OpenSSH 是一个免费的（BSD 许可）、功能完整的 SSH v1 协议实现，与其他 SSH v1 客户端和服务器（例如 Port **/usr/ports/security/ssh**）完全兼容。OpenSSH 提供了该 Port 的所有功能，实际上它基于该软件成为限制性许可之前的一个较旧版本。如果你在 sysinstall 中选择安装 `DES` 加密分发包，FreeBSD 4.0 将开箱即用提供 SSH 客户端/服务器功能。
...
- IPsec 支持已从 KAME 项目导入。这包括 IPsec 隧道模式，用于通过安全网关实现虚拟专用网络，以及 IPsec 传输模式，用于实现安全的套接字级通信。此外，内核内部的加密代码已导入到 **sys/crypto** 中，IPsec 支持已新增到以下用户空间应用程序中：**sbin/ping**、**usr.sbin/inetd**、**usr.sbin/rrenumd**、**usr.sbin/traceroute6**、**usr.sbin/rtadvd**、**usr.sbin/setkey**
...
通过 f2c 实现的 f77 模拟已由本地 F77 编译器替代。
