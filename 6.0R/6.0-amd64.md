...
实用程序 [chflags(1)](http://www.freebsd.org/cgi/man.cgi?query=chflags&sektion=1&manpath=FreeBSD+6.0-RELEASE) 现在支持 `-h` 标志，用于修改符号链接的标志。
...
工具 [ifconfig(8)](http://www.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8&manpath=FreeBSD+6.0-RELEASE) 的 `-vlandev` 参数不再需要网络接口作为其参数。虽然仍然支持该参数以保持向后兼容，但它现在弃用，不推荐使用。
...
程序 [ppp(8)](http://www.freebsd.org/cgi/man.cgi?query=ppp&sektion=8&manpath=FreeBSD+6.0-RELEASE) 现在实现了 `echo` 参数，允许独立启用 LCP ECHO，而不依赖 LQR 报告。旧版本的 [ppp(8)](http://www.freebsd.org/cgi/man.cgi?query=ppp&sektion=8&manpath=FreeBSD+6.0-RELEASE) 在协商失败时会恢复到 LCP ECHO 模式。现在必须指定 `enable echo` 才能获得此行为。[已合并]
...
在之前的 FreeBSD 版本中，`disc1` CD-ROM（或 ISO 镜像）是可启动的安装盘，包含基本系统、Ports 和常用软件包。`disc2` CD-ROM（或 ISO 镜像）是可启动的“修复盘”，带有实时文件系统，用于紧急修复。此布局现变更。
