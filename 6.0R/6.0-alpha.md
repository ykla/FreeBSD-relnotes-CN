...
新增一个加载器菜单选项，用于设置 `hint.atkbd.0.flags=0x1`。此设置能在没有 PS/2 键盘连接的情况下使 USB 键盘正常工作。
...
修复了 libalias 库中的一个错误，该错误在指定 `-reverse` 选项时导致 [natd(8)](http://www.freebsd.org/cgi/man.cgi?query=natd&sektion=8&manpath=FreeBSD+6.0-RELEASE) 产生核心转储。
...
修复了实用程序 [rexecd(8)](http://www.freebsd.org/cgi/man.cgi?query=rexecd&sektion=8&manpath=FreeBSD+6.0-RELEASE) 中的一个 bug，该 bug 导致其行为如同总是指定了 `-i` 选项。[已合并]
