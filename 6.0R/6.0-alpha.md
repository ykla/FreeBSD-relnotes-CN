...
程序 [ppp(8)](http://www.freebsd.org/cgi/man.cgi?query=ppp&sektion=8&manpath=FreeBSD+6.0-RELEASE) 现在实现了 `echo` 参数，允许独立于 LQR 报告启用 LCP ECHO。旧版本的 [ppp(8)](http://www.freebsd.org/cgi/man.cgi?query=ppp&sektion=8&manpath=FreeBSD+6.0-RELEASE) 在协商失败时会恢复到 LCP ECHO 模式。现在必须指定 `enable echo` 才能获得此行为。[已合并]
...
在之前的 FreeBSD 版本中，disc1 CD-ROM（或 ISO 镜像）是一个可启动的安装盘，包含基本系统、Ports 树和常用软件包。disc2 CD-ROM（或 ISO 镜像）是一个可启动的“修复盘”，带有实时文件系统，用于紧急修复。此布局现已变更。
