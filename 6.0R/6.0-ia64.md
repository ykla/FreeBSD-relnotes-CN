# FreeBSD 6.0-RELEASE ia64 发行说明（2005 年 11 月 4 日）
...
本文档包含了 FreeBSD 6.0-RELEASE 在 IA-64 硬件平台上的发行说明。它介绍了 FreeBSD 最近新增、修改或删除的功能。还提供了一些从先前版本 FreeBSD 升级的说明。

该 FreeBSD 6.0-RELEASE 发行版是一个正式发行版本。可以在 [ftp://ftp.FreeBSD.org/](ftp://ftp.freebsd.org/) 或其任何镜像站点找到。获取此（或其他）FreeBSD 发行版本的方法，请参阅 [FreeBSD 手册](http://www.freebsd.org/doc/en_US.ISO8859-1/books/handbook/) 中的 [“获取 FreeBSD”附录](http://www.freebsd.org/doc/en_US.ISO8859-1/books/handbook/mirrors.html)。
...
修复了 TELNET 客户端程序中的两个缓冲区溢出漏洞。这些漏洞可能允许恶意的 TELNET 服务器或主动的网络攻击者导致 [telnet(1)](http://www.freebsd.org/cgi/man.cgi?query=telnet&sektion=1&manpath=FreeBSD+6.0-RELEASE) 以运行该程序的用户的权限执行任意代码。更多信息请参阅安全公告 [FreeBSD-SA-05:01.telnet](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-05:01.telnet.asc)。[已合并]
...
默认的参数 `HZ`（控制各种内核计时器）在 i386 和 ia64 上从 `100` 增加到 `1000`。在 amd64 上从 `1024` 减少到 `1000`，以减少与其他系统时钟的同步效应。
...
驱动程序 [atkbdc(4)](http://www.freebsd.org/cgi/man.cgi?query=atkbdc&sektion=4&manpath=FreeBSD+6.0-RELEASE)、[atkbd(4)](http://www.freebsd.org/cgi/man.cgi?query=atkbd&sektion=4&manpath=FreeBSD+6.0-RELEASE) 和 [psm(4)](http://www.freebsd.org/cgi/man.cgi?query=psm&sektion=4&manpath=FreeBSD+6.0-RELEASE) 以更加独立于总线的方式重写，现在支持在 sparc64 平台上找到的 EBus。
...
[ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+6.0-RELEASE) 的 ipfw fwd 规则现在支持在指定内核选项 **IPFIREWALL_FORWARD_EXTENDED**（除了 **IPFIREWALL_FORWARD** 之外）的情况下，完全操作数据包目标。此内核选项禁用所有限制，以确保本地生成的数据包的正常行为，并允许将数据包重定向到本地配置的 IP 地址。请注意，须谨慎编写 [ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+6.0-RELEASE) 规则，以确保 PMTU 发现等功能不会受到影响。[已合并]
...
改进了 FreeBSD TCP 栈的 RST 处理，使实施重置攻击尽可能困难，同时保持与最广泛 TCP 栈的兼容性。算法如下：对于 ESTABLISHED 状态中的连接，只有与 `last_ack_sent` 完全匹配的序列号才会导致重置；所有其他数据段将被默默丢弃。对于其他所有状态中的连接，窗口中的任何位置的重置都会导致连接重置。所有其他数据段将被默默丢弃。请注意，这种行为在技术上违反了 RFC 793 规范；可以通过将新的 sysctl `net.inet.tcp.insecure_rst` 设置为 `1` 来恢复传统的（但不太安全的）行为。[已合并]
...
驱动程序 [twa(4)](http://www.freebsd.org/cgi/man.cgi?query=twa&sektion=4&manpath=FreeBSD+6.0-RELEASE) 更新为 9.2 版本（适用于 FreeBSD 5.2.1），并从 3ware 网站分发。
...
[lpd(8)](http://www.freebsd.org/cgi/man.cgi?query=lpd&sektion=8&manpath=FreeBSD+6.0-RELEASE) 程序现在在打印来自其他主机的数据文件之前，会检查该文件是否已完全传输。某些 [lpr(1)](http://www.freebsd.org/cgi/man.cgi?query=lpr&sektion=1&manpath=FreeBSD+6.0-RELEASE) 实现会在发送匹配的数据文件之前发送打印作业的控制文件，这在接收主机为忙碌的打印服务器时可能会造成问题。[已合并]
...
守护进程 [moused(8)](http://www.freebsd.org/cgi/man.cgi?query=moused&sektion=8&manpath=FreeBSD+6.0-RELEASE) 现在支持“虚拟滚动”，即按住中键移动鼠标时，将其解释为滚动操作。此功能通过参数 `-V` 启用。[已合并]
...
程序 [ppp(8)](http://www.freebsd.org/cgi/man.cgi?query=ppp&sektion=8&manpath=FreeBSD+6.0-RELEASE) 现在实现了参数 `echo`，允许在 LCP ECHO 报告之外独立启用 LCP ECHO。旧版本的 [ppp(8)](http://www.freebsd.org/cgi/man.cgi?query=ppp&sektion=8&manpath=FreeBSD+6.0-RELEASE) 在协商失败时会恢复到 LCP ECHO 模式。现在必须指定启用 echo 才能获得此行为。[已合并]

为 [ppp(8)](http://www.freebsd.org/cgi/man.cgi?query=ppp&sektion=8&manpath=FreeBSD+6.0-RELEASE) 程序新增了选项 `disable NAS-IP-Address` 和 `disable NAS-Identifier`，以支持 RFC 2865 之前的 RADIUS 服务器。

修复了程序 [pppd(8)](http://www.freebsd.org/cgi/man.cgi?query=pppd&sektion=8&manpath=FreeBSD+6.0-RELEASE) 中的两个 bug，这些 bug 可能导致不正确的 CBCP 响应，违反了 Microsoft PPP 回调控制协议第 3.2 节。[已合并]
...
修复了实用程序 [rexecd(8)](http://www.freebsd.org/cgi/man.cgi?query=rexecd&sektion=8&manpath=FreeBSD+6.0-RELEASE) 中的一个 bug，该 bug 导致它总是表现得像指定了 `-i` 选项。[已合并]
...
现已实现了 ISO/IEC 9899:1999 标准函数：`roundl()`，`lroundl()`，`llroundl()`，`truncl()` 和 `floorl()`。
