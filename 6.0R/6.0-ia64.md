...
默认的 `HZ` 参数（控制各种内核计时器）在 i386 和 ia64 上从 `100` 增加到 `1000`。在 amd64 上从 `1024` 减少到 `1000`，以减少与其他系统时钟的同步效应。
...
守护进程 [moused(8)](http://www.freebsd.org/cgi/man.cgi?query=moused&sektion=8&manpath=FreeBSD+6.0-RELEASE) 现在支持“虚拟滚动”，即按住中键移动鼠标时，将其解释为滚动操作。此功能通过 `-V` 参数启用。[已合并]
