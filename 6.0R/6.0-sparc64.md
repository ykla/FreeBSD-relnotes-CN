...
实用程序 [chflags(1)](http://www.freebsd.org/cgi/man.cgi?query=chflags&sektion=1&manpath=FreeBSD+6.0-RELEASE) 现在支持 `-h` 标志，用于修改符号链接上的标志。
...
程序 [ppp(8)](http://www.freebsd.org/cgi/man.cgi?query=ppp&sektion=8&manpath=FreeBSD+6.0-RELEASE) 现在实现了 `echo` 参数，允许在 LQR 报告之外独立启用 LCP ECHO。旧版本的 [ppp(8)](http://www.freebsd.org/cgi/man.cgi?query=ppp&sektion=8&manpath=FreeBSD+6.0-RELEASE) 在协商失败时会恢复到 LCP ECHO 模式。现在必须指定 `enable echo` 才能获得此行为。[已合并]
...
[what(1)](http://www.freebsd.org/cgi/man.cgi?query=what&sektion=1&manpath=FreeBSD+6.0-RELEASE) 现在支持 `-q` 参数，使其打印匹配的文本，但不进行格式化。
...
- [pkg_create(1)](http://www.freebsd.org/cgi/man.cgi?query=pkg_create&sektion=1&manpath=FreeBSD+6.0-RELEASE) 工具现在支持 `-R` 参数。当从本地已安装的软件包创建软件包文件时，若指定该参数，将同时为该软件包依赖的所有软件包创建软件包文件。

- [pkg_version(1)](http://www.freebsd.org/cgi/man.cgi?query=pkg_version&sektion=1&manpath=FreeBSD+6.0-RELEASE) 工具新增 `-q` 参数，用于隐藏 Ports 版本比较字符（`<`、`=` 和 `>`）的输出。
...
在之前的 FreeBSD 版本中，`disc1` CD-ROM（或 ISO 镜像）是可启动的安装盘，包含基本系统、Ports 树和常用软件包。`disc2` CD-ROM（或 ISO 镜像）是可启动的“修复盘”，带有实时文件系统，用于紧急修复。此布局现已变更。
除 ia64 架构外，`disc1` 镜像现在包含基本系统分发文件、Ports 树和实时文件系统，适合初始安装和修复使用。（由于体积原因，ia64 的实时文件系统放在单独的磁盘上。）软件包被移至单独的磁盘，`disc2` 镜像中包括常见的软件包，如桌面环境。FreeBSD 文档项目的文档也存放在 `disc2` 中。[已合并]
