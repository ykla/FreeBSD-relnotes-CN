# FreeBSD 6.0-RELEASE sparc64 发行说明（2005 年 11 月 4 日）
...
本文档包含了 FreeBSD 6.0-RELEASE 在 UltraSPARC 硬件平台上的发行说明。它介绍了 FreeBSD 最近新增、修改或删除的功能。还提供了一些从先前版本 FreeBSD 升级的说明。
...
所有用户在安装 FreeBSD 前都应查阅发布勘误。勘误文档会更新发布周期结束时或发布后发现的“最新”信息。通常，它包含已知的 bug、安全公告和文档修正。可以在 FreeBSD 网站上找到 FreeBSD 6.0-RELEASE 的最新勘误副本。
...
修复了 TELNET 客户端程序中的两个缓冲区溢出漏洞。这些漏洞可能允许恶意的 TELNET 服务器或主动网络攻击者以运行 [telnet(1)](http://www.freebsd.org/cgi/man.cgi?query=telnet&sektion=1&manpath=FreeBSD+6.0-RELEASE) 的用户权限执行任意代码。更多信息请参阅安全公告 [FreeBSD-SA-05:01.telnet](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-05:01.telnet.asc)。[已合并]
...
`autoboot` 加载器命令现在支持参数 `prompt`。

如果变量 `autoboot_delay` 设置为 `-1`，`autoboot` 加载器命令现在将完全禁止用户中断启动过程。[已合并]
...
新增了以下设备驱动程序，并在 GENERIC 内核中默认启用：[atkbdc(4)](http://www.freebsd.org/cgi/man.cgi?query=atkbdc&sektion=4&manpath=FreeBSD+6.0-RELEASE)、[atkbd(4)](http://www.freebsd.org/cgi/man.cgi?query=atkbd&sektion=4&manpath=FreeBSD+6.0-RELEASE)、creator(4)、machfb(4)、[syscons(4)](http://www.freebsd.org/cgi/man.cgi?query=syscons&sektion=4&manpath=FreeBSD+6.0-RELEASE)、[ohci(4)](http://www.freebsd.org/cgi/man.cgi?query=ohci&sektion=4&manpath=FreeBSD+6.0-RELEASE)、[psm(4)](http://www.freebsd.org/cgi/man.cgi?query=psm&sektion=4&manpath=FreeBSD+6.0-RELEASE)、[ukbd(4)](http://www.freebsd.org/cgi/man.cgi?query=ukbd&sektion=4&manpath=FreeBSD+6.0-RELEASE)、[ums(4)](http://www.freebsd.org/cgi/man.cgi?query=ums&sektion=4&manpath=FreeBSD+6.0-RELEASE) 和 [usb(4)](http://www.freebsd.org/cgi/man.cgi?query=usb&sektion=4&manpath=FreeBSD+6.0-RELEASE)。

新增 [auxio(4)](http://www.freebsd.org/cgi/man.cgi?query=auxio&sektion=4&manpath=FreeBSD+6.0-RELEASE) 驱动程序，支持各种 SBus/EBus UltraSPARC® 模型上的某些辅助 I/O 功能。[已合并]

新增驱动程序 clkbrd，用于支持常见于 Sun Exx00 服务器上的 `clock-board` 设备。
...
新增驱动程序 rtc，用于支持某些 UltraSPARC II 和 III 模型上的 MC146818 兼容时钟。[已合并]

驱动程序 [uart(4)](http://www.freebsd.org/cgi/man.cgi?query=uart&sektion=4&manpath=FreeBSD+6.0-RELEASE) 现在在 GENERIC 内核中启用，成为默认的串口驱动程序。驱动程序 [ofw_console(4)](http://www.freebsd.org/cgi/man.cgi?query=ofw_console&sektion=4&manpath=FreeBSD+6.0-RELEASE) 和 [sab(4)](http://www.freebsd.org/cgi/man.cgi?query=sab&sektion=4&manpath=FreeBSD+6.0-RELEASE) 现在在 GENERIC 内核中禁用。[已合并]
...
已移除 zs 驱动程序，由 [uart(4)](http://www.freebsd.org/cgi/man.cgi?query=uart&sektion=4&manpath=FreeBSD+6.0-RELEASE) 驱动程序替代。

##### 2.2.2.1 多媒体支持

新增驱动程序 [snd_audiocs(4)](http://www.freebsd.org/cgi/man.cgi?query=snd_audiocs&sektion=4&manpath=FreeBSD+6.0-RELEASE)，支持 UltraSPARC 工作站上的 Crystal Semiconductor CS4231 音频控制器。[已合并]
...
已移除 gx(4) 驱动，因为它不再积极维护，而驱动 [em(4)](http://www.freebsd.org/cgi/man.cgi?query=em&sektion=4&manpath=FreeBSD+6.0-RELEASE) 支持所有支持的硬件。
...
已移除 `musycc` 驱动（用于 LanMedia LMC1504 T1/E1 网络接口卡），因为不再使用。
...
[wlan(4)](http://www.freebsd.org/cgi/man.cgi?query=wlan&sektion=4&manpath=FreeBSD+6.0-RELEASE) 框架中的 802.11 设备支持经过大幅重构。除了架构上的变更外，还包括完成的 802.11g、WPA、802.11i、802.1x、WME/WMM、AP 端节能以及加密模块、认证器和访问控制插件框架。特别需要注意的是，现在 WEP 需要加载（或编译到内核中的）`wlan_wep` 模块。
...
[ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+6.0-RELEASE) 的 `ipfw fwd` 规则现在支持在指定内核选项 `IPFIREWALL_FORWARD_EXTENDED`（在 `options IPFIRWALL_FORWARD` 之外）的情况下，完全操作数据包目标。此内核选项禁用所有限制，以确保本地生成的数据包的正常行为，并允许将数据包重定向到本地配置的 IP 地址。请注意，须谨慎编写 [ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+6.0-RELEASE) 规则，以确保像 PMTU 发现这样的问题不会中断。[已合并]
...
已移除驱动程序 [matcd(4)](http://www.freebsd.org/cgi/man.cgi?query=matcd&sektion=4&manpath=FreeBSD+6.0-RELEASE)。[已合并]
...
修复了 NFS 服务器中的内核 Panic。更多信息请参阅勘误通知 [FreeBSD-EN-05:01.nfs](ftp://ftp.freebsd.org/pub/FreeBSD/ERRATA/notices/FreeBSD-EN-05:01.nfs.asc)。[已合并]
...
实用程序 [chflags(1)](http://www.freebsd.org/cgi/man.cgi?query=chflags&sektion=1&manpath=FreeBSD+6.0-RELEASE) 现在支持标志 `-h`，用于修改符号链接上的标志。
...
工具 [ifconfig(8)](http://www.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8&manpath=FreeBSD+6.0-RELEASE) 经过重构。现在，它在支持接口特定功能方面更加模块化和灵活。802.11 支持已更新，以支持对 802.11 子系统和驱动程序的最新变更。
...
[lpd(8)](http://www.freebsd.org/cgi/man.cgi?query=lpd&sektion=8&manpath=FreeBSD+6.0-RELEASE) 程序现在在接收到来自其他主机的数据文件时，会先检查数据文件是否已完全传输，然后再开始打印。某些 [lpr(1)](http://www.freebsd.org/cgi/man.cgi?query=lpr&sektion=1&manpath=FreeBSD+6.0-RELEASE) 实现会在发送匹配的数据文件之前发送打印作业的控制文件，这在接收主机为忙碌的打印服务器时可能会造成问题。[已合并]
...
实用程序 [newsyslog(8)](http://www.freebsd.org/cgi/man.cgi?query=newsyslog&sektion=8&manpath=FreeBSD+6.0-RELEASE) 现在支持 `-N` 选项，禁止旋转任何文件。
...
程序 [ppp(8)](http://www.freebsd.org/cgi/man.cgi?query=ppp&sektion=8&manpath=FreeBSD+6.0-RELEASE) 现在实现了参数 `echo`，允许在 LQR 报告之外独立启用 LCP ECHO。旧版本的 [ppp(8)](http://www.freebsd.org/cgi/man.cgi?query=ppp&sektion=8&manpath=FreeBSD+6.0-RELEASE) 在协商失败时会恢复到 LCP ECHO 模式。现在必须指定 `enable echo` 才能获得此行为。[已合并]
...
修复了实用程序 [rexecd(8)](http://www.freebsd.org/cgi/man.cgi?query=rexecd&sektion=8&manpath=FreeBSD+6.0-RELEASE) 中的 bug，该 bug 导致其行为如同始终指定了 `-i` 选项。[已合并]

已移除实用程序 [rexecd(8)](http://www.freebsd.org/cgi/man.cgi?query=rexecd&sektion=8&manpath=FreeBSD+6.0-RELEASE)。FreeBSD 树中没有 rexec 客户端，客户端功能 [rexec(3)](http://www.freebsd.org/cgi/man.cgi?query=rexec&sektion=3&manpath=FreeBSD+6.0-RELEASE) 仅存在于 libcompat 中。
...
现在，当实用程序 [rm(1)](http://www.freebsd.org/cgi/man.cgi?query=rm&sektion=1&manpath=FreeBSD+6.0-RELEASE) 指定了 `-r` 和 `-f` 选项并尝试删除不存在的目录时，会抑制诊断消息。这种行为是 SUSv3 版本的要求。
...
实用程序 [syslogd(8)](http://www.freebsd.org/cgi/man.cgi?query=syslogd&sektion=8&manpath=FreeBSD+6.0-RELEASE) 现在打开额外的域套接字（默认路径为 **/var/run/logpriv**），权限为 0600，以供特权程序使用。这样可以防止当域套接字由于本地拒绝服务攻击而没有足够的缓冲区空间时，特权程序发生阻塞。[已合并]
...
实用程序 [tail(1)](http://www.freebsd.org/cgi/man.cgi?query=tail&sektion=1&manpath=FreeBSD+6.0-RELEASE) 的 `-f` 选项现在支持同时监控多个文件。[已合并]
...
[what(1)](http://www.freebsd.org/cgi/man.cgi?query=what&sektion=1&manpath=FreeBSD+6.0-RELEASE) 现在支持参数 `-q`，使其打印匹配的文本，但不进行格式化。
...
修复了错误，导致配置文件（如 [hosts(5)](http://www.freebsd.org/cgi/man.cgi?query=hosts&sektion=5&manpath=FreeBSD+6.0-RELEASE)、[services(5)](http://www.freebsd.org/cgi/man.cgi?query=services&sektion=5&manpath=FreeBSD+6.0-RELEASE) 等）的最后一行如果没有以换行符结尾，则被忽略。[已合并]
...
- [pkg_create(1)](http://www.freebsd.org/cgi/man.cgi?query=pkg_create&sektion=1&manpath=FreeBSD+6.0-RELEASE) 工具现在支持参数 `-R`。当从本地已安装的软件包创建软件包文件时，若指定该参数，将同时为该软件包依赖的所有软件包创建软件包文件。
...
在之前的 FreeBSD 版本中，`disc1` CD-ROM（或 ISO 镜像）是可启动的安装盘，包含基本系统、Ports 树和常用软件包。`disc2` CD-ROM（或 ISO 镜像）是可启动的“修复盘”，带有实时文件系统，用于紧急修复。此布局现已变更。除 ia64 架构外，`disc1` 镜像现在包含基本系统分发文件、Ports 树和实时文件系统，适合初始安装和修复使用。（由于体积原因，ia64 的实时文件系统放在单独的磁盘上。）软件包被移至单独的磁盘，`disc2` 镜像中包括常见的软件包，如桌面环境。FreeBSD 文档项目的文档也存放在 `disc2` 中。[已合并]
