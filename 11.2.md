...
FreeBSD 11.2-RELEASE 是个 RELEASE 版本，可通过 [`https://www.FreeBSD.org/releases/`](https://www.freebsd.org/releases/) 或其镜像站点获取。获取此版本（或其他版本）的方法，请参阅 [FreeBSD 手册](https://docs.freebsd.org/en/books/handbook/)中的 [“获取 FreeBSD”附录](https://docs.freebsd.org/en/books/handbook/#mirrors)。
...
典型的发行说明内容包括：在 11.1-RELEASE 发布后发布的最新安全公告、新的驱动程序或硬件支持、新的命令或选项、重大错误修复或贡献的软件更新。此外，还可能列出主要 Port/包或发布工程实践的变更。显然，发行说明无法涉及 FreeBSD 在不同版本之间进行的每个变更；本文档主要关注安全公告、用户可见的变更和重要的体系结构改进。
