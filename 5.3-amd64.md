...
FreeBSD 动态和静态链接器现在支持线程局部存储（TLS），这是一种 GCC 特性，支持为全局和静态变量的声明新增修饰符 `__thread`。此额外的修饰符意味着该变量的值是线程局部的，一个线程更改其值不会影响其他线程中该变量的值。
