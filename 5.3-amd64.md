...
典型的发行说明条目包括发布后发布的安全公告、新驱动程序或硬件支持、新命令或选项、主要 bug 修复或第三方软件升级。它们还可能列出主要 Port/包或发布工程实践的变更。显然，发行说明不能列出每个版本之间对 FreeBSD 所做的所有变更；本文档主要关注安全公告、用户可见的变更以及主要的架构改进。
...
新增了 KDB，这是新的调试框架。它包含新的 GDB 后端，该后端已重写以支持线程、行程长度编码压缩等，以及一个前端，提供一个框架，允许配置多个不同的调试后端，并为这些后端提供基本服务。以下选项已变更：

* 默认通过内核选项启用 KDB，选项为 **KDB**、**GDB** 和 **DDB**。**DDB** 和 **GDB** 指定包含哪些 KDB 后端。
* 已将 **WITNESS_DDB** 重命名为 **WITNESS_KDB**。
* 已将 **DDB_TRACE** 重命名为 **KDB_TRACE**。
* 已将 **DDB_UNATTENDED** 重命名为 **KDB_UNATTENDED**。
* 已将 **SC_HISTORY_DDBKEY** 重命名为 **SC_HISTORY_KDBKEY**。
* 已移除 **DDB_NOKLDSYM**。新的 DDB 后端同时支持预链接符号查找和 KLD 符号查找。
* 已移除 **GDB_REMOTE_CHAT**。允许该功能的 GDB 协议黑客是 FreeBSD 特有的。同时，GDB 协议现在为控制台输出提供数据包。
...
新增了ULE调度器作为附加调度器。请注意，传统调度器 4BSD 仍然是 **GENERIC** 内核中的默认调度器。对于大多数用户来说，交互性在许多情况下得到了改善。这意味着在机器非常忙时，交互式应用程序中的“跳跃”和“卡顿”现象会减少。虽然它不能防止因磁盘子系统过载导致的问题，但确实有助于缓解 CPU 过载。在 SMP 机器上，ULE 有每个 CPU 的运行队列，可以支持 CPU 亲和性、CPU 绑定和高级超线程支持，并为未来的更多优化提供框架。随着细粒度内核锁的继续，调度器将能够更有效地利用可用的并行资源。
...
已将加载器可调选项 `debug.witness_*` 重命名为 `debug.witness.*`。
...
新增新的 Netgraph 方法，以恢复从 4.X 风格 [ng_tee(4)](http://www.freebsd.org/cgi/man.cgi?query=ng_tee&sektion=4&manpath=FreeBSD+5.3-RELEASE) Netgraph 节点变更中丧失的部分行为。
...
已新增内核选项 **MSDOSFS_LARGE**，用于支持大于 128GB 的 FAT32 文件系统。此选项默认禁用。它为磁盘上的每个文件至少使用 32 字节的内核内存；此外，它仅在特定受控情况下使用才安全，例如少于 100 万个文件的只读挂载等。不支持通过 NFS 导出这些大型文件系统。
...
新增了 [kgdb(1)](http://www.freebsd.org/cgi/man.cgi?query=kgdb&sektion=1&manpath=FreeBSD+5.3-RELEASE)，一个内核调试工具，使用 **libgdb** 并理解内核线程、内核模块以及 [kvm(3)](http://www.freebsd.org/cgi/man.cgi?query=kvm&sektion=3&manpath=FreeBSD+5.3-RELEASE)。
...
现已实现 `tgmath.h` C99 头文件。该头文件提供了函数 `math.h` 和 `complex.h` 的类型通用宏，这些函数具有 float、double 和 long double 实现。
...
已新增工具 [logins(1)](http://www.freebsd.org/cgi/man.cgi?query=logins&sektion=1&manpath=FreeBSD+5.3-RELEASE)，用于显示有关用户和系统账户的信息。
...
强烈建议已有FreeBSD系统的用户阅读《FreeBSD 5.3-RELEASE 迁移指南》。此文档通常在发行介质中以 `MIGRATE5.TXT` 文件名提供，或者可以在任何包含发行说明的地方找到。文档中包含从 FreeBSD 4.X 迁移的相关说明，更重要的是，它讨论了升级到 FreeBSD 5.X 与继续运行 FreeBSD 4.X 的相对优点。
