...
[bsdlabel(8)](https://man.freebsd.org/cgi/man.cgi?query=bsdlabel&sektion=8&format=html) 工具已计划在 FreeBSD 16.0 中移除。用户应迁移至 [gpart(8)](https://man.freebsd.org/cgi/man.cgi?query=gpart&sektion=8&format=html) 进行磁盘标签管理。[be24d85ec784](https://cgit.freebsd.org/src/commit/?id=be24d85ec784)。（由 FreeBSD 基金会赞助）
...
sysctl `vfs.zfs.min_auto_ashift` 及其他旧版 ZFS sysctl 在使用时现发出警告，提示已弃用并将移除。用户应更新脚本和配置，使用新的 sysctl 名称，如 `vfs.zfs.vdev.min_auto_ashift`。[f0033878b92e](https://cgit.freebsd.org/src/commit/?id=f0033878b92e)。（由 FreeBSD 基金会赞助）
...
本节涵盖 FreeBSD Ports 集合、软件包基础设施及软件包维护和安装工具的变更。

KDE 已从 **dvd1.iso** 镜像中移除以适应 4.7 GB DVD 容量限制，同时添加了 **editors/emacs** 和 **editors/vim**。[142df248ac06](https://cgit.freebsd.org/src/commit/?id=142df248ac06)。
...
kmods 仓库中的固件软件包已包含在 DVD 安装介质中，确保固件模块在发行版本上可用。[b0fbed20ceb9](https://cgit.freebsd.org/src/commit/?id=b0fbed20ceb9)。（由 <https://www.patreon.com/cperciva> 赞助）
