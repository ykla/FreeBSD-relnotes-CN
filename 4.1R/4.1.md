# FreeBSD 4.1-RELEASE 发行说明（2000 年 7 月 27 日）

**发行说明**

**FreeBSD 4.1-RELEASE**

任何安装失败或崩溃问题应通过使用命令 `send-pr` 报告（偏好使用网页界面的人也可以访问 [http://www.FreeBSD.org/send-pr.html](http://www.FreeBSD.org/send-pr.html)）。

有关 FreeBSD 和 4.1-RELEASE 目录布局的信息（特别是从软盘安装时），请参阅 `ABOUT.TXT`。安装说明请参阅 `INSTALL.TXT` 和 `HARDWARE.TXT` 文件。

对于最新的 4.1-stable 快照（4.1 版本后的快照），请始终访问：[ftp://releng4.FreeBSD.org/pub/FreeBSD](ftp://releng4.FreeBSD.org/pub/FreeBSD)

## 1. 自 4.0-RELEASE 以来的新变化

### 1.1. 内核变化

FreeBSD 4.1-RELEASE 包含了来自 KAME 项目（<http://www.kame.net>）的更新代码，其中包括以下功能：

* 显著改善了 IPSEC 功能。特别是，IPSEC 安全关联不再需要手动配置密钥：新代码支持 `racoon`，这是 KAME 的 IKE 守护进程，位于 **/usr/ports/security/racoon**。经证明 `racoon` 与其他厂商的 IKE 系统能够良好互操作，这意味着 FreeBSD 4.1 可以在异构的 IPSEC 环境中使用。然而，`racoon` 仍在开发中，意味着可能仍然存在错误、配置语法变更等问题。
...
* ALTQ 流量整形系统尚未合并——有望在 4.2 版本发布前加入。更为实验性的 KAME 代码也未合并。如果你需要这些功能，可以考虑使用 4.1-RELEASE+KAME 快照，可从 [ftp://ftp.kame.net](ftp://ftp.kame.net) 获取，这些快照将在 4.1-RELEASE 发布后提供。

* 已知问题：通过 IPSEC 挂载的 NFS 并非在所有情况下都能可靠工作——可能出现挂载卡住和数据损坏的情况。

FreeBSD 内核新增了事件通知机制 `kqueue`。这是一种新接口，能够替代 `poll/select`，提供更好的性能，同时能报告多种不同类型的事件。支持监视套接字、管道、FIFO 和文件的变化，以及信号和进程。

FreeBSD 引导加载程序新增对英特尔 Wired for Management 2.0（PXE）的支持。由于 API 差异，旧版本的 PXE 不受支持。因此可以通过 DHCP 进行网络启动。

对于 FreeBSD 的 Alpha 版本，以下内容也适用：

* FreeBSD/alpha 现在拥有内置 FICL（Forth 支持）的加载器。

* 现在支持并口。

* 新增对多种 Alpha 系统类型的支持。请查看 `HARDWARE.TXT` 获取详细信息。

* AlphaServer 4100（Rawhide）无法使用软盘或 CD-ROM 安装。解决方法是在另一台 Alpha 机器上安装，然后将磁盘移至 AS4100。安装后，FreeBSD 可以正常运行。

* AlphaServer 2100A（Lynx）在此版本中不受支持。注意，AlphaServer 2100（Sable）可以正常工作。

* 现在支持具有板载 IDE 接口且其 SRM 可从 IDE 磁盘启动的机器，以 IDE 磁盘作为根/引导设备。具体的机器细节（如速度、DMA 使用等）请参阅 `HARDWARE.TXT`。

* 请注意，TGA 控制台（无论是内置的还是通过 TGA 扩展卡提供的）将无法工作。你需要使用串行控制台或安装 VGA 显卡。

### 1.2. 安全修复

内核和用户空间经过审计，以检查由 `vfprintf()` 等函数中格式字符串的不当使用导致的错误和安全漏洞。未发现任何漏洞。

有关其他安全修复，请参见位于 [http://www.FreeBSD.org/security/](http://www.FreeBSD.org/security/) 的发布的安全公告列表。

### 1.3. 用户空间变更

如前文所述，支持 KAME IKE 守护进程 `racoon`。

其他几个系统工具（如 `whois`、`fetch`，可能还有其他工具）现在支持通过 IPv6 操作。

**cdcontrol(1)** 现在支持命令 `cdid`，该命令使用与 CDDB 数据库相同的算法计算并显示 CD 的序列号。

**mtree(8)** 现在支持在创建和验证原型时，通过指定一个列出待排除路径名的文件来进行排除。这使得将 `mtree` 作为入侵检测系统的一部分使用更加容易。

OPIE 一次性密码套件更新至 2.32。

OpenSSH 升级至 2.1.0，提供对 SSH2 协议的支持，包括 DSA 密钥。因此，美国的 OpenSSH 用户不再需要依赖受限许可证的 RSAREF 工具包，该工具包是处理 RSA 密钥所必需的。OpenSSH 2.1 与其他 SSH2 客户端和服务器（包括 ssh2 Port）能够良好互操作。更多信息请参阅 [http://www.openssh.com](http://www.openssh.com)。

OpenSSH 现在可以在 SSH1 模式下使用 OPIE 密码进行身份验证。SSH2 模式下尚不支持此功能。

**camcontrol(8)** 现在内置了 `format` 功能，用于对 SCSI 磁盘进行低级格式化。

USB 设备支持添加到 **GENERIC** 内核和安装程序中，以便开箱即用地支持 USB 设备。请注意，在初始安装过程中仍然需要使用 AT 键盘，但之后应该可以正常工作。

整个 i386 启动过程经过重构，以支持自动检测和使用增强型磁盘驱动器 BIOS 扩展，从而支持启动超过第 1023 个柱面的磁盘。作为此变更的一部分，FreeBSD 启动管理器（`boot0`）的大小从 1 个扇区（512 字节）增加到 2 个扇区（1024 字节）。因此，修改了多个用户空间程序，以应对不同大小的 MBR 引导加载程序。

`libfetch` 得到大幅改进。**fetch(1)** 和 pkg 工具现在使用 `libfetch` 而不是 `libftpio`，这意味着 pkg 工具支持 HTTP，且两者都支持 IPv6。

**csh(1)** shell 现由 **tcsh(1)** 取代，尽管仍然可以作为 **csh(1)** 运行。

**more(1)** 命令现由 **less(1)** 取代，尽管仍然可以作为 **more(1)** 运行。

**ls(1)** 现在可以通过标志 `-G` 生成带颜色的列表（并且需要适当的终端支持）。

## 2. 支持的配置

FreeBSD 当前可在多种基于 ISA、VLB、EISA、MCA 和 PCI 总线的 PC 上运行，支持从 386sx 到 Pentium 类机器（不过不推荐使用 386sx）。同时，也提供对通用 IDE 或 ESDI 驱动器配置、各种 SCSI 控制器、网卡和串行卡的支持。
...
真正的 BusLogic MultiMaster 克隆版 AMI FastDisk 控制器也受支持。
...
QLogic 2100 光纤通道适配器（仅限专用环路）。

DTC 3290 EISA SCSI 控制器，1542 仿真模式。

对于所有支持的 SCSI 控制器，全面支持 SCSI-I 和 SCSI-II 外设，包括硬盘、光盘、磁带驱动器（包括 DAT 和 8mm Exabyte）、介质交换器、处理器目标设备和 CDROM 驱动器。支持 CDROM 命令的 WORM 设备（只读访问）由 CDROM 驱动程序支持。WORM/CD-R/CD-RW 写入支持由 Ports 树中的 `cdrecord` 提供。
...
[注：目前正在进行将 UltraStor 驱动程序移植到新 CAM SCSI 框架的工作，但尚不确定是否及何时完成。]
...
* Texas Instruments ThunderLAN PCI NIC，包括以下型号：
  ...
  * Racore 8148 10baseT/100baseTX/100baseFX 多模式
...
你可以从 `ftp.FreeBSD.org`（官方 FreeBSD 发布站点）通过 FTP 下载 FreeBSD 及其任意或全部可选软件包。

有关其他镜像 FreeBSD 软件的站点，请参见文件 `MIRROR.SITES`。请尽量从网络距离最近的站点通过 FTP 下载分发版。我们欢迎更多的镜像站点！如果你想成为官方镜像站点，请联系 `freebsd-admin@FreeBSD.org`，获取更多详细信息。
...
每张 `-RELEASE` 版光盘的费用为 39.95 美元，或使用 FreeBSD 订阅的情况下为 24.95 美元。 FreeBSD SNAPshot 版光盘（如果有货）的费用为 39.95 美元，或使用 FreeBSD-SNAP 订阅的情况下为 14.95 美元。（`-RELEASE` 和 `-SNAP` 订阅是完全独立的）。通过订阅，你将自动收到新发布的更新。每次发货时，你的信用卡将被扣费，你可以随时取消订阅，无需承担额外费用。

运费（按订单计算，而非每张光盘）为美国、加拿大和墨西哥 5 美元，海外为 9.00 美元。他们接受 Visa、Mastercard、Discover、American Express 卡和美元支票，并在美国境内进行货到付款（COD）发货。加利福尼亚州居民请额外支付 8.25% 的销售税。

如果你因任何原因不满意，光盘附有无条件退货政策。

## 4. 从旧版 FreeBSD 升级

如果你正在从以前的 FreeBSD 版本升级，最可能的版本是 3.0，并且根据你选择的升级方式，可能会遇到一些问题。升级 FreeBSD 版本有两种常见方式：
...
你的建议、错误报告以及代码贡献一直都受到重视——如果你发现任何问题（最好能附带修复方案），请随时报告。

如果你的机器能够通过互联网邮件进行通信，提交错误报告的首选方法是使用命令 `send-pr`，或者使用 `http://www.FreeBSD.org/send-pr.html` 上的 CGI 脚本。我们忠实的 `bugfiler` 程序会妥善归档错误报告，你可以放心，我们会尽力尽快回应所有报告的 bug。通过这种方式提交的 bug 也会在我们的官网的支持部分可见，因此既可以作为 bug 报告，也能作为其他用户关注潜在问题的“路标”。

如果由于某些原因，你无法使用命令 `send-pr` 提交错误报告，你可以尝试将其发送到：`freebsd-bugs@FreeBSD.org`

请注意，`send-pr` 本身是个 shell 脚本，应该容易移植到完全不同的系统。如果你能够使用该接口，我们更希望你这样做，因为它让我们能够更方便地跟踪问题报告。然而，在提交之前，请确认该问题是否已被修复。

对于任何问题或技术支持问题，请发送邮件至：`freebsd-questions@FreeBSD.org`

如果你正在跟踪 `-stable` 开发工作，你务必加入 `-stable` 邮件列表，以便了解可能影响你使用和维护系统方式的最新进展和变化：`freebsd-stable@FreeBSD.org`

此外，由于这是一个志愿者项目，我们非常欢迎愿意提供帮助的额外人手加入——目前我们希望实施的改进远远超过我们能单独处理的数量！如需就技术问题联系我们或想提供帮助，请发送邮件至：`freebsd-hackers@FreeBSD.org`

请注意，这些邮件列表可能会产生 *大量* 流量，如果你的邮件访问较慢或较贵，且只对关注 FreeBSD 重大事件感兴趣，你可能会觉得订阅以下邮件列表更为合适：`freebsd-announce@FreeBSD.org`

所有这些邮件列表都可以自由加入，任何有意者皆可参加。请发送邮件至 `MajorDomo@FreeBSD.org`，并在邮件正文中单独写上一行 `help` 关键字。这样你将获得更多关于如何加入各类列表、访问档案等的信息。此处未提及的还有许多面向特定兴趣群体的邮件列表，你可以向 MajorDomo 发送邮件咨询详情！

## 6. 致谢

FreeBSD 凝聚了数十甚至数百位来自世界各地的个人的共同努力，他们为发布这一版本付出了大量的工作。如果你想查看 FreeBSD 项目工作人员的完整名单，请访问：`http://www.FreeBSD.org/handbook/staff.html`
