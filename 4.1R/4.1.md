...
任何安装失败或崩溃问题应通过使用 `send-pr` 命令报告（偏好使用网页界面的人也可以访问 [http://www.FreeBSD.org/send-pr.html](http://www.FreeBSD.org/send-pr.html)）。
...
如果你的机器能够通过互联网邮件进行通信，提交错误报告的首选方法是使用 `send-pr` 命令，或者使用 `http://www.FreeBSD.org/send-pr.html` 上的 CGI 脚本。我们忠实的 `bugfiler` 程序会妥善归档错误报告，你可以放心，我们会尽力尽快回应所有报告的 bug。通过这种方式提交的 bug 也会在我们的官网的支持部分可见，因此既可以作为 bug 报告，也能作为其他用户关注潜在问题的“路标”。
