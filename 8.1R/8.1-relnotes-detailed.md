...
[newsyslog(8)](http://www.freebsd.org/cgi/man.cgi?query=newsyslog&sektion=8&manpath=FreeBSD+8.1-RELEASE) 工具现在不再将 PID 文件不存在视为错误。新的 `-P` 参数可以将其恢复到旧行为。
...
[unifdef(1)](http://www.freebsd.org/cgi/man.cgi?query=unifdef&sektion=1&manpath=FreeBSD+8.1-RELEASE) 工具已更新为版本 1.188。现在支持新的 `-B` 参数，用于压缩已删除部分周围的空行，以防止代码段周围的空行被重复。
...
[usbconfig(8)](http://www.freebsd.org/cgi/man.cgi?query=usbconfig&sektion=8&manpath=FreeBSD+8.1-RELEASE) 工具现在支持新的 `-d` 参数，用于指定 [ugen(4)](http://www.freebsd.org/cgi/man.cgi?query=ugen&sektion=4&manpath=FreeBSD+8.1-RELEASE) 设备，并支持命令 `add_quirk` 和 `remove_quirk`。
...
[zfs(8)](http://www.freebsd.org/cgi/man.cgi?query=zfs&sektion=8&manpath=FreeBSD+8.1-RELEASE) 命令现在支持新的 `receive -u` 参数，用于指定接收到的 ZFS 不应自动挂载。
...
rc.d/ipfw 脚本和 /etc/rc.firewall 现在支持 IPv6，而 rc.d/ip6fw 脚本和 /etc/rc.firewall6 已被弃用。请注意，[rc.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5&manpath=FreeBSD+8.1-RELEASE) 中的 `ipv6_firewall_*` 变量已被 `firewall_client_net_ipv6`、`firewall_simple_iif_ipv6`、`firewall_simple_inet_ipv6`、`firewall_simple_oif_ipv6`、`firewall_simple_onet_ipv6` 替代。
