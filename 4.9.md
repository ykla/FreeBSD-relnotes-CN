...
FreeBSD 4.9-RELEASE 的发布说明包含自 4.8-RELEASE 以来对 FreeBSD 基本系统所做更改的摘要。列出了内核和用户空间的更改，以及自上次发布以来发布的适用于基本系统的安全公告。还简要介绍了升级过程。

## 1 简介

本文档包含 FreeBSD 4.9-RELEASE 在 i386 硬件平台上的发布说明。它描述了自 4.8-RELEASE 以来添加（或更改）的 FreeBSD 新特性。同时提供了有关从以前版本升级的说明。
...
本节描述了自 4.8-RELEASE 以来，FreeBSD 中最显著的新特性或更改。典型的发布说明项目包括新驱动程序或硬件支持、新命令或选项、重要的 bug 修复或第三方的软件升级。同时列出了 4.8-RELEASE 后发布的基础系统安全公告。
...
修复了一个可能导致内核尝试传递无效信号的 bug，该 bug 可能导致内核恐慌。有关更多信息，请参见安全公告 [FreeBSD-SA-03:09](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-03:09.signal.asc)。
...
修复了一个 bug，该 bug 可能导致内核在 ARP 缓存代码中造成资源耗尽，最终导致系统恐慌。有关更多信息，请参见安全公告 [FreeBSD-SA-03:14](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-03:14.arp.asc)。
...
已添加驱动程序 [safe(4)](http://www.freebsd.org/cgi/man.cgi?query=safe&sektion=4&manpath=FreeBSD+4.9-RELEASE)，以支持基于 SafeNet 1141 和 1741 的加密加速器。
...
proatm 驱动程序已添加，以支持 ProSum 的 ProATM（基于 IDT77252）接口。此驱动程序类似于 FreeBSD-CURRENT 中的驱动程序 patm。

已添加网络驱动程序 [rue(4)](http://www.freebsd.org/cgi/man.cgi?query=rue&sektion=4&manpath=FreeBSD+4.9-RELEASE)，提供对基于 RealTek RTL8150 USB 到快速以太网控制器芯片的以太网适配器的支持。

已添加驱动程序 [sbsh(4)](http://www.freebsd.org/cgi/man.cgi?query=sbsh&sektion=4&manpath=FreeBSD+4.9-RELEASE)，以支持 Granch SBNI16 SHDSL 调制解调器。
...
修复了 [ipfw(4)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=4&manpath=FreeBSD+4.9-RELEASE) 限制规则处理中的一个 bug，该 bug 可能导致各种内核恐慌。
...
[ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+4.9-RELEASE) 现在支持 `-n` 参数，用于测试命令的语法，而不实际更改任何内容。
...
Intel ACPI 20030228 发行版（带有本地 FreeBSD 更改和支持代码）已被导入。此功能应被视为实验性，并且在部署到生产环境之前应进行测试。

> **注意：** 与 FreeBSD-CURRENT 不同，ACPI 驱动程序必须通过将设备 acpica 添加到内核配置中静态编译进内核。没有内核模块。此驱动程序不包含在默认的 GENERIC 内核中。
...
如果您正在从 FreeBSD 的早期版本升级，通常有三种选择：

* 使用 [sysinstall(8)](http://www.freebsd.org/cgi/man.cgi?query=sysinstall&sektion=8&manpath=FreeBSD+4.9-RELEASE) 的二进制升级方案。这个方法可能是最快的，尽管它假设您的 FreeBSD 安装没有使用任何特殊的编译选项。
* 执行完整的 FreeBSD 重新安装。严格来说，这不是一种升级方法，无论如何通常比二进制升级更不方便，因为它需要您手动备份和恢复 `/etc` 的内容。然而，在您需要（或想要）更改磁盘分区的情况下，这可能会有用。
...
请在开始升级之前阅读 `INSTALL.TXT` 文件，最好在此之前阅读。如果您从源代码升级，请确保阅读 `/usr/src/UPDATING` 文件。

最后，如果您想使用各种方式跟踪 FreeBSD 的 -STABLE 或 -CURRENT 分支，请务必查阅 [`-CURRENT 与 -STABLE'](http://www.freebsd.org/doc/en_US.ISO8859-1/books/handbook/current-stable.html) 章节，位于 [FreeBSD 手册](http://www.freebsd.org/doc/en_US.ISO8859-1/books/handbook/) 中。
