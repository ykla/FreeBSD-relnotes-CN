...
已从镜像上最初可用的镜像或通过 [freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 在最终版本重新构建前安装 FreeBSD 11.0-RELEASE 的用户，请立即将系统升级至 FreeBSD 11.0-RELEASE-p1。
...
EC2(TM) 用户请阅读 FreeBSD 11.0-RELEASE 的勘误说明，其中涉及在发布周期后期发现的一个问题，从早期 FreeBSD 版本升级时可能导致系统在引导过程中挂起。新的 EC2(TM) 安装不受影响，但运行早期版本的现有安装建议等待该问题在勘误通知中解决后再升级。
...
许多公司捐赠了设备、网络访问或人力时间来支持 FreeBSD 11.0 的发布工程活动，包括 FreeBSD 基金会、Yahoo!、NetApp、Internet Systems Consortium、ByteMark Hosting、Sentex Communications、New York Internet、Juniper Networks、NLNet Labs、iXsystems 和 Yandex。
