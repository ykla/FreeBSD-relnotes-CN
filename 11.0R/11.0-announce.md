...
* OpenSSH 已更新至 7.2p2。
* 为多个驱动新增了 802.11n 的额外无线支持。
* 默认情况下，[ifconfig(8)](https://man.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8&format=html) 实用工具会将无线接口的默认监管域设置为 FCC。因此，使用默认设置新建的无线接口违反各国特定法规的可能性降低。
...
从镜像站点最初提供的镜像或通过 [freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 在最终版本重新构建前安装 FreeBSD 11.0-RELEASE 的用户，强烈建议立即将系统升级至 FreeBSD 11.0-RELEASE-p1。
...
EC2(TM) 用户请阅读 FreeBSD 11.0-RELEASE 的勘误说明，其中涉及在发布周期后期发现的某个问题，从早期 FreeBSD 版本升级时可能导致系统在引导过程中挂起。新的 EC2(TM) 安装不受影响，但运行早期版本的现有安装建议等待该问题在勘误通知中解决后再升级。
