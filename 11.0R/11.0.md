...
[pciconf(8)](http://www.freebsd.org/cgi/man.cgi?query=pciconf&sektion=8&manpath=freebsd-release-ports) 实用程序现在可以识别已附加到驱动程序的 PCI 设备，并通过设备名称而不仅仅是选择器来标识它们。此外，`-l` 标志现在接受可选的设备参数，以列出单个设备的详细信息。[(r260910)](http://svn.freebsd.org/viewvc/base?view=revision&revision=260910)
...
[ps(1)](http://www.freebsd.org/cgi/man.cgi?query=ps&sektion=1&manpath=freebsd-release-ports) 实用程序已更新，包含 `-J` 标志，用于通过匹配 [jail(8)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8&manpath=freebsd-release-ports) ID 和名称过滤输出。此外，可以将参数 `0` 与 `-J` 一起使用，仅列出主机系统上运行的进程。[(r265229)](http://svn.freebsd.org/viewvc/base?view=revision&revision=265229)
...
[pmcstat(8)](http://www.freebsd.org/cgi/man.cgi?query=pmcstat&sektion=8&manpath=freebsd-release-ports) 实用程序已更新，新增了 `-l` 标志，用于在指定的秒数后结束事件收集。[(r266209)](http://svn.freebsd.org/viewvc/base?view=revision&revision=266209)
...
[mkimg(1)](http://www.freebsd.org/cgi/man.cgi?query=mkimg&sektion=1&manpath=freebsd-release-ports) 工具新增 `-c` 标志，允许指定目标磁盘映像的容量。[(r279139)](http://svn.freebsd.org/viewvc/base?view=revision&revision=279139)
...
[chflags(1)](http://www.freebsd.org/cgi/man.cgi?query=chflags&sektion=1&manpath=freebsd-release-ports)、[chgrp(1)](http://www.freebsd.org/cgi/man.cgi?query=chgrp&sektion=1&manpath=freebsd-release-ports)、[chmod(1)](http://www.freebsd.org/cgi/man.cgi?query=chmod&sektion=1&manpath=freebsd-release-ports) 和 [chown(8)](http://www.freebsd.org/cgi/man.cgi?query=chown&sektion=8&manpath=freebsd-release-ports) 工具现在当指定 `-R` 标志时会影响到符号链接，如 [symlink(7)](http://www.freebsd.org/cgi/man.cgi?query=symlink&sektion=7&manpath=freebsd-release-ports) 中所述。[(r282208)](http://svn.freebsd.org/viewvc/base?view=revision&revision=282208)（由 Multiplay 赞助）

[date(1)](http://www.freebsd.org/cgi/man.cgi?query=date&sektion=1&manpath=freebsd-release-ports) 工具已更新，以打印作为 `-r` 标志参数传递的文件的修改时间，提高了与 GNU [date(1)](http://www.freebsd.org/cgi/man.cgi?query=date&sektion=1&manpath=freebsd-release-ports) 工具行为的兼容性。[(r282608)](http://svn.freebsd.org/viewvc/base?view=revision&revision=282608)

[pw(8)](http://www.freebsd.org/cgi/man.cgi?query=pw&sektion=8&manpath=freebsd-release-ports) 工具已更新，新增了 `-R` 标志，用于设置工具操作的根目录。[(r283961)](http://svn.freebsd.org/viewvc/base?view=revision&revision=283961)
...
[jexec(8)](http://www.freebsd.org/cgi/man.cgi?query=jexec&sektion=8&manpath=freebsd-release-ports) 工具已更新，新增了 `-l` 标志，确保在目标 Jail 中使用时环境干净。此外，[jexec(8)](http://www.freebsd.org/cgi/man.cgi?query=jexec&sektion=8&manpath=freebsd-release-ports) 在未指定命令时会在目标 Jail 中运行一个 shell。[(r285420)](http://svn.freebsd.org/viewvc/base?view=revision&revision=285420)
...
[patch(1)](http://www.freebsd.org/cgi/man.cgi?query=patch&sektion=1&manpath=freebsd-release-ports) 工具已更新，为 `-V` 标志新增了一个选项 `none`，用于在应用补丁时禁用备份文件的创建。[(r285772)](http://svn.freebsd.org/viewvc/base?view=revision&revision=285772) (由 EMC / Isilon Storage Division 赞助)

[ar(1)](http://www.freebsd.org/cgi/man.cgi?query=ar&sektion=1&manpath=freebsd-release-ports) 工具现在默认启用确定性模式 (`-D`)。可以通过指定 `-U` 标志禁用此行为。[(r286010)](http://svn.freebsd.org/viewvc/base?view=revision&revision=286010) (由 FreeBSD 基金会赞助)

[xargs(1)](http://www.freebsd.org/cgi/man.cgi?query=xargs&sektion=1&manpath=freebsd-release-ports) 工具已更新，允许将 `0` 作为 `-P`（并行模式）标志的参数，从而允许创建尽可能多的并发进程。[(r286289)](http://svn.freebsd.org/viewvc/base?view=revision&revision=286289) (由 ScaleEngine, Inc. 赞助)
...
[uuencode(1)](http://www.freebsd.org/cgi/man.cgi?query=uuencode&sektion=1&manpath=freebsd-release-ports) 工具已更新，新增 `-r` 标志，在使用时将生成类似 [uudecode(1)](http://www.freebsd.org/cgi/man.cgi?query=uudecode&sektion=1&manpath=freebsd-release-ports) `-r` 标志的原始输出。[(r297678)](http://svn.freebsd.org/viewvc/base?view=revision&revision=297678)
...
[bsdinstall(8)](http://www.freebsd.org/cgi/man.cgi?query=bsdinstall&sektion=8&manpath=freebsd-release-ports) 新增了检测和实现对某些笔记本电脑和主板的解决方法的支持，这些笔记本电脑和主板无法从 GPT 分区的磁盘正常启动。此外，当需要时，`active` 标志将会设置在分区上。[(r285679)](http://svn.freebsd.org/viewvc/base?view=revision&revision=285679)（由 ScaleEngine, Inc. 赞助）
...
[bhyve(8)](http://www.freebsd.org/cgi/man.cgi?query=bhyve&sektion=8&manpath=freebsd-release-ports) 虚拟机管理程序已更新，支持虚拟 AHCI 磁盘的 `DSM TRIM` 命令。[(r279957)](http://svn.freebsd.org/viewvc/base?view=revision&revision=279957)
...
增加了通过 [shutdown(8)](http://www.freebsd.org/cgi/man.cgi?query=shutdown&sektion=8&manpath=freebsd-release-ports) `-p` 标志或调用 [poweroff(8)](http://www.freebsd.org/cgi/man.cgi?query=poweroff&sektion=8&manpath=freebsd-release-ports) 关闭 BeagleBone Black 系统的支持。[(r277042)](http://svn.freebsd.org/viewvc/base?view=revision&revision=277042)
