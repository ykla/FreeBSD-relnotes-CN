...
FreeBSD 11.0-RELEASE 的发行说明包含了对 11.0-STABLE 开发线上 FreeBSD 基本系统所做的变更摘要。本文档列出了自上次发行版以来发布的相关安全公告，以及对 FreeBSD 内核和用户空间的重大变更。此外还简要介绍了升级的相关事项。

## 引言

本文档包含 FreeBSD 11.0-RELEASE 的发行说明。它介绍了 FreeBSD 最近添加、修改或删除的功能，并提供了有关从先前版本的 FreeBSD 升级的一些说明。

此发行版的 FreeBSD 11.0-RELEASE 为 RELEASE 版。可以在 [https://www.FreeBSD.org/releases/](https://www.freebsd.org/releases/) 或其镜像站点找到。获取此（或其他）FreeBSD RELEASE 发行版的更多信息，请参阅 [FreeBSD 手册](https://docs.freebsd.org/en/books/handbook/) 中的 [获取 FreeBSD 附录](https://docs.freebsd.org/en/books/handbook/#mirrors)。

强烈建议所有用户在安装 FreeBSD 之前查阅发布的勘误文档。勘误文档会更新“最新”信息，这些信息是在发布周期后期或发布之后发现的。通常，它包含已知的 bug、安全公告以及文档更正。可以在 FreeBSD 官方网站上找到 FreeBSD 11.0-RELEASE 的最新勘误文档。

本文档介绍了自 10.3-RELEASE 以来 FreeBSD 中对用户可见的最主要的新增或变更功能。通常，本文所述的变更是特定于 11.0-STABLE 分支的，除非特别标记为已合并功能。

典型的发行说明条目包括 10.3-RELEASE 后发布的安全公告、新驱动程序或硬件支持、新命令或选项、主要的 bug 修复或第三方软件升级。它们还可能列出主要的 Port/软件包或发布工程实践的变更。显然，发行说明无法列出每个版本间对 FreeBSD 所做的所有变更；本文档主要集中在安全公告、用户可见的变更和主要的架构改进上。
...
基于源代码的升级（即从源代码重新编译 FreeBSD 基本系统）从先前版本开始即受支持，可以使用 **/usr/src/UPDATING** 中的说明进行。

通过 [freebsd-update(8)](http://www.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&manpath=freebsd-release-ports) 升级的相关信息，请参阅安装页面中的“二进制升级部分”。
...
[mailwrapper(8)](http://www.freebsd.org/cgi/man.cgi?query=mailwrapper&sektion=8&manpath=freebsd-release-ports) 实用程序已更新，改为从 `LOCALBASE` 环境变量指定的目录中读取 [mailer.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=mailer.conf&sektion=5&manpath=freebsd-release-ports)，如果未设置该变量，则默认值为 **/usr/local**。[(r270675)](http://svn.freebsd.org/viewvc/base?view=revision&revision=270675)

已删除 [src.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=src.conf&sektion=5&manpath=freebsd-release-ports) 选项 `MK_ARM_EABI`，EABI 现为 FreeBSD/arm 支持的唯一 ABI。[(r272350)](http://svn.freebsd.org/viewvc/base?view=revision&revision=272350)
...
[pciconf(8)](http://www.freebsd.org/cgi/man.cgi?query=pciconf&sektion=8&manpath=freebsd-release-ports) 实用程序现在可以识别已附加到驱动程序的 PCI 设备，并通过设备名称而不仅仅是选择器来标识它们。此外，标志 `-l` 现在接受可选的设备参数，以列出单个设备的详细信息。[(r260910)](http://svn.freebsd.org/viewvc/base?view=revision&revision=260910)

在 **/etc/ttys** 中新增了标志 `onifconsole`。这允许系统在设备为活动内核控制台时通过串行控制台提供登录提示，否则等同于 `off`。[(r260913)](http://svn.freebsd.org/viewvc/base?view=revision&revision=260913)
...
[ps(1)](http://www.freebsd.org/cgi/man.cgi?query=ps&sektion=1&manpath=freebsd-release-ports) 实用程序已更新，包含标志 `-J`，用于通过匹配 [jail(8)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8&manpath=freebsd-release-ports) ID 和名称过滤输出。此外，可以将参数 `0` 与 `-J` 一起使用，仅列出主机系统上运行的进程。[(r265229)](http://svn.freebsd.org/viewvc/base?view=revision&revision=265229)
...
[pmcstat(8)](http://www.freebsd.org/cgi/man.cgi?query=pmcstat&sektion=8&manpath=freebsd-release-ports) 实用程序已更新，新增了标志 `-l`，用于在指定的秒数后结束事件收集。[(r266209)](http://svn.freebsd.org/viewvc/base?view=revision&revision=266209)

[ps(1)](http://www.freebsd.org/cgi/man.cgi?query=ps&sektion=1&manpath=freebsd-release-ports) 实用程序已更新，新增关键字 `tracer`，用于显示跟踪进程的 PID。[(r270745)](http://svn.freebsd.org/viewvc/base?view=revision&revision=270745)
...
[primes(6)](http://www.freebsd.org/cgi/man.cgi?query=primes&sektion=6&manpath=freebsd-release-ports) 实用程序已更新，正确列出介于 `4295098369` 和 `3825123056546413050` 之间的质数。在此变更之前，返回值可能会被错误地识别为质数。[(r272166)](http://svn.freebsd.org/viewvc/base?view=revision&revision=272166)
...
已为 [dtrace(1)](http://www.freebsd.org/cgi/man.cgi?query=dtrace&sektion=1&manpath=freebsd-release-ports) 添加用户空间 [ctf(5)](http://www.freebsd.org/cgi/man.cgi?query=ctf&sektion=5&manpath=freebsd-release-ports) 支持。通过此变更，[dtrace(1)](http://www.freebsd.org/cgi/man.cgi?query=dtrace&sektion=1&manpath=freebsd-release-ports) 能够解析函数和 USDT 探针参数的类型信息以及函数返回值。[(r272488)](http://svn.freebsd.org/viewvc/base?view=revision&revision=272488)
...
[netstat(1)](http://www.freebsd.org/cgi/man.cgi?query=netstat&sektion=1&manpath=freebsd-release-ports) 工具已更新，使用 [libxo(3)](http://www.freebsd.org/cgi/man.cgi?query=libxo&sektion=3&manpath=freebsd-release-ports) 来可选地生成机器可读的输出。[(r279122)](http://svn.freebsd.org/viewvc/base?view=revision&revision=279122)（由 Juniper Networks, Inc. 赞助）

[mkimg(1)](http://www.freebsd.org/cgi/man.cgi?query=mkimg&sektion=1&manpath=freebsd-release-ports) 工具新增标志 `-c`，允许指定目标磁盘映像的容量。[(r279139)](http://svn.freebsd.org/viewvc/base?view=revision&revision=279139)
...
[chflags(1)](http://www.freebsd.org/cgi/man.cgi?query=chflags&sektion=1&manpath=freebsd-release-ports)、[chgrp(1)](http://www.freebsd.org/cgi/man.cgi?query=chgrp&sektion=1&manpath=freebsd-release-ports)、[chmod(1)](http://www.freebsd.org/cgi/man.cgi?query=chmod&sektion=1&manpath=freebsd-release-ports) 和 [chown(8)](http://www.freebsd.org/cgi/man.cgi?query=chown&sektion=8&manpath=freebsd-release-ports) 工具现在当指定标志 `-R` 时会影响到符号链接，如 [symlink(7)](http://www.freebsd.org/cgi/man.cgi?query=symlink&sektion=7&manpath=freebsd-release-ports) 中所述。[(r282208)](http://svn.freebsd.org/viewvc/base?view=revision&revision=282208)（由 Multiplay 赞助）

[date(1)](http://www.freebsd.org/cgi/man.cgi?query=date&sektion=1&manpath=freebsd-release-ports) 工具已更新，以打印作为标志 `-r` 参数传递的文件的修改时间，提高了与 GNU [date(1)](http://www.freebsd.org/cgi/man.cgi?query=date&sektion=1&manpath=freebsd-release-ports) 工具行为的兼容性。[(r282608)](http://svn.freebsd.org/viewvc/base?view=revision&revision=282608)

[pw(8)](http://www.freebsd.org/cgi/man.cgi?query=pw&sektion=8&manpath=freebsd-release-ports) 工具已更新，新增了标志 `-R`，用于设置工具操作的根目录。[(r283961)](http://svn.freebsd.org/viewvc/base?view=revision&revision=283961)

[lockstat(1)](http://www.freebsd.org/cgi/man.cgi?query=lockstat&sektion=1&manpath=freebsd-release-ports) 工具已更新，包含多项改进：[(r284297)](http://svn.freebsd.org/viewvc/base?view=revision&revision=284297)（由 ClusterHQ 赞助）

* 自旋锁现在报告自旋时间，而非循环迭代次数。
* 读取锁现被识别为可在 FreeBSD 上自旋的适应性锁。
* 现已报告成功的读取尝试锁事件的锁获取事件。
* 自旋和阻塞事件现已在锁获取事件之前报告。
...
[jexec(8)](http://www.freebsd.org/cgi/man.cgi?query=jexec&sektion=8&manpath=freebsd-release-ports) 工具已更新，新增了标志 `-l`，确保在目标 Jail 中使用时环境干净。此外，[jexec(8)](http://www.freebsd.org/cgi/man.cgi?query=jexec&sektion=8&manpath=freebsd-release-ports) 在未指定命令时会在目标 Jail 中运行一个 shell。[(r285420)](http://svn.freebsd.org/viewvc/base?view=revision&revision=285420)
...
[patch(1)](http://www.freebsd.org/cgi/man.cgi?query=patch&sektion=1&manpath=freebsd-release-ports) 工具已更新，为标志 `-V` 新增了一个选项 `none`，用于在应用补丁时禁用备份文件的创建。[(r285772)](http://svn.freebsd.org/viewvc/base?view=revision&revision=285772) (由 EMC / Isilon Storage Division 赞助)

[ar(1)](http://www.freebsd.org/cgi/man.cgi?query=ar&sektion=1&manpath=freebsd-release-ports) 工具现在默认启用确定性模式 (`-D`)。可以通过指定标志 `-U` 禁用此行为。[(r286010)](http://svn.freebsd.org/viewvc/base?view=revision&revision=286010) (由 FreeBSD 基金会赞助)

[xargs(1)](http://www.freebsd.org/cgi/man.cgi?query=xargs&sektion=1&manpath=freebsd-release-ports) 工具已更新，允许将 `0` 作为标志 `-P`（并行模式）的参数，从而允许创建尽可能多的并发进程。[(r286289)](http://svn.freebsd.org/viewvc/base?view=revision&revision=286289) (由 ScaleEngine, Inc. 赞助)
...
无线网络栈已修改，默认不再显示物理无线设备。要查看系统上可用的无线设备，请运行 `sysctl net.wlan.devices`。[(r287197)](http://svn.freebsd.org/viewvc/base?view=revision&revision=287197) (由 Netflix, Nginx, Inc. 赞助)
...
解析库已更新，当 **/etc/resolv.conf** 文件的修改时间发生变化时，会重新加载该文件。[(r289315)](http://svn.freebsd.org/viewvc/base?view=revision&revision=289315) (由 Dell, Inc. 赞助)

[uuencode(1)](http://www.freebsd.org/cgi/man.cgi?query=uuencode&sektion=1&manpath=freebsd-release-ports) 工具已更新，新增标志 `-r`，在使用时将生成类似 [uudecode(1)](http://www.freebsd.org/cgi/man.cgi?query=uudecode&sektion=1&manpath=freebsd-release-ports) 标志 `-r` 的原始输出。[(r297678)](http://svn.freebsd.org/viewvc/base?view=revision&revision=297678)

默认情况下，[ifconfig(8)](http://www.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8&manpath=freebsd-release-ports) 工具将无线接口的默认监管域设置为 `FCC`。因此，使用默认设置创建的新无线接口将更不可能违反国家特定法规。[(r300738)](http://svn.freebsd.org/viewvc/base?view=revision&revision=300738)
...
[ssh(1)](http://www.freebsd.org/cgi/man.cgi?query=ssh&sektion=1&manpath=freebsd-release-ports) 工具已更新，以重新实现主机名规范化，在 `known_hosts` 中定位主机之前进行处理。[(r285642)](http://svn.freebsd.org/viewvc/base?view=revision&revision=285642)（由 Dell, Inc. 赞助）
...
[libblacklist(3)](http://www.freebsd.org/cgi/man.cgi?query=libblacklist&sektion=3&manpath=freebsd-release-ports) 库和应用程序已从 NetBSD 项目移植。为 [pf(4)](http://www.freebsd.org/cgi/man.cgi?query=pf&sektion=4&manpath=freebsd-release-ports) 数据包过滤系统实现了数据包过滤支持。黑名单系统提供了 blacklistd 守护进程、用于修改正在运行的数据包过滤系统的帮助脚本 blacklistd-helper，以及 blacklistctl 控制程序。一些系统守护进程，包括 fingerd、ftpd、rlogind 和 rshd，已被修改以支持向 blacklistd 守护进程发送通知。[(r301169)](http://svn.freebsd.org/viewvc/base?view=revision&revision=301169)（由 FreeBSD 基金会赞助）
...
OpenSSH 默认禁用了对 DSA 的支持。[(r303719)](http://svn.freebsd.org/viewvc/base?view=revision&revision=303719)
...
[bsdconfig(8)](http://www.freebsd.org/cgi/man.cgi?query=bsdconfig&sektion=8&manpath=freebsd-release-ports) 工具已更新，当在虚拟机中运行时，将跳过初始的 [tzsetup(8)](http://www.freebsd.org/cgi/man.cgi?query=tzsetup&sektion=8&manpath=freebsd-release-ports) UTC 与墙上时钟时间提示，是否在虚拟机中运行通过 [sysctl(8)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&manpath=freebsd-release-ports) `kern.vm_guest` 是否设置为 `1` 来判断。[(r274394)](http://svn.freebsd.org/viewvc/base?view=revision&revision=274394)
...
[bsdinstall(8)](http://www.freebsd.org/cgi/man.cgi?query=bsdinstall&sektion=8&manpath=freebsd-release-ports) 新增了检测和实现 1Mb 边界对齐分区的支持。[(r285557)](http://svn.freebsd.org/viewvc/base?view=revision&revision=285557)（由 ScaleEngine, Inc. 赞助）

[bsdinstall(8)](http://www.freebsd.org/cgi/man.cgi?query=bsdinstall&sektion=8&manpath=freebsd-release-ports) 新增了检测和实现对某些笔记本电脑和主板的解决方法的支持，这些笔记本电脑和主板无法从 GPT 分区的磁盘正常启动。此外，当需要时，标志 `active` 将会设置在分区上。[(r285679)](http://svn.freebsd.org/viewvc/base?view=revision&revision=285679)（由 ScaleEngine, Inc. 赞助）
...
[bsdinstall(8)](http://www.freebsd.org/cgi/man.cgi?query=bsdinstall&sektion=8&manpath=freebsd-release-ports) 工具已更新，加入了在安装过程中的各种系统安全加固选项。[(r303447)](http://svn.freebsd.org/viewvc/base?view=revision&revision=303447)
...
[procctl(2)](http://www.freebsd.org/cgi/man.cgi?query=procctl&sektion=2&manpath=freebsd-release-ports) 系统调用已更新，包含一项功能，允许非 [init(8)](http://www.freebsd.org/cgi/man.cgi?query=init&sektion=8&manpath=freebsd-release-ports) 进程声明为子进程及其后代的清理进程。[(r275800)](http://svn.freebsd.org/viewvc/base?view=revision&revision=275800)（由 FreeBSD 基金会赞助）

新增了 `futimens()` 和 `utimensat()` 系统调用。更多信息请参见 [utimensat(2)](http://www.freebsd.org/cgi/man.cgi?query=utimensat&sektion=2&manpath=freebsd-release-ports)。[(r277610)](http://svn.freebsd.org/viewvc/base?view=revision&revision=277610)
...
多个标准头文件已更新，利用 gcc 属性，如 `__result_use_check()`、`__alloc_size()` 和 `__nonnull()`。[(r281130)](http://svn.freebsd.org/viewvc/base?view=revision&revision=281130)

已添加文件验证的 MAC 支持。[(r281845)](http://svn.freebsd.org/viewvc/base?view=revision&revision=281845)

`libgomp` 库现在仅在从基本系统构建 GCC 时才构建。最新版本可以在 Ports 中作为 `devel/libiomp5-devel` 获取。[(r282973)](http://svn.freebsd.org/viewvc/base?view=revision&revision=282973)（由 FreeBSD 基金会赞助）
...
堆栈保护器已升级为“strong（强）”级别，增强了对缓冲区溢出的保护。尽管这显著提高了系统安全性，但经过了广泛测试，确保不会对性能或功能产生可测量的副作用。[(r288669)](http://svn.freebsd.org/viewvc/base?view=revision&revision=288669)
...
修复了一个内核错误，该错误导致在启用 Turbo Boost™ 的 Intel® 处理器上 `dev.cpu.0.freq` [sysctl(8)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&manpath=freebsd-release-ports) 无法正常工作。[(r265876)](http://svn.freebsd.org/viewvc/base?view=revision&revision=265876)
...
[config(8)](http://www.freebsd.org/cgi/man.cgi?query=config&sektion=8&manpath=freebsd-release-ports) 工具已更新，允许使用非标准的 `src/` 树，该树作为参数 `-s` 的参数进行指定。[(r277904)](http://svn.freebsd.org/viewvc/base?view=revision&revision=277904)
...
为树莓派添加了模块，用于创建 `rpi.dtb`。[(r278338)](http://svn.freebsd.org/viewvc/base?view=revision&revision=278338)
...
为 ARM AM335x 系统添加了 `dtb` 模块创建模块。[(r279824)](http://svn.freebsd.org/viewvc/base?view=revision&revision=279824)
...
[amd64, i386] [nvd(4)](http://www.freebsd.org/cgi/man.cgi?query=nvd&sektion=4&manpath=freebsd-release-ports) 和 [nvme(4)](http://www.freebsd.org/cgi/man.cgi?query=nvme&sektion=4&manpath=freebsd-release-ports) 驱动程序现在默认包含在 **GENERIC** 内核配置中。[(r282921)](http://svn.freebsd.org/viewvc/base?view=revision&revision=282921)（由 Intel Corporation 赞助）

添加了内核配置选项 **EM_MULTIQUEUE**，它启用 [em(4)](http://www.freebsd.org/cgi/man.cgi?query=em&sektion=4&manpath=freebsd-release-ports) 驱动程序的多队列支持。[(r283959)](http://svn.freebsd.org/viewvc/base?view=revision&revision=283959)（由 Limelight Networks 赞助）
...
内核调试符号现在安装到 **/usr/lib/debug/boot/kernel/** 中。若要保留以前的行为，请在 [src.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=src.conf&sektion=5&manpath=freebsd-release-ports) 中添加 `KERN_DEBUGDIR=""`。[(r288176)](http://svn.freebsd.org/viewvc/base?view=revision&revision=288176)（由 FreeBSD 基金会赞助）

现在内核默认包含 POSIX 异步 I/O 支持。已删除内核选项 **VFS_AIO** 和内核模块 **aio.ko**。默认情况下，允许对套接字、本地文件和磁盘设备进行异步 I/O 操作。然而，对其他文件类型的操作则禁用。更多信息请参见 [aio(4)](http://www.freebsd.org/cgi/man.cgi?query=aio&sektion=4&manpath=freebsd-release-ports) 手册页。[(r296277)](http://svn.freebsd.org/viewvc/base?view=revision&revision=296277)（由 Chelsio Communications 赞助）
...
设备文件系统 [devfs(5)](http://www.freebsd.org/cgi/man.cgi?query=devfs&sektion=5&manpath=freebsd-release-ports) 已变更，以秒级精度更新读/写操作的时间戳。添加了 [sysctl(8)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&manpath=freebsd-release-ports) `vfs.devfs.dotimes`，当设置为非零值时，启用这些操作的默认精度时间戳。[(r280949)](http://svn.freebsd.org/viewvc/base?view=revision&revision=280949)（由 iXsystems 和 FreeBSD 基金会赞助）

添加了 [sysctl(8)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&manpath=freebsd-release-ports) `kern.racct.enable`，当设置为非零值时，允许在 **GENERIC** 内核上使用 [rctl(8)](http://www.freebsd.org/cgi/man.cgi?query=rctl&sektion=8&manpath=freebsd-release-ports)。同时也添加了内核配置选项 **RACCT_DISABLED**。[(r282213)](http://svn.freebsd.org/viewvc/base?view=revision&revision=282213)（由 FreeBSD 基金会赞助）
...
添加了 [sysctl(8)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&manpath=freebsd-release-ports) `net.inet.tcp.hostcache.purgenow`，当在运行时设置为 `1` 时，将刷新所有 `net.inet.tcp.hostcache` 条目。[(r283136)](http://svn.freebsd.org/viewvc/base?view=revision&revision=283136)（由 Limelight Networks 赞助）

添加了 [sysctl(8)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&manpath=freebsd-release-ports) `hw.model`，可显示 CPU 型号信息。[(r285524)](http://svn.freebsd.org/viewvc/base?view=revision&revision=285524)

[uart(4)](http://www.freebsd.org/cgi/man.cgi?query=uart&sektion=4&manpath=freebsd-release-ports) 驱动程序已更新，允许在运行时调整 CTS 线上捕获的每秒脉冲数，而之前无需重新构建内核即可使用的只有 DCD 线。[(r286591)](http://svn.freebsd.org/viewvc/base?view=revision&revision=286591)
...
已添加 [full(4)](http://www.freebsd.org/cgi/man.cgi?query=full&sektion=4&manpath=freebsd-release-ports) 设备，并移除了 **lindev(4)** 设备。在此变更之前，**lindev(4)** 仅提供 **/dev/full** 字符设备，在写入时返回 `ENOSPC` 错误。由于该设备并非 Linux® 专用，因此添加了 FreeBSD 原生版本。[(r265132)](http://svn.freebsd.org/viewvc/base?view=revision&revision=265132)
...
[psm(4)](http://www.freebsd.org/cgi/man.cgi?query=psm&sektion=4&manpath=freebsd-release-ports) 驱动程序已更新，改进了对较新款 Synaptics® 触控板以及较新款联想™笔记本电脑上 ClickPad® 鼠标的支持。[(r281440)](http://svn.freebsd.org/viewvc/base?view=revision&revision=281440)
...
已将对博通芯片组 BCM57764、BCM57767、BCM57782、BCM57786 和 BCM57787 的支持添加到 [bge(4)](http://www.freebsd.org/cgi/man.cgi?query=bge&sektion=4&manpath=freebsd-release-ports) 中。[(r258830)](http://svn.freebsd.org/viewvc/base?view=revision&revision=258830)
...
新增了 **if_nf10bmac(4)** 设备，支持 NetFPGA-10G 嵌入式 CPU 以太网核心。[(r264601)](http://svn.freebsd.org/viewvc/base?view=revision&revision=264601)（由 DARPA, AFRL 赞助）
...
新增了 [iwn(4)](http://www.freebsd.org/cgi/man.cgi?query=iwn&sektion=4&manpath=freebsd-release-ports) 驱动程序，支持 Intel® Centrino™ Wireless-N 105 和 135 芯片组。[(r266770)](http://svn.freebsd.org/viewvc/base?view=revision&revision=266770)
...
已变更 [pf(4)](http://www.freebsd.org/cgi/man.cgi?query=pf&sektion=4&manpath=freebsd-release-ports) 数据包过滤器的默认哈希算法，从 `Jenkins` 改为 `Murmur3`，每秒数据包处理性能提升了 3%。[(r272906)](http://svn.freebsd.org/viewvc/base?view=revision&revision=272906)
...
本节涉及物理机器、hypervisor 和虚拟化环境的通用硬件支持，以及与硬件相关的变更和更新，这些内容无法归入文档其他部分。
...
OpenCrypto 框架已更新，增加了 `AES-ICM` 和 `AES-GCM` 模式，这些模式也添加到了 [aesni(4)](http://www.freebsd.org/cgi/man.cgi?query=aesni&sektion=4&manpath=freebsd-release-ports) 驱动程序中。[(r275732)](http://svn.freebsd.org/viewvc/base?view=revision&revision=275732)（由 FreeBSD 基金会和 Netgate 赞助）
...
[uart(4)](http://www.freebsd.org/cgi/man.cgi?query=uart&sektion=4&manpath=freebsd-release-ports) 驱动程序已更新，支持较新系统上的 AMT 设备。

[arm64] FreeBSD/arm64 平台已增加初始的 SMP 支持。[(r285316)](http://svn.freebsd.org/viewvc/base?view=revision&revision=285316)（由 FreeBSD 基金会赞助）

[enc(4)](http://www.freebsd.org/cgi/man.cgi?query=enc&sektion=4&manpath=freebsd-release-ports) 驱动程序已更新，允许在运行时通过 [kldload(8)](http://www.freebsd.org/cgi/man.cgi?query=kldload&sektion=8&manpath=freebsd-release-ports) 创建接口，而无需额外的内核和/或用户空间变更。[(r291292)](http://svn.freebsd.org/viewvc/base?view=revision&revision=291292)（由 Yandex LLC 赞助）
...
已增加对 FreeBSD/amd64 Xen 客户实例以 PVH 模式运行的支持。PVH 模式是“Para-Virtualized Hardware”的缩写，使用 para-virtualized 驱动程序进行启动和 I/O，并使用硬件虚拟化扩展处理所有其他任务，无需仿真。[(r267536)](http://svn.freebsd.org/viewvc/base?view=revision&revision=267536)（由 Citrix Systems R&D 赞助）

[bhyve(8)](http://www.freebsd.org/cgi/man.cgi?query=bhyve&sektion=8&manpath=freebsd-release-ports) 虚拟机管理程序已更新，支持带有 SVM 和 AMD-V 硬件扩展的 AMD® 处理器。[(r273375)](http://svn.freebsd.org/viewvc/base?view=revision&revision=273375)
...
已引入对 PCI 单根 I/O 虚拟化（SR-IOV）的支持，允许为支持 SR-IOV 的设备驱动程序创建 PCI 虚拟功能（VF）。创建和配置 VF 的方法，请参见 [iovctl(8)](http://www.freebsd.org/cgi/man.cgi?query=iovctl&sektion=8&manpath=freebsd-release-ports)。[(r279463)](http://svn.freebsd.org/viewvc/base?view=revision&revision=279463)（由 Sandvine, Inc. 赞助）

[bhyve(8)](http://www.freebsd.org/cgi/man.cgi?query=bhyve&sektion=8&manpath=freebsd-release-ports) 虚拟机管理程序已更新，支持虚拟 AHCI 磁盘的命令 `DSM TRIM`。[(r279957)](http://svn.freebsd.org/viewvc/base?view=revision&revision=279957)

[arm] 已增加对 QEMU `virt` 系统的支持。[(r281439)](http://svn.freebsd.org/viewvc/base?view=revision&revision=281439)

Hyper-V™ 驱动程序已更新，包含多个增强功能：[(r282212)](http://svn.freebsd.org/viewvc/base?view=revision&revision=282212)（由 Microsoft Open Source Technology Center 赞助）
...
已删除对 [xen(4)](http://www.freebsd.org/cgi/man.cgi?query=xen&sektion=4&manpath=freebsd-release-ports) 半虚拟化 `domU` 内核的支持。[(r282274)](http://svn.freebsd.org/viewvc/base?view=revision&revision=282274)

[hv_netvsc(4)](http://www.freebsd.org/cgi/man.cgi?query=hv_netvsc&sektion=4&manpath=freebsd-release-ports) 驱动程序已更新，支持校验和卸载和 TSO。[(r284746)](http://svn.freebsd.org/viewvc/base?view=revision&revision=284746)（由 Microsoft Open Source Technology Center 赞助）
...
[bhyve(8)](http://www.freebsd.org/cgi/man.cgi?query=bhyve&sektion=8&manpath=freebsd-release-ports) 虚拟机管理程序已增加对本地图形的支持。[(r302332)](http://svn.freebsd.org/viewvc/base?view=revision&revision=302332)

### ARM 支持

[nand(4)](http://www.freebsd.org/cgi/man.cgi?query=nand&sektion=4&manpath=freebsd-release-ports) 设备默认对 ARM 设备启用。[(r260921)](http://svn.freebsd.org/viewvc/base?view=revision&revision=260921)
...
增加了通过 [shutdown(8)](http://www.freebsd.org/cgi/man.cgi?query=shutdown&sektion=8&manpath=freebsd-release-ports) 标志 `-p` 或调用 [poweroff(8)](http://www.freebsd.org/cgi/man.cgi?query=poweroff&sektion=8&manpath=freebsd-release-ports) 关闭 BeagleBone Black 系统的支持。[(r277042)](http://svn.freebsd.org/viewvc/base?view=revision&revision=277042)
...
增加了对 HiSilicon HI6220 SoC 的支持。[(r287371)](http://svn.freebsd.org/viewvc/base?view=revision&revision=287371)（由 ABT Systems, Ltd. 赞助）
...
默认启用 amd64、arm64 和 powerpc 的原生 PCIe 热插拔支持。此功能暴露了某些硬件上的兼容性问题，导致设备丢失或启动时挂起。为避免此类问题，请在引导加载程序中运行 `set hw.pci.enable_pcie_hp=0`，并将 `hw.pci.enable_pcie_hp=0` 添加到 **/boot/loader.conf** 中。[(r299142)](http://svn.freebsd.org/viewvc/base?view=revision&revision=299142)

## 存储

本节涉及文件系统和其他存储子系统的变更和新增功能，包括本地存储和网络存储。
...
[camdd(8)](http://www.freebsd.org/cgi/man.cgi?query=camdd&sektion=8&manpath=freebsd-release-ports) 工具已添加，允许在 SCSI 设备、文件、块设备和磁带驱动器之间顺序复制数据。如果源和/或目标是 SCSI 磁盘，[camdd(8)](http://www.freebsd.org/cgi/man.cgi?query=camdd&sektion=8&manpath=freebsd-release-ports) 可以使用异步的 [pass(4)](http://www.freebsd.org/cgi/man.cgi?query=pass&sektion=4&manpath=freebsd-release-ports) 接口来排队多个 I/O，从而提高速度。（[camdd(8)](http://www.freebsd.org/cgi/man.cgi?query=camdd&sektion=8&manpath=freebsd-release-ports) 的 ATA 直通支持正在开发中。）[(r291716)](http://svn.freebsd.org/viewvc/base?view=revision&revision=291716)（由 Spectra Logic 赞助）
...
[ahci(4)](http://www.freebsd.org/cgi/man.cgi?query=ahci&sektion=4&manpath=freebsd-release-ports) 驱动程序已更新，增加了对支持 NCQ TRIM 的驱动器的支持。[(r298002)](http://svn.freebsd.org/viewvc/base?view=revision&revision=298002)（由 Netflix 赞助）

>**注意**
>
> 已将那些宣传支持此功能但未能正确支持的驱动器列入黑名单。启用 NCQ TRIM 后出现流量问题的系统，可以将可调参数 `kern.cam.ada.%d.quirks` 设置为 `2`（适用于 512k 扇区）或 `3`（适用于 4096k 扇区），并将 `%d` 替换为驱动器编号。

[cam(4)](http://www.freebsd.org/cgi/man.cgi?query=cam&sektion=4&manpath=freebsd-release-ports) 驱动程序已更新，允许根据工作负载和驱动器特性调优 I/O 调度。此选项默认为关闭，可以通过将 `option CAM_IOSCHED_ADAPTIVE` 添加到内核配置并重新编译内核来启用。[(r298002)](http://svn.freebsd.org/viewvc/base?view=revision&revision=298002)（由 Netflix 赞助）

[camcontrol(8)](http://www.freebsd.org/cgi/man.cgi?query=camcontrol&sektion=8&manpath=freebsd-release-ports) 命令可以通过 `reprobe` 子命令在磁盘调整大小后手动强制更新容量数据。[(r299371)](http://svn.freebsd.org/viewvc/base?view=revision&revision=299371)（由 FreeBSD 基金会赞助）

在填充 CAM 序列号时，SCSI 磁盘序列号的前导空格现已去除。这影响了 [diskinfo(8)](http://www.freebsd.org/cgi/man.cgi?query=diskinfo&sektion=8&manpath=freebsd-release-ports) 的输出和 **/dev/diskid/DISK-*** 设备节点的名称等内容。[(r300880)](http://svn.freebsd.org/viewvc/base?view=revision&revision=300880)（由 Spectra Logic 赞助）
...
新增了在不附加到目标的情况下发现 iSCSI 目标的功能，支持在命令 [iscsictl(8)](http://www.freebsd.org/cgi/man.cgi?query=iscsictl&sektion=8&manpath=freebsd-release-ports) 中使用。[(r301033)](http://svn.freebsd.org/viewvc/base?view=revision&revision=301033)（由 FreeBSD 基金会赞助）
...
已添加 zfsd 守护进程，用于管理热备盘以及在发布物理路径的驱动器槽中进行替换。[(r300906)](http://svn.freebsd.org/viewvc/base?view=revision&revision=300906)（由 iXsystems、Spectra Logic 赞助）
...
已更新 [gpart(8)](http://www.freebsd.org/cgi/man.cgi?query=gpart&sektion=8&manpath=freebsd-release-ports) 工具，新增了 GPT 分区的新属性 `lenovofix`，当设置时，能够规避在几款联想™笔记本上报告的 BIOS 兼容性问题。[(r285594)](http://svn.freebsd.org/viewvc/base?view=revision&revision=285594)（由 ScaleEngine, Inc. 赞助）

## 引导加载程序变更

本节涉及引导加载程序、引导菜单和其他与引导相关的变更。
...
引导加载程序已更新，以支持在加载内核之前输入 GELI 密码短语。要启用此行为，请在 [loader.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=loader.conf&sektion=5&manpath=freebsd-release-ports) 中添加 `geom_eli_passphrase_prompt="YES"`。[(r281616)](http://svn.freebsd.org/viewvc/base?view=revision&revision=281616)
...
已移除对 IPX 网络传输协议的支持，并且在 FreeBSD 11 及以后的版本中不再支持。[(r263140)](http://svn.freebsd.org/viewvc/base?view=revision&revision=263140)

已为 [tcp(4)](http://www.freebsd.org/cgi/man.cgi?query=tcp&sektion=4&manpath=freebsd-release-ports) 堆栈添加了对 PLPMTUD 黑洞检测（RFC 4821）的支持，默认禁用。新增的可调参数如下：[(r272720)](http://svn.freebsd.org/viewvc/base?view=revision&revision=272720)（由 Limelight Networks 赞助）
...
已添加对原子 IP 数据报标识的支持（RFC 6864）。可以通过 [sysctl(8)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&manpath=freebsd-release-ports) `net.inet.ip.rfc6864` 启用或禁用此功能，默认启用。[(r280971)](http://svn.freebsd.org/viewvc/base?view=revision&revision=280971)（由 Netflix, Nginx, Inc. 赞助）
...
网络栈已更新，以修复 IPv6 On-Link 重定向的处理问题。[(r287798)](http://svn.freebsd.org/viewvc/base?view=revision&revision=287798)（由 Dell, Inc. 赞助）

已添加将根目录重新挂载到 NFSv4 卷的支持。[(r299848)](http://svn.freebsd.org/viewvc/base?view=revision&revision=299848)（由 FreeBSD 基金会赞助）
