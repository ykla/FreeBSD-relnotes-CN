...
[ahc(4)](https://man.freebsd.org/cgi/man.cgi?query=ahc&sektion=4&format=html) 驱动支持以下 `SCSI` host 适配器芯片和 `SCSI` 控制器卡：
...
光纤通道控制器芯片组支持多种速度和系统。Apple 光纤通道 HBA 实际上是 FC949ES 卡。
...
注意，此硬件仅对 ATA DMA 命令支持命令队列和基于 FIS 的切换。ATAPI 和非 DMA ATA 命令按每个端口逐一执行。
...
当前版本的 `atse` 驱动支持以太网 MegaCore，如 Altera 文档版本 11.1 中所述，当设备配置有内部 FIFO 时。
...
大多数 Intel 以太网 700 系列中带有 SFP+/SFP28/QSFP+ 接口的适配器，其固件要求使用 Intel 认证模块；这些认证模块列在下面。驱动无法禁用该认证检查。
...
[msk(4)](https://man.freebsd.org/cgi/man.cgi?query=msk&sektion=4&format=html) 驱动为基于 Marvell/SysKonnect Yukon II 千兆以太网控制器芯片的各种网卡提供支持，包括：
...
[my(4)](https://man.freebsd.org/cgi/man.cgi?query=my&sektion=4&format=html) 驱动为基于 Myson 芯片组的各种网卡提供支持。支持的型号包括：
...
（参见 **/sys/dev/usb/serial/u3g.c** 了解上述每个供应商所支持卡片的完整列表。）
...
**uhso** 驱动应能与 Option 的大多数设备配合工作。以下设备经验证可正常工作
...
**rtsx** 驱动支持不同规范兼容的芯片。以下芯片经验证可正常工作：
...
**sdhci** 驱动支持不同规格的兼容芯片。以下芯片经验证可正常工作：
