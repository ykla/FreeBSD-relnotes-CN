...
本发行说明适用于“RELEASE”，此 RELEASE 代表了开发分支 13-STABLE 自创建以来的最新状态。此分支上预编译的二进制“RELEASE”的信息，请访问 [https://www.FreeBSD.org/releases/](https://www.freebsd.org/releases/)。
...
FreeBSD 13.4-RELEASE 是个“RELEASE”，可在 [https://www.FreeBSD.org/releases/](https://www.freebsd.org/releases/) 及其镜像站找到。要获取此版本（或其他 FreeBSD“发行版”）的更多信息，请参阅 [FreeBSD 手册](https://docs.freebsd.org/en/books/handbook/) 的 [获取 FreeBSD 附录](https://docs.freebsd.org/en/books/handbook/mirrors)。

建议用户在安装 FreeBSD 之前，先查阅发行勘误（errata）。勘误文件包含在发布周期后期及发布后发现的最新信息，通常包括已知错误、安全公告和文档修正。可以在 FreeBSD 官方网站上找到 FreeBSD 13.4-RELEASE 的最新勘误。

本文件概述了自 13.3-RELEASE 以来对用户最具影响的新功能或变更。通常，本文所述的更改仅适用于 13-STABLE 分支，除非特别标注为 MERGED（已合并）的功能。

典型的发行说明内容包括 13.3-RELEASE 之后发布的安全公告、新增的驱动或硬件支持、新命令或选项、重要的错误修复、第三方软件的升级，可能还涉及主要 Ports/软件包的变更或发布工程实践的调整。当然，发行说明无法列出 FreeBSD 在不同版本之间的所有更改，而是主要关注安全公告、用户可见的变更及重要的架构改进。

## 从旧版 FreeBSD 升级

在不同的 RELEASE 版本（以及各安全分支的快照）之间进行二进制升级，可使用 [freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 工具。请参阅特定于此发行版的升级流程：[FreeBSD 13.4-RELEASE 升级信息](https://www.freebsd.org/releases/13.4R/installation/#upgrade-binary)，并在 FreeBSD 手册的 [二进制升级流程](https://docs.freebsd.org/en/books/handbook/cutting-edge/#freebsdupdate-upgrade) 中了解更多详情。该工具可更新未经修改的用户空间工具以及官方 FreeBSD 发行版中提供的未经修改的 **GENERIC** 内核。[freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 需要升级主机具有 Internet 连接。

从旧版本基于源代码的升级（即从源代码重新编译 FreeBSD 基本系统）同样受支持，具体方法请参阅 **/usr/src/UPDATING** 文件中的说明。
...
| [FreeBSD-SA-24:06.ktrace](https://www.freebsd.org/security/advisories/FreeBSD-SA-24:06.ktrace.asc) | 2024 年 8 月 7 日 | [ktrace(2)](https://man.freebsd.org/cgi/man.cgi?query=ktrace&sektion=2&format=html) 在执行 setuid 二进制文件时未能分离 |
...
| [FreeBSD-SA-24:12.bhyve](https://www.freebsd.org/security/advisories/FreeBSD-SA-24:12.bhyve.asc) | 2024 年 9 月 4 日 | [bhyve(8)](https://man.freebsd.org/cgi/man.cgi?query=bhyve&sektion=8&format=html) USB 控制器导致特权级虚拟机逃逸 |
...
| [FreeBSD-EN-24:11.ldns](https://www.freebsd.org/security/advisories/FreeBSD-EN-24:11.ldns.asc) | 2024 年 6 月 19 日 | LDNS 仍使用 **resolv.conf** 中被注释的 nameserver |
| [FreeBSD-EN-24:12.killpg](https://www.freebsd.org/security/advisories/FreeBSD-EN-24:12.killpg.asc) | 2024 年 6 月 19 日 | killpg 发生锁顺序反转导致活锁 |
| [FreeBSD-EN-24:13.libc++](https://www.freebsd.org/security/advisories/FreeBSD-EN-24:13.libc++.asc) | 2024 年 6 月 19 日 | 在 delete 堆分配的 `std::string` 时传递了错误的大小 |
...
本节包含了对用户空间应用程序、第三方软件和系统实用工具的更改和新增内容。
...
- `sqlite3` 升级至 3.46.0。
- `OpenSSH` 升级至 9.7p1。该版本主要包含错误修复。此外，它将 DSA 签名算法的支持改为编译时可选，计划在今年晚些时候在上游禁用，并在 2025 年完全移除该支持。
- LLVM 及 `clang` 编译器升级至 18.1.5。
- `bc` 更新至 6.7.6。
- `atf` 更新至 0.22 快照 55c21b2c。
- `libarchive` 更新至 3.7.4。
- `capsicum-test` 更新至快照 eab7a83b。
...
- [irdma(4)](https://man.freebsd.org/cgi/man.cgi?query=irdma&sektion=4&format=html) 驱动更新。
- [ice(4)](https://man.freebsd.org/cgi/man.cgi?query=ice&sektion=4&format=html) 驱动更新。
- [u3g(4)](https://man.freebsd.org/cgi/man.cgi?query=u3g&sektion=4&format=html) 驱动新增对 SIM7600G 的支持。
- 原生及基于 LinuxKPI 的无线驱动程序修复了许多稳定性问题。（由 FreeBSD 基金会赞助）
...
对网络协议栈进行了许多改进，包括性能优化及 [sctp(4)](https://man.freebsd.org/cgi/man.cgi?query=sctp&sektion=4&format=html) 协议栈的错误修复。具体而言，回环接口（loopback）新增了对 SCTP 校验和卸载功能的支持。
...
移除了对传统 `disklabel` 实用工具的引用，改用 `gpart`。后续版本的 FreeBSD 将彻底移除 `disklabel` 工具。
...
- DVD 软件包集进行了现代化改造。
- [archivers/unzip](https://cgit.freebsd.org/ports/tree/archivers/unzip/) 移除，因为它现已包含在基本系统中。
- [emulators/linux_base-c7](https://cgit.freebsd.org/ports/tree/emulators/linux_base-c7/) 移除，因为如果不安装其他 Linux 软件包，它几乎没有用处。
- [ports-mgmt/portmaster](https://cgit.freebsd.org/ports/tree/ports-mgmt/portmaster/) 移除，官方更推荐使用 `pkg` 和二进制软件包。
- [x11-drivers/xf86-video-vmware](https://cgit.freebsd.org/ports/tree/x11-drivers/xf86-video-vmware/) 移除，因为它对当前版本的 xorg-server 不再有用。
- [devel/git](https://cgit.freebsd.org/ports/tree/devel/git/) 由 [devel/git@lite](https://cgit.freebsd.org/ports/tree/devel/git/) 替代，因为后者足以满足大多数需求。
- [sysutils/seatd](https://cgit.freebsd.org/ports/tree/sysutils/seatd/) 和 [x11-wm/sway](https://cgit.freebsd.org/ports/tree/x11-wm/sway/) 新增，以支持 Wayland。

## 关于后续 FreeBSD 版本的一般说明

FreeBSD 15.0 预计不会支持除 armv7 以外的 32 位平台。armv6、i386 和 powerpc 平台已弃用，并将被移除。64 位系统仍然可以运行旧的 32 位二进制文件。

FreeBSD 项目计划在 FreeBSD 15.0 和 stable/15 中将 armv7 作为二级架构提供支持。然而，项目预计可能会在 FreeBSD 16.0 中移除 armv7。项目将在 15.0 发布时提供有关 15.x 和 16.x 版本中 armv7 状态的最新信息。

在 64 位平台上通过选项 `COMPAT_FREEBSD32` 执行 32 位二进制文件的支持将至少持续到 stable/15 和 stable/16 分支。此外，使用 `cc -m32` 编译单独的 32 位应用程序的支持也将至少持续到 stable/15 分支，该分支仍包含适当的头文件（位于 **/usr/include**）和库（位于 **/usr/lib32**）。

自 FreeBSD 15.0 起，Ports 将不再支持已弃用的 32 位平台。这些未来版本将不会包含这些平台的二进制软件包，也不支持从 Ports 构建软件包。

FreeBSD stable/14 及更早的分支仍将保留现有的 32 位内核和用户空间支持。在 stable/14 及更早的分支中，Ports 仍将支持构建 32 位系统的 Ports 和软件包，前提是这些分支仍受 Ports 支持。然而，所有 32 位平台均为二级架构或三级架构，随着上游软件逐步弃用 32 位平台，各个 Port 的支持可能会逐渐下降。
...
当 FreeBSD 15.0 发布时，项目可能会调整计划，在 15.0 或更高版本中为部分已弃用的平台提供一定程度的扩展支持。任何调整都将取决于社区反馈以及对这些平台的实际支持投入。
