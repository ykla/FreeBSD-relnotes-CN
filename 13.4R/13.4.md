...
典型的发行说明内容包括 13.3-RELEASE 之后发布的安全公告、新增的驱动或硬件支持、新命令或选项、重要的错误修复、贡献软件的升级，可能还涉及主要 Ports/软件包 的变更或发布工程实践的调整。当然，发行说明无法列出 FreeBSD 在不同版本之间的所有更改，而是主要关注安全公告、用户可见的变更及重要的架构改进。
...
- `sqlite3` 已升级至 3.46.0。
- `OpenSSH` 已升级至 9.7p1。该版本主要包含错误修复。此外，它将 DSA 签名算法的支持改为编译时可选，计划在今年晚些时候在上游禁用，并在 2025 年完全移除该支持。
...
- [irdma(4)](https://man.freebsd.org/cgi/man.cgi?query=irdma&sektion=4&format=html) 驱动已更新。
- [ice(4)](https://man.freebsd.org/cgi/man.cgi?query=ice&sektion=4&format=html) 驱动已更新。
- 驱动 [u3g(4)](https://man.freebsd.org/cgi/man.cgi?query=u3g&sektion=4&format=html) 新增对 SIM7600G 的支持。
- 原生及基于 LinuxKPI 的无线驱动程序已修复许多稳定性问题。（由 FreeBSD 基金会赞助）
...
对传统 `disklabel` 实用工具的引用已移除，改用 `gpart`。后续版本的 FreeBSD 将彻底移除工具 `disklabel`。
...
- DVD 软件包集已现代化。
- [archivers/unzip](https://cgit.freebsd.org/ports/tree/archivers/unzip/) 已移除，因为它现已包含在基本系统中。
- [emulators/linux_base-c7](https://cgit.freebsd.org/ports/tree/emulators/linux_base-c7/) 已移除，因为如果不安装其他 Linux 软件包，它几乎没有用处。
- [ports-mgmt/portmaster](https://cgit.freebsd.org/ports/tree/ports-mgmt/portmaster/) 已移除，官方更推荐使用 `pkg` 和二进制软件包。
- [x11-drivers/xf86-video-vmware](https://cgit.freebsd.org/ports/tree/x11-drivers/xf86-video-vmware/) 已移除，因为它对当前版本的 xorg-server 不再有用。
...
FreeBSD 15.0 预计不会支持除 armv7 以外的 32 位平台。armv6、i386 和 powerpc 平台已弃用，并将在 15.0 中移除。64 位系统仍然可以运行旧的 32 位二进制文件。
...
FreeBSD stable/14 及更早的分支仍将保留现有的 32 位内核和用户空间支持。在 stable/14 及更早的分支中，Ports 仍将支持构建 32 位系统的 Ports 和软件包，前提是这些分支仍受 Ports 支持。然而，所有 32 位平台均为二级架构或三级架构，随着上游软件逐步弃用 32 位平台，部分 Port 的支持可能会逐渐下降。
