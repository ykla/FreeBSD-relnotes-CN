...
程序 [ppp(8)](http://www.freebsd.org/cgi/man.cgi?query=ppp&sektion=8&manpath=FreeBSD+5.4-RELEASE) 现在实现了 `echo` 参数，可以独立启用 LCP ECHO，而不依赖 LQR 报告。旧版本的 [ppp(8)](http://www.freebsd.org/cgi/man.cgi?query=ppp&sektion=8&manpath=FreeBSD+5.4-RELEASE) 在协商失败时会回退到 LCP ECHO 模式。现在需要指定 `enable echo` 才能获得此行为。
