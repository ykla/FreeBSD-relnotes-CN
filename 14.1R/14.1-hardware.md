# FreeBSD 14.1-RELEASE 硬件兼容列表

## 引言

本硬件兼容性说明包含 FreeBSD 14.1 的硬件兼容性信息。其中列出了 FreeBSD 支持的硬件平台以及各类硬件设备（存储控制器、网络接口等），并给出已知可正常工作的设备实例。
...
FreeBSD 具备完善的 ARM 架构支持，在各种基于 ARM 的设备上提升了兼容性和性能。
...
* IBM POWER4 及更高版本（使用 pseries VM 平台）
...
[ahci(4)](https://man.freebsd.org/cgi/man.cgi?query=ahci&sektion=4&format=html) 驱动还支持作为 PCI 桥接 **nvme(4)** 的 AHCI 设备，使用 Intel Rapid Storage Technology（RST）。要使用 **nvme(4)** 设备，必须在 BIOS 中将 SATA 模式设置为 AHCI（从 RST 切换），或者接受因中断共享导致 RST 启用时的性能。FreeBSD 会自动检测处于 RST 模式且具有此扩展的 AHCI 设备。当检测到时，[ahci(4)](https://man.freebsd.org/cgi/man.cgi?query=ahci&sektion=4&format=html) 会挂载 **nvme(4)** 子设备到 [ahci(4)](https://man.freebsd.org/cgi/man.cgi?query=ahci&sektion=4&format=html) 设备。
...
[hptiop(4)](https://man.freebsd.org/cgi/man.cgi?query=hptiop&sektion=4&format=html) 驱动还支持以下 SAS 和 SATA RAID 控制器，这些控制器已经停产：
...
[isci(4)](https://man.freebsd.org/cgi/man.cgi?query=isci&sektion=4&format=html) 驱动为 Intel C600 提供支持 `SAS` 控制器。
...
Qlogic 267x/836x（又名 2031/8031）

> 16Gb FC/FCoE PCIe 卡。

Qlogic 2690/2692/2694（又名 2684/2692）

> 16Gb 光纤通道 PCIe 卡。

Qlogic 2740/2742/2764（又名 2722/2714）

> 32Gb 光纤通道 PCIe 卡。

Qlogic QLE2770/QLE2772（又名 2812）

> 32Gb 光纤通道 PCIe 卡。

Qlogic QLE2774（又名 2814）

> 32Gb 光纤通道 PCIe 卡。

Qlogic QLE2870/QLE2872（又名 2812）

> 64Gb 光纤通道 PCIe 卡。

Qlogic QLE2874（又名 2814）
...
支持所有主要固件版本（2.x、3.x、4.x 和 5.x），但是始终建议升级到控制器可用的最新固件版本。

未列出的兼容 Mylex 控制器应当可用，但尚未经过验证。
...
当前版本的 [atse(4)](https://man.freebsd.org/cgi/man.cgi?query=atse&sektion=4&format=html) 驱动支持 Ethernet MegaCore，如版本 11.1 的 Altera 文档所述，当设备配置为内部 FIFO 时。
...
[axe(4)](https://man.freebsd.org/cgi/man.cgi?query=axe&sektion=4&format=html) 驱动支持 ASIX Electronics AX88172/AX88178/AX88772/AX88772A/AX88772B/AX88760 基于 USB 的以太网适配器，包括：
...
[bce(4)](https://man.freebsd.org/cgi/man.cgi?query=bce&sektion=4&format=html) 驱动为基于 QLogic NetXtreme II 千兆以太网控制器系列提供支持，包括以下设备：
...
[bge(4)](https://man.freebsd.org/cgi/man.cgi?query=bge&sektion=4&format=html) 驱动为基于 Broadcom BCM570x 千兆以太网控制器芯片系列提供支持，包括以下设备：
...
[bnxt(4)](https://man.freebsd.org/cgi/man.cgi?query=bnxt&sektion=4&format=html) 驱动为基于 Broadcom NetXtreme-C 和 NetXtreme-E 千兆以太网控制器芯片系列提供支持，包括以下设备：
...
[bxe(4)](https://man.freebsd.org/cgi/man.cgi?query=bxe&sektion=4&format=html) 驱动为基于 QLogic NetXtreme II 10Gb 以太网控制器芯片系列提供支持，包括以下设备：
...
[cxgbev(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbev&sektion=4&format=html) 驱动支持 100Gb 和 25Gb 以太网适配器上的 Virtual Functions（基于 T6 ASIC）：
...
[cxgbev(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbev&sektion=4&format=html) 驱动支持 40Gb、10Gb 和 1Gb 以太网适配器上的 Virtual Functions（基于 T5 ASIC）：
...
[cxgbev(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbev&sektion=4&format=html) 驱动支持基于 10Gb 和 1Gb 以太网适配器的 Virtual Functions（基于 T4 ASIC）：
...
[em(4)](https://man.freebsd.org/cgi/man.cgi?query=em&sektion=4&format=html) 驱动支持基于 Intel 的千兆以太网适配器 82540, 82541ER, 82541PI, 82542, 82543, 82544, 82545, 82546, 82546EB, 82546GB, 82547, 82571, 82572, 82573, 82574, 82575, 82576, and 82580 控制器芯片：
...
用于 Elastic Network Adapter（ENA）系列的 FreeBSD 内核驱动（[ena(4)](https://man.freebsd.org/cgi/man.cgi?query=ena&sektion=4&format=html) 驱动）
...
[genet(4)](https://man.freebsd.org/cgi/man.cgi?query=genet&sektion=4&format=html) 驱动支持 Broadcom BCM2711 的以太网控制器部分（用于 树莓派 4 Model B 及相关系统）。它采用 BCM54213PE PHY。
...
[ice(4)](https://man.freebsd.org/cgi/man.cgi?query=ice&sektion=4&format=html) 驱动支持 Intel Ethernet 800 系列。此系列中带 SFP28/QSFP28 接口的大多数适配器其固件要求使用 Intel 认证模块；这些认证模块如下所列。此限定检查无法由驱动禁用。
...
[ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 驱动支持 Intel Ethernet 700 系列。此系列中带 SFP+/SFP28/QSFP+ 接口的大多数适配器其固件要求使用 Intel 认证模块；这些认证模块如下所列。此限定检查无法由驱动禁用。
...
注意，适配器还支持所有无源和有源限制性直连电缆，符合 SFF-8431 v4.1 和 SFF-8472 v10.4 规范。

此列表并非详尽无遗；请查阅产品文档以获取最新的支持介质列表。
...
*	ConnectX-4 LX 支持 10/25/40/50Gb/s 速率，功耗更低。

[mlx5ib(4)](https://man.freebsd.org/cgi/man.cgi?query=mlx5ib&sektion=4&format=html) 驱动支持 100Gb、50Gb、40Gb、25Gb 和 10Gb 网络适配器。ConnectX-4 支持：10/20/25/40/50/56/100Gb/s 速率。ConnectX-4 LX 支持：10/25/40/50Gb/s 速率（功耗更低）：
...
[msk(4)](https://man.freebsd.org/cgi/man.cgi?query=msk&sektion=4&format=html) 驱动为基于 Marvell/SysKonnect 的各种 NIC 提供支持 Yukon II 千兆以太网控制器芯片，包括：
...
[mxge(4)](https://man.freebsd.org/cgi/man.cgi?query=mxge&sektion=4&format=html) 驱动支持基于 Myricom LANai Z8E 芯片：
...
[qlnxe(4)](https://man.freebsd.org/cgi/man.cgi?query=qlnxe&sektion=4&format=html) 驱动支持基于以下的 25/40/100 千兆以太网和 CNA 适配器芯片组：

*	QLogic 45000 series
*	QLogic 41000 series

[qlxgb(4)](https://man.freebsd.org/cgi/man.cgi?query=qlxgb&sektion=4&format=html) 驱动支持基于以下的 10 千兆以太网和 CNA 适配器芯片组：

*	QLogic 3200 series
*	QLogic 8200 series

[qlxgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=qlxgbe&sektion=4&format=html) 驱动支持基于以下的 10 千兆以太网和 CNA 适配器芯片组：

*	QLogic 8300 series

[qlxge(4)](https://man.freebsd.org/cgi/man.cgi?query=qlxge&sektion=4&format=html) 驱动支持基于以下的 10 千兆以太网和 CNA 适配器芯片组：

*	QLogic 8100 series

[re(4)](https://man.freebsd.org/cgi/man.cgi?query=re&sektion=4&format=html) 驱动支持 RealTek RTL8139C+、RTL8169、RTL816xS、RTL811xS、RTL8168、RTL810xE 和 RTL8111 快速以太网及千兆以太网适配器，包括：
...
[sfxge(4)](https://man.freebsd.org/cgi/man.cgi?query=sfxge&sektion=4&format=html) 驱动支持所有基于 Solarflare SFC9000 的 10Gb 以太网适配器系列控制器。
...
[sis(4)](https://man.freebsd.org/cgi/man.cgi?query=sis&sektion=4&format=html) 驱动支持基于 Silicon Integrated Systems SiS 900 和 SiS 7016 的快速以太网适配器及嵌入式控制器，以及基于 National Semiconductor DP83815 (MacPhyter) 和 DP83816 芯片的快速以太网适配器。支持的适配器包括：
...
[ste(4)](https://man.freebsd.org/cgi/man.cgi?query=ste&sektion=4&format=html) 驱动支持基于 Sundance Technologies ST201 的快速以太网适配器及嵌入式控制器，包括：

*	D-Link DFE-530TXS
*	D-Link DFE-550TX
*	D-Link DFE-580TX

[stge(4)](https://man.freebsd.org/cgi/man.cgi?query=stge&sektion=4&format=html) 驱动为基于 Sundance/Tamarack 的各种 NIC 提供支持 TC9021 千兆以太网控制器芯片，包括：
...
[ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动支持基于 Alteon Tigon I 和 II 芯片。[ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动已在以下适配器上测试：
...
控制器的增强版本（eTSEC），集成在以下设备中，也受此驱动支持：
...
该设备具有一个称为“Zero-CD”的模式，其中包含 Microsoft Windows 的驱动程序；这是设备的默认模式。[uhso(4)](https://man.freebsd.org/cgi/man.cgi?query=uhso&sektion=4&format=html) 驱动自动将设备从“Zero-CD”模式切换到调制解调器模式。此行为可通过设置 `hw.usb.uhso.auto_switch` 为 0（使用 **sysctl(8)**）来禁用。
...
[ath(4)](https://man.freebsd.org/cgi/man.cgi?query=ath&sektion=4&format=html) 驱动支持所有 Atheros Cardbus、ExpressCard、PCI 和 PCIe 卡，但不包括基于 AR5005VL 芯片组的卡。
...
使用较旧 Broadcom 芯片组（BCM4301、BCM4303 和 BCM4306 rev 2）的用户必须使用 **bwi(4)**，因为固件的 v4 版本不支持这些芯片。较新的固件过大，无法放入这些旧芯片。

[ipw(4)](https://man.freebsd.org/cgi/man.cgi?query=ipw&sektion=4&format=html) 驱动支持 Intel PRO/Wireless 2100 MiniPCI 网络适配器。
...
[iwlwifi(4)](https://man.freebsd.org/cgi/man.cgi?query=iwlwifi&sektion=4&format=html) 驱动支持来自 [mvm(4)](https://man.freebsd.org/cgi/man.cgi?query=mvm&sektion=4&format=html) 子驱动的以下芯片组代：
...
[otus(4)](https://man.freebsd.org/cgi/man.cgi?query=otus&sektion=4&format=html) 驱动为 Atheros AR9170 USB IEEE 802.11b/g/n 提供支持无线网络适配器，包括：
...
[ral(4)](https://man.freebsd.org/cgi/man.cgi?query=ral&sektion=4&format=html) 驱动支持基于 Ralink Technology 的 PCI/PCIe/CardBus 无线适配器芯片组，包括：
...
[rsu(4)](https://man.freebsd.org/cgi/man.cgi?query=rsu&sektion=4&format=html) 驱动为 Realtek RTL8188SU/RTL8192SU USB IEEE 802.11b/g/n 提供支持无线网络适配器，包括：
...
[rtwn_usb(4)](https://man.freebsd.org/cgi/man.cgi?query=rtwn_usb&sektion=4&format=html) 驱动支持基于特定 Realtek RTL 8188/8192/8812 和 8821 芯片组，包括：
...
[rum(4)](https://man.freebsd.org/cgi/man.cgi?query=rum&sektion=4&format=html) 驱动支持 USB 2.0 无线基于 Ralink RT2501USB 和 RT2601USB 芯片组的适配器，包括：
...
[upgt(4)](https://man.freebsd.org/cgi/man.cgi?query=upgt&sektion=4&format=html) 驱动支持 USB 2.0 Conexant/Intersil PrismGT 系列无线基于 GW3887 芯片组的适配器，包括：
...
[ural(4)](https://man.freebsd.org/cgi/man.cgi?query=ural&sektion=4&format=html) 驱动支持 USB 2.0 无线适配器，基于 Ralink Technology RT2500USB 芯片组，包括：
...
[u3g(4)](https://man.freebsd.org/cgi/man.cgi?query=u3g&sektion=4&format=html) 驱动支持以下适配器：
...
（参阅 */sys/dev/usb/serial/u3g.c* 获取上述各厂商所支持卡片的完整列表。）

受支持的 3G 卡为 ppp 或 mpd 连接提供必要的调制解调器端口，并提供额外端口（视具体设备而定）以实现其他功能（附加命令端口、诊断端口、SIM 工具包端口）。

在某些设备中，存在由 **umass(4)** 驱动支持的大容量存储设备，其中包含 Windows 和 Mac OS X 驱动程序。设备以磁盘模式启动（TruInstall、ZeroCD 等），需要额外的命令才能切换到调制解调器模式。如果你的设备未自动切换，请尝试添加 quirks。参阅 **usbconfig(8)** 和 **usb_quirk(4)**。

### 串行接口

核心组件和内核接口通过硬件接口连接。此接口作为硬件的抽象，允许不同的 UART 用于串行通信。
...
[puc(4)](https://man.freebsd.org/cgi/man.cgi?query=puc&sektion=4&format=html) 驱动支持以下基于 PCI 的多端口串行板：
...
[ucycom(4)](https://man.freebsd.org/cgi/man.cgi?query=ucycom&sektion=4&format=html) 驱动目前支持以下包含 Cypress USB 到 RS232 桥接芯片的设备：
...
部分板载 CS4610 芯片附带 CS423x ISA 编解码器而非 CS4297 AC97 编解码器。此类配置不受 [snd_csa(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_csa&sektion=4&format=html) 驱动支持。
...
*	Creative Sound Blaster Audigy 2 和 Creative Sound Blaster Audigy 4（CA0102 芯片组）。PCM 支持限于 48kHz/16 位立体声（此芯片组的 192kHz/24 位部分不受支持）。
*	Creative Sound Blaster Audigy 2 Value（CA0108 芯片组）。PCM 支持限于 48kHz/16 位立体声（此芯片组的 192kHz/24 位部分不受支持）。此卡不支持 MIDI。

[snd_emu10kx(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_emu10kx&sektion=4&format=html) 驱动 **不** 支持以下声卡（尽管其名称与某些受支持的卡相似）：
...
[snd_hda(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hda&sektion=4&format=html) 驱动支持具有 PCI 类 4（多媒体）的控制器和子类 3（HDA），兼容 Intel HDA 规范。

[snd_hda(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hda&sektion=4&format=html) 驱动支持超过两百种不同的控制器和 CODEC。在此列出所有这些没有意义，因为在大多数情况下特定的 CODEC 配置和接线比 CODEC 本身的类型更重要。
...
> **注意**：
>
> USB 网络接口可在 [以太网接口](https://www.freebsd.org/releases/14.1R/hardware.html#ethernet) 或 [无线网络接口](https://www.freebsd.org/releases/14.1R/hardware.html#wlan) 中查阅。

> **注意**：
>
> USB 蓝牙适配器可在 [蓝牙](https://www.freebsd.org/releases/14.1R/hardware.html#bluetooth) 中查阅。

> **注意**：
>
> USB 人机接口设备可在 [其他设备](https://www.freebsd.org/releases/14.1R/hardware.html#misc) 中查阅。
...
[uipaq(4)](https://man.freebsd.org/cgi/man.cgi?query=uipaq&sektion=4&format=html) 驱动支持以下适配器：
...
[ulpt(4)](https://man.freebsd.org/cgi/man.cgi?query=ulpt&sektion=4&format=html) 驱动为 USB 打印机和并行打印机转换电缆提供支持，包括以下：
...
> 已不再生产。有 128KB SRAM 型号或 2MB DRAM 型号。
...
> 唯一基于 7811 的板卡（比 7751 更快，并带有随机数生成器）。

NetSec 7751

> 支持最多 IPsec 会话，带 1MB SRAM。

Soekris Engineering vpn1201 和 vpn1211

> 参见 *<http://www.soekris.com/>*。包含 7951，支持对称运算和随机数运算。

Soekris Engineering vpn1401 和 vpn1411

> 参见 *<http://www.soekris.com/>*。包含 7955，支持对称运算和随机数运算。

[safe(4)](https://man.freebsd.org/cgi/man.cgi?query=safe&sektion=4&format=html) 驱动支持以下 SafeXcel 芯片：

SafeNet 1141

> 原始芯片组。支持 DES、Triple-DES、AES、MD5 和 SHA-1 对称加密运算、RNG、公钥运算以及完整的 IPsec 包处理。
...
* 通过 USB、I2C 或 Apple SPI 连接的 HID 键盘（[hkbd(4)](https://man.freebsd.org/cgi/man.cgi?query=hkbd&sektion=4&format=html) 驱动）
...
* AT 式键盘（[atkbd(4)](https://man.freebsd.org/cgi/man.cgi?query=atkbd&sektion=4&format=html) 驱动）
* Hyper-V 键盘（**hv_kbd** 驱动）

指针设备包括：

* 通过 USB、I2C、Hyper-V 或 Apple SPI 连接的 HID 鼠标（[hms(4)](https://man.freebsd.org/cgi/man.cgi?query=hms&sektion=4&format=html) 驱动）和触摸设备（[hmt(4)](https://man.freebsd.org/cgi/man.cgi?query=hmt&sektion=4&format=html) 驱动）
...
* Apple 笔记本触控板（[wsp(4)](https://man.freebsd.org/cgi/man.cgi?query=wsp&sektion=4&format=html) 或 [apt(4)](https://man.freebsd.org/cgi/man.cgi?query=apt&sektion=4&format=html) 驱动）
* Elantech I2C 触控板（[ietp(4)](https://man.freebsd.org/cgi/man.cgi?query=ietp&sektion=4&format=html) 驱动）
* Chromebook Cypress APA I2C 触控板（[cyapa(4)](https://man.freebsd.org/cgi/man.cgi?query=cyapa&sektion=4&format=html) 驱动）
...
> **注意**：
>
> [moused(8)](https://man.freebsd.org/cgi/man.cgi?query=moused&sektion=8&format=html) 提供了在 FreeBSD 中使用指针设备的更多信息。有关在 Xorg 中使用指针设备的信息，请参阅 [http://www.x.org/](http://www.x.org/)。
...
所支持的设备彼此相似，差异在于精度和分辨率。这些设备需要单线进行数据通信，使用自定义协议，与 Maxim 的 1-wire(tm) 不兼容。AM320x 设备还支持连接到 i2c 总线，但该驱动仅支持单线连接选项。
...
[mx25l(4)](https://man.freebsd.org/cgi/man.cgi?query=mx25l&sektion=4&format=html) 驱动为以下设备提供支持：
