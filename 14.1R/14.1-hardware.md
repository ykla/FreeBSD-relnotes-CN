...
该设备具有“Zero-CD”模式，其中包含 Microsoft Windows 的驱动程序；这是设备的默认模式。[uhso(4)](https://man.freebsd.org/cgi/man.cgi?query=uhso&sektion=4&format=html) 驱动自动将设备从“Zero-CD”模式切换到调制解调器模式。此行为可通过设置 `hw.usb.uhso.auto_switch` 为 0（使用 **sysctl(8)**）来禁用。
