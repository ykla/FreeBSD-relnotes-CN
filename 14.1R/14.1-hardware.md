...
本硬件兼容性说明包含 FreeBSD 14.1 的硬件兼容性信息。
...
NetXtreme-E 千兆以太网控制器芯片系列提供支持，包括以下
设备：
...
RTL810xE 和 RTL8111 快速以太网及千兆以太网适配器，包括：
...
快速以太网适配器及嵌入式控制器，以及基于 National Semiconductor DP83815 (MacPhyter) 和 DP83816
芯片的快速以太网适配器。
支持的适配器包括：
...
该设备具有一个称为
“Zero-CD”
的模式，其中包含 Microsoft Windows 的驱动程序；这是设备的默认
模式。
[uhso(4)](https://man.freebsd.org/cgi/man.cgi?query=uhso&sektion=4&format=html)
驱动自动将设备从
“Zero-CD”
...
驱动支持基于特定
Realtek RTL 8188/8192/8812 和 8821 芯片组，包括：
...
驱动支持 USB 2.0 无线
基于 Ralink RT2501USB 和 RT2601USB 芯片组的适配器，
包括：
...
驱动支持 USB 2.0 无线适配器，基于 Ralink Technology
RT2500USB 芯片组，包括：
...
受支持的 3G 卡为 ppp 或 mpd 连接提供必要的调制解调器端口，
并提供额外端口（视具体设备而定）以实现其他功能
（附加命令端口、诊断端口、SIM 工具包端口）。

在某些设备中，存在由
umass(4)
驱动支持的大容量存储设备，其中包含 Windows 和 Mac OS X 驱动程序。
设备以磁盘模式启动（TruInstall、ZeroCD 等），
需要额外的命令才能切换到调制解调器模式。
如果你的设备未自动切换，请尝试添加 quirks。
参阅
usbconfig(8)
和
usb_quirk(4)。
...
*	Creative Sound Blaster Audigy 2 和 Creative Sound Blaster Audigy 4（CA0102
	芯片组）。
	PCM 支持限于 48kHz/16 位立体声（此芯片组的 192kHz/24 位部分
	不受支持）。
*	Creative Sound Blaster Audigy 2 Value（CA0108 芯片组）。
	PCM 支持限于
	48kHz/16 位立体声（此芯片组的 192kHz/24 位部分不受支持）。
	此卡不支持 MIDI。
...
> **注意**：
>
> USB 网络接口可在 <<ethernet,以太网接口>> 或 <<wlan,无线网络接口>> 中查阅。

> **注意**：
>
> USB 蓝牙适配器可在 <<bluetooth,蓝牙>> 中查阅。

> **注意**：
>
> USB 人机接口设备可在 <<misc,其他设备>> 中查阅。
