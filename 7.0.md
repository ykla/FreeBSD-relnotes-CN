...
典型的发行说明条目记录自 6.0-RELEASE 以来发布的安全公告、新的驱动程序或硬件支持、新的命令或选项、重大 bug 修复或第三方软件升级。它们也可能列出对主要 Ports/packages 或发布工程实践的变更。显然，发行说明不能列出每个版本之间对 FreeBSD 所做的所有变更；本文档主要关注安全公告、用户可见的变更和重大架构改进。
...
现已实现 RedZone，内核 [malloc(9)](http://www.freebsd.org/cgi/man.cgi?query=malloc&sektion=9&manpath=FreeBSD+7.0-RELEASE) 的缓冲区损坏保护功能。它可以在 [free(9)](http://www.freebsd.org/cgi/man.cgi?query=free&sektion=9&manpath=FreeBSD+7.0-RELEASE) 和 [realloc(9)](http://www.freebsd.org/cgi/man.cgi?query=realloc&sektion=9&manpath=FreeBSD+7.0-RELEASE) 上运行时检测缓冲区下溢和溢出，并打印出内存分配和释放位置的回溯信息。更多信息请参阅 [redzone(9)](http://www.freebsd.org/cgi/man.cgi?query=redzone&sektion=9&manpath=FreeBSD+7.0-RELEASE) 手册页。
...
[snd_cmi(4)](http://www.freebsd.org/cgi/man.cgi?query=snd_cmi&sektion=4&manpath=FreeBSD+7.0-RELEASE) 驱动现已支持 MPSAFE。
...
[snd_solo(4)](http://www.freebsd.org/cgi/man.cgi?query=snd_solo&sektion=4&manpath=FreeBSD+7.0-RELEASE) 驱动现已支持 MPSAFE。[已合并]
...
现已为 [bge(4)](http://www.freebsd.org/cgi/man.cgi?query=bge&sektion=4&manpath=FreeBSD+7.0-RELEASE) 驱动的 Jumbo 帧支持实现 MPSAFE。

[bge(4)](http://www.freebsd.org/cgi/man.cgi?query=bge&sektion=4&manpath=FreeBSD+7.0-RELEASE) 驱动现已支持大端架构，如 sparc64。

[cm(4)](http://www.freebsd.org/cgi/man.cgi?query=cm&sektion=4&manpath=FreeBSD+7.0-RELEASE) 驱动现已支持 MPSAFE。
...
802.11 协议栈已进行了显著重构。新功能包括支持背景扫描和 AP 之间的漫游，以及 802.11n 设备所需的支持。
...
已移除 KAME IPsec 实现。现在，**FAST_IPSEC** 是 FreeBSD 内核唯一支持的 IPsec 实现。以前启用 KAME IPsec 的内核配置选项 **IPSEC** 现在启用 **FAST_IPSEC**。**FAST_IPSEC** 现支持 IPv4 和 IPv6，使用细粒度内核锁，并支持硬件加密加速。
...
[natm(4)](http://www.freebsd.org/cgi/man.cgi?query=natm&sektion=4&manpath=FreeBSD+7.0-RELEASE)，原生模式 ATM 协议层现已 MPSAFE。
...
[twa(4)](http://www.freebsd.org/cgi/man.cgi?query=twa&sektion=4&manpath=FreeBSD+7.0-RELEASE) 驱动已更新到 3ware 网站上的 3.70.03.007 版本，现已支持 AMCC 的 3ware 9650 系列 SATA 控制器。[已合并]
...
scsi_sg 驱动模拟了 Linux SCSI SG 直通设备 API 的重要子集，现已添加。它旨在允许在 Linux 仿真下运行的程序（以及本地 FreeBSD 应用程序）访问 Linux 支持的 **/dev/sg\*** 设备。[已合并]
...
[kgdb(1)](http://www.freebsd.org/cgi/man.cgi?query=kgdb&sektion=1&manpath=FreeBSD+7.0-RELEASE) 现支持 `-w` 选项，以读写模式打开基于 kmem 的目标。这使得可以使用 kgdb 在 **/dev/mem** 上操作，并能够在实时系统上修补内存。
...
[sade(8)](http://www.freebsd.org/cgi/man.cgi?query=sade&sektion=8&manpath=FreeBSD+7.0-RELEASE) 磁盘分区和标签工具新增了。该工具基于 [sysinstall(8)](http://www.freebsd.org/cgi/man.cgi?query=sysinstall&sektion=8&manpath=FreeBSD+7.0-RELEASE) 的分区编辑部分。
...
现已实现 `wcsdup()` 函数。此函数在微软和 GNU 系统中广泛使用。
