...
FreeBSD 目前可以运行在多种基于 ISA、VLB、EISA 和 PCI 总线的 PC 上，从 386sx 到奔腾级别的机器（虽然不推荐使用 386sx）。同时支持通用 IDE 或 ESDI 驱动配置、各种 SCSI 控制器、网络卡和串行卡。
...
**注意**：无法从 SoundBlaster 卡启动，因为它们没有板载 BIOS，而启动设备映射到系统 BIOS I/O 矢量中需要此功能。然而，这些卡完全可用于外部磁带、CD-ROM 等。对于任何没有启动 ROM 的基于 AIC-6x60 的卡也是如此。某些系统确实有启动 ROM，这通常会在系统首次开机或重置时显示消息，在这种情况下你也将能够从这些卡启动。请参阅你的系统/主板文档以了解更多信息。
...
你可以从 FreeBSD 的官方发布站点 `ftp.FreeBSD.org` 上通过 FTP 下载 FreeBSD 及其任何或所有可选包。
