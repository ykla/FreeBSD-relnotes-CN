...
除了基础发行版，FreeBSD 还提供了新的移植软件集，其中包含超过 390 款常用程序。这个 Ports 列表涉及从 HTTP（WWW）服务器到游戏、编程语言、文本编辑器及几乎所有其他类别的软件。整个 Ports 只需要 10MB 的存储空间，所有 Port 都以“增量”的形式表示，即相对于原始源码的修改。这样，我们可以更容易地更新 Port，并大大减少 Ports 所占用的磁盘空间。要编译 Port，你只需要进入你想安装的程序目录，输入 `make`，然后让系统完成其余的工作。你构建的每个 Port 的完整原始发行版会动态地从 CDROM 或本地 FTP 站点获取，因此你只需足够的磁盘空间来构建你需要的 Port。（几乎）每个 Port 也都提供了一个预编译的“包”，可以通过简单的命令（`pkg_add`）进行安装。有关包的特别便捷的接口，请参见配置菜单中的新 Packages 选项。
...
FreeBSD 的核心不包含任何会妨碍其出口到美国以外地区的 DES 代码。核心发行版提供附加包，仅供美国国内使用，包含通常使用 DES 的程序。单独提供的辅助包任何人都可以使用。对于非美国用户，FreeBSD 还提供了一个可自由出口的 DES 版本，下载地址为 `ftp://ftp.internat.FreeBSD.org/pub/FreeBSD`。
