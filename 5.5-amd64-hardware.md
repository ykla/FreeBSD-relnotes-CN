# FreeBSD 5.5-RELEASE amd64 硬件兼容列表

- 原文链接：[FreeBSD/amd64 5.5-RELEASE Hardware Notes](https://www.freebsd.org/releases/5.5R/hardware-amd64/)
...
本文档包含 FreeBSD 5.5-RELEASE 在 AMD64 硬件平台（也称为 FreeBSD/amd64 5.5-RELEASE）上的硬件兼容性说明。它列出了已知在此平台上工作的设备，以及一些在尝试为新设备配置支持时可能有用的引导时内核自定义说明。
...
Specialix SI/XIO/SX 多端口串行卡，包括较旧的 SIHOST2.x 和“enhanced”（基于 transputer，又称 JET）主机卡（ISA、EISA 和 PCI）都受支持。注意，较新的 SX+ PCI 卡目前不受支持。（`si(4)` 驱动程序）
...
> **注意**
>
> USB 以太网适配器可在列出 [以太网接口](#) 的章节中找到。

> **注意**
>
> USB 蓝牙适配器可在 [蓝牙](#) 章节中找到。
