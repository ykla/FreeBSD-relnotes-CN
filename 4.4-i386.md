...
本文档包含了 FreeBSD 4.4-RELEASE 在 i386 硬件平台上的发布说明。它描述了自 4.3-RELEASE 以来，FreeBSD 新增（或更改）的新特性。
...
已新增 [nmdm(4)](http://www.freebsd.org/cgi/man.cgi?query=nmdm&sektion=4) 空串行端口终端驱动程序。
...
已新增一个基于哈希的简单查找优化方法，名为 dirhash，专门用于大目录。基于 UFS_DIRHASH 内核选项，它提高了对非常大目录的操作速度，但会消耗一些内存。
...
[install(1)](http://www.freebsd.org/cgi/man.cgi?query=install&sektion=1) 现在具有一些新功能，包括用于备份现有目标文件的 -b 和 -B 选项，以及用于“安全”（原子复制）操作的 -S 选项。默认情况下，-c（复制）参数现在是默认选项，-D（调试）参数已被撤销。[install(1)](http://www.freebsd.org/cgi/man.cgi?query=install&sektion=1) 现在会在同时使用 -d（创建目录）和 -C（仅复制已更改文件）选项时发出警告。
