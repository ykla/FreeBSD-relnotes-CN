...
[wlan(4)](http://www.freebsd.org/cgi/man.cgi?query=wlan&sektion=4&manpath=FreeBSD+6.0-RELEASE) 框架中的 802.11 设备支持已得到大幅改进。除了架构上的变更外，还包括完成的 802.11g、WPA、802.11i、802.1x、WME/WMM、AP端节能以及加密模块、认证器和访问控制插件框架。特别需要注意的是，现在 WEP 需要加载（或编译到内核中的）`wlan_wep` 模块。
