...
由于处理器使用超线程技术 (HTT) 时可能存在信息泄露漏洞，已添加 sysctl 变量 `machdep.hyperthreading_allowed` 。在 FreeBSD CURRENT 上默认为 `1`（启用 HTT），在 4-STABLE 和 5-STABLE 开发分支以及受支持的安全修复分支上默认为 `0`（禁用 HTT）。更多信息请参阅安全公告 [FreeBSD-SA-05:09.htt](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-05:09.htt.asc)。[已合并]
