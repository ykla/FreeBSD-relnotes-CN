...
- **fetch(1)** 工具中的一个漏洞已被修复，该漏洞允许恶意的 HTTP 服务器导致客户端的内存部分被任意覆盖。更多信息请参见安全公告 [FreeBSD-SA-04:16.fetch](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-04:16.fetch.asc)。[已合并]

- **procfs(5)** 和 **linprocfs(5)** 中的一个漏洞已被修复，攻击者可以通过该漏洞使恶意本地用户读取内核内存的部分内容，或者通过引发系统崩溃来执行本地拒绝服务攻击。更多信息请参见安全公告 [FreeBSD-SA-04:17.procfs](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-04:17.procfs.asc)。[已合并]

- 修复了 TELNET 客户端程序中的两个缓冲区溢出漏洞，恶意的 TELNET 服务器或活跃的网络攻击者可能利用该漏洞导致 [telnet(1)](http://www.freebsd.org/cgi/man.cgi?query=telnet&sektion=1&manpath=FreeBSD+6.0-RELEASE) 执行任意代码，并获取运行该程序的用户权限。更多信息请参见安全公告 [FreeBSD-SA-05:01.telnet](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-05:01.telnet.asc)。[已合并]

- **sendfile(2)** 系统调用中的一个信息泄露漏洞已被修复，攻击者可能利用该漏洞传输随机的内核内存内容。更多详细信息请参见安全公告 [FreeBSD-SA-05:02.sendfile](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-05:02.sendfile.asc)。[已合并]

- 修复了一个可能的特权提升漏洞，该漏洞影响 FreeBSD/amd64，允许未授权用户直接访问某些硬件，这些硬件在没有提升权限的情况下无法访问。更多信息请参见安全公告 [FreeBSD-SA-05:03.amd64](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-05:03.amd64.asc)。[已合并]

- 修复了 **SIOCGIFCONF ioctl(2)** 中的信息泄露漏洞，该漏洞泄露了 12 字节的内核内存。更多信息请参见安全公告 [FreeBSD-SA-05:04.ifconf](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-05:04.ifconf.asc)。[已合并]

- 修复了 [cvs(1)](http://www.freebsd.org/cgi/man.cgi?query=cvs&sektion=1&manpath=FreeBSD+6.0-RELEASE) 中的多个编程错误，攻击者可能通过这些错误在 CVS 服务器上执行任意代码。更多信息请参见安全公告 [FreeBSD-SA-05:05.cvs](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-05:05.cvs.asc)。[已合并]

- 修复了 /dev/iir 设备节点默认权限的错误，允许未经授权的本地用户向由 [iir(4)](http://www.freebsd.org/cgi/man.cgi?query=iir&sektion=4&manpath=FreeBSD+6.0-RELEASE) 驱动程序支持的硬件发送命令。更多信息请参见安全公告 [FreeBSD-SA-05:06.iir](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-05:06.iir.asc)。[已合并]

- 修复了 [i386_get_ldt(2)](http://www.freebsd.org/cgi/man.cgi?query=i386_get_ldt&sektion=2&manpath=FreeBSD+6.0-RELEASE) 系统调用输入参数的验证漏洞，攻击者可能利用该漏洞泄露内核内存给用户进程。更多信息请参见安全公告 [FreeBSD-SA-05:07.ldt](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-05:07.ldt.asc)。[已合并]

- 修复了多个内核部分的信息泄露漏洞。更多信息请参见安全公告 [FreeBSD-SA-05:08.kmem](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-05:08.kmem.asc)。[已合并]

- 由于使用超线程技术 (HTT) 的处理器存在信息泄露漏洞，新增了 sysctl 变量 `machdep.hyperthreading_allowed` 。该变量在 FreeBSD CURRENT 中默认设置为 `1`（启用 HTT），在 4-STABLE 和 5-STABLE 开发分支及支持的安全修复分支中默认设置为 `0`（禁用 HTT）。更多信息请参见安全公告 [FreeBSD-SA-05:09.htt](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-05:09.htt.asc)。[已合并]

- 修复了工具 [tcpdump(1)](http://www.freebsd.org/cgi/man.cgi?query=tcpdump&sektion=1&manpath=FreeBSD+6.0-RELEASE) 中的一个漏洞，允许恶意远程用户通过特制的包导致服务拒绝。更多信息请参见安全公告 [FreeBSD-SA-05:10.tcpdump](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-05:10.tcpdump.asc)。[已合并]

- 修复了工具 [gzip(1)](http://www.freebsd.org/cgi/man.cgi?query=gzip&sektion=1&manpath=FreeBSD+6.0-RELEASE) 中的两个问题，这些问题可能允许本地用户在解压文件时修改任意文件的权限并覆盖本地文件。更多信息请参见安全公告 [FreeBSD-SA-05:11.gzip](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-05:11.gzip.asc)。[已合并]

- 修复了 **BIND 9** DNSSEC 中的一个漏洞。当启用 DNSSEC 时，恶意远程攻击者可能注入特制的包，导致 [named(8)](http://www.freebsd.org/cgi/man.cgi?query=named&sektion=8&manpath=FreeBSD+6.0-RELEASE) 终止。更多信息请参见安全公告 [FreeBSD-SA-05:12.bind9](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-05:12.bind9.asc)。[已合并]

- 修复了 [ipfw(4)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=4&manpath=FreeBSD+6.0-RELEASE) 中的一个错误，可能导致数据包在查找表中被错误匹配。此错误仅影响 SMP 机器或启用了 PREEMPTION 内核选项的 UP 机器。更多信息请参见安全公告 [FreeBSD-SA-05:13.ipfw](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-05:13.ipfw.asc)。[已合并]

- 修复了 [bzip2(1)](http://www.freebsd.org/cgi/man.cgi?query=bzip2&sektion=1&manpath=FreeBSD+6.0-RELEASE) 中的两个与安全相关的问题，包括潜在的服务拒绝和未经授权的文件权限操作。更多信息请参见安全公告 [FreeBSD-SA-05:14.bzip2](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-05:14.bzip2.asc)。[已合并]

- 修复了 FreeBSD TCP 堆栈中的两个问题，这些问题可能允许攻击者使现有的 TCP 连接停滞，从而造成服务拒绝情况。更多信息请参见安全公告 [FreeBSD-SA-05:15.tcp](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-05:15.tcp.asc)。[已合并]
...
已添加 USB 通信设备类以太网驱动 [cdce(4)](http://www.freebsd.org/cgi/man.cgi?query=cdce&sektion=4&manpath=FreeBSD+6.0-RELEASE) 。[已合并]
...
已为 [named(8)](http://www.freebsd.org/cgi/man.cgi?query=named&sektion=8&manpath=FreeBSD+6.0-RELEASE) 动态区域添加了一个单独的目录，该目录由 bind 用户拥有（用于创建区域日志文件）。更多细节，请参见示例 [named.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=named.conf&sektion=5&manpath=FreeBSD+6.0-RELEASE) 中的动态区域示例。[已合并]

实用程序 [ncal(1)](http://www.freebsd.org/cgi/man.cgi?query=ncal&sektion=1&manpath=FreeBSD+6.0-RELEASE) 现在支持 `-m` 标志，用于生成当前年份指定月份的日历。[已合并]

实用程序 [newfs(8)](http://www.freebsd.org/cgi/man.cgi?query=newfs&sektion=8&manpath=FreeBSD+6.0-RELEASE) 现在支持 `-n` 标志，用于在新文件系统上抑制创建 `.snap` 目录。此功能旨在用于不需要快照支持的内存或 vnode 文件系统。[已合并]
...
已添加编译时开关 `NO_NIS`。顾名思义，启用此 Makefile 变量将导致排除 NIS 支持，并使 NIS 工具不被构建。[已合并]
