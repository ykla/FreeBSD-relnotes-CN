# FreeBSD 10.2-RELEASE 硬件兼容列表

## 引言

本文档包含 FreeBSD 10.2-RELEASE 的硬件兼容性说明。其中列出了 FreeBSD 支持的硬件平台，以及各类硬件设备（存储控制器、网络接口等），并给出已知可正常工作的设备实例。
...
如果你有不在此列表中的系统，它可能尚未经过 FreeBSD 10.2-RELEASE 测试。我们鼓励你尝试并致信 [FreeBSD SPARC porting mailing list](http://lists.FreeBSD.org/mailman/listinfo/freebsd-sparc64) 反馈结果，包括哪些设备可用、哪些不可用。
