...
* OpenSSH 更新至 7.5p1 版本。
* OpenSSL 更新至 1.0.2o 版本。
* `clang`、`llvm`、`lldb` 和 `compiler-rt` 工具更新至 6.0.0 版本。
* **libarchive(3)** 库更新至 3.3.2 版本。
* **libxo(3)** 库更新至 0.9.0 版本。
* 设备驱动更新涉及 **cxgbe(4)**、**ixl(4)** 和 **ng_pppoe(4)**，并新增了 **mlx5io(4)**、**ocs_fw(4)** 和 **smartpqi(4)** 驱动。
* 新增 **dwatch(1)**、**efibootmgr(8)** 和 **etdump(1)** 工具。
* 对内核、用户空间应用程序和库进行了若干杂项更新。
* KDE 更新至 4.14.3 版本。
* GNOME 更新至 3.18.0 版本。
...
为方便没有控制台访问权限的用户，默认提供 `freebsd` 用户（密码为 `freebsd`）用于 **ssh(1)** 访问。此外，`root` 用户密码设置为 `root`，强烈建议在获得系统访问权限后更改这两个用户的密码。
...
许多公司捐赠了设备、网络访问或工时来支持 FreeBSD 11.2 的发布工程活动，包括 FreeBSD 基金会、Yahoo!、NetApp、Internet Systems Consortium、ByteMark Hosting、Sentex Communications、New York Internet、Juniper Networks、NLNet Labs、iXsystems 和 Yandex。
