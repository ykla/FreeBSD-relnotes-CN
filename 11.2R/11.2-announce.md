# FreeBSD 11.2-RELEASE 发行公告

日期：2018 年 6 月 28 日

FreeBSD 发布工程团队很高兴地宣布 FreeBSD 11.2-RELEASE 现已发布。这是 stable/11 分支的第三个版本。

部分亮点如下：

* OpenSSH 已更新至 7.5p1 版本。
* OpenSSL 已更新至 1.0.2o 版本。
* `clang`、`llvm`、`lldb` 和 `compiler-rt` 工具已更新至 6.0.0 版本。
* [libarchive(3)](https://man.freebsd.org/cgi/man.cgi?query=libarchive&sektion=3&format=html) 库已更新至 3.3.2 版本。
* [libxo(3)](https://man.freebsd.org/cgi/man.cgi?query=libxo&sektion=3&format=html) 库已更新至 0.9.0 版本。
* 设备驱动更新涉及 `cxgbe(4)`、`ixl(4)` 和 `ng_pppoe(4)`，并新增了 `mlx5io(4)`、`ocs_fw(4)` 和 `smartpqi(4)` 驱动。
* 新增 `dwatch(1)`、`efibootmgr(8)` 和 `etdump(1)` 工具。
* 对内核、用户空间应用程序和库进行了多项杂项更新。
* KDE 已更新至 4.14.3 版本。
* GNOME 已更新至 3.18.0 版本。

完整的新功能和已知问题列表，请参阅在线发行说明和勘误列表，地址如下：

* <https://www.FreeBSD.org/releases/11.2R/relnotes/>
* <https://www.FreeBSD.org/releases/11.2R/errata/>
...
FreeBSD 11.2-RELEASE 现已支持 amd64、i386、powerpc、powerpc64、sparc64、armv6 和 aarch64 架构。

FreeBSD 11.2-RELEASE 可从可启动 ISO 镜像或通过网络安装。部分架构还支持从 USB 闪存盘安装。所需文件可按以下说明下载。

本消息底部包含发行版 ISO、闪存盘和 SD 卡镜像的 SHA512 和 SHA256 校验和。

发行版镜像的 PGP 签名校验和也可从以下地址获取：

* <https://www.FreeBSD.org/releases/11.2R/signatures/>
...
```sh
# dd if=FreeBSD-11.2-RELEASE-amd64-memstick.img of=/dev/da0 bs=10240 conv=sync
```
...
```sh
# dd if=FreeBSD-11.2-RELEASE-amd64-mini-memstick.img of=/dev/da0 bs=1m conv=sync
```
...
```sh
# dd if=FreeBSD-11.2-RELEASE-arm-armv6-KERNEL.img of=/dev/da0 bs=1m conv=sync
```

务必确保目标（`of=`）正确。

FreeBSD 11.2-RELEASE 也可从多家供应商处购买 CD-ROM 或 DVD 介质。提供 FreeBSD 11.2 相关产品的供应商之一是：

* FreeBSD Mall, Inc. <https://www.freebsdmall.com>

预安装的虚拟机镜像也可用于 amd64（x86_64）、i386（x86_32）和 AArch64（arm64）架构，提供 `QCOW2`、`VHD` 和 `VMDK` 磁盘镜像格式，以及原始（未格式化）镜像。

FreeBSD 11.2-RELEASE amd64 还可在以下云托管平台上使用：
...
FreeBSD 预计还将在 Google Compute Engine(TM) Marketplace 中提供，待其完成第三方特定验证后，访问：
...
* Microsoft Azure(TM)：
FreeBSD 虚拟机镜像将在完成第三方特定验证后提供，访问：
<https://azuremarketplace.microsoft.com/marketplace/apps/Microsoft.FreeBSD112?tab=Overview>

## 下载

FreeBSD 11.2-RELEASE 可从以下站点通过 https 下载：

* <https://download.freebsd.org/ftp/releases/ISO-IMAGES/11.2/>

FreeBSD 11.2-RELEASE 虚拟机镜像可从以下地址下载：

* <https://download.freebsd.org/ftp/releases/VM-IMAGES/11.2-RELEASE/>

关于安装 FreeBSD 或将现有机器更新至 11.2-RELEASE 的说明，请参阅：

* <https://www.FreeBSD.org/releases/11.2R/installation/>

## 支持

根据新的 FreeBSD 支持模型，FreeBSD 11 发行系列将至少支持至 2021 年 9 月 30 日。此点发行版 FreeBSD 11.2-RELEASE 将至少支持至 FreeBSD 11.3-RELEASE 发布后三个月。更多支持信息请参阅：

* <https://www.FreeBSD.org/security/>

请注意，11.1 将支持至 11.2 发布日期起三个月后，目前计划为 2018 年 9 月 30 日。

## 致谢

许多公司捐赠了设备、网络访问或人力时间来支持 FreeBSD 11.2 的发布工程活动，包括 FreeBSD 基金会、Yahoo!、NetApp、Internet Systems Consortium、ByteMark Hosting、Sentex Communications、New York Internet、Juniper Networks、NLNet Labs、iXsystems 和 Yandex。

11.2-RELEASE 的发布工程团队包括：

| 成员 | 角色 |
| :--- | :--- |
| Glen Barber <gjb@FreeBSD.org> | 发布工程负责人，11.2-RELEASE 发布工程师 |
...
| Xin Li <delphij@FreeBSD.org> | 发布工程，安全官名誉职位 |
...
特别感谢 Colin Percival <cperciva@FreeBSD.org> 协助解决 Amazon EC2(TM) 上 FreeBSD 的若干最后时刻问题。
...
喜欢 FreeBSD？请通过向 FreeBSD 基金会[捐赠](https://www.freebsdfoundation.org/donate/) 来支持此版本及未来版本的发布！
