...
FreeBSD 会在支持此特性的 CPU 上利用 SMT（Symmetric MultiThreading，对称多线程，在 Intel CPU 上又称 HyperThreading）。默认安装的 `GENERIC` 内核会自动检测额外的逻辑处理器。默认的 FreeBSD 调度器能识别系统上的处理器拓扑，并选择逻辑和物理处理器以获得最佳性能。**smp(4)** 手册页有更多细节。

FreeBSD 会在支持此特性的 CPU 上利用物理地址扩展（PAE）支持。启用 `PAE` 特性的内核会检测 4 GB 以上的内存并允许系统使用。此特性对 FreeBSD 其他可用的设备驱动和特性构成限制；详情请参阅 **pae(4)** 手册页。
...
大多数现代笔记本电脑（以及许多台式机）使用高级配置与电源管理（ACPI）标准。FreeBSD 通过 Intel 的 ACPI Component Architecture 参考实现支持 ACPI，如 **acpi(4)** 手册页所述。在某些机器上使用 ACPI 会导致不稳定，可能需要禁用 ACPI 驱动（通常通过内核模块加载）。可通过在 **/boot/device.hints** 中添加以下行来实现：

```sh
hint.acpi.0.disabled="1"
```

调试 ACPI 相关问题的用户可能会发现禁用部分 ACPI 功能很有用。**acpi(4)** 手册页提供了通过 loader 可调参数进行此操作的更多信息。

ACPI 依赖于每台机器 BIOS 提供的 Differentiated System Descriptor Table（DSDT）。部分机器的 DSDT 有缺陷或不完整，导致 ACPI 无法正常工作。部分机器的替换 DSDT 可在 [ACPI4Linux](http://acpi.sourceforge.net/) 项目的 [DSDT](http://acpi.sourceforge.net/dsdt/index.php) 部分找到。FreeBSD 可使用这些 DSDT 覆盖 BIOS 提供的 DSDT；更多信息请参阅 **acpi(4)** 手册页。
...
使用 `GENERIC` 内核时，未配备 **creator(4)**（Sun Creator、Sun Creator3D 和 Sun Elite3D）或 **machfb(4)**（Sun PGX 和 Sun PGX64，以及例如 Sun Blade(TM) 100、Sun Blade(TM) 150、Sun Ultra(TM) 5 和 Sun Ultra(TM) 10 板载的 ATI Mach64 芯片）驱动所支持帧缓冲的 FreeBSD/sparc64 系统必须使用串行控制台。
...
以下系统受 FreeBSD 部分支持。特别是基于 SBus 的系统中的光纤通道控制器不受支持。但可使用 **esp(4)** 驱动支持的 SCSI 控制器（Sun ESP SCSI、Sun FAS Fast-SCSI 和 Sun FAS366 Fast-Wide SCSI 控制器）来使用这些系统。
...
[amd64, i386, pc98, sparc64] IDE/ATA 控制器（**ata(4)** 驱动）

[pc98] IDE/ATA 控制器（wdc 驱动）

* On-board IDE controller

[i386, amd64] **aac(4)** 驱动支持以下控制器：
...
[i386, pc98, amd64] **adv(4)** 驱动支持以下 SCSI 控制器：
...
[i386, pc98, amd64] **adw(4)** 驱动支持 SCSI 控制器，包括：
...
**ahc(4)** 驱动支持以下 SCSI 主机适配器芯片和 SCSI 控制器卡：
...
[i386, pc98, amd64] **aic(4)** 驱动支持的适配器包括：
...
[i386, amd64] **bt(4)** 驱动支持以下 BusLogic MultiMaster“W”、“C”、“S”和“A”系列及兼容 SCSI 主机适配器：
...
AMI FastDisk 主机适配器中真正的 BusLogic MultiMaster 克隆也受 **bt(4)** 驱动支持。

[i386, amd64] **ciss(4)** 驱动支持以下控制器：
...
[i386, amd64] **dpt(4)** 驱动支持以下 RAID 适配器：
...
[i386, amd64] **hpt27xx(4)** 驱动支持以下 SAS 控制器：
...
[i386, amd64] **hptiop(4)** 驱动支持以下 SAS 和 SATA RAID 控制器：
...
**hptiop(4)** 驱动还支持以下已停产的 SAS 和 SATA RAID 控制器：
...
[i386, amd64] **hptmv(4)** 驱动支持以下 ATA RAID 控制器：

* HighPoint’s RocketRAID 182x series

[i386, amd64] **hptrr(4)** 驱动支持以下 RAID 控制器：
...
[i386, amd64] **isci(4)** 驱动支持 Intel C600 SAS 控制器。
...
**mpt(4)** 驱动支持的 Ultra 320 SCSI 控制器芯片可在许多系统的板载上找到，包括：
...
[i386, pc98, amd64] **ncr(4)** 驱动支持以下 NCR/Symbios SCSI 控制器芯片：
...
[i386] **pst(4)** 驱动支持 Promise Supertrak SX6000 ATA 硬件 RAID 控制器。
...
**sym(4)** 驱动支持以下 Symbios/LSI Logic PCI SCSI 控制器：
...
**sym(4)** 支持的 SCSI 控制器可以板载于主板，或位于以下扩展板之一：
...
[i386, amd64] **trm(4)** 驱动支持以下 SCSI 控制器：
...
[i386, amd64] **twa(4)** 驱动支持以下 SATA RAID 控制器：
...
[i386, amd64] **twe(4)** 驱动支持以下 PATA/SATA RAID 控制器：
...
[i386, amd64] **tws(4)** 驱动支持以下 SATA/SAS RAID 控制器：

* LSI’s 3ware SAS 9750 series

[i386] **vpo(4)** 驱动支持以下并行转 SCSI 接口：
...
对于所有受支持的 SCSI 控制器，完整支持 SCSI-I、SCSI-II 和 SCSI-III 外设，包括硬盘、光盘、磁带机（包括 DAT、8mm Exabyte、Mammoth 和 DLT）、介质更换器、处理器目标设备和 CD-ROM 驱动器。支持 CD-ROM 命令的 WORM 设备可由 CD-ROM 驱动程序（如 **cd(4)**）以只读访问方式支持。WORM/CD-R/CD-RW 写入支持由 **cdrecord(1)** 提供，它是 Ports 中 [sysutils/cdrtools](http://www.freebsd.org/cgi/url.cgi?ports/sysutils/cdrtools/pkg-descr) port 的一部分。

目前支持以下 CD-ROM 类型系统：

* SCSI 接口（也包括 ProAudio Spectrum 和 SoundBlaster SCSI）（**cd(4)**）
* [i386] Sony 专用接口（所有型号）（**scd(4)**）
* ATAPI IDE 接口（**acd(4)**）
...
**ae(4)** 驱动支持 Attansic/Atheros L2 PCIe FastEthernet 控制器，已知支持以下硬件：
...
**age(4)** 驱动支持基于 Attansic/Atheros L1 千兆以太网控制器芯片的 LOM，包括：
...
[i386, pc98, amd64, powerpc] **aue(4)** 驱动支持的适配器包括：
...
[i386, pc98, amd64, powerpc] **axe(4)** 驱动支持基于 ASIX Electronics AX88172/AX88178/AX88772/AX88772A/AX88772B/AX88760 的 USB 以太网适配器，包括：
...
[i386, amd64] **bce(4)** 驱动支持基于 QLogic NetXtreme II 系列千兆以太网控制器的各种 NIC，包括以下：
...
[amd64, i386] 基于 Broadcom BCM4401 的快速以太网适配器（**bfe(4)** 驱动）

[i386, pc98, sparc64, amd64] **bge(4)** 驱动支持基于 Broadcom BCM570x 系列千兆以太网控制器芯片的各种 NIC，包括以下：
...
**bnxt(4)** 驱动支持基于 Broadcom NetXtreme-C 和 NetXtreme-E 系列千兆以太网控制器芯片的各种 NIC，包括以下：
...
[i386, amd64] **bxe(4)** 驱动支持基于 QLogic NetXtreme II 系列的 10Gb 以太网控制器芯片的各种 NIC，包括以下：
...
[i386, pc98, amd64, powerpc] **cdce(4)** 驱动支持以下设备：
...
[i386, pc98, amd64, powerpc] **cue(4)** 驱动支持基于 CATC USB-EL1210A 的 USB 以太网适配器，包括：
...
[i386, amd64] **cxgb(4)** 驱动支持基于 T3 和 T3B 芯片组的 10 千兆和 1 千兆以太网适配器：

* Chelsio 10GBase-CX4
* Chelsio 10GBase-LR
* Chelsio 10GBase-SR

**cxgbe(4)** 驱动支持基于 T6 ASIC 的 100Gb 和 25Gb 以太网适配器：
...
**cxgbe(4)** 驱动支持基于 T5 ASIC 的 40Gb、10Gb 和 1Gb 以太网适配器：
...
**cxgbe(4)** 驱动支持基于 T4 ASIC 的 10Gb 和 1Gb 以太网适配器：
...
[i386, pc98, amd64] **de(4)** 驱动支持的适配器包括：
...
**em(4)** 驱动支持基于 Intel 82540、82541ER、82541PI、82542、82543、82544、82545、82546、82546EB、82546GB、82547、82571、82572、82573 和 82574 控制器芯片的千兆以太网适配器：
...
[i386, pc98, amd64] **ep(4)** 驱动支持基于 3Com 3C5x9 Etherlink III Parallel Tasking 芯片组的以太网适配器，包括：
...
[i386, pc98, amd64] **fe(4)** 驱动支持的控制器和卡包括：
...
[i386] **e(4)** 驱动支持以下基于 Intel i82586 芯片的 8 位和 16 位 ISA 以太网卡：
...
**igb(4)** 驱动支持基于 Intel 82575 和 82576 控制器芯片的千兆以太网适配器：
...
[i386, pc98, amd64, powerpc] **kue(4)** 驱动支持基于 Kawasaki LSI KL5KLUSB101B 的 USB 以太网适配器，包括：
...
[i386, pc98, amd64] **lge(4)** 驱动支持的适配器包括：

* SMC TigerCard 1000 (SMC9462SX)
* D-Link DGE-500SX

**mlx4en(4)** 驱动支持以下网络适配器：

* Mellanox ConnectX-2 (ETH)
* Mellanox ConnectX-3 (ETH)

**mlx5en(4)** 驱动支持 100Gb、50Gb、40Gb、25Gb 和 10Gb 以太网适配器。ConnectX-4 支持：10/20/25/40/50/56/100Gb/s 速度。ConnectX-4 LX 支持：10/25/40/50Gb/s 速度（且功耗更低）：
...
[i386, amd64] **msk(4)** 驱动支持基于 Marvell/SysKonnect Yukon II 千兆以太网控制器芯片的各种 NIC，包括：
...
[i386, amd64] **mxge(4)** 驱动支持基于 Myricom LANai Z8E 芯片的 10 千兆以太网适配器：

* Myricom 10GBase-CX4 (10G-PCIE-8A-C, 10G-PCIE-8AL-C)
* Myricom 10GBase-R (10G-PCIE-8A-R, 10G-PCIE-8AL-R)
* Myricom 10G XAUI over ribbon fiber (10G-PCIE-8A-Q, 10G-PCIE-8AL-Q)

[i386, pc98] **my(4)** 驱动支持基于 Myson 芯片组的各种 NIC。支持的型号包括：

* Myson MTD800 PCI Fast Ethernet chip
* Myson MTD803 PCI Fast Ethernet chip
* Myson MTD89X PCI Gigabit Ethernet chip

[i386, amd64] **nfe(4)** 驱动支持以下 NVIDIA MCP 板载适配器：
...
**nge(4)** 驱动支持基于 National Semiconductor DP83820 和 DP83821 的千兆以太网适配器，包括：
...
[i386, amd64] **nxge(4)** 驱动支持 <http://www.neterion.com/how/pricing.html> 中列出的 Neterion Xframe 10 千兆以太网适配器。

[i386, amd64] **oce(4)** 驱动支持以下网络适配器：

* Emulex BladeEngine 2
* Emulex BladeEngine 3
* Emulex Lancer

[i386, pc98, amd64] **pcn(4)** 驱动支持基于 AMD PCnet/FAST、PCnet/FAST+、PCnet/FAST III、PCnet/PRO 和 PCnet/Home 快速以太网芯片的适配器和嵌入式控制器：
...
**qlnxe(4)** 驱动支持基于以下芯片组的 25/40/100 千兆以太网和 CNA 适配器：

* QLogic 45000 series

[amd64] **qlxgb(4)** 驱动支持基于以下芯片组的 10 千兆以太网和 CNA 适配器：

* QLogic 3200 series
* QLogic 8200 series

[amd64] **qlxgbe(4)** 驱动支持基于以下芯片组的 10 千兆以太网和 CNA 适配器：

* QLogic 8300 series

[amd64] **qlxge(4)** 驱动支持基于以下芯片组的 10 千兆以太网和 CNA 适配器：

* QLogic 8100 series

**re(4)** 驱动支持基于 RealTek RTL8139C+、RTL8169、RTL816xS、RTL811xS、RTL8168、RTL810xE 和 RTL8111 的快速以太网和千兆以太网适配器，包括：
...
[i386, pc98, amd64] **rue(4)** 驱动支持基于 RealTek RTL8150 的 USB 以太网适配器，包括：
...
**sfxge(4)** 驱动支持所有基于 Solarflare SFC9000 系列控制器的 10Gb 以太网适配器。
...
**sis(4)** 驱动支持基于 Silicon Integrated Systems SiS 900 和 SiS 7016 的快速以太网适配器和嵌入式控制器，以及基于 National Semiconductor DP83815（MacPhyter）和 DP83816 芯片的快速以太网适配器。支持的适配器包括：
...
[i386, amd64] **sn(4)** 驱动支持基于 SMC91Cxx 的 ISA 和 PCMCIA 卡，包括：
...
**sn(4)** 驱动支持来自 SMC 的 SMC 91C90、SMC 91C92、SMC 91C94、SMC 91C95、SMC 91C96、SMC91C100 和 SMC 91C100FD 芯片。

Farallon EtherWave 和 EtherMac 卡有两种变体。**ep(4)** 驱动支持 595 和 895 卡。这些卡正面有蓝色箭头和 3Com 标志。Farallon 595a 卡正面有红色箭头，也称为 EtherWave 和 EtherMac。它们由 **sn(4)** 驱动支持。
...
**snc(4)** 驱动还包括对 National Semiconductor NS46C46 作为 64 * 16 位微波串行 EEPROM 的支持。

**ste(4)** 驱动支持基于 Sundance Technologies ST201 的快速以太网适配器和嵌入式控制器，包括：

* D-Link DFE-530TXS
* D-Link DFE-550TX
* D-Link DFE-580TX

**stge(4)** 驱动支持基于 Sundance/Tamarack TC9021 千兆以太网控制器芯片的各种 NIC，包括：
...
[i386, pc98, amd64, sparc64] **ti(4)** 驱动支持基于 Alteon Tigon I 和 II 芯片的千兆以太网适配器。**ti(4)** 驱动已测试过以下适配器：
...
[i386, pc98, amd64] **tl(4)** 驱动支持基于 Texas Instruments ThunderLAN 的以太网和快速以太网适配器，包括大量 Compaq PCI 以太网适配器。还支持：
...
**tl(4)** 驱动还支持各种 Compaq Prosignia 服务器和 Compaq Deskpro 台式机的内置以太网适配器，包括：
...
[i386, pc98, amd64] **udav(4)** 驱动支持以下适配器：

* Corega FEther USB-TXC
* ShanTou ST268 USB NIC

**urndis(4)** 驱动支持许多 Android 设备的功能。

**vge(4)** 驱动支持基于 VIA Networking VT6120、VT6122、VT6130 和 VT6132 的千兆以太网适配器，包括：

* VIA Networking LAN-on-motherboard Gigabit Ethernet
* ZyXEL GN650-T 64-bit PCI Gigabit Ethernet NIC (ZX1701)
* ZyXEL GN670-T 32-bit PCI Gigabit Ethernet NIC (ZX1702)

**vr(4)** 驱动支持基于 VIA Technologies Rhine I、Rhine II 和 Rhine III 的快速以太网适配器，包括：
...
[i386, amd64] **vxge(4)** 驱动支持 <http://www.neterion.com> 中列出的 Neterion X3100 10 千兆以太网适配器。

[i386, pc98, amd64] **wb(4)** 驱动支持基于 Winbond W89C840F 的快速以太网适配器和嵌入式控制器，包括：

* Trendware TE100-PCIE

[i386, amd64] **xe(4)** 驱动支持以下卡：
...
3C656 系列 CardBus 卡和 3C556 系列 MiniPCI 卡均内置专用调制解调器。**xl(4)** 驱动和其他 FreeBSD 驱动均不支持此调制解调器。
...
[i386, pc98] 基于 IDT77252 的 ATM 卡（**patm(4)** 驱动）

### 无线网络接口

[amd64, i386, pc98] Cisco/Aironet 802.11b wireless adapters (**an(4)** driver)

[i386, pc98, amd64, sparc64] **ath(4)** 驱动支持所有 Atheros Cardbus 和 PCI 卡，基于 AR5005VL 芯片组的除外。

**bwi(4)** 驱动支持基于 Broadcom BCM43xx 的无线设备，包括：
...
**bwi(4)** 驱动使用 Broadcom 固件的旧版本 v3。虽然此旧固件支持大多数 BCM43xx 部件，但 **bwn(4)** 驱动在其支持的新芯片上工作得更好。如果使用较旧的 Broadcom 芯片组（BCM4301、BCM4303 和 BCM4306 rev 2），必须使用 **bwi(4)** 驱动。**bwn(4)** 使用的 v4 版本固件不支持这些芯片。

**bwn(4)** 驱动支持基于 Broadcom BCM43xx 的无线设备，包括：
...
[i386, amd64] **otus(4)** 驱动支持 Atheros AR9170 USB IEEE 802.11b/g/n 无线网络适配器，包括：
...
[i386, amd64] **ral(4)** 驱动支持基于 Ralink Technology 芯片组的 PCI/PCIe/CardBus 无线适配器，包括：
...
[i386, amd64] **rsu(4)** 驱动支持 Realtek RTL8188SU/RTL8192SU USB IEEE 802.11b/g/n 无线网络适配器，包括：
...
基于 Realtek RTL8188CE 的 PCIe IEEE 802.11b/g/n 无线网络适配器（**rtwn(4)** 驱动）

[i386, amd64] **rum(4)** 驱动支持基于 Ralink RT2501USB 和 RT2601USB 芯片组的 USB 2.0 无线适配器，包括：
...
**upgt(4)** 驱动支持基于 GW3887 芯片组的 USB 2.0 Conexant/Intersil PrismGT 系列无线适配器，其中包括：
...
[i386, amd64] **ural(4)** 驱动支持基于 Ralink Technology RT2500USB 芯片组的 USB 2.0 无线适配器，包括：
...
**urtw(4)** 驱动支持基于 Realtek RTL8187B/L 的无线网络设备，包括：
...
**urtwn(4)** 驱动支持基于 Realtek RTL8188CU/RTL8188RU/RTL8188EU/RTL8192CU 的 USB IEEE 802.11b/g/n 无线网络适配器，包括：
...
[i386, pc98] **ce(4)** 驱动支持以下型号的 Tau-PCI/32 WAN 适配器：
...
[i386, pc98] **cp(4)** 驱动支持以下型号的 Tau-PCI WAN 适配器：
...
[i386] **rc(4)** 驱动提供对 SDL Communications RISCom/8 板卡的支持。

[i386, amd64] 支持 Specialix SI/XIO/SX 多端口串行卡，包括较早的 SIHOST2.x 和“增强型”（基于 transputer，又称 JET）主机卡（ISA、EISA 和 PCI）。注意，较新的 SX+ PCI 卡目前不受支持。（**si(4)** 驱动）
...
[i386, amd64] **snd_ad1816(4)** 驱动支持以下声卡：

* Analog Devices AD1816

[i386] **snd_als4000(4)** 驱动支持以下声卡：

* Avance Logic ALS4000

[i386, amd64] **snd_atiixp(4)** 驱动支持以下音频芯片组：

* ATI IXP 200
* ATI IXP 300
* ATI IXP 400

[sparc64] **snd_audiocs(4)** 驱动支持以下音频设备：

* CS4231 on SBus based UltraSPARC
* CS4231 on PCI/EBus based UltraSPARC

[i386, amd64] **snd_cmi(4)** 驱动支持以下声卡：
...
[i386, amd64] **snd_cs4281(4)** 驱动支持以下声卡：

* Crystal Semiconductor CS4281

[i386, amd64] **snd_csa(4)** 驱动支持以下声卡：
...
部分板载 CS4610 芯片搭配的是 CS423x ISA 编解码器而非 CS4297 AC97 编解码器。**snd_csa(4)** 驱动尚不支持此类配置。

[i386, amd64] **snd_ds1(4)** 驱动支持以下声卡：

* Yamaha DS-1
* Yamaha DS-1E

[i386, amd64] **snd_emu10k1(4)** 驱动支持以下声卡：
...
[i386, amd64] **snd_emu10kx(4)** 驱动支持以下声卡：
...
**snd_emu10kx(4)** 驱动 *不* 支持以下声卡（尽管它们的名称与某些受支持的声卡相似）：
...
[i386, amd64] **snd_envy24(4)** 驱动支持以下音频设备：
...
[i386, amd64] **snd_envy24ht(4)** 驱动支持以下音频设备：
...
[i386, sparc64, amd64] **snd_es137x(4)** 驱动支持以下声卡：
...
[i386, amd64] **snd_ess(4)** 驱动支持以下声卡：

* Ensoniq ESS ISA PnP/non-PnP

[i386, amd64] **snd_fm801(4)** 驱动支持基于以下芯片组的音频设备：

* Forte Media FM801

[i386, amd64] **snd_gusc(4)** 驱动支持以下声卡：

* Gravis UltraSound MAX
* Gravis UltraSound PnP

[i386, amd64] **snd_hda(4)** 驱动支持具有 PCI 类 4（多媒体）和子类 3（HDA）、兼容 Intel HDA 规范的控制器。

**snd_hda(4)** 驱动支持超过两百种不同的控制器和 CODEC。在此全部列出没有意义，因为在大多数情况下，具体的 CODEC 配置和接线比 CODEC 本身的类型更为重要。

[i386, amd64] **snd_hdspe(4)** 驱动支持以下音频设备：

* RME HDSPe AIO
* RME HDSPe RayDAT

[i386, amd64] **snd_ich(4)** 驱动支持以下音频设备：
...
[i386, amd64] **snd_maestro(4)** 驱动支持以下 PCI 声卡：

* ESS Technology Maestro-1
* ESS Technology Maestro-2
* ESS Technology Maestro-2E

[i386, amd64] **snd_maestro3(4)** 驱动支持以下音频设备：

* ESS Technology Allegro-1
* ESS Technology Maestro3

[i386] **snd_mss(4)** 驱动支持以下音频设备：
...
[i386, amd64] **snd_neomagic(4)** 驱动支持以下音频设备：

* NeoMagic 256AV
* NeoMagic 256ZX

[i386, amd64] **snd_sbc(4)** 驱动支持以下声卡：
...
[i386, amd64] **snd_solo(4)** 驱动支持以下声卡：
...
[i386, amd64] **snd_spicds(4)** 驱动支持以下编解码器：
...
[i386, amd64, sparc64] **snd_t4dwave(4)** 驱动支持以下音频设备：
...
[i386, amd64] **snd_via8233(4)** 驱动支持以下音频芯片组：
...
[i386, amd64] **snd_via82c686(4)** 驱动支持基于以下芯片组的音频设备：

* VIA 82C686A

[i386, amd64] **snd_vibes(4)** 驱动支持基于以下芯片组的音频设备：
...
[i386, pc98] **bktr(4)** 驱动支持基于 Brooktree Bt848/849/878/879 芯片的视频捕获卡，以及 Pinnacle PCTV 卡，包括：
...
[amd64, i386, pc98] 支持各种 USB 外设；已知可用的设备列于本节。由于大多数 USB 设备具有通用性，除少数例外，任何指定类别的设备都将受支持，即使未在此明确列出。

> **注意**：
>
> [amd64, i386, pc98] USB 以太网适配器可在列出 [以太网接口](https://www.freebsd.org/releases/11.2R/hardware.html#ethernet) 的章节中找到。

> **注意**：
>
> [amd64, i386, pc98] USB 蓝牙适配器可在 [Bluetooth](https://www.freebsd.org/releases/11.2R/hardware.html#bluetooth) 章节中找到。

[i386, pc98, amd64, powerpc] **ohci(4)** 驱动支持所有符合 OHCI v1.0 规范的控制器，包括：
...
[i386, pc98, amd64, powerpc] **uhci(4)** 驱动支持所有符合 UHCI v1.1 规范的控制器，包括：
...
[i386, pc98, amd64, powerpc] 以下设备受 **urio(4)** 驱动支持：

* Diamond MultiMedia Rio 500
* Diamond MultiMedia Rio 600
* Diamond MultiMedia Rio 800

[i386, pc98, amd64] **umodem(4)** 驱动支持的设备包括：
...
[i386, pc98, amd64, powerpc] **ulpt(4)** 驱动提供对 USB 打印机和并口打印转换线缆的支持，包括以下设备：
...
[i386, pc98, amd64] **ubsa(4)** 驱动支持以下适配器：
...
[i386, pc98, amd64] **ubser(4)** 驱动提供对 BWCT 控制台管理串行适配器的支持。

[i386, pc98, amd64] **uftdi(4)** 驱动支持以下适配器：
...
[i386, pc98, amd64] **uplcom(4)** 驱动支持以下设备和适配器：
...
[i386, pc98, amd64, powerpc] **umass(4)** 驱动支持 USB 大容量存储设备，包括：
...
[i386, pc98, amd64] **uvisor(4)** 驱动支持以下设备：
...
[i386, sparc64, amd64, powerpc] **fwohci(4)** 驱动提供对 PCI/CardBus FireWire 接口卡的支持。该驱动支持以下 IEEE 1394 OHCI 芯片组：
...
[i386, pc98, amd64] **ng_bt3c(4)** 驱动提供对 3Com/HP 3CRWB6096-A PCCARD 蓝牙适配器的支持。

[i386, pc98, amd64] **ng_ubt(4)** 驱动支持所有符合蓝牙规范 v1.1 的蓝牙 USB 设备，包括：
...
[i386, pc98, amd64] **hifn(4)** 驱动支持包含 Hifn 7751、7951、7811、7955 和 7956 芯片组的各类卡，例如：
...
[i386, pc98, amd64] **safe(4)** 驱动支持包含以下任一芯片的卡：

* SafeNet 1141
* SafeNet 1741

[i386, pc98, amd64] **ubsec(4)** 驱动支持包含以下任一芯片的卡：
...
* [amd64, i386, pc98] USB keyboards (**ukbd(4)** driver)
...
* [amd64, i386, pc98] USB mice (**ums(4)** driver)

> **注意**：
>
> **moused(8)** 手册页提供了在 FreeBSD 中使用指针设备的更多信息。关于在 Xorg 中使用指针设备的信息可在 `http://www.x.org/` 找到。

[amd64, i386] “PC standard”并口（**ppc(4)** 驱动）

[pc98] “PC-9821 standard”并口（**ppc(4)** 驱动）

[i386, amd64] PC 兼容操纵杆（**joy(4)** 驱动）

[pc98] SoundBlaster(98) 的操纵杆端口（**joy(4)** 驱动）
