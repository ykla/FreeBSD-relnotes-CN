...
在 2.0.5 之前，FreeBSD 会在探测 SCSI 设备时动态分配设备编号，这使得 SCSI 设备故障可能改变设备编号，使仍然正常工作的磁盘上的文件系统无法挂载。硬编码则可基于 SCSI ID 和总线的静态分配设备编号（即设备名称）。SCSI 配置在内核配置文件中进行。可以在 `scsi(4)` 手册页或 `LINT` 内核配置文件中找到配置语法示例。
