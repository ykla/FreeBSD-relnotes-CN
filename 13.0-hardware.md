...
所有 64 位 x86 处理器均受支持，包括 AMD 的 AMD64 CPU 和 Intel 的 Intel(R) 64 CPU（包括 Xeon 和 Core 处理器）。FreeBSD/amd64 支持多核和 SMT（Symmetric MultiThreading，对称多线程，在 Intel(R) CPU 上又称 HyperThreading）处理器。

注意，此架构有多种名称，包括 x86-64、AMD64、Intel EM64T 和 IA-32e。

FreeBSD/amd64 需要高级配置与电源管理（ACPI）支持。ACPI 由 Intel(R) 的 ACPI Component Architecture 参考实现提供支持，如 [acpi(4)](https://man.freebsd.org/cgi/man.cgi?query=acpi&sektion=4&format=html) 手册页所述。
...
所有带浮点单元且支持 CMPXCHG 指令（自 80486 起引入）的 32 位 x86 兼容处理器均受支持。包括 Intel 的 Pentium(R) 系列处理器，以及 AMD、Cyrix、NexGen 和 VIA 的 x86 兼容处理器。
...
对于所有受支持的 SCSI 控制器，完整支持 SCSI-I、SCSI-II 和 SCSI-III 外围设备，包括硬盘、光盘、磁带驱动器（包括 DAT、8mm Exabyte、Mammoth 和 DLT）、介质更换器、处理器目标设备和 CD-ROM 驱动器。支持 CD-ROM 命令的 WORM 设备可由 CD-ROM 驱动器（如 [cd(4)](https://man.freebsd.org/cgi/man.cgi?query=cd&sektion=4&format=html)）进行只读访问。WORM/CD-R/CD-RW 写入支持由 [cdrecord(1)](https://man.freebsd.org/cgi/man.cgi?query=cdrecord&sektion=1&format=html) 提供，它是 Ports Collection 中 sysutils/cdrtools port 的一部分。
