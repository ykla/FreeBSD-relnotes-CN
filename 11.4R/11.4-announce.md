# FreeBSD 11.4-RELEASE 发行公告

日期：2020 年 6 月 23 日

FreeBSD 发布工程团队很高兴地宣布 FreeBSD 11.4-RELEASE 现已发布。这是 stable/11 分支的第五个，也是最后一个版本。

部分亮点如下：

* `clang`、`llvm`、`lld`、`lldb` 和 `compiler-rt` 工具以及 `libc++` 已更新至上游版本 10.0.0。
* OpenSSL 已更新至 1.0.2u 版本。
* Unbound 已更新至 1.9.6 版本。
* [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html) 工具已更新至 1.13.2 版本。
* KDE 桌面环境已更新至 5.18.4.1.19.12.3 版本。
* GNOME 桌面环境已更新至 3.28 版本。
* 新增对重命名 ZFS 书签的支持。
* 新增 [certctl(8)](https://man.freebsd.org/cgi/man.cgi?query=certctl&sektion=8&format=html) 工具。
* 对用户态应用程序进行了多项功能添加和更新。
* 在未来版本中弃用的功能警告现在将在所有 FreeBSD 版本上打印。
* 新增针对 RFC 6649 和 8429 中弃用的 Kerberos GSS API 算法的警告。
* 以及更多……

完整的新功能和已知问题列表，请参阅在线发行说明和勘误列表，地址如下：

* <https://www.FreeBSD.org/releases/11.4R/relnotes/>
...
FreeBSD 项目将 FreeBSD 11.4-RELEASE 献给 Bruce Evans 以表纪念。

## 获取途径

FreeBSD 11.4-RELEASE 现已支持 amd64、i386、powerpc、powerpc64、sparc64、armv6 和 aarch64 架构。

FreeBSD 11.4-RELEASE 可从可启动 ISO 镜像或通过网络安装。部分架构还支持从 USB 闪存盘安装。所需文件可按以下说明下载。

本消息底部包含发行版 ISO、闪存盘和 SD 卡镜像的 SHA512 和 SHA256 校验和。

发行版镜像的 PGP 签名校验和也可从以下地址获取：

* <https://www.FreeBSD.org/releases/11.4R/signatures/>

本公告的 PGP 签名版本可在以下地址获取：

* <https://www.FreeBSD.org/releases/11.4R/announce.asc>
...
```sh
# dd if=FreeBSD-11.4-RELEASE-arm-armv6-KERNEL.img \
    of=/dev/da0 bs=1m conv=sync
```

务必确保目标（`of=`）正确。

FreeBSD 11.4-RELEASE 也可从多家供应商处购买 CD-ROM 或 DVD 介质。提供 FreeBSD 11.4 相关产品的供应商之一是：

* FreeBSD Mall, Inc. <https://www.freebsdmall.com>

预安装的虚拟机镜像也可用于 amd64（x86_64）、i386（x86_32）和 AArch64（arm64）架构，提供 `QCOW2`、`VHD` 和 `VMDK` 磁盘镜像格式，以及原始（未格式化）镜像。

FreeBSD 11.4-RELEASE amd64 还可在以下云托管平台上使用：
...
FreeBSD 11.4-RELEASE 预计还将在 Google Compute Engine(TM) Marketplace 中提供，待其完成第三方特定验证后，访问：
...
FreeBSD 11.4-RELEASE 可从以下站点通过 https 下载：

* <https://download.freebsd.org/ftp/releases/ISO-IMAGES/11.4/>

FreeBSD 11.4-RELEASE 虚拟机镜像可从以下地址下载：

* <https://download.freebsd.org/ftp/releases/VM-IMAGES/11.4-RELEASE/>

关于安装 FreeBSD 或将现有机器更新至 11.4-RELEASE 的说明，请参阅：

* <https://www.FreeBSD.org/releases/11.4R/installation/>

## 支持

根据新的 FreeBSD 支持模型，FreeBSD 11 发行系列将至少支持至 2021 年 9 月 30 日。

* <https://www.FreeBSD.org/security/>

请注意，11.3 将支持至 11.4 发行日期起三个月后，目前计划为 2020 年 9 月 30 日。

## 致谢

许多公司捐赠了设备、网络访问或人力时间来支持 FreeBSD 11.4 的发布工程活动，包括：
...
| [National Chiao Tung University](https://www.cs.nctu.edu.tw) |
| [NLNet Labs](https://www.nlnetlabs.nl) |
| [iXsystems](https://www.ixsystems.com) |

11.4-RELEASE 的发布工程团队包括：

| 成员 | 角色 |
| :--- | :--- |
| Glen Barber <gjb@FreeBSD.org> | 发布工程负责人，11.4-RELEASE 发布工程师 |
