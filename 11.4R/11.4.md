...
FreeBSD 11.4-RELEASE 的发行说明包含在 11.4-STABLE 开发分支上对 FreeBSD 基本系统所做变更的总结。本文档列出了自上一个版本发布以来发布的相关安全公告，以及对 FreeBSD 内核和用户空间的重大变更。还简要介绍了升级的相关说明。

## 引言

本文档包含 FreeBSD 11.4-RELEASE 的发行说明。它介绍了 FreeBSD 中最近添加、变更或删除的功能。还提供了一些关于从旧版本 FreeBSD 升级的说明。
...
本节涉及对用户空间应用程序、第三方软件和系统实用程序的变更和新增内容。
...
[camcontrol(8)](https://man.freebsd.org/cgi/man.cgi?query=camcontrol&sektion=8&format=html) 工具已更新，支持通过 `modepage` 子命令使用块描述符。[r351582](http://svn.freebsd.org/viewvc/base?view=revision&revision=351582)
...
[usbconfig(8)](https://man.freebsd.org/cgi/man.cgi?query=usbconfig&sektion=8&format=html) 工具新增命令 `detach_kernel_driver`。[r351843](http://svn.freebsd.org/viewvc/base?view=revision&revision=351843)
...
[env(1)](https://man.freebsd.org/cgi/man.cgi?query=env&sektion=1&format=html) 工具新增了选项 `-L` 和 `-U`，分别用于根据 **login.conf** 和 **~/.login_conf** 为指定用户设置环境。[r357791](http://svn.freebsd.org/viewvc/base?view=revision&revision=357791)
...
增加了修复以正确链接支持 DTrace 的 Ports 与 lld 的功能。[r361217](http://svn.freebsd.org/viewvc/base?view=revision&revision=361217)
...
[ena(4)](https://man.freebsd.org/cgi/man.cgi?query=ena&sektion=4&format=html) 驱动已更新至 2.2.0 版本。[r361539](http://svn.freebsd.org/viewvc/base?view=revision&revision=361539)（由 Amazon, Inc. 赞助）
