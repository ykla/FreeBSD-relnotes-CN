...
Farallon EtherWave 和 EtherMac 卡有两种变体。**ep(4)** 驱动支持 595 和 895 卡。这些卡正面有蓝色箭头和 3Com 标志。Farallon 595a 卡正面有红色箭头，也称为 EtherWave 和 EtherMac。它们由 [sn(4)](https://man.freebsd.org/cgi/man.cgi?query=sn&sektion=4&format=html) 驱动支持。
...
3C656 系列 CardBus 卡和 3C556 系列 MiniPCI 卡均内置专有调制解调器。[xl(4)](https://man.freebsd.org/cgi/man.cgi?query=xl&sektion=4&format=html) 驱动和其他 FreeBSD 驱动均不支持此调制解调器。
...
* Creative Sound Blaster Audigy 2 Value（CA0108 芯片组）。PCM 支持仅限于 48kHz/16 位立体声（不支持该芯片组的 192kHz/24 位部分）。此声卡不支持 MIDI。
...
[amd64, i386, pc98] 支持一系列 USB 外设；本节列出了已知可用的设备。由于大多数 USB 设备具有通用性，除少数例外，任何给定类别的设备都将获得支持，即使在此未明确列出。

> **注意**：
>
> [amd64, i386, pc98] USB 以太网适配器可在列出 [以太网接口](https://www.freebsd.org/releases/11.4R/hardware.html#ethernet) 的章节中找到。

> **注意**：
>
> [amd64, i386, pc98] USB 蓝牙适配器可在 [Bluetooth](https://www.freebsd.org/releases/11.4R/hardware.html#bluetooth) 章节中找到。
...
[amd64, i386, pc98] 使用 EHCI 接口的 USB 2.0 控制器（[ehci(4)](https://man.freebsd.org/cgi/man.cgi?query=ehci&sektion=4&format=html) 驱动）

[amd64, i386, pc98] 集线器

[amd64, i386, pc98] 键盘（[ukbd(4)](https://man.freebsd.org/cgi/man.cgi?query=ukbd&sektion=4&format=html) 驱动）
...
[amd64, i386, pc98] 鼠标（[ums(4)](https://man.freebsd.org/cgi/man.cgi?query=ums&sektion=4&format=html) 驱动）
...
* [pc98] 标准键盘
* [amd64, i386, pc98] USB 键盘（[ukbd(4)](https://man.freebsd.org/cgi/man.cgi?query=ukbd&sektion=4&format=html) 驱动）

[amd64, i386, pc98] 指点设备包括：
...
* 串口鼠标及兼容设备
* [amd64, i386, pc98] USB 鼠标（[ums(4)](https://man.freebsd.org/cgi/man.cgi?query=ums&sektion=4&format=html) 驱动）

> **注意**：
>
> [moused(8)](https://man.freebsd.org/cgi/man.cgi?query=moused&sektion=8&format=html) 包含关于在 FreeBSD 中使用指点设备的更多信息。关于在 Xorg 中使用指点设备的信息可在 `http://www.x.org/` 找到。

[amd64, i386] “PC 标准”并口（[ppc(4)](https://man.freebsd.org/cgi/man.cgi?query=ppc&sektion=4&format=html) 驱动）

[pc98] “PC-9821 标准”并口（[ppc(4)](https://man.freebsd.org/cgi/man.cgi?query=ppc&sektion=4&format=html) 驱动）
...
[i386] 与来自 [Virtual Computers](http://www.vcc.com/) 的 HOT1 兼容的、基于 Xilinx XC6200 的可重配置硬件卡（xrpu 驱动）。
