...
务必确保目标（`of=`）正确。
disc1::
包含 FreeBSD 基础操作系统。还支持启动到基于「livefs」的救援模式。
不含预构建的 package。
...
SD 卡镜像包含完整的 FreeBSD 安装，可安装到小至 5 GiB 的 SD 卡上。

为方便无控制台访问系统的用户，默认提供 `freebsd` 用户（密码为 `freebsd`）用于 `ssh(1)` 访问。
此外，`root` 用户密码设为 `root`，强烈建议在获取系统访问权限后立即更改这两个用户的密码。
...
喜欢 FreeBSD 吗？
请通过向 The FreeBSD Foundation [捐赠](https://freebsdfoundation.org/donate/) 来支持本版本及未来版本的发布！
