...
FreeBSD 支持 AMD64（"Hammer"）和 Intel(R) EM64T 架构。
...
Raspberry Pi，更多信息请参阅 [Raspberry Pi Wiki page](https://wiki.freebsd.org/arm/Raspberry%20Pi)：
...
WORM devices that support CD-ROM commands are supported for read-only access by the CD-ROM drivers (such as [cd(4)](https://man.freebsd.org/cgi/man.cgi?query=cd&sektion=4&format=html)).
WORM/CD-R/CD-RW writing support is provided by [cdrecord(1)](https://man.freebsd.org/cgi/man.cgi?query=cdrecord&sektion=1&format=html), which is a part of the package:sysutils/cdrtools[] port in the Ports Collection.
...
Intel(R) Core(TM) 处理器（自第四代起）、Intel(R)
