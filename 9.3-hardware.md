# FreeBSD 9.3-RELEASE 硬件兼容列表

## 引言

本文档包含 FreeBSD 9.3-RELEASE 的硬件兼容性说明。其中列出了 FreeBSD 支持的硬件平台，以及各类硬件设备（存储控制器、网络接口等），并给出已知可正常工作的设备实例。
...
* 所有 Intel(R) Core 2（非 Core Duo）及更新处理器。
* 所有 Intel(R) Pentium(R) D 处理器。
* 采用 "Cedar Mill" 核心的 Intel(R) Pentium(R) 4 和 Celeron D 支持 EM64T。
* 采用 "Prescott" 核心的部分 Intel(R) Pentium(R) 4 和 Celeron D 支持 EM64T。关于 Intel 处理器的 EM64T 支持情况，请参阅 [Intel Processor Spec Finder](http://processorfinder.intel.com) 获取确切答案。
...
本节介绍 FreeBSD 在 PowerPC 平台上当前已知支持的系统。此列表并非详尽无遗。

通常，所有 New World 架构的 Apple 硬件均受支持，以及少量非 Apple 机器。

以下列出的所有系统均受完全支持，但部分 Power Macintosh G5 型号目前缺少软件风扇控制。在所有拥有 1 个以上处理器的系统上支持 SMP。
...
* Apple iBook G3
* Apple iBook G4
* Apple PowerBook G3（Lombard 和 Pismo）
* Apple PowerBook G4
* Apple XServe G4
* Apple XServe G5
* Apple Mac Mini
...
如果你有不在此列表中的系统，它可能尚未经过 FreeBSD 9.3-RELEASE 测试。我们鼓励你尝试并致信 [FreeBSD SPARC porting mailing list](http://lists.FreeBSD.org/mailman/listinfo/freebsd-sparc64) 反馈结果，包括哪些设备可用、哪些不可用。
...
此外，与 ata(4) 的 atamarvell 和 atajmicron 驱动协作，它还支持 legacy-PATA + AHCI-SATA 组合控制器中的 AHCI 部分，如 JMicron JMB36x 和 Marvell 88SE61xx。
...
[hptiop(4)](https://man.freebsd.org/cgi/man.cgi?query=hptiop&sektion=4&format=html) 驱动还支持以下已停止生产的 SAS 和 SATA RAID 控制器：
...
[sym(4)](https://man.freebsd.org/cgi/man.cgi?query=sym&sektion=4&format=html) 驱动支持的 SCSI 控制器可以是主板内置的，也可以是以下附加卡之一：
...
[i386, amd64] [bce(4)](https://man.freebsd.org/cgi/man.cgi?query=bce&sektion=4&format=html) 驱动支持基于 QLogic NetXtreme II 系列 Gigabit Ethernet 控制器芯片的多种 NIC，包括以下：
...
目前已知与 [cas(4)](https://man.freebsd.org/cgi/man.cgi?query=cas&sektion=4&format=html) 驱动配合工作的附加卡包括：
...
[amd64] [qlxgb(4)](https://man.freebsd.org/cgi/man.cgi?query=qlxgb&sektion=4&format=html) 驱动支持基于以下芯片组的 10 Gigabit Ethernet & CNA Adapter：

* QLogic 3200 series
* QLogic 8200 series

[amd64] [qlxgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=qlxgbe&sektion=4&format=html) 驱动支持基于以下芯片组的 10 Gigabit Ethernet & CNA Adapter：

* QLogic 8300 series

[amd64] [qlxge(4)](https://man.freebsd.org/cgi/man.cgi?query=qlxge&sektion=4&format=html) 驱动支持基于以下芯片组的 10 Gigabit Ethernet & CNA Adapter：
...
[sfxge(4)](https://man.freebsd.org/cgi/man.cgi?query=sfxge&sektion=4&format=html) 驱动支持所有基于 Solarflare SFC9000 系列控制器的 10Gb Ethernet 适配器。
...
[vte(4)](https://man.freebsd.org/cgi/man.cgi?query=vte&sektion=4&format=html) 驱动支持以下 Ethernet 控制器：
...
[bwi(4)](https://man.freebsd.org/cgi/man.cgi?query=bwi&sektion=4&format=html) 驱动使用 Broadcom 固件的较旧 v3 版本。虽然此较旧固件支持大多数 BCM43xx 部件，但 bwn(4) 驱动对其支持的较新芯片工作得更好。如果使用较旧的 Broadcom 芯片组（BCM4301、BCM4303 和 BCM4306 rev 2），必须使用 [bwi(4)](https://man.freebsd.org/cgi/man.cgi?query=bwi&sektion=4&format=html) 驱动。bwn(4) 使用的固件 v4 版本不支持这些芯片。
...
最新的列表可在 <http://customerproducts.atheros.com/customerproducts/default.asp> 找到。
...
最新的列表可在 <http://ralink.rapla.net/> 找到。
...
[i386, amd64] [snd_hda(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hda&sektion=4&format=html) 驱动支持具有 PCI 类 4（multimedia）和子类 3（HDA）、与 Intel HDA 规范兼容的控制器。
...
> **注意**：
>
> [amd64, i386, ia64, pc98] USB 以太网适配器可在 <<ethernet,以太网接口>> 章节中找到。

> **注意**：
>
> [amd64, i386, ia64, pc98] USB Bluetooth 适配器可在 <<bluetooth,Bluetooth>> 章节中找到。
...
> **注意**：
>
> 关于特定显卡及与 Xorg 兼容性的信息，请访问 `http://www.x.org/`。
...
> **注意**：
>
> [moused(8)](https://man.freebsd.org/cgi/man.cgi?query=moused&sektion=8&format=html) 提供了在 FreeBSD 中使用定点设备的更多信息。关于在 Xorg 中使用定点设备的信息，请访问 `http://www.x.org/`。
