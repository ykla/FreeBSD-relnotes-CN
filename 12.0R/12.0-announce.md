...
* OpenSSL 更新至版本 1.1.1a (LTS)。
* Unbound 更新至版本 1.8.1，并默认启用 DANE-TA。
* OpenSSH 更新至版本 7.8p1。
...
* Clang、LLVM、LLD、LLDB、compiler-rt 和 libc++ 更新至版本 6.0.1。
* [vt(4)](https://man.freebsd.org/cgi/man.cgi?query=vt&sektion=4&format=html) Terminus BSD 控制台字体更新至版本 4.46。
* [bsdinstall(8)](https://man.freebsd.org/cgi/man.cgi?query=bsdinstall&sektion=8&format=html) 工具现在支持 UEFI+GELI 作为安装选项。
* `VIMAGE` 内核配置选项默认启用。
* 选项 `NUMA` 默认启用，适用于 amd64 **GENERIC** 和 **MINIMAL** 内核配置。
* 新增 [netdump(4)](https://man.freebsd.org/cgi/man.cgi?query=netdump&sektion=4&format=html) 驱动，通过它可以在系统崩溃后将内核崩溃转储传输到远程主机。
* [vt(4)](https://man.freebsd.org/cgi/man.cgi?query=vt&sektion=4&format=html) 驱动性能得到改进，文本绘制速度提高了 2 到 6 倍。
* 对当前一代硬件的图形支持进行了多项改进。
* 默认启用了 armv6 和 armv7 上的 [capsicum(4)](https://man.freebsd.org/cgi/man.cgi?query=capsicum&sektion=4&format=html) 支持。
* UFS/FFS 文件系统更新，合并了 `TRIM`/`BIO_DELETE` 命令，由于同时发送的 `TRIM` 消息减少，从而减少了读写请求。
* NFS 版本 4.1 服务器更新，包含 pNFS 服务器支持。
...
* [bhyve(8)](https://man.freebsd.org/cgi/man.cgi?query=bhyve&sektion=8&format=html) 工具更新，增加了 NVMe 设备仿真。
...
为方便没有控制台访问权限的用户，默认提供 `freebsd` 用户（密码为 `freebsd`）用于 [ssh(1)](https://man.freebsd.org/cgi/man.cgi?query=ssh&sektion=1&format=html) 访问。此外，`root` 用户密码设置为 `root`，强烈建议在获得系统访问权限后更改这两个用户的密码。
...
FreeBSD 12.0-RELEASE 预计还将在 Google Compute Engine(TM) Marketplace 中提供，待其完成第三方特定验证后，访问：
...
根据围绕 FreeBSD 支持模型审查的讨论，FreeBSD 12 发行系列将至少支持至 2020 年 6 月 30 日。更多信息请参阅关于支持模型即将进行的讨论的 [官方公告](https://lists.freebsd.org/pipermail/freebsd-announce/2018-November/001854.html)。

* <https://www.FreeBSD.org/security/>

## 致谢

许多公司捐赠了设备、网络访问或工时来支持 FreeBSD 12.0 的发布工程活动，包括 FreeBSD 基金会、Yahoo!、NetApp、Internet Systems Consortium、ByteMark Hosting、Sentex Communications、New York Internet、Juniper Networks、NLNet Labs、iXsystems、CyberOne Data 和 National Chiao Tung University。
...
热爱 FreeBSD？请向 FreeBSD 基金会[捐赠](https://www.freebsdfoundation.org/donate/) 以支持此版本及未来版本！
