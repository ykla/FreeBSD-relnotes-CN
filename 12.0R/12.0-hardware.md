...
[mpt(4)](https://man.freebsd.org/cgi/man.cgi?query=mpt&sektion=4&format=html) 驱动支持的 Ultra 320 SCSI 控制器芯片在许多系统中以板载形式存在，包括：
...
SAS 控制器芯片也存在于许多基于 AMD/Opteron 的新系统中，例如 Sun 4100。注意，此控制器可同时驱动 SAS 和 SATA 硬盘，或同时混合使用两者。这些控制器可用的 Integrated RAID Mirroring 充其量只有有限的支持。

光纤通道控制器芯片组在多种速率和系统中受支持。Apple 光纤通道 HBA 实际上就是 FC949ES 卡。

此驱动还支持光纤通道卡的目标模式。可通过 LSI Logic 固件工具设置核心所需的角色来启用此支持，该工具用于设定卡可承担的角色——无需单独编译。
...
[amd64, i386] Crystal Semiconductor CS89x0-based NICs ([cs(4)](https://man.freebsd.org/cgi/man.cgi?query=cs&sektion=4&format=html) 驱动)
...
[ed(4)](https://man.freebsd.org/cgi/man.cgi?query=ed&sektion=4&format=html) 驱动不支持以下以太网网卡：
...
Agere ET1310 Gigabit Ethernet adapters ([et(4)](https://man.freebsd.org/cgi/man.cgi?query=et&sektion=4&format=html) 驱动)
...
部分板载 CS4610 芯片搭配的是 CS423x ISA 编解码器，而非 CS4297 AC97 编解码器。snd_csa 驱动目前尚不支持此类配置。
...
* Creative Sound Blaster Audigy (CA0100 和 CA0101 Chipset)。PCM 和两个 MIDI 接口可用。
* Creative Sound Blaster Audigy 2 和 Creative Sound Blaster Audigy 4 (CA0102 Chipset)。PCM 支持仅限于 48kHz/16 位立体声（不支持该芯片组的 192kHz/24 位部分）。
* Creative Sound Blaster Audigy 2 Value (CA0108 Chipset)。PCM 支持仅限于 48kHz/16 位立体声（不支持该芯片组的 192kHz/24 位部分）。此卡不支持 MIDI。

[snd_emu10kx(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_emu10kx&sektion=4&format=html) 驱动不支持以下声卡（尽管它们的名称与某些受支持的声卡相似）：
...
[amd64, i386] 支持多种 USB 外设；本节列出了已知可正常工作的设备。由于大多数 USB 设备具有通用性，除少数例外，任何特定类别的设备都会获得支持，即使未在此处明确列出。

> **注意**：
>
> [amd64, i386] USB 以太网适配器可在 [以太网接口](https://www.freebsd.org/releases/12.0R/hardware.html#ethernet) 章节中找到。

> **注意**：
>
> [amd64, i386] USB Bluetooth 适配器可在 [Bluetooth](https://www.freebsd.org/releases/12.0R/hardware.html#bluetooth) 章节中找到。
...
> **注意**：
>
> 有关特定显卡及与 Xorg 兼容性的信息，请访问 <http://www.x.org/>。
...
> **注意**：
>
> [moused(8)](https://man.freebsd.org/cgi/man.cgi?query=moused&sektion=8&format=html) 提供了有关在 FreeBSD 中使用指点设备的更多信息。有关在 Xorg 中使用指点设备的信息，请访问 <http://www.x.org/>。
