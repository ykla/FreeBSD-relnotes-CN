...
建议所有用户在安装 FreeBSD 前，先查看发布勘误（Release 勘误）。勘误文档会更新在发布周期后期或发布后发现的“最新”信息，通常包含已知的错误、安全公告及文档修正。可以在 FreeBSD 网站上找到最新的 FreeBSD 12.0-RELEASE 勘误。
...
新增了对 Microchip® LAN78xx™ USB3-GigE 控制器的支持。[[r333713]](http://svn.freebsd.org/viewvc/base?view=revision&revision=333713)（由 FreeBSD 基金会赞助）
...
Amazon® EC2™ AMI 实例现在默认安装 `sysutils/amazon-ssm-agent`，但该服务默认在 [rc.conf(5)](https://man.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5&format=html) 中未启用。要启用该服务，请添加： [[r325254]](http://svn.freebsd.org/viewvc/base?view=revision&revision=325254)（由 DARPA, AFRL 赞助）
...
Amazon® EC2™ AMI 实例现在默认禁用 PS/2 键盘和鼠标设备，使整体启动时间减少 2.5 秒。[[r338321]](http://svn.freebsd.org/viewvc/base?view=revision&revision=338321)
...
[pf(4)](https://man.freebsd.org/cgi/man.cgi?query=pf&sektion=4&format=html) ioctl 接口和 [pfctl(8)](https://man.freebsd.org/cgi/man.cgi?query=pfctl&sektion=8&format=html) 现在支持 `altq(4)` 带宽参数为 `2^32` bps 或更大的值。HFSC 调度规则已升级，可正确处理高达 100 Gbps 的带宽参数，提供给未升级调度规则的带宽参数将饱和至之前的 32 位限制。[[r338253]](http://svn.freebsd.org/viewvc/base?view=revision&revision=338253)
