...
- **负责人**: core
- **提交者**: Dietmar Friede (<dfriede@drnhh.neuhaus.de>) 和 Juergen Krause (<r@saarlink.de>)
- **相关源码**: gnu/isdn/*
...
- **负责人**: Olof Johansson (<fe@ludd.luth.se>)
...
一些控制器在处理大于 16MB 的内存时有一些限制，因为 ISA 总线只有 24 位的 DMA 地址空间。如果进行计算，你会发现这使得无法直接进行 DMA 到任何大于 16MB 的地址。某些 EISA 控制器（通常是 32 位的）在配置为模拟 ISA 卡时，也会遇到此问题，尤其是当它们配置为以 ISA 方式运行时。此问题不会影响 IDE 控制器（它们不使用 DMA）、真正的 EISA 控制器（如 UltraStor、Adaptec 1742A 或 Adaptec 2742）和大多数 VLB（本地总线）控制器。对于需要解决的情况，系统将使用“回弹缓冲区”与控制器进行通信，这样你仍然可以使用超过 16MB 内存而不遇到问题。
...
Jordan 还特别感谢 Poul-Henning Kamp 和 Gary Palmer，两人都投入了大量时间，帮助他构建新的安装工具。Poul 作为骄傲的新父亲，特别忙碌，但仍然设法投入了大量精力。这次发布没有他们的帮助是无法实现的！非常感谢你们！
