...
欢迎使用 3.3-RELEASE，这是对 1999 年 5 月发布的 3.2-RELEASE 的全面后续版本。在自 3.2 发布以来的几个月中，系统进行了许多错误修复和一般增强。请查看以下相关细节。

任何安装失败或崩溃应使用命令 `send-pr` 报告（喜欢使用基于网页的界面的用户也可以查看此页面）。

有关 FreeBSD 及 3.3-RELEASE 目录结构的信息（尤其是在从软盘安装时！），请参阅 `ABOUT.TXT`。有关安装说明，请参阅文件 `INSTALL.TXT` 和 `HARDWARE.TXT`。
...
- **Linux 模式** 进行了重大错误修复和改进。

- **i386 引导程序** 针对一些问题系统进行了增强。

- 增加了对基于 Lucent Hermes 芯片组的 IEEE 802.11 PCMCIA 无线网卡的驱动支持，包括 Lucent WaveLAN/IEEE 802.11 和 Cabletron RoamAbout。2Mbps 和 6Mbps Turbo 适配器均受支持。[合并]
...
- 集成了对 M-systems DiskOnChip 产品的驱动支持。

- 增加了对 3Com 3c905C-TX 的驱动支持。

- 增加了对 3Com 3x574-TX 16 位 FastEtherlink PC 卡的驱动支持。
...
- **APM 支持** 得到增强。修复了与旧版 APM BIOS 相关的内核 Panic 问题。此外，挂起/待机转换变得更加稳健。
...
- 更新了 Port `wu-ftpd`，加入最新补丁以防止可能的远程 root 漏洞利用。

- 更新了 Port `proftpd`，加入最新补丁以防止可能的远程 root 漏洞利用。

- 更新了 Port `samba`，加入最新补丁以防止可能的远程 root 漏洞利用。

- 更新了 Port `inn`，修复了一些缓冲区溢出问题。

- 自 FreeBSD 3.0-RELEASE 版本以来，修复了网络栈中的许多小问题，这些问题可能被利用进行拒绝服务攻击。

### 1.3. 用户空间变更

- **Linux 模式** 的支持环境终于更新。Port `linux_lib` 和 `linux_devel` 被分别替换为 `linux_base` 和 `linux_devtools`。这些新 Port 基于 Red Hat 5.2 的软件包，并支持基于 glibc2 和 libc5 的应用程序。

- `Sysinstall` 现在包含了 DHCP 客户端支持。

- **inetd(8)** 中的 TCP Wrapper 支持现在通过命令行选项进行控制，除了此前包装过的服务类型之外，还可以包装数据报（UDP）服务。详细信息请参阅手册页，因为没有命令行选项的 `inetd` 不会进行包装。

- ISC 的 DHCP 客户端升级到了 2.0 版本。

- GNU 解析器生成器 **Bison** 升级到了 1.28 版本。

- 新增 **高级电源管理监控守护进程（apmd(8)）**。该守护进程能让用户选择由用户空间处理的 APM 事件，并为给定事件指定命令。这使得 APM 行为可以灵活地配置。详细信息请参阅手册页。

## 2. 支持的配置

FreeBSD 当前支持各种基于 ISA、VLB、EISA 和 PCI 总线的 PC，包括从 386sx 到 Pentium 级别的机器（尽管不推荐使用 386sx）。还支持通用 IDE 或 ESDI 硬盘配置、各种 SCSI 控制器、网卡和串口卡。

以下是目前已知可在 FreeBSD 下工作的所有外设列表。其他配置也可能工作，但我们尚未收到确认。
...
- **DPT SmartCACHE Plus, SmartCACHE III, SmartRAID III, SmartCACHE IV 和 SmartRAID IV SCSI/RAID 控制器**均支持。DPT SmartRAID/CACHE V 尚不支持。
...
对于所有支持的 SCSI 控制器，提供对 SCSI-I 和 SCSI-II 外设的完全支持，包括硬盘、光盘、磁带驱动器（包括 DAT 和 8mm Exabyte）、介质交换器、处理器目标设备和 CD-ROM 驱动器。支持 CD-ROM 命令的 WORM 设备可由 CD-ROM 驱动程序进行只读访问。WORM/CD-R/CD-RW 写入支持通过 `cdrecord` 提供，该工具位于 Ports 中。
...
[注意：目前正在进行将 AIC-6260/6360 和 UltraStor 驱动程序移植到新的 CAM SCSI 框架的工作，但尚无完成的时间表，也不确定能否完成。]
...
- Lucent Technologies WaveLAN/IEEE 802.11 PCMCIA 和 ISA 标准速率（2Mbps）以及 Turbo 速率（6Mbps）无线网络适配器及其兼容产品（NCR WaveLAN/IEEE 802.11，Cabletron RoamAbout 802.11 DS）。注：这些适配器的 ISA 版本实际上是 PCMCIA 卡与 ISA 到 PCMCIA 桥卡的组合，因此这两种设备都使用相同的驱动程序。
...
其他镜像站点信息，请参阅文件 `MIRROR.SITES`。请尽量从最接近你的站点（就网络连接而言）进行 FTP 下载。欢迎更多的镜像站点！如果你希望成为官方镜像站点，请联系 `freebsd-admin@FreeBSD.org` 获取更多信息。
...
如果你正在从先前版本的 FreeBSD 升级，最有可能是从 2.2.x 或 2.1.x 版本（在某些较少的情况下）升级，以下问题可能会影响到你，具体取决于你选择的升级方式。升级 FreeBSD 发行版有两种常见方法：

- 使用源代码，通过 **/usr/src**
- 还可使用 `sysinstall` 的（二进制）升级选项。

如果使用源代码，你需要关注两个目标：标准的 `upgrade` 目标，它将 2.x 或 3.0 系统升级到 3.3，和 `world` 目标，它将升级后的系统与自初次升级以来的任何变更保持同步。

如果使用二进制升级选项，系统将直接升级到 3.3/ELF，并且也会为向后兼容旧二进制文件而填充 **/<basepath>/lib/aout** 目录。

无论哪种方式，转到 ELF 格式意味着你将拥有更小的二进制文件，并且可以访问移植到其他 ELF 环境中的大量编译器功能（我们较旧且有些陈旧的 a.out 格式大多数其他软件项目不再支持）。那些希望保留对旧版 a.out 动态可执行文件访问的人应确保安装 `compat22` 发行版。请注意，a.out 库在系统重启之前无法访问，这可能会导致某些 a.out 包出现问题。
...
你的建议、错误报告和代码贡献始终受到重视——如果你发现任何问题，请随时报告（如果可能，附上修复方案！）。

从具有互联网邮件连接的机器上提交错误报告的首选方法是使用命令 `send-pr`，或者通过 [http://www.FreeBSD.org/send-pr.html](http://www.FreeBSD.org/send-pr.html) 使用 CGI 脚本。我们忠实的 bugfiler 程序会妥善归档错误报告，你可以放心，我们会尽力尽快回复所有报告的错误。通过这种方式提交的错误也会在我们的网站的支持部分可见，因此它们既作为错误报告，也作为其他用户关注潜在问题的“路标”。

如果由于某些原因你无法使用命令 `send-pr` 提交错误报告，你可以尝试将报告发送到：`freebsd-bugs@FreeBSD.org`

请注意，`send-pr` 本身是一个 shell 脚本，应该很容易移植到完全不同的系统上。我们更倾向于你使用这个接口，因为它使我们更容易跟踪问题报告。然而，在提交之前，请尽量确认该问题是否已经修复。

另外，如果你有任何问题或技术支持问题，请将邮件发送到：`freebsd-questions@FreeBSD.org`

此外，由于这是一个志愿者项目，我们始终欢迎有意帮助的额外人手——目前希望的增强功能远远超过我们自己能够管理的！要与我们联系技术问题或提供帮助，请发送邮件到：`freebsd-hackers@FreeBSD.org`
...
FreeBSD 凝聚了来自世界各地的数十甚至数百位个人的累积工作，这些人付出了巨大努力，才将这个版本带给了你。欲了解完整的 FreeBSD 项目工作人员名单，请访问：`http://www.FreeBSD.org/handbook/staff.html`

或者，如果你加载了文档包：**file:/usr/share/doc/handbook/staff.html**
