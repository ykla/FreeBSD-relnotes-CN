...
如果由于某种原因，您无法使用 `send-pr` 命令提交错误报告，可以尝试将报告发送到：`freebsd-bugs@FreeBSD.org`

请注意，`send-pr` 本身是个 shell 脚本，即使在完全不同的系统上也应容易迁移。我们更倾向于您使用此界面，因为它有助于更容易地跟踪问题报告。然而，在提交之前，请尽量确保该问题是否已经被修复。
...
请注意，这些邮件列表可能会经历 *大量* 的流量，如果您有慢速或昂贵的邮件访问，并且仅对关注 FreeBSD 的重大事件感兴趣，您可能会发现订阅以下列表更为方便：`freebsd-announce@FreeBSD.org`
...
FreeBSD 代表了来自世界各地许多个人的共同努力，这些人不懈工作，最终为您带来了这个版本。欲查看完整的 FreeBSD 项目工作人员名单，请访问：[http://www.FreeBSD.org/handbook/staff.html](http://www.FreeBSD.org/handbook/staff.html)

或，如果您已加载文档分发包：`file:/usr/share/doc/handbook/staff.html`
