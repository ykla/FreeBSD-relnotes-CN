...
这些发行说明适用于 snapshot 发行版，它表示 10.4-STABLE 开发分支中位于 10.3-RELEASE 和未来的 10.4-STABLE 之间的某个节点。关于沿该分支的预构建二进制快照发行版的信息可以在 [https://www.FreeBSD.org/releases/](https://www.freebsd.org/releases/) 查找。
...
典型的发行说明条目记录了自 10.3-RELEASE 以来发布的最新安全公告、新的驱动程序或硬件支持、新的命令或选项、主要的 bug 修复或第三方的软件升级。它们还可能列出主要 Port/包或发布工程实践的变更。显然，发行说明不能列出每一个在两个版本之间对 FreeBSD 做出的变更；本文档主要聚焦于安全公告、用户可见的变化以及主要的架构改进。
...
[gdb(1)](https://www.freebsd.org/cgi/man.cgi?query=gdb&sektion=1&manpath=freebsd-release-ports) 和 [kgdb(1)](https://www.freebsd.org/cgi/man.cgi?query=kgdb&sektion=1&manpath=freebsd-release-ports) 工具已被标记为弃用，计划未来从基本系统中移除。可以在 Port `devel/gdb` 中找到较新的版本。[(r320824)](http://svn.freebsd.org/viewvc/base?view=revision&revision=320824)
...
通过 [bhyve(4)](https://www.freebsd.org/cgi/man.cgi?query=bhyve&sektion=4&manpath=freebsd-release-ports) 的 PCI 直通支持更多动态配置，允许在运行时标记设备用于直通或主机使用。[(r306472)](http://svn.freebsd.org/viewvc/base?view=revision&revision=306472)（由 Chelsio 赞助）
...
Dummynet AQM，独立实现的 CoDel 和 FQ-CoDel 用于 ipfw/dummynet，已被导入到基本系统中。[(r301772)](http://svn.freebsd.org/viewvc/base?view=revision&revision=301772)
