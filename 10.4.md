...
本节涵盖了用户空间应用程序、贡献软件和系统工具的变更和新增内容。
...
[truss(1)](https://www.freebsd.org/cgi/man.cgi?query=truss&sektion=1&manpath=freebsd-release-ports) 工具已更新，新增 `-H` 参数，该参数记录调用系统调用的线程 ID。[(r298427)](http://svn.freebsd.org/viewvc/base?view=revision&revision=298427)
...
[pw(8)](https://www.freebsd.org/cgi/man.cgi?query=pw&sektion=8&manpath=freebsd-release-ports) 工具已更新，以正确遵循 [pw.conf(5)](https://www.freebsd.org/cgi/man.cgi?query=pw.conf&sektion=5&manpath=freebsd-release-ports)，如果存在的话。[(r316348)](http://svn.freebsd.org/viewvc/base?view=revision&revision=316348)
...
[pw(8)](https://www.freebsd.org/cgi/man.cgi?query=pw&sektion=8&manpath=freebsd-release-ports) 工具已更新，支持通过 `-G` 参数传递空的次要组，确保目标用户没有次要组成员身份。[(r323333)](http://svn.freebsd.org/viewvc/base?view=revision&revision=323333)
...
[kvm_close(3)](https://www.freebsd.org/cgi/man.cgi?query=kvm_close&sektion=3&manpath=freebsd-release-ports) 函数已更新，以返回前一个 [close(2)](https://www.freebsd.org/cgi/man.cgi?query=close&sektion=2&manpath=freebsd-release-ports) 调用的累积错误。 [(r316040)](http://svn.freebsd.org/viewvc/base?view=revision&revision=316040)

[syslog(3)](https://www.freebsd.org/cgi/man.cgi?query=syslog&sektion=3&manpath=freebsd-release-ports) 函数已更新，以提高对线程取消的韧性，在支持的延迟模式下消除了可能发生的死锁，避免了多线程应用程序在频繁创建和取消线程时的锁死问题，例如 `net/mpd5`。 [(r320312)](http://svn.freebsd.org/viewvc/base?view=revision&revision=320312)

[stdio(3)](https://www.freebsd.org/cgi/man.cgi?query=stdio&sektion=3&manpath=freebsd-release-ports) 函数已更新为延迟取消安全，从而消除了多线程应用程序在使用如 [funopen(3)](https://www.freebsd.org/cgi/man.cgi?query=funopen&sektion=3&manpath=freebsd-release-ports)、[fropen(3)](https://www.freebsd.org/cgi/man.cgi?query=fropen&sektion=3&manpath=freebsd-release-ports) 和 [fwopen(3)](https://www.freebsd.org/cgi/man.cgi?query=fwopen&sektion=3&manpath=freebsd-release-ports) 等函数时可能出现的死锁问题。 [(r321074)](http://svn.freebsd.org/viewvc/base?view=revision&revision=321074)
...
[ptrace(2)](https://www.freebsd.org/cgi/man.cgi?query=ptrace&sektion=2&manpath=freebsd-release-ports) 现在支持线程创建和销毁事件，允许更可靠地调试线程化进程。 [(r304017)](http://svn.freebsd.org/viewvc/base?view=revision&revision=304017)
...
[ipf(4)](https://www.freebsd.org/cgi/man.cgi?query=ipf&sektion=4&manpath=freebsd-release-ports) 数据包过滤器已更新，防止 `keep state` 错误地暗示 `keep frags`，与 [ipf(5)](https://www.freebsd.org/cgi/man.cgi?query=ipf&sektion=5&manpath=freebsd-release-ports) 中记录的行为相匹配。 [(r317434)](http://svn.freebsd.org/viewvc/base?view=revision&revision=317434)

[geom(4)](https://www.freebsd.org/cgi/man.cgi?query=geom&sektion=4&manpath=freebsd-release-ports) `JOURNAL` 类别已更新，以修复 `flush_queue` 处理。 [(r322793)](http://svn.freebsd.org/viewvc/base?view=revision&revision=322793)
...
本节涵盖了自 10.3-RELEASE 以来设备和设备驱动程序的变化和新增内容。
...
[cxgbev(4)](https://www.freebsd.org/cgi/man.cgi?query=cxgbev&sektion=4&manpath=freebsd-release-ports) 驱动已添加，提供对 Chelsio T4 和 T5 适配器上的虚拟功能设备 (VFs) 的支持。 [(r309447)](http://svn.freebsd.org/viewvc/base?view=revision&revision=309447)（由 Chelsio 赞助）

[cxgbe(4)](https://www.freebsd.org/cgi/man.cgi?query=cxgbe&sektion=4&manpath=freebsd-release-ports) 驱动已更新，提供对 Chelsio T6 适配器上虚拟功能设备 (VFs) 的支持。 [(r309560)](http://svn.freebsd.org/viewvc/base?view=revision&revision=309560)（由 Chelsio 赞助）
...
本节涵盖物理机器、虚拟机监控器和虚拟化环境的通用硬件支持，以及不适合其他部分的硬件变化和更新。

### 虚拟化支持

PCI 透传通过 [bhyve(4)](https://www.freebsd.org/cgi/man.cgi?query=bhyve&sektion=4&manpath=freebsd-release-ports) 在虚拟机启动和停止时通过 FLR 重置功能。 [(r306520)](http://svn.freebsd.org/viewvc/base?view=revision&revision=306520)（由 Chelsio 赞助）

PCI 透传通过 [bhyve(4)](https://www.freebsd.org/cgi/man.cgi?query=bhyve&sektion=4&manpath=freebsd-release-ports) 支持更多动态配置，允许在运行时标记设备用于透传或主机使用。 [(r306472)](http://svn.freebsd.org/viewvc/base?view=revision&revision=306472)（由 Chelsio 赞助）

### 存储

本节涵盖文件系统和其他存储子系统的变化和新增内容，包括本地和网络存储。

### 网络存储

NFS 客户端现在可以正确处理来自 NFS 服务器的 `NFS4ERR_BAD_SESSION` 错误。此外，内核 RPC 客户端已更新，以防止在接收到来自 [sosend(9)](https://www.freebsd.org/cgi/man.cgi?query=sosend&sektion=9&manpath=freebsd-release-ports) 的 `ERESTART` 时创建新的 TCP 连接。 [(r318675)](http://svn.freebsd.org/viewvc/base?view=revision&revision=318675)
...
本节涵盖 FreeBSD Ports、包基础设施以及包维护和安装工具的变化。
