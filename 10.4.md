...
典型的发行说明条目记录了自 10.3-RELEASE 以来发布的最新安全公告、新的驱动程序或硬件支持、新的命令或选项、主要的 bug 修复或第三方贡献软件升级。它们还可能列出主要 Port/包或发布工程实践的变更。显然，发行说明无法列出在两个版本之间对 FreeBSD 做出的每一项变更；本文档主要聚焦于安全公告、用户可见的变化以及主要的架构改进。
