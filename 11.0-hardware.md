# FreeBSD 11.0-RELEASE 硬件兼容列表

## 引言

本文档包含 FreeBSD 11.0-RELEASE 的硬件兼容性说明。其中列出了 FreeBSD 支持的硬件平台，以及各类硬件设备（存储控制器、网络接口等），并给出已知可正常工作的设备实例。
...
如果你有不在此列表中的系统，它可能尚未经过 FreeBSD 11.0-RELEASE 测试。我们鼓励你尝试并致信 [FreeBSD SPARC porting mailing list](http://lists.FreeBSD.org/mailman/listinfo/freebsd-sparc64) 反馈结果，包括哪些设备可用、哪些不可用。
...
[i386, amd64] [aac(4)](https://man.freebsd.org/cgi/man.cgi?query=aac&sektion=4&format=html) 驱动支持以下控制器：
...
[i386, pc98, amd64] [adw(4)](https://man.freebsd.org/cgi/man.cgi?query=adw&sektion=4&format=html) 驱动支持 SCSI 控制器，包括：
...
[i386, sparc64, amd64] [ahd(4)](https://man.freebsd.org/cgi/man.cgi?query=ahd&sektion=4&format=html) 驱动支持以下设备：
...
[i386, amd64] [bt(4)](https://man.freebsd.org/cgi/man.cgi?query=bt&sektion=4&format=html) 驱动支持以下 BusLogic MultiMaster "W"、"C"、"S" 和 "A" 系列及兼容 SCSI 主机适配器：
...
AMI FastDisk 主机适配器中真正的 BusLogic MultiMaster 克隆也受 [bt(4)](https://man.freebsd.org/cgi/man.cgi?query=bt&sektion=4&format=html) 驱动支持。

[i386, amd64] [ciss(4)](https://man.freebsd.org/cgi/man.cgi?query=ciss&sektion=4&format=html) 驱动支持以下控制器：
...
[i386, amd64] [dpt(4)](https://man.freebsd.org/cgi/man.cgi?query=dpt&sektion=4&format=html) 驱动支持以下 RAID 适配器：
...
[hptiop(4)](https://man.freebsd.org/cgi/man.cgi?query=hptiop&sektion=4&format=html) 驱动还支持以下已停产的 SAS 和 SATA RAID 控制器：
...
[i386, amd64] [iir(4)](https://man.freebsd.org/cgi/man.cgi?query=iir&sektion=4&format=html) 驱动支持以下控制器：
...
[i386, amd64] [mfi(4)](https://man.freebsd.org/cgi/man.cgi?query=mfi&sektion=4&format=html) 驱动支持以下硬件：
...
[i386, amd64] [mlx(4)](https://man.freebsd.org/cgi/man.cgi?query=mlx&sektion=4&format=html) 驱动支持以下控制器：
...
未列出的兼容 Mylex 控制器应可工作，但未经证实。

> **注意**：
>
> [amd64, i386] 支持从这些控制器启动。不支持 EISA 适配器。

[i386, amd64] [mly(4)](https://man.freebsd.org/cgi/man.cgi?query=mly&sektion=4&format=html) 驱动支持以下控制器：
...
这些系统还包含集成 RAID 镜像和增强型集成 RAID 镜像，此驱动也支持它们。

SAS 控制器芯片也出现在许多新的基于 AMD/Opteron 的系统上，例如 Sun 4100。注意，此控制器可同时驱动 SAS 和 SATA 驱动器或两者的混合。这些控制器可用的集成 RAID 镜像支持充其量是有限的。

光纤通道控制器芯片组支持多种速度和系统。Apple Fibre Channel HBA 实际上是 FC949ES 卡。
...
注意，此硬件仅对 ATA DMA 命令支持命令队列和基于 FIS 的切换。ATAPI 和非 DMA ATA 命令对每个端口逐一执行。
...
[sym(4)](https://man.freebsd.org/cgi/man.cgi?query=sym&sektion=4&format=html) 支持的 SCSI 控制器可以板载于主板，或位于以下扩展板之一：
...
对于所有受支持的 SCSI 控制器，完整支持 SCSI-I、SCSI-II 和 SCSI-III 外设，包括硬盘、光盘、磁带机（包括 DAT、8mm Exabyte、Mammoth 和 DLT）、介质更换器、处理器目标设备和 CD-ROM 驱动器。支持 CD-ROM 命令的 WORM 设备可由 CD-ROM 驱动程序（如 [cd(4)](https://man.freebsd.org/cgi/man.cgi?query=cd&sektion=4&format=html)）以只读访问方式支持。WORM/CD-R/CD-RW 写入支持由 [cdrecord(1)](https://man.freebsd.org/cgi/man.cgi?query=cdrecord&sektion=1&format=html) 提供，它是 Ports Collection 中 [sysutils/cdrtools](http://www.freebsd.org/cgi/url.cgi?ports/sysutils/cdrtools/pkg-descr) port 的一部分。
...
[alc(4)](https://man.freebsd.org/cgi/man.cgi?query=alc&sektion=4&format=html) 驱动支持以下以太网控制器：
...
[i386, pc98, amd64, powerpc] [aue(4)](https://man.freebsd.org/cgi/man.cgi?query=aue&sektion=4&format=html) 驱动支持的适配器包括：
...
[i386, amd64] [bce(4)](https://man.freebsd.org/cgi/man.cgi?query=bce&sektion=4&format=html) 驱动支持基于 QLogic NetXtreme II 系列千兆以太网控制器的各种 NIC，包括以下：
...
[i386, pc98, sparc64, amd64] [bge(4)](https://man.freebsd.org/cgi/man.cgi?query=bge&sektion=4&format=html) 驱动支持基于 Broadcom BCM570x 系列千兆以太网控制器芯片的各种 NIC，包括以下：
...
[bnxt(4)](https://man.freebsd.org/cgi/man.cgi?query=bnxt&sektion=4&format=html) 驱动支持基于 Broadcom NetXtreme-C 和 NetXtreme-E 系列千兆以太网控制器芯片的各种 NIC，包括以下：
...
[i386, amd64] [bxe(4)](https://man.freebsd.org/cgi/man.cgi?query=bxe&sektion=4&format=html) 驱动支持基于 QLogic NetXtreme II 系列的 10Gb 以太网控制器芯片的各种 NIC，包括以下：
...
目前已知与 [cas(4)](https://man.freebsd.org/cgi/man.cgi?query=cas&sektion=4&format=html) 驱动配合工作的扩展卡包括：
...
[cxgbev(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbev&sektion=4&format=html) 驱动支持基于 T6 ASIC 的 100Gb 和 25Gb 以太网适配器上的虚拟功能：
...
[cxgbev(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbev&sektion=4&format=html) 驱动支持基于 T5 ASIC 的 40Gb、10Gb 和 1Gb 以太网适配器上的虚拟功能：
...
[cxgbev(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbev&sektion=4&format=html) 驱动支持基于 T4 ASIC 的 10Gb 和 1Gb 以太网适配器上的虚拟功能：
...
目前已知与 [gem(4)](https://man.freebsd.org/cgi/man.cgi?query=gem&sektion=4&format=html) 驱动配合工作的扩展卡包括：
...
[i386] [e(4)](https://man.freebsd.org/cgi/man.cgi?query=e&sektion=4&format=html) 驱动支持以下基于 Intel i82586 芯片的 8 位和 16 位 ISA 以太网卡：
...
[ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 驱动支持以下 SFP+ 可插拔光模块：
...
[mlx5en(4)](https://man.freebsd.org/cgi/man.cgi?query=mlx5en&sektion=4&format=html) 驱动支持 100Gb、50Gb、40Gb、25Gb 和 10Gb 以太网适配器。ConnectX-4 支持：10/20/25/40/50/56/100Gb/s 速度。ConnectX-4 LX 支持：10/25/40/50Gb/s 速度（且功耗更低）：
...
[i386, amd64] [msk(4)](https://man.freebsd.org/cgi/man.cgi?query=msk&sektion=4&format=html) 驱动支持基于 Marvell/SysKonnect Yukon II 千兆以太网控制器芯片的各种 NIC，包括：
...
[i386, pc98] [my(4)](https://man.freebsd.org/cgi/man.cgi?query=my&sektion=4&format=html) 驱动支持基于 Myson 芯片组的各种 NIC。支持的型号包括：
...
[i386, amd64] [nxge(4)](https://man.freebsd.org/cgi/man.cgi?query=nxge&sektion=4&format=html) 驱动支持 http://www.neterion.com/how/pricing.html 中列出的 Neterion Xframe 10 千兆以太网适配器。
...
[i386, pc98, amd64] [pcn(4)](https://man.freebsd.org/cgi/man.cgi?query=pcn&sektion=4&format=html) 驱动支持基于 AMD PCnet/FAST、PCnet/FAST+、PCnet/FAST III、PCnet/PRO 和 PCnet/Home 快速以太网芯片的适配器和嵌入式控制器：
...
[amd64] [qlxgb(4)](https://man.freebsd.org/cgi/man.cgi?query=qlxgb&sektion=4&format=html) 驱动支持基于以下芯片组的 10 千兆以太网和 CNA 适配器：

* QLogic 3200 series
* QLogic 8200 series

[amd64] [qlxgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=qlxgbe&sektion=4&format=html) 驱动支持基于以下芯片组的 10 千兆以太网和 CNA 适配器：

* QLogic 8300 series

[amd64] [qlxge(4)](https://man.freebsd.org/cgi/man.cgi?query=qlxge&sektion=4&format=html) 驱动支持基于以下芯片组的 10 千兆以太网和 CNA 适配器：
...
[sn(4)](https://man.freebsd.org/cgi/man.cgi?query=sn&sektion=4&format=html) 驱动支持来自 SMC 的 SMC 91C90、SMC 91C92、SMC 91C94、SMC 91C95、SMC 91C96、SMC91C100 和 SMC 91C100FD 芯片。

Farallon EtherWave 和 EtherMac 卡有两种变体。[ep(4)](https://man.freebsd.org/cgi/man.cgi?query=ep&sektion=4&format=html) 驱动支持 595 和 895 卡。这些卡正面有蓝色箭头和 3Com 标志。Farallon 595a 卡正面有红色箭头，也称为 EtherWave 和 EtherMac。它们由 [sn(4)](https://man.freebsd.org/cgi/man.cgi?query=sn&sektion=4&format=html) 驱动支持。
...
[snc(4)](https://man.freebsd.org/cgi/man.cgi?query=snc&sektion=4&format=html) 驱动还包括对 National Semiconductor NS46C46 作为 64 * 16 位微波串行 EEPROM 的支持。
...
[stge(4)](https://man.freebsd.org/cgi/man.cgi?query=stge&sektion=4&format=html) 驱动支持基于 Sundance/Tamarack TC9021 千兆以太网控制器芯片的各种 NIC，包括：
...
[i386, pc98, amd64, sparc64] [ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动支持基于 Alteon Tigon I 和 II 芯片的千兆以太网适配器。[ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动已测试过以下适配器：
...
[i386, amd64] [vxge(4)](https://man.freebsd.org/cgi/man.cgi?query=vxge&sektion=4&format=html) 驱动支持 http://www.neterion.com 中列出的 Neterion X3100 10 千兆以太网适配器。
...
3C656 系列 CardBus 卡和 3C556 系列 MiniPCI 卡均内置专用调制解调器。[xl(4)](https://man.freebsd.org/cgi/man.cgi?query=xl&sektion=4&format=html) 驱动和其他 FreeBSD 驱动均不支持此调制解调器。
...
[bwi(4)](https://man.freebsd.org/cgi/man.cgi?query=bwi&sektion=4&format=html) 驱动使用 Broadcom 固件的旧版本 v3。虽然此旧固件支持大多数 BCM43xx 部件，但 bwn(4) 驱动在其支持的新芯片上工作得更好。如果使用较旧的 Broadcom 芯片组（BCM4301、BCM4303 和 BCM4306 rev 2），必须使用 [bwi(4)](https://man.freebsd.org/cgi/man.cgi?query=bwi&sektion=4&format=html) 驱动。bwn(4) 使用的 v4 版本固件不支持这些芯片。
...
较旧 Broadcom 芯片组（BCM4301、BCM4303 和 BCM4306 rev 2）的用户必须使用 bwi(4)，因为 v4 版本的固件不支持这些芯片。新固件太大，无法装入这些旧芯片。
...
[uath(4)](https://man.freebsd.org/cgi/man.cgi?query=uath&sektion=4&format=html) 驱动应可与以下适配器配合使用：
...
[i386] [cm(4)](https://man.freebsd.org/cgi/man.cgi?query=cm&sektion=4&format=html) 驱动支持以下型号的网卡：
...
* PC-9801 on-board
* PC-9821 2'ned CCU (flags 0x12000000)
...
[snd_emu10kx(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_emu10kx&sektion=4&format=html) 驱动 _不_ 支持以下声卡（尽管它们的名称与某些受支持的声卡相似）：
...
[i386, amd64] [snd_hda(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hda&sektion=4&format=html) 驱动支持具有 PCI 类 4（多媒体）和子类 3（HDA）、兼容 Intel HDA 规范的控制器。

[snd_hda(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hda&sektion=4&format=html) 驱动支持超过两百种不同的控制器和 CODEC。在此全部列出没有意义，因为在大多数情况下，具体的 CODEC 配置和接线比 CODEC 本身的类型更为重要。
...
注意，基于 ES18xx 芯片组的较旧 ESS ISA 卡通过 snd_ess(4) 和/或 snd_sbc(4) 支持。
...
> **注意**：
>
> [amd64, i386, pc98] USB 以太网适配器可在列出 <<ethernet,Ethernet interfaces>> 的章节中找到。

> **注意**：
>
> [amd64, i386, pc98] USB 蓝牙适配器可在 <<bluetooth,Bluetooth>> 章节中找到。

[i386, pc98, amd64, powerpc] [ohci(4)](https://man.freebsd.org/cgi/man.cgi?query=ohci&sektion=4&format=html) 驱动支持所有符合 OHCI v1.0 规范的控制器，包括：
...
[i386, pc98, amd64, powerpc] [uhci(4)](https://man.freebsd.org/cgi/man.cgi?query=uhci&sektion=4&format=html) 驱动支持所有符合 UHCI v1.1 规范的控制器，包括：
...
[i386, pc98, amd64, powerpc] [ulpt(4)](https://man.freebsd.org/cgi/man.cgi?query=ulpt&sektion=4&format=html) 驱动提供对 USB 打印机和并口打印转换线缆的支持，包括以下设备：
...
[i386, sparc64, amd64, powerpc] [fwohci(4)](https://man.freebsd.org/cgi/man.cgi?query=fwohci&sektion=4&format=html) 驱动提供对 PCI/CardBus FireWire 接口卡的支持。该驱动支持以下 IEEE 1394 OHCI 芯片组：
...
[i386, pc98, amd64] [hifn(4)](https://man.freebsd.org/cgi/man.cgi?query=hifn&sektion=4&format=html) 驱动支持包含 Hifn 7751、7951、7811、7955 和 7956 芯片组的各类卡，例如：
...
> **注意**：
>
> 关于具体显卡及其与 Xorg 兼容性的信息可在 `http://www.x.org/` 找到。
