...
典型的发行说明条目记录了自 8.2-RELEASE 之后发布的最新安全公告、新驱动程序或硬件支持、新命令或选项、主要错误修复或引入的软件升级。它们可能还会列出主要的 Port/包更改或发行工程实践的调整。显然，发行说明不可能列出两个版本之间 FreeBSD 所做的每一个更改；本文档主要关注安全公告、用户可见的更改以及重要的架构改进。
