...
添加了驱动程序 `cm` 以支持 SMC COM90cx6 ARCNET 网络适配器。

驱动 [dc(4)](http://www.freebsd.org/cgi/man.cgi?query=dc&sektion=4&manpath=FreeBSD+4.6-RELEASE) 现在支持 VLAN。

GENERIC 内核现在通过驱动 [em(4)](http://www.freebsd.org/cgi/man.cgi?query=em&sektion=4&manpath=FreeBSD+4.6-RELEASE) 支持 Intel 千兆以太网卡，而非 [wx(4)](http://www.freebsd.org/cgi/man.cgi?query=wx&sektion=4&manpath=FreeBSD+4.6-RELEASE) 驱动，后者自 FreeBSD 4.5-RELEASE 起已被弃用。
...
[4.6.2] 修复了 [pppd(8)](http://www.freebsd.org/cgi/man.cgi?query=pppd&sektion=8&manpath=FreeBSD+4.6-RELEASE) 中的竞争条件漏洞，该漏洞可能被用来更改任意文件的权限。有关更多信息，请参见安全公告 [FreeBSD-SA-02:32](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-02:32.pppd.asc)。

[4.6.2] 修复了 **OpenSSL** 中的多个缓冲区溢出漏洞，方法是升级了基础系统中的 **OpenSSL** 版本。更多详细信息参见安全公告 [FreeBSD-SA-02:33](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-02:33.openssl.asc)。
...
`/etc/rc.firewall` 和 `/etc/rc.firewall6` 当变量 `firewall_type` 中有规则文件或防火墙类型不存在时，不再添加硬编码的规则（此更改的目的是避免对站点的防火墙策略做出假设）。此外，闭合防火墙类型（closed firewall type）现在按照 [rc.firewall(8)](http://www.freebsd.org/cgi/man.cgi?query=rc.firewall&sektion=8&manpath=FreeBSD+4.6-RELEASE) 手册页的文档工作。
...
[ifconfig(8)](http://www.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8&manpath=FreeBSD+4.6-RELEASE) 的 ether 地址族现已更改为更通用的 link 地址族（仍支持 ether 以兼容旧版）。
...
[k5su(8)](http://www.freebsd.org/cgi/man.cgi?query=k5su&sektion=8&manpath=FreeBSD+4.6-RELEASE) 默认不再安装为 SUID root。需要此功能的用户可以手动更改 [k5su(8)](http://www.freebsd.org/cgi/man.cgi?query=k5su&sektion=8&manpath=FreeBSD+4.6-RELEASE) 可执行文件的权限，或者在源码升级前在 `/etc/make.conf` 中添加 `ENABLE_SUID_K5SU=yes`。
...
地区设置支持已与 FreeBSD CURRENT 的代码同步，此更改引入了对 `LC_NUMERIC`、`LC_MONETARY` 和 `LC_MESSAGES` 类别的支持，并改进了 [strftime(3)](http://www.freebsd.org/cgi/man.cgi?query=strftime&sektion=3&manpath=FreeBSD+4.6-RELEASE)、修订的地区定义以及许多基本系统程序的本地化改进。
...
[4.6.2] **OpenSSH** 已更新为 3.4p1 版本。更改包括：
...
* [ssh-add(1)](http://www.freebsd.org/cgi/man.cgi?query=ssh-add&sektion=1&manpath=FreeBSD+4.6-RELEASE) 现在会添加所有三种默认密钥。
...
通过 `/etc/mail/Makefile` 构建的 **sendmail** 别名和映射数据库的权限现在默认为模式 `0640`，以防止文件锁定导致的本地拒绝服务。可以通过设置新的 make.conf 选项 `SENDMAIL_MAP_PERMS` 来更改。

**sendmail** 统计文件 `/var/log/sendmail.st` 的权限已从模式 `0644` 更改为模式 `0640`，以防止文件锁定导致的本地拒绝服务。
...
Ports 基础设施现在使用 **XFree86** 4.2.0 作为默认版本的 X Window 系统，以满足依赖关系。如果想恢复使用 **XFree86** 3.3.6，请将以下行添加到 `/etc/make.conf`：
...
如果您正在从之前的 FreeBSD 版本进行升级，通常有三种选择：

* 使用 [sysinstall(8)](http://www.freebsd.org/cgi/man.cgi?query=sysinstall&sektion=8&manpath=FreeBSD+4.6-RELEASE) 的二进制升级选项。这个选项可能是最快的，尽管它假设您的 FreeBSD 安装没有使用特殊的编译选项。
* 完全重新安装 FreeBSD。严格来说，这不是一种升级方法，而且通常比二进制升级更不方便，因为它要求您手动备份并恢复 `/etc` 的内容。然而，如果您想（或需要）更改磁盘的分区方式，它可能会很有用。
...
请阅读 `INSTALL.TXT` 文件以获取更多信息，最好是在开始升级之前。如果您是从源代码升级，请确保阅读 `/usr/src/UPDATING` 文件。

最后，如果您希望使用各种方式跟踪 FreeBSD 的 -STABLE 或 -CURRENT 分支，请务必查阅 [`-CURRENT 与 -STABLE'](http://www.freebsd.org/doc/en_US.ISO8859-1/books/handbook/current-stable.html) 部分的 [FreeBSD 手册](http://www.freebsd.org/doc/en_US.ISO8859-1/books/handbook/)。
