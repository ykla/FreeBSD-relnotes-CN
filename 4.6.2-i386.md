...
本节描述了自 4.5-RELEASE 以来 FreeBSD 中最显著的新功能或变更。通常的发行说明条目包括新驱动程序或硬件支持、新命令或选项、主要错误修复或贡献软件的更新。此外，还列出了 4.5-RELEASE 之后基本系统发布的安全公告。
...
`/etc/rc.firewall` 和 `/etc/rc.firewall6` 当变量 `firewall_type` 中有规则文件或防火墙类型不存在时，不再新增硬编码的规则（此更改的目的是避免对站点的防火墙策略做出假设）。此外，闭合防火墙类型（closed firewall type）现在按照 [rc.firewall(8)](http://www.freebsd.org/cgi/man.cgi?query=rc.firewall&sektion=8&manpath=FreeBSD+4.6-RELEASE) 手册页的文档工作。
...
[k5su(8)](http://www.freebsd.org/cgi/man.cgi?query=k5su&sektion=8&manpath=FreeBSD+4.6-RELEASE) 默认不再安装为 SUID root。需要此功能的用户可以手动更改 [k5su(8)](http://www.freebsd.org/cgi/man.cgi?query=k5su&sektion=8&manpath=FreeBSD+4.6-RELEASE) 可执行文件的权限，或者在源码升级前在 `/etc/make.conf` 中新增 `ENABLE_SUID_K5SU=yes`。
