...
手册页新的网络(7)包含了连接系统到网络（包括 Wi-Fi）的快速入门指南，并链接到了其他手册页和手册。39f92a4c4c49
...
根据当前的生命周期，stable/14 将在 FreeBSD 14.0-RELEASE 发行 5 年后结束生命周期（EOL）。stable/14 的生命周期终点标志着对包括源代码发布、预构建软件包以及从ports构建应用程序的弃用 32 位平台的支持结束。随着 2023 年 11 月 14.0-RELEASE 的发布，对已弃用的 32 位平台支持将在 2028 年 11 月结束。
