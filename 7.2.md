...
FreeBSD 7.2-RELEASE 的发行说明包含对 7.2-STABLE 开发线中 FreeBSD 基础系统所做更改的总结。本文档列出了自上次发布以来发布的相关安全通告，以及对 FreeBSD 内核和用户态的重大更改。还提供了一些关于升级的简要说明。
...
典型的发行说明条目包括在 7.1-RELEASE 之后发布的最新安全通告、新的驱动程序或硬件支持、新的命令或选项、主要的错误修复或贡献的软件升级。它们还可能列出主要 Port/包或发布工程实践的变化。显然，发行说明无法列出每一个在版本之间做出的更改；本文档主要集中在安全通告、用户可见的变化和重大架构改进上。
...
这也改变了在 [jail(8)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8&manpath=FreeBSD+7.2-RELEASE) 内部选择 IP 的语义，因为现在它使用与监狱外部相同的逻辑。
...
根据 `/usr/src/UPDATING` 中的说明，支持基于源代码的升级（即从源代码重新编译 FreeBSD 基础系统）到先前的版本。
