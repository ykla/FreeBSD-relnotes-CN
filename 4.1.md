...
任何安装失败或崩溃问题应通过使用 `send-pr` 命令报告（偏好使用网页界面的人也可以访问 `http://www.FreeBSD.org/send-pr.html`）。
...
* ALTQ 流量整形系统尚未合并——有望在 4.2 版本发布前加入。更为实验性的 KAME 代码也未合并。如果你需要这些功能，可以考虑使用 4.1-RELEASE+KAME 快照，可从 `ftp://ftp.kame.net` 获取，这些快照将在 4.1-RELEASE 发布后提供。

* 已知问题：通过 IPSEC 挂载的 NFS 在所有情况下似乎不可靠——可能出现挂载卡住和数据损坏的情况。

FreeBSD 内核新增了事件通知机制 `kqueue`。这是一种新接口，能够替代 `poll/select`，提供更好的性能，同时能报告多种不同类型的事件。支持监视套接字、管道、FIFO 和文件的变化，以及信号和进程。

FreeBSD 引导加载程序新增对英特尔 Wired for Management 2.0（PXE）的支持。由于 API 差异，旧版本的 PXE 不受支持。从而可通过 DHCP 实现网络启动。
...
内核和用户空间经过审计，检查由 `vfprintf()` 等函数中格式字符串的不当使用导致的错误和安全漏洞。未发现任何漏洞。
...
OpenSSH 升级至 2.1.0，提供对 SSH2 协议的支持，包括 DSA 密钥。因此，美国的 OpenSSH 用户不再需要依赖受限许可证的 RSAREF 工具包，该工具包是处理 RSA 密钥所必需的。OpenSSH 2.1 与其他 SSH2 客户端和服务器（包括 Port ssh2）互操作良好。更多信息请参阅 [http://www.openssh.com](http://www.openssh.com)。
...
`ls(1)` 现在可以通过参数 `-G` 生成带颜色的列表（并且需要适当的终端支持）。
