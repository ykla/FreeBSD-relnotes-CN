...
* FreeBSD "base" 系统现在可以使用 [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html) 包管理器进行安装和管理（参见下文"打包基系统"）。
...
FreeBSD 15.0 的一项重大变更是引入了使用 [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html) 包管理器安装和管理基系统的新方法。
...
从 CD 启动后，需要执行基于网络的安装（例如从 HTTP 或 FTP 服务器）。
+
此外，可将其写入 USB 闪存盘（U 盘）用于 amd64 架构，在可从 USB 启动的机器上进行安装。
还支持启动到基于「livefs」的救援模式。
不含预构建 package。
...
务必确保目标（`of=`）正确。
FreeBSD/arm SD 卡镜像::
可写入 SD 卡用于启动受支持的 arm 系统。
SD 卡镜像包含完整的 FreeBSD 安装，可安装到小至 5 GB 的 SD 卡。
+
为方便无控制台访问系统的用户，默认提供 `freebsd` 用户（密码为 `freebsd`）用于 [ssh(1)](https://man.freebsd.org/cgi/man.cgi?query=ssh&sektion=1&format=html) 访问。
此外，`root` 用户密码设为 `root`；强烈建议获得系统访问权限后立即更改这两位用户的密码。
+
...
| [The FreeBSD Foundation](https://www.freebsdfoundation.org) | [Juniper Networks](https://www.juniper.net) |
