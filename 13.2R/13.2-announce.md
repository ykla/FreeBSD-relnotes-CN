# FreeBSD 13.2-RELEASE 发行公告

日期：2023 年 4 月 11 日

FreeBSD 发布工程团队很高兴地宣布 FreeBSD 13.2-RELEASE 现已发布。这是 stable/13 分支的第三个版本。

部分亮点如下：

* OpenSSH 升级至 9.3p1 版本。
* OpenSSL 升级至 1.1.1t 版本。
* bhyve hypervisor 现在在客户机中支持超过 16 个 vCPU。
* 地址空间布局随机化（ASLR）现在默认对 64 位可执行文件启用。
* ZFS 升级至 OpenZFS 2.1.9 版本。
* 现在可以在使用日志软更新时对 UFS 文件系统进行快照。
* 内核 `wg(4)` WireGuard 驱动现已可用。
* 内核 `netlink(4)` 网络配置协议现已可用。
* 以及更多……
...
FreeBSD 13.2-RELEASE 现已支持 amd64、i386、powerpc、powerpc64、powerpc64le、powerpcspe、armv6、armv7、aarch64 和 riscv64 架构。

FreeBSD 13.2-RELEASE 可从可启动 ISO 镜像或通过网络安装。部分架构还支持从 USB 闪存盘安装。所需文件可按以下说明下载。
...
可写入 SD 卡并用于启动受支持的 arm 系统。SD 卡镜像包含完整的 FreeBSD 安装，可安装到小至 512 MB 的 SD 卡上。

为方便无控制台访问系统的用户，默认提供 `freebsd` 用户（密码为 `freebsd`）用于 `ssh(1)` 访问。此外，`root` 用户密码设为 `root`，强烈建议在获取系统访问权限后立即更改这两个用户的密码。

要将 FreeBSD/arm 镜像写入 SD 卡，使用 `dd(1)` 工具，将 *KERNEL* 替换为系统对应的内核配置名称。
...
这些 AMI ID 可从各区域的 Systems Manager Parameter Store 中使用以下键获取：

```sh
/aws/service/freebsd/amd64/base/ufs/13.2/RELEASE
```

AMI 也可从 Amazon(R) Marketplace 获取：
...
FreeBSD 13.2-RELEASE 预计也将在 Google Compute Engine(TM) Marketplace 上提供，待其完成第三方特定验证后可访问：
...
FreeBSD 13.2-RELEASE 可通过 https 从以下站点下载：

* <https://download.freebsd.org/ftp/releases/ISO-IMAGES/13.2/>

FreeBSD 13.2-RELEASE 虚拟机镜像可从以下地址下载：

* <https://download.freebsd.org/ftp/releases/VM-IMAGES/13.2-RELEASE/>

FreeBSD 13.2-RELEASE amd64 BASIC-CI 镜像可从以下地址下载：

* <https://download.freebsd.org/ftp/releases/CI-IMAGES/13.2-RELEASE/>

有关安装 FreeBSD 或将现有机器更新至 13.2-RELEASE 的说明，请参阅：

* <https://www.FreeBSD.org/releases/13.2R/installation/>

## 支持

基于新的 FreeBSD 支持模型，FreeBSD 13 发行系列将支持至至少 2026 年 1 月 31 日。此版本 FreeBSD 13.2-RELEASE 将支持至至少 FreeBSD 13.3-RELEASE 发布后三个月。更多支持信息可在以下地址找到：

* <https://www.FreeBSD.org/security/>

请注意，13.1 将支持至 13.2 发布日期起三个月，即 2023 年 7 月 31 日。

## 致谢

许多公司捐赠了设备、网络访问或人力时间，以支持 FreeBSD 13.2 的发布工程活动，包括：

| | |
| :--- | :--- |
| [FreeBSD 基金会](https://www.freebsdfoundation.org) | [Rubicon Communications, LLC (Netgate)](https://www.netgate.com) |
| [Tarsnap](https://www.tarsnap.com) | [NetApp](https://www.netapp.com) |
| [Internet Systems Consortium](https://www.isc.org) | [ByteMark Hosting](https://bytemark.co.uk) |
| [NextArray](https://nextarray.com) | [Sentex Data Communications](https://www.sentex.ca) |
| [New York Internet](https://www.nyi.net) | [Juniper Networks](https://www.juniper.net) |
| [NetActuate](https://www.netactuate.com) | [Department of Computer Science, National Chiao Tung University](https://www.cs.nctu.edu.tw) |
| [NLNet Labs](https://www.nlnetlabs.nl) | [iXsystems](https://www.ixsystems.com) |

13.2-RELEASE 的发布工程团队包括：
...
| Ed Maste <emaste@FreeBSD.org> | 安全官副负责人 |
| Colin Percival <cperciva@FreeBSD.org> | 发布工程副负责人、13.2-RELEASE 发布工程师 |
...
### 13.2-RELEASE aarch64 PINE64-LTS:
...
### 13.2-RELEASE aarch64 ROCKPRO64:
...
### 13.2-RELEASE riscv64 GENERICSD:
...
### 13.2-RELEASE amd64 (x86_64) BASIC-CI:
...
喜欢 FreeBSD？请向 FreeBSD 基金会[捐赠](https://www.freebsdfoundation.org/donate/) 以支持本版本及未来版本！
