# FreeBSD 11.3-RELEASE 硬件兼容列表

## 引言

本文档包含 FreeBSD 11.3-RELEASE 的硬件兼容性说明。其中列出了 FreeBSD 支持的硬件平台，以及各类硬件设备（存储控制器、网络接口等），并给出已知可正常工作的设备实例。
...
* 所有 Intel(R) Core(TM) 2（非 Core(TM) Duo）及更新处理器。
* 所有 Intel(R) Core(TM) i 系列处理器。
* 所有 Intel(R) Pentium(R) D 处理器。
* 所有 Intel(R) Centrino(R) Duo 和 Centrino(R) Pro 平台。
* 采用 "Cedar Mill" 核心的 Intel(R) Pentium(R) 4 和 Celeron(R) D 支持 EM64T。

采用 "Prescott" 核心的部分 Intel(R) Pentium(R) 4 和 Celeron(R) D 支持 EM64T。关于 Intel 处理器的 EM64T 支持情况，请参阅 [Intel Processor Spec Finder](http://processorfinder.intel.com) 获取确切答案。
...
FreeBSD 会在支持此特性的 CPU 上利用 SMT（Symmetric MultiThreading，对称多线程，在 Intel(R) CPU 上又称 HyperThreading）。默认安装的 `GENERIC` 内核会自动检测额外的逻辑处理器。默认的 FreeBSD 调度器能识别系统上的处理器拓扑，并选择逻辑和物理处理器以获得最佳性能。[smp(4)](https://man.freebsd.org/cgi/man.cgi?query=smp&sektion=4&format=html) 手册页有更多细节。
...
FreeBSD 通常可在基于 i386 的笔记本电脑上运行，但对某些硬件特性（如声音、图形、电源管理和 PCCARD 扩展槽）的支持程度因机器而异。这些特性在不同机器上的表现差异较大，通常需要在 FreeBSD 中进行特殊处理以规避硬件缺陷或其他异常。如有疑问，查阅 [FreeBSD laptop computer mailing list](http://lists.FreeBSD.org/mailman/listinfo/freebsd-mobile) 的归档可能有所帮助。
...
NEC PC-9801/9821 系列搭配几乎所有 i386(TM) 兼容处理器，包括 80486、Pentium(R)、Pentium(R) Pro、Pentium(R) II 及其变种。所有 AMD、Cyrix、IBM 和 IDT 的 i386(TM) 兼容处理器也受支持。

NEC FC-9801/9821 系列以及 NEC SV-98 系列（两者均与 PC-9801/9821 系列兼容）应当受支持。

EPSON PC-386/486/586 系列与 NEC PC-9801 系列兼容，受支持。

不支持高分辨率模式。NEC PC-98XA/XL/RL/XL^2 和 NEC PC-H98 系列仅在普通（PC-9801 兼容）模式下受支持。

虽然存在部分多处理器系统（如 Rs20/B20），但 FreeBSD 的 SMP 相关特性尚未受支持。

支持 PC-9801/9821 标准总线（C-Bus）、PC-9801NOTE 扩展总线（110 针）和 PCI 总线。不支持 New Extend Standard Architecture（NESA）总线（用于 PC-H98、SV-H98 和 FC-H98 系列）。
...
如果你有不在此列表中的系统，它可能尚未经过 FreeBSD 11.3-RELEASE 测试。我们鼓励你尝试并致信 [FreeBSD SPARC porting mailing list](http://lists.FreeBSD.org/mailman/listinfo/freebsd-sparc64) 反馈结果，包括哪些设备可用、哪些不可用。
...
以下系统受 FreeBSD 部分支持。特别是基于 SBus 的系统中的光纤通道控制器不受支持。但可搭配 [esp(4)](https://man.freebsd.org/cgi/man.cgi?query=esp&sektion=4&format=html) 驱动支持的 SCSI 控制器（Sun ESP SCSI、Sun FAS Fast-SCSI 和 Sun FAS366 Fast-Wide SCSI 控制器）使用。

* Sun Enterprise(TM) 3500
* Sun Enterprise(TM) 4500

自 7.2-RELEASE 起，基于 Sun UltraSPARC(R) III 及更新型号的 sparc64 系统也受 FreeBSD 支持，包括以下已知可工作的系统：
...
* Sun Fire(TM) V880
* Sun Fire(TM) V890（支持首次出现于 7.4-RELEASE 和 8.1-RELEASE，仅限非混合 UltraSPARC(R) IV/IV+ CPU 配置）
* Netra(TM) 20/Netra(TM) T4

以下 Sun UltraSPARC(R) 系统未经测试，但据信也受 FreeBSD 支持：

* Sun Fire(TM) V125
* Sun Fire(TM) V490（支持首次出现于 7.4-RELEASE 和 8.1-RELEASE，仅限非混合 UltraSPARC(R) IV/IV+ CPU 配置）
...
[amd64, i386, pc98, sparc64] IDE/ATA controllers ([ata(4)](https://man.freebsd.org/cgi/man.cgi?query=ata&sektion=4&format=html) driver)
...
[adw(4)](https://man.freebsd.org/cgi/man.cgi?query=adw&sektion=4&format=html) 驱动支持 SCSI 控制器，包括：
...
[ahb(4)](https://man.freebsd.org/cgi/man.cgi?query=ahb&sektion=4&format=html) 驱动支持以下 SCSI 主机适配器：
...
[ahci(4)](https://man.freebsd.org/cgi/man.cgi?query=ahci&sektion=4&format=html) 驱动支持具有 PCI class 1（mass storage）、subclass 6（SATA）和 programming interface 1（AHCI）的 AHCI 兼容控制器。

此外，与 ata(4) 的 atamarvell 和 atajmicron 驱动配合时，它支持传统 PATA + AHCI-SATA 组合控制器的 AHCI 部分，例如 JMicron JMB36x 和 Marvell 88SE61xx。
...
[amr(4)](https://man.freebsd.org/cgi/man.cgi?query=amr&sektion=4&format=html) 驱动支持的控制器包括：
...
[bt(4)](https://man.freebsd.org/cgi/man.cgi?query=bt&sektion=4&format=html) 驱动支持 BusLogic MultiMaster "W"、"C"、"S" 和 "A" 系列及兼容的 SCSI 主机适配器：
...
AMI FastDisk Host Adapters that are true BusLogic MultiMaster clones are also supported by the [bt(4)](https://man.freebsd.org/cgi/man.cgi?query=bt&sektion=4&format=html) driver.

[ciss(4)](https://man.freebsd.org/cgi/man.cgi?query=ciss&sektion=4&format=html) 驱动支持的控制器包括：
...
[ct(4)](https://man.freebsd.org/cgi/man.cgi?query=ct&sektion=4&format=html) 驱动支持以下适配器：
...
[esp(4)](https://man.freebsd.org/cgi/man.cgi?query=esp&sektion=4&format=html) 驱动支持的控制器包括：
...
[iir(4)](https://man.freebsd.org/cgi/man.cgi?query=iir&sektion=4&format=html) 驱动支持的控制器包括：
...
[mlx(4)](https://man.freebsd.org/cgi/man.cgi?query=mlx&sektion=4&format=html) 驱动支持的控制器包括：
...
支持所有主要的固件版本（2.x、3.x、4.x 和 5.x），但始终建议升级到该控制器可用的最新固件。

未列出的兼容 Mylex 控制器应当可用，但未经验证。

> **注意**：
>
> [amd64, i386] 支持从这些控制器启动。不支持 EISA 适配器。

[mly(4)](https://man.freebsd.org/cgi/man.cgi?query=mly&sektion=4&format=html) 驱动支持的控制器包括：
...
这些系统还包含本驱动同样支持的 Integrated RAID Mirroring 和 Integrated RAID Mirroring Enhanced。

SAS 控制器芯片也出现在许多新的 AMD/Opteron 系统上，例如 Sun 4100。注意，此控制器可同时驱动 SAS 和 SATA 驱动器或两者的混用。这些控制器的 Integrated RAID Mirroring 充其量仅得到有限支持。
...
注意，此硬件仅对 ATA DMA 命令支持命令队列和基于 FIS 的交换。ATAPI 和非 DMA ATA 命令在每个端口上逐条执行。
...
[nsp(4)](https://man.freebsd.org/cgi/man.cgi?query=nsp&sektion=4&format=html) 驱动支持的控制器包括：
...
[pst(4)](http://www.FreeBSD.org/cgi/man.cgi?query=pst&sektion=4&manpath=freebsd-release-ports) 驱动支持 Promise Supertrak SX6000 ATA 硬件 RAID 控制器。
...
[stg(4)](https://man.freebsd.org/cgi/man.cgi?query=stg&sektion=4&format=html) 驱动支持的控制器包括：
...
[sym(4)](https://man.freebsd.org/cgi/man.cgi?query=sym&sektion=4&format=html) 驱动支持的 SCSI 控制器可内置于主板，或位于以下附加卡之一：
...
[vpo(4)](https://man.freebsd.org/cgi/man.cgi?query=vpo&sektion=4&format=html) 驱动支持以下并行转 SCSI 接口：
...
wds(4) 驱动支持 WD7000 SCSI 控制器。

对于所有受支持的 SCSI 控制器，全面支持 SCSI-I、SCSI-II 和 SCSI-III 外设，包括硬盘、光盘、磁带机（包括 DAT、8mm Exabyte、Mammoth 和 DLT）、介质更换器、处理器目标设备和 CD-ROM 驱动器。支持 CD-ROM 命令的 WORM 设备可由 CD-ROM 驱动（如 [cd(4)](https://man.freebsd.org/cgi/man.cgi?query=cd&sektion=4&format=html)）以只读访问方式支持。WORM/CD-R/CD-RW 写入支持由 [cdrecord(1)](https://man.freebsd.org/cgi/man.cgi?query=cdrecord&sektion=1&format=html) 提供，它是 Ports Collection 中 [sysutils/cdrtools](http://www.freebsd.org/cgi/url.cgi?ports/sysutils/cdrtools/pkg-descr) port 的一部分。
...
* Mitsumi proprietary CD-ROM interface (all models) ([man(4)](https://man.freebsd.org/cgi/man.cgi?query=man&sektion=4&format=html))
...
[age(4)](https://man.freebsd.org/cgi/man.cgi?query=age&sektion=4&format=html) 驱动为基于 Attansic/Atheros L1 Gigabit Ethernet 控制器芯片的 LOM 提供支持，包括：
...
[bce(4)](https://man.freebsd.org/cgi/man.cgi?query=bce&sektion=4&format=html) 驱动为基于 QLogic NetXtreme II Gigabit Ethernet 控制器系列的各种 NIC 提供支持，包括以下型号：
...
[bge(4)](https://man.freebsd.org/cgi/man.cgi?query=bge&sektion=4&format=html) 驱动为基于 Broadcom BCM570x Gigabit Ethernet 控制器芯片系列的各种 NIC 提供支持，包括以下型号：
...
[bnxt(4)](https://man.freebsd.org/cgi/man.cgi?query=bnxt&sektion=4&format=html) 驱动为基于 Broadcom NetXtreme-C 和 NetXtreme-E Gigabit Ethernet 控制器芯片系列的各种 NIC 提供支持，包括以下型号：
...
[bxe(4)](https://man.freebsd.org/cgi/man.cgi?query=bxe&sektion=4&format=html) 驱动为基于 QLogic NetXtreme II 10Gb Ethernet 控制器芯片系列的各种 NIC 提供支持，包括以下型号：
...
已知以下附加卡目前可与 [cas(4)](https://man.freebsd.org/cgi/man.cgi?query=cas&sektion=4&format=html) 驱动配合工作：
...
[cxgb(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgb&sektion=4&format=html) 驱动支持基于 T3 和 T3B 芯片组的 10 Gigabit 和 1 Gigabit 以太网适配器：
...
已知以下 NIC 目前可与 [dc(4)](https://man.freebsd.org/cgi/man.cgi?query=dc&sektion=4&format=html) 驱动配合工作：
...
[de(4)](https://man.freebsd.org/cgi/man.cgi?query=de&sektion=4&format=html) 驱动支持的适配器包括：
...
[ed(4)](https://man.freebsd.org/cgi/man.cgi?query=ed&sektion=4&format=html) 驱动支持以下以太网 NIC：
...
[ep(4)](https://man.freebsd.org/cgi/man.cgi?query=ep&sektion=4&format=html) 驱动支持基于 3Com 3C5x9 Etherlink III Parallel Tasking 芯片组的以太网适配器，包括：
...
[ex(4)](https://man.freebsd.org/cgi/man.cgi?query=ex&sektion=4&format=html) 驱动支持以下以太网适配器：
...
[fe(4)](https://man.freebsd.org/cgi/man.cgi?query=fe&sektion=4&format=html) 驱动支持的控制器和卡包括：
...
已知以下附加卡目前可与 [gem(4)](https://man.freebsd.org/cgi/man.cgi?query=gem&sektion=4&format=html) 驱动配合工作：
...
[e(4)](https://man.freebsd.org/cgi/man.cgi?query=e&sektion=4&format=html) 驱动支持基于 Intel i82586 芯片的以下 8 位和 16 位 ISA 以太网卡：
...
[ixgb(4)](https://man.freebsd.org/cgi/man.cgi?query=ixgb&sektion=4&format=html) 驱动支持以下卡：
...
[ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 驱动支持这些 SFP+ Pluggable Optics：

* Intel TRIPLE RATE 1G/10G/40G QSFP+ SR (bailed) E40GQSFPSR
* Intel TRIPLE RATE 1G/10G/40G QSFP+ SR (bailed) E40GQSFPLR

[ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 驱动支持带 SR 模块的 10Gb 和 1Gb 以太网适配器：

* Intel DUAL RATE 1G/10G SFP+ SR (bailed) FTLX8571D3BCV-IT
* Intel DUAL RATE 1G/10G SFP+ SR (bailed) AFBR-703SDZ-IN2

[ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 驱动支持带 LR 模块的 10Gb 和 1Gb 以太网适配器：

* Intel DUAL RATE 1G/10G SFP+ LR (bailed) FTLX1471D3BCV-IT
* Intel DUAL RATE 1G/10G SFP+ LR (bailed) AFCT-701SDZ-IN2

注意，基于 X710/XL710 的 SFP+ 适配器还支持所有符合 SFF-8431 v4.1 和 SFF-8472 v10.4 规范的无源和有源限幅直连电缆。
...
[mlx5en(4)](https://man.freebsd.org/cgi/man.cgi?query=mlx5en&sektion=4&format=html) 驱动支持 100Gb、50Gb、40Gb、25Gb 和 10Gb 以太网适配器。ConnectX-4 支持：10/20/25/40/50/56/100Gb/s 速率。ConnectX-4 LX 支持：10/25/40/50Gb/s 速率（且功耗降低）：
...
[msk(4)](https://man.freebsd.org/cgi/man.cgi?query=msk&sektion=4&format=html) 驱动为基于 Marvell/SysKonnect Yukon II Gigabit Ethernet 控制器芯片系列的各种 NIC 提供支持，包括：
...
[mxge(4)](https://man.freebsd.org/cgi/man.cgi?query=mxge&sektion=4&format=html) 驱动支持基于 Myricom LANai Z8E 芯片的 10 Gigabit Ethernet 适配器：
...
[nxge(4)](https://man.freebsd.org/cgi/man.cgi?query=nxge&sektion=4&format=html) 驱动支持列于 <http://www.neterion.com/how/pricing.html> 的 Neterion Xframe 10 Gigabit Ethernet 适配器。
...
[pcn(4)](https://man.freebsd.org/cgi/man.cgi?query=pcn&sektion=4&format=html) 驱动支持基于 AMD PCnet/FAST、PCnet/FAST+、PCnet/FAST III、PCnet/PRO 和 PCnet/Home Fast Ethernet 芯片的适配器和嵌入式控制器：
...
[qlnxe(4)](https://man.freebsd.org/cgi/man.cgi?query=qlnxe&sektion=4&format=html) 驱动支持基于以下芯片组的 25/40/100 Gigabit Ethernet 和 CNA 适配器：

* QLogic 45000 series
* QLogic 41000 series

[qlxgb(4)](https://man.freebsd.org/cgi/man.cgi?query=qlxgb&sektion=4&format=html) 驱动支持基于以下芯片组的 10 Gigabit Ethernet 和 CNA 适配器：

* QLogic 3200 series
* QLogic 8200 series

[qlxgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=qlxgbe&sektion=4&format=html) 驱动支持基于以下芯片组的 10 Gigabit Ethernet 和 CNA 适配器：

* QLogic 8300 series

[qlxge(4)](https://man.freebsd.org/cgi/man.cgi?query=qlxge&sektion=4&format=html) 驱动支持基于以下芯片组的 10 Gigabit Ethernet 和 CNA 适配器：
...
[i386, pc98, amd64] [rue(4)](https://man.freebsd.org/cgi/man.cgi?query=rue&sektion=4&format=html) 驱动支持基于 RealTek RTL8150 的 USB Ethernet 适配器，包括：
...
[sfxge(4)](https://man.freebsd.org/cgi/man.cgi?query=sfxge&sektion=4&format=html) 驱动支持基于 Solarflare SFC9000 系列控制器的所有 10Gb Ethernet 适配器。

[sge(4)](https://man.freebsd.org/cgi/man.cgi?query=sge&sektion=4&format=html) 驱动支持以下以太网控制器：
...
Farallon EtherWave 和 EtherMac 卡有两种变体。[ep(4)](https://man.freebsd.org/cgi/man.cgi?query=ep&sektion=4&format=html) 驱动支持 595 和 895 卡。这些卡正面有蓝色箭头和 3Com 标志。Farallon 595a 卡正面有红色箭头，也称为 EtherWave 和 EtherMac，由 [sn(4)](https://man.freebsd.org/cgi/man.cgi?query=sn&sektion=4&format=html) 驱动支持。
...
[snc(4)](https://man.freebsd.org/cgi/man.cgi?query=snc&sektion=4&format=html) 驱动还支持作为 **64 \* 16** 位 Microwave Serial EEPROM 的 National Semiconductor NS46C46。
...
[stge(4)](https://man.freebsd.org/cgi/man.cgi?query=stge&sektion=4&format=html) 驱动支持基于 Sundance/Tamarack TC9021 Gigabit Ethernet 控制器芯片的多种 NIC，包括：
...
[i386, pc98, amd64, sparc64] [ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动支持基于 Alteon Tigon I 和 II 芯片的 Gigabit Ethernet 适配器。[ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动已用以下适配器测试：
...
[i386, pc98, amd64] [tl(4)](https://man.freebsd.org/cgi/man.cgi?query=tl&sektion=4&format=html) 驱动支持基于 Texas Instruments ThunderLAN 的 Ethernet 和 Fast Ethernet 适配器，包括大量 Compaq PCI Ethernet 适配器。还支持：
...
[tl(4)](https://man.freebsd.org/cgi/man.cgi?query=tl&sektion=4&format=html) 驱动还支持多种 Compaq Prosignia 服务器和 Compaq Deskpro 台式机的内置 Ethernet 适配器，包括：
...
[vte(4)](https://man.freebsd.org/cgi/man.cgi?query=vte&sektion=4&format=html) 驱动支持以下以太网控制器：

* DM&P Vortex86 RDC R6040 Fast Ethernet controller

[i386, pc98, amd64] [vx(4)](https://man.freebsd.org/cgi/man.cgi?query=vx&sektion=4&format=html) 驱动支持以下卡：
...
[i386, amd64] [vxge(4)](https://man.freebsd.org/cgi/man.cgi?query=vxge&sektion=4&format=html) 驱动支持列于 <http://www.neterion.com> 的 Neterion X3100 10 Gigabit Ethernet 适配器。
...
3C656 系列 CardBus 卡和 3C556 系列 MiniPCI 卡均有内置的专有调制解调器。[xl(4)](https://man.freebsd.org/cgi/man.cgi?query=xl&sektion=4&format=html) 驱动和其他 FreeBSD 驱动均不支持此调制解调器。
...
[i386, pc98, amd64, sparc64] [ath(4)](https://man.freebsd.org/cgi/man.cgi?query=ath&sektion=4&format=html) 驱动支持所有 Atheros Cardbus 和 PCI 卡，但基于 AR5005VL 芯片组的除外。
...
[bwi(4)](https://man.freebsd.org/cgi/man.cgi?query=bwi&sektion=4&format=html) 驱动使用 Broadcom 固件的旧版 v3。虽然此旧版固件支持大多数 BCM43xx 部件，但 [bwn(4)](https://man.freebsd.org/cgi/man.cgi?query=bwn&sektion=4&format=html) 驱动对其支持的较新芯片工作更好。如果使用较旧的 Broadcom 芯片组（BCM4301、BCM4303 和 BCM4306 rev 2），必须使用 [bwi(4)](https://man.freebsd.org/cgi/man.cgi?query=bwi&sektion=4&format=html) 驱动。[bwn(4)](https://man.freebsd.org/cgi/man.cgi?query=bwn&sektion=4&format=html) 驱动使用的 v4 版固件不支持这些芯片。
...
较旧 Broadcom 芯片组（BCM4301、BCM4303 和 BCM4306 rev 2）的用户必须使用 bwi(4)，因为 v4 版固件不支持这些芯片。较新的固件太大，无法装入这些旧芯片。
...
[i386, amd64] [otus(4)](https://man.freebsd.org/cgi/man.cgi?query=otus&sektion=4&format=html) 驱动支持 Atheros AR9170 USB IEEE 802.11b/g/n 无线网络适配器，包括：
...
Realtek RTL8188CE based PCIe IEEE 802.11b/g/n wireless network adapters ([rtwn(4)](https://man.freebsd.org/cgi/man.cgi?query=rtwn&sektion=4&format=html) driver)
...
最新列表可在 <http://customerproducts.atheros.com/customerproducts/default.asp> 找到。
...
[urtwn(4)](https://man.freebsd.org/cgi/man.cgi?query=urtwn&sektion=4&format=html) 驱动支持基于 Realtek RTL8188CU/RTL8188RU/RTL8188EU/RTL8192CU 的 USB IEEE 802.11b/g/n 无线网络适配器，包括：
...
[i386, amd64] Specialix SI/XIO/SX multiport serial cards, with both the older SIHOST2.x and the "enhanced" (transputer based, aka JET) host cards (ISA, EISA and PCI) are supported. Note that the newer SX+ PCI cards are not currently supported. ([si(4)](https://man.freebsd.org/cgi/man.cgi?query=si&sektion=4&format=html) driver)
...
部分板载 CS4610 芯片搭配的是 CS423x ISA 编解码器，而非 CS4297 AC97 编解码器。[snd_csa(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_csa&sektion=4&format=html) 驱动尚不支持此类配置。
...
[snd_emu10kx(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_emu10kx&sektion=4&format=html) 驱动 _不_ 支持以下声卡（尽管它们的名称与某些受支持的卡相似）：
...
[i386, amd64] [snd_hda(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hda&sektion=4&format=html) 驱动支持具有 PCI 类 4（multimedia）和子类 3（HDA）且与 Intel HDA 规范兼容的控制器。

[snd_hda(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hda&sektion=4&format=html) 驱动支持超过两百种不同的控制器和 CODEC。在此逐一列出意义不大，因为在多数情况下，特定 CODEC 配置和接线比 CODEC 本身的类型更重要。

[i386, amd64] [snd_hdspe(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hdspe&sektion=4&format=html) 驱动支持以下音频设备：
...
注意，较旧的基于 ES18xx 芯片组的 ESS ISA 卡由 snd_ess(4) 和/或 snd_sbc(4) 支持。
...
[i386, amd64, sparc64] [snd_t4dwave(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_t4dwave&sektion=4&format=html) 驱动支持以下音频设备：
...
[amd64, i386, pc98] 支持多种 USB 外设；已知可工作的设备列于本节。由于大多数 USB 设备具有通用性，除少数例外，任何属于给定类别的设备均受支持，即使未在此显式列出。

> **注意**：
> [amd64, i386, pc98] USB Ethernet 适配器可在以太网接口列表部分找到。

> **注意**：
> [amd64, i386, pc98] USB Bluetooth 适配器可在 Bluetooth 部分找到。

[i386, pc98, amd64, powerpc] [ohci(4)](https://man.freebsd.org/cgi/man.cgi?query=ohci&sektion=4&format=html) 驱动支持所有符合 OHCI v1.0 的控制器，包括：
...
[i386, pc98, amd64, powerpc] [uhci(4)](https://man.freebsd.org/cgi/man.cgi?query=uhci&sektion=4&format=html) 驱动支持所有符合 UHCI v1.1 的控制器，包括：

* Intel 82371AB/EB (PIIX4)
* Intel 82371SB (PIIX3)
* VIA 83C572

[amd64, i386, pc98] USB 2.0 controllers using the EHCI interface ([ehci(4)](https://man.freebsd.org/cgi/man.cgi?query=ehci&sektion=4&format=html) driver)

[amd64, i386, pc98] Hubs

[amd64, i386, pc98] Keyboards ([ukbd(4)](https://man.freebsd.org/cgi/man.cgi?query=ukbd&sektion=4&format=html) driver)
...
[amd64, i386, pc98] Mice ([ums(4)](https://man.freebsd.org/cgi/man.cgi?query=ums&sektion=4&format=html) driver)

[i386, pc98, amd64, powerpc] [ulpt(4)](https://man.freebsd.org/cgi/man.cgi?query=ulpt&sektion=4&format=html) 驱动支持 USB 打印机和并行打印转换线缆，包括：
...
[i386, pc98, amd64, powerpc] [umass(4)](https://man.freebsd.org/cgi/man.cgi?query=umass&sektion=4&format=html) 驱动支持 USB Mass Storage 设备，包括：
...
[i386, sparc64, amd64, powerpc] [fwohci(4)](https://man.freebsd.org/cgi/man.cgi?query=fwohci&sektion=4&format=html) 驱动支持 PCI/CardBus FireWire 接口卡。该驱动支持以下 IEEE 1394 OHCI 芯片组：
...
[i386, pc98, amd64] [ng_bt3c(4)](https://man.freebsd.org/cgi/man.cgi?query=ng_bt3c&sektion=4&format=html) 驱动支持 3Com/HP 3CRWB6096-A PCCARD 蓝牙适配器。
...
[i386, pc98, amd64] [hifn(4)](https://man.freebsd.org/cgi/man.cgi?query=hifn&sektion=4&format=html) 驱动支持多种包含 Hifn 7751、7951、7811、7955 和 7956 芯片组的卡，例如：
...
* [amd64, i386, pc98] USB keyboards ([ukbd(4)](https://man.freebsd.org/cgi/man.cgi?query=ukbd&sektion=4&format=html) driver)
...
* [amd64, i386, pc98] USB mice ([ums(4)](https://man.freebsd.org/cgi/man.cgi?query=ums&sektion=4&format=html) driver)

> **注意**：
> [moused(8)](https://man.freebsd.org/cgi/man.cgi?query=moused&sektion=8&format=html) 手册页提供了关于在 FreeBSD 中使用定点设备的更多信息。关于在 Xorg 中使用定点设备的信息可在 `http://www.x.org/` 找到。
