...
FreeBSD 4.7-RELEASE 的发行版本可以通过 [ftp://ftp.FreeBSD.org/](ftp://ftp.freebsd.org/) 或其任何镜像站点获取。获取此版本（或其他版本）的方法，请参阅 [《FreeBSD 手册》](http://www.freebsd.org/doc/en_US.ISO8859-1/books/handbook/) 中的 [“获取 FreeBSD”](http://www.freebsd.org/doc/en_US.ISO8859-1/books/handbook/mirrors.html) 附录。
...
新增了 RLIMIT_VMEM 支持。此功能定义了新的资源限制，覆盖进程的整个虚拟内存空间，包括 [mmap(2)](http://www.freebsd.org/cgi/man.cgi?query=mmap&sektion=2&manpath=FreeBSD+4.7-RELEASE) 空间。此限制可通过 [login.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=login.conf&sektion=5&manpath=FreeBSD+4.7-RELEASE) 中的新变量 `vmemoryuse` 配置。
...
新增设备驱动程序 [ucom(4)](http://www.freebsd.org/cgi/man.cgi?query=ucom&sektion=4&manpath=FreeBSD+4.7-RELEASE)，用于支持 USB 调制解调器、串口设备及其他需要仿真为 tty 的程序。相关的驱动程序 [uplcom(4)](http://www.freebsd.org/cgi/man.cgi?query=uplcom&sektion=4&manpath=FreeBSD+4.7-RELEASE) 和 [uvscom(4)](http://www.freebsd.org/cgi/man.cgi?query=uvscom&sektion=4&manpath=FreeBSD+4.7-RELEASE) 分别为 Prolific PL-2303 串口适配器和 SUNTAC Slipper U VS-10U 提供支持。
...
[em(4)](http://www.freebsd.org/cgi/man.cgi?query=em&sektion=4&manpath=FreeBSD+4.7-RELEASE) 驱动程序现在支持 Intel 82545EM 和 82545EB 芯片，并且支持 VLAN。

[rp(4)](http://www.freebsd.org/cgi/man.cgi?query=rp&sektion=4&manpath=FreeBSD+4.7-RELEASE) 驱动程序已更新至版本 3.02，现在可以作为模块构建。

#### 2.1.4 网络协议

新增了 [ipfw(4)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=4&manpath=FreeBSD+4.7-RELEASE) 的新版（通常称为“IPFW2”）作为选项。它在内核中采用了类似 [bpf(4)](http://www.freebsd.org/cgi/man.cgi?query=bpf&sektion=4&manpath=FreeBSD+4.7-RELEASE) 指令的可变大小规则表示法。尽管大部分外部可见的行为（即通过 [ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+4.7-RELEASE)）保持不变，[ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+4.7-RELEASE) 现在支持匹配字段之间的逻辑或连接操作。此新版本默认未启用。如需使用：

1. 在内核配置中新增 **options IPFW2**（尚未记录）。
2. 使用 `make` 并指定 `-DIPFW2` 选项来编译并安装 libalias 和 [ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+4.7-RELEASE)。
...
[tcp(4)](http://www.freebsd.org/cgi/man.cgi?query=tcp&sektion=4&manpath=FreeBSD+4.7-RELEASE) 协议的重传计时器现在可以通过两个 sysctl 变量（`net.inet.tcp.rexmit_min` 和 `net.inet.tcp.rexmit_slop`）调整。
...
VT8233 音频控制器现在拥有自己的驱动程序，以支持所有已知的硬件修订版。可以通过在内核配置中新增 **device pcm**，或在 **/boot/loader.conf** 中新增 `snd_via8233="YES"`，以在启动时加载此驱动程序。支持此项工作的文档由 VIA 提供。
...
最初对安全公告 SA-02:23 的修复（解决了 set-user-id 或 set-group-id 程序使用文件描述符的问题）存在错误。对于使用 [procfs(5)](http://www.freebsd.org/cgi/man.cgi?query=procfs&sektion=5&manpath=FreeBSD+4.7-RELEASE) 或 [linprocfs(5)](http://www.freebsd.org/cgi/man.cgi?query=linprocfs&sektion=5&manpath=FreeBSD+4.7-RELEASE) 的系统，仍可能被利用。此错误已修正，修订版的安全公告 [FreeBSD-SA-02:23](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-02:23.stdio.asc) 提供了更多细节。

修复了解析器中的缓冲区溢出漏洞，该漏洞可能被恶意域名服务器或伪造 DNS 消息的攻击者利用。详见安全公告 [FreeBSD-SA-02:28](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-02:28.resolv.asc)。

修复了 [tcpdump(1)](http://www.freebsd.org/cgi/man.cgi?query=tcpdump&sektion=1&manpath=FreeBSD+4.7-RELEASE) 中的缓冲区溢出漏洞，该漏洞可能因格式错误的 NFS 数据包触发。详见安全公告 [FreeBSD-SA-02:29](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-02:29.tcpdump.asc)。

[ktrace(1)](http://www.freebsd.org/cgi/man.cgi?query=ktrace&sektion=1&manpath=FreeBSD+4.7-RELEASE) 不再允许跟踪先前具有特权的进程操作，从而防止了该进程在放弃特权之前可能获取的敏感信息泄露。此问题的讨论请参阅安全公告 [FreeBSD-SA-02:30](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-02:30.ktrace.asc)。
...
通过升级基本系统中的 **OpenSSL** 版本，修复了其多个缓冲区溢出漏洞。更多信息请参阅安全公告 [FreeBSD-SA-02:33](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-02:33.openssl.asc)。
...
修复了一个可能导致使用 libkvm 的应用程序泄露敏感文件描述符的漏洞。详见安全公告 [FreeBSD-SA-02:39](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-02:39.libkvm.asc)。

### 2.3 用户空间的变更

[biff(1)](http://www.freebsd.org/cgi/man.cgi?query=biff&sektion=1&manpath=FreeBSD+4.7-RELEASE) 现在接受 `b` 参数，用于启用新邮件的“响铃通知”（不会影响终端内容显示，区别于 biff y）。

[cp(1)](http://www.freebsd.org/cgi/man.cgi?query=cp&sektion=1&manpath=FreeBSD+4.7-RELEASE) 现新增（非标准）`-n` 选项，当需要覆盖文件时自动回答“否”。
...
[finger(1)](http://www.freebsd.org/cgi/man.cgi?query=finger&sektion=1&manpath=FreeBSD+4.7-RELEASE) 增加了 `-g` 选项，将 GECOS 信息的输出限制为仅用户全名。
...
[ftp(1)](http://www.freebsd.org/cgi/man.cgi?query=ftp&sektion=1&manpath=FreeBSD+4.7-RELEASE) 新增了命令 `epsv4`，用于在使用 IPv4 时在 EPSV/EPRT 和传统 PASV/PORT 请求之间切换。此命令解决了运行 [ftp(1)](http://www.freebsd.org/cgi/man.cgi?query=ftp&sektion=1&manpath=FreeBSD+4.7-RELEASE) 时因 IPFilter 或其他防火墙不支持新 FTP 请求而导致的问题。
...
[ifconfig(8)](http://www.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8&manpath=FreeBSD+4.7-RELEASE) 增加了命令 `eui64`，自动填充 IPv6 地址的低 64 位，使得 [prefix(8)](http://www.freebsd.org/cgi/man.cgi?query=prefix&sektion=8&manpath=FreeBSD+4.7-RELEASE) 过时。
...
[inetd(8)](http://www.freebsd.org/cgi/man.cgi?query=inetd&sektion=8&manpath=FreeBSD+4.7-RELEASE) 现在能够限制来自单个 IP 地址的每个服务的最大同时调用数。
...
[ls(1)](http://www.freebsd.org/cgi/man.cgi?query=ls&sektion=1&manpath=FreeBSD+4.7-RELEASE) 程序新增了 `-m` 选项以跨页显示文件，`-p` 选项强制在目录后显示 `/`，以及 `-x` 选项以跨页对文件名排序。
...
[sh(1)](http://www.freebsd.org/cgi/man.cgi?query=sh&sektion=1&manpath=FreeBSD+4.7-RELEASE) 的内置命令 `cd` 现在支持 `-L` 和 `-P` 参数，分别用于调用逻辑模式或物理模式。默认使用逻辑模式，但可以通过 **sh(1)** 的物理选项变更默认设置。

[sh(1)](http://www.freebsd.org/cgi/man.cgi?query=sh&sektion=1&manpath=FreeBSD+4.7-RELEASE) 的内置命令 `jobs` 现在支持 `-s` 参数，仅输出进程 ID（PID），以及 `-l` 参数，将 PID 添加到输出中。
...
- **OpenSSH** 已更新至 3.4p1 版本，主要变化包括：
  - `*2` 文件已弃用（例如，`~/.ssh/known_hosts` 可包含 `~/.ssh/known_hosts2` 的内容）。
  ...
  - [ssh-add(1)](http://www.freebsd.org/cgi/man.cgi?query=ssh-add&sektion=1&manpath=FreeBSD+4.7-RELEASE) 现在会添加所有三个默认密钥。
  ...
  - 修复了一些漏洞，包括一个可能导致整数溢出和权限提升的安全漏洞。
...
- **tcp_wrappers** 的贡献版本现已包含辅助守护进程 [tcpd(8)](http://www.freebsd.org/cgi/man.cgi?query=tcpd&sektion=8&manpath=FreeBSD+4.7-RELEASE)。尽管在标准 FreeBSD 安装中并非必需（因为 [inetd(8)](http://www.freebsd.org/cgi/man.cgi?query=inetd&sektion=8&manpath=FreeBSD+4.7-RELEASE) 已支持此功能），但对 **xinetd** 等 [inetd(8)](http://www.freebsd.org/cgi/man.cgi?query=inetd&sektion=8&manpath=FreeBSD+4.7-RELEASE) 替代程序可能有用。
...
现在，可以在 FreeBSD 4-STABLE 主机上生成 FreeBSD 5-CURRENT 的版本，反之亦然。此外，还支持跨架构发布（在不同架构的主机上为目标架构构建发布版本）。详见 [release(7)](http://www.freebsd.org/cgi/man.cgi?query=release&sektion=7&manpath=FreeBSD+4.7-RELEASE)。
...
- 执行 FreeBSD 的完整重装。严格来说，这不是一种升级方法，并且通常不如二进制升级方便，因为需要手动备份和恢复 **/etc** 的内容。然而，当你需要或想要变更磁盘分区时，这可能是有用的。
