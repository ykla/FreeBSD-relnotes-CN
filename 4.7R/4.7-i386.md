...
[ftp(1)](http://www.freebsd.org/cgi/man.cgi?query=ftp&sektion=1&manpath=FreeBSD+4.7-RELEASE) 新增了 `epsv4` 命令，用于在使用 IPv4 时在 EPSV/EPRT 和传统 PASV/PORT 请求之间切换。此命令解决了运行 [ftp(1)](http://www.freebsd.org/cgi/man.cgi?query=ftp&sektion=1&manpath=FreeBSD+4.7-RELEASE) 时因 IPFilter 或其他防火墙不支持新 FTP 请求而导致的问题。
...
[ifconfig(8)](http://www.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8&manpath=FreeBSD+4.7-RELEASE) 增加了 `eui64` 命令，自动填充 IPv6 地址的低 64 位，使得 [prefix(8)](http://www.freebsd.org/cgi/man.cgi?query=prefix&sektion=8&manpath=FreeBSD+4.7-RELEASE) 过时。
...
[sh(1)](http://www.freebsd.org/cgi/man.cgi?query=sh&sektion=1&manpath=FreeBSD+4.7-RELEASE) 的内置 `jobs` 命令现在支持 `-s` 参数，仅输出进程 ID（PID），以及 `-l` 参数，将 PID 添加到输出中。
