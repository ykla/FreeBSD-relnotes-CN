...
在 ATAPI CD-ROM 驱动器上，[cdcontrol(1)](http://www.freebsd.org/cgi/man.cgi?query=cdcontrol&sektion=1&manpath=FreeBSD+4.6-RELEASE) 现在支持 `speed` 命令，可设置驱动器的最大使用速度。
...
**/etc/rc.firewall** 和 **/etc/rc.firewall6** 当 `firewall_type` 变量指定了规则文件或防火墙类型不存在时，不再添加硬编码规则（此变更的目的是避免对站点的防火墙策略做出假设）。此外，`closed` 防火墙类型现在按照 [rc.firewall(8)](http://www.freebsd.org/cgi/man.cgi?query=rc.firewall&sektion=8&manpath=FreeBSD+4.6-RELEASE) 手册页中的说明工作。
...
[m4(1)](http://www.freebsd.org/cgi/man.cgi?query=m4&sektion=1&manpath=FreeBSD+4.6-RELEASE) 现在接受 `-s` 参数，使其输出 `#line` 指令供 [cpp(1)](http://www.freebsd.org/cgi/man.cgi?query=cpp&sektion=1&manpath=FreeBSD+4.6-RELEASE) 使用。
...
[ngctl(8)](http://www.freebsd.org/cgi/man.cgi?query=ngctl&sektion=8&manpath=FreeBSD+4.6-RELEASE) 现在支持 `write` 命令，用于沿指定挂钩发送数据包。

[patch(1)](http://www.freebsd.org/cgi/man.cgi?query=patch&sektion=1&manpath=FreeBSD+4.6-RELEASE) 现在接受 `-i` 命令行参数，用于从文件而非标准输入读取补丁。

[4.6.2] 当指定了 `no_fake_prompts` 变量时，[pam_opie(8)](http://www.freebsd.org/cgi/man.cgi?query=pam_opie&sektion=8&manpath=FreeBSD+4.6-RELEASE) 不再发出虚假挑战。
...
[reboot(8)](http://www.freebsd.org/cgi/man.cgi?query=reboot&sektion=8&manpath=FreeBSD+4.6-RELEASE) 现在接受 `-k` 参数，用于指定下次启动的内核。
