...
FreeBSD 是一款免费提供完整源代码的 4.4 BSD Lite 系统版本，适用于 Intel i386/i486/Pentium（或兼容）架构的 PC。它主要基于加州大学伯克利分校 CSRG 团队的软件，同时融入了来自 NetBSD、386BSD 和自由软件基金会的一些增强功能。
...
许多软件包也得到了升级或新增，例如 XFree86 3.1、xview 3.2、elm、nntp、mh、InterViews 以及几十个其他工具，这些工具已移植并可作为附加组件使用。有关完整的摘要，请查看 Ports（或软件包集合）。

贡献者列表请参阅文件 **CONTRIB.FreeBSD**，它应与你的 bindist 发行包一起提供。

另请参阅新的 **REGISTER.FreeBSD** 文件，以了解如何注册到“Free BSD 用户统计”系统。我们还提供了 **ROSTER.FreeBSD** 文件，列出了各领域的负责人（以便你可以直接联系他们）。建议使用此文件，以更快地解决可能遇到的问题！

FreeBSD 的核心不包含会限制出口到美国以外的 DES 代码。对于仅在美国境内使用的用户，我们提供了一个核心发行版的附加软件包，其中包含通常使用 DES 的程序。单独提供的辅助软件包任何人都可以使用。此外，还有一个为非美国用户准备的、可（从美国境外）自由出口的欧洲 DES 发行版，可以在 FreeBSD FAQ 中找到其详细信息。
...
正如前文所述，此版本完全基于 CSRG 最新（也是最后）发布的 BSD 版本——4.4 Lite。相比于 4.2BSD（Net/2），4.4 Lite 带来了许多改进，其中包括以下内容：
...
- **支持许多新的文件系统类型**
  新增了许多文件系统类型，例如堆叠文件系统（stackable filesystems）、联合文件系统（union filesystems）、“门户”（portals）、kernfs、简单的日志结构化文件系统、新的 NFS 版本（NQNFS）等。虽然其中某些文件系统尚不够完善，还需要额外的工作来实现更强的稳健性，但这只是一个好的开始。
...
目前，大多数文件系统可以按需动态加载，但 UFS 系列（FFS、LFS 和 MFS）除外。除 NFS 外，所有引用解除挂载后，这些文件系统即可卸载。为支持该功能，C 库中新增了 **getvfsbyname(3)** 函数族，**lsvfs(1)** 命令则在 shell 层面提供相同的信息。请注意以下当前限制：
...
要启用 S/Key 支持，可在 **/etc** 中创建一个 `skey.access` 文件，并根据需求进行填写。详见 **skey.access(5)** 和示例文件 **/usr/share/examples/etc/skey.access**。
...
**负责人：** phk **相关源码：** `isa/lpt.c`

#### ProAudioSpectrum SCSI 驱动

如果你有一块带 CD-ROM 的 PAS 卡，并且 MS-DOS 驱动程序名为 `TSLCDR.SYS`，那么“pas”驱动应该可以正常工作。你可以连接磁盘、CD-ROM 和磁带，但由于相关硬件限制，传输速率限制为小于 690 kbyte/秒。这对于 CD-ROM 的使用来说已经足够。

**负责人：** phk **相关源码：** `isa/pas.c`
...
此版本中提供了一项实验性功能，可以直接执行 gzip 压缩的二进制文件。启用后，你只需 gzip 压缩二进制文件，移除 `.gz` 扩展名并将其设置为可执行文件。尽管启用此功能会显著降低速度并增加内存消耗，但对于笔记本用户来说可能是值得的，最多可节省约 10 MB 的磁盘空间。
...
相比于 1.1.5，2.0 版本中的无盘启动功能得到了显著改进。启动程序位于 `src/sys/i386/boot/netboot` 中，可以从 MSDOS 系统运行，也可以烧录到 EPROM 中。本地 swap 也成为可能。目前支持 WD、SMC、3COM 和 Novell 的以太网卡。
...
内核现在能更好地记录哪些设备驱动正在运行以及设备的连接情况。用户程序可通过新的管理接口 **sysctl(3)** 获取这些信息。目前应用包括：
...
通过 4.4-Lite，我们拥有了一个更好的管理接口，能够处理之前通过读取和写入 **/dev/kmem** 操作的内核变量和参数。许多程序已经改写以使用该接口，但仍有许多旧程序未改动。一些之前无法访问的变量现在可通过 **sysctl(1)** 程序获取。除了标准的 4.4BSD MIB 变量外，我们还新增了以下支持：
...
FreeBSD 现在支持运行 iBCS2 兼容的二进制文件（目前支持 SCO UNIX 3.2.2 和 3.2.4，以及 ISC 2.2 COFF 格式）。尽管 iBCS2 模拟器仍处于早期阶段，但其功能已经可用。由于缺少商业应用，我们无法进行详尽测试，但几乎所有 SCO 3.2.2 二进制文件都可以正常运行，例如旧版 INFORMIX-2.10 for SCO。需要进一步测试来完成此项目。此外，还在进行 ELF 和 XOUT 加载器的开发，大部分 svr4 系统调用包装器已经完成。
...
支持的 SCSI 控制器完全支持 SCSI-I 和 SCSI-II 外设，包括磁盘、磁带机（包括 DAT）和 CD-ROM 驱动器。**注意**：目前，CD-ROM 驱动器只能通过此类 SCSI 控制器或 Mitsumi CD-ROM 接口卡（`mcd` 驱动）连接到 FreeBSD 系统。我们不支持 SoundBlaster（非 SCSI）CD-ROM 接口或其他“非 SCSI”适配器。ProAudio Spectrum SCSI 和 SoundBlaster SCSI 控制器是受支持的。
...
在需要时，系统将使用“回弹缓冲区”（bounce buffers）与控制器通信，从而仍然可以轻松使用超过 16MB 的内存。
...
镜像站点信息，请参阅文件 **MIRROR.SITES**，并从网络最近的站点下载。

如果没有互联网访问权限，可以通过电子邮件发送请求至 `ftpmail@decwrl.dec.com` 获取文件。在邮件正文中输入关键词“help”以获取更多关于从 freebsd.cdrom.com 下载文件的信息。**注意**：此方法可能会通过邮件产生大量（数十兆字节）数据，仅作为最后手段使用！
...
也可通过互联网发邮件至 `orders@cdrom.com` 订购。其最新目录可通过 FTP 获取，地址为 `ftp.cdrom.com:/cdrom/catalog`。

光盘售价为 $39.95。运费按每笔订单（而非每张光盘）计算，美国、加拿大和墨西哥 $5，其他国家 $10。支持 Visa、Mastercard、American Express，并提供美国货到付款服务。加州居民需加 8.25% 销售税。

光盘可无条件退货，但 Walnut Creek CDROM 不提供技术支持，技术问题请联系 FreeBSD 团队（详见“报告问题、提供建议、提交代码”一节）。
...
你需要 **10 张 1.44MB 软盘** 或 **12 张 1.2MB 软盘** 来存储 **bindist**（二进制发行版）。这些软盘 **必须** 使用 MS-DOS 格式化，可以通过 MS-DOS 的命令 `FORMAT` 或 Microsoft Windows 的文件管理器来完成（当然，如果是出厂预格式化的软盘且未被用于其他用途，也可以直接使用）。

格式化软盘后，需要将文件复制到软盘中。bindist 包含 **56 个文件**，此外还有三个小文件（`CKSUMS`、`do_cksum.sh` 和 `extract.sh`）供安装程序使用。这些文件 **都必须** 被复制到软盘上。bindist 的文件名为 `bindist.??`，其中 `??` 是从 `aa` 到 `cd` 的字母序列。将这些文件依次复制到软盘中，并将三个小型安装文件复制到最后一张软盘中。文件的复制顺序并不重要，但按照字母顺序会更容易给软盘贴标签。
...
如果从 MS-DOS 分区安装，只需将发行版文件复制到一个与发行版同名的目录下。例如，要准备安装 **bindist**，则在 C 盘上创建目录 **C:\BINDIST**，并将文件复制到该目录中。这样安装程序就会自动找到这些文件。

### 3. QIC/SCSI 磁带安装

从磁带安装是最简单的安装方式之一，仅次于在线使用 FTP 或从 CD-ROM 安装。安装程序要求文件以 tar 格式写入磁带。在获取所有相关的发行版文件后，在仅包含这些文件的目录中，可以使用如下命令将其写入磁带：
...
你的建议、错误报告和代码贡献对我们非常重要——如果发现任何问题，请随时报告（如果可以，请附上修复方案！）。

如果你的机器可以通过互联网发送邮件，建议使用命令 `send-pr` 提交错误报告。你提交的错误报告会由我们忠实的错误归档程序认真归档，我们会尽力尽快回应所有报告的问题。

如果因为某种原因无法使用 `send-pr` 提交错误报告，也可以将问题发送到：`bugs@FreeBSD.org`

如果你有任何问题或建议，可以发送邮件到：`questions@FreeBSD.org`

此外，由于这是一个志愿者项目，我们始终欢迎额外的帮助——目前需要完成的增强功能远远超过我们能够独自完成的范围！如需讨论技术问题或提供帮助，请发送邮件至：`hackers@FreeBSD.org`

由于这些邮件列表的流量可能很大，如果你的邮件访问速度慢或费用高昂，而只想关注 FreeBSD 的重要事件，可以选择订阅：`announce@FreeBSD.org`
...
FreeBSD 是全球众多开发者共同努力的结晶，这些人付出了巨大的努力，才有了这一版本的问世。虽然要列举出所有为 FreeBSD 贡献的人几乎是不可能的，但我们仍然希望尽可能地做到（当然，是按字母顺序排列）。如果你的名字未被提及，请放心，这纯属意外遗漏。

### 致谢名单

**加州大学伯克利分校 (U.C. Berkeley) 计算机系统研究组 (CSRG)**

**Bill Jolitz**，他在 386BSD 上做了大量工作。
...
以及 **蒙大拿州立大学 (Montana State University)** 的所有成员，给予了最初的支持。

感谢所有人，尤其是那些未被提及的朋友！我们真诚希望你能喜爱这个版本的 FreeBSD！
