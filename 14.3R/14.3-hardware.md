# FreeBSD 14.3-RELEASE 硬件兼容列表

## 引言

本硬件兼容性说明包含 FreeBSD 14.3 的硬件兼容性信息。其中列出了 FreeBSD 支持的硬件平台以及各类硬件设备（存储控制器、网络接口等），并给出已知可正常工作的设备实例。

## 支持的处理器和系统板

本节提供各架构所支持的特定处理器和系统的架构相关信息。

### 64 位 x86（amd64）架构支持

FreeBSD 支持 AMD64（“Hammer”）和 Intel EM64T 架构。AMD64（也称为 x86-64）是受完全支持的 Tier 1 架构。AMD64 支持所有现代 x86 处理器。本发行版整合了多项优化和增强功能，以充分发挥基于 FreeBSD/amd64 系统的全部能力，在各种工作负载下提供更佳性能。

### 32 位 x86（i386）架构支持

虽然 FreeBSD 持续演进并为现代架构进行优化，我们仍在本发行版中致力于维护对老旧 i386（x86）架构的支持。尽管 i386 硬件日益少见，但对于仍依赖它的用户，无论是用于遗留系统还是特定用途，FreeBSD 仍是可靠且通用的选择。

### 64 位 ARM（aarch64）架构支持

FreeBSD 将 64 位 ARM（又称 arm64 或 AArch64）作为 Tier-1 架构支持。
...
Allwinner (64-bit)，更多信息请参阅 [Allwinner Wiki page](https://wiki.freebsd.org/arm/Allwinner)：
...
树莓派，更多信息请参阅 [树莓派 Wiki page](https://wiki.freebsd.org/arm/Raspberry%20Pi)：

* 树莓派 3
* 树莓派 4

RockChip (64-bit)，更多信息请参阅 [RockChip Wiki page](https://wiki.freebsd.org/arm/RockChip)：
...
### 32 位 ARM（armv6、armv7）架构支持

FreeBSD 目前支持以下设备：

Allwinner (32-bit)，更多信息请参阅 [Allwinner Wiki page](https://wiki.freebsd.org/arm/Allwinner)：
...
NXP i.MX6 (32-bit)，更多信息请参阅 [NXP i.MX6 Wiki page](https://wiki.freebsd.org/arm/imx6)：
...
FreeBSD 对 RISC-V 的支持标志着这一开源操作系统发展中的重要里程碑。FreeBSD 是首个内置支持 RISC-V 引导的操作系统，体现了其对新兴硬件架构的承诺。
...
更多信息请参阅 [RISC-V Wiki page](https://wiki.freebsd.org/riscv)。

### PowerPC 架构支持

支持许多 PowerPC 平台，包括但不限于：

* Apple PowerMac G4 及更高版本。
* IBM POWER4 及更高版本（使用 PAPR）
* IBM POWER8 及更高版本（裸机 powernv）
** 多数开发使用 Raptor TALOS-II 完成
* 嵌入式 NXP QorIQ 和 PowerQUICC III（MPC85XX、Pxxx）
** 已在 AmigaOne X5000 和 AmigaOne A1222 上测试

## 支持的设备

本节介绍 FreeBSD 当前已知支持的设备。其他配置也可能可用，但尚未经过测试。欢迎对本列表提出反馈、更新和修正。

在可能的情况下，此处列出适用于各设备或设备类的驱动。如果相关驱动在 FreeBSD 基本系统中拥有手册页（多数驱动都有），则在此处给出引用。关于所支持的设备、控制器等的具体型号等信息，可在手册页中查阅。
...
[ahci(4)](https://man.freebsd.org/cgi/man.cgi?query=ahci&sektion=4&format=html) 驱动还支持作为 PCI 桥接 nvme(4) 的 AHCI 设备，使用 Intel Rapid Storage Technology（RST）。要使用 nvme(4) 设备，必须在 BIOS 中将 SATA 模式设置为 AHCI（从 RST 切换），或者接受因中断共享导致 RST 启用时的性能。FreeBSD 会自动检测处于 RST 模式且具有此扩展的 AHCI 设备。当检测到时，[ahci(4)](https://man.freebsd.org/cgi/man.cgi?query=ahci&sektion=4&format=html) will attach nvme(4) children to the [ahci(4)](https://man.freebsd.org/cgi/man.cgi?query=ahci&sektion=4&format=html) device.

[ahd(4)](https://man.freebsd.org/cgi/man.cgi?query=ahd&sektion=4&format=html) 驱动支持以下：
...
上述部分芯片可配置为 AHCI 模式。在此情况下，它们由 ahci(4) 驱动替代。
...
[hptiop(4)](https://man.freebsd.org/cgi/man.cgi?query=hptiop&sektion=4&format=html) 驱动还支持以下 SAS 和 SATA RAID 控制器，这些控制器 are already End-of-Life:
...
更新的 ServeRAID 控制器由 aac(4) or mfi(4) driver.

[isci(4)](https://man.freebsd.org/cgi/man.cgi?query=isci&sektion=4&format=html) 驱动为 Intel C600 提供支持 `SAS` controllers.

[isp(4)](https://man.freebsd.org/cgi/man.cgi?query=isp&sektion=4&format=html) 驱动支持以下光纤通道光适配器：
...
支持所有主要固件版本（2.x、3.x、4.x 和 5.x），但是 it is always advisable to upgrade to the most recent firmware available for the controller.
...
[mpi3mr(4)](https://man.freebsd.org/cgi/man.cgi?query=mpi3mr&sektion=4&format=html) 驱动支持以下控制器：
...
[mpt(4)](https://man.freebsd.org/cgi/man.cgi?query=mpt&sektion=4&format=html) 可在许多系统的板载中找到，包括：

*	Dell PowerEdge 1750 through 2850
*	IBM eServer xSeries 335

这些系统还包含集成 RAID 镜像和集成 RAID 镜像增强功能，此驱动也支持。

`SAS` 控制器芯片也出现在许多新的 AMD/Opteron 系统上，例如 Sun 4100。注意，此控制器可同时驱动 SAS 和 SATA 驱动器或其混合。集成 RAID 镜像可用于这些控制器的支持充其量是有限的。

`Fibre Channel` 控制器芯片组支持多种速度和系统。`Apple` 光纤通道 HBA 实际上是 `FC949ES` 卡。

此驱动还支持光纤通道卡的目标模式。可通过 LSI Logic 固件实用程序设置核心的所需角色来启用此支持，该实用程序确定卡可以承担的角色——无需单独编译。
...
注意，此硬件仅对 ATA DMA 命令支持命令队列和基于 FIS 的交换。ATAPI 和非 DMA ATA 命令按端口逐条执行。

[ocs_fc(4)](https://man.freebsd.org/cgi/man.cgi?query=ocs_fc&sektion=4&format=html) 驱动支持以下光纤通道适配器：
...
[pst(4)](https://man.freebsd.org/cgi/man.cgi?query=pst&sektion=4&format=html) 驱动支持 Promise Supertrak SX6000 ATA 硬件 RAID controller.
...
[smartpqi(4)](https://man.freebsd.org/cgi/man.cgi?query=smartpqi&sektion=4&format=html) 驱动支持以下控制器（包括但不限于）：
...
[sym(4)](https://man.freebsd.org/cgi/man.cgi?query=sym&sektion=4&format=html) 驱动为以下 Symbios/LSI Logic PCI SCSI controllers:
...
[sym(4)](https://man.freebsd.org/cgi/man.cgi?query=sym&sektion=4&format=html) 驱动的 SCSI 控制器可以板载集成，也可以是以下附加卡：
...
对于所有受支持的 SCSI 控制器，均提供对 SCSI-I、SCSI-II 和 SCSI-III 外设的完整支持，包括硬盘、光盘、磁带驱动器（包括 DAT、8mm Exabyte、Mammoth 和 DLT）、介质更换器、处理器目标设备和 CD-ROM 驱动器。支持 CD-ROM 命令的 WORM 设备可由 CD-ROM 驱动（如 [cd(4)](https://man.freebsd.org/cgi/man.cgi?query=cd&sektion=4&format=html)）以只读方式访问。WORM/CD-R/CD-RW 写入支持由 [cdrecord(1)](https://man.freebsd.org/cgi/man.cgi?query=cdrecord&sektion=1&format=html) 提供，该工具是 Ports 中 [sysutils/cdrtools](http://www.freebsd.org/cgi/url.cgi?ports/sysutils/cdrtools/pkg-descr) port 的一部分。
...
[ae(4)](https://man.freebsd.org/cgi/man.cgi?query=ae&sektion=4&format=html) 驱动支持 Attansic/Atheros L2 PCIe 快速以太网控制器，并且已知支持以下硬件：

*	ASUS EeePC 701
*	ASUS EeePC 900

其他硬件可能可以也可能不与此驱动配合工作。

[age(4)](https://man.freebsd.org/cgi/man.cgi?query=age&sektion=4&format=html) 驱动为基于 Attansic/Atheros L1 Gigabit Ethernet 控制器芯片，包括：
...
[alc(4)](https://man.freebsd.org/cgi/man.cgi?query=alc&sektion=4&format=html) 设备驱动为以下以太网控制器提供支持：
...
当前版本的 [atse(4)](https://man.freebsd.org/cgi/man.cgi?query=atse&sektion=4&format=html) 驱动支持 Ethernet MegaCore，如版本 11.1 的 Altera’s documentation when the device is configured with internal FIFOs.

[aue(4)](https://man.freebsd.org/cgi/man.cgi?query=aue&sektion=4&format=html) 驱动支持以下适配器：
...
[axe(4)](https://man.freebsd.org/cgi/man.cgi?query=axe&sektion=4&format=html) 驱动支持 ASIX Electronics AX88172/AX88178/AX88772/AX88772A/AX88772B/AX88760 based USB Ethernet adapters including:
...
[bce(4)](https://man.freebsd.org/cgi/man.cgi?query=bce&sektion=4&format=html) 驱动为基于 QLogic NetXtreme II family of Gigabit Ethernet controllers, including the following:
...
[bge(4)](https://man.freebsd.org/cgi/man.cgi?query=bge&sektion=4&format=html) 驱动为基于 Broadcom BCM570x family of Gigabit Ethernet controller chips, including the following:
...
[bnxt(4)](https://man.freebsd.org/cgi/man.cgi?query=bnxt&sektion=4&format=html) 驱动为基于 Broadcom NetXtreme-C 和 NetXtreme-E families of Gigabit Ethernet controller chips, including the following:
...
[bxe(4)](https://man.freebsd.org/cgi/man.cgi?query=bxe&sektion=4&format=html) 驱动为基于 QLogic NetXtreme II family of 10Gb Ethernet controller chips, including the following:
...
以下芯片由 [cas(4)](https://man.freebsd.org/cgi/man.cgi?query=cas&sektion=4&format=html) 驱动：

*	National Semiconductor DP83065 Saturn Gigabit Ethernet
*	Sun Cassini Gigabit Ethernet
*	Sun Cassini+ Gigabit Ethernet

[cas(4)](https://man.freebsd.org/cgi/man.cgi?query=cas&sektion=4&format=html) 驱动目前支持以下附加卡：
...
[cue(4)](https://man.freebsd.org/cgi/man.cgi?query=cue&sektion=4&format=html) 驱动支持基于 CATC USB-EL1210A 的 USB 以太网 adapters including:
...
[cxgbev(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbev&sektion=4&format=html) 驱动支持 100Gb 和 25Gb 以太网适配器上的 Virtual Functions based on the T6 ASIC:
...
[cxgbev(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbev&sektion=4&format=html) 驱动支持 40Gb、10Gb 和 1Gb 以太网适配器上的 Virtual Functions based on the T5 ASIC:
...
[cxgbev(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbev&sektion=4&format=html) 驱动支持基于 10Gb 和 1Gb 以太网适配器的 Virtual Functions on the T4 ASIC:
...
[dc(4)](https://man.freebsd.org/cgi/man.cgi?query=dc&sektion=4&format=html) 驱动目前支持以下网卡：
...
[dtsec(4)](https://man.freebsd.org/cgi/man.cgi?query=dtsec&sektion=4&format=html) 驱动已知支持内置在以下 Freescale 片上系统设备中的千兆以太网控制器：
...
[em(4)](https://man.freebsd.org/cgi/man.cgi?query=em&sektion=4&format=html) 驱动支持基于 Intel 的千兆以太网适配器 82540, 82541ER, 82541PI, 82542, 82543, 82544, 82545, 82546, 82546EB, 82546GB, 82547, 82571, 82572, 82573, 82574, 82575, 82576, and 82580 controller chips:
...
FreeBSD kernel driver for Elastic Network Adapter (ENA) family ([ena(4)](https://man.freebsd.org/cgi/man.cgi?query=ena&sektion=4&format=html) driver)

[enic(4)](https://man.freebsd.org/cgi/man.cgi?query=enic&sektion=4&format=html) 驱动支持所有已知的 Cisco VIC 卡。

Agere ET1310 Gigabit Ethernet adapters ([et(4)](https://man.freebsd.org/cgi/man.cgi?query=et&sektion=4&format=html) driver)

[fxp(4)](https://man.freebsd.org/cgi/man.cgi?query=fxp&sektion=4&format=html) 驱动支持以下适配器：
...
[gem(4)](https://man.freebsd.org/cgi/man.cgi?query=gem&sektion=4&format=html) 驱动支持以下芯片：

*	Apple GMAC
*	Sun GEM Gigabit Ethernet

[gem(4)](https://man.freebsd.org/cgi/man.cgi?query=gem&sektion=4&format=html) 驱动目前支持以下附加卡：

*	Sun Gigabit Ethernet PCI 2.0/3.0 (GBE/P) (part no. 501-4373)

[genet(4)](https://man.freebsd.org/cgi/man.cgi?query=genet&sektion=4&format=html) 驱动支持 Broadcom BCM2711 的以太网控制器部分 on the 树莓派 4 Model B and related systems. 它采用 BCM54213PE PHY。
...
[ice(4)](https://man.freebsd.org/cgi/man.cgi?query=ice&sektion=4&format=html) 驱动支持 Intel Ethernet 800 系列。此系列中带 SFP28/QSFP28 接口的大多数适配器 have firmware that requires that Intel qualified modules are used; these qualified modules are listed below. 此限定检查无法由驱动禁用。

[ice(4)](https://man.freebsd.org/cgi/man.cgi?query=ice&sektion=4&format=html) 驱动支持带以下 QSFP28 模块的 100Gb 以太网适配器：

[igc(4)](https://man.freebsd.org/cgi/man.cgi?query=igc&sektion=4&format=html) 驱动支持以下型号：
...
[ix(4)](https://man.freebsd.org/cgi/man.cgi?query=ix&sektion=4&format=html) 驱动支持 Intel 10Gb 以太网 PCIe 适配器，包括：
...
[ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 驱动支持 Intel Ethernet 700 系列。此系列中带 SFP+/SFP28/QSFP+ 接口的大多数适配器 have firmware that requires that Intel qualified modules are used; these qualified modules are listed below. 此限定检查无法由驱动禁用。

[ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 驱动支持带以下 QSFP+ 模块的 40Gb 以太网适配器：

*	Intel 4x10G/40G QSFP+ 40GBASE-SR4 E40GQSFPSR
*	Intel 4x10G/40G QSFP+ 40GBASE-LR4 E40GQSFPLR

[ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 驱动支持带以下 SFP28 模块的 25Gb 以太网适配器：

*	Intel 10G/25G SFP28 25GBASE-SR E25GSFP28SR
*	Intel 10G/25G SFP28 25GBASE-SR E25GSFP28SRX (Extended Temp)

[ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 驱动支持带以下 SFP+ 模块的 25Gb 和 10Gb 以太网适配器：
...
注意，适配器还支持所有无源和有源 limiting direct attach cables that comply with SFF-8431 v4.1 and SFF-8472 v10.4 specifications.

此列表并非详尽无遗；请查阅产品文档以获取 up-to-date list of supported media.
...
[kue(4)](https://man.freebsd.org/cgi/man.cgi?query=kue&sektion=4&format=html) 驱动支持基于 Kawasaki LSI KL5KLUSB101B 的 USB 以太网 adapters including:
...
[lge(4)](https://man.freebsd.org/cgi/man.cgi?query=lge&sektion=4&format=html) 驱动支持以下适配器：

*	SMC TigerCard 1000 (SMC9462SX)
*	D-Link DGE-500SX

[liquidio(4)](https://man.freebsd.org/cgi/man.cgi?query=liquidio&sektion=4&format=html) 驱动支持以下卡：

*	LiquidIO II CN2350 210SV/225SV
*	LiquidIO II CN2360 210SV/225SV

[mgb(4)](https://man.freebsd.org/cgi/man.cgi?query=mgb&sektion=4&format=html) 驱动支持：
...
[mlx4ib(4)](https://man.freebsd.org/cgi/man.cgi?query=mlx4ib&sektion=4&format=html) 驱动支持以下网络适配器：
...
[mlx5ib(4)](https://man.freebsd.org/cgi/man.cgi?query=mlx5ib&sektion=4&format=html) 驱动支持 100Gb、50Gb、40Gb、25Gb 和 10Gb 网络适配器。ConnectX-4 supports: 10/20/25/40/50/56/100Gb/s speeds. ConnectX-4 LX supports: 10/25/40/50Gb/s speeds (and reduced power consumption):
...
[mos(4)](https://man.freebsd.org/cgi/man.cgi?query=mos&sektion=4&format=html) 驱动支持以下适配器：

*	Sitecom LN030

[msk(4)](https://man.freebsd.org/cgi/man.cgi?query=msk&sektion=4&format=html) 驱动为基于 Marvell/SysKonnect 的各种 NIC 提供支持 Yukon II based Gigabit Ethernet 控制器芯片，包括：
...
[muge(4)](https://man.freebsd.org/cgi/man.cgi?query=muge&sektion=4&format=html) 驱动支持：
...
[mxge(4)](https://man.freebsd.org/cgi/man.cgi?query=mxge&sektion=4&format=html) 驱动支持基于 Myricom LANai Z8E chips:
...
[nge(4)](https://man.freebsd.org/cgi/man.cgi?query=nge&sektion=4&format=html) 驱动支持基于 National Semiconductor DP83820 和 DP83821 的 Gigabit Ethernet adapters including:
...
[qlnxe(4)](https://man.freebsd.org/cgi/man.cgi?query=qlnxe&sektion=4&format=html) 驱动支持基于以下的 25/40/100 千兆以太网和 CNA 适配器 chipsets:

*	QLogic 45000 series
*	QLogic 41000 series

[qlxgb(4)](https://man.freebsd.org/cgi/man.cgi?query=qlxgb&sektion=4&format=html) 驱动支持基于以下的 10 千兆以太网和 CNA 适配器 chipsets:

*	QLogic 3200 series
*	QLogic 8200 series

[qlxgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=qlxgbe&sektion=4&format=html) 驱动支持基于以下的 10 千兆以太网和 CNA 适配器 chipsets:

*	QLogic 8300 series

[qlxge(4)](https://man.freebsd.org/cgi/man.cgi?query=qlxge&sektion=4&format=html) 驱动支持基于以下的 10 千兆以太网和 CNA 适配器 chipsets:

*	QLogic 8100 series

[re(4)](https://man.freebsd.org/cgi/man.cgi?query=re&sektion=4&format=html) 驱动支持 RealTek RTL8139C+、RTL8169、RTL816xS、RTL811xS、RTL8168、RTL810xE and RTL8111 based Fast Ethernet and Gigabit Ethernet adapters including:
...
[rl(4)](https://man.freebsd.org/cgi/man.cgi?query=rl&sektion=4&format=html) 驱动支持以下适配器：
...
[rue(4)](https://man.freebsd.org/cgi/man.cgi?query=rue&sektion=4&format=html) 驱动支持基于 RealTek RTL8150 的 USB 以太网 adapters including:
...
[sfxge(4)](https://man.freebsd.org/cgi/man.cgi?query=sfxge&sektion=4&format=html) 驱动支持所有基于 Solarflare SFC9000 的 10Gb 以太网适配器 family controllers.
...
[sis(4)](https://man.freebsd.org/cgi/man.cgi?query=sis&sektion=4&format=html) 驱动支持基于 Silicon Integrated Systems SiS 900 和 SiS 7016 的 Fast Ethernet adapters and embedded controllers, as well as Fast Ethernet adapters based on the National Semiconductor DP83815 (MacPhyter) and DP83816 chips. 支持的适配器包括：
...
[sk(4)](https://man.freebsd.org/cgi/man.cgi?query=sk&sektion=4&format=html) 驱动支持以下适配器：
...
[ste(4)](https://man.freebsd.org/cgi/man.cgi?query=ste&sektion=4&format=html) 驱动支持基于 Sundance Technologies ST201 的快速以太网 adapters and embedded controllers including:

*	D-Link DFE-530TXS
*	D-Link DFE-550TX
*	D-Link DFE-580TX

[stge(4)](https://man.freebsd.org/cgi/man.cgi?query=stge&sektion=4&format=html) 驱动为基于 Sundance/Tamarack 的各种 NIC 提供支持 TC9021 based Gigabit Ethernet 控制器芯片，包括：
...
[ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动支持基于 Alteon Tigon I and II chips. [ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动已在以下适配器上测试：
...
[tsec(4)](https://man.freebsd.org/cgi/man.cgi?query=tsec&sektion=4&format=html) 驱动已知支持内置在以下 Freescale 片上系统设备中的千兆以太网控制器：

*	MPC8349
*	MPC8533, MPC8541, MPC8555

控制器的增强版本（eTSEC），集成在以下 devices, is also supported by this driver:
...
[ufoma(4)](https://man.freebsd.org/cgi/man.cgi?query=ufoma&sektion=4&format=html) 驱动支持以下设备：

*	SHARP FOMA SH902i
*	KYOCERA PHS AH-K3001V (a.k.a Kyopon)
*	SANYO Vodafone3G V801SA

[uhso(4)](https://man.freebsd.org/cgi/man.cgi?query=uhso&sektion=4&format=html) 驱动应可与 Option 的大多数设备配合工作。以下设备已验证可工作：
...
该设备具有一个称为 “Zero-CD” which contains drivers for Microsoft Windows; this is the default mode for the device. [uhso(4)](https://man.freebsd.org/cgi/man.cgi?query=uhso&sektion=4&format=html) 驱动自动将设备从 “Zero-CD” mode to modem mode. 此行为可通过设置 *hw.usb.uhso.auto_switch* to 0 using sysctl(8).

[ure(4)](https://man.freebsd.org/cgi/man.cgi?query=ure&sektion=4&format=html) 驱动支持以下 USB 以太网控制器：
...
[urndis(4)](https://man.freebsd.org/cgi/man.cgi?query=urndis&sektion=4&format=html) 驱动支持 “tethering” functionality of many Android devices.

[vge(4)](https://man.freebsd.org/cgi/man.cgi?query=vge&sektion=4&format=html) 驱动支持基于 VIA Networking VT6120、VT6122、VT6130 和 VT6132 的 Gigabit Ethernet adapters including:

*	VIA Networking LAN-on-motherboard Gigabit Ethernet
*	ZyXEL GN650-T 64-bit PCI Gigabit Ethernet NIC (ZX1701)
*	ZyXEL GN670-T 32-bit PCI Gigabit Ethernet NIC (ZX1702)

[vr(4)](https://man.freebsd.org/cgi/man.cgi?query=vr&sektion=4&format=html) 驱动支持基于 VIA Technologies Rhine I、Rhine II 和 Rhine III 的 Fast Ethernet adapters including:
...
3C656 系列的 CardBus 卡和 3C556 系列的 MiniPCI 卡均内置了专有调制解调器。[xl(4)](https://man.freebsd.org/cgi/man.cgi?query=xl&sektion=4&format=html) 驱动和任何其他 FreeBSD 驱动都不支持此调制解调器。

### 以太网交换机

[adm6996fc(4)](https://man.freebsd.org/cgi/man.cgi?query=adm6996fc&sektion=4&format=html) 驱动支持 Infineon ADM6996FC 快速以太网交换机控制器。

[arswitch(4)](https://man.freebsd.org/cgi/man.cgi?query=arswitch&sektion=4&format=html) 驱动支持以下以太网交换控制器：
...
[e6000sw(4)](https://man.freebsd.org/cgi/man.cgi?query=e6000sw&sektion=4&format=html) 驱动支持以下千兆以太网交换控制器：
...
[e6060sw(4)](https://man.freebsd.org/cgi/man.cgi?query=e6060sw&sektion=4&format=html) 驱动支持以下快速以太网交换控制器：
...
[ip17x(4)](https://man.freebsd.org/cgi/man.cgi?query=ip17x&sektion=4&format=html) 驱动支持以下快速以太网交换控制器：
...
[mtkswitch(4)](https://man.freebsd.org/cgi/man.cgi?query=mtkswitch&sektion=4&format=html) 驱动支持以下以太网交换控制器：
...
[ath(4)](https://man.freebsd.org/cgi/man.cgi?query=ath&sektion=4&format=html) 驱动支持所有 Atheros Cardbus、ExpressCard、PCI 和 PCIe 卡，except those that are based on the AR5005VL chipset.

[ath_hal(4)](https://man.freebsd.org/cgi/man.cgi?query=ath_hal&sektion=4&format=html) 模块支持以下卡（包括但不限于）：
...
使用较旧 Broadcom 芯片组（BCM4301、BCM4303 和 BCM4306 rev 2）的用户 must use bwi(4) because the v4 version of the firmware does not support these chips. 较新的固件过大，无法放入这些旧芯片。

[ipw(4)](https://man.freebsd.org/cgi/man.cgi?query=ipw&sektion=4&format=html) 驱动为 Intel PRO/Wireless 2100a/b MiniPCI network adapter.

[iwi(4)](https://man.freebsd.org/cgi/man.cgi?query=iwi&sektion=4&format=html) 驱动支持以下无线网络设备：
...
[iwlwifi(4)](https://man.freebsd.org/cgi/man.cgi?query=iwlwifi&sektion=4&format=html) 驱动支持来自 [mvm(4)](https://man.freebsd.org/cgi/man.cgi?query=mvm&sektion=4&format=html) sub-driver with the following chipset generations:
...
[iwlwifi(4)](https://man.freebsd.org/cgi/man.cgi?query=iwlwifi&sektion=4&format=html) 驱动支持来自 [mld(4)](https://man.freebsd.org/cgi/man.cgi?query=mld&sektion=4&format=html) sub-driver with the following chipset generations:
...
*	Killer Wireless-AC 1550s Wireless Network Adapter (9560NGW)
...
*	Killer Wi-Fi 6 AX1650s 160MHz Wireless Network Adapter (201NGW)
*	Killer Wi-Fi 6 AX1650i 160MHz Wireless Network Adapter (201D2W)
...
*	Killer Wi-Fi 6E AX1675s 160MHz Wireless Network Adapter (211NGW)
...
*	Intel Wireless-AC 9270 160MHz
*	Intel Wireless-AC 9270
*	Intel Wireless-AC 9162 160MHz
*	Intel Wireless-AC 9162
*	Intel Wireless-AC 9260 160MHz
*	Intel Wireless-AC 9260
*	Intel Wi-Fi 6 AX101
*	Intel Wi-Fi 6 AX203
*	Intel Wi-Fi 6E AX231 160MHz
*	Intel Wi-Fi 7 BE201 320MHz
*	Intel Wi-Fi 7 BE200 320MHz
*	Intel Wi-Fi 7 BE202 160MHz
*	Intel TBD Sc device
*	Intel TBD Sc2 device
*	Intel TBD Sc2f device

[iwm(4)](https://man.freebsd.org/cgi/man.cgi?query=iwm&sektion=4&format=html) 驱动支持 following PCIe Wi-Fi devices:
...
[iwn(4)](https://man.freebsd.org/cgi/man.cgi?query=iwn&sektion=4&format=html) 驱动为以下提供支持：
...
[malo(4)](https://man.freebsd.org/cgi/man.cgi?query=malo&sektion=4&format=html) 驱动支持以下卡（包括但不限于）：
...
[otus(4)](https://man.freebsd.org/cgi/man.cgi?query=otus&sektion=4&format=html) 驱动为 Atheros AR9170 USB IEEE 802.11b/g/n 提供支持 wireless network adapters, including:
...
[ral(4)](https://man.freebsd.org/cgi/man.cgi?query=ral&sektion=4&format=html) 驱动支持基于 Ralink Technology 的 PCI/PCIe/CardBus 无线适配器 chipsets, including:
...
[rsu(4)](https://man.freebsd.org/cgi/man.cgi?query=rsu&sektion=4&format=html) 驱动为 Realtek RTL8188SU/RTL8192SU USB IEEE 802.11b/g/n 提供支持 wireless network adapters, including:
...
[rtw88(4)](https://man.freebsd.org/cgi/man.cgi?query=rtw88&sektion=4&format=html) 驱动支持带以下芯片组的 PCIe 设备：
...
[rtw89(4)](https://man.freebsd.org/cgi/man.cgi?query=rtw89&sektion=4&format=html) 驱动支持带以下芯片组的 PCIe 设备：
...
[rtwn_pci(4)](https://man.freebsd.org/cgi/man.cgi?query=rtwn_pci&sektion=4&format=html) 驱动支持 following PCIe Wi-Fi devices:

*	Realtek 802.11n wireless 8188  (RTL8188EE)
*	Realtek 802.11n wireless 8192  (RTL8192CE)

[rtwn_usb(4)](https://man.freebsd.org/cgi/man.cgi?query=rtwn_usb&sektion=4&format=html) 驱动支持基于特定 Realtek RTL 8188/8192/8812 and 8821 chipsets, including:
...
[rum(4)](https://man.freebsd.org/cgi/man.cgi?query=rum&sektion=4&format=html) 驱动支持 USB 2.0 无线 adapters based on the Ralink RT2501USB and RT2601USB chipsets, 包括：
...
[run(4)](https://man.freebsd.org/cgi/man.cgi?query=run&sektion=4&format=html) 驱动支持 following wireless adapters:
...
[uath(4)](https://man.freebsd.org/cgi/man.cgi?query=uath&sektion=4&format=html) 驱动应可与以下适配器配合工作：
...
[upgt(4)](https://man.freebsd.org/cgi/man.cgi?query=upgt&sektion=4&format=html) 驱动支持 USB 2.0 Conexant/Intersil PrismGT 系列无线 adapters based on the GW3887 chipset, among them:
...
[ural(4)](https://man.freebsd.org/cgi/man.cgi?query=ural&sektion=4&format=html) 驱动支持 USB 2.0 无线 adapters based on the Ralink Technology RT2500USB chipset, including:
...
[wpi(4)](https://man.freebsd.org/cgi/man.cgi?query=wpi&sektion=4&format=html) 驱动为 Intel PRO/Wireless 3945ABG Mini PCIe network adapter.

[zyd(4)](https://man.freebsd.org/cgi/man.cgi?query=zyd&sektion=4&format=html) 驱动已知支持以下设备：
...
[u3g(4)](https://man.freebsd.org/cgi/man.cgi?query=u3g&sektion=4&format=html) 驱动支持 following cellular modems:
...
还有更多受支持的设备，请参阅 */sys/dev/usb/serial/u3g.c* for the complete list.

### 串行接口

核心组件和内核接口通过 hardware interface. 此接口作为硬件的抽象，允许不同的 UARTs to be used for serial communications.

[uart(4)](https://man.freebsd.org/cgi/man.cgi?query=uart&sektion=4&format=html) 驱动支持 following classes of UARTs:
...
[scc(4)](https://man.freebsd.org/cgi/man.cgi?query=scc&sektion=4&format=html) 驱动支持 following classes of SCCs:

*	QUICC: Freescale/NXP QUad Integrated Communications Controllers.
*	Z8530: Zilog 8530 based serial communications controllers.

[puc(4)](https://man.freebsd.org/cgi/man.cgi?query=puc&sektion=4&format=html) 驱动支持 following PCI 多端口串行和并行适配器：
...
[uark(4)](https://man.freebsd.org/cgi/man.cgi?query=uark&sektion=4&format=html) 驱动支持以下适配器：
...
[uchcom(4)](https://man.freebsd.org/cgi/man.cgi?query=uchcom&sektion=4&format=html) 驱动支持以下适配器：

*	HL USB-RS232

[ucycom(4)](https://man.freebsd.org/cgi/man.cgi?query=ucycom&sektion=4&format=html) 驱动目前支持以下包含 Cypress USB to RS232 bridge chips:

*	DeLorme Earthmate USB GPS receiver

### 声音设备

[snd_als4000(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_als4000&sektion=4&format=html) 驱动支持 following sound cards:

*	Avance Logic ALS4000

[snd_atiixp(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_atiixp&sektion=4&format=html) 驱动支持 following audio chipsets:

*	ATI IXP 200
*	ATI IXP 300
*	ATI IXP 400

[snd_cmi(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_cmi&sektion=4&format=html) 驱动支持 following sound cards:
...
[snd_cs4281(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_cs4281&sektion=4&format=html) 驱动支持 following sound cards:

*	Crystal Semiconductor CS4281

[snd_csa(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_csa&sektion=4&format=html) 驱动支持 following sound cards:
...
部分板载 CS4610 芯片附带 CS423x ISA 编解码器 instead of the CS4297 AC97 codec. 此类配置不 supported by the [snd_csa(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_csa&sektion=4&format=html) 驱动。

[snd_emu10k1(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_emu10k1&sektion=4&format=html) 驱动支持 following sound cards:
...
[snd_emu10kx(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_emu10kx&sektion=4&format=html) 驱动支持 following sound cards:
...
[snd_emu10kx(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_emu10kx&sektion=4&format=html) 驱动 *not* support the following sound cards (although they have names similar to some supported ones):
...
[snd_envy24(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_envy24&sektion=4&format=html) 驱动支持 following audio devices:

*	M-Audio Audiophile 2496
*	M-Audio Delta Dio 2496
*	Terratec DMX 6fire

仅支持模拟播放。不支持这些卡的录音和其他功能。

[snd_envy24ht(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_envy24ht&sektion=4&format=html) 驱动支持 following audio devices:
...
[snd_es137x(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_es137x&sektion=4&format=html) 驱动支持 following sound cards:
...
[snd_hda(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hda&sektion=4&format=html) 驱动支持具有 PCI 类 4（多媒体）的控制器和 subclass 3 (HDA), compatible with Intel HDA specification.

[snd_hda(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hda&sektion=4&format=html) 驱动支持超过两百种不同的控制器和 CODEC。在此列出所有这些没有意义，因为在大多数情况下特定的 CODEC configuration and wiring are more important then type of the CODEC itself.

[snd_hdspe(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hdspe&sektion=4&format=html) 驱动支持 following audio devices:

*	RME HDSPe AIO
*	RME HDSPe RayDAT

默认情况下，每个 pcm(4) 设备对应声卡上的一个物理端口。ADAT 端口支持 8 声道、4 声道和 2 声道格式。根据所选的采样率和声道格式，并非所有 pcm 声道都能映射到 ADAT 声道，反之亦然。

[snd_ich(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_ich&sektion=4&format=html) 驱动支持 following audio devices:
...
[snd_maestro3(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_maestro3&sektion=4&format=html) 驱动支持 following audio devices:

*	ESS Technology Allegro-1
*	ESS Technology Maestro3

[snd_neomagic(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_neomagic&sektion=4&format=html) 驱动支持 following audio devices:

*	NeoMagic 256AV
*	NeoMagic 256ZX

[snd_solo(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_solo&sektion=4&format=html) 驱动支持 following sound cards:

*	ESS Solo-1 (ES1938 Chipset)
*	ESS Solo-1E (ES1946 Chipset)

[snd_spicds(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_spicds&sektion=4&format=html) 驱动支持 following codecs:
...
[snd_t4dwave(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_t4dwave&sektion=4&format=html) 驱动支持 following audio devices:
...
[snd_via8233(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_via8233&sektion=4&format=html) 驱动支持 following audio chipsets:
...
支持一系列 USB 外设；已知可工作的设备列于本节中。由于大多数 USB 设备的通用性，除部分例外，任何给定类别的设备都将受到支持，即使未在此明确列出。

> **注意**：
>
> USB 网络接口可在 [以太网接口](https://www.freebsd.org/releases/14.3R/hardware.html#ethernet) 或 [无线网络接口](https://www.freebsd.org/releases/14.3R/hardware.html#wlan) 中查阅。

> **注意**：
>
> USB 蓝牙适配器可在 [蓝牙](https://www.freebsd.org/releases/14.3R/hardware.html#bluetooth) 中查阅。

> **注意**：
>
> USB 人机接口设备可在 [其他设备](https://www.freebsd.org/releases/14.3R/hardware.html#misc) 中查阅。

[atp(4)](https://man.freebsd.org/cgi/man.cgi?query=atp&sektion=4&format=html) 驱动为 following Product IDs:
...
[atopcase(4)](https://man.freebsd.org/cgi/man.cgi?query=atopcase&sektion=4&format=html) 驱动支持 following MacBooks produced in 2015-2018 years:
...
[abtn(4)](https://man.freebsd.org/cgi/man.cgi?query=abtn&sektion=4&format=html) 驱动支持以下设备上的扩展键盘键（特殊 F 键）：

*	Apple iBook Keyboard
*	Apple PowerBook Keyboard

[akbd(4)](https://man.freebsd.org/cgi/man.cgi?query=akbd&sektion=4&format=html) 驱动支持以下设备：
...
[ams(4)](https://man.freebsd.org/cgi/man.cgi?query=ams&sektion=4&format=html) 驱动支持以下设备：
...
[ohci(4)](https://man.freebsd.org/cgi/man.cgi?query=ohci&sektion=4&format=html) 驱动支持所有符合 OHCI v1.0 标准的控制器，包括：
...
[ugold(4)](https://man.freebsd.org/cgi/man.cgi?query=ugold&sektion=4&format=html) 驱动支持以下设备：

*	RDing TEMPer1V1.2

[uhci(4)](https://man.freebsd.org/cgi/man.cgi?query=uhci&sektion=4&format=html) 驱动支持所有符合 UHCI v1.1 标准的控制器，包括：

*	Intel 82371AB/EB (PIIX4)
*	Intel 82371SB (PIIX3)
*	VIA 83C572

[uipaq(4)](https://man.freebsd.org/cgi/man.cgi?query=uipaq&sektion=4&format=html) 驱动支持 following iPAQ devices:
...
[urio(4)](https://man.freebsd.org/cgi/man.cgi?query=urio&sektion=4&format=html) 驱动支持以下设备：

*	Diamond MultiMedia Rio 500
*	Diamond MultiMedia Rio 600
*	Diamond MultiMedia Rio 800

[umodem(4)](https://man.freebsd.org/cgi/man.cgi?query=umodem&sektion=4&format=html) 驱动支持以下设备：
...
[rtsx(4)](https://man.freebsd.org/cgi/man.cgi?query=rtsx&sektion=4&format=html) 驱动支持不同规范的兼容芯片。以下芯片已验证可工作：
...
[ulpt(4)](https://man.freebsd.org/cgi/man.cgi?query=ulpt&sektion=4&format=html) 驱动为 USB 打印机和并行打印机 conversion cables, including the following:
...
[ubser(4)](https://man.freebsd.org/cgi/man.cgi?query=ubser&sektion=4&format=html) 驱动为 BWCT console management serial adapters.
...
[uplcom(4)](https://man.freebsd.org/cgi/man.cgi?query=uplcom&sektion=4&format=html) 驱动支持 following devices and adapters:
...
[umcs(4)](https://man.freebsd.org/cgi/man.cgi?query=umcs&sektion=4&format=html) 驱动在以下适配器上进行了测试：
...
[uslcom(4)](https://man.freebsd.org/cgi/man.cgi?query=uslcom&sektion=4&format=html) 驱动应支持以下设备： ：
...
[uvisor(4)](https://man.freebsd.org/cgi/man.cgi?query=uvisor&sektion=4&format=html) 驱动支持 following devices:
...
[uvscom(4)](https://man.freebsd.org/cgi/man.cgi?query=uvscom&sektion=4&format=html) 驱动支持以下适配器：
...
[xhci(4)](https://man.freebsd.org/cgi/man.cgi?query=xhci&sektion=4&format=html) 驱动支持具有 PCI 类 12（串行总线）、子类 3（USB）和编程接口 48（XHCI）的 `XHCI` 兼容控制器。

### FireWire（IEEE 1394）设备

[fwohci(4)](https://man.freebsd.org/cgi/man.cgi?query=fwohci&sektion=4&format=html) 驱动为 PCI/CardBus FireWire 接口卡提供支持。 驱动支持以下 IEEE 1394 OHCI 芯片组：
...
[amd64, i386] Serial Bus Protocol 2（SBP-2）存储设备（[sbp(4)](https://man.freebsd.org/cgi/man.cgi?query=sbp&sektion=4&format=html) 驱动）
...
[ccr(4)](https://man.freebsd.org/cgi/man.cgi?query=ccr&sektion=4&format=html) 驱动支持基于 T6 ASIC 的适配器上集成的加密加速引擎：
...
[qat(4)](https://man.freebsd.org/cgi/man.cgi?query=qat&sektion=4&format=html) 驱动支持以下 Intel QuickAssist 技术引擎：
...
[safe(4)](https://man.freebsd.org/cgi/man.cgi?query=safe&sektion=4&format=html) 驱动支持以下 SafeXcel 芯片：
...
[safexcel(4)](https://man.freebsd.org/cgi/man.cgi?query=safexcel&sektion=4&format=html) 驱动支持某些 Marvell 片上系统中的 Inside Secure EIP-97 设备的加密加速功能。

### 杂项

键盘包括：

* HID keyboards ([hkbd(4)](https://man.freebsd.org/cgi/man.cgi?query=hkbd&sektion=4&format=html) driver) over USB, I2C, or Apple SPI
* USB keyboards ([ukbd(4)](https://man.freebsd.org/cgi/man.cgi?query=ukbd&sektion=4&format=html) driver)
* PS/2 keyboards ([atkbd(4)](https://man.freebsd.org/cgi/man.cgi?query=atkbd&sektion=4&format=html) driver)
* ADB keyboards ([akbd(4)](https://man.freebsd.org/cgi/man.cgi?query=akbd&sektion=4&format=html) driver)
* AT-style keyboards ([atkbd(4)](https://man.freebsd.org/cgi/man.cgi?query=atkbd&sektion=4&format=html) driver)
* Hyper-v keyboards (**hv_kbd** driver)

指针设备包括：

* HID mice ([hms(4)](https://man.freebsd.org/cgi/man.cgi?query=hms&sektion=4&format=html) driver) and touch devices ([hmt(4)](https://man.freebsd.org/cgi/man.cgi?query=hmt&sektion=4&format=html) driver) over USB, I2C, Hyper-V, or Apple SPI
* USB mice ([ums(4)](https://man.freebsd.org/cgi/man.cgi?query=ums&sektion=4&format=html) driver) and touch devices ([wmt(4)](https://man.freebsd.org/cgi/man.cgi?query=wmt&sektion=4&format=html) driver)
* Apple laptop trackpads ([wsp(4)](https://man.freebsd.org/cgi/man.cgi?query=wsp&sektion=4&format=html) or [apt(4)](https://man.freebsd.org/cgi/man.cgi?query=apt&sektion=4&format=html) drivers)
* Elantech I2C touchpads ([ietp(4)](https://man.freebsd.org/cgi/man.cgi?query=ietp&sektion=4&format=html) driver)
* Chromebook Cypress APA I2C touchpads ([cyapa(4)](https://man.freebsd.org/cgi/man.cgi?query=cyapa&sektion=4&format=html) driver)
* PS/2 mice and compatible devices, including many laptop pointing devices ([psm(4)](https://man.freebsd.org/cgi/man.cgi?query=psm&sektion=4&format=html) driver)
* ADB mice and touch devices ([ams(4)](https://man.freebsd.org/cgi/man.cgi?query=ams&sektion=4&format=html) driver)
...
> **注意**：
>
> 关于特定显卡以及与 Xorg 兼容性的信息，可在 [http://www.x.org/](http://www.x.org/) 查阅。
...
[ads111x(4)](https://man.freebsd.org/cgi/man.cgi?query=ads111x&sektion=4&format=html) 驱动为 following devices:

	ADS1013     ADS1113
	ADS1014     ADS1114
	ADS1015     ADS1115

[amdsbwd(4)](https://man.freebsd.org/cgi/man.cgi?query=amdsbwd&sektion=4&format=html) 驱动支持以下芯片组：
...
[at45d(4)](https://man.freebsd.org/cgi/man.cgi?query=at45d&sektion=4&format=html) 驱动为 following devices:
...
[gpioths(4)](https://man.freebsd.org/cgi/man.cgi?query=gpioths&sektion=4&format=html) 驱动为 following devices:

	DHT11       DHT12
	DHT21       DHT22
	AM3201      AM3202

所支持的设备彼此相似，差异在于 primarily in accuracy and resolution. 这些设备需要单线进行数据通信，使用 自定义协议，与 Maxim 的 1-wire(tm) 不兼容。 AM320x 设备还支持连接到 i2c 总线， 但该驱动仅支持单线连接选项。

[ig4(4)](https://man.freebsd.org/cgi/man.cgi?query=ig4&sektion=4&format=html) 驱动支持基于 Synopsys DesignWare IP 的 I2C 控制器，可见于 Intel Core(TM) 处理器（自第四代起）、Intel Bay Trail、Apollo Lake SoC 系列以及部分 AMD 系统。

[intpm(4)](https://man.freebsd.org/cgi/man.cgi?query=intpm&sektion=4&format=html) 驱动支持以下芯片组：
...
[mx25l(4)](https://man.freebsd.org/cgi/man.cgi?query=mx25l&sektion=4&format=html) 驱动为 following devices:
...
当前版本的 [netfpga10g_nf10bmac(4)](https://man.freebsd.org/cgi/man.cgi?query=netfpga10g_nf10bmac&sektion=4&format=html) 驱动支持 NetFPGA-10G Embedded CPU Ethernet Core version 1.00a 的一个 PIO 模式接口。

[ntb_hw_plx(4)](https://man.freebsd.org/cgi/man.cgi?query=ntb_hw_plx&sektion=4&format=html) 驱动支持以下 PLX/Avago/Broadcom 芯片：
...
这些温度传感器由 [ow_temp(4)](https://man.freebsd.org/cgi/man.cgi?query=ow_temp&sektion=4&format=html) 驱动：
...
> 1-Wire, Parasite-Power, Ambient Temperature Sensor
 ...
 驱动支持系列代码 0x10、0x22、0x28 和 0x3b。

[sdhci(4)](https://man.freebsd.org/cgi/man.cgi?query=sdhci&sektion=4&format=html) 驱动支持不同规范的兼容芯片。以下芯片已验证可工作：
...
[superio(4)](https://man.freebsd.org/cgi/man.cgi?query=superio&sektion=4&format=html) 驱动支持 Nuvoton（前身为 Winbond）和 ITE 生产的众多 Super I/O 控制器，即：
