...
本硬件兼容性说明包含 FreeBSD 14.3 的硬件兼容性信息。
其中列出了 FreeBSD 支持的硬件平台以及各类硬件设备（存储控制器、网络接口等），并给出已知可正常工作的设备实例。
...
FreeBSD 支持 AMD64（“Hammer”）和 Intel EM64T 架构。
AMD64（也称为 x86-64）是受完全支持的 Tier 1 架构。
AMD64 支持所有现代 x86 处理器。
本发行版整合了多项优化和增强功能，以充分发挥基于 FreeBSD/amd64 系统的全部能力，在各种工作负载下提供更佳性能。

### 32 位 x86（i386）架构支持

虽然 FreeBSD 持续演进并为现代架构进行优化，我们仍在本发行版中致力于维护对老旧 i386（x86）架构的支持。
尽管 i386 硬件日益少见，但对于仍依赖它的用户，无论是用于遗留系统还是特定用途，FreeBSD 仍是可靠且通用的选择。
...
FreeBSD 对 RISC-V 的支持标志着这一开源操作系统发展中的重要里程碑。
...
本节介绍 FreeBSD 当前已知支持的设备。
其他配置也可能可用，但尚未经过测试。
欢迎对本列表提出反馈、更新和修正。

在可能的情况下，此处列出适用于各设备或设备类的驱动。
如果相关驱动在 FreeBSD 基本系统中拥有手册页（多数驱动都有），则在此处给出引用。
关于所支持的设备、控制器等的具体型号等信息，可在手册页中查阅。
...
要使用 nvme(4) 设备，必须在 BIOS 中将 SATA 模式设置为 AHCI（从 RST 切换），或者接受因中断共享导致 RST 启用时的性能。
...
支持所有主要固件版本（2.x、3.x、4.x 和 5.x），但是
...
注意，此硬件仅对 ATA DMA 命令支持命令队列和基于 FIS 的交换。
...
WORM/CD-R/CD-RW 写入支持由 [cdrecord(1)](https://man.freebsd.org/cgi/man.cgi?query=cdrecord&sektion=1&format=html) 提供，该工具是 Ports 中 [sysutils/cdrtools](http://www.freebsd.org/cgi/url.cgi?ports/sysutils/cdrtools/pkg-descr) port 的一部分。
...
驱动为基于
Attansic/Atheros L1 Gigabit Ethernet 控制器芯片，包括：
...
当前版本的
[atse(4)](https://man.freebsd.org/cgi/man.cgi?query=atse&sektion=4&format=html)
驱动支持 Ethernet MegaCore，如版本 11.1 的
...
驱动支持 ASIX Electronics AX88172/AX88178/AX88772/AX88772A/AX88772B/AX88760
...
驱动支持 100Gb 和 25Gb 以太网适配器上的 Virtual Functions
...
[cxgbev(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbev&sektion=4&format=html)
驱动支持 40Gb、10Gb 和 1Gb 以太网适配器上的 Virtual Functions
...
[cxgbev(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbev&sektion=4&format=html)
驱动支持基于 10Gb 和 1Gb 以太网适配器的 Virtual Functions
on the T4 ASIC:
...
on the 树莓派 4 Model B and related systems.
它采用 BCM54213PE PHY。
...
驱动为基于 Marvell/SysKonnect 的各种 NIC 提供支持
Yukon II based Gigabit Ethernet 控制器芯片，包括：
...
驱动支持基于 National Semiconductor DP83820 和 DP83821 的
...
驱动支持基于以下的 25/40/100 千兆以太网和 CNA 适配器
chipsets:
...
驱动支持 RealTek RTL8139C+、RTL8169、RTL816xS、RTL811xS、RTL8168、
...
驱动支持所有基于 Solarflare SFC9000 的 10Gb 以太网适配器
family controllers.
...
驱动支持基于 Silicon Integrated Systems SiS 900 和 SiS 7016 的
...
驱动为基于 Sundance/Tamarack 的各种 NIC 提供支持
TC9021 based Gigabit Ethernet 控制器芯片，包括：
...
驱动支持基于 VIA Networking VT6120、VT6122、VT6130 和 VT6132 的
Gigabit Ethernet adapters including:
...
驱动支持基于 VIA Technologies Rhine I、Rhine II 和 Rhine III 的
...
3C656 系列的 CardBus 卡和 3C556 系列的 MiniPCI 卡均内置了专有调制解调器。
[xl(4)](https://man.freebsd.org/cgi/man.cgi?query=xl&sektion=4&format=html)
驱动和任何其他 FreeBSD 驱动都不支持此调制解调器。
...
驱动支持所有 Atheros Cardbus、ExpressCard、PCI 和 PCIe 卡，
...
使用较旧 Broadcom 芯片组（BCM4301、BCM4303 和 BCM4306 rev 2）的用户
must use
bwi(4)
...
驱动支持来自
[mvm(4)](https://man.freebsd.org/cgi/man.cgi?query=mvm&sektion=4&format=html)
...
[iwlwifi(4)](https://man.freebsd.org/cgi/man.cgi?query=iwlwifi&sektion=4&format=html)
驱动支持来自
[mld(4)](https://man.freebsd.org/cgi/man.cgi?query=mld&sektion=4&format=html)
sub-driver with the following chipset generations:
...
驱动支持 following PCIe Wi-Fi devices:
...
驱动为 Atheros AR9170 USB IEEE 802.11b/g/n 提供支持
...
驱动支持基于 Ralink Technology 的 PCI/PCIe/CardBus 无线适配器
chipsets, including:
...
驱动为 Realtek RTL8188SU/RTL8192SU USB IEEE 802.11b/g/n 提供支持
wireless network adapters, including:
...
驱动支持 following wireless adapters:
...
驱动支持 USB 2.0 Conexant/Intersil PrismGT 系列无线
...
驱动支持 USB 2.0 无线 adapters based on the Ralink Technology
...
驱动支持 following cellular modems:
...
驱动支持 following classes of UARTs:
...
驱动支持 following classes of SCCs:
...
部分板载 CS4610 芯片附带 CS423x ISA 编解码器
...
驱动支持具有 PCI 类 4（多媒体）的控制器和
subclass 3 (HDA), compatible with Intel HDA specification.

[snd_hda(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hda&sektion=4&format=html)
驱动支持超过两百种不同的控制器和 CODEC。
在此列出所有这些没有意义，因为在大多数情况下特定的 CODEC
...
默认情况下，每个 pcm(4) 设备对应声卡上的一个物理端口。
ADAT 端口支持 8 声道、4 声道和 2 声道格式。
根据所选的采样率和声道格式，并非所有 pcm 声道都能
映射到 ADAT 声道，反之亦然。
...
支持一系列 USB 外设；已知可工作的设备列于本节中。
由于大多数 USB 设备的通用性，除部分例外，任何给定类别的设备都将受到支持，即使未在此明确列出。
...
驱动支持 following MacBooks produced in 2015-2018 years:
...
驱动为 BWCT console management serial adapters.
...
驱动支持 following devices and adapters:
...
驱动支持具有 PCI 类 12（串行总线）、子类 3（USB）和编程接口 48（XHCI）的 `XHCI` 兼容控制器。
...
驱动支持包含 Hifn 7751、7951、7811、7955 和 7956 芯片组的各种卡，例如：
...
* HID mice ([hms(4)](https://man.freebsd.org/cgi/man.cgi?query=hms&sektion=4&format=html) driver) and touch devices ([hmt(4)](https://man.freebsd.org/cgi/man.cgi?query=hmt&sektion=4&format=html) driver)
...
including many laptop pointing devices ([psm(4)](https://man.freebsd.org/cgi/man.cgi?query=psm&sektion=4&format=html) driver)
...
这些设备需要单线进行数据通信，使用
自定义协议，与 Maxim 的 1-wire(tm) 不兼容。
AM320x 设备还支持连接到 i2c 总线，
但该驱动仅支持单线连接选项。

[ig4(4)](https://man.freebsd.org/cgi/man.cgi?query=ig4&sektion=4&format=html)
驱动支持基于 Synopsys DesignWare IP 的 I2C 控制器，可见于
Intel Core(TM) 处理器（自第四代起）、Intel
...
驱动支持 NetFPGA-10G Embedded CPU Ethernet Core version 1.00a 的一个 PIO 模式接口。
...
驱动支持 Nuvoton（前身为 Winbond）和 ITE 生产的众多 Super I/O 控制器，即：
