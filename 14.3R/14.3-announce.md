# FreeBSD 14.3-RELEASE 发行公告

日期：2025 年 6 月 10 日

FreeBSD 发布工程团队很高兴地宣布 FreeBSD 14.3-RELEASE 现已发布。这是 stable/14 分支的第四个版本。

部分亮点：

* iwlwifi 驱动现支持许多笔记本电脑中现代芯片组的 802.11ac。

* OCI 容器镜像现已在 Docker 和 GitHub 仓库中发布。

* LLVM 及相关工具升级到 19.1.7 版本。

* OpenSSH 升级到 9.9p2 版本。
...
FreeBSD 14.3-RELEASE 现已支持 amd64、i386、aarch64、armv7、powerpc、powerpc64 和 riscv64 架构。

FreeBSD 14.3-RELEASE 可从可启动 ISO 镜像或通过网络安装。部分架构还支持从 USB 闪存盘安装。所需文件可按以下说明下载。
...
此外，可将其写入 USB 闪存盘（U 盘）用于 amd64 架构，在可从 USB 启动的机器上进行安装。还支持启动到基于“livefs”的救援模式。不含预构建的第三方软件包。
...
可写入 USB 闪存盘（U 盘）用于启动机器，但介质本身不包含安装发行版，与 bootonly 镜像类似。还支持启动到基于“livefs”的救援模式。不含预构建的第三方软件包。
...
FreeBSD 14.3-RELEASE 还可从多家供应商购买 DVD 介质。以下供应商将提供基于 FreeBSD 14.3 的产品：
...
FreeBSD 14.3-RELEASE 还可在以下云托管平台上获取：
...
* Google Compute Engine(TM):
  ...
  FreeBSD 镜像还可在 Google Compute Engine(TM) Marketplace 获取：<https://console.cloud.google.com/marketplace/browse?filter=category:os&filter=price:free&q=freebsd>

* Oracle Cloud Infrastructure:

  FreeBSD 镜像可在 Oracle Cloud Infrastructure Marketplace 获取，支持 amd64 和 aarch64 架构，包括 UFS 和 ZFS 文件系统：<https://cloudmarketplace.oracle.com/marketplace/app/freebsd-release>

* Microsoft Azure(TM):

  FreeBSD 镜像预计将在 Azure Marketplace 提供：<https://azuremarketplace.microsoft.com/marketplace/apps/thefreebsdfoundation.freebsd-14_3>

## 下载

FreeBSD 14.3-RELEASE 可从以下站点通过 https 下载：

* <https://download.freebsd.org/releases/ISO-IMAGES/14.3/>

FreeBSD 14.3-RELEASE 虚拟机镜像可从以下地址下载：

* <https://download.freebsd.org/releases/VM-IMAGES/14.3-RELEASE/>

FreeBSD 14.3-RELEASE BASIC-CI 镜像可从以下地址下载：

* <https://download.freebsd.org/releases/CI-IMAGES/14.3-RELEASE/>

FreeBSD 14.3-RELEASE OCI 容器镜像可从以下地址下载：

* <https://download.freebsd.org/releases/OCI-IMAGES/14.3-RELEASE/>

也可在 Docker 和 GitHub 容器仓库中获取。

关于安装 FreeBSD 或将现有机器更新到 14.3-RELEASE 的说明，请参阅：

* <https://www.FreeBSD.org/releases/14.3R/installation/>

## 支持

本版本 FreeBSD 14.3-RELEASE 将受支持至 2026 年 6 月 30 日。上一版本 FreeBSD 14.2-RELEASE 将于 2025 年 9 月 30 日到达生命周期终点。FreeBSD 14 发布系列将受支持至 2028 年 11 月 30 日。更多支持信息可在以下地址找到：
...
许多公司为支持 FreeBSD 14.3 的发布工程活动捐赠了设备、网络访问或人力，包括：
...
| Colin Percival <cperciva@FreeBSD.org> | 发布工程负责人、14.3-RELEASE 发布工程师 |
...
喜爱 FreeBSD？请向 FreeBSD 基金会[捐赠](https://www.freebsdfoundation.org/donate/) 以支持本版本及未来的发行版！
