...
务必确保目标（`of=`）正确。
bootonly::
支持使用 CDROM 驱动器启动机器，但不包含用于从 CD 本身安装 FreeBSD 的安装发行版。
你需要从 CD 启动后执行基于网络的安装（例如从 FTP 服务器）。
memstick::
...
为方便没有控制台访问权限的用户，默认提供 `freebsd` 用户（密码为 `freebsd`）用于 [ssh(1)](https://www.freebsd.org/cgi/man.cgi?query=ssh&sektion=1&manpath=freebsd-release-ports) 访问。
...
许多公司捐赠了设备、网络访问或人力时间来支持 FreeBSD 11.3 的发布工程活动，包括 The FreeBSD Foundation、Rubicon Communications, LLC (Netgate)、Yahoo!、NetApp、Internet Systems Consortium、ByteMark Hosting、CyberOne Data、Sentex Communications、New York Internet、Juniper Networks、NetActuate、National Chiao Tung University、NLNet Labs 和 iXsystems。
