# FreeBSD 12.2-RELEASE 发行公告

日期：2020 年 10 月 27 日

FreeBSD 发布工程团队很高兴地宣布 FreeBSD 12.2-RELEASE 现已发布。这是 stable/12 分支的第三个版本。

部分亮点如下：

* 引入了对无线网络栈和各种驱动程序的更新，以提供更好的 802.11n 和 802.11ac 支持。
* 新增了 **ice(4)** 驱动程序，支持 Intel 100Gb 以太网卡。
* **jail(8)** 工具已更新，允许在 Jail 环境中运行 Linux。
* OpenSSL 已更新至 1.1.1h。
* OpenSSH 已更新至 7.9p1。
* `clang`、`llvm`、`lld`、`lldb`、`compiler-rt` 工具和 `libc++` 已更新至 10.0.1。
* 以及更多……
...
FreeBSD 12.2-RELEASE 现已支持 amd64、i386、powerpc、powerpc64、sparc64、armv6 和 aarch64 架构。

FreeBSD 12.2-RELEASE 可从可启动 ISO 镜像或通过网络安装。部分架构还支持从 USB 闪存盘安装。所需文件可按以下说明下载。

本消息底部包含发行版 ISO、闪存盘和 SD 卡镜像的 SHA512 和 SHA256 校验和。

发行版镜像的 PGP 签名校验和也可从以下地址获取：

* <https://www.FreeBSD.org/releases/12.2R/signatures/>
...
FreeBSD 12.2-RELEASE 也可从多家供应商处购买 CD-ROM 或 DVD 介质。提供 FreeBSD 12.2 相关产品的供应商之一是：

* FreeBSD Mall, Inc. <https://www.freebsdmall.com>

预安装的虚拟机镜像也可用于 amd64（x86_64）、i386（x86_32）和 AArch64（arm64）架构，提供 `QCOW2`、`VHD` 和 `VMDK` 磁盘镜像格式，以及原始（未格式化）镜像。

FreeBSD 12.2-RELEASE amd64 还可在以下云托管平台上使用：
...
AMI 预计还将在 Amazon Marketplace 中提供，待第三方验证完成后访问：<https://aws.amazon.com/marketplace/pp/B07L6QV354/>
...
AMI 也可在 Amazon Marketplace 中获取：<https://aws.amazon.com/marketplace/pp/B081NF7BY7/>
...
FreeBSD 12.2-RELEASE 预计还将在 Google Compute Engine(TM) Marketplace 中提供，待其完成第三方特定验证后，访问：<https://console.cloud.google.com/launcher/browse?filter=category:os&filter=price:free>
...
FreeBSD 12.2-RELEASE 可从以下站点通过 https 下载：

* <https://download.freebsd.org/ftp/releases/ISO-IMAGES/12.2/>

FreeBSD 12.2-RELEASE 虚拟机镜像可从以下地址下载：

* <https://download.freebsd.org/ftp/releases/VM-IMAGES/12.2-RELEASE/>

关于安装 FreeBSD 或将现有机器更新至 12.2-RELEASE 的说明，请参阅：

* <https://www.FreeBSD.org/releases/12.2R/installation/>

## 支持

根据新的 FreeBSD 支持模型，FreeBSD 12 发行系列将至少支持至 2024 年 6 月 30 日。本点发行版 FreeBSD 12.2-RELEASE 将支持至 FreeBSD 12.3-RELEASE 发行后至少三个月。其他支持信息可在以下地址找到：

* <https://www.FreeBSD.org/security/>

请注意，12.1 将支持至 12.2 发行日期起三个月后，目前计划为 2021 年 1 月 31 日。

## 致谢

许多公司捐赠了设备、网络访问或人力时间来支持 FreeBSD 12.2 的发布工程活动，包括：

| 供应商 |
| :----- |
| [FreeBSD 基金会](https://www.freebsdfoundation.org) |
| [Rubicon Communications, LLC (netgate.com)](https://www.netgate.com) |
| [Tarsnap](https://www.tarsnap.com) |
| [NetApp](https://www.netapp.com) |
| [Internet Systems Consortium](https://www.isc.org) |
| [ByteMark Hosting](https://bytemark.co.uk) |
| [CyberOne Data](https://www.cyberonedata.com) |
...
12.2-RELEASE 的发布工程团队包括：

| 成员 | 角色 |
| :--- | :--- |
| Glen Barber <gjb@FreeBSD.org> | 发布工程负责人，12.2-RELEASE 发布工程师 |
