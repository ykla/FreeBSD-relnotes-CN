...
典型的发行说明条目记录了自 12.1-RELEASE 以来发布的安全公告、新驱动程序或硬件支持、新命令或选项、主要 bug 修复或第三方软件升级。它们还可能列出对主要 Ports/软件包或发布工程实践的变更。显然，发行说明无法列出各个版本之间对 FreeBSD 所做的每一项变更；本文档主要聚焦于安全公告、用户可见的变更和重要的架构改进。
