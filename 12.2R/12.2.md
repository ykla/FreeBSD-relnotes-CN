...
FreeBSD 12.2-RELEASE 的发行说明包含了 FreeBSD 基本系统在 12-STABLE 开发线上的变更摘要。本文档列出了自上一个版本以来发布的适用的安全公告，以及 FreeBSD 内核和用户空间的重大变更。本文档还简要介绍了升级过程。

## 引言

本文档包含 FreeBSD 12.2-RELEASE 的发行说明。它介绍了 FreeBSD 最近新增、修改或删除的功能。它还提供了一些关于从以前版本升级到当前版本的说明。

这些发行说明适用于自 12-STABLE 开发分支创建以来的最新版本。有关沿该分支的预编译二进制发布版本的信息，可以在 [https://www.FreeBSD.org/releases/](https://www.freebsd.org/releases/) 找到。

这些发行说明适用于 12-STABLE 开发分支的一个点，该点位于 12.1-RELEASE 和未来的 12.3-RELEASE 之间。有关沿该分支的预编译二进制发布版本的信息也可以在 [https://www.FreeBSD.org/releases/](https://www.freebsd.org/releases/) 找到。

FreeBSD 12.2-RELEASE 是一个 RELEASE 版本，可在 [https://www.FreeBSD.org/releases/](https://www.freebsd.org/releases/) 或其镜像站点获取。关于获取此（或其他）RELEASE 版本的更多信息，可参阅 [FreeBSD 手册](https://docs.freebsd.org/en/books/handbook/) 中的 [获取 FreeBSD 附录](https://docs.freebsd.org/en/books/handbook/mirrors/)。

建议所有用户在安装 FreeBSD 之前查阅发行勘误。该勘误文档会以发布周期后期或发布后才发现的“最新”信息进行更新。通常，它包含已知的错误、安全公告和文档修正。最新的 FreeBSD 12.2-RELEASE 勘误副本可以在 FreeBSD 网站上找到。

本文档介绍了自 12.1-RELEASE 以来 FreeBSD 中最显著的新功能或变更。通常，文中描述的变更是 12-STABLE 分支特有的，除非特别标注为合并特性。

典型的发行说明条目记录了自 12.1-RELEASE 以来发布的安全公告、新驱动程序或硬件支持、新命令或选项、主要 bug 修复或第三方软件升级。它们还可能列出对主要 Ports/软件包或发布工程实践的变更。显然，发行说明无法列出各个版本之间对 FreeBSD 所做的每一个变更；本文档主要聚焦于安全公告、用户可见的变更和重要的架构改进。

## 从旧版 FreeBSD 升级

[freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 工具支持在 RELEASE 版本之间进行二进制升级（以及各安全分支的快照）。二进制升级过程将更新未修改的用户空间工具，以及作为官方 FreeBSD 发行版的一部分分发的未修改 **GENERIC** 内核。[freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 工具要求升级的主机具有互联网连接。

基于源代码的升级（即从源代码重新编译 FreeBSD 基本系统）也受支持，请按照 **/usr/src/UPDATING** 中的说明进行。

> **重要**：在进行 FreeBSD 升级前，必须先备份 *所有* 数据和配置文件。

## 安全与勘误

本节列出了自 12.1-RELEASE 以来发布的各种安全公告和勘误。
...
| [FreeBSD-SA-20:08.jail](https://www.freebsd.org/security/advisories/FreeBSD-SA-20:08.jail.asc) | 2020 年 3 月 18 日 | 嵌套 Jail 导致的内核内存信息泄漏 |
...
| [FreeBSD-SA-20:13.libalias](https://www.freebsd.org/security/advisories/FreeBSD-SA-20:13.libalias.asc) | 2020 年 5 月 12 日 | 内存信息泄漏漏洞 |
| [FreeBSD-SA-20:15.cryptodev](https://www.freebsd.org/security/advisories/FreeBSD-SA-20:15.cryptodev.asc) | 2020 年 5 月 12 日 | 释放后使用条件 |
...
| [FreeBSD-SA-20:20.ipv6](https://www.freebsd.org/security/advisories/FreeBSD-SA-20:20.ipv6.asc) | 2020 年 7 月 8 日 | 竞态条件与释放后使用 |
...
| [FreeBSD-SA-20:25.sctp](https://www.freebsd.org/security/advisories/FreeBSD-SA-20:25.sctp.asc) | 2020 年 9 月 2 日 | 释放后使用 bug |
...
[devd(8)](https://man.freebsd.org/cgi/man.cgi?query=devd&sektion=8&format=html) 工具已更新，默认的 [syslogd(8)](https://man.freebsd.org/cgi/man.cgi?query=syslogd&sektion=8&format=html) 恢复通知已从 `kern` 改为 `kernel`。[[r365540]](http://svn.freebsd.org/viewvc/base?view=revision&revision=365540)

### 用户空间应用程序变更

[cron(8)](https://man.freebsd.org/cgi/man.cgi?query=cron&sektion=8&format=html) 工具已更新，支持在 [crontab(5)](https://man.freebsd.org/cgi/man.cgi?query=crontab&sektion=5&format=html) 中使用两个新选项，`-n` 和 `-q`，分别用于抑制成功运行时的邮件通知和抑制命令执行的日志记录。[[r353134]](http://svn.freebsd.org/viewvc/base?view=revision&revision=353134)

[dd(1)](https://man.freebsd.org/cgi/man.cgi?query=dd&sektion=1&format=html) 工具已更新，包含以下新操作符：[[r355520]](http://svn.freebsd.org/viewvc/base?view=revision&revision=355520)
...
使用方法详见 [dd(1)](https://man.freebsd.org/cgi/man.cgi?query=dd&sektion=1&format=html)。

[fsck_msdosfs(8)](https://man.freebsd.org/cgi/man.cgi?query=fsck_msdosfs&sektion=8&format=html) 工具已更新，包含多项增强功能，包括减少内存占用、新增选项 `-M`（该选项禁用 [mmap(2)](https://man.freebsd.org/cgi/man.cgi?query=mmap&sektion=2&format=html) 的使用）等。[[r357568]](http://svn.freebsd.org/viewvc/base?view=revision&revision=357568)

[showmount(8)](https://man.freebsd.org/cgi/man.cgi?query=showmount&sektion=8&format=html) 工具已更新，支持长选项。[[r357078]](http://svn.freebsd.org/viewvc/base?view=revision&revision=357078)

新增了 [certctl(8)](https://man.freebsd.org/cgi/man.cgi?query=certctl&sektion=8&format=html) 工具。[[r357082]](http://svn.freebsd.org/viewvc/base?view=revision&revision=357082)
...
[sesutil(8)](https://man.freebsd.org/cgi/man.cgi?query=sesutil&sektion=8&format=html) 工具已更新，新增子命令 `show`，以用户友好的方式打印输出。[[r364115]](http://svn.freebsd.org/viewvc/base?view=revision&revision=364115)
...
[jail(8)](https://man.freebsd.org/cgi/man.cgi?query=jail&sektion=8&format=html) 工具已更新，允许在 Jail 环境中运行 Linux。

### 第三方软件

[tcsh(1)](https://man.freebsd.org/cgi/man.cgi?query=tcsh&sektion=1&format=html) 工具已更新至版本 6.21.00。[[r354191]](http://svn.freebsd.org/viewvc/base?view=revision&revision=354191)（由 DARPA, AFRL 赞助）
...
[unbound(8)](https://man.freebsd.org/cgi/man.cgi?query=unbound&sektion=8&format=html) 工具已更新至版本 1.10.1。[[r361435]](http://svn.freebsd.org/viewvc/base?view=revision&revision=361435)

[libarchive(3)](https://man.freebsd.org/cgi/man.cgi?query=libarchive&sektion=3&format=html) 库已更新至版本 3.4.3。[[r362132]](http://svn.freebsd.org/viewvc/base?view=revision&revision=362132)
...
[ntpd(8)](https://man.freebsd.org/cgi/man.cgi?query=ntpd&sektion=8&format=html) 工具套件已更新至版本 4.2.8p15。[[r362716]](http://svn.freebsd.org/viewvc/base?view=revision&revision=362716)

[file(1)](https://man.freebsd.org/cgi/man.cgi?query=file&sektion=1&format=html) 工具已更新至版本 5.39。[[r362842]](http://svn.freebsd.org/viewvc/base?view=revision&revision=362842)
...
[nc(1)](https://man.freebsd.org/cgi/man.cgi?query=nc&sektion=1&format=html) 工具已更新，新增选项 `--sctp`。[[r363474]](http://svn.freebsd.org/viewvc/base?view=revision&revision=363474)

clang、llvm、lld、lldb、compiler-rt 工具和 libc++ 已更新至版本 10.0.1。[[r363494]](http://svn.freebsd.org/viewvc/base?view=revision&revision=363494)

OpenSSL 已更新至版本 1.1.1h。[[r366177]](http://svn.freebsd.org/viewvc/base?view=revision&revision=366177)

### 已弃用应用程序

[amd(8)](https://man.freebsd.org/cgi/man.cgi?query=amd&sektion=8&format=html) 工具已标记为弃用，并计划在 FreeBSD 13.0 中移除。[[r355075]](http://svn.freebsd.org/viewvc/base?view=revision&revision=355075)
...
`machdep.kdb_on_nmi` [sysctl(8)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&format=html) 已移除。可调参数 `machdep.panic_on_nmi` [sysctl(8)](https://man.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&format=html) 已变更为直接进入调试器。[[r364002]](http://svn.freebsd.org/viewvc/base?view=revision&revision=364002)

新增了对 APEI（ACPI 平台错误接口）的支持。[[r364003]](http://svn.freebsd.org/viewvc/base?view=revision&revision=364003)（由 iXsystems 赞助）
...
[ubsec(4)](https://man.freebsd.org/cgi/man.cgi?query=ubsec&sektion=4&format=html) 驱动程序已标记为弃用，并将在 FreeBSD 13.0 中移除。[[r361044]](http://svn.freebsd.org/viewvc/base?view=revision&revision=361044)

[ufm(4)](https://man.freebsd.org/cgi/man.cgi?query=ufm&sektion=4&format=html) 驱动程序已标记为弃用，并将在 FreeBSD 13.0 中移除。[[r364431]](http://svn.freebsd.org/viewvc/base?view=revision&revision=364431)

[apm(4)](https://man.freebsd.org/cgi/man.cgi?query=apm&sektion=4&format=html) 驱动程序已标记为弃用，并将在 FreeBSD 13.0 中移除。[[r365542]](http://svn.freebsd.org/viewvc/base?view=revision&revision=365542)

[ctau(4)](https://man.freebsd.org/cgi/man.cgi?query=ctau&sektion=4&format=html) 和 [cx(4)](https://man.freebsd.org/cgi/man.cgi?query=cx&sektion=4&format=html) 驱动程序已标记为弃用，并将在 FreeBSD 13.0 中移除。[[r365542]](http://svn.freebsd.org/viewvc/base?view=revision&revision=365542)（由 FreeBSD 基金会赞助）
...
[virtio_blk(4)](https://man.freebsd.org/cgi/man.cgi?query=virtio_blk&sektion=4&format=html) 驱动程序已更新，以支持 TRIM。[[r365702]](http://svn.freebsd.org/viewvc/base?view=revision&revision=365702)（由 Klara Systems 赞助）
...
现在在引导加载程序中显示控制台，可以在可用的控制台设备之间切换。[[r366691]](http://svn.freebsd.org/viewvc/base?view=revision&revision=366691)
...
[tap(4)](https://man.freebsd.org/cgi/man.cgi?query=tap&sektion=4&format=html) 和 [tun(4)](https://man.freebsd.org/cgi/man.cgi?query=tun&sektion=4&format=html) 设备已更新，以便在重命名时创建 **/dev** 别名。[[r354060]](http://svn.freebsd.org/viewvc/base?view=revision&revision=354060)

[ipfw(4)](https://man.freebsd.org/cgi/man.cgi?query=ipfw&sektion=4&format=html) 驱动程序已更新，支持 RFC6598/Carrier Grade NAT 子网。[[r359694]](http://svn.freebsd.org/viewvc/base?view=revision&revision=359694)

[ng_nat(4)](https://man.freebsd.org/cgi/man.cgi?query=ng_nat&sektion=4&format=html) 驱动程序已更新，允许附加到以太网接口。[[r359697]](http://svn.freebsd.org/viewvc/base?view=revision&revision=359697)

[ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 驱动程序已更新至版本 1.11.29。[[r363876]](http://svn.freebsd.org/viewvc/base?view=revision&revision=363876)（由 Intel Corporation 赞助）

[ena(4)](https://man.freebsd.org/cgi/man.cgi?query=ena&sektion=4&format=html) 驱动程序已更新至版本 2.2.0。[[r365381]](http://svn.freebsd.org/viewvc/base?view=revision&revision=365381)（由 Amazon, Inc. 赞助）

更新了无线网络栈和各种驱动程序，以提供更好的 802.11n 和 802.11ac 支持。[[r365670]](http://svn.freebsd.org/viewvc/base?view=revision&revision=365670)（由 Rubicon Communications, LLC (Netgate) 赞助）

**ice(4)** 驱动程序已添加，支持 Intel 100Gb 以太网卡。[[r365733]](http://svn.freebsd.org/viewvc/base?view=revision&revision=365733)（由 Intel Corporation 赞助）

[cxgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbe&sektion=4&format=html) 驱动程序已更新至版本 1.25.0.0。[[r365961]](http://svn.freebsd.org/viewvc/base?view=revision&revision=365961)（由 Chelsio Communications 赞助）
...
[pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html) 工具已更新至版本 1.15.10。
...
这意味着，默认情况下，生成的二进制文件将要求使用 686 类 CPU，包括但不限于 FreeBSD Release Engineering 团队提供的二进制文件。FreeBSD 13.0 将继续支持旧的 CPU，但需要此功能的用户需自行构建发行版以获得官方支持。
...
由于大多数 32 位测试是由开发人员在启用了内核选项 `COMPAT_FREEBSD32` 的 64 位硬件上使用 lib32 库进行的，因此这一变更确保了更好的覆盖范围和用户体验。这也符合大多数 Linux 发行版已经实施的做法。

这预计将是 i386 默认 `CPUTYPE` 的最终提升。

> **重要**：此变更不影响 FreeBSD 12.x 或 11.x 版本系列。
