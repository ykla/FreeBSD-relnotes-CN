...
欢迎使用 3.2-RELEASE，这是对 1999 年 2 月发布的 3.1-RELEASE 的完全跟进，标志着 3.0-STABLE 分支的开始。自从 3.1 发布以来的四个月里，系统进行了数百项错误修复和一般增强。详细信息请见下文。

任何安装失败或崩溃问题应使用命令 `send-pr` 报告（喜欢使用基于网页的界面的用户也可以查看此页面）。

有关 FreeBSD 及 3.2-RELEASE 目录布局的信息（特别是如果你是从软盘安装的话！），请参阅 `ABOUT.TXT`。有关安装说明，请参阅文件 `INSTALL.TXT` 和 `HARDWARE.TXT`。
...
- 增加了对 ISO 9660 文件系统中 Joliet 扩展的支持。不过目前仅支持 iso8859-1 字符（拉丁文字符集）。
- 增加了对 Adaptec 2930U2 和 3950U2 SCSI 卡的支持。
- 一些内核变更将会破坏 CAM 透传接口或 **devstat(9)** 统计接口的二进制接口。这些变更是为了修复一些接口缺陷。我们对可能带来的不便表示遗憾，但预计这种影响会非常小，因为目前没有已知的商用二进制应用程序使用这两个接口。这将要求使用这两个接口的程序重新编译。使用 CAM 接口的 Ports 包括 xmcd、tosha、SANE、cdrecord 和 cdda2wav。使用 `devstat` 接口的 Ports 包括 xsysinfo 和 xperfmon。

### 1.2. 安全修复

- 关闭了一个潜在的描述符泄漏漏洞，本地用户可能利用该漏洞发起拒绝服务攻击。参考：KKIS.05051999.003b。

### 1.3. 用户空间变化

- 将互联网软件联盟的 DHCP 客户端加入基本系统。
...
- Gdb 更新到 4.18 版本，现在是 FreeBSD/alpha 标准发布版本的一部分。
- `camcontrol` 现在允许用户查看任何给定设备的标记队列的打开次数，并允许用户设置支持标记队列的设备的标记队列的打开次数。
- `camcontrol` 还允许用户变更某些控制器上设备的 SCSI 协商参数（例如同步速率、偏移量、总线宽度、断开连接）。请注意，这个功能仅在 Adaptec 7xxx 系列控制器、Advansys 窄控制器和 NCR/Symbios 控制器上完全有效。
- `systat`、`vmstat` 和 `iostat` 现在会根据设备的重要性而不是探测顺序打印出更“有趣”的统计信息。
- 根据用户对 3.1-RELEASE 的反馈，修复了大量用户空间和内核中的错误。

## 2. 支持的配置

FreeBSD 目前可以在各种 ISA、VLB、EISA 和 PCI 总线的 PC 上运行，支持的机器从 386sx 到 Pentium 级别（尽管不推荐使用 386sx）。同时也支持常规的 IDE 或 ESDI 硬盘配置、各种 SCSI 控制器、网卡和串口卡。

下面是目前已知能够与 FreeBSD 配合工作的所有外设列表。其他配置也可能兼容，但我们尚未收到确认。
...
- BusLogic MultiMaster 控制器：
  ...
  [请注意，BusLogic/Mylex“Flashpoint”适配器**尚不支持**]
...
- 也支持作为真正 BusLogic MultiMaster 克隆的 AMI FastDisk 控制器。

- DPT SmartCACHE Plus、SmartCACHE III、SmartRAID III、SmartCACHE IV 和 SmartRAID IV SCSI/RAID 控制器均支持。DPT SmartRAID/CACHE V 尚不支持。
...
对于所有支持的 SCSI 控制器，提供对 SCSI-I 和 SCSI-II 外设的全面支持，包括硬盘、光盘、磁带驱动器（包括 DAT 和 8mm Exabyte）、介质交换器、处理器目标设备和 CD-ROM 驱动器。CD-ROM 驱动程序可对支持 CD-ROM 命令的 WORM 设备进行只读访问。WORM/CD-R/CD-RW 写入支持由 `cdrecord` 提供，且该工具可以在 Ports 树中找到。
...
[注：目前正在将 AIC-6260/6360 和 UltraStor 驱动程序移植到新的 CAM SCSI 框架，但无法估计完成时间，也无法保证能否完成。]
...
- 基于 Xilinx XC6200 的可重构硬件卡，兼容 Virtual Computers（www.vcc.com）的 HOT1。
...
你可以从 `ftp.FreeBSD.org`（官方 FreeBSD 发布站点）下载 FreeBSD 及其所有或部分可选软件包。

其他镜像站点详情，请参阅文件 `MIRROR.SITES`。请从网络距离最近的站点下载发行版。我们欢迎更多镜像站点的加入！如果你希望成为官方镜像站点，请联系 `freebsd-admin@FreeBSD.org` 获取更多信息。
...
还可通过互联网订购：`orders@cdrom.com`，网址：**http://www.freebsdmall.com**。他们的当前目录可以通过 FTP 获取：**ftp://ftp.cdrom.com/cdrom/catalog**
...
如果你从以前的 FreeBSD 版本升级，最可能是 2.2.x 或 2.1.x（在少数罕见情况下），以下某些问题可能会影响你，具体取决于你选择的升级方式。升级 FreeBSD 版本有两种常见方式：

- 使用源代码，通过 **/usr/src**
- 还可使用 `sysinstall` 的（二进制）升级选项。

如果使用源代码升级，你需要了解两个目标：标准的 `upgrade` 目标，它将把 2.x 或 3.0 系统升级到 3.2，以及 `world` 目标，它将保持升级后的系统与初始升级后发生的任何变更同步。

如果使用二进制升级选项，系统将直接升级到 3.2/ELF，同时填充 **/<basepath>/lib/aout** 目录，以便与旧版二进制文件保持兼容。

无论哪种方式，升级到 ELF 意味着你将拥有更小的二进制文件，并能够使用更多移植到其他 ELF 环境的编译器功能（我们较为陈旧、略显粗糙的 a.out 格式，其他软件项目大多不支持）。希望继续使用旧版 a.out 动态可执行文件的用户，应确保安装 `compat22` 发行版。请注意，a.out 库在系统重新启动之前无法使用，这可能会导致某些 a.out 软件包出现问题。

此外，不要使用以前版本的安装盘或 `sysinstall`，因为 3.1 版本引入了新的引导过程，要求安装新的引导块（由于 ELF 内核），而 3.2 版本进一步修改了引导过程。

[其他重要的升级注意事项应放在此处]

## 5. 报告问题、提出建议、提交代码

你的建议、错误报告和代码贡献始终受到重视——如果你发现任何问题，请随时报告（如果可能，请附上修复方案！）。

如果你的机器有互联网邮件连接，建议使用命令 `send-pr` 或通过 [http://www.FreeBSD.org/send-pr.html](http://www.FreeBSD.org/send-pr.html) 的 CGI 脚本提交错误报告。我们忠实的 `bugfiler` 程序会负责归档错误报告，你可以放心，我们会尽最大努力尽快响应所有报告的错误。通过这种方式提交的错误报告也会在我们的网站支持部分显示，因此它们不仅是错误报告，也是对其他用户关于潜在问题的“路标”。

如果由于某种原因，你无法使用命令 `send-pr` 提交错误报告，你可以尝试发送邮件到：`freebsd-bugs@FreeBSD.org`

请注意，`send-pr` 本身是个 shell 脚本，即使移植到完全不同的系统上也能轻松运行。我们更希望你能使用这种接口，因为它使跟踪问题报告更加容易。然而，在提交报告之前，请确认该问题是否在此期间修复完成。

对于其他问题或技术支持，请发送邮件至：`freebsd-questions@FreeBSD.org`

此外，由于这是一个志愿者项目，我们非常乐意欢迎更多愿意提供帮助的人——目前我们期望的增强功能远超自身的处理能力！如果你希望在技术方面提供帮助或以其他方式协助，请发送邮件至：`freebsd-hackers@FreeBSD.org`

请注意，这些邮件列表可能会产生 *大量* 的流量，如果你的邮件获取速度较慢或成本较高，并且你只关心 FreeBSD 的重要事件，你可能会觉得订阅以下邮件列表更合适：`freebsd-announce@FreeBSD.org`

所有邮件列表都可以自由加入，任何有兴趣的人都可以加入。发送邮件到 `MajorDomo@FreeBSD.org`，并在邮件正文中单独包含 `help` 关键词。这将为你提供更多关于加入各种列表、访问档案等的详细信息。还有一些未在此处列出的特殊兴趣小组邮件列表，也可通过发送邮件到 `majordomo` 查询！

## 6. 致谢

FreeBSD 凝聚了来自世界各地数十甚至数百名个人的累积工作，这些人付出了极大的努力才将此版本带给你。有关 FreeBSD 项目工作人员的完整名单，请参阅：[http://www.FreeBSD.org/handbook/staff.html](http://www.FreeBSD.org/handbook/staff.html)

或者，如果你加载了文档分发版：**file:/usr/share/doc/handbook/staff.html**
