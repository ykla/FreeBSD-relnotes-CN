...
典型的发行说明内容包括 11.2-RELEASE 之后发布的最新安全公告、新增的驱动程序或硬件支持、新命令或选项、重大错误修复或第三方软件升级。这些内容还可能包括主要 Port/软件包的变更或发布工程的改进。显然，发行说明无法列出 FreeBSD 在版本之间的每一个变更；本文主要关注安全公告、用户可见的变更以及重要的架构改进。
...
[jail(8)](https://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8&manpath=freebsd-release-ports) 工具已更新，新增参数 `-e`，该参数以 [jail.conf(5)](https://www.freebsd.org/cgi/man.cgi?query=jail.conf&sektion=5&manpath=freebsd-release-ports) 参数作为输入，并打印具有指定参数的非通配符 Jail 列表。[(r341790)](http://svn.freebsd.org/viewvc/base?view=revision&revision=341790)
