...
FreeBSD 11.3-RELEASE 的发行说明总结了 11.3-STABLE 开发分支中对 FreeBSD 基本系统所做的变更。本文档列出自上次发布以来公布的适用安全公告，以及对 FreeBSD 内核和用户空间的重要变更。同时，还提供了有关升级的简要说明。

## 引言

本文档包含 FreeBSD 11.3-RELEASE 的发行说明，介绍了 FreeBSD 最近新增、变更或删除的功能，并提供了从 FreeBSD 之前版本升级的相关说明。

可以从 [https://www.FreeBSD.org/releases/](https://www.freebsd.org/releases/) 或其任何镜像站点获取 FreeBSD 11.3-RELEASE 的发行版本。获取该版本（或其他版本）发行文件的方法，请参阅 [FreeBSD 手册](https://docs.freebsd.org/en/books/handbook/) 的 [“获取 FreeBSD”](https://docs.freebsd.org/en/books/handbook/#mirrors) 附录。

强烈建议所有用户在安装 FreeBSD 之前查阅发行勘误。勘误包含在发行周期末期或发行后发现的最新信息，通常包括已知问题、安全公告和文档修正。可以在 FreeBSD 网站上找到 FreeBSD 11.3-RELEASE 的最新勘误。

本文档介绍了自 11.2-RELEASE 以来 FreeBSD 中最显而易见的新增或变更功能。通常，本文描述的变更是 11.3-STABLE 分支所独有的，除非明确标注为合并（MERGED）功能。

典型的发行说明内容包括 11.2-RELEASE 之后发布的最新安全公告、新增的驱动程序或硬件支持、新命令或选项、重大错误修复或第三方软件升级。这些内容还可能包括主要 Ports/软件包的变更或发布工程的改进。显然，发行说明无法列出 FreeBSD 在版本之间的每一个变更；本文主要关注安全公告、用户可见的变更以及重要的架构改进。

## 从旧版 FreeBSD 升级

[amd64, i386] 使用 [freebsd-update(8)](https://www.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&manpath=freebsd-release-ports) 工具可以在不同 RELEASE 版本之间（以及各种安全分支的快照之间）进行二进制升级。二进制升级过程会更新未修改的用户空间工具和官方 FreeBSD 发行版中分发的未修改 **GENERIC** 内核。该工具要求待升级的主机具备互联网连接。

同样支持从之前版本进行基于源码的升级（即通过从源码重新编译 FreeBSD 基本系统的方式升级），可参考 **/usr/src/UPDATING** 中的说明。
...
| [FreeBSD-EN-18:13.icmp](https://www.freebsd.org/security/advisories/FreeBSD-EN-18:13.icmp.asc) | 2018 年 11 月 27 日 | ICMP 缓冲区下溢漏洞 |
...
[jail(8)](https://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8&manpath=freebsd-release-ports) 工具已更新，新增 [jail.conf(5)](https://www.freebsd.org/cgi/man.cgi?query=jail.conf&sektion=5&manpath=freebsd-release-ports) 参数 `allow.read_msgbuf`，该参数可防止 Jail 中的进程和用户访问 [dmesg(8)](https://www.freebsd.org/cgi/man.cgi?query=dmesg&sektion=8&manpath=freebsd-release-ports) 缓冲区。该参数默认设置为 `false`。[(r339446)](http://svn.freebsd.org/viewvc/base?view=revision&revision=339446)

系统的 [crontab(5)](https://www.freebsd.org/cgi/man.cgi?query=crontab&sektion=5&manpath=freebsd-release-ports) 配置文件 **/etc/crontab** 已更新，设置了 `PATH` 以与 [cron(8)](https://www.freebsd.org/cgi/man.cgi?query=cron&sektion=8&manpath=freebsd-release-ports) 守护进程保持一致。[(r342103)](http://svn.freebsd.org/viewvc/base?view=revision&revision=342103)
...
为 [kenv(1)](https://www.freebsd.org/cgi/man.cgi?query=kenv&sektion=1&manpath=freebsd-release-ports) 新增了变量 `init_exec`，允许 [init(8)](https://www.freebsd.org/cgi/man.cgi?query=init&sektion=8&manpath=freebsd-release-ports) 在打开控制台后运行一个可执行文件，并替代 [init(8)](https://www.freebsd.org/cgi/man.cgi?query=init&sektion=8&manpath=freebsd-release-ports) 作为 PID 1。[(r346479)](http://svn.freebsd.org/viewvc/base?view=revision&revision=346479)
...
[newfs_msdos(8)](https://www.freebsd.org/cgi/man.cgi?query=newfs_msdos&sektion=8&manpath=freebsd-release-ports) 工具已更新，新增了 `-T` 标志，用于指定时间戳以实现构建可重复性。[(r336328)](http://svn.freebsd.org/viewvc/base?view=revision&revision=336328)

[dd(1)](https://www.freebsd.org/cgi/man.cgi?query=dd&sektion=1&manpath=freebsd-release-ports) 工具新增操作数 `status=progress`，可每秒以单行显示当前状态。[(r338364)](http://svn.freebsd.org/viewvc/base?view=revision&revision=338364)
...
[jail(8)](https://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8&manpath=freebsd-release-ports) 工具已更新，新增标志 `-e`，该标志以 [jail.conf(5)](https://www.freebsd.org/cgi/man.cgi?query=jail.conf&sektion=5&manpath=freebsd-release-ports) 参数作为输入，并打印具有指定参数的非通配符 Jail 列表。[(r341790)](http://svn.freebsd.org/viewvc/base?view=revision&revision=341790)

[ktrdump(8)](https://www.freebsd.org/cgi/man.cgi?query=ktrdump&sektion=8&manpath=freebsd-release-ports) 工具已更新，新增了 `-l` 标志，指定时启用“实时”模式。[(r342706)](http://svn.freebsd.org/viewvc/base?view=revision&revision=342706)
...
[gzip(1)](https://www.freebsd.org/cgi/man.cgi?query=gzip&sektion=1&manpath=freebsd-release-ports) 工具已更新，新增对 [xz(1)](https://www.freebsd.org/cgi/man.cgi?query=xz&sektion=1&manpath=freebsd-release-ports) 文件的 `-l` 支持。[(r343251)](http://svn.freebsd.org/viewvc/base?view=revision&revision=343251)

[newfs(8)](https://www.freebsd.org/cgi/man.cgi?query=newfs&sektion=8&manpath=freebsd-release-ports) 和 [tunefs(8)](https://www.freebsd.org/cgi/man.cgi?query=tunefs&sektion=8&manpath=freebsd-release-ports) 工具已更新，允许标签名称中使用下划线。[(r343538)](http://svn.freebsd.org/viewvc/base?view=revision&revision=343538)（由 Netflix 赞助）
...
向 **/etc/rc.initdiskless** 添加了对辅助 RAM 的支持。[(r340611)](http://svn.freebsd.org/viewvc/base?view=revision&revision=340611)
...
向 [rc.subr(8)](https://www.freebsd.org/cgi/man.cgi?query=rc.subr&sektion=8&manpath=freebsd-release-ports) 添加变量 `rc_service`，默认值为执行服务的路径，以便在服务需要重新调用自身时使用。[(r343046)](http://svn.freebsd.org/viewvc/base?view=revision&revision=343046)
...
已新增 [pthread_get_name_np(3)](https://www.freebsd.org/cgi/man.cgi?query=pthread_get_name_np&sektion=3&manpath=freebsd-release-ports) 函数，用于检索与线程关联的线程名。[(r338405)](http://svn.freebsd.org/viewvc/base?view=revision&revision=338405)
...
MSI IRQ 数量已从常量转换为可调参数。默认值仍为 `512`，现在可以通过 [sysctl(8)](https://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&manpath=freebsd-release-ports) `machdep.num_msi_irqs` 在启动时变更。[(r342656)](http://svn.freebsd.org/viewvc/base?view=revision&revision=342656)

内核在记录进程退出时，现在会一并记录 [jail(8)](https://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8&manpath=freebsd-release-ports) ID。ID `0` 代表未 Jail 的进程。[(r343084)](http://svn.freebsd.org/viewvc/base?view=revision&revision=343084)（由 Modirum MDPay 赞助）

对于未来版本中将弃用的功能，现在会在所有 FreeBSD 版本中打印警告。[(r348753)](http://svn.freebsd.org/viewvc/base?view=revision&revision=348753)
...
已标记驱动程序 **ae(4)**、**bm(4)**、**cs(4)**、**de(4)**、**dme(4)**、**ed(4)**、**ep(4)**、**ex(4)**、**fe(4)**、**pcn(4)**、**sf(4)**、**sn(4)**、**tl(4)**、**tx(4)**、**txp(4)**、**vx(4)**、**wb(4)** 和 **xe(4)** 为弃用，且在 FreeBSD 13.0 中不存在。[(r347962)](http://svn.freebsd.org/viewvc/base?view=revision&revision=347962)
...
新增 [ccr(4)](https://www.freebsd.org/cgi/man.cgi?query=ccr&sektion=4&manpath=freebsd-release-ports) 驱动程序，支持 Chelsio T6™ 加密加速器。[(r345040)](http://svn.freebsd.org/viewvc/base?view=revision&revision=345040)（由 Chelsio Communications 赞助）
...
在创建 [geli(8)](https://www.freebsd.org/cgi/man.cgi?query=geli&sektion=8&manpath=freebsd-release-ports) 提供者时，添加了对较弱算法的弃用警告。[(r348588)](http://svn.freebsd.org/viewvc/base?view=revision&revision=348588)
...
ZFS 文件系统已更新，实现了并行挂载。[(r346690)](http://svn.freebsd.org/viewvc/base?view=revision&revision=346690)（由 Gandi.net 赞助）
...
`zfsloader` 提供的功能已添加到 [loader(8)](https://www.freebsd.org/cgi/man.cgi?query=loader&sektion=8&manpath=freebsd-release-ports) 中。在按照 `UPDATING` 更新系统引导块之后，就不再需要 `zfsloader`。为了方便过渡，添加了指向 [loader(8)](https://www.freebsd.org/cgi/man.cgi?query=loader&sektion=8&manpath=freebsd-release-ports) 的硬链接。[(r344399)](http://svn.freebsd.org/viewvc/base?view=revision&revision=344399)
...
虚拟机磁盘镜像的默认大小已从 30GB 减少到 3GB。`raw` 磁盘镜像可以通过 [truncate(1)](https://www.freebsd.org/cgi/man.cgi?query=truncate&sektion=1&manpath=freebsd-release-ports) 调整大小，之后 [rc(8)](https://www.freebsd.org/cgi/man.cgi?query=rc&sektion=8&manpath=freebsd-release-ports) 脚本 `growfs` 将调整虚拟机中的文件系统大小。其他磁盘镜像格式应使用所用虚拟化监视器提供的适当工具调整大小。[(r347037)](http://svn.freebsd.org/viewvc/base?view=revision&revision=347037)（由 FreeBSD 基金会赞助）
