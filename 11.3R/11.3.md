...
典型的发行说明内容包括 11.2-RELEASE 之后发布的最新安全公告、新增的驱动程序或硬件支持、新命令或选项、重大错误修复或第三方软件升级。这些内容还可能包括主要 Ports/软件包的变更或发布工程的改进。显然，发行说明无法列出 FreeBSD 在版本之间的每一项变更；本文主要关注安全公告、用户可见的变更以及重要的架构改进。
...
[newfs_msdos(8)](https://www.freebsd.org/cgi/man.cgi?query=newfs_msdos&sektion=8&manpath=freebsd-release-ports) 工具已更新，新增了标志 `-T`，用于指定时间戳以实现构建可重复性。[(r336328)](http://svn.freebsd.org/viewvc/base?view=revision&revision=336328)
...
[ktrdump(8)](https://www.freebsd.org/cgi/man.cgi?query=ktrdump&sektion=8&manpath=freebsd-release-ports) 工具已更新，新增了标志 `-l`，指定时启用“实时”模式。[(r342706)](http://svn.freebsd.org/viewvc/base?view=revision&revision=342706)
