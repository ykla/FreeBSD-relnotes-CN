...
包含安装 FreeBSD 基本系统、文档、调试发行版和少量预构建软件包（旨在启动图形工作站）所需的全部内容。还支持启动到基于“livefs”的救援模式。若你能刻烧并使用 DVD 介质，这应是你所需的全部。
...
```sh
# dd if=FreeBSD-11.3-RELEASE-amd64-dvd1.iso \
...
```sh
# dd if=FreeBSD-11.3-RELEASE-amd64-disc1.iso \
    of=/dev/da0 bs=10240 conv=sync
```
...
* <https://download.freebsd.org/releases/ISO-IMAGES/11.3/>

FreeBSD 11.3-RELEASE 虚拟机镜像可从以下地址下载：

* <https://download.freebsd.org/releases/VM-IMAGES/11.3-RELEASE/>
