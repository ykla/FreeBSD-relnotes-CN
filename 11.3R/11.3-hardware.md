...
[amd64, i386, pc98, sparc64] IDE/ATA 控制器（[ata(4)](https://man.freebsd.org/cgi/man.cgi?query=ata&sektion=4&format=html) 驱动）
...
基于 Realtek RTL8188CE 的 PCIe IEEE 802.11b/g/n 无线网络适配器（[rtwn(4)](https://man.freebsd.org/cgi/man.cgi?query=rtwn&sektion=4&format=html) 驱动）
...
[i386, amd64] 支持 Specialix SI/XIO/SX 多端口串行卡，包括较早的 SIHOST2.x 和“增强型”（基于 transputer，又称 JET）主机卡（ISA、EISA 和 PCI）。注意，较新的 SX+ PCI 卡目前不受支持。（[si(4)](https://man.freebsd.org/cgi/man.cgi?query=si&sektion=4&format=html) 驱动）
