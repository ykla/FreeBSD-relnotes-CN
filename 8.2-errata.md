...
```ini
$FreeBSD: stable/8/release/doc/en_US.ISO8859-1/errata/article.sgml 218978 2011-02-23 19:33:42Z hrs $
```
...
本文列出 FreeBSD 8.2-RELEASE 的勘误项，包含发行后或在发行周期后期发现的、未能纳入发行版文档的重要信息。这些信息包括安全公告，以及与可能影响软件或文档运行或可用性相关的消息。在安装本版本 FreeBSD 之前，应始终查阅本文档的最新版本。

FreeBSD 8.2-RELEASE 的勘误文档将维护至 FreeBSD 8.3-RELEASE 发布。

## 1 引言

本勘误文档包含关于 FreeBSD 8.2-RELEASE 的“最新消息”。在安装本版本之前，务必查阅本文档，以了解可能在发布后已经发现并修复的问题。
...
在 8.2-RELEASE 发行流程的后期阶段，发现 **OpenSSL** 中一个可能导致其解析越过消息末尾的 bug。8.2-RELEASE 通过从 OpenSSL CVS 导入相关部分修复了此问题。此问题可由格式不正确的 ClientHello SSL/TLS 握手消息触发。详细信息请参阅 <http://www.openssl.org/news/secadv_20110208.txt>。
