...
**发行说明 - FreeBSD 3.4-RELEASE**

欢迎使用 3.4-RELEASE，这是继 1999 年 10 月发布的 3.3-RELEASE 之后的完整后续版本。在 3.3 发布后的几个月内，系统进行了许多错误修复和功能增强。相关详细信息请参见下文。

任何安装失败或系统崩溃问题，请通过命令 `send-pr` 提交报告（如果你更喜欢基于网页的界面，也可以访问 [http://www.FreeBSD.org/send-pr.html](http://www.FreeBSD.org/send-pr.html)）。

有关 FreeBSD 及 3.4-RELEASE 目录布局的信息（尤其是在使用软盘安装时），请参阅文件 **ABOUT.TXT**。有关安装说明，请参阅文件 **INSTALL.TXT** 和 **HARDWARE.TXT**。

## 1. 自 3.3-RELEASE 以来的更新内容

### 1.1. 内核变更

- 恢复了对 Adaptec 152x/151x/AIC-6360 SCSI 控制器的支持。
- 新增 **netgraph(4)** 支持。Netgraph 提供了统一且模块化的系统，用于实现执行各种网络功能的内核对象。要启用此功能，请在内核配置中新增 `options NETGRAPH`。
- **i4b(4)** 升级至版本 00.83.00，提高了稳健性和稳定性，并支持多款新卡（包括 Asuscom ISDNlink 128K、AVM Fritz!Card PCI、AVM Fritz!Card PnP、ELSA PCC-16、ITK ix1 micro V.3、Siemens I-Surf 2.0）。
- **vinum(8)** 现在新增对 RAID-5 的支持。
- 增加了对 Intel PIIX4 和 AcerLabs M15x3 电源管理控制器的驱动支持。

### 1.2. 安全性变更

- 新增支持阻止以下内容：传入的 ICMP 重定向消息、外发的 RST 数据包以及传入的 SYN|FIN 数据包，从而减轻或消除某些类型的 DoS 攻击的影响。
...
- 新的网络安全功能包括：
  ...
  - 如何使用这些新功能，详见 **rc.conf(5)**。

### 1.3. 用户空间变更

- **Inetd** 增加了对 **ident** 的内置支持，并针对意外阻塞（在接受连接时）提供了解决方案。
- **ppp(8)** 增加了对 PPPoE（以太网上的 PPP，由 pppoe netgraph 节点提供）的支持，同时支持 PPP over ISDN，包括标准的 ISDN 链路绑定。
  - 现在使用命令行参数 `-nat` 和命令 `nat` 控制网络地址转换（NAT）。旧的命令 `[-]alias` 仍然有效，但会产生警告，并将在未来移除。
  - 新功能包括：
    - 自动加载 `tun` 模块并支持在命令行指定 `tun` 单元。
    - 提供命令行选项 `-foreground`。
    - 支持快速队列配置。
    - 提供功能性的命令 `set autoload`（用于按需多链路 PPP）。
    - 支持 GRE 数据包过滤。
    - 修复了许多已知问题。
- 新增了守护进程 **pppoed**，用于处理 PPPoE 请求。详细信息请参阅 **rc.conf(5)** 和 **pppoed(8)**。

## 2. 支持的配置

FreeBSD 目前可以运行在多种基于 ISA、VLB、EISA 和 PCI 总线的 PC 上，支持从 386sx 到 Pentium 级别的机器（虽然不推荐使用 386sx）。支持常规的 IDE 或 ESDI 驱动配置，以及多种 SCSI 控制器、网卡和串口卡。
...
- Adaptec AIC7850, AIC7860, AIC7880, AIC789x 系列板载 SCSI 控制器
...
支持与 BusLogic MultiMaster 完全兼容的真正的 AMI FastDisk 控制器。
...
- SCSI-I 和 SCSI-II 外围设备（硬盘、光盘、磁带机（包括 DAT 和 8mm Exabyte）、介质交换器、处理器目标设备和 CD-ROM 驱动器）
...
**注意**：UltraStor 驱动正在迁移至新 CAM SCSI 框架，但何时或能否完成尚无定论。

##### 未维护的驱动，可能正常工作，也可能无法工作
...
型号：98713, 98713A, 98715, 98715A 和 98725，包括：
...
- 基于 DEC DC21040、DC21041 或 DC21140 的 NIC（如 SMC Etherpower 8432T、DE245 等）
...
你可以通过 `ftp.FreeBSD.org`（FreeBSD 官方发布站点）使用 FTP 下载 FreeBSD 及其任意或全部可选包。
...
如果你无法访问互联网，但电子邮件是你唯一的求助方式，仍可通过向 `ftpmail@ftpmail.vix.com` 发送邮件获取文件。邮件中请包含关键字“help”，以获取有关如何通过此机制获取文件的详细信息。但请注意，这种方式会通过邮件发送 *几十兆字节* 的数据，因此仅在万不得已的情况下使用！
...
每张 `-RELEASE` 光盘的费用为 $39.95 或 $24.95（如果你有 FreeBSD 订阅）。如果有 FreeBSD SNAPshot 光盘发售，其价格为 $39.95，订阅用户价格为 $14.95（`-RELEASE` 和 `-SNAP` 的订阅是完全独立的）。订阅后，你会在每次发布更新时自动收到新版本。信用卡将在每张光盘发货时扣费，你可以随时取消订阅，无需承担后续费用。
...
- 可执行文件体积更小
- 能够访问更多已经移植到其他 ELF 环境的编译器特性（旧的 a.out 格式较为陈旧，许多其他软件项目已不再支持）。
...
1. **不要使用旧版本的安装盘或 sysinstall**
   ...
   - FreeBSD 3.2 进一步修改了引导加载程序机制，因此旧版本的安装工具无法正常工作。
...
如果你的机器有互联网邮件连接，提交错误报告的首选方式是使用命令 `send-pr`，或者使用 CGI 脚本 [http://www.FreeBSD.org/send-pr.html](http://www.FreeBSD.org/send-pr.html)。我们忠实的 bugfiler 程序会妥善归档错误报告，你可以放心，我们会尽力尽快响应所有报告的错误。通过这种方式提交的错误报告也会在我们的网站的支持部分公开，因此不仅作为错误报告有价值，而且还能为其他用户提供潜在问题的“路标”。

如果由于某些原因你无法使用命令 `send-pr` 提交错误报告，可以尝试将报告发送至：`freebsd-bugs@FreeBSD.org`

请注意，`send-pr` 本身是一个 shell 脚本，应该很容易移植到完全不同的系统上。如果你能使用此接口，我们将更为欢迎，因为它有助于我们更好地追踪问题报告。不过，在提交报告之前，请确认该问题是否已被修复。
...
此外，由于这是一个志愿者项目，我们始终欢迎更多愿意提供帮助的人——目前我们面临的期望增强功能远远超过了我们能够独立管理的范围！如果你希望与我们讨论技术问题，或提供帮助，请发送邮件至：`freebsd-hackers@FreeBSD.org`
...
FreeBSD 凝聚了来自世界各地的数十甚至数百位贡献者的努力，他们辛勤工作，才有了今天这个版本。要查看完整的 FreeBSD 项目工作人员名单，请访问：[http://www.FreeBSD.org/handbook/staff.html](http://www.FreeBSD.org/handbook/staff.html)
