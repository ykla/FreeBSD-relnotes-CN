...
- [ipfw(4)](https://man.freebsd.org/cgi/man.cgi?query=ipfw&sektion=4&format=html) 数据包过滤规则语法中的参数 `table` 现在支持 IP 地址、接口名称、端口号和 Jail ID。[r234597](http://svn.freebsd.org/viewvc/base?view=revision&revision=234597)
...
- FreeBSD 的 ZFS 文件系统通过合并 illumos 项目的改进更新了。以下是所做的更改：[r229578](http://svn.freebsd.org/viewvc/base?view=revision&revision=229578), [r232328](http://svn.freebsd.org/viewvc/base?view=revision&revision=232328)
  ...
  - [zfs(8)](https://man.freebsd.org/cgi/man.cgi?query=zfs&sektion=8&format=html) 命令 `get` 现在支持 `-t type` 参数，用于指定数据类型。
