...
工具 [daemon(8)](https://man.freebsd.org/cgi/man.cgi?query=daemon&sektion=8&format=html) 现在支持 `-H` 选项，用于在接收到 SIGHUP 信号时关闭并重新打开输出文件。这允许通过 [newsyslog(8)](https://man.freebsd.org/cgi/man.cgi?query=newsyslog&sektion=8&format=html) 实现输出文件轮换。[4cd407ec933](https://cgit.freebsd.org/src/commit/?id=4cd407ec933)
...
工具 [freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 现在支持 `-p` 选项，以通过 [pwd_mkdb(8)](https://man.freebsd.org/cgi/man.cgi?query=pwd_mkdb&sektion=8&format=html) 确保 **/etc/passwd** 包含密码数据库的变更。[9b6591109e8](https://cgit.freebsd.org/src/commit/?id=9b6591109e8)（由 FreeBSD 基金会赞助）
...
移除了 [cryptodev(4)](https://man.freebsd.org/cgi/man.cgi?query=cryptodev&sektion=4&format=html) 和内核加密框架中此前弃用的加密算法支持。
...
[ctld(8)](https://man.freebsd.org/cgi/man.cgi?query=ctld&sektion=8&format=html) 工具现在支持 `-t` 选项，用于测试配置文件的有效性。[7fcbecd004f](https://cgit.freebsd.org/src/commit/?id=7fcbecd004f)
...
默认情况下，尝试读取目录将失败并返回 `EISDIR`。使用 [grep(1)](https://man.freebsd.org/cgi/man.cgi?query=grep&sektion=1&format=html) 的 `-d skip` 选项可以在非递归地搜索包含目录的列表时，抑制 stderr 中的错误。[dcef4f65ae39](https://cgit.freebsd.org/src/commit/?id=dcef4f65ae39)
...
工具 [efibootmgr(8)](https://man.freebsd.org/cgi/man.cgi?query=efibootmgr&sektion=8&format=html) 现在支持 `-b` 选项，用于指定创建或修改的特定引导条目索引。[a2581e80212](https://cgit.freebsd.org/src/commit/?id=a2581e80212)（由 Netflix 赞助）

工具 [efibootmgr(8)](https://man.freebsd.org/cgi/man.cgi?query=efibootmgr&sektion=8&format=html) 现在支持 `-E` 选项，用于查询引导系统时使用的 EFI 系统分区。[1cdb8eb8fe1](https://cgit.freebsd.org/src/commit/?id=1cdb8eb8fe1)（由 Netflix 赞助）

工具 [efibootmgr(8)](https://man.freebsd.org/cgi/man.cgi?query=efibootmgr&sektion=8&format=html) 现在支持 `-f` 和 `-F` 选项，用于设置或清除请求在下次引导时进入 UEFI 用户界面的选项。[83c4237258d](https://cgit.freebsd.org/src/commit/?id=83c4237258d)（由 Ampere Computing, Inc. 赞助）

此前版本中 **/boot/boot1.efifat** 包含完整的 MS-DOS 格式化文件系统。旧版 FreeBSD 会将此文件系统映像安装到原始分区中。然而，由于 ESP（EFI 系统分区）的使用日益广泛，这种不灵活的方式已不再适合用户需求。用户对分区大小的需求多种多样，多重引导设置需要更详细的访问方式。要更新旧 ESP 分区，用户应停止使用 [gpart(8)](https://man.freebsd.org/cgi/man.cgi?query=gpart&sektion=8&format=html) 工具。取而代之的是，应将 ESP 分区作为 MS-DOS 文件系统挂载到 **/boot/efi**，如果使用默认设置，则应将 **/boot/loader.efi** 复制到 **/boot/efi/efi/boot/bootx64.efi**。如果使用 [efibootmgr(8)](https://man.freebsd.org/cgi/man.cgi?query=efibootmgr&sektion=8&format=html) 自定义引导环境，应将此文件复制到使用 `-l` 选项设置的位置。
