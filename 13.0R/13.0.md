...
> **重要**：UEFI ESP 分区（固件引导所使用的分区）的更新方式已变更。详见 [引导加载程序的变更](https://www.freebsd.org/releases/13.0R/relnotes/#boot)。
...
用于 [crashinfo(8)](https://man.freebsd.org/cgi/man.cgi?query=crashinfo&sektion=8&format=html) 的旧版 GNU 调试器（原安装于 **/usr/libexec**）已删除。详细的内核崩溃信息可通过从 Ports 或软件包安装现代 GDB 获得。[1c0ea326aa6d](https://cgit.freebsd.org/src/commit/?id=1c0ea326aa6d)
...
函数 [cap_random(3)](https://man.freebsd.org/cgi/man.cgi?query=cap_random&sektion=3&format=html) 已删除，因为它已被 [getrandom(2)](https://man.freebsd.org/cgi/man.cgi?query=getrandom&sektion=2&format=html) 取代。[a76f78dc3f43](https://cgit.freebsd.org/src/commit/?id=a76f78dc3f43)
...
[cryptodev(4)](https://man.freebsd.org/cgi/man.cgi?query=cryptodev&sektion=4&format=html) 和内核加密框架中此前已弃用的加密算法支持已移除。
...
**GEOM_SCHED** 类及其附带的工具 [gsched(8)](https://man.freebsd.org/cgi/man.cgi?query=gsched&sektion=8&format=html) 已移除。[86c06ff8864](https://cgit.freebsd.org/src/commit/?id=86c06ff8864)
...
此前版本中 **/boot/boot1.efifat** 包含完整的 MS-DOS 格式化文件系统。旧版 FreeBSD 会将此文件系统映像安装到原始分区中。然而，由于 ESP（EFI 系统分区）的使用日益广泛，这种不灵活的方式已不再适合用户需求。用户对分区大小的需求多种多样，多重引导设置需要更详细的访问方式。要更新旧 ESP 分区，用户应停止使用 [gpart(8)](https://man.freebsd.org/cgi/man.cgi?query=gpart&sektion=8&format=html) 工具。取而代之的是，应将 ESP 分区作为 MS-DOS 文件系统挂载到 **/boot/efi**，如果使用默认设置，则应将 **/boot/loader.efi** 复制到 **/boot/efi/efi/boot/bootx64.efi**。如果使用 [efibootmgr(8)](https://man.freebsd.org/cgi/man.cgi?query=efibootmgr&sektion=8&format=html) 自定义引导环境，应将此文件复制到使用 `-l` 参数设置的位置。
...
用于禁用 IPv6 scope 解嵌的 sysctl `net.inet6.ip6.deembed_scopeid` 已移除。[bec053ffe01d](https://cgit.freebsd.org/src/commit/?id=bec053ffe01d)
...
其他大量性能改进——在批量构建所有软件包时，性能至少提升 60%。
