...
所有内置 USB 的 Apple PowerPC 机器均受支持，此外还支持少量非 Apple 机器，包括 POWER7 上的 KVM。
...
* Intel 和 ICP RAID 控制器的所有过往和未来版本。
...
SAS 控制器芯片也存在于许多新的 AMD/Opteron 系统上，例如 Sun 4100。注意，此控制器可同时驱动 SAS 和 SATA 硬盘或二者混合。这些控制器可用的集成 RAID 镜像至多只能算是有限支持。
...
[bwi(4)](https://man.freebsd.org/cgi/man.cgi?query=bwi&sektion=4&format=html) 驱动使用 Broadcom 旧版 v3 固件。虽然此旧固件支持大多数 BCM43xx 部件，但 [bwn(4)](https://man.freebsd.org/cgi/man.cgi?query=bwn&sektion=4&format=html) 驱动对其支持的较新芯片表现更好。如果使用较旧的 Broadcom 芯片组（BCM4301、BCM4303 和 BCM4306 rev 2），必须使用 [bwi(4)](https://man.freebsd.org/cgi/man.cgi?query=bwi&sektion=4&format=html) 驱动。[bwn(4)](https://man.freebsd.org/cgi/man.cgi?query=bwn&sektion=4&format=html) 使用的 v4 版本的固件不支持这些芯片。
...
* Creative Sound Blaster Live! 24-Bit，FreeBSD 标识为
* Creative Sound Blaster Audigy LS / ES，FreeBSD 标识为
