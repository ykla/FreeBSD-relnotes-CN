...
* 内核现在支持在 TCP 套接字上对 TLS 1.0 至 1.3 版本的传输层安全（TLS）数据进行内核内成帧和加密。对于使用 AES-CBC 的 MtE 密码套件以及使用 AES-GCM 的 AEAD 密码套件，支持通过内核内加密驱动进行传输卸载。对于 TLS 1.2 的 AES-GCM 密码套件，支持通过内核内加密驱动进行接收卸载。使用 KTLS 需要支持 KTLS 的用户态 SSL 库。基本系统中包含的 OpenSSL 库默认不启用 KTLS 支持，但可通过使用 `WITH_OPENSSL_KTLS` 选项构建来启用支持。
