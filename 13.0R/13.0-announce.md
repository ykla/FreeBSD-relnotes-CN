# FreeBSD 13.0-RELEASE 发行公告

日期：2021 年 4 月 13 日

FreeBSD 发布工程团队很高兴地宣布 FreeBSD 13.0-RELEASE 现已发布。这是 stable/13 分支的第一个版本。

部分亮点如下：

* clang、lld 和 lldb 工具以及 compiler-rt、llvm、libunwind 和 libc++ 库升级至 11.0.1 版本。
* 移除了安装到 **/usr/libexec** 供 [crashinfo(8)](https://man.freebsd.org/cgi/man.cgi?query=crashinfo&sektion=8&format=html) 使用的过时 GNU 调试器版本。可通过从 Ports 或软件包安装现代 GDB 来获取详细的内核崩溃信息。
* 从源代码树中移除了过时的 binutils 2.17 和 [gcc(1)](https://man.freebsd.org/cgi/man.cgi?query=gcc&sektion=1&format=html) 4.2.1。所有受支持的架构现在均使用 LLVM/clang 工具链。
* 现在默认安装 BSD 版本的 [grep(1)](https://man.freebsd.org/cgi/man.cgi?query=grep&sektion=1&format=html)。此前默认的过时 GNU 版本已被移除。
* 从 [libalias(3)](https://man.freebsd.org/cgi/man.cgi?query=libalias&sektion=3&format=html) 中移除了 CU-SeeMe 支持。
* 新增 [qat(4)](https://man.freebsd.org/cgi/man.cgi?query=qat&sektion=4&format=html) 驱动，支持 Intel QuickAssist (QAT) 设备的部分加密加速功能。[qat(4)](https://man.freebsd.org/cgi/man.cgi?query=qat&sektion=4&format=html) 驱动支持集成于 Atom C2000 和 C3000 以及 Xeon C620 和 D-1500 平台的 QAT 设备，以及 Intel QAT Adapter 8950。
* 移除了若干弃用的驱动。
* 若干驱动移植至 PowerPC64 架构。
* 内核现在支持在 TCP 套接字上对 TLS 1.0 至 1.3 版本的传输层安全（TLS）数据进行内核内成帧和加密。对于使用 AES-CBC 的 MtE 密码套件以及使用 AES-GCM 的 AEAD 密码套件，支持通过内核内加密驱动进行传输卸载。对于 TLS 1.2 的 AES-GCM 密码套件，支持通过内核内加密驱动进行接收卸载。使用 KTLS 需要支持 KTLS 的用户态 SSL 库。基本系统中包含的 OpenSSL 库默认不启用 KTLS 支持，但可通过使用选项 `WITH_OPENSSL_KTLS` 构建来启用支持。
* 64 位 ARM 架构（即 arm64 或 AArch64）在 FreeBSD 13 中提升至 Tier-1 状态。
* 以及更多……
...
FreeBSD 13.0-RELEASE 现已支持 amd64、i386、powerpc、powerpc64、powerpc64le、powerpcspe、armv6、armv7、aarch64 和 riscv64 架构。

FreeBSD 13.0-RELEASE 可从可启动 ISO 镜像或通过网络安装。部分架构还支持从 USB 闪存盘安装。所需文件可按以下说明下载。
...
FreeBSD 13.0-RELEASE 也可从多家供应商处购买 CD-ROM 或 DVD 介质。提供 FreeBSD 13.0 相关产品的供应商之一是：

* FreeBSD Mall, Inc. <https://www.freebsdmall.com>

预安装的虚拟机镜像也可用于 amd64（x86_64）、i386（x86_32）、AArch64（arm64）和 RISCV 架构，提供 `QCOW2`、`VHD` 和 `VMDK` 磁盘镜像格式，以及原始（未格式化）镜像。

FreeBSD 13.0-RELEASE amd64 还可在以下云托管平台上使用：
...
AMI 也可在 Amazon Marketplace 中获取：<https://aws.amazon.com/marketplace/pp/B0928XNW6D>

FreeBSD/arm64 Amazon EC2(TM)：AMI 在以下区域可用：
...
AMI 也可在 Amazon Marketplace 中获取：<https://aws.amazon.com/marketplace/pp/B09291VW11>
...
FreeBSD 13.0-RELEASE 预计还将在 Google Compute Engine(TM) Marketplace 中提供，待其完成第三方特定验证后，访问：<https://console.cloud.google.com/launcher/browse?filter=category:os&filter=price:free>
...
FreeBSD 13.0-RELEASE 可从以下站点通过 https 下载：

* <https://download.freebsd.org/ftp/releases/ISO-IMAGES/13.0/>

FreeBSD 13.0-RELEASE 虚拟机镜像可从以下地址下载：

* <https://download.freebsd.org/ftp/releases/VM-IMAGES/13.0-RELEASE/>

FreeBSD 13.0-RELEASE amd64 BASIC-CI 镜像可从以下地址下载：

* <https://download.freebsd.org/ftp/releases/CI-IMAGES/13.0-RELEASE/>

关于安装 FreeBSD 或将现有机器更新至 13.0-RELEASE 的说明，请参阅：

* <https://www.FreeBSD.org/releases/13.0R/installation/>

## 支持

根据新的 FreeBSD 支持模型，FreeBSD 13 发行系列将至少支持至 2026 年 1 月 31 日。本点发行版 FreeBSD 13.0-RELEASE 将至少支持至 FreeBSD 13.1-RELEASE 发布后三个月。其他支持信息可在以下地址找到：

* <https://www.FreeBSD.org/security/>

请注意，12.2 将支持至 13.0 发行日期起三个月后，截至本文撰写时该日期尚未确定。

## 致谢

许多公司捐赠了设备、网络访问或人力时间来支持 FreeBSD 13.0 的发布工程活动，包括：

| 供应商 |
| :----- |
| [FreeBSD 基金会](https://www.freebsdfoundation.org) |
| [Rubicon Communications, LLC (Netgate)](https://www.netgate.com) |
...
13.0-RELEASE 的发布工程团队包括：

| 成员 | 角色 |
| :--- | :--- |
| Glen Barber <gjb@FreeBSD.org> | 发布工程负责人，13.0-RELEASE 发布工程师 |
...
| Colin Percival <cperciva@FreeBSD.org> | 发布工程副负责人 |
...
```sh
SHA512 (FreeBSD-13.0-RELEASE-arm64-aarch64-RPI.img.xz) = ff4c97003eb59e27ad193657cced8914702bd2482136bf66341e97782b0e03cfd751c939d91e31b1b5a5e0980f9db3a3425d802699499427915e38ea0b6452d4d
...
喜欢 FreeBSD？请通过向 FreeBSD 基金会[捐赠](https://www.freebsdfoundation.org/donate/) 来支持本版本及未来版本！
