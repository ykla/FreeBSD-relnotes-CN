...
`caroot` CA 捆绑处理器已更新，以支持带有 `DISTRUST_AFTER` 条目的证书。
...
实用程序 [bhyve(8)](https://man.freebsd.org/cgi/man.cgi?query=bhyve&sektion=8&format=html) 中 [nvme(4)](https://man.freebsd.org/cgi/man.cgi?query=nvme&sektion=4&format=html) 模拟的大规模 IO 支持已修复。
...
实用程序 [mksnap_ffs(8)](https://man.freebsd.org/cgi/man.cgi?query=mksnap_ffs&sektion=8&format=html) 中导致 `Panic: snapacct_ufs2: bad block` 的崩溃问题已修复。
...
[unzip(1)](https://man.freebsd.org/cgi/man.cgi?query=unzip&sektion=1&format=html) 在目标归档文件包含损坏的名称时引发的段错误已修复。
...
驱动程序 [cam(4)](https://man.freebsd.org/cgi/man.cgi?query=cam&sektion=4&format=html) 的 SCSI 快速拔插问题已修复。
...
设备驱动程序 [ixgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=ixgbe&sektion=4&format=html) 的 x550em 10G NIC 链接状态问题已修复，其中自动协商功能未正确报告。

设备驱动程序 [ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 新增了可调参数 `hw.ix.flow_control`。

设备驱动程序 [ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 的共享代码已更新，2.5G 和 5G 网络速度问题也已修复。

设备驱动程序 [iwm(4)](https://man.freebsd.org/cgi/man.cgi?query=iwm&sektion=4&format=html) 现支持 Intel® Killer® Wireless-AC 1550i。

文件系统驱动程序 [msdosfs(5)](https://man.freebsd.org/cgi/man.cgi?query=msdosfs&sektion=5&format=html) 的挂起问题已修复。
...
设备驱动程序 [nvme(4)](https://man.freebsd.org/cgi/man.cgi?query=nvme&sektion=4&format=html) 已合并多个错误修复。
...
在 `wpa` EAP/PEAP MSCHAPv2 身份验证过程中发生的段错误已修复。
...
对记录 EC2 AMI Ids 的支持已添加到 `release/Makefile.ec2`，以允许 SSM 参数名称采用类似 **/aws/service/freebsd/amd64/base/ufs/12.3/RELEASE** 的格式，使用公共前缀 **/aws/service/freebsd**。
