# FreeBSD 13.4-RELEASE 硬件兼容列表

## 引言

本文档包含 FreeBSD 13.4 的硬件兼容性说明。其中列出了 FreeBSD 支持的硬件平台，以及各类硬件设备（存储控制器、网络接口等），并给出已知可正常工作的设备实例。
...
FreeBSD 具备完善的 ARM 架构支持，增强了与各类基于 ARM 设备的兼容性和性能。
...
对于所有受支持的 SCSI 控制器，全面支持 SCSI-I、SCSI-II 和 SCSI-III 外设，包括硬盘、光盘、磁带机（包括 DAT、8mm Exabyte、Mammoth 和 DLT）、介质更换器、处理器目标设备和 CD-ROM 驱动器。支持 CD-ROM 命令的 WORM 设备可由 CD-ROM 驱动（如 [cd(4)](https://man.freebsd.org/cgi/man.cgi?query=cd&sektion=4&format=html)）进行只读访问。WORM/CD-R/CD-RW 写入支持由 [cdrecord(1)](https://man.freebsd.org/cgi/man.cgi?query=cdrecord&sektion=1&format=html) 提供，它是 Ports Collection 中 `sysutils/cdrtools` port 的一部分。
...
当前版本的 `atse` 驱动支持以太网 MegaCore，如 Altera 文档版本 11.1 中所述，当设备配置为内部 FIFO 时。
...
`genet` 驱动支持 Raspberry Pi 4 Model B 及相关系统上 Broadcom BCM2711 的以太网控制器部分。它使用 BCM54213PE PHY。

Google 虚拟网卡 (gVNIC) 以太网驱动（[gve(4)](https://man.freebsd.org/cgi/man.cgi?query=gve&sektion=4&format=html) 驱动）
...
[ixgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=ixgbe&sektion=4&format=html) 驱动支持以下网卡：
...
大多数 Intel 以太网 700 系列中带有 SFP+/SFP28/QSFP+ 接口的适配器，其固件要求使用 Intel 认证模块；这些认证模块列在下面。该认证检查无法由驱动禁用。
...
`snd_atiixp` 驱动支持以下 audio 芯片 ets：
...
`snd_hda` 驱动支持具有 PCI 类 4（多媒体）和子类 3（HDA）的控制器，兼容 Intel HDA 规范。
...
Altera 大学计划安全数据卡 IP 核心（[altera_sdcard(4)](https://man.freebsd.org/cgi/man.cgi?query=altera_sdcard&sektion=4&format=html) 驱动）
...
[ccr(4)](https://man.freebsd.org/cgi/man.cgi?query=ccr&sektion=4&format=html) 驱动支持基于 T6 ASIC 的适配器上包含的加密加速器引擎：
