...
从具有 Internet 邮件连接的机器上提交错误报告的首选方式是使用 `send-pr` 命令，或者使用位于 `http://www.FreeBSD.org/send-pr.html` 的 CGI 脚本。我们忠实的 bugfiler 程序会妥善归档错误报告，你可以放心，我们会尽力尽快响应所有报告的错误。以这种方式提交的错误报告也会在我们的网站支持部分中可见，因此既作为错误报告有价值，也作为“路标”向其他用户提醒潜在问题。
