# FreeBSD 7.2-RELEASE 硬件兼容列表

## 引言

本文档包含 FreeBSD 7.2-RELEASE 的硬件兼容性说明。其中列出了 FreeBSD 支持的硬件平台，以及各类硬件设备（存储控制器、网络接口等），并给出已知可正常工作的设备实例。
...
* 除 Sossaman 外，所有多核 Intel(R) Xeon(TM) 处理器均支持 EM64T。
...
* 部分使用 "Prescott" 核心的 Intel(R) Pentium 4 和 Celeron D 支持 EM64T。关于 Intel 处理器对 EM64T 的支持情况，请参阅 [Intel Processor Spec Finder](http://processorfinder.intel.com) 以获取权威答案。
...
在许多方面，FreeBSD/amd64 与 FreeBSD/i386 在支持的驱动方面类似。部分（尤其是较老的）驱动可能存在 64 位清洁性问题。通常，在其他 64 位平台上正常工作的驱动应该可用。

FreeBSD/amd64 是 FreeBSD 中相当年轻的平台。虽然核心 FreeBSD 内核和基本系统组件通常相当稳健，但仍可能存在粗糙之处，尤其是第三方软件包方面。
...
本节介绍 FreeBSD 在 Sun UltraSPARC(R) 平台上当前已知支持的系统。关于各种硬件设计的背景信息，请参阅 [Sun System Handbook](http://sunsolve.sun.com/handbook_pub/)。

在所有拥有 1 个以上处理器的系统上支持 SMP。

使用 `GENERIC` 内核时，未配备 [creator(4)](https://man.freebsd.org/cgi/man.cgi?query=creator&sektion=4&format=html)（Sun Creator、Sun Creator3D 和 Sun Elite3D）或 [machfb(4)](https://man.freebsd.org/cgi/man.cgi?query=machfb&sektion=4&format=html)（Sun PGX 和 Sun PGX64，以及例如 Sun Blade(TM) 100、Sun Blade 150、Sun Ultra(TM) 5 和 Sun Ultra 10 板载的 ATI Mach64 芯片）驱动所支持帧缓冲的 FreeBSD/sparc64 系统必须使用串行控制台。

如果你有不在此列表中的系统，它可能尚未经过 FreeBSD 7.2-RELEASE 测试。我们鼓励你尝试并致信 [FreeBSD SPARC porting mailing list](http://lists.FreeBSD.org/mailman/listinfo/freebsd-sparc64) 反馈结果，包括哪些设备可用、哪些不可用。

以下系统受 FreeBSD 完整支持：

* Sun Blade 100
* Sun Blade 150
* Sun Enterprise(TM) 100
* Sun Enterprise 220R
* Sun Enterprise 250
* Sun Enterprise 420R
* Sun Enterprise 450
...
* Sun Fire 280R
* Sun Fire V210
* Sun Fire V440（板载 NIC 除外）
* Sun Fire V880
* Netra 20/Netra T4

以下 Sun UltraSPARC IIIi 系统未经测试，但据信也受 FreeBSD 支持：
...
* Adaptec AHA-154xCP
* Adaptec AHA-1640
* Adaptec AHA-174x（154x 仿真模式）
* DTC 3290 SCSI 控制器（1542 仿真模式）
* Tekram SCSI 控制器（154x 仿真模式）
...
[i386] [asr(4)](https://man.freebsd.org/cgi/man.cgi?query=asr&sektion=4&format=html) 驱动目前支持的适配器包括：
...
[i386, amd64] 真正的 BusLogic MultiMaster 克隆版 AMI FastDisk 主机适配器也受 [bt(4)](https://man.freebsd.org/cgi/man.cgi?query=bt&sektion=4&format=html) 驱动支持。
...
> **注意**：支持从这些控制器启动。不支持 EISA 适配器。

[sparc64] [esp(4)](https://man.freebsd.org/cgi/man.cgi?query=esp&sektion=4&format=html) 驱动支持在各种系统和外围板卡中广泛使用的 Qlogic FAS216 和 FAS408 SCSI 控制器芯片。这包括大多数 Sun Ultra 1e 和 Ultra 2 机器中使用的 Qlogic SCSI 卡。对于 Qlogic PCI SCSI 主机适配器，应使用 isp(4) 驱动而非 [esp(4)](https://man.freebsd.org/cgi/man.cgi?query=esp&sektion=4&format=html) 驱动。
...
* 所有过往及未来的 Intel 和 ICP RAID 控制器版本。
...
[i386, ia64, amd64] 支持所有主要固件版本（2.x、3.x、4.x 和 5.x），但始终建议升级到控制器可用的最新固件。未列出的兼容 Mylex 控制器应该可用，但尚未经过验证。
...
[i386, ia64, amd64] 未列出的兼容 Mylex 控制器应该可用，但尚未经过验证。
...
这些系统还包含 Integrated Raid Mirroring 和 Integrated Raid Mirroring Enhanced，本驱动也支持这些功能。SAS 控制器芯片也出现在许多基于 AMD/Opteron 的新系统上，例如 Sun 4100。注意，此控制器可同时驱动 SAS 和 SATA 硬盘，或同时混合使用两者。这些控制器可用的 Integrated Raid Mirroring 充其量仅能得到有限支持。Fibre Channel 控制器芯片组支持多种速度和系统。Apple Fibre Channel HBA 实际上就是 FC949ES 卡。此驱动还支持 Fibre Channel 卡的目标模式。可通过 LSI Logic 固件实用程序设置核心的所需角色来启用此支持，该实用程序用于确定卡可承担的角色，无需单独编译。
...
[i386, pc98, sparc64, amd64] 以下附加板卡已知受支持：

* I-O DATA SC-98/PCI (PC-98)
* I-O DATA SC-PCI (PC-98)

[i386, pc98] [ncv(4)](https://man.freebsd.org/cgi/man.cgi?query=ncv&sektion=4&format=html) 驱动目前支持以下设备：
...
[sym(4)](https://man.freebsd.org/cgi/man.cgi?query=sym&sektion=4&format=html) 驱动支持的 SCSI 控制器可以内置于主板，也可以是以下附加板卡之一：
...
在使用所有受支持的 SCSI 控制器时，全面支持 SCSI-I、SCSI-II 和 SCSI-III 外设，包括硬盘、光盘、磁带机（包括 DAT、8mm Exabyte、Mammoth 和 DLT）、介质更换器、处理器目标设备和 CD-ROM 驱动器。支持 CD-ROM 命令的 WORM 设备可由 CD-ROM 驱动（如 [cd(4)](https://man.freebsd.org/cgi/man.cgi?query=cd&sektion=4&format=html)）以只读方式访问。WORM/CD-R/CD-RW 写入支持由 [cdrecord(1)](https://man.freebsd.org/cgi/man.cgi?query=cdrecord&sektion=1&format=html) 提供，该工具是 Ports Collection 中 [**sysutils/cdrtools**](http://www.FreeBSD.org/cgi/url.cgi?ports/sysutils/cdrtools/pkg-descr) port 的一部分。
...
[i386, amd64] [bce(4)](https://man.freebsd.org/cgi/man.cgi?query=bce&sektion=4&format=html) 驱动支持基于 Broadcom NetXtreme II 系列 Gigabit 以太网控制器的各种 NIC，包括：
...
[i386, pc98, sparc64, ia64, amd64] [bge(4)](https://man.freebsd.org/cgi/man.cgi?query=bge&sektion=4&format=html) 驱动支持基于 Broadcom BCM570x 系列 Gigabit 以太网控制器芯片的各种 NIC，包括：
...
[em(4)](https://man.freebsd.org/cgi/man.cgi?query=em&sektion=4&format=html) 驱动支持基于 Intel 82540、82541ER、82541PI、82542、82543、82544、82545、82546、82546EB、82546GB、82547、82571、82572、82573 和 82574 控制器芯片的 Gigabit 以太网适配器：
...
目前已知可与 [gem(4)](https://man.freebsd.org/cgi/man.cgi?query=gem&sektion=4&format=html) 驱动配合工作的附加卡包括：
...
[igb(4)](https://man.freebsd.org/cgi/man.cgi?query=igb&sektion=4&format=html) 驱动支持基于 Intel 82575 和 82576 控制器芯片的 Gigabit 以太网适配器：
...
[i386, amd64] [msk(4)](https://man.freebsd.org/cgi/man.cgi?query=msk&sektion=4&format=html) 驱动支持基于 Marvell/SysKonnect Yukon II 系列 Gigabit 以太网控制器芯片的各种 NIC，包括：
...
[i386, amd64] [mxge(4)](https://man.freebsd.org/cgi/man.cgi?query=mxge&sektion=4&format=html) 驱动支持基于 Myricom LANai Z8E 芯片的 10 Gigabit 以太网适配器：
...
[i386, pc98, amd64] [nge(4)](https://man.freebsd.org/cgi/man.cgi?query=nge&sektion=4&format=html) 驱动支持基于 National Semiconductor DP83820 和 DP83821 的 Gigabit 以太网适配器，包括：
...
[i386, amd64] [nve(4)](https://man.freebsd.org/cgi/man.cgi?query=nve&sektion=4&format=html) 驱动支持基于以下芯片组主板的 NVIDIA MCP 板载适配器：
...
[i386, amd64] [nxge(4)](https://man.freebsd.org/cgi/man.cgi?query=nxge&sektion=4&format=html) 驱动支持 [http://www.neterion.com/how/pricing.html](http://www.neterion.com/how/pricing.html) 中列出的 Neterion Xframe 10 Gigabit 以太网适配器。
...
[re(4)](https://man.freebsd.org/cgi/man.cgi?query=re&sektion=4&format=html) 驱动支持基于 RealTek RTL8139C+、RTL8169、RTL816xS、RTL811xS 和 RTL8101E 的快速以太网和 Gigabit 以太网适配器，包括：
...
[i386, pc98, ia64, amd64] [sis(4)](https://man.freebsd.org/cgi/man.cgi?query=sis&sektion=4&format=html) 驱动支持基于 Silicon Integrated Systems SiS 900 和 SiS 7016 的 Fast Ethernet 适配器和嵌入式控制器，以及基于 National Semiconductor DP83815（MacPhyter）和 DP83816 芯片的 Fast Ethernet 适配器。支持的适配器包括：
...
[i386, amd64] [sn(4)](https://man.freebsd.org/cgi/man.cgi?query=sn&sektion=4&format=html) 驱动支持 SMC 的 SMC 91C90、SMC 91C92、SMC 91C94、SMC 91C95、SMC 91C96、SMC91C100 和 SMC 91C100FD 芯片。Farallon EtherWave 和 EtherMac 卡有两种变体。ep(4) 驱动支持 595 和 895 卡。这些卡正面有蓝色箭头及 3Com 标志。Farallon 595a 卡正面有红色箭头，也称 EtherWave 和 EtherMac。它们由 [sn(4)](https://man.freebsd.org/cgi/man.cgi?query=sn&sektion=4&format=html) 驱动支持。
...
[i386, amd64, sparc64] [stge(4)](https://man.freebsd.org/cgi/man.cgi?query=stge&sektion=4&format=html) 驱动支持基于 Sundance/Tamarack TC9021 系列 Gigabit 以太网控制器芯片的各种 NIC，包括：
...
[i386, pc98, amd64, sparc64] 以下适配器应当也受支持，但尚未经过测试：
...
[i386, pc98, amd64] [vge(4)](https://man.freebsd.org/cgi/man.cgi?query=vge&sektion=4&format=html) 驱动支持基于 VIA Networking VT3119 和 VT6122 的 Gigabit 以太网适配器，包括：
...
[i386, pc98, amd64, sparc64] [ath(4)](https://man.freebsd.org/cgi/man.cgi?query=ath&sektion=4&format=html) 驱动支持所有 Atheros Cardbus 和 PCI 卡，基于 AR5005VL 芯片组的除外。受支持的卡列表可在 [http://customerproducts.atheros.com/customerproducts/default.asp](http://customerproducts.atheros.com/customerproducts/default.asp) 查阅。
...
[i386, pc98, amd64] 原始的 Xircom Netwave AirSurfer 受 cnw(4) 驱动支持。
...
[i386, amd64] 最新列表可在 [http://damien.bergamini.free.fr/ral/list.html](http://damien.bergamini.free.fr/ral/list.html) 找到。
...
[i386, amd64] 最新列表可在 [http://ralink.rapla.net/](http://ralink.rapla.net/) 找到。

[amd64, i386, pc98] Lucent Technologies WaveLAN/IEEE 802.11b 无线网络适配器及使用 Lucent Hermes、Intersil PRISM-II、Intersil PRISM-2.5、Intersil Prism-3 和 Symbol Spectrum24 芯片组的兼容产品（[wi(4)](https://man.freebsd.org/cgi/man.cgi?query=wi&sektion=4&format=html) 驱动）

[i386] NCR / AT&T / Lucent Technologies WaveLan T1 速度 ISA/无线 LAN 卡（[wl(4)](https://man.freebsd.org/cgi/man.cgi?query=wl&sektion=4&format=html) 驱动）
...
[i386, amd64] 目前已知受 [zyd(4)](https://man.freebsd.org/cgi/man.cgi?query=zyd&sektion=4&format=html) 驱动支持的设备包括：
...
[amd64, i386] “PC 标准”基于 8250、16450 和 16550 的串口（[sio(4)](https://man.freebsd.org/cgi/man.cgi?query=sio&sektion=4&format=html) 驱动）
...
[amd64, i386] AST 4 端口串口卡（使用共享 IRQ）

[i386] ARNET 串口卡（[ar(4)](https://man.freebsd.org/cgi/man.cgi?query=ar&sektion=4&format=html) 驱动）
...
[i386] STB 4 端口卡（使用共享 IRQ）

[i386] DigiBoard 智能串口卡（digi 驱动）
...
[i386] [rc(4)](https://man.freebsd.org/cgi/man.cgi?query=rc&sektion=4&format=html) 驱动支持 SDL Communications RISCom/8 板卡。
...
[i386, amd64] Specialix SI/XIO/SX 多端口串口卡，包括较老的 SIHOST2.x 和“增强型”（基于 transputer，又称 JET）主机卡（ISA、EISA 和 PCI）均受支持。注意，较新的 SX+ PCI 卡目前不受支持。（[si(4)](https://man.freebsd.org/cgi/man.cgi?query=si&sektion=4&format=html) 驱动）
...
[i386, amd64] 部分板载 CS4610 芯片搭配 CS423x ISA 编解码器，而非 CS4297 AC97 编解码器。[snd_csa(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_csa&sektion=4&format=html) 驱动目前尚不支持此类配置。
...
[i386, amd64] [snd_emu10kx(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_emu10kx&sektion=4&format=html) 驱动确实支持以下声卡（尽管它们的名称与某些受支持的卡相似）：

* Creative Sound Blaster Live! 24-Bit，标识为 as
* Creative Sound Blaster Audigy LS / ES，标识为 as
...
[i386, amd64] [snd_hda(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hda&sektion=4&format=html) 驱动支持许多 Intel HDA 兼容音频芯片组，包括以下：
...
[i386, amd64] 以下以及许多其他编解码器已验证可正常工作：
...
[i386, amd64] 注意，较老的采用 ES18xx 芯片组的 ESS ISA 卡通过 snd_ess(4) 和/或 snd_sbc(4) 支持。
...
[pc98] NEC PC-9801-73、86 及兼容设备（nss 驱动）
...
[i386, pc98] [bktr(4)](https://man.freebsd.org/cgi/man.cgi?query=bktr&sektion=4&format=html) 驱动支持基于 Brooktree Bt848/849/878/879 芯片的视频采集卡以及 Pinnacle PCTV 卡，包括：
...
[amd64, i386, ia64, pc98] FreeBSD 支持一系列 USB 外设；本节列出已知可工作的设备。由于大多数 USB 设备具有通用性，除部分例外，任何给定类别的设备都将受支持，即使未在此明确列出。

> **注意**：USB 以太网适配器可在列出 [以太网接口](https://www.freebsd.org/releases/7.2R/hardware.html#ETHERNET) 的章节中找到。

> **注意**：USB 蓝牙适配器可在 [蓝牙](https://www.freebsd.org/releases/7.2R/hardware.html#BLUETOOTH) 章节中找到。
...
[i386, pc98, amd64, powerpc] [ulpt(4)](https://man.freebsd.org/cgi/man.cgi?query=ulpt&sektion=4&format=html) 驱动支持 USB 打印机和并行打印机转换电缆，包括以下：
...
[i386, pc98, amd64, powerpc] 由于 USB 扫描仪没有标准设备类，此驱动仅识别 USB ID 在驱动自身的表中明确列出的设备。截至目前支持的设备如下：
...
[i386, pc98, amd64] [ng_ubt(4)](https://man.freebsd.org/cgi/man.cgi?query=ng_ubt&sektion=4&format=html) 驱动支持所有符合 Bluetooth specification v1.1 标准的蓝牙 USB 设备，包括：
...
> **注意**：关于特定显卡及与 Xorg 兼容性的信息，可参阅 [http://www.x.org/](http://www.x.org/)。
...
> **注意**：[moused(8)](https://man.freebsd.org/cgi/man.cgi?query=moused&sektion=8&format=html) 有关于在 FreeBSD 中使用指点设备的更多信息。关于在 Xorg 中使用指点设备的信息，可参阅 [http://www.x.org/](http://www.x.org/)。
