...
自从我们发布 FreeBSD 1.0 版本以来，已经修复了数百个错误，新增了功能，系统整体质量得到了极大提升。FreeBSD 1.1.5 是近 18 个月的工作成果，我们的全志愿者工作组投入了数千工时。希望你能喜欢这个版本！

许多软件包也已升级或新增，比如 XFree86 2.1、xview 3.2、elm、nntp、mh、InterViews 和其他数十个杂项工具已移植，现在可以作为附加组件使用。更多详细信息请参阅本文档的下一节。
...
FreeBSD 核心部分未包含 DES 代码，因此不会阻碍其出口到美国以外的地区。核心发行版附带了一款仅限美国使用的附加包，其中包含通常使用 DES 的程序。单独提供的辅助包任何人都可以使用。对于我们的非美国用户，还有一款可从美国以外地区自由出口的欧洲版 DES，FreeBSD FAQ 中有详细说明。
...
所有支持的 SCSI 控制器均全面支持 SCSI-I 和 SCSI-II 外设，包括磁盘、磁带驱动器（包括 DAT）和 CD-ROM 驱动器。

**注意**：目前将 CD-ROM 驱动器接入 FreeBSD 系统的唯一方式是通过 SCSI 控制器和 mcd 驱动程序（Mitsumi CD-ROM 接口卡）。我们不支持 SoundBlaster CD-ROM 接口和其他“mini SCSI”适配器。

某些控制器在处理超过 16MB 内存时存在局限性，因为 ISA 总线只有 24 位的 DMA 地址空间。如果你进行计算，你会发现这使得无法直接进行 DMA 操作访问任何超过 16MB 的地址。这一限制甚至在某些 EISA 控制器（通常是 32 位的）中也同样适用，当它们配置为模拟 ISA 卡时，会在所有方面完全遵循 ISA 的限制。

IDE 控制器（不使用 DMA）、真正的 EISA 控制器（如 UltraStor 和 Adaptec 1742A）和大多数 VLB（局部总线）控制器可以完全避免这个问题。在必要的情况下，系统将使用“回弹缓冲区”（bounce buffers）与控制器进行通信，这样即使内存超过 16MB，也能顺利运行。
...
请注意，Walnut Creek CDROM 不提供 FreeBSD 的技术支持，你需要联系 FreeBSD 团队获取技术支持。更多信息请参见第 4 部分。

请注意，以免你产生误解——“FreeBSD” 始终是免费软件。在“核心团队”中几乎没有人从发行版和与 FreeBSD 相关的任何事务中获利。我们仅提供这些信息，作为公共服务，帮助那些希望从网络以外的地方获取发行版的用户（而且获取我们的软件越方便，我们越高兴）。

## 4. 报告问题、提出建议、提交代码

我们非常重视你的建议、错误报告和代码贡献——如果你发现任何问题，请随时向我们报告（如果可能的话，附上修复方案会更好！）。

从具备互联网邮件功能的机器上提交错误报告的首选方法是使用 `sendbug` 命令。错误报告将由我们忠实的 bugfiler 程序归档，你可以放心，我们会尽最大努力尽快回复所有报告的问题。
...
另外，由于这是一项志愿者项目，我们始终欢迎更多人手来帮忙——目前还有远远更多的增强功能需要完成，我们自己无法独立完成！如有技术问题或愿意提供帮助，可以通过邮件与我们联系：`FreeBSD-hackers@freefall.cdrom.com`
...
FreeBSD 凝聚了来自世界各地数十甚至数百位个人的辛勤努力，他们辛勤工作为你带来了这一版本。列举所有为 FreeBSD 贡献的人是非常困难的，甚至可以说是不可能的，但我们仍然会尽力列出（按字母顺序排列，当然）。如果你的名字没有被提及，请放心，这完全是疏漏所致。
