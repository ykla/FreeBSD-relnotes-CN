...
与大多数支持通用标准的驱动不同，**sdhci** 需要大量变通项（quirks）来应对硬件缺陷、专有寄存器和规范不清的电源管理。虽然来自 Intel、Xilinx、Rockchip、Frescale、Ricoh 和 TI 的许多芯片组都有这些条目，但使用某些控制器时可能导致性能欠佳。当设备通过 fdt(4) 或 acpi(4) 配置时，通常需要变通项和自定义配置。
