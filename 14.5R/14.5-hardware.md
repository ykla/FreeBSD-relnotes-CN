...
本硬件兼容性说明列出所有已知可在 FreeBSD 14.5 上工作的硬件。其他配置也可能可用，但尚未经过测试。欢迎对本列表提出反馈、更新和修正！
...
[ahci(4)](https://man.freebsd.org/cgi/man.cgi?query=ahci&sektion=4&format=html) 驱动还支持作为 PCI 桥接 nvme(4) 的 AHCI 设备，使用 Intel Rapid Storage Technology（RST）。要使用 nvme(4) 设备，需在 BIOS 中将 SATA 模式从 RST 设置为 AHCI，否则需接受因 RST 启用中断共享而带来的性能影响。FreeBSD 会自动检测处于 RST 模式且具有此扩展的 AHCI 设备。当检测到时，[ahci(4)](https://man.freebsd.org/cgi/man.cgi?query=ahci&sektion=4&format=html) 驱动会将 nvme(4) 子设备附加到 [ahci(4)](https://man.freebsd.org/cgi/man.cgi?query=ahci&sektion=4&format=html) 设备。
...
上述部分芯片可配置为 AHCI 模式。在此情况下，它们由 [ahci(4)](https://man.freebsd.org/cgi/man.cgi?query=ahci&sektion=4&format=html) 驱动替代。
...
此外，还有若干 HP Smart Array 控制器仅通过 PCI 子设备 ID 获得支持，因为没有可用的型号名称：0x3220、0x3222、0x3230、0x3231、0x3232、0x3233、0x3236、0x3238、0x3239、0x323A、0x323B、0x323C 和 0x324B（PCI 子供应商均为 0x103C）。
...
[hptiop(4)](https://man.freebsd.org/cgi/man.cgi?query=hptiop&sektion=4&format=html) 驱动还支持以下 SAS 和 SATA RAID 控制器，这些控制器已停产：
...
[isci(4)](https://man.freebsd.org/cgi/man.cgi?query=isci&sektion=4&format=html) 驱动支持 Intel C600 `SAS` 控制器。
...
[mrsas(4)](https://man.freebsd.org/cgi/man.cgi?query=mrsas&sektion=4&format=html) 驱动支持以下 Broadcom/LSI SATA/SAS RAID 控制器：
...
注意，此硬件仅对 ATA DMA 命令支持命令排队和基于 FIS 的切换。ATAPI 和非 DMA ATA 命令在每个端口上逐一执行。

[nda(4)](https://man.freebsd.org/cgi/man.cgi?query=nda&sektion=4&format=html) 驱动支持通过 PCIe 连接或通过 CAM 子系统经 NVMF（NVMe over Fabric）连接的 NVMe（非易失性内存快速通道）存储设备。
...
[sdhci(4)](https://man.freebsd.org/cgi/man.cgi?query=sdhci&sektion=4&format=html) 驱动支持 SD 主机控制器规范。通过 PCI 总线连接时，控制器会自动配置。许多 SoC 芯片提供直接映射到 I/O 内存的 SDHCI 控制器。对于这些控制器，可以使用你的主板供应商提供的 fdt(4) 或 acpi(4) 方法进行配置。

与大多数支持通用标准的驱动不同，**sdhci** 需要大量变通项（quirks）来应对硬件缺陷、专有寄存器和规范不清的电源管理。虽然来自 Intel、Xilinx、Rockchip、Freescale、Ricoh 和 TI 的许多芯片组都有这些条目，但使用某些控制器时可能导致性能欠佳。当设备通过 fdt(4) 或 acpi(4) 配置时，通常需要变通项和自定义配置。
...
[sym(4)](https://man.freebsd.org/cgi/man.cgi?query=sym&sektion=4&format=html) 驱动支持的 SCSI 控制器可内置于主板，或位于以下扩展板之一：
...
对于所有受支持的 SCSI 控制器，均完整支持 SCSI-I、SCSI-II 和 SCSI-III 外设，包括硬盘、光盘、磁带机（包括 DAT、8mm Exabyte、Mammoth 和 DLT）、介质更换器、处理器目标设备和 CD-ROM 驱动器。支持 CD-ROM 命令的 WORM 设备可由 CD-ROM 驱动（如 [cd(4)](https://man.freebsd.org/cgi/man.cgi?query=cd&sektion=4&format=html)）以只读方式访问。WORM/CD-R/CD-RW 写入支持由 [cdrecord(1)](https://man.freebsd.org/cgi/man.cgi?query=cdrecord&sektion=1&format=html) 提供，该程序是 Ports 中 `sysutils/cdrtools` Port 的组成部分。
...
[age(4)](https://man.freebsd.org/cgi/man.cgi?query=age&sektion=4&format=html) 驱动为基于 Attansic/Atheros L1 千兆以太网控制器芯片的设备提供支持，包括：
...
[aue(4)](https://man.freebsd.org/cgi/man.cgi?query=aue&sektion=4&format=html) 驱动支持以下基于 ADMtek AN986 Pegasus chipset:
...
[bce(4)](https://man.freebsd.org/cgi/man.cgi?query=bce&sektion=4&format=html) 驱动为基于 QLogic NetXtreme II 千兆以太网控制器系列的设备提供支持，包括以下：
...
[bge(4)](https://man.freebsd.org/cgi/man.cgi?query=bge&sektion=4&format=html) 驱动为基于 Broadcom BCM570x 千兆以太网控制器芯片系列的设备提供支持，包括以下：
...
[bxe(4)](https://man.freebsd.org/cgi/man.cgi?query=bxe&sektion=4&format=html) 驱动为基于 QLogic NetXtreme II 10Gb 以太网控制器芯片系列的设备提供支持，包括以下：
...
[cas(4)](https://man.freebsd.org/cgi/man.cgi?query=cas&sektion=4&format=html) 驱动支持的芯片：
...
[cdce(4)](https://man.freebsd.org/cgi/man.cgi?query=cdce&sektion=4&format=html) 驱动支持实现 USB Communication Device Class Ethernet Control Model (CDC ECM) or Network Control Model (CDC NCM) protocol, such as:
...
[cxgbev(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbev&sektion=4&format=html) 驱动支持基于 T5 ASIC 的 40Gb、10Gb 和 1Gb 以太网适配器上的 Virtual Functions：
...
[cxgbev(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbev&sektion=4&format=html) 驱动支持基于 T4 ASIC 的 10Gb 和 1Gb 以太网适配器上的 Virtual Functions：
...
[em(4)](https://man.freebsd.org/cgi/man.cgi?query=em&sektion=4&format=html) 驱动支持基于 Intel 的千兆以太网适配器 82540、82541ER、82541PI、82542、82543、82544、82545、82546、82546EB、82546GB、82547、82571、82572、82573、82574、82575、82576 和 82580 控制器芯片：
...
[et(4)](https://man.freebsd.org/cgi/man.cgi?query=et&sektion=4&format=html) 驱动支持 Agere ET1310 10/100/千兆以太网适配器。
...
[genet(4)](https://man.freebsd.org/cgi/man.cgi?query=genet&sektion=4&format=html) 驱动支持 Broadcom BCM2711 的以太网控制器部分（位于树莓派 4 Model B 及相关系统）。该驱动使用 BCM54213PE PHY。
...
[ice(4)](https://man.freebsd.org/cgi/man.cgi?query=ice&sektion=4&format=html) 驱动支持 Intel Ethernet 800 系列。此系列中大多数配备 SFP28/QSFP28 笼的适配器都具有要求使用 Intel 认证模块的固件；这些认证模块列表如下。该认证检查无法由驱动禁用。
...
[ice(4)](https://man.freebsd.org/cgi/man.cgi?query=ice&sektion=4&format=html) 驱动支持带以下 SFP28 模块的 25Gb 和 10Gb 以太网适配器：
...
[ice(4)](https://man.freebsd.org/cgi/man.cgi?query=ice&sektion=4&format=html) 驱动支持带以下 SFP+ 模块的 10Gb 和 1Gb 以太网适配器：
...
注意：这些适配器还支持所有符合 SFF-8431 v4.1 和 SFF-8472 v10.4 规范的被动和主动限流直连电缆。

此列表并不详尽；请查阅产品文档以获取受支持介质的最新列表。
...
[ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 驱动支持 Intel Ethernet 700 系列。此系列中大多数配备 SFP+/SFP28/QSFP+ 笼的适配器都具有要求使用 Intel 认证模块的固件；这些认证模块列表如下。该认证检查无法由驱动禁用。
...
注意，适配器还支持所有符合 SFF-8431 v4.1 和 SFF-8472 v10.4 规范的被动和主动限幅直连电缆。

此列表并非详尽无遗；请查阅产品文档以获取最新支持的介质列表。
...
[my(4)](https://man.freebsd.org/cgi/man.cgi?query=my&sektion=4&format=html) 驱动为基于 Myson 芯片的各类 NIC 提供支持。支持的型号包括：
...
[ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动支持基于 Alteon Tigon I 和 II 芯片的千兆以太网适配器。

[ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动已通过以下适配器测试：
...
[uhso(4)](https://man.freebsd.org/cgi/man.cgi?query=uhso&sektion=4&format=html) 驱动应能与 Option 的大多数设备配合工作。以下设备已验证可工作：
...
该设备内置“Zero-CD”大容量存储设备，其中包含 Microsoft Windows 的驱动程序；这是设备的默认模式。

[uhso(4)](https://man.freebsd.org/cgi/man.cgi?query=uhso&sektion=4&format=html) 驱动会自动将设备从“Zero-CD”模式切换为调制解调器模式。此行为可通过将 `hw.usb.uhso.auto_switch` 设置为 0 来禁用，使用 sysctl(8)。
...
[ar40xx_switch(4)](https://man.freebsd.org/cgi/man.cgi?query=ar40xx_switch&sektion=4&format=html) 驱动支持以下千兆以太网交换机控制器：
...
使用较旧 Broadcom 芯片（BCM4301、BCM4303 和 BCM4306 rev 2）的用户必须使用 bwi(4) 驱动，因为 v4 版本的固件不支持这些芯片。较新的固件过大，无法容纳在这些旧芯片中。
...
[iwlwifi(4)](https://man.freebsd.org/cgi/man.cgi?query=iwlwifi&sektion=4&format=html) 驱动支持来自 [mvm(4)](https://man.freebsd.org/cgi/man.cgi?query=mvm&sektion=4&format=html) 子驱动程序的 PCIe 设备，涵盖以下芯片组世代：
...
[iwlwifi(4)](https://man.freebsd.org/cgi/man.cgi?query=iwlwifi&sektion=4&format=html) 驱动支持来自 [mld(4)](https://man.freebsd.org/cgi/man.cgi?query=mld&sektion=4&format=html) 子驱动程序的 PCIe 设备，涵盖以下芯片组世代：
...
[uath(4)](https://man.freebsd.org/cgi/man.cgi?query=uath&sektion=4&format=html) 驱动 should work with the following adapters:
...
核心组件和内核接口通过硬件接口与硬件通信。该接口作为硬件的抽象层，允许使用不同的 UART 进行串行通信。
...
[ucycom(4)](https://man.freebsd.org/cgi/man.cgi?query=ucycom&sektion=4&format=html) 驱动 currently supports the following devices which incorporate Cypress USB to RS232 bridge chips:
...
[umcs(4)](https://man.freebsd.org/cgi/man.cgi?query=umcs&sektion=4&format=html) 驱动 was tested on the following adapters:
...
部分板载 CS4610 芯片搭配 CS423x ISA 编解码器，而非 CS4297 AC97 编解码器。此类配置尚未由 [snd_csa(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_csa&sektion=4&format=html) 驱动支持。
...
*	Creative Sound Blaster Audigy 2 和 Creative Sound Blaster Audigy 4（CA0102 芯片组）。PCM 支持限于 48kHz/16 位立体声（不支持该芯片组的 192kHz/24 位部分）。
*	Creative Sound Blaster Audigy 2 Value（CA0108 芯片组）。PCM 支持限于 48kHz/16 位立体声（不支持该芯片组的 192kHz/24 位部分）。此卡不支持 MIDI。

[snd_emu10kx(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_emu10kx&sektion=4&format=html) 驱动 **不** 支持以下声卡（尽管它们的名称与某些受支持的声卡相似）：
...
默认情况下，每个 pcm(4) 设备对应声卡上的一个物理端口。对于 ADAT 端口，支持 8 声道、4 声道和 2 声道格式。根据所选采样率和声道格式，并非所有 pcm 声道都能映射到 ADAT 声道，反之亦然。
...
[snd_t4dwave(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_t4dwave&sektion=4&format=html) 驱动支持基于以下芯片组的音频设备：
...
支持多种 USB 外设；已知可工作的设备在本节中列出。由于大多数 USB 设备具有通用性，除部分例外，任何属于给定类别的设备都将获得支持，即使在此处未明确列出。

> **注意**：
>
> USB 蓝牙适配器参见 [蓝牙设备](https://www.freebsd.org/releases/14.5R/hardware.html#bluetooth)。USB 人机接口设备参见 [杂项](https://www.freebsd.org/releases/14.5R/hardware.html#misc)。USB 网络接口参见 [以太网接口](https://www.freebsd.org/releases/14.5R/hardware.html#ethernet) 或 [无线网络接口](https://www.freebsd.org/releases/14.5R/hardware.html#wlan)。USB 串行接口参见 [串行接口](https://www.freebsd.org/releases/14.5R/hardware.html#serial)。
...
要发现触摸板的产品 ID，请在 lshal(1) 的输出中搜索 “Trackpad”，并查看属性 **usb_device.product_id**。

[atopcase(4)](https://man.freebsd.org/cgi/man.cgi?query=atopcase&sektion=4&format=html) 驱动支持 2015-2018 年生产的以下 MacBook：
...
[dwcotg(4)](https://man.freebsd.org/cgi/man.cgi?query=dwcotg&sektion=4&format=html) 驱动支持许多嵌入式设备中的 Synopsys DesignWare USB 控制器。

[ehci(4)](https://man.freebsd.org/cgi/man.cgi?query=ehci&sektion=4&format=html) 驱动支持实现增强主机控制器接口标准的 USB 2.0 控制器。
...
[umass(4)](https://man.freebsd.org/cgi/man.cgi?query=umass&sektion=4&format=html) 驱动会尽力避免驱动器问题，但并非所有问题都能自动处理，因此可能需要变通项。有关驱动器的变通项，请参阅 usb_quirk(4) 的“USB 大容量存储变通项”章节。usbconfig(8) 的 **add_dev_quirk_vplh** 和 **add_quirk** 命令可以动态管理这些变通项。变通项也可以通过可调项指定，如 usb_quirk(4) 所述。
...
[ulpt(4)](https://man.freebsd.org/cgi/man.cgi?query=ulpt&sektion=4&format=html) 驱动为 USB 打印机和并口打印机转换线缆提供支持，包括以下：
...
[xhci(4)](https://man.freebsd.org/cgi/man.cgi?query=xhci&sektion=4&format=html) 驱动支持 `XHCI` 兼容控制器，具有 PCI 类 12（串行总线）、子类 3（USB）和编程接口 48（XHCI）。
...
> 已停止生产。提供 128KB SRAM 型号或 2MB DRAM 型号。
...
> 参见 [http://www.soekris.com/](http://www.soekris.com/)。内含 7951，支持对称和随机数运算。

Soekris Engineering vpn1401 和 vpn1411

> 参见 [http://www.soekris.com/](http://www.soekris.com/)。内含 7955，支持对称和随机数运算。
...
[hgame(4)](https://man.freebsd.org/cgi/man.cgi?query=hgame&sektion=4&format=html) 驱动支持 HID 游戏手柄、操纵杆和控制器，例如：
...
* PS/2 鼠标及兼容设备，包括许多笔记本定点设备（[psm(4)](https://man.freebsd.org/cgi/man.cgi?query=psm&sektion=4&format=html) 驱动）
...
受支持的设备彼此相似，主要在精度和分辨率上有所差异。这些设备需要单线进行数据通信，使用与 Maxim 的 1-wire(tm) 不兼容的自定义协议。AM320x 设备也支持连接到 i2c 总线，但该驱动仅支持单线连接方式。

[ig4(4)](https://man.freebsd.org/cgi/man.cgi?query=ig4&sektion=4&format=html) 驱动支持基于 Synopsys DesignWare IP 的 I2C 控制器，这些控制器可见于从第四代起的 Intel Core(TM) 处理器、Intel Bay Trail、Apollo Lake SoC 系列以及部分 AMD 系统。
...
当前版本的 [netfpga10g_nf10bmac(4)](https://man.freebsd.org/cgi/man.cgi?query=netfpga10g_nf10bmac&sektion=4&format=html) 驱动与 NetFPGA-10G Embedded CPU Ethernet Core version 1.00a 的一个 PIO 模式接口配合工作。
...
以下温度传感器由 [ow_temp(4)](https://man.freebsd.org/cgi/man.cgi?query=ow_temp&sektion=4&format=html) 驱动支持：
...
[superio(4)](https://man.freebsd.org/cgi/man.cgi?query=superio&sektion=4&format=html) 驱动支持由 Nuvoton（前称 Winbond）和 ITE 生产的众多 Super I/O 控制器，即：
...
[udl(4)](https://man.freebsd.org/cgi/man.cgi?query=udl&sektion=4&format=html) 驱动支持基于 DisplayLink DL-120 / DL-160 图形芯片的 USB 显示设备，包括：
