...
上述部分芯片可配置为 AHCI 模式。在这种情况下，它们改由 [ahci(4)](https://man.freebsd.org/cgi/man.cgi?query=ahci&sektion=4&format=html) 驱动提供支持。

系统以 PIO 模式支持未知的 ATA 芯片组；如果标准的 busmaster DMA 寄存器存在且包含有效设置，也会启用 DMA，不过最大模式限制为 UDMA33，因为无从知道该芯片组能做什么以及如何编程。
...
此外，还有若干 HP Smart Array 控制器因为没有可用的型号名称，仅靠 PCI 子设备 ID 区分：0x3220、0x3222、0x3230、0x3231、0x3232、0x3233、0x3236、0x3238、0x3239、0x323A、0x323B、0x323C 和 0x324B（PCI 子供应商均为 0x103C）。
...
这些系统还带有集成 RAID 镜像及其增强功能，本驱动同样支持。

`SAS` 控制器芯片也出现在许多较新的基于 AMD/Opteron 的系统上，例如 Sun 4100。注意，此控制器可同时驱动 SAS 和 SATA 驱动器，或二者的混合。这些控制器可用的集成 RAID 镜像功能支持充其量是有限的。

`Fibre Channel` 控制器芯片组被广泛用于各种速率和系统。`Apple` 光纤通道 HBA 实际上是 `FC949ES` 卡。
...
注意，此硬件仅对 ATA DMA 命令支持命令排队和基于 FIS 的切换。ATAPI 和非 DMA ATA 命令在每个端口上逐个执行。
...
[sdhci(4)](https://man.freebsd.org/cgi/man.cgi?query=sdhci&sektion=4&format=html) 驱动支持 SD 主机控制器规范。通过 PCI 总线连接时，控制器会自动配置。许多 SoC 芯片提供直接映射到 I/O 内存的 SDHCI 控制器。对于这些控制器，可以借助主板供应商提供的 fdt(4) 或 acpi(4) 方法进行配置。
...
[aue(4)](https://man.freebsd.org/cgi/man.cgi?query=aue&sektion=4&format=html) 驱动支持以下基于 ADMtek AN986 Pegasus 芯片组的适配器：
...
[bnxt(4)](https://man.freebsd.org/cgi/man.cgi?query=bnxt&sektion=4&format=html) 驱动为基于 Broadcom NetXtreme-C 和 NetXtreme-E 系列千兆以太网控制器芯片的各种网卡提供支持，包括以下：
...
[cdce(4)](https://man.freebsd.org/cgi/man.cgi?query=cdce&sektion=4&format=html) 驱动支持实现 USB 通信设备类以太网控制模型（CDC ECM）或网络控制模型（CDC NCM）协议的 USB 以太网接口，例如：
...
注意：这些适配器还支持所有符合 SFF-8431 v4.1 和 SFF-8472 v10.4 规范的被动和主动限幅直连电缆。
...
[uhso(4)](https://man.freebsd.org/cgi/man.cgi?query=uhso&sektion=4&format=html) 驱动会自动将设备从“Zero-CD”模式切换为调制解调器模式。要禁用此行为，可使用 sysctl(8) 将 `hw.usb.uhso.auto_switch` 设置为 0。
...
[uath(4)](https://man.freebsd.org/cgi/man.cgi?query=uath&sektion=4&format=html) 驱动应与以下适配器配合工作：
...
[ucycom(4)](https://man.freebsd.org/cgi/man.cgi?query=ucycom&sektion=4&format=html) 驱动目前支持以下内置 Cypress USB 转 RS232 桥接芯片的设备：
...
[umcs(4)](https://man.freebsd.org/cgi/man.cgi?query=umcs&sektion=4&format=html) 驱动已在以下适配器上进行过测试：
...
> **注意**：
>
> [moused(8)](https://man.freebsd.org/cgi/man.cgi?query=moused&sektion=8&format=html) 中有更多关于在 FreeBSD 控制台使用传统定点设备的信息。
...
[ig4(4)](https://man.freebsd.org/cgi/man.cgi?query=ig4&sektion=4&format=html) 驱动支持基于 Synopsys DesignWare IP 的 I2C 控制器，这些控制器可在第四代及之后的 Intel Core(TM) 处理器、Intel Bay Trail、Apollo Lake SoC 系列以及部分 AMD 系统中找到。
...
当前版本的 [netfpga10g_nf10bmac(4)](https://man.freebsd.org/cgi/man.cgi?query=netfpga10g_nf10bmac&sektion=4&format=html) 驱动与 NetFPGA-10G 嵌入式 CPU 以太网核心 1.00a 版的一个 PIO 模式接口配合工作。
