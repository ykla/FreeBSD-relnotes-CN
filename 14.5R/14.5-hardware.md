...
[ice(4)](https://man.freebsd.org/cgi/man.cgi?query=ice&sektion=4&format=html) 驱动支持 Intel Ethernet 800 系列。此系列中大多数配备 SFP28/QSFP28 笼的适配器，其固件要求使用 Intel 认证模块；这些认证模块列表如下。驱动无法禁用该认证检查。
...
[ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 驱动支持 Intel Ethernet 700 系列。此系列中大多数配备 SFP+/SFP28/QSFP+ 笼的适配器，其固件要求使用 Intel 认证模块；这些认证模块列表如下。驱动无法禁用该认证检查。
...
支持多种 USB 外设；已知可工作的设备在本节中列出。由于大多数 USB 设备具有通用性，除少数例外，系统都会支持属于同一类别的任何设备，即使此处并未明确列出。
