...
该设备内置一个称为“Zero-CD”的大容量存储设备，其中包含 Microsoft Windows 的驱动程序；这是设备的默认模式。
...
> **注意**：
>
> USB 蓝牙适配器参见 [蓝牙设备](https://www.freebsd.org/releases/14.5R/hardware.html#bluetooth)。
> USB 人机接口设备参见 [杂项](https://www.freebsd.org/releases/14.5R/hardware.html#misc)。
> USB 网络接口参见 [以太网接口](https://www.freebsd.org/releases/14.5R/hardware.html#ethernet) 或 [无线网络接口](https://www.freebsd.org/releases/14.5R/hardware.html#wlan)。
> USB 串行接口参见 [串行接口](https://www.freebsd.org/releases/14.5R/hardware.html#serial)。
...
[umass(4)](https://man.freebsd.org/cgi/man.cgi?query=umass&sektion=4&format=html) 驱动支持以下 USB 大容量存储设备：

*	USB 闪存盘
*	USB 硬盘驱动器
*	USB 软盘驱动器

[umass(4)](https://man.freebsd.org/cgi/man.cgi?query=umass&sektion=4&format=html) 驱动会尽最大努力避免驱动器问题，但并非所有问题都能自动处理，因此可能需要使用 quirks。有关驱动器的 quirks，请参阅 [usb_quirk(4)](https://man.freebsd.org/cgi/man.cgi?query=usb_quirk&sektion=4&format=html) 的 *USB Mass Storage quirks* 章节。[usbconfig(8)](https://man.freebsd.org/cgi/man.cgi?query=usbconfig&sektion=8&format=html) 的命令 `add_dev_quirk_vplh` 和 `add_quirk` 可以动态管理这些 quirks。如 [usb_quirk(4)](https://man.freebsd.org/cgi/man.cgi?query=usb_quirk&sektion=4&format=html) 中所述，quirks 可以通过 tunables 指定。
...
[sbp(4)](https://man.freebsd.org/cgi/man.cgi?query=sbp&sektion=4&format=html) 驱动支持 FireWire Serial Bus Protocol 2（SBP-2）存储设备。
...
> 已停产。分为 128KB SRAM 型号和 2MB DRAM 型号。
...
> 参见 `http://www.soekris.com/`。内含 7951，支持对称和随机数运算。

Soekris Engineering vpn1401 和 vpn1411

> 参见 `http://www.soekris.com/`。内含 7955，支持对称和随机数运算。
...
[mx25l(4)](https://man.freebsd.org/cgi/man.cgi?query=mx25l&sektion=4&format=html) 驱动支持以下 SPI 闪存存储设备：
