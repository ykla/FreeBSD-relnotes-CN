...
FreeBSD 11.3-RELEASE 的发行说明总结了 11.3-STABLE 开发分支中对 FreeBSD 基本系统所做的更改。本文档列出了自上次发布以来发布的适用安全公告，以及对 FreeBSD 内核和用户空间的重要更改。同时，还提供了有关升级的简要说明。
...
本文档介绍了自 11.2-RELEASE 以来 FreeBSD 中最显而易见的新增或变更功能。通常，本文描述的更改是 11.3-STABLE 分支所独有的，除非明确标注为已合并（MERGE）功能。

典型的发行说明内容包括 11.2-RELEASE 之后发布的安全公告、新增的驱动程序或硬件支持、新命令或选项、重大错误修复或第三方软件升级。这些内容还可能包括主要 Port/包的更改或发布工程的改进。显然，发行说明无法列出 FreeBSD 在版本之间的每一个更改；本文主要关注安全公告、用户可见的更改以及重要的架构改进。
...
本节涉及了对用户空间应用程序、第三方软件和系统工具的更改和新增内容。
...
MSI IRQ 数量已从常量转换为可调参数。默认值仍为 `512`，现在可以通过 [sysctl(8)](https://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&manpath=freebsd-release-ports)  `machdep.num_msi_irqs` 在启动时进行更改。[(r342656)](http://svn.freebsd.org/viewvc/base?view=revision&revision=342656)
...
本节涉及了自 11.2-RELEASE 以来对设备和设备驱动程序的更改和新增内容。
...
[lagg(4)](https://www.freebsd.org/cgi/man.cgi?query=lagg&sektion=4&manpath=freebsd-release-ports) 驱动程序已更新，允许更改 MTU 而无需销毁并重新创建接口。[(r342206)](http://svn.freebsd.org/viewvc/base?view=revision&revision=342206)（由 iXsystems 赞助）
...
本节涉及了对物理机器、虚拟化监视器和虚拟化环境的硬件支持，以及不属于本文件其他部分的硬件更改和更新。
...
本节涉及了对文件系统和其他存储子系统（包括本地和网络存储）的更改和新增内容。
...
本节涵盖了 FreeBSD Ports、软件包基础设施以及软件包维护和安装工具的更改。
