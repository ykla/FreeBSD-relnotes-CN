...
本文档包含 FreeBSD 4.8-RELEASE 在 i386 硬件平台上的发布说明。它描述了自 4.7-RELEASE 以来 FreeBSD 添加（或更改）的新功能。它还提供了关于从以前版本的 FreeBSD 升级的一些说明。
...
已为 FireWire 设备（参见 [firewire(4)](http://www.freebsd.org/cgi/man.cgi?query=firewire&sektion=4&manpath=FreeBSD+4.8-RELEASE)）添加初步支持。

添加了对 CanBe 电源管理控制器的支持。

添加了驱动程序 ubsa ，以支持 Belkin F5U103（及兼容）USB 转串口适配器。

已添加 [uftdi(4)](http://www.freebsd.org/cgi/man.cgi?query=uftdi&sektion=4&manpath=FreeBSD+4.8-RELEASE) 驱动程序，以支持 FTDI USB 转串口设备。
...
已添加驱动程序 [gre(4)](http://www.freebsd.org/cgi/man.cgi?query=gre&sektion=4&manpath=FreeBSD+4.8-RELEASE) ，支持使用 GRE（RFC 1701）或最小 IP 封装（用于移动 IP，RFC 2004）来封装 IP 数据包。
...
[targ(4)](http://www.freebsd.org/cgi/man.cgi?query=targ&sektion=4&manpath=FreeBSD+4.8-RELEASE) 驱动程序已被重写，并且在 `/usr/share/examples/scsi_target` 中添加了一个新的用户模式，用于模拟直接访问设备。

已添加驱动程序 trm ，以支持使用 Tekram TRM-S1040 SCSI 芯片组的 SCSI 适配器。
...
已添加工具 [fwcontrol(8)](http://www.freebsd.org/cgi/man.cgi?query=fwcontrol&sektion=8&manpath=FreeBSD+4.8-RELEASE) ，用于帮助用户访问和控制 FireWire 子系统。
...
已添加工具 [getconf(1)](http://www.freebsd.org/cgi/man.cgi?query=getconf&sektion=1&manpath=FreeBSD+4.8-RELEASE) ，用于打印 POSIX 或 X/Open 路径或系统配置变量的值。
...
已添加工具 [pathchk(1)](http://www.freebsd.org/cgi/man.cgi?query=pathchk&sektion=1&manpath=FreeBSD+4.8-RELEASE)，用于检查路径名在 POSIX 系统之间的有效性或可移植性。

[pw(8)](http://www.freebsd.org/cgi/man.cgi?query=pw&sektion=8&manpath=FreeBSD+4.8-RELEASE) 现在可以添加以 `$` 字符结尾的用户名；此更改旨在帮助管理 **Samba** 服务。
...
所有 **bzip2** 套件的应用程序现在已安装在基础系统中（特别的，bzip2recover 现在已构建并安装）。
