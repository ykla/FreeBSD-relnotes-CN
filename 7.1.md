...
所有用户在安装 FreeBSD 之前都应查阅发布的勘误。勘误文档会更新包含“最新”发现的信息，这些信息通常是在发布周期结束或发布后出现的。它通常包含已知的漏洞、安全公告和文档的更正。可以在 FreeBSD 网站上找到 FreeBSD 7.1-RELEASE 的最新勘误。

## 2 新特性

本节介绍了自 7.0-RELEASE 以来，FreeBSD 中最为用户可见的新特性或更改。

典型的发行说明条目记录了 7.0-RELEASE 之后发布的最新安全公告、新的驱动程序或硬件支持、新的命令或选项、重要的 bug 修复或第三方软件升级。它们也可能列出对主要 Port/包或发布工程实践的更改。显然，发行说明无法列出从一个版本到另一个版本之间对 FreeBSD 所做的每一个更改；本文件主要集中在安全公告、用户可见的更改以及主要的架构改进上。

### 2.1 安全公告

以下安全公告中描述的问题已被修复。更多信息请查阅各个公告，详情请访问 [http://security.FreeBSD.org/](http://security.freebsd.org/)。
...
DTrace，一种全面的动态追踪框架和 [dtrace(1)](http://www.freebsd.org/cgi/man.cgi?query=dtrace&sektion=1&manpath=FreeBSD+7.1-RELEASE) 用户空间工具，已从 OpenSolaris 导入。DTrace 提供了强大的 基本设施，使管理员、开发人员和服务人员能够简洁地回答关于操作系统和用户程序行为的任意问题。
...
FreeBSD 的 [linux(4)](http://www.freebsd.org/cgi/man.cgi?query=linux&sektion=4&manpath=FreeBSD+7.1-RELEASE) ABI 现已支持 `sched_setaffinity()` 和 `get_setaffinity()`，使用真实的 CPU 亲和性设置原语。
...
FreeBSD 内核现支持新的内核转储文本格式。文本转储通过机械生成/提取的调试输出提供更高级别的信息，而不是简单的内存转储。该功能可用于生成简短的内核错误报告，这些报告丰富了调试信息，但不依赖于内核符号表或精确同步的源代码。更多信息请参见 [textdump(4)](http://www.freebsd.org/cgi/man.cgi?query=textdump&sektion=4&manpath=FreeBSD+7.1-RELEASE) 手册页。
...
[amd64, i386] 新增了 gptboot 引导加载程序，以支持从 GPT 标签的磁盘引导。新增了引导命令到 [gpt(8)](http://www.freebsd.org/cgi/man.cgi?query=gpt&sektion=8&manpath=FreeBSD+7.1-RELEASE)，通过写入引导加载程序所需的位，创建新的引导分区（如有必要），使 GPT 磁盘可引导。
...
ppp 的 `rc(8)` 脚本现在支持多个实例。更多信息请参见 [rc.conf(5)] 中的变量 `ppp_profile` 所述。
