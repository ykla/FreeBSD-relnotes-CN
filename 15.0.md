...
可以使用 [freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 实用程序在快照版本与 RELEASE 版本之间进行二进制升级。有关更多细节，请参阅特定发行版的升级流程，[FreeBSD 15.0-RELEASE 升级信息](https://www.freebsd.org/releases/15.0R/installation/#upgrade-binary)，以及 FreeBSD 手册中的[二进制升级流程](https://docs.freebsd.org/en/books/handbook/cutting-edge/#freebsdupdate-upgrade)。该过程会更新未被修改的用户态工具，以及作为官方 FreeBSD 发行版一部分分发的未被修改的 GENERIC 内核。[freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&format=html) 实用程序要求被升级的主机具备 Internet 连接。

### 从软件包包升级

>**警告**
>
>对于那些使用 FreeBSD 15.0 的 PRERELEASE、ALPHA 和 BETA 构建的用户，由于 FreeBSD.org 基础设施中出现的后期变更，无法直接使用 [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html) 实用程序进行更新。 

基于软件包的基本系统安装需要手动从 `15.0-RELEASE` 标签的源代码树检出中复制所需文件，或者作为替代方案，从官方发行版基本系统包中强制安装软件包 `FreeBSD-pkg-bootstrap`。

推荐且最安全的方法是使用在 2025-11-27 22:00 UTC 之后的 head、stable/15 或 releng/15.0 分支中的任意一个进行源代码树检出。
...
可以通过编译 releng/15.0 分支来实现基于旧版本从源代码的更新。请参阅 `/usr/src/UPDATING` 中的说明。
