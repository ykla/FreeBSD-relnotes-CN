...
现在工具 [mdo(1)](https://man.freebsd.org/cgi/man.cgi?query=mdo&sektion=1&format=html) 能在目标凭据中完全指定所有用户和组。作为便捷功能，除了完全显式指定外，还能以基线开始，为所有属性提供默认值（可以是密码数据库中某个用户的登录凭据或当前凭据），然后选择性修改这些属性。手册页更新，描述了新选项及其相互作用。[4ffcb1a4a99c](https://cgit.freebsd.org/src/commit/?id=4ffcb1a4a99c) **【由 FreeBSD 基金会和谷歌（2025 谷歌编程之夏）赞助】**
