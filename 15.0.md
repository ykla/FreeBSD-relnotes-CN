...
对于基于软件包的基本系统安装，用户需要手动从 `15.0-RELEASE` 标签的源代码树检出中复制所需文件，或者作为替代方案，从官方发行版基本系统包中强制安装软件包 `FreeBSD-pkg-bootstrap`。
...
现在工具 [mdo(1)](https://man.freebsd.org/cgi/man.cgi?query=mdo&sektion=1&format=html) 能在目标凭据中完全指定所有用户和组。作为便捷功能，除了完全显式指定外，还能以基线开始，为所有属性提供默认值（可以是密码数据库中某个用户的登录凭据或当前凭据），然后选择性修改这些属性。手册页更新，描述了新选项及其相互作用。[4ffcb1a4a99c](https://cgit.freebsd.org/src/commit/?id=4ffcb1a4a99c)（由 FreeBSD 基金会赞助）（由谷歌（2025 谷歌编程之夏）赞助）
...
[mdo(1)](https://man.freebsd.org/cgi/man.cgi?query=mdo&sektion=1&format=html) 手册页的现有内容已增补，以记录对目标凭证中完全指定所有用户和组的新支持。现在的介绍部分更长，并新增了 SECURITY CONSIDERATIONS 部分。[20ebb6ec5ac0](https://cgit.freebsd.org/src/commit/?id=20ebb6ec5ac0)（由 FreeBSD 基金会赞助）（由谷歌（2025 谷歌编程之夏）赞助）
