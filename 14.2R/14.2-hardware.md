...
本硬件兼容性说明包含 FreeBSD 14.2 的硬件兼容性信息。其中列出了 FreeBSD 支持的硬件平台以及各类硬件设备（存储控制器、网络接口等），并给出已知可正常工作的设备实例。
...
[ahci(4)](https://man.freebsd.org/cgi/man.cgi?query=ahci&sektion=4&format=html) 驱动还支持作为 PCI 桥接 **nvme(4)** 的 AHCI 设备，使用 Intel Rapid Storage Technology（RST）。要使用 **nvme(4)** 设备，必须在 BIOS 中将 SATA 模式设置为 AHCI（从 RST 切换），或者接受因中断共享导致 RST 启用时的性能。FreeBSD 会自动检测处于 RST 模式且具有此扩展的 AHCI 设备。当检测到时，[ahci(4)](https://man.freebsd.org/cgi/man.cgi?query=ahci&sektion=4&format=html) 会附加 **nvme(4)** 子设备到 [ahci(4)](https://man.freebsd.org/cgi/man.cgi?query=ahci&sektion=4&format=html) 设备。
...
支持所有主要固件版本（2.x、3.x、4.x 和 5.x），但是始终建议升级到控制器可用的最新固件。
...
当前版本的 [atse(4)](https://man.freebsd.org/cgi/man.cgi?query=atse&sektion=4&format=html) 驱动支持 Ethernet MegaCore，如版本 11.1 的 Altera 文档中所述，当设备配置为使用内部 FIFO 时。
...
[bce(4)](https://man.freebsd.org/cgi/man.cgi?query=bce&sektion=4&format=html) 驱动为基于 QLogic NetXtreme II 千兆以太网控制器系列的各种 NIC 提供支持，包括以下：
...
[bge(4)](https://man.freebsd.org/cgi/man.cgi?query=bge&sektion=4&format=html) 驱动为基于 Broadcom BCM570x 千兆以太网控制器芯片系列的各种 NIC 提供支持，包括以下：
...
[bnxt(4)](https://man.freebsd.org/cgi/man.cgi?query=bnxt&sektion=4&format=html) 驱动为基于 Broadcom NetXtreme-C 和 NetXtreme-E 千兆以太网控制器芯片系列的各种 NIC 提供支持，包括以下：
...
[bxe(4)](https://man.freebsd.org/cgi/man.cgi?query=bxe&sektion=4&format=html) 驱动为基于 QLogic NetXtreme II 10Gb 以太网控制器芯片系列的各种 NIC 提供支持，包括以下：
...
[genet(4)](https://man.freebsd.org/cgi/man.cgi?query=genet&sektion=4&format=html) 驱动支持 Broadcom BCM2711 的以太网控制器部分，用于树莓派 4 Model B 及相关系统。它采用 BCM54213PE PHY。
...
[ice(4)](https://man.freebsd.org/cgi/man.cgi?query=ice&sektion=4&format=html) 驱动支持 Intel Ethernet 800 系列。此系列中带 SFP28/QSFP28 接口的大多数适配器，其固件要求使用 Intel 认证模块；这些认证模块如下所列。此限定检查无法由驱动禁用。
...
[ixl(4)](https://man.freebsd.org/cgi/man.cgi?query=ixl&sektion=4&format=html) 驱动支持 Intel Ethernet 700 系列。此系列中带 SFP+/SFP28/QSFP+ 接口的大多数适配器，其固件要求使用 Intel 认证模块；这些认证模块如下所列。此限定检查无法由驱动禁用。
...
注意，适配器还支持所有符合 SFF-8431 v4.1 和 SFF-8472 v10.4 规范的无源和有源限制直连电缆。

此列表并非详尽无遗；请查阅产品文档以获取受支持介质的最新列表。
...
[msk(4)](https://man.freebsd.org/cgi/man.cgi?query=msk&sektion=4&format=html) 驱动为基于 Marvell/SysKonnect Yukon II 千兆以太网控制器芯片的各种 NIC 提供支持，包括：
...
[qlnxe(4)](https://man.freebsd.org/cgi/man.cgi?query=qlnxe&sektion=4&format=html) 驱动支持基于以下芯片组的 25/40/100 千兆以太网和 CNA 适配器：
...
[re(4)](https://man.freebsd.org/cgi/man.cgi?query=re&sektion=4&format=html) 驱动支持 RealTek RTL8139C+、RTL8169、RTL816xS、RTL811xS、RTL8168、基于 RTL810xE 和 RTL8111 的快速以太网和千兆以太网适配器，包括：
...
[stge(4)](https://man.freebsd.org/cgi/man.cgi?query=stge&sektion=4&format=html) 驱动为基于 Sundance/Tamarack TC9021 千兆以太网控制器芯片的各种 NIC 提供支持，包括：
...
[ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动支持基于 Alteon Tigon I 和 II 芯片的适配器。[ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动已在以下适配器上测试：
...
该设备具有一个称为“Zero-CD”的模式，其中包含 Microsoft Windows 的驱动程序；这是设备的默认模式。[uhso(4)](https://man.freebsd.org/cgi/man.cgi?query=uhso&sektion=4&format=html) 驱动自动将设备从“Zero-CD”模式切换为调制解调器模式。此行为可通过 **sysctl(8)** 将 `hw.usb.uhso.auto_switch` 设置为 0 来禁用。

[urndis(4)](https://man.freebsd.org/cgi/man.cgi?query=urndis&sektion=4&format=html) 驱动支持许多 Android 设备的 RNDIS“tethering”功能。
...
[ath(4)](https://man.freebsd.org/cgi/man.cgi?query=ath&sektion=4&format=html) 驱动支持所有 Atheros Cardbus、ExpressCard、PCI 和 PCIe 卡，但不支持基于 AR5005VL 芯片组的卡。
...
[ipw(4)](https://man.freebsd.org/cgi/man.cgi?query=ipw&sektion=4&format=html) 驱动支持 Intel PRO/Wireless 2100a/b MiniPCI 网络适配器。
...
[iwlwifi(4)](https://man.freebsd.org/cgi/man.cgi?query=iwlwifi&sektion=4&format=html) 驱动支持来自 [mvm(4)](https://man.freebsd.org/cgi/man.cgi?query=mvm&sektion=4&format=html) 子驱动，支持以下芯片组代：
...
[rum(4)](https://man.freebsd.org/cgi/man.cgi?query=rum&sektion=4&format=html) 驱动支持基于 Ralink RT2501USB 和 RT2601USB 芯片组的 USB 2.0 无线 适配器， 包括：
...
[upgt(4)](https://man.freebsd.org/cgi/man.cgi?query=upgt&sektion=4&format=html) 驱动支持 USB 2.0 Conexant/Intersil PrismGT 系列无线适配器（基于 GW3887 芯片组），包括：
...
核心组件和内核接口通过硬件接口与硬件通信。 此接口作为硬件的抽象，允许使用不同的 UART 进行串行通信。
...
*	QUICC：Freescale/NXP 四合一集成通信控制器。
...
[ucycom(4)](https://man.freebsd.org/cgi/man.cgi?query=ucycom&sektion=4&format=html) 驱动目前支持以下包含 Cypress USB 转 RS232 桥接芯片的设备：
...
部分板载 CS4610 芯片附带 CS423x ISA 编解码器，而非 CS4297 AC97 编解码器。 此类配置尚未被 [snd_csa(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_csa&sektion=4&format=html) 驱动支持。
...
默认情况下，每个 pcm(4) 设备对应声卡上的一个物理端口。 ADAT 端口支持 8 声道、4 声道和 2 声道格式。 根据所选的采样率和声道格式，并非所有 pcm 声道都能 映射到 ADAT 声道，反之亦然。
...
> **注意**：
>
> USB 网络接口可在 [以太网接口](https://www.freebsd.org/releases/14.2R/hardware.html#ethernet) 或 [无线网络接口](https://www.freebsd.org/releases/14.2R/hardware.html#wlan) 中查阅。

> **注意**：
>
> USB 蓝牙适配器可在 [蓝牙](https://www.freebsd.org/releases/14.2R/hardware.html#bluetooth) 中查阅。

> **注意**：
>
> USB 人机接口设备可在 [其他设备](https://www.freebsd.org/releases/14.2R/hardware.html#misc) 中查阅。
...
> 不再生产。 有 128KB SRAM 型号或 2MB DRAM 型号。
...
> 唯一基于 7811 的板（比 7751 更快，并带有 随机数生成器）。
...
> 参见 *<http://www.soekris.com/>*。 包含 7951，支持对称和随机数运算。

Soekris Engineering vpn1401 and vpn1411

> 参见 *<http://www.soekris.com/>*。 包含 7955，支持对称和随机数运算。
...
* PS/2 鼠标及兼容设备， 包括许多笔记本电脑指点设备（[psm(4)](https://man.freebsd.org/cgi/man.cgi?query=psm&sektion=4&format=html) 驱动）
* ADB 鼠标和触摸设备（[ams(4)](https://man.freebsd.org/cgi/man.cgi?query=ams&sektion=4&format=html) 驱动）
* 串口鼠标及兼容设备

> **注意**：
>
> **moused(8)** 包含关于在 FreeBSD 控制台中使用传统指点设备的更多信息。
...
[ads111x(4)](https://man.freebsd.org/cgi/man.cgi?query=ads111x&sektion=4&format=html) 驱动支持以下设备：
...
[at45d(4)](https://man.freebsd.org/cgi/man.cgi?query=at45d&sektion=4&format=html) 驱动支持以下设备：
...
[gpioths(4)](https://man.freebsd.org/cgi/man.cgi?query=gpioths&sektion=4&format=html) 驱动支持以下设备：

	DHT11       DHT12
	DHT21       DHT22
	AM3201      AM3202

所支持的设备彼此相似，差异主要在于精度和分辨率。 这些设备需要单线进行数据通信，使用 自定义协议，与 Maxim 的 1-wire(tm) 不兼容。 AM320x 设备还支持连接到 i2c 总线， 但该驱动仅支持单线连接选项。
...
[mx25l(4)](https://man.freebsd.org/cgi/man.cgi?query=mx25l&sektion=4&format=html) 驱动支持以下设备：
