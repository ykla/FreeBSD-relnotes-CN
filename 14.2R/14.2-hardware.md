...
该设备具有“Zero-CD”模式，其中包含 Microsoft Windows 的驱动程序；这是设备的默认模式。[uhso(4)](https://man.freebsd.org/cgi/man.cgi?query=uhso&sektion=4&format=html) 驱动自动将设备从“Zero-CD”模式切换为调制解调器模式。此行为可通过 **sysctl(8)** 将 `hw.usb.uhso.auto_switch` 设置为 0 来禁用。
