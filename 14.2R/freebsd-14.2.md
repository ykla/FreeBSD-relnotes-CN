...
> **重要**：在更新 FreeBSD 之前，应先备份 **所有** 数据和配置文件。
...
- `unbound` 更新至 1.22.0。[0a096a7b3ae8](https://cgit.freebsd.org/src/commit/?id=0a096a7b3ae8)

### 运行时库和 API

[fma(3)](https://man.freebsd.org/cgi/man.cgi?query=fma&sektion=3&format=html) 现在在处理某些较小输入时（如在 Python 测试套件中观察到的情况），能够正确返回带正确符号的零。[dc39004bc670](https://cgit.freebsd.org/src/commit/?id=dc39004bc670)（由 FreeBSD 基金会赞助）

新增了函数 `cap_rights_is_empty`，用于判断 `cap_rights_t` 是否未设置任何权限。[e77813f7e4a3](https://cgit.freebsd.org/src/commit/?id=e77813f7e4a3)（由 FreeBSD 基金会赞助）

### 已弃用应用程序

[gpart(8)](https://man.freebsd.org/cgi/man.cgi?query=gpart&sektion=8&format=html) 已经取代 [fdisk(8)](https://man.freebsd.org/cgi/man.cgi?query=fdisk&sektion=8&format=html) 很久了，但后者未被删除。运行此应用程序时将显示警告，提示用户迁移到 [gpart(8)](https://man.freebsd.org/cgi/man.cgi?query=gpart&sektion=8&format=html)。[3958be5c29da](https://cgit.freebsd.org/src/commit/?id=3958be5c29da)（由 FreeBSD 基金会赞助）
...
- FreeBSD 项目现已发布符合 OCI 标准的容器镜像。[8a688fcc242e](https://cgit.freebsd.org/src/commit/?id=8a688fcc242e)
...
- Amazon EC2 云环境中的 `shutdown` 和 `reboot` API 现已支持 arm64 实例。升级至 FreeBSD 14.2-RELEASE 的旧实例需要在 **/boot/loader.conf** 中设置 `debug.acpi.quirks="8"`。[28b881840df7](https://cgit.freebsd.org/src/commit/?id=28b881840df7)（由亚马逊赞助）
- FreeBSD 项目现已发布“精简版（small）”的 EC2 镜像。精简版镜像是“基础版（base）”镜像移除调试符号、测试、32 位库、LLDB 调试器、Amazon SSM Agent 和 AWS CLI 后的版本。[953142d6baf3](https://cgit.freebsd.org/src/commit/?id=953142d6baf3)（由亚马逊赞助）
...
- `ena` 驱动更新至 2.8.0。[6bf02434bd9a](https://cgit.freebsd.org/src/commit/?id=6bf02434bd9a)（由亚马逊赞助）
- `ice` 驱动更新至 1.43.2-k。[38a1655adcb3](https://cgit.freebsd.org/src/commit/?id=38a1655adcb3)（由英特尔赞助）
- `ice_ddp` 更新至 1.3.41.0。[a9d78bb714e3](https://cgit.freebsd.org/src/commit/?id=a9d78bb714e3)（由英特尔赞助）
...
- 驱动程序 [ig4(4)](https://man.freebsd.org/cgi/man.cgi?query=ig4&sektion=4&format=html) 新增了对 Meteor Lake 的支持。[56f0fc0011c2](https://cgit.freebsd.org/src/commit/?id=56f0fc0011c2)
- 一款支持部分 Realtek 芯片组的新无线驱动现已可用：[rtw89(4)](https://man.freebsd.org/cgi/man.cgi?query=rtw89&sektion=4&format=html)。[a2d1e07f6451](https://cgit.freebsd.org/src/commit/?id=a2d1e07f6451)（由 FreeBSD 基金会赞助）
...
- 新增对 ACPI GPIO _AEI 对象的支持。[1db6ffb2a482](https://cgit.freebsd.org/src/commit/?id=1db6ffb2a482)（由亚马逊赞助）
- 在所有架构上启用 [nvme(4)](https://man.freebsd.org/cgi/man.cgi?query=nvme&sektion=4&format=html) 和 [nvmecontrol(8)](https://man.freebsd.org/cgi/man.cgi?query=nvmecontrol&sektion=8&format=html)。[24687a65dd7f](https://cgit.freebsd.org/src/commit/?id=24687a65dd7f)，[aba2d7f89dcf](https://cgit.freebsd.org/src/commit/?id=aba2d7f89dcf)（由 Chelsio Communications 和 Netflix 赞助）

### 已弃用和已移除的驱动程序

- [agp(4)](https://man.freebsd.org/cgi/man.cgi?query=agp&sektion=4&format=html) 计划在 FreeBSD 15.0 中删除，手册页标注为弃用。[92af7c97e197](https://cgit.freebsd.org/src/commit/?id=92af7c97e197)
- [syscons(4)](https://man.freebsd.org/cgi/man.cgi?query=syscons&sektion=4&format=html) 计划在后续版本中删除，手册页标注为弃用，建议用户迁移至 [vt(4)](https://man.freebsd.org/cgi/man.cgi?query=vt&sektion=4&format=html)。[2bc5b1d60512](https://cgit.freebsd.org/src/commit/?id=2bc5b1d60512)（由 FreeBSD 基金会赞助）
...
OpenZFS 更新至版本 2.2.6。[755e773877e9](https://cgit.freebsd.org/src/commit/?id=755e773877e9)

## 引导加载程序变更

本节介绍引导加载程序、启动菜单及其他与启动相关的变更。

### 引导加载程序变更

BIOS 引导加载程序重新添加了对 gzip 和 bzip2 的支持，但默认情况下移除了图形模式的支持，以解决大小问题。（EFI 引导加载程序未受影响，仍支持上述所有功能。）[4d3b05a8530e](https://cgit.freebsd.org/src/commit/?id=4d3b05a8530e)（由 Netflix 赞助）
...
网络栈获得了大量改进，涉及性能提升以及对 [sctp(4)](https://man.freebsd.org/cgi/man.cgi?query=sctp&sektion=4&format=html) 栈的错误修复。

通过 [sctp_peeloff(2)](https://man.freebsd.org/cgi/man.cgi?query=sctp_peeloff&sektion=2&format=html) 返回的描述符现在会继承父套接字的能力。[ae3d7e27abc9](https://cgit.freebsd.org/src/commit/?id=ae3d7e27abc9)（由 FreeBSD 基金会赞助）

### 通用网络

- 为驱动 [igc(4)](https://man.freebsd.org/cgi/man.cgi?query=igc&sektion=4&format=html) 新增了 AIM（Adaptive Interrupt Moderation，自适应中断调节）支持。[472a0ccf847a](https://cgit.freebsd.org/src/commit/?id=472a0ccf847a)（由 Rubicon Communications, LLC (“Netgate”) 和 BBOX.io 赞助）
- 此功能也添加到了驱动程序 [lem(4)](https://man.freebsd.org/cgi/man.cgi?query=lem&sektion=4&format=html)、[em(4)](https://man.freebsd.org/cgi/man.cgi?query=em&sektion=4&format=html) 和 [igb(4)](https://man.freebsd.org/cgi/man.cgi?query=igb&sektion=4&format=html) 中。FreeBSD 12.0 中引入的 UDP 性能严重下降问题（包括基于 UDP 的 NFS）预计通过此变更得到修复。[49f12d5b38f6](https://cgit.freebsd.org/src/commit/?id=49f12d5b38f6)（由 Rubicon Communications, LLC (“Netgate”) 和 BBOX.io 赞助）

### 无线网络

LinuxKPI（尤其是针对 802.11）的功能得到增强，提高了无线驱动程序（如 [iwlwifi(4)](https://man.freebsd.org/cgi/man.cgi?query=iwlwifi&sektion=4&format=html)）的稳定性。（由 FreeBSD 基金会赞助）

## 硬件支持

本节介绍了对物理机、虚拟机管理程序、虚拟化环境和硬件的一般支持，以及不属于其他章节的硬件更新。
...
IOAPIC ID 最大值增加至 255，修复了某些高核心数 AMD CPU 启动失败的问题。[18119e711f1c](https://cgit.freebsd.org/src/commit/?id=18119e711f1c)（由 FreeBSD 基金会赞助）

新增对 POWER10 和 POWER11 的初步支持。[f9f006df1535](https://cgit.freebsd.org/src/commit/?id=f9f006df1535)

### 虚拟化支持

- 现已可识别 NVMM 虚拟机管理程序。[34f40baca641](https://cgit.freebsd.org/src/commit/?id=34f40baca641)
- 在 [bhyve(8)](https://man.freebsd.org/cgi/man.cgi?query=bhyve&sektion=8&format=html) 的 VNC 服务器中使用 [www/novnc](https://cgit.freebsd.org/ports/tree/www/novnc/) 客户端时，现在可以正确显示色彩。[f9e09dc5b1d5](https://cgit.freebsd.org/src/commit/?id=f9e09dc5b1d5)
- 在 Hyper-V 环境下，TLB 刷新现在通过超调用（hypercalls）（而非 IPI）执行，这使 TLB 性能有了最高 40% 的提升。[7ece5993b787](https://cgit.freebsd.org/src/commit/?id=7ece5993b787)（由微软赞助）

### Linux 二进制兼容层

现在，Linux 二进制兼容层所有的 `stat()` 及其变体都将忽略标志 `AT_NO_AUTOMOUNT`——因为该标志指定的行为已经与 FreeBSD 的行为一致，这提高了 Linux 应用程序的兼容性。[99d3ce80ba07](https://cgit.freebsd.org/src/commit/?id=99d3ce80ba07)（由 FreeBSD 基金会赞助）

## 多媒体

对音频堆栈进行了许多改进，包括在 [mixer(8)](https://man.freebsd.org/cgi/man.cgi?query=mixer&sektion=8&format=html) 中支持热插拔功能，以及新增 [mididump(1)](https://man.freebsd.org/cgi/man.cgi?query=mididump&sektion=1&format=html) 工具。[cf9d2fb18433](https://cgit.freebsd.org/src/commit/?id=cf9d2fb18433)（由 FreeBSD 基金会赞助） [7224e9f2d4af](https://cgit.freebsd.org/src/commit/?id=7224e9f2d4af)（由 FreeBSD 基金会赞助）

## Ports 与软件包基础设施

本节介绍 FreeBSD Ports、软件包基础设施以及软件包维护和安装工具的变更。

### 安装程序

FreeBSD 安装程序 [bsdinstall(8)](https://man.freebsd.org/cgi/man.cgi?query=bsdinstall&sektion=8&format=html) 现在能在 FreeBSD 基本系统安装完成后下载和安装固件软件包。[03c07bdc8b31](https://cgit.freebsd.org/src/commit/?id=03c07bdc8b31)（由 FreeBSD 基金会赞助）

### 软件包变更

软件包 [net/wifi-firmware-kmod@release](https://cgit.freebsd.org/ports/tree/net/wifi-firmware-kmod/) 添加到 DVD 软件包中，以提供 WiFi 驱动所需固件。[8c6df7ead19c](https://cgit.freebsd.org/src/commit/?id=8c6df7ead19c)（由 FreeBSD 基金会赞助）

## 关于后续 FreeBSD 版本的一般说明

预计 FreeBSD 15.0 将不再支持除 armv7 以外的 32 位平台。armv6、i386 和 powerpc 平台弃用并将被删除。但 64 位系统仍可运行旧的 32 位二进制文件。
...
FreeBSD **stable/14** 及更早分支将保留现有的 32 位内核和系统支持。只要这些分支仍受 Ports 系统支持，Ports 就将继续支持为 32 位系统构建 Ports 和软件包。然而，所有 32 位平台均为二级、三级（Tier-2、Tier-3），因此随着上游停止对 32 位平台的支持，个别 Port 的支持可能会逐步下降。

按照当前的支持计划，**stable/14** 的生命周期将在 FreeBSD 14.0-RELEASE 发布后约 5 年结束。**stable/14** 的生命周期结束（EOL）将标志着被弃用的 32 位平台支持的终结，包括源代码发布、预构建软件包和通过 Ports 构建应用程序的支持。鉴于 14.0-RELEASE 已于 2023 年 11 月发布，对被弃用的 32 位平台的支持将于 2028 年 11 月结束。

在 FreeBSD 15.0 发布时，项目方可能选择调整此方案，为 15.0 或后续版本中的一个或多个被弃用平台提供一定程度的支持。任何调整都将基于社区反馈以及对支持这些平台所投入的努力。
