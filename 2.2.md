...
- **硬件支持**：
  ...
  - `ed` 和 `lnc` 驱动程序现在支持相应 PCI 以太网卡的自动配置，包括许多 NE2000 兼容卡和 AMD PCnet 芯片。Tekram 为其 DC390 和 DC390T 控制器（基于 AMD 53c974）提供了驱动程序，也能驱动其他基于该芯片的 SCSI 控制器。
- **性能优化**：
...
- **兼容性改进**：
  ...
  - SysV COFF 模拟（即 SCO 模拟）据报告现在也能正常工作。
...
请注意，`send-pr` 本身是个脚本，应该可以很容易地移植到完全不同的系统上。我们更希望你能使用这个接口，因为它能更方便地追踪问题报告。然而，在提交之前，请尽量确认该问题是否已修复。
