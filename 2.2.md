...
您可以从以下渠道订购 FreeBSD 2.1.7-RELEASE 和 2.2-RELEASE 的光盘：  
...
您可以通过 FTP 获取他们的最新目录：[ftp://ftp.cdrom.com/cdrom/catalog](ftp://ftp.cdrom.com/cdrom/catalog)  

**价格：**  
- 每张 RELEASE 光盘的价格为 $39.95，或者 $24.95（如果您订阅了 FreeBSD）。  
- FreeBSD 3.0-SNAP CD 的价格为 $29.95，或者 $14.95（如果您订阅了 FreeBSD-SNAP）。  
（RELEASE 和 SNAP 订阅是完全独立的。）  

订阅后，您将在新版本发布时自动收到更新。每次发货时，您的信用卡将被扣款，您可以随时取消订阅，无需承担额外费用。  
...
如果由于某种原因，您无法使用命令 `send-pr` 提交错误报告，可以尝试将报告发送至：`freebsd-bugs@FreeBSD.org` 
...
另外，作为一个志愿者项目，我们总是很高兴有更多愿意提供帮助的伙伴——目前希望实施的增强功能远远超过我们自己能完成的数量！如果您有技术问题或希望提供帮助，请发送邮件至：`freebsd-hackers@FreeBSD.org` 

请注意，这些邮件列表可能会产生 *大量* 的邮件流量，如果您的邮件访问速度较慢或费用较高，并且您只对了解 FreeBSD 的重大事件感兴趣，您可能会更倾向于订阅：`freebsd-announce@FreeBSD.org` 
...
如果由于某种原因，您无法使用 `send-pr` 命令提交错误报告，可以尝试将报告发送至：`freebsd-bugs@FreeBSD.org`  
...
或者，如果您已经安装了文档分发包：`file:/usr/share/doc/handbook/staff.html`  
