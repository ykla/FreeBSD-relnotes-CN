...
有关在发布后发现的重要信息或发布周期中的最后一刻变更，请参阅 [Errata](http://www.freebsd.org/releases/4.10R/errata.html) 文档。



## 2 新特性

本节介绍了自 4.9-RELEASE 以来 FreeBSD 中最显著的用户可见新特性或变更。典型的发布说明项目记录了新驱动程序或硬件支持、新命令或选项、主要的 bug 修复，或贡献的软件更新。还列出了自 4.9-RELEASE 之后发布的与基本系统相关的安全公告。
...
在 ports 树中新增了两个文件以追踪值得注意的变化：`ports/CHANGES` 列出了 Ports 及其基础设施的重大变化，`ports/UPDATING` 介绍了更新某些端口时可能遇到的潜在问题，类似于基本系统中的 `src/UPDATING`。
