...
本文档包含了 FreeBSD 4.10-RELEASE 在 i386 硬件平台上的发布说明。它描述了自 4.9-RELEASE 以来新增（或更改）的 FreeBSD 新特性。它还提供了一些关于从先前版本的 FreeBSD 升级的注意事项。
...
在 ports 树中新增了两个文件以追踪值得注意的变化：`ports/CHANGES` 列出了 Ports 及其基础设施的重大变化，`ports/UPDATING` 描述了更新某些端口时可能遇到的潜在问题，类似于基本系统中的 `src/UPDATING`。
