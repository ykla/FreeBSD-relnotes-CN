...
FreeBSD 6.2-RELEASE 的发行说明包含了在 6.2-STABLE 开发线中对 FreeBSD 基本系统所做的变更的摘要。本文档列出了自上次发布以来发布的相关安全公告，以及对 FreeBSD 内核和用户空间的重大变更。还简要介绍了升级的一些注意事项。

## 1 引言

本文件包含 FreeBSD 6.2-RELEASE 在 AMD64 硬件平台上的发行说明。它介绍了 FreeBSD 中最近新增、修改或删除的功能，并提供了一些关于从以前版本的 FreeBSD 升级的说明。

此版本的 FreeBSD 6.2-RELEASE 是一个发布版本，可以在 [http://www.FreeBSD.org/snapshots/](http://www.freebsd.org/snapshots/) 或其任何镜像站点上找到。更多信息请参见 [《获取 FreeBSD》附录](http://www.freebsd.org/doc/en_US.ISO8859-1/books/handbook/mirrors.html) 中的内容，该附录位于 [FreeBSD 手册](http://www.freebsd.org/doc/en_US.ISO8859-1/books/handbook/) 中。

建议所有用户在安装 FreeBSD 之前查阅发布勘误。勘误文档会更新发布周期结束时或发布后发现的“最新”信息。通常，它包含已知错误、安全公告和文档更正的内容。可以在 FreeBSD 网站上找到 FreeBSD 6.2-RELEASE 的最新勘误副本。

## 2 新特性

本节介绍了自 6.1-RELEASE 以来 FreeBSD 中最为用户可见的新特性或变化。

典型的发行说明条目包括发布后发布的最新安全公告、新驱动程序或硬件支持、新命令或选项、主要的 bug 修复或第三方软件升级。它们还可能列出主要 Port/包或发布工程实践的变更。显然，发行说明无法列出每个版本之间对 FreeBSD 所做的所有变更；本文档主要关注安全公告、用户可见的变化和主要的架构改进。
...
FreeBSD 内核现在支持安全事件审计，并通过内核配置选项 **AUDIT** 启用。更多信息可以在 [audit(4)](http://www.freebsd.org/cgi/man.cgi?query=audit&sektion=4&manpath=FreeBSD+6.2-RELEASE) 手册页中找到。
...
FreeBSD 现在可以在 Xbox 上运行，Xbox 的架构与 i386 几乎相同。最新开发的详细信息，请参见 [http://www.FreeBSD.org/platforms/xbox.html](http://www.freebsd.org/platforms/xbox.html)。
...
已移除内核选项 **IPFIREWALL_FORWARD_EXTENDED**。该选项用于允许 [ipfw(4)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=4&manpath=FreeBSD+6.2-RELEASE) 重定向具有本地目标的包。现在，只要启用内核选项，**IPFIREWALL_FORWARD** 此行为总是启用。
 ...
 数据包过滤器 [ipfw(4)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=4&manpath=FreeBSD+6.2-RELEASE) 现在支持规则关键字 `tag` 和 `untag`。当数据包匹配带有 tag 关键字的规则时，给定范围内的数字标签（从 `0` 到 `65535`）将附加到数据包上。标签作为内部标记（不会通过网络传输），可以用于稍后识别这些数据包，例如，通过使用带标签规则选项。更多信息请参阅 [ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+6.2-RELEASE)。
...
新增了 TrustedBSD **OpenBSM**（版本 1.0 alpha 12），它是已记录的 Sun Basic Security Module (BSM) 审计 API 和文件格式的实现，以及支持 Mac OS X 和 FreeBSD 操作系统的本地扩展。此版本还包括用于审计轨迹缩减和转换为文本的命令行工具，以及命令、文件格式和 API 的文档。为了实现这些功能，已新增内核选项 **AUDIT**、**/var/audit** 目录和审计组。
