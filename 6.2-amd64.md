...
```
$FreeBSD: src/release/doc/en\_US.ISO8859-1/relnotes/common/new.sgml,v 1.883.2.52.2.7 2007/01/11 19:53:37 bmah Exp \$
```
...
FreeBSD 6.2-RELEASE 的发布说明包含了在 6.2-STABLE 开发线中对 FreeBSD 基本系统所做的更改的摘要。本文档列出了自上次发布以来发布的相关安全公告，以及对 FreeBSD 内核和用户空间的重大更改。还简要介绍了升级的一些注意事项。

## 1 介绍

本文件包含 FreeBSD 6.2-RELEASE 在 AMD64 硬件平台上的发布说明。它描述了 FreeBSD 中最近添加、修改或删除的功能，并提供了一些关于从以前版本的 FreeBSD 升级的说明。
...
建议所有用户在安装 FreeBSD 之前查阅发布 errata。errata 文档会更新发布周期结束时或发布后发现的“最后时刻”信息。通常，它包含已知错误、安全公告和文档更正的内容。FreeBSD 6.2-RELEASE 的最新 errata 副本可以在 FreeBSD 网站上找到。



## 2 新特性

本节描述了自 6.1-RELEASE 以来 FreeBSD 中最为用户可见的新特性或变化。

典型的发布说明项包括发布后发布的最新安全公告、新驱动程序或硬件支持、新命令或选项、主要的 bug 修复或贡献的软件升级。它们还可能列出主要 Port/包或发布工程实践的更改。显然，发布说明无法列出每个版本之间对 FreeBSD 所做的所有更改；本文档主要关注安全公告、用户可见的变化和主要的架构改进。
...
已为 [tty(4)](http://www.freebsd.org/cgi/man.cgi?query=tty&sektion=4&manpath=FreeBSD+6.2-RELEASE) 层的几个 bug 添加了工作绕过；这些更改防止了观察到的多种内核 panic。
...
已添加驱动程序 [amdsmb(4)](http://www.freebsd.org/cgi/man.cgi?query=amdsmb&sektion=4&manpath=FreeBSD+6.2-RELEASE) ，提供对 AMD-8111 SMBus 2.0 控制器的支持。

已添加驱动程序 [ipmi(4)](http://www.freebsd.org/cgi/man.cgi?query=ipmi&sektion=4&manpath=FreeBSD+6.2-RELEASE) ，它与 OpenIPMI 兼容。OpenIPMI（智能平台管理接口）是一个开放标准，旨在启用对服务器、网络和电信平台的远程监控和控制。

已向子系统 [crypto(9)](http://www.freebsd.org/cgi/man.cgi?query=crypto&sektion=9&manpath=FreeBSD+6.2-RELEASE) 添加对 VIA C3、Eden 和 C7 处理器中的 PadLock 安全协处理器的支持。更多信息请参见 [padlock(4)](http://www.freebsd.org/cgi/man.cgi?query=padlock&sektion=4&manpath=FreeBSD+6.2-RELEASE) 手册页。

已添加支持 NVIDIA nForce 2/3/4 SMBus 2.0 控制器的驱动程序 [nfsmb(4)](http://www.freebsd.org/cgi/man.cgi?query=nfsmb&sektion=4&manpath=FreeBSD+6.2-RELEASE) 。
...
已对驱动程序 [bce(4)](http://www.freebsd.org/cgi/man.cgi?query=bce&sektion=4&manpath=FreeBSD+6.2-RELEASE) 进行了若干改进，以提高其稳定性。更改的一个显著方面是，涉及 IP 分片的传输端校验和卸载已被禁用，因为硬件未能正确处理此功能。

已将驱动程序 [em(4)](http://www.freebsd.org/cgi/man.cgi?query=em&sektion=4&manpath=FreeBSD+6.2-RELEASE) 更新为来自 Intel 的版本 6.2.9。除其他更改外，它还支持多种新适配器。通过定义 EM_FAST_INTR 预处理符号编译驱动程序将启用可选的“快速中断”处理。“快速中断”处理在此版本的驱动程序中已被证明在某些情况下能显著提高性能，但在其他情况下与稳定性问题有关。
...
已将驱动程序 [nve(4)](http://www.freebsd.org/cgi/man.cgi?query=nve&sektion=4&manpath=FreeBSD+6.2-RELEASE) 更新为版本 1.0-0310（2005年11月23日）。它现在也支持 [altq(4)](http://www.freebsd.org/cgi/man.cgi?query=altq&sektion=4&manpath=FreeBSD+6.2-RELEASE)。

驱动程序 [sk(4)](http://www.freebsd.org/cgi/man.cgi?query=sk&sektion=4&manpath=FreeBSD+6.2-RELEASE) 现在是 MPSAFE。

已添加驱动程序 [stge(4)](http://www.freebsd.org/cgi/man.cgi?query=stge&sektion=4&manpath=FreeBSD+6.2-RELEASE) 。它支持 Sundance/Tamarack TC9021 Gigabit Ethernet 控制器，并已从 NetBSD 移植过来。

#### 2.2.3 网络协议

通过不同监听器 [bpf(4)](http://www.freebsd.org/cgi/man.cgi?query=bpf&sektion=4&manpath=FreeBSD+6.2-RELEASE) 接收到的多个数据包现在都有相同的时间戳。

已添加IPsec 过滤伪设备 [enc(4)](http://www.freebsd.org/cgi/man.cgi?query=enc&sektion=4&manpath=FreeBSD+6.2-RELEASE) 。它允许使用 [pfil(9)](http://www.freebsd.org/cgi/man.cgi?query=pfil&sektion=9&manpath=FreeBSD+6.2-RELEASE) 框架的防火墙包在出站加密前和入站解密后检查（并过滤）IPsec 流量。
...
已移除内核选项 `IPFIREWALL_FORWARD_EXTENDED`。该选项用于允许 [ipfw(4)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=4&manpath=FreeBSD+6.2-RELEASE) 重定向具有本地目标的包。现在，只要启用内核选项， `IPFIREWALL_FORWARD` 此行为总是启用。
 ...
 数据包过滤器 [ipfw(4)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=4&manpath=FreeBSD+6.2-RELEASE) 现在支持 tag 和 untag 规则关键字。当数据包匹配带有 tag 关键字的规则时，给定范围内的数字标签（从 0 到 65535）将附加到数据包上。标签作为内部标记（不会通过网络传输），可以用于稍后识别这些数据包，例如，通过使用带标签规则选项。更多详情请参见 [ipfw(8)](http://www.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&manpath=FreeBSD+6.2-RELEASE)。
...
已添加 Netgraph 节点 [ng_tag(4)](http://www.freebsd.org/cgi/man.cgi?query=ng_tag&sektion=4&manpath=FreeBSD+6.2-RELEASE) ，用于支持操作内核中附加到数据的 mbuf 标签。

已向 [tun(4)](http://www.freebsd.org/cgi/man.cgi?query=tun&sektion=4&manpath=FreeBSD+6.2-RELEASE) 驱动程序添加对 [kqueue(2)](http://www.freebsd.org/cgi/man.cgi?query=kqueue&sektion=2&manpath=FreeBSD+6.2-RELEASE) 操作的支持。

FreeBSD 6.1-RELEASE 包含一个 IPv6 实现中的 bug，导致点对点接口出现多余的错误信息。此问题已被修正。
...
驱动程序 [ata(4)](http://www.freebsd.org/cgi/man.cgi?query=ata&sektion=4&manpath=FreeBSD+6.2-RELEASE) 现在支持 USB 大容量存储类设备。要启用此功能，需要在内核配置文件中添加行 `device atausb` 或加载 atausb 内核模块。请注意，此功能不能与驱动程序 [umass(4)](http://www.freebsd.org/cgi/man.cgi?query=umass&sektion=4&manpath=FreeBSD+6.2-RELEASE) 共存。
...
已添加伪文件系统驱动 [linsysfs(5)](http://www.freebsd.org/cgi/man.cgi?query=linsysfs&sektion=5&manpath=FreeBSD+6.2-RELEASE) 。它提供了 Linux sys 文件系统的一个子集，并且是某些 Linux 二进制文件（如 LSI MegaRAID SAS 工具）正确操作所必需的。
...
已添加 OpenBSM 用户空间工具，包括 [audit(8)](http://www.freebsd.org/cgi/man.cgi?query=audit&sektion=8&manpath=FreeBSD+6.2-RELEASE)、[auditd(8)](http://www.freebsd.org/cgi/man.cgi?query=auditd&sektion=8&manpath=FreeBSD+6.2-RELEASE)、[auditreduce(1)](http://www.freebsd.org/cgi/man.cgi?query=auditreduce&sektion=1&manpath=FreeBSD+6.2-RELEASE) 和 [praudit(1)](http://www.freebsd.org/cgi/man.cgi?query=praudit&sektion=1&manpath=FreeBSD+6.2-RELEASE)。
...
已导入工具 [csup(1)](http://www.freebsd.org/cgi/man.cgi?query=csup&sektion=1&manpath=FreeBSD+6.2-RELEASE) 。这是一个用 C 语言编写的 CVSup 兼容客户端的实现。请注意，它目前仅支持检出模式。
...
已添加工具 [freebsd-update(8)](http://www.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&manpath=FreeBSD+6.2-RELEASE) ，这是一个管理 FreeBSD 基本系统二进制更新的工具。

工具 [fsdb(8)](http://www.freebsd.org/cgi/man.cgi?query=fsdb&sektion=8&manpath=FreeBSD+6.2-RELEASE) 现在支持使用新的命令 `btime` 更改 UFS2 文件系统上文件的创建时间。
...
工具 [getent(1)](http://www.freebsd.org/cgi/man.cgi?query=getent&sektion=1&manpath=FreeBSD+6.2-RELEASE) 已从 NetBSD 导入。它通过 [nsswitch.conf(5)](http://www.freebsd.org/cgi/man.cgi?query=nsswitch.conf&sektion=5&manpath=FreeBSD+6.2-RELEASE) 中指定的查找顺序从管理数据库（例如 hosts）中检索并显示信息。
...
程序 [jail(8)](http://www.freebsd.org/cgi/man.cgi?query=jail&sektion=8&manpath=FreeBSD+6.2-RELEASE) 现在支持 `-s` 选项，用于指定监狱的 securelevel。
...
已将兼容 GLIBC 的 [printf(3)](http://www.freebsd.org/cgi/man.cgi?query=printf&sektion=3&manpath=FreeBSD+6.2-RELEASE) 的可扩展实现添加到 libc。只有在定义了环境变量 `USE_XPRINTF`、调用了其中扩展函数，或者全局变量 `__use_xprintf` 的值大于 0 时才会使用它。目前支持五个扩展：`%H`（十六进制转储）、`%T`（`time_t` 和与时间相关的结构）、`%M`（错误号消息）、`%Q`（双引号包围的转义字符串）、`%V`（[strvis(3)](http://www.freebsd.org/cgi/man.cgi?query=strvis&sektion=3&manpath=FreeBSD+6.2-RELEASE) 格式字符串）。
...
已添加 OpenBSM [auditd(8)](http://www.freebsd.org/cgi/man.cgi?query=auditd&sektion=8&manpath=FreeBSD+6.2-RELEASE) 的 auditd 脚本。

修复了 rc.d/jail 启动脚本中的一个错误，该错误导致用户在 FreeBSD 6.1-RELEASE 上使用 jails 时遇到一些问题。
...
TrustedBSD **OpenBSM**，版本 1.0 alpha 12，已添加。它是已记录的 Sun Basic Security Module (BSM) 审计 API 和文件格式的实现，以及支持 Mac OS X 和 FreeBSD 操作系统的本地扩展。此版本还包括用于审计轨迹缩减和转换为文本的命令行工具，以及命令、文件格式和 API 的文档。为了实现这些功能，已添加了 AUDIT 内核选项、`/var/audit` 目录和审计组。
...
添加了目录 `/media`，用于包含可移动媒体的挂载点，如 CD-ROM、软盘、USB 驱动器等。
...
通过添加以下手册页，改进了现有功能的文档：[nanobsd(8)](http://www.freebsd.org/cgi/man.cgi?query=nanobsd&sektion=8&manpath=FreeBSD+6.2-RELEASE)，[sysctl(9)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=9&manpath=FreeBSD+6.2-RELEASE)。
